...
| `profile_store.py` | 6899 | Profiles, storage (`Store` v11), Stage-5 grouping, envelopes/conformance/artifacts, pure-stats heuristics, match-ranking history, terminal-drop baselines, phase-profile cache, selective export/import (taxonomy + `_scan_data` + manifest + merge apply). | [02](reference/02-matching-and-profile-store.md) |
...
| `ws_api.py` | 5767 | 102 WebSocket command handlers (RBAC-guarded, executor/background). | [08](reference/08-ws-api-and-tasks.md) |
| `ws_schema.py` | 1028 | Typed request/response contract for the 102 commands (drives `ws-types.d.ts` + `docs/WS_API.md`). | [08](reference/08-ws-api-and-tasks.md) |
...
| `playground.py` | 1960 | Headless, executor-safe replay behind the Playground tab. Reuses the real detector + matcher + `progress.py` (byte-identical to live). | [09](reference/09-playground.md) |
...
78. `[ADDED 2026-08-11]` **Panel font-size slider (accessibility, user request)** (`store_account.py:_DEFAULT_PREFS["font_scale"]` + clamp in `async_set_prefs`, panel `_htmlPanelPrefs` slider + `_applyFontScale` + `_wire` live-preview + save-prefs + `_applyPanelConfig`): new `font_scale` user pref (default 1.0). `:host` has no explicit font-size and all panel sizes are `em`, so `_applyFontScale` sets the host `font-size` as a percentage and the whole panel scales. Slider in My Preferences (gear -> Display): live-preview on `input`, persist on `change` via `_setPref`, also captured by Save. Clamped [0.7, 2.0] both server-side (defends against a direct WS value) and client-side. Tests: `tests/test_store_account.py::test_font_scale_pref_stored_and_clamped`, E2E `playwright-tests/tests/font-size.spec.ts`. Pending non-EN translation: `lbl.font_size`, `msg.font_size_hint`.

79. `[ADDED 2026-08-11]` **Config-flow links to the panel (user request)** (`strings.json` + `translations/en.json`, config `user`/`reconfigure` + options `init` descriptions): each flow description now includes a markdown link to `/ha-washdata` so users know where to go post-setup. Strings-only; strings.json and en.json kept identical (hassfest). Pending non-EN: the three description strings now differ from the translated (link-less) versions until a subagent pass; English fallback is hassfest-safe.

77. `[ADDED 2026-08-11]` **Door-auto-open dishwasher support (#342)** (`const.py:CONF_/DEFAULT_DOOR_OPENS_AT_END`+`DOOR_END_DWELL_SECONDS`, `manager.py:_handle_door_sensor_change` + `_arm/_cancel_door_end_dwell` + `_door_end_dwell_fired`, panel Triggers/Door fields + en.json): a door-open on a running cycle set a sticky user-pause (`_is_user_paused`), stranding AirDry dishwashers in user_paused. **Change:** when `door_opens_at_end` is on and the detector is RUNNING/ENDING, a door-open arms an `async_call_later` dwell (`door_end_dwell_seconds`, default 60) INSTEAD of the sticky pause; if it fires (door still open - cancelled on door-close) it finalizes via `detector.user_stop()` (completed, same path as the External End Trigger). Door-close cancels a pending dwell; dwell cancelled in `async_shutdown`. Legacy pause-on-open kept for non-opt-in devices / other active states. Opt-in, needs a door sensor. Tests: `tests/test_issue_342_door_auto_open.py` (arms-not-pauses, fires->user_stop, close-cancels, no-flag->legacy). Pending non-EN translation: `setting.door_opens_at_end.{label,doc}`, `setting.door_end_dwell_seconds.{label,doc}`.

76. `[ADDED 2026-08-11]` **Per-setting mute for auto-tune suggestions (#343)** (`profile_store.py:get_locked_suggestions`/`set_suggestion_locked`, `learning.py:_apply_suggestions_and_notify` filter, `ws_api.py:ws_set_suggestion_lock` + `get_suggestions` locked list, `ws_schema.py`, panel mute button + muted banner + `set_suggestion_lock` wire, `translations/panel/en.json`): the tuner re-proposed dismissed values every cycle (no rejection memory), poisoning anti-crease devices with thresholds just above the pulse baseline. **Change:** a persisted `locked_suggestions` list; locking a key drops its pending suggestion and the surface path (`_apply_suggestions_and_notify`) skips+deletes locked keys forever until unlocked. WS `set_suggestion_lock` (registered in `ws_schema` WS_COMMANDS + WS_RESPONSE_TYPES + WS_OPEN_RESPONSES); `get_suggestions` now returns `locked_suggestions`. Panel: 🔕 mute button per suggestion card + a "N muted / Reset muted" banner (`sug-unmute-all`). Regenerated `ws-types.d.ts` + `docs/WS_API.md`. Tests: `tests/test_issue_343_suggestion_lock.py` (store + surface filter). **Part 1 (context-aware, now DONE):** `SuggestionEngine._strip_anti_crease_tail(ordered_powers)` drops the post-cycle tail from the min-active statistic in `run_simulation` AND `run_batch_simulation`. Tail = everything after the last index where `power >= anti_wrinkle_max_power` (a pulse above the ceiling ends anti-wrinkle, so the tail never reaches it). Gated on `device_type in {washing_machine, dryer, washer_dryer}` + `CONF_ANTI_WRINKLE_ENABLED`; guarded so no high-power sample -> no strip (byte-identical for gentle programs / non-anti-crease devices). Stop/start thresholds detect the MAIN cycle; the tail has its own `anti_wrinkle_*` settings, so excluding it is principled, not lossy. Tests: `tests/test_issue_343_anti_crease_exclusion.py` (dryer excludes -> 160 W not 2.4 W; disabled/ineligible/no-ceiling all unchanged). Pending non-EN translation: `btn.mute_suggestion`, `btn.reset_muted`, `msg.sug_muted`, `msg.sug_mute_failed`, `msg.sug_unmuted_all`, `msg.n_suggestions_muted`.
