...
- `_clear_live_progress_notification` — purges queued LIVE/start/pre-complete entries; on finish (`clear_services=False`) sends no service clear because the caller ends the activity itself *after* the finished notification is dispatched (`_end_live_activity`, #446), so the lock screen is never momentarily empty; on shutdown clears BOTH tags, since no finished notification follows. `_hand_over_lifecycle_to_live_activity` fires on the first live tick to drop the start card (the live tag no longer replaces it) and, as a side effect, ends any activity a pre-0.5.7 build left running under the old shared tag.
