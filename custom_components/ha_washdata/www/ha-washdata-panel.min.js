...
`;function ot(h){if(h==null||h<0)return"-";const t=Math.floor(h/3600),e=Math.floor(h%3600/60),s=Math.floor(h%60);return t>0?`${t}h ${e}m`:e>0?`${e}m ${s}s`:`${s}s`}function Vt(h){return h==null?"-":h>=100?`${Math.round(h)} W`:`${h.toFixed(1)} W`}function gt(h){return h==null?"-":`${h.toFixed(2)} kWh`}let Ut="relative";function ne(h){const t=Math.round((h-Date.now())/1e3);let e;try{e=new Intl.RelativeTimeFormat(void 0,{numeric:"auto"})}catch{return Gt(h)}const s=Math.abs(t),n=[["year",31536e3],["month",2592e3],["week",604800],["day",86400],["hour",3600],["minute",60]];for(const[a,r]of n)if(s>=r)return e.format(Math.round(t/r),a);return e.format(t,"second")}function Gt(h){return new Date(h).toLocaleString(void 0,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}function st(h,t){if(!h)return"-";let e;if(typeof h=="number")e=h>=1e12?h:h*1e3;else{const s=String(h),n=/^(\d{4})-(\d{2})-(\d{2})$/.exec(s);e=n?new Date(+n[1],+n[2]-1,+n[3]).getTime():new Date(s).getTime()}return isNaN(e)?"-":(t||Ut)==="relative"?ne(e):Gt(e)}function Pt(h){if(h==null)return"unknown error";if(h instanceof Error)return`${h.name}: ${h.message}`;const t=h.code!=null?String(h.code):"",e=h.message!=null?String(h.message):"";if(!t&&!e)try{return JSON.stringify(h)}catch{return String(h)}const s=t==="unknown_command"?" (the integration is probably still starting up; this should stop on its own)":"";return`${t||"error"}${e?": "+e:""}${s}`}function d(h){return String(h??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function re(h){const t=String(h??"").trim();return/^https?:\/\//i.test(t)?t:""}function St(h,t){const e=String(h??"").trim();let s=e.match(/^#([0-9a-fA-F]{3,8})$/);if(s){let n=s[1];if((n.length===3||n.length===4)&&(n=n.split("").map(a=>a+a).join("")),n.length===6||n.length===8){const a=parseInt(n.slice(0,2),16),r=parseInt(n.slice(2,4),16),o=parseInt(n.slice(4,6),16);return`rgba(${a}, ${r}, ${o}, ${t})`}}if(s=e.match(/^rgba?\(([^)]+)\)$/i),s){const n=s[1].split(",").map(a=>a.trim());if(n.length>=3)return`rgba(${n[0]}, ${n[1]}, ${n[2]}, ${t})`}return e}function Kt(h,t){const e=parseFloat(h);return isNaN(e)?t:e}function zt(h){return h?Array.from(h.querySelectorAll('a[href],button:not([disabled]),input:not([disabled]),select:not([disabled]),textarea:not([disabled]),[tabindex]:not([tabindex="-1"])')).filter(e=>e.getClientRects().length>0):[]}function Tt(h){return h==null||h===""?"(none)":h===!0?"on":h===!1?"off":Array.isArray(h)?h.join(", ")||"(none)":String(h)}function Xt(h,t,e){return h.slice().sort((s,n)=>{const a=t(s),r=t(n);return a==null&&r==null?0:a==null?1:r==null?-1:(a<r?-1:a>r?1:0)*e})}function ft(h,t,e,s,n,a,r){const o=e?s===1?" \u25B2":" \u25BC":' <span style="opacity:.35">\u2195</span>',i=a==="right"?"text-align:right;":"",l=r?` title="${d(r)}"`:"";return`<th class="wd-th-sort" data-sortcol="${t}" data-sortact="${n}" style="cursor:pointer;user-select:none;${i}"${l}>${h}${o}</th>`}function Mt(h){h=Math.max(0,Math.round(h));const t=Math.floor(h/3600),e=Math.floor(h%3600/60),s=h%60;return t>0?`${t}:${String(e).padStart(2,"0")}:${String(s).padStart(2,"0")}`:`${e}:${String(s).padStart(2,"0")}`}function Yt(h,t){const e={pause:["lbl.artifact_interruption","Interruption"],dip:["lbl.artifact_low_power","Low power"],spike:["lbl.artifact_high_power","High power"]},[s,n]=e[h]||["lbl.artifact_anomaly","Anomaly"];return t?t(s,{},n):n}function Jt(h){return h.toLowerCase().replace(/[\s&/\-]+/g,"_").replace(/^_+|_+$/g,"").replace(/_+/g,"_")}function Qt(h){const t=new Set;return String(h??"").split(",").forEach(e=>{const s=parseInt(e.trim(),10);Number.isFinite(s)&&s>0&&t.add(s)}),Array.from(t).sort((e,s)=>e-s)}function Dt(h,t){if(!h||!h.length)return null;if(t<=h[0][0])return h[0][1];if(t>=h[h.length-1][0])return h[h.length-1][1];for(let e=1;e<h.length;e++)if(h[e][0]>=t){const s=h[e-1],n=h[e],a=n[0]-s[0]||1;return s[1]+(n[1]-s[1])*((t-s[0])/a)}return h[h.length-1][1]}function oe(h,t,e){e=e||{};const s=h.key,n=h.unit?`${h.label} (${h.unit})`:h.label,a=h.unit?` ${h.unit}`:"",r=h.doc?it(h.doc,h.diagram||le[s]):"",o=e.changed?`<span class="wd-chg-dot" title="${d(e.changed)}" aria-label="${d(e.changed)}"></span>`:"";if(h.type==="checkbox")return`<div class="wd-field wd-field-switch"><div class="wd-switch-row"><label class="wd-switch-lbl"><span class="wd-switch"><input type="checkbox" data-opt="${s}" ${t?"checked":""}><span class="wd-switch-slider"></span></span><span class="wd-switch-text">${d(h.label)}</span></label>${o}${r}</div>${h.hint?`<div class="wd-field-hint">${d(h.hint)}</div>`:""}</div>`;let i="";const l=t??"";if(h.type==="select"||h.type==="devicetype"||h.type==="device"){const C=(e.opts||[]).map(([R,B])=>`<option value="${d(R)}" ${String(l)===String(R)?"selected":""}>${d(B)}</option>`).join("");i=`<select data-opt="${s}" data-ftype="${h.type}">${C}</select>`}else if(h.type==="textarea")i=`<textarea data-opt="${s}" data-ftype="textarea">${d(l)}</textarea>`;else if(h.type==="json"){const S=l===""||l==null?"":typeof l=="string"?l:JSON.stringify(l,null,2);i=`<textarea data-opt="${s}" data-ftype="json" rows="3" placeholder='${d(e.t("placeholder.json_buttons",{},'[{"action":"ID","title":"Label"}]'))}'>${d(S)}</textarea>`}else if(h.type==="checkboxlist"){const S=Array.isArray(t)&&t.length?t.map(String):Array.isArray(h.def)?h.def.map(String):[];i=`<div data-opt="${s}" data-ftype="checkboxlist" class="wd-checkboxlist">`+(h.choices||[]).map(([C,R])=>`<label class="wd-switch-lbl" style="display:flex;align-items:center;gap:8px;margin:2px 0"><span class="wd-switch"><input type="checkbox" data-choice="${d(C)}" ${S.includes(String(C))?"checked":""}><span class="wd-switch-slider"></span></span><span class="wd-switch-text">${d(e.t?e.t("lbl.evidence_"+C,{},R):R)}</span></label>`).join("")+"</div>"}else if(h.type==="entitylist"){const C=(Array.isArray(t)?t:t?[t]:[]).map(R=>`<span class="wd-pill" data-val="${d(R)}">${d(R)}<button type="button" class="wd-pill-x" aria-label="${d(e.t?e.t("btn.remove",{},"Remove"):"Remove")}">\xD7</button></span>`).join("");i=`<div class="wd-pillbox" data-opt="${s}" data-ftype="entitylist">${C}<div class="wd-combo wd-combo-pill"><input type="text" class="wd-pill-add" autocomplete="off" spellcheck="false" placeholder="${d(e.t("placeholder."+(h.domain||"add"),{},h.placeholder||"add\u2026"))}"><div class="wd-combo-drop" hidden></div></div></div>`}else if(h.type==="timerlist"){const S=Array.isArray(l)?l:[],C=e.t?e.t("lbl.timer_min",{},"min"):"min",R=e.t?e.t("lbl.timer_msg_placeholder",{},"Message (optional, {device}/{program}/{minutes})"):"Message (optional, {device}/{program}/{minutes})",B=e.t?e.t("lbl.timer_auto_pause",{},"Auto-pause"):"Auto-pause",W=e.t?e.t("btn.remove_timer",{},"Delete"):"Delete",O=e.t?e.t("btn.add_timer",{},"+ Add timer"):"+ Add timer",D=(E,M)=>{const H=E&&E.offset_minutes?String(E.offset_minutes):"",q=E&&E.message?d(E.message):"",Q=E&&E.auto_pause?" checked":"";return`<div class="wd-timer-row" data-tidx="${M}"><div class="wd-timer-top"><input type="number" min="1" placeholder="${d(C)}" data-field="offset_minutes" value="${d(H)}"><textarea placeholder="${d(R)}" data-field="message">${q}</textarea></div><div class="wd-timer-footer"><label class="wd-switch-lbl"><span class="wd-switch"><input type="checkbox" data-field="auto_pause"${Q}><span class="wd-switch-slider"></span></span><span class="wd-switch-text">${d(B)}</span></label><button type="button" class="wd-btn wd-btn-sm wd-btn-danger wd-timer-remove">${d(W)}</button></div></div>`},A=S.map((E,M)=>D(E,M)).join("");i=`<div class="wd-timerlist" data-opt="${s}" data-ftype="timerlist">${A}<button type="button" class="wd-btn wd-btn-sm wd-btn-secondary wd-timer-add">${d(O)}</button></div>`}else if(h.type==="entity"){const S=h.placeholder?` placeholder="${d(h.placeholder)}"`:"";i=`<div class="wd-combo"><input type="text" class="wd-combo-inp" data-opt="${s}" data-ftype="entity" value="${d(l)}" autocomplete="off" spellcheck="false"${S}><div class="wd-combo-drop" hidden></div></div>`}else if(h.type==="list"){const S=Array.isArray(l)?l.join(", "):d(l);i=`<input type="text" data-opt="${s}" data-ftype="list" value="${d(S)}" placeholder="${d(e.t("placeholder.notify_services_list",{},"notify.mobile_app_phone, ..."))}">`}else if(h.type==="intlist"){const S=Array.isArray(l)?l.join(", "):String(l??""),C=h.placeholder?` placeholder="${d(h.placeholder)}"`:"";i=`<input type="text" data-opt="${s}" data-ftype="intlist" value="${d(S)}"${C}>`}else{const S=h.type==="number"?"number":"text",C=e.datalistId?` list="${e.datalistId}"`:"",R=h.step!=null?` step="${h.step}"`:"",B=h.min!=null?` min="${h.min}"`:"",W=h.max!=null?` max="${h.max}"`:"",O=h.placeholder?` placeholder="${d(h.placeholder)}"`:"";i=`<input type="${S}" data-opt="${s}" data-ftype="${h.type}" value="${d(l)}"${R}${B}${W}${C}${O}>${e.datalist||""}`}const c=e.suggestion,_=e.mlSuggestion,p=c&&c.suggested!=null&&!Ft(c.suggested,t)?c.suggested:null,g=_&&_.value!=null&&!Ft(_.value,t)?_.value:null,u=e.t,v=c?c.reason_key?u(c.reason_key,c.reason_params||{},c.reason||""):c.reason||"":"",b=_?_.reason_key?u(_.reason_key,_.reason_params||{},_.reason||""):_.reason||"":"",$=S=>`<button type="button" class="wd-sug-use" data-sugkey="${s}" data-sugval="${d(S)}">${e.useBtnLabel||"Use"}</button>`;let k="";if(p!=null&&g!=null){const S=parseFloat(p),C=parseFloat(g);if((!isNaN(S)&&!isNaN(C)?Math.abs(S-C)/Math.max(Math.abs(S),Math.abs(C),1e-9):1)<.05){const B=u("suggestion.calibrated_label",{},"Calibrated"),W=it([v,b?`${B}: ${b}`:""].filter(Boolean).join(`

`));k=`<div class="wd-sug"><span class="wd-sug-chip wd-sug-chip-obs">\u{1F4A1} ${d(u("suggestion.both_agree",{},"WashData recommends"))}</span><span class="wd-sug-val">${d(p)}${a}</span>${$(p)}${W}</div>`}else{const B=v?it(v):"",W=b?it(b):"",O=u("suggestion.observed_label",{},"Observed"),D=u("suggestion.calibrated_label",{},"Calibrated");let A="",E="";if(!isNaN(S)&&!isNaN(C)){const q=C>S;E=u(`suggestion.impact.${s}.${q?"higher":"lower"}`,{},""),A=u(`suggestion.impact.${s}.${q?"lower":"higher"}`,{},"")}const M=A?`<div class="wd-sug-impact-line">${d(A)}</div>`:"",H=E?`<div class="wd-sug-impact-line">${d(E)}</div>`:"";k=`<div class="wd-sug wd-sug-split"><div class="wd-sug-opt"><span class="wd-sug-chip wd-sug-chip-obs">\u{1F4A1} ${d(O)}</span><span class="wd-sug-val">${d(p)}${a}</span>${$(p)}${B}${M}</div><div class="wd-sug-opt"><span class="wd-sug-chip wd-sug-chip-cal">\u{1F916} ${d(D)}</span><span class="wd-sug-val">${d(g)}${a}</span>${$(g)}${W}${H}</div></div>`}}else if(p!=null){const S=v?it(v):"",C=t!=null&&t!==""?` <span style="opacity:.6;font-size:.9em">(now ${d(t)}${a})</span>`:"";k=`<div class="wd-sug"><span class="wd-sug-chip wd-sug-chip-obs">\u{1F4A1} ${d(u("suggestion.observed_label",{},"Observed"))}</span><span class="wd-sug-val">${d(p)}${a}</span>${C}${$(p)}${S}</div>`}else if(g!=null){const S=b?it(b):"";k=`<div class="wd-sug"><span class="wd-sug-chip wd-sug-chip-cal">\u{1F916} ${d(u("suggestion.calibrated_label",{},"Calibrated"))}</span><span class="wd-sug-val">${d(g)}${a}</span>${$(g)}${S}</div>`}if(k&&s){const S=d(u("btn.mute_suggestion",{},"Stop suggesting this setting")),C=`<button type="button" class="wd-sug-lock" data-suglock="${s}" title="${S}" aria-label="${S}">\u{1F515}</button>`;k=k.replace(/<\/div>\s*$/,C+"</div>")}return`<div class="wd-field" data-field="${s}"><div class="wd-label-row"><label style="margin:0">${d(n)}</label>${o}${r}</div>${i}${h.hint?`<div class="wd-field-hint">${d(h.hint)}</div>`:""}<div class="wd-conflict-err" data-cerr="${s}" hidden></div>${k}</div>`}function Ft(h,t){if(h==null||t==null)return!1;const e=parseFloat(h),s=parseFloat(t);return!isNaN(e)&&!isNaN(s)?Math.abs(e-s)<1e-6:String(h)===String(t)}const le={min_power:"min_power",off_delay:"off_delay",smoothing_window:"smoothing",start_threshold_w:"hysteresis",stop_threshold_w:"hysteresis",start_energy_threshold:"start_energy",running_dead_zone:"dead_zone",profile_duration_tolerance:"duration_tolerance",profile_match_min_duration_ratio:"match_ratios",profile_match_max_duration_ratio:"match_ratios",progress_reset_delay:"progress_reset",completion_min_seconds:"min_duration",min_off_gap:"min_off_gap",start_duration_threshold:"start_duration",end_energy_threshold:"end_energy_thresh",end_repeat_count:"end_repeat",profile_match_threshold:"confidence",profile_unmatch_threshold:"confidence",auto_label_confidence:"confidence",learning_confidence:"confidence",no_update_active_timeout:"watchdog_timeout",anti_wrinkle_enabled:"anti_wrinkle",anti_wrinkle_max_power:"anti_wrinkle",anti_wrinkle_max_duration:"anti_wrinkle",anti_wrinkle_exit_power:"anti_wrinkle",anti_wrinkle_idle_timeout:"anti_wrinkle",sampling_interval:"sampling"};function de(h){let t=0,e=window.innerWidth;for(let s=h.parentElement;s;s=s.parentElement){if(getComputedStyle(s).overflowX==="visible")continue;const n=s.getBoundingClientRect();n.left>t&&(t=n.left),n.right<e&&(e=n.right)}return{left:t,right:e}}function it(h,t){return`<span class="wd-tip">i<span class="wd-tip-pop">${t?ce(t):""}<span class="wd-tip-txt">${d(h)}</span></span></span>`}function Ot(h,t,e=""){return`<label class="wd-switch-lbl"><span class="wd-switch"><input type="checkbox" ${h}><span class="wd-switch-slider"></span></span><span class="wd-switch-text wd-switch-text--plain">${d(t)}</span></label>${e||""}`}function Et(h,t,e="",s=""){return`<div class="wd-field wd-field-switch"><div class="wd-switch-row">${Ot(h,t,e)}</div>${s?`<div class="wd-field-hint">${d(s)}</div>`:""}</div>`}function ce(h){const t=s=>`<svg class="wd-dg" viewBox="0 0 200 90" preserveAspectRatio="xMidYMid meet">${s}</svg>`,e='<line class="ax" x1="8" y1="78" x2="192" y2="78"/>';switch(h){case"smoothing":return t(`${e}
    ...
    `}_htmlHeader(){const e=Array.from(this._busy).some(i=>!(i==="pg-sweep"||i==="pg-history"||i==="reprocess"||i.startsWith("ml-train-now")))?`<span class="wd-badge" style="margin:0 0 0 12px;color:var(--app-header-text-color,#fff);background:rgba(255,255,255,.15)">${this._t("status.working",{},"Working\u2026")}</span>`:"",s=this._constants.iconUrl,n=s?`<img class="wd-logo" src="${d(s)}" width="30" height="30" alt="WashData" aria-hidden="true" style="border-radius:6px;object-fit:contain">`:`<svg class="wd-logo" viewBox="0 0 24 24" width="26" height="26" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" aria-hidden="true">
    ...
    </svg>`,a=this._constants.version;return`
      <div class="wd-header">
      ...
      <svg viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" aria-hidden="true"><line x1="4" y1="7" x2="20" y2="7"/><line x1="4" y1="12" x2="20" y2="12"/><line x1="4" y1="17" x2="20" y2="17"/></svg>
        ...
        ${n}
        <div><h1>WashData</h1><div class="wd-sub">${a?`v${d(a)} &middot; `:""}${this._t("msg.appliance_monitor",{},"Appliance monitor")}</div></div>
        ${e}
    ...
    `}_htmlBody(){if(!this._devices.length)return`<div class="wd-empty"><div class="wd-icon">\u{1F9FA}</div>${this._t("msg.no_devices",{},"No WashData devices configured yet.")}</div>`;const t=this._mlSugKeys().size,e=this._suggestions.length||t?" \u{1F4A1}":"",s=this._conflictKeysFromOpts().size>0?" \u26A0":"",a=this._busy.has("pg-sim")||this._busy.has("pg-sweep")?'<span class="wd-spin" style="margin-left:4px;vertical-align:middle"></span>':"",r={status:this._t("tab.status",{},"Overview"),history:this._t("tab.history",{},"Cycles"),profiles:this._t("tab.profiles",{},"Profiles"),settings:this._t("tab.settings",{},"Settings")+s+e,playground:this._t("tab.playground",{},"Playground")+a,store:this._t("tab.store",{},"Store"),advanced:this._t("tab.advanced",{},"Advanced")},o=this._visibleTabIds();o.includes(this._tab)||(this._tab="status");const i=o.map(c=>`<button class="wd-tab ${this._tab===c?"active":""}" role="tab" id="wd-tab-${c}" aria-selected="${this._tab===c}" tabindex="${this._tab===c?"0":"-1"}" data-tab="${c}">${r[c]}</button>`).join(""),l=(c,_)=>o.includes(c)?`<div class="wd-pane ${this._tab===c&&!this._tabLoading?"active":""}" role="tabpanel" aria-labelledby="wd-tab-${c}">${_}</div>`:"";return`
      ${this._htmlDeviceBar()}
    ...
    `:"",O=((this._powerData||{}).live||[]).length>1,D=this._pref("show_expected",!0),A=this._pref("show_raw_active",!1),E=`<div class="wd-leg">
      ...
      <div class="wd-card">
        ...
        <div class="wd-badge ${o?"wd-running":""}" style="color:${a};background:color-mix(in srgb, ${a} 13%, transparent);">
          <span class="wd-dot"></span>${d(r)}
      ...
      ${U}
      ${f}
    `}_htmlSetupCard(t,e=!1){if(!t)return"";const{phase:s,message_key:n,message_params:a,cta_label_key:r,cta_action:o,secondary_label_key:i,secondary_action:l,skippable:c,dismissible:_,step_key:p}=t;if(s==="phase4")return`
        ...
        </div>`;const g=this._t(n,a||{},""),u=this._t(r,{},"Continue"),v=i?this._t(i,{},""):null,b=c?`
      ...
      <div class="wd-setup-card" data-phase="${d(s)}">
        ...
        <div style="display:flex;flex-wrap:wrap;gap:8px;align-items:center">
          <button class="wd-btn wd-btn-primary" data-action="setup-cta"
                  data-cta-action="${d(o||"")}"
                  data-cta-params="${d(JSON.stringify(a||{}))}">
            ${d(u)}
          </button>
    ...
    </div>`,G=k.length!==s.length?this._t("lbl.n_shown",{n:k.length},`, ${k.length} shown`):"",X=e.length?this._t("lbl.n_imported_note",{n:e.length},`, ${e.length} imported`):"",ct=this._t("lbl.cycles_title",{n:`${t.length}${X}${G}`},`Cycles (${t.length}${X}${G})`),U=n?`<div class="wd-card-actions" style="margin:0 0 4px;justify-content:flex-end">
      ...
      <button class="wd-btn ${a?"wd-btn-primary":"wd-btn-secondary"} wd-btn-sm" data-action="cyc-select-toggle">${a?this._t("btn.done",{},"Done"):this._t("btn.select",{},"Select")}</button>
    </div>`:"",at=new Set(e.map(w=>w.id)),f=[...r].some(w=>at.has(w)),m=a?`<div class="wd-card-actions" style="margin:0 0 10px">
      <span class="wd-info" style="margin:0">${this._t("lbl.n_selected",{n:r.size},`${r.size} selected`)}</span>
      <button class="wd-btn wd-btn-secondary wd-btn-sm" data-action="cyc-compare" ${r.size<2?"disabled":""}>${this._t("btn.compare",{},"Compare")}${r.size>=2?` (${r.size})`:""}</button>
      <button class="wd-btn wd-btn-secondary wd-btn-sm" data-action="cyc-merge" ${r.size<2||f?"disabled":""}${f?` title="${d(this._t("msg.merge_no_imports",{},"Imported recordings cannot be merged into a real cycle. Deselect them to merge."))}"`:""}>${this._t("btn.merge",{},"Merge")}${r.size>=2?` (${r.size})`:""}</button>
      <button class="wd-btn wd-btn-secondary wd-btn-sm" data-action="cyc-relabel" ${r.size<1?"disabled":""}>${this._t("btn.relabel",{count:r.size},`Relabel (${r.size})`)}</button>
      <button class="wd-btn wd-btn-danger wd-btn-sm" data-action="cyc-bulk-del" ${r.size<1?"disabled":""}>${this._t("btn.delete",{},"Delete")}${r.size>=1?` (${r.size})`:""}</button>
      ...
      <div class="wd-prof-wrap">
        <button class="wd-profile-card" type="button" data-action="open-profile" data-pname="${d(t.name)}">
          ...
          <div class="wd-profile-meta">${t.cycle_count||0} cycles \xB7 ${e}${s}${n}${r}</div>
        </button>
      </div>`}_paintSparkline(t,e){if(!t||!Array.isArray(e)||e.length<3)return;const s=(getComputedStyle(this).getPropertyValue("--primary-color")||"#03a9f4").trim()||"#03a9f4",n=window.devicePixelRatio||1,a=t.getBoundingClientRect(),r=t.width=Math.max(1,Math.round((a.width||64)*n)),o=t.height=Math.max(1,Math.round((a.height||20)*n)),i=t.getContext("2d");i.clearRect(0,0,r,o);const l=Math.max(...e,1),c=2*n,_=g=>c+(e.length===1?0:g/(e.length-1)*(r-2*c)),p=g=>o-c-Math.max(0,g)/l*(o-2*c);i.beginPath(),e.forEach((g,u)=>{const v=_(u),b=p(g);u===0?i.moveTo(v,b):i.lineTo(v,b)}),i.strokeStyle=s,i.lineWidth=1.5*n,i.lineJoin="round",i.lineCap="round",i.stroke()}_drawProfileSparklines(){const t=this.shadowRoot;if(!t)return;const e=t.querySelectorAll("canvas[data-spark-prof]");if(!e.length)return;const s={};for(const n of this._profiles||[])s[n.name]=n;e.forEach(n=>{const a=s[n.dataset.sparkProf];this._paintSparkline(n,a&&a.signature_curve||[])})}_htmlProfiles(){const t=this._busy.has("rebuild-envelopes"),e=this._canEdit(),s={};this._profiles.forEach(u=>{s[u.name]=u});const n=this._profileGroups||{groups:[],suggestions:[]},a=new Set;n.groups.forEach(u=>(u.members||[]).forEach(v=>a.add(v)));const r=n.groups.map(u=>{const v=(u.members||[]).map(C=>s[C]?this._profileCardHtml(s[C]):"").join(""),b=Math.round((u.cohesion!=null?u.cohesion:1)*100),$=u.cohesive?`<span class="wd-badge" style="color:var(--success-color,#4caf50);background:rgba(76,175,80,.14);margin-bottom:0">${this._t("lbl.cohesion_good",{pct:b},"cohesion "+b+"%")}</span>`:`<span class="wd-badge" style="color:var(--warning-color,#ff9800);background:rgba(255,152,0,.14);margin-bottom:0">${this._t("lbl.cohesion_low",{pct:b},"\u26A0 low cohesion "+b+"%")}</span>`,k=u.cohesive?"":`<p class="wd-info" style="margin:0 0 8px;color:var(--warning-color,#ff9800)">${this._t("msg.group_not_cohesive",{},"These profiles aren't similar enough to group reliably, so matching treats them individually until you remove the outlier or split the group.")}</p>`;return`<div class="wd-card" style="margin-bottom:12px">
      ...
      </div>`}).join(""),o=this._profiles.filter(u=>!a.has(u.name)),i=o.map(u=>this._profileCardHtml(u)).join(""),l=this._profiles.length===0&&!(this._cycles||[]).length&&!(this._refCycles||[]).length,_=(e&&this._onlineEnabled()&&l?`
    ...
    `}_htmlProfileGroupModal(t){const e=this._busy.has("pg-save"),s=this._profileEnvCache||{},n=p=>et[Math.max(0,this._profiles.findIndex(g=>g.name===p))%et.length],a=t.members||[],r=this._profiles.map(p=>{const g=a.includes(p.name),u=p.avg_duration?`~${Math.round(p.avg_duration/60)}m`:"",v=p.avg_energy!=null?` \xB7 ${gt(p.avg_energy)}`:"",b=g?`<span style="display:inline-block;width:10px;height:10px;border-radius:2px;background:${n(p.name)};margin:0 2px"></span>`:"";return`<label class="wd-rev-tag"><input type="checkbox" class="wd-pg-mem" value="${d(p.name)}" ${g?"checked":""}> ${b}${d(p.name)} <span style="color:var(--secondary-text-color);font-size:.85em">${u}${v}</span></label>`}).join(""),o=a.filter(p=>s[p]&&(s[p].avg||[]).length),i=o.length?`<div class="wd-leg">${o.map(p=>`<span class="wd-leg-i"><span class="wd-leg-sw" style="background:${n(p)}"></span> ${d(p)}</span>`).join("")}</div>`:"",l=o.length?`<div class="wd-canvas-wrap" style="margin-top:8px"><canvas id="wd-pgroup-canvas" role="img" aria-label="${d(this._t("lbl.aria_envelope_chart",{},"Profile power envelope chart"))}" style="height:150px"></canvas></div>${i}`:`<p class="wd-info">${this._t("msg.group_preview_hint",{},"Tick 2+ members to preview and compare their power curves.")}</p>`,c=((this._profileGroups||{}).groups||[]).find(p=>p.name===t.orig),_=c&&c.cohesion!=null?`<span class="wd-badge" style="color:${c.cohesive?"var(--success-color,#4caf50)":"var(--warning-color,#ff9800)"};background:${c.cohesive?"rgba(76,175,80,.14)":"rgba(255,152,0,.14)"}">${c.cohesive?this._t("lbl.cohesion_good",{pct:Math.round(c.cohesion*100)},"cohesion "+Math.round(c.cohesion*100)+"%"):this._t("lbl.cohesion_low",{pct:Math.round(c.cohesion*100)},"\u26A0 low cohesion "+Math.round(c.cohesion*100)+"%")}</span>`:"";return`<h2>${t.orig?this._t("modal.edit_group",{},"Edit profile group"):this._t("modal.new_group",{},"New profile group")}</h2>
      ...
      ${l}
      <div class="wd-rev-sub">${this._t("lbl.members",{},"Members")}${a.length?` (${a.length})`:""}</div>
      <div class="wd-rev-tags">${r||`<span class="wd-info">${this._t("msg.no_profiles_yet_short",{},"No profiles yet.")}</span>`}</div>
    ...
    </label>`,b=n?`<p class="wd-info" style="margin:0 0 10px;font-size:.82em">${this._t("msg.settings_basic_note",{},"Showing essential settings. Switch to Advanced for the full list.")}</p>`:"",$=this._busy.has("save-settings"),k=l.size,S=k!==1?"s":"",C=k?`
      ...
      </div>`:"",R=this._suggestions.length,B=[...r].filter(U=>!a.has(U)).length,W=R+B,O=this._settingsSugOnly&&!this._settingsSearch,D=R?`<button class="wd-btn wd-btn-sm wd-btn-primary" data-action="sug-apply-all">${this._t("btn.apply_all",{},"Apply all")}</button>`:"",A=R?`<button class="wd-btn wd-btn-sm wd-btn-secondary" data-action="sug-dismiss">${this._t("btn.dismiss",{},"Dismiss")}</button>`:"",E=W?O?`
    ...
    `}_htmlSettingsHistory(){const t=this._settingsChangelog||[];if(!t.length)return"";const e=this._settingsHistoryOpen,s=this._canEdit(),n=t.slice(0,100).map(a=>{const r=s?`<button class="wd-btn wd-btn-secondary wd-btn-xs" data-action="settings-revert-key" data-key="${d(a.key)}" data-val="${d(JSON.stringify(a.old))}" title="${d(this._t("btn.revert_to_previous",{},"Restore this setting to its previous value"))}">${this._t("btn.revert",{},"Revert")}</button>`:"";return`<tr>
    ...
    </div>`:"";return`<div class="wd-field"><label>${d(n)} ${a?it(a):""}${c}${_}</label>
      <div class="wd-combo-row">
        <div class="wd-combo">
          <input type="text" id="wd-store-model" class="wd-combo-inp" data-opt="${t}" data-ftype="text" value="${d(s)}" placeholder="${r}" autocomplete="off" spellcheck="false">
          <div class="wd-combo-drop" hidden></div>
        </div>
      ...
      </div>`}async _loadDeviceAutomations(t){this._deviceAutomations=[];const e=this._hass;if(!(!e||!e.callWS))try{let s=null;const n=e.devices||{};for(const i of Object.values(n))if((i.config_entries||[]).includes(t)){s=i.id;break}const a=s?await e.callWS({type:"search/related",item_type:"device",item_id:s}):await e.callWS({type:"search/related",item_type:"config_entry",item_id:t});if(!this._isActiveEntry(t))return;const r=a&&a.automation||[],o=e.states||{};this._deviceAutomations=r.map(i=>{const l=o[i]&&o[i].attributes||{};return{entity_id:i,id:l.id,name:l.friendly_name||i,enabled:o[i]?o[i].state==="on":!0}}).filter(i=>i.id)}catch{this._isActiveEntry(t)&&(this._deviceAutomations=[])}}_navigate(t){try{history.pushState(null,"",t),this.dispatchEvent(new CustomEvent("location-changed",{bubbles:!0,composed:!0,detail:{replace:!1}}))}catch{try{window.location.assign(t)}catch{}}}async _newAutomationFromEvent(t){const e=this._devices[this._selIdx];if(!e)return;const s=this._hass,n=t==="started"?"ha_washdata_cycle_started":"ha_washdata_cycle_ended",a=t==="started"?"started":"finished",r={alias:`${e.title||"WashData"}: cycle ${a}`,description:`Runs when the WashData ${e.title||""} cycle ${a}. Add your actions (notify, lights, ...).`,mode:"single",trigger:[{platform:"event",event_type:n,event_data:{entry_id:e.entry_id}}],condition:[],action:[]},o="washdata_"+Date.now().toString(36);try{s&&s.callApi?(await s.callApi("POST","config/automation/config/"+o,r),this._navigate("/config/automation/edit/"+o)):this._navigate("/config/automation/edit/new")}catch(i){this._showToast(this._t("msg.toast_automation_failed",{error:i.message||i},"Could not create automation: "+(i.message||i)),"error")}}async _convertLegacyActions(){const t=this._devices[this._selIdx],e=this._hass,s=Array.isArray(this._opts.notify_actions)?this._opts.notify_actions:[];if(!t||!s.length)return;const n={alias:`${t.title||"WashData"}: migrated custom actions`,description:"Migrated from WashData legacy custom actions (which ran on cycle start, finish and live). Trim the triggers as needed. Note: old {device}/{duration}-style placeholders are NOT templated here - replace them with Jinja templates such as {{ trigger.event.data.device_name }}.",mode:"single",trigger:[{platform:"event",event_type:"ha_washdata_cycle_started",event_data:{entry_id:t.entry_id}},{platform:"event",event_type:"ha_washdata_cycle_ended",event_data:{entry_id:t.entry_id}}],condition:[],action:s},a="washdata_"+Date.now().toString(36);if(!e||!e.callApi){this._showToast(this._t("msg.toast_no_automation",{},"Cannot create automation here"),"error");return}let r=!1;try{await e.callApi("POST","config/automation/config/"+a,n),r=!0,await this._ws({type:`${y}/set_options`,entry_id:t.entry_id,options:{notify_actions:[]}}),this._isActiveEntry(t.entry_id)&&(this._opts={...this._opts,notify_actions:[]}),this._showToast(this._t("msg.toast_automation_migrated",{},"Actions migrated to an automation; opening editor")),this._navigate("/config/automation/edit/"+a)}catch(o){const i=o.message||o;if(!r){this._showToast(this._t("msg.toast_convert_failed",{error:i},"Convert failed: "+i),"error");return}let l=null;try{const c=await this._ws({type:`${y}/get_options`,entry_id:t.entry_id}),_=c&&c.options||{},p=Array.isArray(_.notify_actions)?_.notify_actions:[];l=p.length>0,this._isActiveEntry(t.entry_id)&&(this._opts={...this._opts,notify_actions:p})}catch{l=null}if(l===!1){this._showToast(this._t("msg.toast_automation_migrated",{},"Actions migrated to an automation; opening editor")),this._navigate("/config/automation/edit/"+a);return}if(l===!0){try{await e.callApi("DELETE","config/automation/config/"+a),this._showToast(this._t("msg.toast_convert_rolled_back",{error:i},"Migration failed and was rolled back (no automation left behind): "+i),"error")}catch{this._showToast(this._t("msg.toast_convert_orphan",{},"The automation was created, but clearing the old actions failed. Do not retry: remove the legacy actions manually to avoid a duplicate automation."),"error")}return}this._showToast(this._t("msg.toast_convert_orphan",{},"The automation was created, but clearing the old actions failed. Do not retry: remove the legacy actions manually to avoid a duplicate automation."),"error")}}_htmlSettingsSection(t){const e=this._opts&&this._opts.device_type||"",s=this._settingsLevel()==="basic",n=l=>!(l.id==="ml_training"||e&&l.notDeviceTypes&&l.notDeviceTypes.includes(e)||e&&l.onlyDeviceTypes&&!l.onlyDeviceTypes.includes(e)||s&&!this._secHasBasicFields(l)),a=mt.find(l=>l.id===this._settingsSec&&n(l))||mt.find(l=>n(l))||mt[0],r=a.intro||this._t("section."+a.id+".intro",{},"")?`<p class="wd-sec-intro">${d(this._t("section."+a.id+".intro",{},a.intro||""))}</p>`:"",o="";if(a.id==="notifications"){const l=`<p class="wd-info" style="margin-bottom:16px">${this._t("msg.notify_services_hint",{entity:"<code>notify.&lt;name&gt;</code>",vars:"<code>"+d($t)+"</code>"},"Use {entity} service IDs (comma-separated for multiple). Template variables: {vars}.")}</p>`,c=a.groups.map(p=>{const g=(p.fields||[]).filter(u=>this._settingFieldVisible(u)).map(u=>this._renderField(u,t)).filter(Boolean).join("");return g?`<div class="wd-subhead">${d(this._t("setting_group."+Jt(p.sub)+".label",{},p.sub))}</div><div class="wd-form-grid">${g}</div>`:""}).join("");return`${s?"":this._htmlAutomations()}${l}${c}`}if(a.groups){const l=a.groups.map(c=>{const _=c.sub?`<div class="wd-subhead">${d(this._t("setting_group."+Jt(c.sub)+".label",{},c.sub))}</div>`:"",p=(c.fields||[]).filter(g=>this._settingFieldVisible(g)).map(g=>this._renderField(g,t)).filter(Boolean).join("");return p?`${_}<div class="wd-form-grid">${p}</div>`:""}).join("");return`${r}${o}${l}`}const i=(a.fields||[]).filter(l=>this._settingFieldVisible(l)).map(l=>this._renderField(l,t)).filter(Boolean).join("");return`${r}${o}<div class="wd-form-grid">${i}</div>`}_htmlSettingsSearch(t,e){const s=this._opts&&this._opts.device_type||"",n=mt.filter(i=>!(i.id==="ml_training"||s&&i.notDeviceTypes&&i.notDeviceTypes.includes(s)||s&&i.onlyDeviceTypes&&!i.onlyDeviceTypes.includes(s))),a=i=>`${i.label||""} ${i.key||""} ${i.doc||""} ${i.hint||""}`.toLowerCase().includes(e);let r="",o=0;for(const i of n){const c=(i.fields||(i.groups||[]).flatMap(p=>p.fields||[])).filter(a);if(!c.length)continue;const _=c.map(p=>this._renderField(p,t)).filter(Boolean).join("");_&&(o+=c.length,r+=`<div class="wd-subhead">${d(this._t("section."+i.id+".label",{},i.label))}</div><div class="wd-form-grid">${_}</div>`)}return o?r:`<p class="wd-info" style="padding:12px">${this._t("msg.no_settings_match",{q:e},`No settings match "${d(e)}"`)}</p>`}_mlSugKeys(t){const e=t||Object.assign({},this._optDefaults,this._opts,this._pendingSettings||{});return this._mlSugKeysFrom(this._mlSettings,e,this._lockedSuggestions)}_mlSugKeysFrom(t,e,s){const n=new Set(s||[]),a=e||{},r=new Set;for(const[o,i]of Object.entries(t||{}))n.has(o)||i&&i.ml_value!=null&&!Ft(i.ml_value,a[o])&&r.add(o);return r}_sugCountsForDevice(t){if(!t)return{classic:0,ml:0,total:0};const e=this._devices[this._selIdx],s=!!(e&&e.entry_id===t.entry_id),n=Array.isArray(t.suggestion_keys)?t.suggestion_keys:null,a=n?n.length:t.suggestions_count||0,r=s?Object.assign({},t.option_defaults||{},t.options||{},this._opts,this._pendingSettings||{}):Object.assign({},t.option_defaults||{},t.options||{}),o=this._mlSugKeysFrom(s?this._mlSettings:this._mlSettingsByEntry[t.entry_id],r,s?this._lockedSuggestions:this._lockedByEntry[t.entry_id]),i=n?[...o].filter(l=>!n.includes(l)).length:o.size;return{classic:a,ml:i,total:a+i}}_htmlSettingsSugOnly(t){const e=new Set((this._suggestions||[]).map(r=>r.key));for(const r of this._mlSugKeys(t))e.add(r);if(!e.size)return`<p class="wd-info" style="padding:12px">${this._t("msg.no_suggestions",{},"No active suggestions.")}</p>`;const s=this._opts&&this._opts.device_type||"",n=mt.filter(r=>!(r.id==="ml_training"||s&&r.notDeviceTypes&&r.notDeviceTypes.includes(s)||s&&r.onlyDeviceTypes&&!r.onlyDeviceTypes.includes(s)));let a="";for(const r of n){const i=(r.fields||(r.groups||[]).flatMap(c=>c.fields||[])).filter(c=>e.has(c.key));if(!i.length)continue;const l=i.map(c=>this._renderField(c,t)).filter(Boolean).join("");l&&(a+=`<div class="wd-subhead">${d(this._t("section."+r.id+".label",{},r.label))}</div><div class="wd-form-grid">${l}</div>`)}return a||`<p class="wd-info" style="padding:12px">${this._t("msg.no_suggestions",{},"No active suggestions.")}</p>`}_htmlMlTab(){const t=this._opts;if(!Object.keys(t).length)return`<div class="wd-empty"><div class="wd-icon">\u{1F916}</div>${this._t("msg.loading",{},"Loading\u2026")}</div>`;const e=this._mlTrainingStatus,s=this._devices[this._selIdx],n=s&&s.entry_id,a=mt.find(i=>i.id==="ml_training"),r=a?(a.fields||[]).map(i=>this._renderField(i,t)).filter(Boolean).join(""):"",o=this._busy.has("save-settings");return`
      ...
      <div class="wd-card" style="margin-top:12px">
        ...
        <form id="wd-ml-form"><div class="wd-form-grid">${r}</div></form>
    ...
    `}_htmlMlStatusSection(t,e){const s=e&&this._busy.has("ml-train-now:"+e)||t&&t.running,n=this._canEdit()?`<button class="wd-btn wd-btn-primary wd-btn-sm" data-action="ml-train-now" ${s?"disabled":""}>${s?`<span class="wd-spin"></span> ${this._t("status.training",{},"Training\u2026")}`:this._t("btn.train_now",{},"Train now")}</button>`:"";if(!t)return`<div class="wd-card"><div class="wd-card-title" style="margin:0 0 4px">${this._t("hdr.status",{},"Status")}</div><p class="wd-info" style="margin:0">${this._t("msg.loading",{},"Loading\u2026")}</p></div>`;const a=Object.keys(t.on_device_models||{}).length,r=a?`<span style="color:var(--success-color,#4caf50);font-weight:600">${this._t("ml.personalized",{},"\u25CF Personalized to this machine")}</span> <span style="color:var(--secondary-text-color)">${this._t("lbl.models_fine_tuned",{count:a},"(fine-tuned models: "+a+")")}</span>`:`<span style="color:var(--secondary-text-color)">${this._t("ml.builtin_models",{},"\u25CF Using built-in models")}</span>`,o=t.cycle_count||0,i=t.min_cycles||0,l=o>=i,c=i>0?Math.min(100,Math.round(o/i*100)):100,_=l?"var(--success-color,#4caf50)":"var(--warning-color,#ff9800)",p=Math.max(0,i-o),g=l?this._t("msg.enough_data",{current:o,min:i},`Enough data to learn from (${o}/${i} cycles).`):this._t("msg.collecting_data",{need:p,current:o,min:i},`Collecting data. Cycles still needed before fine-tuning can start: ${p} (${o}/${i}).`),u=`<div style="height:8px;border-radius:6px;background:var(--secondary-background-color);overflow:hidden;margin:8px 0"><div style="width:${c}%;height:100%;background:${_}"></div></div>`,v=t.last_trained?st(t.last_trained):"never",b=s?`<span style="color:var(--info-color,#2196f3)"><span class="wd-spin"></span> ${this._t("status.fine_tuning",{},"fine-tuning now\u2026")}</span>`:t.enabled?this._t("lbl.auto_fine_tune_on",{hour:String(t.hour).padStart(2,"0")},`auto fine-tune on (around ${String(t.hour).padStart(2,"0")}:00)`):this._t("lbl.auto_fine_tune_off",{},"auto fine-tune off");return`<div class="wd-card">
    ...
    </div>`}_htmlMlLearnedSection(t){if(!t)return"";const e=t.on_device_models||{},s=Object.keys(e),n=this._busy.has("ml-revert-models");let a;if(!s.length)a=`<p class="wd-info" style="margin:0">${this._t("msg.no_fine_tuned",{},"Nothing fine-tuned yet \u2014 WashData is using its built-in models.")}</p>`;else{const r=s.map(i=>{const l=e[i]||{},c=l.trained_at?st(l.trained_at):"unknown";return`<div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid var(--divider-color)">
      ...
      ${a}
    </div>`}_mlQualityChip(t){let e=0,s="",n=t.metric_key?this._t(t.metric_key,t.metric_params||{},t.metric||""):t.metric||"";if(t.auc!=null)e=Math.max(0,Math.min(1,(t.auc-.5)/.5))*100,s=t.auc>=.85?this._t("ml.fit_strong",{},"Strong"):t.auc>=.75?this._t("ml.fit_good",{},"Good"):t.auc>=.65?this._t("ml.fit_fair",{},"Fair"):this._t("ml.fit_weak",{},"Weak");else if(t.model_mae!=null&&t.naive_mae!=null&&t.naive_mae>0){const r=Math.max(0,(t.naive_mae-t.model_mae)/t.naive_mae);e=Math.min(1,r)*100,s=r>=.5?this._t("ml.fit_strong",{},"Strong"):r>=.2?this._t("ml.fit_good",{},"Good"):this._t("ml.fit_slight",{},"Slight"),n=this._t("ml.better_than_baseline",{pct:(r*100).toFixed(0),metric:n},`${(r*100).toFixed(0)}% better than the baseline estimate (${n})`)}else return"";const a=e>=70?"var(--success-color,#4caf50)":e>=40?"var(--warning-color,#ff9800)":"var(--secondary-text-color)";return`<div title="${d(n)}" style="text-align:right;flex:0 0 auto">
      <div style="font-size:.8em;font-weight:600;color:${a}">${s} ${this._t("ml.fit_word",{},"fit")}</div>
      <div style="height:6px;width:90px;border-radius:5px;background:var(--secondary-background-color);overflow:hidden;margin:3px 0 0 auto"><div style="width:${e.toFixed(0)}%;height:100%;background:${a}"></div></div>
    </div>`}_mlTrendBadge(t){if(!t)return"";const s={improving:[this._t("badge.improving",{},"\u2197 improving"),"var(--success-color,#4caf50)",this._t("ml.trend_improving_tip",{},"This model's fit has improved across recent re-checks.")],declining:[this._t("badge.declining",{},"\u2198 declining"),"var(--warning-color,#ff9800)",this._t("ml.trend_declining_tip",{},"This model's fit has slipped across recent re-checks \u2014 reviewing more cycles may help it re-learn.")],steady:[this._t("badge.steady",{},"\u2192 steady"),"var(--secondary-text-color)",this._t("ml.trend_steady_tip",{},"This model's fit has held roughly steady across recent re-checks.")]}[t];return s?` <span title="${d(s[2])}" style="font-size:.72em;font-weight:600;color:${s[1]};margin-left:6px">${s[0]}</span>`:""}_htmlMatchingTuningCard(){const t=this._mlTrainingStatus,e=t&&t.matching;if(!e)return"";const s=e.defaults||{},n=e.tuned||null,a=n&&n.config||null,r=e.active==="tuned"&&a,o=this._busy.has("ml-revert-match"),i=g=>g==null||isNaN(g)?"-":Number(g).toFixed(2),l=[["corr_weight","Shape (correlation)"],["duration_weight","Duration agreement"],["energy_weight","Energy agreement"],["dtw_ensemble_w","DTW derivative blend (DDTW)"]].map(([g,u])=>{const v=s[g],b=r?a[g]:s[g],$=r&&v!=null&&b!=null&&Math.abs(v-b)>1e-9;return`<tr>
        <td>${u}</td>
      ...
      </tr>`}).join(""),c=r?`<span class="wd-badge" style="color:var(--success-color,#4caf50);background:rgba(76,175,80,.14)">${this._t("badge.using_tuned",{},"Using tuned weights")}</span>`:`<span class="wd-badge" style="color:var(--secondary-text-color);background:var(--secondary-background-color)">${this._t("badge.using_defaults",{},"Using shipped defaults")}</span>`;let _="";if(r){const g=n.trained_at?st(n.trained_at):"unknown",u=n.baseline_test_top1,v=n.tuned_test_top1,b=u!=null&&v!=null?` \xB7 held-out top-1 ${(u*100).toFixed(0)}% \u2192 <strong>${(v*100).toFixed(0)}%</strong>`:"";_=`<p class="wd-info" style="margin:8px 0 0">Tuned ${d(g)} from ${n.cycle_count||0} cycles${b}.</p>`}const p=r?`<button class="wd-btn wd-btn-secondary wd-btn-sm" data-action="ml-revert-match" ${o?"disabled":""}>${o?'<span class="wd-spin"></span> '+this._t("status.reverting",{},"Reverting\u2026"):this._t("btn.reset_to_defaults",{},"Reset to defaults")}</button>`:"";return`<div class="wd-card" style="margin-top:12px">
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;gap:8px;flex-wrap:wrap">
        ...
        <span style="font-size:.75em">${a}</span>
      </div>
      ...
      ${l}
      ${c}
    </div>`}_pgApplyPresetValues(t){this._pgThreshStart=null,this._pgThreshStop=null,this._pgParamOverrides={};const e=this._pgEffective||{};for(const[s,n]of Object.entries(t||{})){if(n==null)continue;const a=e[s];a!=null&&this._pgSameVal(n,a)||this._pgSetStaged(s,n)}}async _pgSavePreset(){const t=this._devices[this._selIdx],e=(this._pgPresetName||"").trim();!t||!this._canEdit()||!e||(this._pgPresets||[]).some(s=>s.name===e)&&!confirm(this._t("msg.pg_preset_overwrite",{name:e},`Overwrite the preset "${e}"?`))||(await this._busyRun("pg-preset-save",async()=>{try{const s=await this._ws({type:`${y}/save_playground_preset`,entry_id:t.entry_id,name:e,values:this._pgCurrentValues()});if(!this._isActiveEntry(t.entry_id))return;this._pgPresets=Array.isArray(s.presets)?s.presets:this._pgPresets,this._pgPresetSel=e,this._pgPresetName="",this._showToast(this._t("toast.pg_preset_saved",{name:e},`Preset "${e}" saved`))}catch(s){this._showToast(this._t("msg.toast_save_failed",{error:s.message||s},"Save failed: "+(s.message||s)),"error")}}),this._render())}async _pgDeletePreset(){const t=this._devices[this._selIdx],e=this._pgPresetSel;!t||!this._canEdit()||!e||confirm(this._t("msg.pg_preset_delete_confirm",{name:e},`Delete the preset "${e}"?`))&&(await this._busyRun("pg-preset-delete",async()=>{try{const s=await this._ws({type:`${y}/delete_playground_preset`,entry_id:t.entry_id,name:e});if(!this._isActiveEntry(t.entry_id))return;this._pgPresets=Array.isArray(s.presets)?s.presets:(this._pgPresets||[]).filter(n=>n.name!==e),this._pgPresetSel=""}catch(s){this._showToast(this._t("msg.toast_error",{error:s.message||s},"Error: "+(s.message||s)),"error")}}),this._render())}async _pgLoadLive(){const t=this._devices[this._selIdx];if(!t)return;const e=this._pgChangedKeys();e.length&&!confirm(this._t("msg.pg_load_live_confirm",{n:e.length},`Discard ${e.length} Playground edit(s) and reload the integration's current settings?`))||(await this._busyRun("pg-load-live",async()=>{this._pgThreshStart=null,this._pgThreshStop=null,this._pgParamOverrides={},await this._pgFetchSettings(t.entry_id)}),this._render(),requestAnimationFrame(()=>this._pgDrawCanvas()))}_pgLoadSuggested(t){const e=t==="ml"?this._pgSuggMl||{}:this._pgSuggClassic||{},s=this._pgEffective||{};let n=0;for(const[a,r]of Object.entries(e)){if(r==null)continue;const o=s[a];o!=null&&this._pgSameVal(r,o)||(this._pgSetStaged(a,r),n++)}if(n===0)this._showToast(this._t("msg.pg_sugg_none",{},"All suggestions already match the current settings"));else{const a=t==="ml"?"toast.pg_sugg_ml_loaded":"toast.pg_sugg_loaded",r=t==="ml"?`Staged ${n} ML-calibrated value(s) - run the Playground to compare`:`Staged ${n} suggested value(s) - run the Playground to compare`;this._showToast(this._t(a,{n},r))}this._render(),requestAnimationFrame(()=>this._pgDrawCanvas())}async _pgPublishOne(t){const e=this._devices[this._selIdx];if(!e||!this._canEdit()||!t||!this._pgIsPublishable(t))return;const s=this._pgStagedVal(t);if(s==null)return;const n=this._t("setting."+t+".label",{},t);confirm(this._t("msg.pg_publish_one_confirm",{label:n,value:s},`Save ${n} = ${s} to this device's settings?`))&&(await this._busyRun("pg-publish-"+t,async()=>{try{if(await this._ws({type:`${y}/set_options`,entry_id:e.entry_id,options:{[t]:s}}),!this._isActiveEntry(e.entry_id))return;this._opts={...this._opts,[t]:s},this._pgEffective&&(this._pgEffective={...this._pgEffective,[t]:s}),this._pgClearStaged(t),this._showToast(this._t("toast.settings_saved",{},"Settings saved; integration reloading"))}catch(a){this._showToast(this._t("msg.toast_save_failed",{error:a.message||a},"Save failed: "+(a.message||a)),"error")}}),this._render())}_htmlPlayground(){if(!this._devices[this._selIdx])return`<div class="wd-empty">${this._t("msg.no_device_selected",{},"No device selected.")}</div>`;const e=this._cycles||[],s=this._profiles||[],n=e.map(u=>{const v=u.profile_name||u.matched_profile||this._t("lbl.unlabelled",{},"Unlabelled"),b=u.duration?` \xB7 ${Math.round(u.duration/60)} min`:"",$=u.start_time?` \xB7 ${st(u.start_time)}`:"";return`<option value="${d(u.id)}" ${this._pgCycleId===u.id?"selected":""}>${d(v+b+$)}</option>`}).join(""),a=`<option value="">${d(this._t("lbl.auto_detect",{},"Auto-detect"))}</option>`+s.map(u=>`<option value="${d(u.name)}" ${this._pgProfileName===u.name?"selected":""}>${d(u.name)}</option>`).join(""),r=this._pgLoading,o=`<div style="display:flex;gap:10px;flex-wrap:wrap;align-items:flex-end;margin-bottom:4px">
      <div class="wd-field" style="min-width:180px;margin:0"><label>${this._t("lbl.cycle",{},"Cycle")}</label><select id="wd-pg-cyc-sel" ${r?"disabled":""}>${n||'<option value="">\u2014</option>'}</select></div>
      <div class="wd-field" style="min-width:160px;margin:0"><label>${this._t("lbl.profile",{},"Profile")}</label><select id="wd-pg-prof-sel" ${r?"disabled":""}>${a}</select></div>
      <div style="display:flex;gap:6px;align-items:flex-end;padding-bottom:2px">
        <button class="wd-btn wd-btn-primary" data-action="pg-run" ${r?"disabled":""} style="min-width:72px">\u25B6 ${this._t("btn.run",{},"Run")}</button>
        ${r?`<button class="wd-btn" data-action="pg-cancel-run" style="min-width:72px">\u2715 ${this._t("btn.cancel",{},"Cancel")}</button>`:""}
      </div>
    </div>`,i=r?`<div class="wd-pg-simbar" role="progressbar" aria-label="${d(this._t("msg.pg_simulating",{},"Simulating cycle\u2026"))}"><div class="wd-pg-simbar-fill"></div></div>
         <div style="font-size:.78em;color:var(--secondary-text-color);margin:4px 0 0">${this._t("msg.pg_simulating",{},"Simulating cycle\u2026")}</div>`:"",l=!this._pgPowerPts&&!r?`<div style="position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;pointer-events:none;gap:6px">
      ...
      ${[["history",this._t("lbl.pg_mode_history",{},"Test on history")],["sweep",this._t("lbl.pg_mode_optimize",{},"Optimize")]].map(([a,r])=>`<button role="tab" aria-selected="${t===a}" class="wd-pg-subtab${t===a?" active":""}" data-action="pg-analysis-tab" data-subtab="${a}">${d(r)}</button>`).join("")}
    ...
    </div>`}_htmlPgAlerts(){const t=this._pgDetail;if(!t)return"";const e=t.outcome||{},s={error:"var(--error-color,#f44336)",warn:"var(--warning-color,#ff9800)",info:"var(--info-color,#2196f3)"},n=Array.isArray(t.alerts)?t.alerts:[],a=n.length?n.map(c=>`<div class="wd-pg-alert" style="border-left-color:${s[c.severity]||s.info}">
    ...
    </div>`,a=t?this._htmlPgBatchBar():"",r=`<p class="wd-sec-intro" style="margin:0 0 8px">${this._t("msg.pg_history_intro2",{},"Replay your recent cycles through the real detector and matcher with the settings above. Click any row to load that cycle in the graph; edit a setting to see a before/after comparison.")}</p>`;if(!e||!Array.isArray(e.rows))return`${r}${n}${this._htmlPgRecentRuns("pg_history")}${a}${t?"":`<div class="wd-empty" style="padding:24px">${this._t("msg.pg_history_empty",{},"Press Run to replay your cycles.")}</div>`}`;let o="";if(e.diff){const u=(v,b,$)=>`<span class="wd-pg-diffbadge" style="background:${b}22;color:${b}">${v} ${d($)}</span>`;o=`<div style="margin:0 0 10px">
    ...
    </tr></thead><tbody>${p}</tbody></table>`;return`${r}${n}${this._htmlPgRecentRuns("pg_history")}${o}${l}${g}`}_htmlPgBatchBar(){const t=this._pgBatchProgress;return t?`<div class="wd-pg-batchrow">
    ...
    </div>`,o=t?this._htmlPgBatchBar():"";return`${a}${r}${this._htmlPgRecentRuns("pg_sweep")}${o}${this._htmlPgSweepResult()}`}_htmlPgSweepResult(){const t=this._pgSweepNew;if(!t||!Array.isArray(t.points)||!t.points.length)return"";const e=this._pgSweepObjectives().find(p=>p[0]===t.objective),s=e?e[2]:!1,n=t.points.filter(p=>p.metric!=null).map(p=>p.metric);if(!n.length)return`<div class="wd-empty" style="padding:16px">${this._t("msg.pg_sweep_no_metric",{},"Not enough data to score this objective.")}</div>`;const a=Math.min(...n),r=Math.max(...n),o=s?Math.min(...n):Math.max(...n),i=(t.points.find(p=>p.metric===o)||{}).value,l=p=>t.objective==="median_overrun"?Math.round(p*100)+"% off":Math.round(p*100)+"%",c=t.points.map(p=>{const g=r>a&&p.metric!=null?(p.metric-a)/(r-a):p.metric!=null?1:0,u=p.metric===o,v=t.current_value!=null&&Math.abs(p.value-t.current_value)<1e-6,b=u?"var(--success-color,#4caf50)":"var(--primary-color)";return`<div style="display:flex;align-items:center;gap:8px;margin:3px 0;font-size:.82em">
      ...
      ${c}
      <div style="margin-top:8px">${_}</div>`}async _pgRunSweep2(){const t=this._devices[this._selIdx];if(!t)return;const e=parseFloat(this._pgSweepFrom),s=parseFloat(this._pgSweepTo);if(isNaN(e)||isNaN(s)||e===s){this._showToast(this._t("msg.toast_name_required",{},"Set a valid From/To range."),"error");return}const n=Math.max(2,Math.min(12,this._pgSweepSteps||5)),a=Array.from({length:n},(l,c)=>+(e+(s-e)*c/(n-1)).toFixed(3)),r=this._pgSweepParam||"off_delay",o=this._pgSweepObjective||"match_accuracy",i={type:`${y}/start_playground_sweep`,entry_id:t.entry_id,param:r,values:a,objective:o};this._pgBatchProgress={done:0,total:a.length},this._busy.add("pg-sweep"),this._render();try{const l=await this._ws(i);if(this._pgSweepTaskId=l&&l.task_id,this._pgNeedsRestart=!1,!this._pgSweepTaskId)throw new Error("no task id");this._addProvisionalTask(this._pgSweepTaskId,"pg_sweep",t.entry_id,a.length),this._tasksSubscribed||this._pgPollTask(this._pgSweepTaskId)}catch(l){this._busy.delete("pg-sweep"),this._pgBatchProgress=null,this._pgIsUnknownCmd(l)?this._pgNeedsRestart=!0:this._showToast(this._t("msg.toast_error",{error:l.message||l},"Error: "+(l.message||l)),"error"),this._render()}}async _pgApplyToSettings(){const t=this._devices[this._selIdx];if(!t||!this._canEdit())return;const e={};for(const a of this._pgChangedKeys())this._pgIsPublishable(a)&&(e[a]=this._pgStagedVal(a));const s=Object.keys(e);if(!s.length)return;const n=s.map(a=>this._t("setting."+a+".label",{},a)).join(", ");confirm(this._t("msg.pg_apply_settings_confirm",{n:s.length,list:n},`Save these ${s.length} setting(s) to this device? ${n}`))&&(await this._busyRun("pg-apply-settings",async()=>{try{if(await this._ws({type:`${y}/set_options`,entry_id:t.entry_id,options:e}),!this._isActiveEntry(t.entry_id))return;this._opts={...this._opts,...e},this._pgEffective&&(this._pgEffective={...this._pgEffective,...e});for(const a of s)this._pgClearStaged(a);this._showToast(this._t("toast.settings_saved",{},"Settings saved; integration reloading"))}catch(a){this._showToast(this._t("msg.toast_save_failed",{error:a.message||a},"Save failed: "+(a.message||a)),"error")}}),this._render())}async _pgApplySweepValue(t){const e=this._devices[this._selIdx];if(!e||!this._canEdit()||t==null)return;const s=this._pgSweepParam,n=this._t("setting."+s+".label",{},s);confirm(this._t("msg.pg_apply_confirm",{label:n,value:t},"Apply best value: "+n+" = "+t+"?"))&&await this._busyRun("pg-sweep-apply",async()=>{try{await this._ws({type:`${y}/set_options`,entry_id:e.entry_id,options:{[s]:+t}}),this._opts={...this._opts,[s]:+t},s==="start_threshold_w"?this._pgThreshStart=+t:s==="stop_threshold_w"?this._pgThreshStop=+t:this._pgParamOverrides[s]=+t,this._showToast(this._t("toast.settings_saved",{},"Settings saved; integration reloading")),this._render()}catch(a){this._showToast(this._t("msg.toast_save_failed",{error:a.message||a},"Save failed: "+(a.message||a)),"error")}})}_htmlPgStrip(){return`<div class="wd-pg-strip" id="wd-pg-strip">
      ...
      <span style="font-size:.75em;color:var(--secondary-text-color);text-transform:uppercase">${this._t("lbl.phase",{},"Phase")} <span id="wd-pg-phase" style="color:var(--primary-text-color,inherit)">\u2014</span></span>
        ...
        <span class="wd-pg-cand-pct">${i}%</span>
      </div>`}return`<div>${s||`<p class="wd-info" style="margin:0">${this._t("msg.pg_analysis_hint2",{},"Pick a cycle and press Run to load match analysis.")}</p>`}</div>`}async _pgLoad(){const t=this._devices[this._selIdx];if(!t||this._pgLoading)return;const e=this._pgCycleId||this._cycles?.[0]?.id||"";if(!e)return;this._pgCycleId=e,this._pgLoading=!0,this._pgView=null,this._pgHoverT=null,this._pgPowerPts=null,this._pgDtwData=null,this._pgEnvData=null,this._pgDetail=null;const s=++this._pgLoadSeq;this._render();try{const n=await this._ws({type:`${y}/get_cycle_power_data`,entry_id:t.entry_id,cycle_id:e}),a=n.samples||[],r=[];for(const l of a){if(!Array.isArray(l)||l.length<2)continue;const c=+l[0],_=+l[1];!isNaN(c)&&!isNaN(_)&&r.push({t:c,w:_})}if(this._pgPowerPts=r.length?r:null,typeof n.full_duration_s=="number"&&n.full_duration_s>0){const l=(this._cycles||[]).find(c=>c.id===e);l&&(l._pg_duration=n.full_duration_s)}const o=this._pgProfileName||(this._cycles||[]).find(l=>l.id===e)?.profile_name||"";if(r.length)try{const l={type:`${y}/get_dtw_debug`,entry_id:t.entry_id,cycle_id:e};o&&(l.profile_name=o),this._pgDtwData=await this._ws(l),this._pgNeedsRestart=!1}catch(l){this._pgIsUnknownCmd(l)&&(this._pgNeedsRestart=!0),this._pgDtwData=null}const i=this._pgDtwData?.profile_name||o;if(i)try{const l=await this._ws({type:`${y}/get_profile_envelope`,entry_id:t.entry_id,profile_name:i});this._pgEnvData=l.envelope||null}catch{this._pgEnvData=null}if(s!==this._pgLoadSeq)return;await this._pgLoadDetail(t.entry_id,e)}catch(n){this._showToast(this._t("msg.toast_error",{error:n.message||n},"Error: "+(n.message||n)),"error")}s===this._pgLoadSeq&&(this._pgLoading=!1,this._render(),requestAnimationFrame(()=>{this._pgDrawCanvas(),this._pgUpdateStripAt(null)}),this._pgNeedsRestart&&this._tab==="playground"?(this._pgRestartRetries||0)<5&&!this._pgRestartRetryTimer&&(this._pgRestartRetries=(this._pgRestartRetries||0)+1,this._pgRestartRetryTimer=setTimeout(()=>{this._pgRestartRetryTimer=null,this._pgNeedsRestart&&this._tab==="playground"&&this._pgLoad()},3e3)):this._pgRestartRetries=0)}_pgCancelRun(){this._pgLoadSeq++,this._pgLoading=!1,this._render()}_pgSelectCycle(t){t&&(this._pgCycleId=t,this._pgProfileName="",this._pgView=null,this._pgHoverT=null,this._pgLoading=!1,this._pgLoad(),requestAnimationFrame(()=>{const e=this.shadowRoot&&this.shadowRoot.getElementById("wd-pg-canvas");e&&typeof e.scrollIntoView=="function"&&e.scrollIntoView({behavior:"smooth",block:"center"})}))}async _pgLoadDetail(t,e){const s={...this._pgParamOverrides};this._pgThreshStart!=null&&(s.start_threshold_w=this._pgThreshStart),this._pgThreshStop!=null&&(s.stop_threshold_w=this._pgThreshStop);try{const n=await this._ws({type:`${y}/start_playground_cycle_detail`,entry_id:t,cycle_id:e,settings_override:s,stress_tail:this._pgStressTail,stress_idle_w:this._pgStressIdleW!=null?parseFloat(this._pgStressIdleW):null}),a=n&&n.task_id;if(!a)throw new Error("no task id");this._addProvisionalTask(a,"pg_detail",t,0);const r=await new Promise(o=>{this._taskCallbacks[a]=async l=>{if(l.state==="done"||l.state==="cancelled")try{const c=await this._ws({type:`${y}/get_task_result`,task_id:l.id});o(c&&c.result)}catch{o(null)}else o(null)};const i=this._tasks[a];i&&i.state!=="running"?this._settleTaskCallback(i):this._tasksSubscribed||this._pollTaskGeneric(a)});if(!this._isActiveEntry(t))return;this._pgDetail=r&&!r.error?r:null,this._pgNeedsRestart=!1}catch(n){this._pgIsUnknownCmd(n)&&(this._pgNeedsRestart=!0),this._pgDetail=null}}_pgRerunDetail(){const t=this._devices[this._selIdx];!t||!this._pgCycleId||(clearTimeout(this._pgDetailDebounceTimer),this._pgDetailDebounceTimer=setTimeout(async()=>{this._pgDetailBusy=!0,this._render(),await this._pgLoadDetail(t.entry_id,this._pgCycleId),this._pgDetailBusy=!1,this._render(),requestAnimationFrame(()=>this._pgDrawCanvas())},220))}_pgMapState(t){return t==="running"||t==="paused"?"running":t==="ending"?"ending":t==="starting"?"detecting":t==="anti_wrinkle"?"anti_wrinkle":"idle"}_pgSeriesAt(t){const e=this._pgDetail&&this._pgDetail.series;if(!e||!e.length)return null;let s=e[0];for(const n of e)if(n.t<=t)s=n;else break;return s}_pgStateSegsFromSeries(t){const e=this._pgDetail&&this._pgDetail.series;if(!e||!e.length)return[];const s=[];let n=null;for(const a of e){const r=this._pgMapState(a.state);!n||n.state!==r?(n={start:a.t,end:a.t,state:r},s.push(n)):n.end=a.t}return s.length&&(s[s.length-1].end=t),s}_pgDrawCanvas(){if(this._tab!=="playground")return;const t=this.shadowRoot,e=t&&t.getElementById("wd-pg-canvas");if(!e)return;const s=this._pgPowerPts,n=e.getBoundingClientRect(),a=window.devicePixelRatio||1,r=Math.max(1,Math.round(n.width*a)),o=Math.max(1,Math.round((n.height||280)*a));(e.width!==r||e.height!==o)&&(e.width=r,e.height=o);const i=e.getContext("2d"),l=getComputedStyle(this),c=(l.getPropertyValue("--primary-color")||"#03a9f4").trim(),_=(l.getPropertyValue("--divider-color")||"rgba(127,127,127,.2)").trim(),p=(l.getPropertyValue("--secondary-text-color")||"#888").trim(),g=(l.getPropertyValue("--secondary-background-color")||"#1a1a1a").trim();i.clearRect(0,0,r,o);const u=34*a,v=14*a,b=Bt*a,$=44*a,k=8*a,S=b+8*a,C=u+v+4*a,R=o-S-C;if(!s||!s.length){this._pgLoading&&(i.fillStyle=p,i.font=`${12*a}px sans-serif`,i.textAlign="center",i.textBaseline="middle",i.fillText(this._t("msg.loading",{},"Loading\u2026"),r/2,o/2));return}const B=this._pgDetail&&this._pgDetail.outcome&&this._pgDetail.outcome.stress,W=B&&B.enabled?B.synthetic_from_s:null,O=W!=null&&this._pgDetail&&this._pgDetail.series?this._pgDetail.series.filter(T=>T.t>=W):[];let D=(this._cycles||[]).find(T=>T.id===this._pgCycleId)?._pg_duration||s[s.length-1].t||1;B&&B.terminated&&B.terminated_after_s!=null&&W!=null?D=Math.max(D,W+B.terminated_after_s):O.length&&(D=Math.max(D,O[O.length-1].t));const A=Math.max(...s.map(T=>T.w),...O.map(T=>T.power||0),1),E=this._pgThreshStart??this._pgFieldVal("start_threshold_w",{})??50,M=this._pgThreshStop??this._pgFieldVal("stop_threshold_w",{})??5;let H=0,q=D;this._pgView&&this._pgView.max-this._pgView.min>1&&(H=Math.max(0,this._pgView.min),q=Math.min(D,this._pgView.max),q-H<=1&&(H=0,q=D));const Q=Math.max(1e-6,q-H),J=r-$-k,G=T=>$+(T-H)/Q*J,X=T=>S+(1-Math.max(0,T)/A)*R;this._pgMap={vMin:H,vMax:q,totalDur:D,padLpx:$/a,plotWpx:J/a},i.strokeStyle="rgba(127,127,127,0.12)",i.lineWidth=a,i.setLineDash([]),[.25,.5,.75,1].map(T=>Math.round(T*A/100)*100||Math.round(T*A)).forEach(T=>{const N=X(T);N<S||N>S+R||(i.beginPath(),i.moveTo($,N),i.lineTo(r-k,N),i.stroke(),i.fillStyle=p,i.font=`${9*a}px sans-serif`,i.textAlign="right",i.textBaseline="middle",i.fillText(T+"W",$-4*a,N))}),i.save(),i.beginPath(),i.rect($,S,J,R),i.clip();const U=this._pgEnvData,at=U&&Array.isArray(U.avg)?U.avg.filter(T=>Array.isArray(T)&&T.length>=2):[];if(at.length){const T=Math.max(...at.map(V=>V[0]))||1,N=V=>G(V/T*D),tt=Array.isArray(U.min)?U.min.filter(V=>Array.isArray(V)&&V.length>=2):[],Y=Array.isArray(U.max)?U.max.filter(V=>Array.isArray(V)&&V.length>=2):[];if(tt.length&&Y.length){i.beginPath(),Y.forEach((V,lt)=>lt?i.lineTo(N(V[0]),X(V[1])):i.moveTo(N(V[0]),X(V[1])));for(let V=tt.length-1;V>=0;V--)i.lineTo(N(tt[V][0]),X(tt[V][1]));i.closePath(),i.fillStyle="#eda10012",i.fill()}else i.beginPath(),i.strokeStyle="#eda100",i.lineWidth=2*a,i.setLineDash([5*a,4*a]),at.forEach((V,lt)=>lt?i.lineTo(N(V[0]),X(V[1])):i.moveTo(N(V[0]),X(V[1]))),i.stroke(),i.setLineDash([])}const f=this._pgDtwData,m=f&&Array.isArray(f.profile_trace)?f.profile_trace.filter(T=>Array.isArray(T)&&T.length>=2):[],x=f&&Array.isArray(f.cycle_trace)?f.cycle_trace.filter(T=>Array.isArray(T)&&T.length>=2):[],L=f&&Array.isArray(f.warp_path)?f.warp_path:[];if(x.length&&m.length&&L.length){const T=Math.max(...m.map(Y=>Y[0]))||1,N=Math.max(...x.map(Y=>Y[0]))||1,tt=Math.max(1,Math.floor(L.length/25));i.save(),i.globalAlpha=.13,i.strokeStyle="#fff",i.lineWidth=a;for(let Y=0;Y<L.length;Y+=tt){const V=L[Y];if(!Array.isArray(V)||V.length<2)continue;const lt=Math.min(V[0],x.length-1),pt=Math.min(V[1],m.length-1),bt=G(x[lt][0]/N*D),_t=X(x[lt][1]),yt=G(m[pt][0]/T*D),dt=X(m[pt][1]);i.beginPath(),i.moveTo(bt,_t),i.lineTo(yt,dt),i.stroke()}i.restore()}if(m.length){const T=Math.max(...m.map(N=>N[0]))||1;i.beginPath(),i.strokeStyle="#eda100",i.lineWidth=2*a,i.setLineDash([6*a,4*a]),m.forEach((N,tt)=>{const Y=G(N[0]/T*D),V=X(N[1]);tt?i.lineTo(Y,V):i.moveTo(Y,V)}),i.stroke(),i.setLineDash([])}if(i.beginPath(),i.moveTo(G(0),X(0)),s.forEach(T=>i.lineTo(G(T.t),X(T.w))),i.lineTo(G(s[s.length-1].t),X(0)),i.closePath(),i.fillStyle=St(c,.1),i.fill(),i.beginPath(),i.strokeStyle=c,i.lineWidth=2*a,s.forEach((T,N)=>N?i.lineTo(G(T.t),X(T.w)):i.moveTo(G(T.t),X(T.w))),i.stroke(),O.length){const T=s[s.length-1];i.beginPath(),i.moveTo(G(T.t),X(T.w)),O.forEach(N=>i.lineTo(G(N.t),X(N.power))),i.lineTo(G(O[O.length-1].t),X(0)),i.lineTo(G(T.t),X(0)),i.closePath(),i.fillStyle=St(c,.06),i.fill(),i.beginPath(),i.strokeStyle=c,i.lineWidth=1.5*a,i.setLineDash([4*a,4*a]),i.moveTo(G(T.t),X(T.w)),O.forEach(N=>i.lineTo(G(N.t),X(N.power))),i.stroke(),i.setLineDash([])}if(W!=null&&W<q){const T=G(W);i.fillStyle="rgba(192,57,43,0.07)",i.fillRect(T,S,Math.max(0,$+J-T),R),i.save(),i.strokeStyle="rgba(192,57,43,0.5)",i.lineWidth=a,i.setLineDash([4*a,4*a]),i.beginPath(),i.moveTo(T,S),i.lineTo(T,S+R),i.stroke(),i.restore()}i.restore();const j=(T,N,tt)=>{const Y=X(T);Y<S-2||Y>S+R+2||(i.save(),i.strokeStyle=N,i.lineWidth=2*a,i.setLineDash([8*a,4*a]),i.beginPath(),i.moveTo($,Y),i.lineTo(r-k,Y),i.stroke(),i.setLineDash([]),i.fillStyle=N,i.beginPath(),i.arc($+(r-$-k)*.06,Y,5*a,0,Math.PI*2),i.fill(),i.fillStyle=N,i.font=`bold ${9*a}px sans-serif`,i.textAlign="left",i.textBaseline="bottom",i.fillText(tt+" "+Math.round(T)+"W",$+14*a,Y-2*a),i.restore())};j(+E,"#2a78d6",this._t("lbl.start",{},"Start")),j(+M,"#e34948",this._t("btn.stop",{},"Stop"));const I={idle:g,detecting:"#42a5f566",running:"#66bb6a66",ending:"#ef535066",anti_wrinkle:"#ab47bc66"},w={idle:this._t("lbl.pg_idle",{},"Idle"),detecting:this._t("lbl.pg_detecting",{},"Detecting"),running:this._t("lbl.pg_ev_running",{},"Running"),ending:this._t("lbl.pg_ev_ending",{},"Ending"),anti_wrinkle:this._t("lbl.pg_anti_wrinkle",{},"Anti-wrinkle")},P=o-u-v;i.fillStyle=g,i.fillRect($,P,r-$-k,u),i.save(),i.beginPath(),i.rect($,P,J,u),i.clip(),this._pgStateSegsFromSeries(D).forEach(T=>{const N=G(T.start),tt=G(T.end);i.fillStyle=I[T.state]||_,i.fillRect(N,P,Math.max(1,tt-N),u),tt-N>50*a&&(i.fillStyle=p,i.font=`${8*a}px sans-serif`,i.textAlign="center",i.textBaseline="middle",i.fillText((w[T.state]||T.state).toUpperCase(),(N+tt)/2,P+u/2))}),i.restore();const F=(this._cycles||[]).find(T=>T.id===this._pgCycleId),K=this._pgDtwData?.profile_name||this._pgProfileName||F?.profile_name,Z=K?(this._profiles||[]).find(T=>T.name===K):null,rt=o-v;Z&&Array.isArray(Z.phases)&&Z.phases.length&&(i.save(),i.beginPath(),i.rect($,rt,J,v),i.clip(),Z.phases.forEach((T,N)=>{const tt=G((T.start||0)*D),Y=G((T.end||1)*D),V=N*47%360;i.fillStyle=`hsla(${V},60%,55%,0.55)`,i.fillRect(tt,rt,Math.max(1,Y-tt),v),Y-tt>40*a&&(i.fillStyle="#fff",i.font=`${7*a}px sans-serif`,i.textAlign="center",i.textBaseline="middle",i.fillText(T.name,(tt+Y)/2,rt+v/2))}),i.restore()),this._pgEventHits=[];const ht=this._pgDetail&&Array.isArray(this._pgDetail.events)?this._pgDetail.events.filter(T=>T.type!=="state"&&T.t>=H&&T.t<=q).slice().sort((T,N)=>T.t-N.t):[],nt=11*a,Ct=S-b/2-2*a,Zt=nt*2+3*a;let Nt=-1/0;const xt=this._pgHoverEvent;if(ht.forEach(T=>{const N=this._pgEventMeta(T.type),tt=G(T.t);let Y=Math.max(tt,Nt+Zt);Y=Math.max($+nt,Math.min(r-k-nt,Y)),Nt=Y;const V=xt&&xt.t===T.t&&xt.type===T.type;i.save(),i.strokeStyle=N.color,i.globalAlpha=V?.9:.5,i.lineWidth=(V?2:1)*a,i.beginPath(),i.moveTo(Y,Ct+nt),i.lineTo(tt,S),i.stroke(),V&&(i.globalAlpha=.3,i.setLineDash([2*a,3*a]),i.beginPath(),i.moveTo(tt,S),i.lineTo(tt,o-v),i.stroke(),i.setLineDash([])),i.restore(),i.save(),i.beginPath(),i.arc(Y,Ct,nt,0,Math.PI*2),i.fillStyle=V?N.color:g||"#1a1a1a",i.fill(),i.lineWidth=(V?2:1.5)*a,i.strokeStyle=N.color,i.stroke(),i.fillStyle=V?"#fff":N.color,i.font=`${14*a}px sans-serif`,i.textAlign="center",i.textBaseline="middle",i.fillText(N.glyph,Y,Ct+a),i.restore(),this._pgEventHits.push({cx:Y/a,cy:Ct/a,r:nt/a,type:T.type,label:N.label,detail:T.detail,t:T.t})}),xt){const T=this._pgEventHits.find(N=>N.t===xt.t&&N.type===xt.type);if(T){const N=this._pgEventMeta(T.type),tt=ut=>`${Math.floor(ut/60)}:${String(Math.round(ut%60)).padStart(2,"0")}`,Y=this._pgEventDescription(T.type),V=[{t:T.label,bold:!0,col:N.color},...Y?[{t:Y}]:[],...T.detail?[{t:T.detail,dim:!0}]:[],{t:`${this._t("lbl.from_start",{},"From start")} ${tt(T.t)}`,dim:!0}];i.save(),i.font=`${10*a}px sans-serif`;const lt=Math.min(220*a,r-2*$),pt=[];V.forEach(ut=>{const At=String(ut.t).split(" ");let vt="";At.forEach(Lt=>{const jt=vt?vt+" "+Lt:Lt;i.measureText(jt).width>lt&&vt?(pt.push({...ut,t:vt}),vt=Lt):vt=jt}),pt.push({...ut,t:vt})});const bt=14*a,_t=Math.min(lt+14*a,r-8*a),yt=pt.length*bt+10*a;let wt=T.cx*a-_t/2;wt=Math.max(4*a,Math.min(r-_t-4*a,wt));const It=Ct+nt+6*a;i.fillStyle=g,i.globalAlpha=.97,i.fillRect(wt,It,_t,yt),i.globalAlpha=1,i.strokeStyle=N.color,i.lineWidth=a,i.strokeRect(wt,It,_t,yt),i.textAlign="left",i.textBaseline="top",pt.forEach((ut,At)=>{i.font=`${ut.bold?"600 ":""}${10*a}px sans-serif`,i.fillStyle=ut.col||(ut.dim?p:(l.getPropertyValue("--primary-text-color")||"#ddd").trim()),i.fillText(ut.t,wt+7*a,It+5*a+At*bt)}),i.restore()}}if(this._pgHoverT!=null&&this._pgHoverT>=H&&this._pgHoverT<=q){const T=G(this._pgHoverT);i.save(),i.strokeStyle="#e34948",i.lineWidth=1.5*a,i.setLineDash([4*a,3*a]),i.beginPath(),i.moveTo(T,S),i.lineTo(T,o-v),i.stroke(),i.setLineDash([]);const N=this._pgInterpPower(s,this._pgHoverT),tt=X(N);i.fillStyle="#e34948",i.beginPath(),i.arc(T,tt,3.5*a,0,Math.PI*2),i.fill();const Y=dt=>`${Math.floor(dt/60)}:${String(Math.round(dt%60)).padStart(2,"0")}`,V=dt=>dt>=1e3?(dt/1e3).toFixed(2)+" kW":Math.round(dt)+" W",lt=[`${this._t("lbl.from_start",{},"From start")} ${Y(this._pgHoverT)}`,`${this._t("lbl.to_end",{},"To end")} ${Y(Math.max(0,D-this._pgHoverT))}`,`${this._t("lbl.power",{},"Power")} ${V(N)}`];i.font=`${9.5*a}px sans-serif`;const pt=Math.max(...lt.map(dt=>i.measureText(dt).width))+12*a,bt=lt.length*13*a+8*a;let _t=T+8*a;_t+pt>r-k&&(_t=T-pt-8*a);const yt=S+4*a;i.fillStyle=g,i.globalAlpha=.95,i.fillRect(_t,yt,pt,bt),i.globalAlpha=1,i.strokeStyle=_,i.lineWidth=a,i.strokeRect(_t,yt,pt,bt),i.fillStyle=p,i.textAlign="left",i.textBaseline="top",lt.forEach((dt,wt)=>i.fillText(dt,_t+6*a,yt+5*a+wt*13*a)),i.restore()}i.fillStyle=p,i.font=`${9*a}px sans-serif`,i.textAlign="center",i.textBaseline="top";const te=T=>`${Math.floor(T/60)}:${String(Math.round(T%60)).padStart(2,"0")}`;for(let T=0;T<=4;T++){const N=H+Q*T/4,tt=G(N);i.textAlign=T===0?"left":T===4?"right":"center",i.fillText(te(N),Math.max($,Math.min(r-k,tt)),P+u+1*a)}Q<D-1&&(i.fillStyle=p,i.font=`${8*a}px sans-serif`,i.textAlign="right",i.textBaseline="top",i.fillText(this._t("lbl.pg_zoom_hint",{},"double-click to reset zoom"),r-k,S))}_pgEventMeta(t){const s={detected:["\u25CF","#42a5f5",this._t("lbl.pg_ev_detected",{},"Detected")],match_commit:["\u25C6","#66bb6a",this._t("lbl.pg_ev_match",{},"Match committed")],match_changed:["\u25C7","#eda100",this._t("lbl.pg_ev_match_changed",{},"Match changed")],match_ambiguous:["\u25C8","#eda100",this._t("lbl.pg_ev_ambiguous",{},"Ambiguous")],unmatched:["\u25CB","#9e9e9e",this._t("lbl.pg_ev_unmatched",{},"Unmatched")],notify_start:["\u{1F514}","#2a78d6",this._t("lbl.pg_ev_notify_start",{},"Start notification")],notify_pre_complete:["\u{1F514}","#2a78d6",this._t("lbl.pg_ev_notify_pre",{},"Almost-done notification")],notify_finish:["\u{1F514}","#2a78d6",this._t("lbl.pg_ev_notify_finish",{},"Finish notification")],notify_milestone:["\u{1F3C6}","#2a78d6",this._t("lbl.pg_ev_notify_milestone",{},"Milestone notification")],notify_held:["\u{1F319}","#7e57c2",this._t("lbl.pg_ev_notify_held",{},"Notification held (quiet hours)")],finished:["\u2713","#4caf50",this._t("lbl.pg_ev_finished",{},"Finished")]}[t]||["\u2022","var(--secondary-text-color)",t];return{glyph:s[0],color:s[1],label:s[2]}}_pgEventDescription(t){return{detected:this._t("pg_evd.detected",{},"The detector recognized a cycle had started."),match_commit:this._t("pg_evd.match_commit",{},"The matcher committed to a program with enough confidence."),match_changed:this._t("pg_evd.match_changed",{},"The leading program changed as more of the cycle was seen."),match_ambiguous:this._t("pg_evd.match_ambiguous",{},"Two programs scored close together, so the match was uncertain."),unmatched:this._t("pg_evd.unmatched",{},"No saved program fit this cycle."),notify_start:this._t("pg_evd.notify_start",{},"A start notification would be sent."),notify_pre_complete:this._t("pg_evd.notify_pre_complete",{},"An almost-done notification would be sent."),notify_finish:this._t("pg_evd.notify_finish",{},"A finish notification would be sent."),notify_milestone:this._t("pg_evd.notify_milestone",{},"A milestone notification would be sent."),notify_held:this._t("pg_evd.notify_held",{},"A notification was held back for quiet hours."),finished:this._t("pg_evd.finished",{},"The cycle reached a terminal state and ended.")}[t]||""}_pgUpdateParamInput(t,e){const s=this.shadowRoot,n=s&&s.querySelector(`input[data-pgkey="${t}"]`);n&&(n.dataset.pgtype==="bool"?n.checked=!!e:n.value=typeof e=="number"?Math.round(e):e)}_pgUpdateStripAt(t){const e=this._pgPowerPts;if(!e?.length)return;const n=(this._cycles||[]).find(D=>D.id===this._pgCycleId)?._pg_duration||e[e.length-1].t||1,a=t??n,r=this._pgInterpPower(e,a),o=this._pgSeriesAt(a),i=o?this._pgMapState(o.state):"idle",l={idle:[this._t("lbl.pg_idle",{},"Idle"),"var(--secondary-background-color)"],detecting:[this._t("lbl.pg_detecting",{},"Detecting"),"#42a5f5"],running:[this._t("lbl.pg_ev_running",{},"Running"),"#66bb6a"],ending:[this._t("lbl.pg_ev_ending",{},"Ending"),"#ef5350"],anti_wrinkle:[this._t("lbl.pg_anti_wrinkle",{},"Anti-wrinkle"),"#ab47bc"]},[c,_]=l[i]||l.idle,p=o&&o.progress!=null?Math.round(o.progress):null,g=o&&o.remaining_s!=null?o.remaining_s:null,u=o&&o.energy_wh!=null?o.energy_wh:this._pgTrapEnergy(e,a),v=o&&o.confidence!=null?Math.round(o.confidence*100)+"%":"\u2014",b=o&&o.phase?o.phase:"\u2014",$=D=>Math.floor(D/60)+":"+String(Math.round(D%60)).padStart(2,"0"),k=D=>D>=1e3?(D/1e3).toFixed(2)+" kWh":D.toFixed(0)+" Wh",S=D=>D>=1e3?(D/1e3).toFixed(1)+" kW":Math.round(D)+" W",C=this.shadowRoot,R=D=>C&&C.getElementById(D),B=(D,A)=>{const E=R(D);E&&(E.textContent=A)},W=(D,A,E)=>{const M=R(D);M&&(M.style[A]=E)},O=R("wd-pg-state-badge");O&&(O.textContent=c,O.style.background=_,O.style.color=_.includes("var")?"":"#fff"),B("wd-pg-power",S(r)),B("wd-pg-pct",p!=null?p+"%":"\u2014%"),W("wd-pg-pbar","width",(p??0)+"%"),B("wd-pg-rem",g!=null?$(g):"\u2014"),B("wd-pg-energy",k(u)),B("wd-pg-conf",v),B("wd-pg-phase",b)}_pgIsUnknownCmd(t){const e=t&&(t.code||t.error&&t.error.code)||"",s=(t&&(t.message||t.error)||"").toString().toLowerCase();return e==="unknown_command"||s.includes("unknown command")||s.includes("unknown_command")}_pgInterpPower(t,e){if(!t.length)return 0;if(e<=t[0].t)return t[0].w;for(let s=1;s<t.length;s++)if(e<=t[s].t){const n=t[s].t-t[s-1].t;if(n<=0)return t[s].w;const a=(e-t[s-1].t)/n;return t[s-1].w+a*(t[s].w-t[s-1].w)}return t[t.length-1].w}_pgTrapEnergy(t,e){let s=0;for(let n=1;n<t.length;n++){const a=t[n-1].t,r=t[n-1].w,o=Math.min(t[n].t,e),i=this._pgInterpPower(t,o);if(!(o<=a)&&(s+=(r+i)/2*(o-a)/3600,o>=e))break}return s}_drawPlaygroundCanvases(){this._tab==="playground"&&this._pgDrawCanvas()}_htmlPhases(){const t=this._devices[this._selIdx],e=t&&t.options.device_type||"washing_machine",s=this._canEdit(),n=this._phases.map(o=>{const i=o.is_default,l=o.translation_key?this._t(o.translation_key,{},o.description||""):o.description||"",c=s?`
        <td>
        ...
        ${c}
      </tr>`}).join(""),a=s?`<th>${this._t("lbl.actions",{},"Actions")}</th>`:"",r=s?`<div class="wd-card-actions" style="margin-bottom:14px"><button class="wd-btn wd-btn-primary" data-action="create-phase" data-dtype="${d(e)}">${this._t("btn.new_phase",{},"+ New Phase")}</button></div>`:"";return`
      <div class="wd-card">
        ...
        ${r}
        ${this._phases.length===0?`<p class="wd-info">${this._t("msg.no_phases",{},"No phases defined.")}</p>`:`<table class="wd-table"><thead><tr><th>${this._t("lbl.phase_name",{},"Name")}</th><th>${this._t("lbl.description",{},"Description")}</th>${a}</tr></thead><tbody>${n}</tbody></table>`}
      ...
      <div class="wd-form-grid">
        ${s.map(g=>`<div class="wd-field"><label>${d(this._maintLabel(g))}</label><input type="number" min="0" step="1" data-maint-rem="${d(g)}" value="${r[g]!=null?d(r[g]):""}" placeholder="${d(this._t("lbl.reminder_every",{},"Remind every (cycles)"))}"></div>`).join("")}
      </div>
      ...
      ${p}`}_htmlPanel(){const t=this._canEdit(),e=t&&this._constants&&this._constants.mlTrainingAvailable,s=new Set(["maintenance"]);t&&s.add("diagnostics"),e&&s.add("ml");let n=this._panelSubtab;s.has(n)||(n=this._panelSubtab="maintenance");const a=[["maintenance",this._t("tab.maintenance",{},"Maintenance")]];t&&a.push(["diagnostics",this._t("tab.diagnostics",{},"Diagnostics")]),e&&a.push(["ml",this._t("tab.ml",{},"ML Training")]);const r=a.map(([i,l])=>`<button class="wd-subtab ${n===i?"active":""}" data-ptab="${i}">${l}</button>`).join(""),o=n==="diagnostics"&&t?this._htmlDiagnostics():n==="ml"&&e?this._htmlMlTab():this._htmlMaintenance();return`<div class="wd-subtabs">${r}</div>${o}`}_levelSelect(t,e,s){const n=(s?[["inherit",this._t("access.inherit",{},"Inherit")]]:[]).concat([["none",this._t("access.none",{},"None (hidden)")],["read",this._t("access.read",{},"Read")],["edit",this._t("access.edit",{},"Edit")],["full",this._t("access.full",{},"Full")]]);return`<select ${t}>${n.map(([a,r])=>`<option value="${a}" ${e===a?"selected":""}>${r}</option>`).join("")}</select>`}_htmlPanelPrefs(){const t=this._panelCfg&&this._panelCfg.prefs||{},e=this._hass&&this._hass.locale&&this._hass.locale.language||"en",n=[["",this._t("pref.use_panel_default",{},"(panel default)")],["status",this._t("tab.status",{},"Overview")],["history",this._t("tab.history",{},"Cycles")],["profiles",this._t("tab.profiles",{},"Profiles")],["settings",this._t("tab.settings",{},"Settings")],["playground",this._t("tab.playground",{},"Playground")]].map(([l,c])=>`<option value="${l}" ${(t.default_tab||"")===l?"selected":""}>${d(c)}</option>`).join(""),r=[["relative",this._t("pref.date_relative",{},"Relative (e.g. 2 hours ago)")],["absolute",this._t("pref.date_absolute",{},"Absolute (e.g. 14:32 on 2 Jul)")]].map(([l,c])=>`<option value="${l}" ${(t.date_format||"relative")===l?"selected":""}>${d(c)}</option>`).join(""),o=t.lang_override||"",i=[["",this._t("pref.lang_auto",{lang:e.toUpperCase()},"System default ("+e.toUpperCase()+")")],["en",this._t("pref.lang_en",{},"English")]].map(([l,c])=>`<option value="${l}" ${o===l?"selected":""}>${d(c)}</option>`).join("");return`<div class="wd-card">
      ...
      <div class="wd-form-grid">
        ...
        <div class="wd-field"><label>${this._t("lbl.cycle_date_display",{},"Cycle date display")}</label><select id="wd-pref-datefmt">${r}</select></div>
    ...
    </div>`}_htmlPanelSettings(){const t=this._panelCfg&&this._panelCfg.panel||{},e=[["status",this._t("tab.status",{},"Overview")],["history",this._t("tab.history",{},"Cycles")],["profiles",this._t("tab.profiles",{},"Profiles")],["settings",this._t("tab.settings",{},"Settings")],["playground",this._t("tab.playground",{},"Playground")]],s=e.map(([r,o])=>`<option value="${r}" ${(t.default_tab||"status")===r?"selected":""}>${d(o)}</option>`).join(""),n=t.hidden_tabs||[],a=e.filter(([r])=>r!=="status").map(([r,o])=>Ot(`data-hidetab="${r}" ${n.includes(r)?"checked":""}`,o)).join("");return`<div class="wd-card">
      ...
      <div class="wd-field"><label>${this._t("lbl.hide_tabs",{},"Hide tabs for non-admins")}</label><div style="display:flex;flex-wrap:wrap;gap:10px 22px;margin-top:6px">${a}</div></div>
    ...
    </div>`}_htmlPanelAccess(){const t=this._panelCfg&&this._panelCfg.rbac||{enabled:!1,default_level:"none",users:{}},e=this._panelCfg&&this._panelCfg.users||[],s=this._devices||[],n=e.filter(r=>!r.is_admin).map(r=>{const o=(t.users||{})[r.id]||{default:"none",devices:{}},i=s.map(l=>`<div class="wd-seg-row"><span style="min-width:160px">${d(l.title)}</span>${this._levelSelect(`data-rbacuser="${d(r.id)}" data-rbacdev="${d(l.entry_id)}"`,(o.devices||{})[l.entry_id]||"inherit",!0)}</div>`).join("");return`<div class="wd-card" style="background:var(--secondary-background-color)">
        <div class="wd-profile-name">${d(r.name)}</div>
        <div class="wd-seg-row"><span style="min-width:160px">${this._t("lbl.default_other",{},"Default (other devices)")}</span>${this._levelSelect(`data-rbacuser="${d(r.id)}" data-rbacdev="__default__"`,o.default||"none",!1)}</div>
        ${i}
      </div>`}).join(""),a=e.filter(r=>r.is_admin).map(r=>`<span class="wd-pill">${d(r.name)} - full (admin)</span>`).join(" ");return`<div class="wd-card">
      ...
      ${a?`<div class="wd-field"><label>${this._t("lbl.administrators",{},"Administrators")}</label><div>${a}</div></div>`:""}
    ...
    ${n||`<div class="wd-card"><p class="wd-info">${this._t("msg.no_other_users",{},"No other Home Assistant users found.")}</p></div>`}`}_htmlStore(){if(!this._canEdit())return`<div class="wd-empty">${this._t("msg.no_device_selected",{},"No device selected.")}</div>`;const t=this._storeStatus,e=`<a href="https://3dg1luk43.github.io/washdata-store" target="_blank" rel="noopener noreferrer" style="font-size:.8em;font-weight:400;color:var(--primary-color);text-decoration:none;white-space:nowrap" title="${d(this._t("store.website_tip",{},"Open the community store website"))}">${this._t("store.website",{},"Store website \u2197")}</a>`,s=`<div style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap"><div class="wd-card-title" style="margin:0">${this._t("hdr.community_store",{},"Community Store")}</div>${e}</div>`;if(!this._onlineEnabled()||t&&t.enabled===!1)return`<div class="wd-card">${s}
      ...
      ${n}
        ...
        <button class="wd-btn wd-btn-primary wd-btn-sm" data-action="store-goto-identity">${this._t("btn.set_brand_model",{},"Set brand & model")}</button>`;const n=`<p class="wd-info">${this._t("store.no_results",{},"No matching appliances found. Try a different search.")}</p>`,a=this._storeLoading?this._htmlStoreLoading():t.length?`<div class="wd-store-rows">${s}</div>`:n,r=!this._storeLoading&&t.length>1?`<p class="wd-info" style="margin-bottom:8px">${this._t("msg.store_sibling_hint",{},"Nothing shared for your exact model? A closely-related model from the same brand is usually a good starting point.")}</p>`:"";return`
      <div class="wd-store-search">
      ...
      ${r}
      ${a}`}_htmlStoreDevice(){const t=this._storeProfiles||[],e=t.map(o=>`<button class="wd-store-row" data-action="store-open-profile" data-profile-id="${d(o.id)}">
      <span class="wd-store-row-main"><span class="wd-store-row-title">${d(o.program||"")}${this._statusTag(o)}</span></span>
      <span class="wd-store-row-arrow" aria-hidden="true">\u203A</span>
    </button>`).join(""),s=this._storeLoading?this._htmlStoreLoading():t.length?`<div class="wd-store-rows">${e}</div>`:`<p class="wd-info">${this._t("store.no_programs",{},"No shared programs for this appliance yet.")}</p>`,n=this._storeDevice,a=this._busy.has("store-download-device");return(this._canEdit()&&n&&t.length?`<div class="wd-card" style="margin-bottom:10px;display:flex;align-items:center;gap:12px;flex-wrap:wrap">
      ...
      ${Ot(`data-action="store-toggle-dl-settings" ${this._dlSettings?"checked":""} ${a?"disabled":""}`,this._t("lbl.adopt_settings",{},"Also adopt settings"),it(this._t("msg.adopt_settings_hint",{},"Overwrite this device's detection & matching thresholds with the shared ones. Your notifications, entities and energy price are never changed.")))}
      <button class="wd-btn wd-btn-primary wd-btn-sm" data-action="store-download-device" data-device-id="${d(n.id)}" ${a?"disabled":""}>${a?'<span class="wd-spin"></span> ':"\u2B07 "}${this._t("btn.download_device",{},"Download this setup")}</button>
    </div>`:"")+s}_htmlStoreProfile(){const t=this._storeCycles||[],e=t.map(n=>{const a=n.stats||{},r=this._storeSparkline(n.trace&&n.trace.points||[]),o=a.duration!=null?ot(a.duration):"-",i=a.energy_wh!=null?gt(a.energy_wh/1e3):"-",l=a.peak_w!=null?Vt(a.peak_w):"-",c=d(n.uploaderName||this._t("store.anon",{},"anonymous")),_=this._statusTag(n),p=n.rating||{},g=p.avg!=null&&p.count?this._t("store.rating_summary",{avg:Number(p.avg).toFixed(1),n:p.count},`\u2605 ${Number(p.avg).toFixed(1)} (${p.count})`):this._t("store.no_ratings",{},"No ratings yet");return`<div class="wd-card">
      ...
      ${t?a:""}
    </div>`}_htmlStorePrefs(t){const e=this._constants&&this._constants.storePrefs||{},s=ie.map(a=>{const r=e[a.key]!==!1;return Et(`data-action="store-toggle-pref" data-pref="${d(a.key)}" ${r?"checked":""} ${t?"disabled":""}`,this._t(a.labelKey,{},a.labelFb),it(this._t(a.docKey,{},a.docFb)))}).join(""),n=this._busy.has("store-refresh-catalog");return s+`
      ...
      <div class="wd-log-drawer-body">
    ...
    </div>`}_drawCurves(t,e){const s=this.shadowRoot&&this.shadowRoot.getElementById(t);if(!s)return null;const n=s.getBoundingClientRect(),a=window.devicePixelRatio||1,r=Math.max(1,Math.round(n.width*a)),o=Math.max(1,Math.round((n.height||240)*a));(s.width!==r||s.height!==o)&&(s.width=r,s.height=o);const i=s.getContext("2d"),l=getComputedStyle(this),c=(l.getPropertyValue("--primary-color")||"#03a9f4").trim()||"#03a9f4",_=(l.getPropertyValue("--divider-color")||"rgba(127,127,127,.3)").trim()||"rgba(127,127,127,.3)",p=(l.getPropertyValue("--secondary-text-color")||"#888").trim()||"#888",g=44*a,u=12*a,v=12*a,b=22*a,$=e.series||[];let k=e.xMax||0;k||($.forEach(A=>(A.points||[]).forEach(E=>{E[0]>k&&(k=E[0])})),e.band&&(e.band.max||[]).forEach(A=>{A[0]>k&&(k=A[0])})),k=k||1;let S=e.yMax||0;if(!S){const A=E=>(E||[]).forEach(M=>{M[1]>S&&(S=M[1])});$.forEach(E=>{E.noScale||A(E.points)}),e.band&&A(e.band.max),S=(S||10)*1.08}const C=this._canvasZoom&&this._canvasZoom[t],R=C?C.xMin:0,B=C?C.xMax:k,W=r-g-u,O=A=>g+(A-R)/(B-R)*W,D=A=>o-b-A/S*(o-v-b);i.clearRect(0,0,r,o),i.strokeStyle=_,i.lineWidth=a,i.fillStyle=p,i.font=`${11*a}px sans-serif`,i.textAlign="right",i.textBaseline="middle";for(let A=0;A<=2;A++){const E=v+A/2*(o-v-b);i.beginPath(),i.moveTo(g,E),i.lineTo(r-u,E),i.stroke(),i.fillText(Math.round(S*(1-A/2))+"W",g-4*a,E)}if(i.save(),i.beginPath(),i.rect(g,v,W,o-v),i.clip(),(e.bands||[]).forEach(A=>{i.fillStyle=A.fill;const E=O(A.x0),M=O(A.x1);i.fillRect(Math.min(E,M),v,Math.abs(M-E),o-v-b)}),e.band&&(e.band.min||[]).length&&(e.band.max||[]).length){i.beginPath(),e.band.max.forEach((A,E)=>E?i.lineTo(O(A[0]),D(A[1])):i.moveTo(O(A[0]),D(A[1])));for(let A=e.band.min.length-1;A>=0;A--)i.lineTo(O(e.band.min[A][0]),D(e.band.min[A][1]));i.closePath(),i.fillStyle=e.band.fill||St(c,.13),i.fill()}return $.forEach(A=>{const E=A.points||[];if(!E.length)return;const M=A.stroke==="primary"?c:A.stroke;if(A.fill){i.beginPath(),E.forEach((q,Q)=>Q?i.lineTo(O(q[0]),D(q[1])):i.moveTo(O(q[0]),D(q[1]))),i.lineTo(O(E[E.length-1][0]),D(0)),i.lineTo(O(E[0][0]),D(0)),i.closePath();const H=i.createLinearGradient(0,v,0,o-b);H.addColorStop(0,St(M,.33)),H.addColorStop(1,St(M,.03)),i.fillStyle=H,i.fill()}i.beginPath(),E.forEach((H,q)=>q?i.lineTo(O(H[0]),D(H[1])):i.moveTo(O(H[0]),D(H[1]))),i.strokeStyle=M,i.lineWidth=(A.width||1.5)*a,i.lineJoin="round",A.dash&&i.setLineDash([6*a,4*a]),i.globalAlpha=A.alpha!=null?A.alpha:1,i.stroke(),i.globalAlpha=1,A.dash&&i.setLineDash([])}),i.textAlign="center",i.textBaseline="top",(e.vlines||[]).forEach(A=>{const E=O(A.x);i.beginPath(),i.moveTo(E,v),i.lineTo(E,o-b),i.strokeStyle=A.color,i.lineWidth=2*a,i.setLineDash([4*a,3*a]),i.stroke(),i.setLineDash([]),A.handle&&(i.fillStyle=A.color,i.beginPath(),i.arc(E,v+4*a,4.5*a,0,Math.PI*2),i.fill()),A.label&&(i.fillStyle=A.color,i.fillText(A.label,E,v+(A.handle?12*a:2*a)))}),i.restore(),i.fillStyle=p,i.font=`${11*a}px sans-serif`,i.textBaseline="bottom",C&&(i.textAlign="left",i.fillText((R/60).toFixed(1)+" min",g,o-2*a)),i.textAlign="right",i.fillText((B/60).toFixed(0)+" min",r-u,o-2*a),s._wd={xMax:k,xMin:R,xViewMax:B,yMax:S,dpr:a,padT:v,padB:b,ch:o,primary:c,Xpx:O,Ypx:D,xToCss:A=>O(A)/a,cssToX:A=>Math.max(R,Math.min(B,R+(A*a-g)/W*(B-R))),series:(e.series||[]).map(A=>({points:A.points,stroke:A.stroke,name:A.name,cid:A.cid})),band:e.band||null,artifacts:e.artifacts||null,_opts:e},s._wd}_drawModalCanvas(){const t=this._modal;t&&(t.type==="cycle-detail"?this._drawCycleEditor():t.type==="compare-cycles"?this._drawCompareCanvas():t.type==="profile-group"?this._drawGroupCanvas():t.type==="profile-panel"&&(t.tab==="stats"?this._drawProfileEnvelope():t.tab==="phases"?this._drawPhaseEditor():t.tab==="cleanup"&&this._drawSpaghetti()))}_redrawCanvas(t){if(t==="wd-status-canvas")this._drawStatusCurve();else if(t==="wd-cyc-canvas")this._drawCycleEditor();else if(t==="wd-compare-canvas")this._drawCompareCanvas();else if(t==="wd-env-canvas")this._drawProfileEnvelope();else if(t==="wd-phase-canvas")this._drawPhaseEditor();else if(t==="wd-spag-canvas")this._drawSpaghetti();else if(t==="wd-pgroup-canvas")this._drawGroupCanvas();else{const e=this.shadowRoot&&this.shadowRoot.getElementById(t);e&&e._wd&&e._wd._opts&&this._drawCurves(t,e._wd._opts)}}_pref(t,e){const s=this._panelCfg&&this._panelCfg.prefs||{};return s[t]===void 0?e:s[t]}_setPref(t,e){this._panelCfg||(this._panelCfg={}),this._panelCfg.prefs={...this._panelCfg.prefs||{},[t]:e},this._ws({type:`${y}/set_user_prefs`,prefs:{[t]:e}}).catch(()=>{})}_drawStatusCurve(){const t=this._powerData||{},e=t.live||[];if(e.length<2)return;const s=this._statusEnv,n=this._pref("show_expected",!0),a=this._pref("show_raw_active",!1),r=[];let o=e[e.length-1][0];if(t.cycle_active&&s&&(s.avg||[]).length&&n){const l=s.target_duration||s.avg[s.avg.length-1][0];r.push({points:s.avg,stroke:"#ff9800",width:2,alpha:.4,name:this._t("lbl.expected",{},"Expected")}),o=Math.max(o,l)}r.push({points:e,stroke:"primary",fill:!0,width:2,name:this._t("lbl.power",{},"Power")}),a&&(t.raw||[]).length>1&&r.push({points:t.raw,stroke:"#9e9e9e",width:1,alpha:.65,name:this._t("lbl.raw_socket",{},"Raw socket"),noScale:!0});const i=[];(t.restart_gaps||[]).forEach(l=>{if(!e.length)return;const c=t.cycle_start_iso;if(!c)return;const _=new Date(c).getTime(),p=Math.max(0,(new Date(l.start_ts).getTime()-_)/1e3),g=Math.max(p+1,(new Date(l.end_ts).getTime()-_)/1e3);i.push({x0:p,x1:g,fill:"rgba(96,125,139,.20)"})}),this._drawCurves("wd-status-canvas",{series:r,xMax:o,bands:i})}_attachHover(t){const e=this.shadowRoot,s=e&&e.getElementById(t);s&&(s.addEventListener("pointermove",n=>this._onGraphHover(n,t)),s.addEventListener("pointerleave",()=>this._hideGraphTip()),s.addEventListener("wheel",n=>{const a=s._wd;if(!a)return;n.preventDefault();const r=s.getBoundingClientRect(),o=a.cssToX(n.clientX-r.left),i=this._canvasZoom[t],l=a.xMax,c=i?i.xMin:0,p=(i?i.xMax:l)-c,g=p*(n.deltaY>0?1.3:.75);if(g>=l*.99)delete this._canvasZoom[t];else{const u=(o-c)/p,v=Math.min(l,o-u*g+g),b=Math.max(0,v-g);this._canvasZoom[t]={xMin:b,xMax:Math.min(l,b+g)}}this._redrawCanvas(t)},{passive:!1}),s.addEventListener("dblclick",()=>{delete this._canvasZoom[t],this._redrawCanvas(t)}))}_onGraphHover(t,e){const s=t.clientX,n=t.clientY;if(this._hoverRafId){this._hoverPending={px:s,py:n,id:e};return}this._hoverPending={px:s,py:n,id:e},this._hoverRafId=requestAnimationFrame(()=>{this._hoverRafId=null;const{px:a,py:r,id:o}=this._hoverPending||{};o&&this._onGraphHoverInner(a,r,o)})}_onGraphHoverInner(t,e,s){const n=this.shadowRoot&&this.shadowRoot.getElementById(s),a=n&&n._wd;if(!a)return;const r=n.getBoundingClientRect(),o=a.cssToX(t-r.left),i=(e-r.top)*a.dpr;this._redrawCanvas(s);const l=n.getContext("2d"),c=a.Xpx(o);l.save(),l.strokeStyle="rgba(140,140,140,.75)",l.lineWidth=a.dpr,l.setLineDash([3*a.dpr,3*a.dpr]),l.beginPath(),l.moveTo(c,a.padT),l.lineTo(c,a.ch-a.padB),l.stroke(),l.setLineDash([]);const _=v=>v.stroke==="primary"?a.primary:v.stroke,p=(v,b)=>{l.fillStyle=b,l.beginPath(),l.arc(c,a.Ypx(v),3.4*a.dpr,0,6.2832),l.fill()},g=[`${this._t("lbl.from_start",{},"From start")}: <b>${Mt(o)}</b>`,`${this._t("lbl.to_end",{},"To end")}: <b>${Mt(Math.max(0,a.xMax-o))}</b>`],u=a.series||[];if(this._hoverNearest=null,u.length>4){let v=null,b=1/0;if(u.forEach($=>{const k=Dt($.points,o);if(k==null)return;const S=Math.abs(a.Ypx(k)-i);S<b&&(b=S,v={s:$,v:k})}),v){const $=_(v.s);l.strokeStyle=$,l.lineWidth=3*a.dpr,l.beginPath(),(v.s.points||[]).forEach((k,S)=>S?l.lineTo(a.Xpx(k[0]),a.Ypx(k[1])):l.moveTo(a.Xpx(k[0]),a.Ypx(k[1]))),l.stroke(),p(v.v,$),g.push(`${d(v.s.name||"")}: <b>${v.v.toFixed(v.v<100?1:0)} W</b>`),v.s.cid&&(g.push(`<span style="opacity:.7">${this._t("lbl.click_to_select",{},"click to select")}</span>`),this._hoverNearest={id:s,cid:v.s.cid})}}else u.forEach(v=>{const b=Dt(v.points,o);b!=null&&(p(b,_(v)),g.push(`${d(v.name||this._t("lbl.power",{},"Power"))}: <b>${b.toFixed(b<100?1:0)} W</b>`))});if(a.band){const v=Dt(a.band.min,o),b=Dt(a.band.max,o);v!=null&&b!=null&&g.push(`${this._t("lbl.envelope",{},"Envelope")}: ${v.toFixed(0)}\u2013${b.toFixed(0)} W`)}(a.artifacts||[]).forEach(v=>{if(o>=v.start_s&&o<=v.end_s){const b=v.detail_key?this._t(v.detail_key,v.detail_params||{},v.detail||""):v.detail||"";g.push(`<span style="color:var(--warning-color,#ff9800)">\u26A0 ${d(Yt(v.type,($,k,S)=>this._t($,k,S)))}</span>: ${d(b)}`)}}),this._canvasZoom[s]&&g.push(`<span style="opacity:.45">${this._t("lbl.zoom_hint",{},"scroll to zoom \xB7 dblclick to reset")}</span>`),l.restore(),this._showGraphTip(t,e,g),this._syncSpagRowHighlight(this._hoverNearest?this._hoverNearest.cid:null)}_showGraphTip(t,e,s){const n=this._gtip;if(!n)return;n.innerHTML=s.join("<br>"),n.style.display="block";const a=n.offsetWidth,r=n.offsetHeight,o=16;let i=t+o,l=e+o;i+a>window.innerWidth-6&&(i=t-a-o),l+r>window.innerHeight-6&&(l=e-r-o),n.style.left=Math.max(6,i)+"px",n.style.top=Math.max(6,l)+"px"}_hideGraphTip(){this._hoverRafId&&(cancelAnimationFrame(this._hoverRafId),this._hoverRafId=null,this._hoverPending=null),this._gtip&&(this._gtip.style.display="none"),this._syncSpagRowHighlight(null)}_positionTip(t){const e=t.querySelector(".wd-tip-pop");if(!e)return;e.style.transform="";const s=!e.offsetWidth,n=s?e.style.cssText:"";s&&(e.style.visibility="hidden",e.style.display="block");const a=e.getBoundingClientRect();if(s&&(e.style.cssText=n),!a.width)return;const r=de(t),o=6,i=r.left+o,l=r.right-o-a.width,c=l<i?i:Math.min(Math.max(a.left,i),l),_=Math.round(c-a.left);_&&(e.style.transform=`translateX(calc(-50% + ${_}px))`)}_syncSpagRowHighlight(t){if(t===this._spagHoverCid)return;this._spagHoverCid=t;const e=this.shadowRoot;e&&e.querySelectorAll("tr[data-cid]").forEach(s=>{s.style.backgroundColor=t&&s.dataset.cid===t?"var(--secondary-background-color,rgba(0,0,0,.06))":""})}_showToast(t,e="success",s={}){this._toastTimer&&clearTimeout(this._toastTimer);const n=s.duration||3500;this._toast={msg:t,cls:`wd-toast-${e}`,actionLabel:s.actionLabel||null,actionToken:s.actionToken||null},this._render(),this._toastTimer=setTimeout(()=>{this._toast=null,this._render()},n)}_profileOptions(t){return(this._profiles||[]).map(e=>`<option value="${d(e.name)}" ${String(t)===String(e.name)?"selected":""}>${d(e.name)}</option>`).join("")}_htmlModal(){const t=this._modal;if(t.type==="cycle-detail")return`<div class="wd-overlay"><div class="wd-modal wd-modal-lg" role="dialog" aria-modal="true" aria-labelledby="wd-modal-title" tabindex="-1">${this._htmlCycleModal(t)}</div></div>`;if(t.type==="profile-panel")return`<div class="wd-overlay"><div class="wd-modal wd-modal-lg" role="dialog" aria-modal="true" aria-labelledby="wd-modal-title" tabindex="-1">${this._htmlProfilePanel(t)}</div></div>`;if(t.type==="profile-group")return`<div class="wd-overlay"><div class="wd-modal wd-modal-lg" role="dialog" aria-modal="true" aria-labelledby="wd-modal-title" tabindex="-1">${this._htmlProfileGroupModal(t)}</div></div>`;if(t.type==="compare-cycles")return`<div class="wd-overlay"><div class="wd-modal wd-modal-lg" role="dialog" aria-modal="true" aria-labelledby="wd-modal-title" tabindex="-1">${this._htmlCompareModal(t)}</div></div>`;if(t.type==="gear-settings")return`<div class="wd-overlay"><div class="wd-modal wd-modal-lg" role="dialog" aria-modal="true" aria-labelledby="wd-modal-title" tabindex="-1">${this._htmlGearModal(t)}</div></div>`;if(t.type==="export-select")return`<div class="wd-overlay"><div class="wd-modal wd-modal-lg" role="dialog" aria-modal="true" aria-labelledby="wd-modal-title" tabindex="-1">${this._htmlExportSelectModal(t)}</div></div>`;if(t.type==="import-wizard")return`<div class="wd-overlay"><div class="wd-modal wd-modal-lg" role="dialog" aria-modal="true" aria-labelledby="wd-modal-title" tabindex="-1">${this._htmlImportWizardModal(t)}</div></div>`;if(t.type==="history-import")return`<div class="wd-overlay"><div class="wd-modal wd-modal-lg" role="dialog" aria-modal="true" aria-labelledby="wd-modal-title" tabindex="-1">${this._htmlHistoryImportModal(t)}</div></div>`;let e="";if(t.type==="confirm")e=`<h2>${d(t.title)}</h2><p class="wd-info">${d(t.message)}</p>
        ...
        <button class="wd-btn wd-btn-primary" data-maction="store-share-ok" ${this._busy.has("store-share")?"disabled":""}>${this._busy.has("store-share")?'<span class="wd-spin"></span> ':""}${this._t("btn.share",{},"Share")}</button></div>`}else t.type==="store-share-device"&&(e=this._htmlShareDeviceModal(t));return`<div class="wd-overlay"><div class="wd-modal" role="dialog" aria-modal="true" aria-labelledby="wd-modal-title" tabindex="-1">${e}</div></div>`}_htmlShareDeviceModal(t){const e=this._shareableByProgram(),s=this._busy.has("store-share-device"),n=t.selected||new Set,a=!!t.consented,r=e.reduce((u,v)=>u+v.cycles.filter(b=>n.has(b.id)).length,0),o=e.some(u=>!u.noCycles);let i;t.loading?i=`<div class="wd-empty" style="padding:24px"><div class="wd-icon">\u23F3</div>${this._t("msg.loading",{},"Loading\u2026")}</div>`:e.length?i=e.map(u=>{if(u.noCycles)return`<div class="wd-sd-group wd-sd-group-nocyc">
          ...
          </div>`;const v=u.cycles.every(C=>n.has(C.id)),b=!v&&u.cycles.some(C=>n.has(C.id)),$=u.cycles.map(C=>{const R=C.start_time?st(C.start_time):"",B=C.duration!=null?ot(C.duration):"";return`<label class="wd-sd-cyc">
            <input type="checkbox" data-maction="sd-toggle-cyc" data-cid="${d(C.id)}" ${n.has(C.id)?"checked":""} ${s?"disabled":""}>
          ...
          <label class="wd-sd-prof">
            ...
            <span class="wd-sd-prof-name">${d(u.program)}</span>
            <span class="wd-sd-count">${u.cycles.filter(C=>n.has(C.id)).length}/${u.cycles.length}</span>
          </label>
      ...
      <input type="checkbox" data-maction="sd-toggle-consent" ${a?"checked":""} ${s?"disabled":""}>
      ...
      <div class="wd-modal-actions">
        ...
        <button class="wd-btn wd-btn-primary" data-maction="store-share-device-ok" ${s||!r||!a?"disabled":""}>${s?'<span class="wd-spin"></span> ':""}${this._t("btn.share_n",{n:r},`Share ${r} cycle(s)`)}</button>
      </div>`}_wizCatOrder(){return["profiles","real_cycles","reference_cycles","custom_phases","profile_groups","settings","matching_config","ml_models","feedback","suggestions","maintenance_log","history_logs","lifetime_stats"]}_wizCatLabel(t){return{profiles:this._t("lbl.cat_profiles",{},"Profiles (programs)"),real_cycles:this._t("lbl.cat_real_cycles",{},"Cycles (run history)"),reference_cycles:this._t("lbl.cat_reference_cycles",{},"Reference cycles (imported)"),custom_phases:this._t("lbl.cat_custom_phases",{},"Custom phases"),profile_groups:this._t("lbl.cat_profile_groups",{},"Profile groups"),settings:this._t("lbl.cat_settings",{},"Detection & matching settings"),matching_config:this._t("lbl.cat_matching_config",{},"Matcher tuning"),ml_models:this._t("lbl.cat_ml_models",{},"ML models"),feedback:this._t("lbl.cat_feedback",{},"Feedback & review labels"),suggestions:this._t("lbl.cat_suggestions",{},"Suggestions"),maintenance_log:this._t("lbl.cat_maintenance_log",{},"Maintenance log"),history_logs:this._t("lbl.cat_history_logs",{},"History & change logs"),lifetime_stats:this._t("lbl.cat_lifetime_stats",{},"Lifetime totals")}[t]||t}_wizInitSel(t,e){const s=new Set,n=new Set,a=new Set,r=new Set,o=t&&t.categories||{},i=l=>!!(l&&l.present&&(!e||l.importable!==!1));return i(o.profiles)&&(o.profiles.items||[]).forEach(l=>n.add(l.name)),i(o.real_cycles)&&(o.real_cycles.groups||[]).forEach(l=>l.cycles.forEach(c=>{c.id!=null&&a.add(String(c.id))})),i(o.reference_cycles)&&(o.reference_cycles.groups||[]).forEach(l=>l.cycles.forEach(c=>{c.id!=null&&r.add(String(c.id))})),Object.keys(o).forEach(l=>{["profiles","real_cycles","reference_cycles"].includes(l)||i(o[l])&&s.add(l)}),{cats:s,profiles:n,realIds:a,refIds:r}}_wizSelectionPayload(t){const e=t.sel,s=[];e.profiles.size&&s.push("profiles"),e.realIds.size&&s.push("real_cycles"),e.refIds.size&&s.push("reference_cycles"),e.cats.forEach(a=>s.push(a));const n={categories:s};return e.profiles.size&&(n.profiles=Array.from(e.profiles)),e.realIds.size&&(n.real_cycle_ids=Array.from(e.realIds)),e.refIds.size&&(n.reference_cycle_ids=Array.from(e.refIds)),n}_wizGroupIds(t,e,s){const a=(((t.categories||{})[e]||{}).groups||[]).find(r=>r.profile===s);return a?a.cycles.filter(r=>r.id!=null).map(r=>String(r.id)):[]}_wizCatState(t,e,s){const n=t.sel,a=(s.categories||{})[e]||{};if(e==="profiles"){const o=a.items||[],i=o.length,l=o.filter(c=>n.profiles.has(c.name)).length;return{sel:l,total:i,state:l===0?"none":l===i?"all":"some"}}if(e==="real_cycles"||e==="reference_cycles"){const o=e==="real_cycles"?n.realIds:n.refIds,i=a.count||0;let l=0;return(a.groups||[]).forEach(c=>c.cycles.forEach(_=>{o.has(String(_.id))&&l++})),{sel:l,total:i,state:l===0?"none":l===i?"all":"some"}}const r=n.cats.has(e);return{sel:r?1:0,total:1,state:r?"all":"none"}}_htmlSelectionTree(t,e,s={}){const n=e&&e.categories||{},a=new Set;s.conflicts&&(n.profiles&&n.profiles.items||[]).forEach(_=>{_.conflict&&a.add(_.name)});const r=this._wizCatOrder().filter(_=>n[_]&&n[_].present);let o=!1,i=!0;r.forEach(_=>{if(s.importableOnly&&n[_].importable===!1)return;const p=this._wizCatState(t,_,e);p.state!=="none"&&(o=!0),p.state!=="all"&&(i=!1)});const l=`<label class="wd-sd-prof" style="border-bottom:1px solid var(--divider-color, #444)">
      ...
      <span class="wd-sd-prof-name"><strong>${this._t("btn.select_all",{},"Select all")}</strong></span>
    </label>`,c=r.map(_=>{const p=n[_],g=s.importableOnly&&p.importable===!1,u=this._wizCatState(t,_,e),b=["profiles","real_cycles","reference_cycles"].includes(_)?`${u.sel}/${p.count}`:"",$=`<label class="wd-sd-prof">
        ...
        <span class="wd-sd-count">${b}</span>
          ...
          <span class="wd-sd-cyc-meta">${d(S.name)} \xB7 ${(S.real_cycles||0)+(S.reference_cycles||0)} ${this._t("lbl.cycles_short",{},"cycles")}${a.has(S.name)?` <span class="wd-tag" style="color:var(--warning-color,#e6a700)">${this._t("badge.exists",{},"exists")}</span>`:""}</span>
      ...
      <div class="wd-modal-actions">
        ...
        </div>`}const n=t.manifest||{},a=n.device_type_match===!1,r=d(n.source_device_type||"?"),o=d(n.local_device_type||"?"),i=a?`<div class="wd-banner wd-banner-warn" style="margin-bottom:12px;padding:8px 12px;border-radius:8px;background:var(--warning-color,#e6a700);color:#111">
      ${this._t("msg.device_type_mismatch_warn",{src:r,local:o},"This export is from a different appliance type ("+r+" vs "+o+"). Programs and cycles can still be imported as reference data, but device-specific settings and real-history import are disabled.")}
    ...
    </div>`;let g="";const v=(n.categories&&n.categories.profiles&&n.categories.profiles.items||[]).filter($=>$.conflict&&t.sel.profiles.has($.name));t.mode==="merge"&&v.length&&(g=`<div class="wd-field"><label>${this._t("lbl.conflict_resolution",{},"Name clashes")}</label>
      ...
      <div class="wd-modal-actions">
        ...
        <div class="wd-field">
          ...
          <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
            <input type="number" id="wd-hist-days" min="1" max="14" step="1" value="${d(String(t.days||10))}" style="width:88px" ${k}>
        ...
        </div>`}const n=t.result||{},a=n.segments||[],r=n.parse||{},o=this._busy.has("hist-apply"),i=t.accept||new Set,l=[];r.rows_total&&l.push(this._t("msg.hist_rows_read",{n:r.rows_total},`${r.rows_total} readings read`)),r.first&&r.last&&l.push(`${st(r.first)} \u2013 ${st(r.last)}`),r.breaks&&l.push(this._t("msg.hist_breaks",{n:r.breaks},`${r.breaks} gaps where the sensor was unavailable`)),r.rows_other_entity&&l.push(this._t("msg.hist_other_entity",{n:r.rows_other_entity},`${r.rows_other_entity} readings for other entities ignored`));const c=n.skipped||[],_={};c.forEach($=>{_[$.reason]=(_[$.reason]||0)+1});const p=Object.entries(_).map(([$,k])=>`${k} \xD7 ${this._histSkipReason($)}`).join(", "),g=n.settings||{},u=g.min_power!=null?this._t("msg.hist_settings_used",{w:g.min_power,s:g.off_delay},`Detected using this device's current settings (minimum power ${g.min_power} W, off delay ${g.off_delay} s).`):"";if(!a.length)return`${e}
        ...
        </div>`;const v=a.map($=>{const k=i.has($.index),S=this._histSegReason($.reason);return`<tr data-hist-row="${$.index}" style="${k?"":"opacity:.55"}">
      ...
      </tr>`}).join(""),b=a.every($=>i.has($.index));return`${e}
      <p class="wd-info" style="margin-bottom:8px">${this._t("msg.hist_found",{n:a.length},`Found ${a.length} cycles. Untick anything that does not look like a real run - nothing is stored until you import.`)}</p>
      ...
      <button class="wd-btn wd-btn-sm ${t.mode==="review"?"wd-btn-primary":"wd-btn-secondary"}" data-maction="cyc-review" title="${v?this._t("hdr.automation_needs_review",{},"This cycle needs review"):this._t("hdr.automation_review_this_cycle",{},"Review this cycle")}">${this._t("btn.review",{},"Review")}${b}</button>
        ...
        </div>`}let B="";const W=t.mode==="view"||t.mode==="review"?e.artifacts||[]:[];if(W.length){const E=W.map(M=>{const H=M.detail_key?this._t(M.detail_key,M.detail_params||{},M.detail||""):M.detail||"";return`<li><b>${d(Yt(M.type,(q,Q,J)=>this._t(q,Q,J)))}</b> ${this._t("lbl.at",{},"at")} ${Mt(M.start_s)}\u2013${Mt(M.end_s)} \u2014 ${d(H)}</li>`}).join("");B=`<div class="wd-card" style="margin:10px 0 0;padding:10px 12px;border-left:3px solid var(--warning-color,#ff9800)">
      ...
      ${R}`}_htmlProfilePanel(t){const e=this._canEdit();t.tab==="danger"&&!e&&(t.tab="stats");const s=[["stats",this._t("tab.pp_overview",{},"Overview")],["phases",this._t("tab.pp_phases",{},"Phases")],["cleanup",this._t("tab.pp_cleanup",{},"Cleanup")]];e&&s.push(["danger",this._t("tab.pp_manage",{},"Manage")]);const n=s.map(([o,i])=>`<button class="wd-mini-tab ${t.tab===o?"active":""}" data-maction="pp-tab-${o}">${i}</button>`).join("");let a="";if(!t.loaded)a=`<div class="wd-empty" style="padding:32px"><div class="wd-icon">\u23F3</div>${this._t("msg.loading",{},"Loading\u2026")}</div>`;else if(t.tab==="stats"){const o=t.stats||{},i=t.env||{},l=this._hass&&this._hass.config&&this._hass.config.currency||"",c=o.avg_energy!=null&&o.cycle_count?o.avg_energy*o.cycle_count:null,_=b=>b?Math.round(b/60)+"m":"-",p=(this._profileHealth||{})[t.name],g=(this._profileTrends||{})[t.name],u=p&&p.health_status!=="unknown"?(()=>{const b={healthy:["var(--success-color,#4caf50)","rgba(76,175,80,.12)"],fair:["var(--warning-color,#ff9800)","rgba(255,152,0,.12)"],poor:["var(--error-color,#f44336)","rgba(244,67,54,.12)"]},[$,k]=b[p.health_status]||b.fair,S=Math.round((p.health_score||0)*100),C=p.duration_cv!=null?` \xB7 ${this._t("stat.duration_cv",{pct:Math.round(p.duration_cv*100)},`duration CV ${Math.round(p.duration_cv*100)}%`)}`:"",R=p.confidence_mean!=null?` \xB7 ${this._t("stat.avg_confidence",{pct:Math.round(p.confidence_mean*100)},`avg confidence ${Math.round(p.confidence_mean*100)}%`)}`:"";return`<div style="margin:8px 0 4px;padding:8px 12px;border-radius:6px;background:${k};border:1px solid color-mix(in srgb, ${$} 13%, transparent);display:flex;align-items:center;gap:8px;flex-wrap:wrap">
        ...
        ${i.avg&&i.avg.length?`<div class="wd-canvas-wrap"><canvas id="wd-env-canvas" role="img" aria-label="${d(this._t("lbl.aria_envelope_chart",{},"Profile power envelope chart"))}"></canvas></div>`:`<p class="wd-info">${this._t("msg.no_envelope",{},"No envelope yet - rebuild after labelling cycles.")}</p>`}`}else if(t.tab==="phases"){const o=t.catalog||[],i=(t.phases||[]).map((c,_)=>{const p=o.map(g=>`<option value="${d(g)}" ${c.name===g?"selected":""}>${d(g)}</option>`).join("");return`<div class="wd-phase-row"><span class="wd-swatch" style="background:${et[_%et.length]}"></span>
          ...
          <button class="wd-btn wd-btn-danger wd-btn-sm" data-maction="pp-phase-rm" data-idx="${_}">\u2715</button></div>`}).join(""),l=this._busy.has("pp-phase-save");a=`<p class="wd-info" style="margin-bottom:10px">${this._t("msg.phase_ranges_intro",{},"Phase ranges (minutes from cycle start) overlaid on the average curve. Edit values to preview live.")}</p>
        ...
        ${e?"<th></th>":""}
      </tr></thead>`,v=this._busy.has("pp-cleanup-del");a=`<p class="wd-info" style="margin-bottom:10px">${this._t("msg.cleanup_intro",{},"Every labelled cycle overlaid. Tick outliers and delete to clean up the profile.")}</p>
        ...
        </div>`}const r=this._onlineEnabled()&&this._storeDeviceDeclared()?`<button class="wd-btn wd-btn-ghost wd-btn-sm" type="button" data-action="store-share-profile" data-prog="${d(t.name)}" title="${d(this._t("btn.share_device_tip",{},"Share this appliance and its recorded reference cycles to the community store so others with the same machine can adopt them"))}">\u2B06 ${this._t("btn.share_to_store",{},"Share to store")}</button>`:"";return`<h2>Profile \xB7 ${d(t.name)}</h2>
      <div class="wd-mini-tabs">${n}</div>
      ${a}
      <div class="wd-modal-actions" style="margin-top:14px">
        <button class="wd-btn wd-btn-secondary" data-maction="cancel">${this._t("btn.close",{},"Close")}</button>
        ${r}
      </div>`}_drawCycleEditor(){const t=this._modal;if(!t||t.type!=="cycle-detail"||!t.loaded)return;const e=t.curve||{},s=e.samples||[];if(!s.length)return;let n=e.full_duration_s||s[s.length-1][0]||1;const a=[],r=t.profileEnv;if((t.mode==="view"||t.mode==="review")&&r&&(r.avg||[]).length&&(a.push({points:r.avg,stroke:"#ff9800",width:2,alpha:.45,name:`${this._t("lbl.expected",{},"Expected")} (${e.profile_name||"profile"})`}),n=Math.max(n,r.target_duration||r.avg[r.avg.length-1][0]||0)),t.mode==="review"&&(t.overlays||[]).length){const c=this._profileEnvCache||{};(t.overlays||[]).forEach(_=>{const p=c[_];if(!p||!(p.avg||[]).length)return;const g=et[Math.max(0,(this._profiles||[]).findIndex(v=>v.name===_))%et.length];a.push({points:p.avg,stroke:g,width:1.6,alpha:.7,name:_});const u=p.avg[p.avg.length-1];n=Math.max(n,p.target_duration||(u?u[0]:0))})}a.push({points:s,stroke:"primary",fill:!0,width:2,name:this._t("lbl.power",{},"Power")});const o=[],i=[];let l=[];if(t.mode==="trim"){const c=t.trim.start,_=t.trim.end;o.push({x0:0,x1:c,fill:"rgba(244,67,54,.18)"}),o.push({x0:_,x1:n,fill:"rgba(244,67,54,.18)"}),i.push({x:c,color:"#f44336",label:"S"},{x:_,color:"#f44336",label:"E"})}else if(t.mode==="split")(t.split.offsets||[]).slice().sort((c,_)=>c-_).forEach((c,_)=>i.push({x:c,color:"#ff9800",label:"#"+(_+1)}));else{l=e.artifacts||[];const c={pause:"rgba(255,152,0,.22)",dip:"rgba(33,150,243,.18)",spike:"rgba(244,67,54,.18)"};l.forEach(p=>o.push({x0:p.start_s,x1:Math.max(p.end_s,p.start_s+1),fill:c[p.type]||"rgba(158,158,158,.18)"}));const _=e.start_time;(e.restart_gaps||[]).forEach(p=>{if(!_)return;const g=new Date(_).getTime(),u=Math.max(0,(new Date(p.start_ts).getTime()-g)/1e3),v=Math.max(u+1,(new Date(p.end_ts).getTime()-g)/1e3);o.push({x0:u,x1:v,fill:"rgba(96,125,139,.20)",label:"\u21BB"})})}this._drawCurves("wd-cyc-canvas",{series:a,xMax:n,bands:o,vlines:i,artifacts:l})}_htmlCompareModal(t){const e=t.ids||[],s={};(this._cycles||[]).forEach(o=>{s[o.id]=o});const n=t.hidden||new Set,a=e.map((o,i)=>{const l=s[o]||{},c=et[i%et.length],_=!n.has(o),p=!!(t.cycles&&t.cycles[o]),g=`${st(l.start_time)||String(o).slice(0,8)} \xB7 ${Math.round((l.duration||0)/60)}m \xB7 ${d(l.profile_name||this._t("lbl.unlabelled",{},"Unlabelled"))}`;return`<label class="wd-rev-tag"><input type="checkbox" class="wd-compare-cyc" value="${d(o)}" ${_?"checked":""} ${p?"":"disabled"}><span style="display:inline-block;width:10px;height:10px;border-radius:2px;background:${c};margin:0 2px"></span>${g}${p?"":` <span class="wd-info">${this._t("lbl.loading_paren",{},"(loading\u2026)")}</span>`}</label>`}).join(""),r=(this._profiles||[]).map(o=>{const i=(t.overlays||[]).includes(o.name),l=et[Math.max(0,(this._profiles||[]).findIndex(_=>_.name===o.name))%et.length],c=i?`<span style="display:inline-block;width:10px;height:10px;border-radius:2px;background:${l};margin:0 2px;opacity:.55"></span>`:"";return`<label class="wd-rev-tag"><input type="checkbox" class="wd-compare-overlay" value="${d(o.name)}" ${i?"checked":""}> ${c}${d(o.name)}</label>`}).join("")||`<span class="wd-info">${this._t("msg.no_profiles_overlay",{},"No profiles to overlay.")}</span>`;return`<h2>${this._t("msg.compare_cycles_title",{count:e.length},`Compare ${e.length} cycles`)}</h2>
      ...
      <div class="wd-modal-actions" style="margin-top:16px">
        <button class="wd-btn wd-btn-secondary" data-maction="cancel">${this._t("btn.close",{},"Close")}</button>
      </div>`}_drawCompareCanvas(){const t=this._modal;if(!t||t.type!=="compare-cycles")return;const e=t.ids||[],s={};(this._cycles||[]).forEach(i=>{s[i.id]=i});const n=t.hidden||new Set,a=[];let r=0;const o=this._profileEnvCache||{};(t.overlays||[]).forEach(i=>{const l=o[i];if(!l||!(l.avg||[]).length)return;const c=et[Math.max(0,(this._profiles||[]).findIndex(p=>p.name===i))%et.length];a.push({points:l.avg,stroke:c,width:2,alpha:.4,name:i});const _=l.avg[l.avg.length-1];r=Math.max(r,l.target_duration||(_?_[0]:0))}),e.forEach((i,l)=>{if(n.has(i))return;const c=t.cycles&&t.cycles[i],_=c&&c.samples;if(!_||!_.length)return;const p=et[l%et.length],g=s[i]||{};a.push({points:_,stroke:p,width:1.8,alpha:.9,name:st(g.start_time)||String(i).slice(0,8)}),r=Math.max(r,c.full_duration_s||_[_.length-1][0]||0)}),this._drawCurves("wd-compare-canvas",{series:a,xMax:r||1})}_drawProfileEnvelope(){const t=this._modal;if(!t||!t.env||!(t.env.avg||[]).length)return;const e=t.env;this._drawCurves("wd-env-canvas",{series:[{points:e.avg,stroke:"primary",width:2,name:"Average"}],band:{min:e.min,max:e.max},xMax:e.target_duration||e.avg[e.avg.length-1][0]})}_drawPhaseEditor(){const t=this._modal;if(!t||!t.env||!(t.env.avg||[]).length)return;const e=t.env,s=e.target_duration||e.avg[e.avg.length-1][0],n=(t.phases||[]).map((r,o)=>({x0:r.start,x1:r.end,fill:St(et[o%et.length],.2)})),a=[];(t.phases||[]).forEach((r,o)=>{const i=et[o%et.length];a.push({x:r.start,color:i,label:r.name?r.name.slice(0,7):"",handle:!0}),a.push({x:r.end,color:i,handle:!0})}),this._drawCurves("wd-phase-canvas",{series:[{points:e.avg,stroke:"primary",width:2,name:"Average"}],band:{min:e.min,max:e.max},bands:n,vlines:a,xMax:s})}_drawSpaghetti(){const t=this._modal;if(!t||!t.cleanup||!(t.cleanup.cycles||[]).length)return;const e=t.cleanup.cycles,s=t.cleanup.selected||new Set,n=this._spagTableHoverCid||null;let a=1;e.forEach(o=>{const i=o.samples||[];i.length&&i[i.length-1][0]>a&&(a=i[i.length-1][0])});const r=e.map((o,i)=>{const l=s.has(o.cycle_id),c=n===o.cycle_id;return{points:o.samples||[],stroke:et[i%et.length],width:l||c?2.6:1,alpha:s.size?l?1:.22:n?c?1:.22:.7,name:st(o.start_time),cid:o.cycle_id}});this._drawCurves("wd-spag-canvas",{series:r,xMax:a})}_wire(){const t=this.shadowRoot;if(!t)return;const e=t.getElementById("wd-burger");e&&e.addEventListener("click",()=>{this.dispatchEvent(new CustomEvent("hass-toggle-menu",{bubbles:!0,composed:!0}))}),t.querySelectorAll(".wd-devcard[data-idx]").forEach(f=>f.addEventListener("click",()=>this._selectDevice(parseInt(f.dataset.idx,10)))),t.querySelectorAll("[data-tab]").forEach(f=>f.addEventListener("click",()=>{f.dataset.tab!=="settings"&&(this._pendingSettings={}),this._tab=f.dataset.tab,this._fetchTabData()})),t.querySelectorAll("[data-sec]").forEach(f=>f.addEventListener("click",()=>{this._snapshotFormToPending(t),this._settingsSec=f.dataset.sec,this._settingsSearch="",this._settingsSugOnly=!1,this._render()})),t.querySelectorAll("[data-ptab]").forEach(f=>f.addEventListener("click",()=>{const m=this._panelSubtab=f.dataset.ptab;this._render();const x=this._devices[this._selIdx];x&&(m==="diagnostics"&&!this._diag?this._fetchToolsData(x.entry_id).then(()=>{this._panelSubtab==="diagnostics"&&this._render()}):m==="logs"?this._fetchLogs().then(()=>{this._panelSubtab==="logs"&&this._render()}):m==="maintenance"?this._fetchMaintenance(x.entry_id).then(()=>{this._panelSubtab==="maintenance"&&this._render()}):m==="ml"&&this._fetchTabData())})),t.querySelectorAll("[data-gtab]").forEach(f=>f.addEventListener("click",()=>{this._gearTab=f.dataset.gtab,this._modal&&this._modal.type==="gear-settings"&&(this._modal.tab=this._gearTab);const m=this._devices[this._selIdx];this._gearTab==="online"&&m&&this._onlineEnabled()&&(this._ensureStoreConnectListener(),this._loadStoreStatus(m.entry_id).then(()=>{this._modal&&this._modal.type==="gear-settings"&&this._render()})),this._render()}));const s=t.getElementById("wd-store-brand");s&&s.addEventListener("change",()=>{const f=s.value.trim();this._opts={...this._opts,store_brand:f},delete this._pendingSettings.store_brand,this._catalog.forBrand=f,this._catalog.devices=void 0,this._render()});const n=t.getElementById("wd-store-model");n&&n.addEventListener("change",()=>{this._opts={...this._opts,store_model:n.value.trim()},delete this._pendingSettings.store_model,this._render()});const a=t.getElementById("wd-pg-canvas");if(a){const f=()=>{const I=a.getBoundingClientRect(),w=I.height||330,P=Bt+8,z=w-P-52;return{rect:I,padT:P,powerH:z}},m=(I,w)=>{const P=a.getBoundingClientRect(),z=I-P.left,F=w-P.top;if(F>Bt+4)return null;const K=this._pgEventHits||[];let Z=null,rt=1/0;for(const ht of K){const nt=(z-ht.cx)**2+(F-ht.cy)**2;nt<=(ht.r+4)**2&&nt<rt&&(Z=ht,rt=nt)}return Z},x=I=>{const{rect:w,padT:P,powerH:z}=f(),F=this._pgPowerPts;if(!F?.length)return 0;const K=Math.max(...F.map(Z=>Z.w),1);return Math.max(0,(1-Math.max(0,I-w.top-P)/z)*K)},L=I=>{const{rect:w,padT:P,powerH:z}=f(),F=this._pgPowerPts;if(!F?.length)return 0;const K=Math.max(...F.map(Z=>Z.w),1);return w.top+P+(1-Math.max(0,+I)/K)*z},j=I=>{const w=this._pgMap;if(!w)return null;const{rect:P}=f(),z=(I-P.left-w.padLpx)/Math.max(1,w.plotWpx);return w.vMin+Math.max(0,Math.min(1,z))*(w.vMax-w.vMin)};a.addEventListener("pointermove",I=>{if(this._pgDragging==="start_thr"){this._pgThreshStart=Math.max(0,x(I.clientY)),this._pgUpdateParamInput("start_threshold_w",this._pgThreshStart),this._pgDrawCanvas();return}if(this._pgDragging==="stop_thr"){this._pgThreshStop=Math.max(0,x(I.clientY)),this._pgUpdateParamInput("stop_threshold_w",this._pgThreshStop),this._pgDrawCanvas();return}if(this._pgDragging==="pan"&&this._pgPanStart){const Z=this._pgMap,rt=(I.clientX-this._pgPanStart.clientX)/Math.max(1,Z.plotWpx),ht=this._pgPanStart.vMax-this._pgPanStart.vMin;let nt=this._pgPanStart.vMin-rt*ht;nt=Math.max(0,Math.min(this._pgPanStart.totalDur-ht,nt)),this._pgView={min:nt,max:nt+ht},this._pgDrawCanvas();return}if(!this._pgPowerPts?.length)return;const w=m(I.clientX,I.clientY);if(w){a.style.cursor="pointer",this._pgHoverEvent={t:w.t,type:w.type},this._pgHoverT=null,this._pgDrawCanvas();return}this._pgHoverEvent&&(this._pgHoverEvent=null,this._pgDrawCanvas());const P=this._pgThreshStart??this._pgFieldVal("start_threshold_w",{})??50,z=this._pgThreshStop??this._pgFieldVal("stop_threshold_w",{})??5,F=Math.abs(I.clientY-L(P))<8||Math.abs(I.clientY-L(z))<8;a.style.cursor=F?"ns-resize":"crosshair";const K=j(I.clientX);K!=null&&(this._pgHoverT=K,this._pgUpdateStripAt(K),this._pgDrawCanvas())}),a.addEventListener("pointerdown",I=>{if(!this._pgPowerPts?.length||m(I.clientX,I.clientY))return;a.setPointerCapture(I.pointerId);const w=this._pgThreshStart??this._pgFieldVal("start_threshold_w",{})??50,P=this._pgThreshStop??this._pgFieldVal("stop_threshold_w",{})??5;Math.abs(I.clientY-L(w))<10?this._pgDragging="start_thr":Math.abs(I.clientY-L(P))<10?this._pgDragging="stop_thr":this._pgMap&&(this._pgDragging="pan",this._pgPanStart={clientX:I.clientX,vMin:this._pgMap.vMin,vMax:this._pgMap.vMax,totalDur:this._pgMap.totalDur},a.classList.add("wd-pg-panning"))}),a.addEventListener("pointerup",I=>{try{a.releasePointerCapture(I.pointerId)}catch{}const w=this._pgDragging==="start_thr"||this._pgDragging==="stop_thr";this._pgDragging=null,this._pgPanStart=null,a.classList.remove("wd-pg-panning"),w&&this._pgDrawCanvas()}),a.addEventListener("pointerleave",()=>{this._pgDragging||(this._pgHoverT=null,this._pgHoverEvent=null,this._pgUpdateStripAt(null),this._pgDrawCanvas())}),a.addEventListener("wheel",I=>{if(!this._pgPowerPts?.length||!this._pgMap)return;I.preventDefault();const w=this._pgMap,P=j(I.clientX);if(P==null)return;const z=w.vMax-w.vMin,F=I.deltaY>0?1.25:.8,K=Math.max(30,Math.min(w.totalDur,z*F)),Z=z>0?(P-w.vMin)/z:.5;let rt=P-Z*K;rt=Math.max(0,Math.min(w.totalDur-K,rt)),this._pgView=K>=w.totalDur-1?null:{min:rt,max:rt+K},this._pgDrawCanvas()},{passive:!1}),a.addEventListener("dblclick",()=>{this._pgView=null,this._pgDrawCanvas()})}t.querySelectorAll("input[data-pgkey]").forEach(f=>{f.addEventListener("input",()=>{const m=f.dataset.pgkey;if(f.dataset.pgtype==="bool"){const w=f.checked;m==="start_threshold_w"?this._pgThreshStart=w:m==="stop_threshold_w"?this._pgThreshStop=w:this._pgParamOverrides[m]=w,this._render();const P=t.querySelector(`input[data-pgkey="${m}"]`);P&&P.focus(),requestAnimationFrame(()=>this._pgDrawCanvas());return}const x=f.value.trim();if(!x)m==="start_threshold_w"?this._pgThreshStart=null:m==="stop_threshold_w"?this._pgThreshStop=null:delete this._pgParamOverrides[m];else{const w=parseFloat(x);isNaN(w)||(m==="start_threshold_w"?this._pgThreshStart=w:m==="stop_threshold_w"?this._pgThreshStop=w:this._pgParamOverrides[m]=w)}const L=f.selectionStart,j=f.value;this._render();const I=t.querySelector(`input[data-pgkey="${m}"]`);if(I){I.value=j,I.focus();try{I.setSelectionRange(L,L)}catch{}}requestAnimationFrame(()=>this._pgDrawCanvas())})});const r=t.getElementById("wd-pg-stress-toggle");r&&r.addEventListener("change",()=>{this._pgStressTail=r.checked,this._pgStressIdleW=null,this._render()});const o=t.getElementById("wd-pg-stress-idle-w");o&&o.addEventListener("input",()=>{const f=parseFloat(o.value);this._pgStressIdleW=Number.isFinite(f)&&f>=0?f:null});const i=t.getElementById("wd-pg-cyc-sel");i&&i.addEventListener("change",()=>this._pgSelectCycle(i.value));const l=t.getElementById("wd-pg-prof-sel");l&&l.addEventListener("change",()=>{this._pgProfileName=l.value,this._pgLoad()});const c=t.getElementById("wd-pg-preset-sel");c&&c.addEventListener("change",()=>{this._pgPresetSel=c.value,this._render()});const _=t.getElementById("wd-pg-preset-name");_&&_.addEventListener("input",()=>{this._pgPresetName=_.value;const f=(this._pgPresetName||"").trim(),m=this._pgPresetLimit>0&&(this._pgPresets||[]).length>=this._pgPresetLimit&&!(this._pgPresets||[]).some(j=>j.name===f),x=t.querySelector('[data-action="pg-preset-save"]');x&&(x.disabled=!(f&&!m));const L=t.getElementById("wd-pg-preset-limit-note");L&&(L.style.display=m?"":"none")});const p=t.getElementById("wd-pg-simn");p&&p.addEventListener("input",()=>{this._pgSimCycles=Math.max(1,Math.min(200,parseInt(p.value,10)||20))});const g=t.getElementById("wd-pg-sw-param");g&&g.addEventListener("change",()=>{this._pgSweepParam=g.value,this._pgSweepNew=null,this._render()});const u=t.getElementById("wd-pg-sw-obj");u&&u.addEventListener("change",()=>{this._pgSweepObjective=u.value,this._pgSweepNew=null,this._render()});const v=t.getElementById("wd-pg-sw-paramy"),b=t.getElementById("wd-pg-sw-from");b&&b.addEventListener("input",()=>{this._pgSweepFrom=b.value});const $=t.getElementById("wd-pg-sw-to");$&&$.addEventListener("input",()=>{this._pgSweepTo=$.value});const k=t.getElementById("wd-pg-sw-steps");k&&k.addEventListener("input",()=>{this._pgSweepSteps=parseInt(k.value,10)||5}),t.querySelectorAll("[data-statustoggle]").forEach(f=>f.addEventListener("change",async()=>{const m=f.dataset.statustoggle,x=f.checked;this._panelCfg||(this._panelCfg={}),this._panelCfg.prefs={...this._panelCfg.prefs||{},[m]:x},this._ws({type:`${y}/set_user_prefs`,prefs:{[m]:x}}).catch(()=>{});const L=this._devices[this._selIdx];if(L&&this._tab==="status")try{this._powerData=await this._ws({type:`${y}/get_power_history`,entry_id:L.entry_id,with_raw:this._pref("show_raw_active",!1)})}catch{}this._render()})),t.querySelectorAll("[data-sortact]").forEach(f=>f.addEventListener("click",()=>{const m=f.dataset.sortact,x=f.dataset.sortcol,L=j=>{j.col===x?j.dir*=-1:(j.col=x,j.dir=x==="date"?-1:1)};m==="cycsort"?L(this._cycleSort):m==="cleanupsort"&&L(this._cleanupSort),this._render()}));const S=t.getElementById("wd-cyc-filter-text");S&&S.addEventListener("input",f=>{const m=f.target.selectionStart;this._cycleFilter.text=S.value,this._render();const x=this.shadowRoot.getElementById("wd-cyc-filter-text");x&&(x.focus(),x.setSelectionRange(m,m))});const C=t.getElementById("wd-settings-search");C&&C.addEventListener("input",f=>{const m=f.target.selectionStart;this._settingsSearch=C.value,C.value.trim()&&(this._settingsSugOnly=!1),this._render();const x=this.shadowRoot.getElementById("wd-settings-search");x&&(x.focus(),x.setSelectionRange(m,m))});const R=t.getElementById("wd-cyc-filter-status");R&&R.addEventListener("change",()=>{this._cycleFilter.status=R.value,this._render()}),t.querySelectorAll(".wd-pillbox").forEach(f=>{const m=f.querySelector(".wd-pill-add"),x=j=>{const I=document.createElement("span");I.className="wd-pill",I.dataset.val=j,I.appendChild(document.createTextNode(j));const w=document.createElement("button");return w.type="button",w.className="wd-pill-x",w.setAttribute("aria-label","Remove"),w.textContent="\xD7",w.addEventListener("click",()=>I.remove()),I.appendChild(w),I},L=j=>{const I=String(j||"").trim();if(!I)return;Array.from(f.querySelectorAll(".wd-pill")).some(P=>P.dataset.val===I)||f.insertBefore(x(I),m.closest(".wd-combo")||m),m&&(m.value="")};f.querySelectorAll(".wd-pill-x").forEach(j=>j.addEventListener("click",()=>j.closest(".wd-pill")?.remove())),m&&(m.addEventListener("change",()=>L(m.value)),m.addEventListener("keydown",j=>{j.key==="Enter"&&(j.preventDefault(),L(m.value))}),m.addEventListener("blur",()=>L(m.value)))}),t.querySelectorAll(".wd-combo").forEach(f=>{const m=f.querySelector(".wd-combo-inp, .wd-pill-add"),x=f.querySelector(".wd-combo-drop");if(!m||!x)return;const L=f.classList.contains("wd-combo-pill"),j=m.dataset.opt||f.closest("[data-opt]")?.dataset.opt,I=P=>{this._ensureCatalogList(j,P);const z=(this._entityListCache||{})[j]||[],F=(P||"").toLowerCase(),K=F?z.filter(Z=>Z.toLowerCase().includes(F)).slice(0,40):z.slice(0,20);if(!K.length){x.hidden=!0;return}x.innerHTML=K.map(Z=>`<div class="wd-combo-item" data-val="${d(Z)}">${d(Z)}</div>`).join(""),x._kbd=-1,x.hidden=!1},w=P=>{if(P){if(L){const z=f.closest(".wd-pillbox");if(z&&!Array.from(z.querySelectorAll(".wd-pill")).some(F=>F.dataset.val===P)){const F=document.createElement("span");F.className="wd-pill",F.dataset.val=P,F.appendChild(document.createTextNode(P));const K=document.createElement("button");K.type="button",K.className="wd-pill-x",K.setAttribute("aria-label","Remove"),K.textContent="\xD7",K.addEventListener("click",()=>F.remove()),F.appendChild(K),z.insertBefore(F,f)}m.value=""}else m.value=P,m.dispatchEvent(new Event("change",{bubbles:!0}));x.hidden=!0}};m.addEventListener("focus",()=>I(m.value)),m.addEventListener("input",()=>I(m.value)),m.addEventListener("blur",()=>setTimeout(()=>{x.hidden=!0},150)),m.addEventListener("keydown",P=>{if(x.hidden&&P.key!=="ArrowDown")return;const z=x.querySelectorAll(".wd-combo-item");let F=x._kbd||-1;if(P.key==="ArrowDown"){if(P.preventDefault(),x.hidden){I(m.value);return}F=Math.min(F+1,z.length-1)}else if(P.key==="ArrowUp")P.preventDefault(),F=Math.max(F-1,0);else if(P.key==="Enter"&&!x.hidden){P.preventDefault(),F>=0?w(z[F].dataset.val):L&&m.value.trim()&&w(m.value.trim());return}else if(P.key==="Escape"){x.hidden=!0;return}else return;x._kbd=F,z.forEach((K,Z)=>K.classList.toggle("kbd",Z===F)),z[F]?.scrollIntoView({block:"nearest"})}),x.addEventListener("mousedown",P=>{const z=P.target.closest(".wd-combo-item");z&&(P.preventDefault(),w(z.dataset.val))})}),t.querySelectorAll(".wd-timerlist").forEach(f=>{const m=f.dataset.opt,x=()=>{if(!Array.isArray(this._pendingSettings[m])){const z=Array.isArray(this._opts&&this._opts[m])?this._opts[m]:[];this._pendingSettings[m]=z.map(F=>({...F}))}return this._pendingSettings[m]},L=z=>({offset_minutes:parseFloat(z.querySelector('[data-field="offset_minutes"]').value)||0,message:(z.querySelector('[data-field="message"]').value||"").trim(),auto_pause:z.querySelector('[data-field="auto_pause"]').checked}),j=z=>{const F=parseInt(z.dataset.tidx,10);if(isNaN(F))return;const K=x();K[F]=L(z)},I=z=>{const F=parseInt(z.dataset.tidx,10);isNaN(F)||(this._pendingSettings[m]=x().filter((K,Z)=>Z!==F)),z.remove(),f.querySelectorAll(".wd-timer-row").forEach((K,Z)=>{K.dataset.tidx=Z})},w=z=>{const F=z.querySelector(".wd-timer-remove");F&&F.addEventListener("click",()=>I(z)),z.querySelector('[data-field="auto_pause"]')?.addEventListener("change",()=>j(z)),z.querySelectorAll('[data-field="offset_minutes"],[data-field="message"]').forEach(K=>K.addEventListener("input",()=>j(z)))};f.querySelectorAll(".wd-timer-row").forEach(z=>w(z));const P=f.querySelector(".wd-timer-add");P&&P.addEventListener("click",()=>{const z=f.querySelectorAll(".wd-timer-row").length;x().push({offset_minutes:0,message:"",auto_pause:!1});const F=document.createElement("div");F.className="wd-timer-row",F.dataset.tidx=z,F.innerHTML=`<div class="wd-timer-top"><input type="number" min="1" placeholder="${this._t("lbl.timer_min",{},"min")}" data-field="offset_minutes"><textarea placeholder="${this._t("lbl.timer_msg_placeholder",{},"Message (optional, {device}/{program}/{minutes})")}" data-field="message"></textarea></div><div class="wd-timer-footer"><label class="wd-switch-lbl"><span class="wd-switch"><input type="checkbox" data-field="auto_pause"><span class="wd-switch-slider"></span></span><span class="wd-switch-text">${this._t("lbl.timer_auto_pause",{},"Auto-pause")}</span></label><button type="button" class="wd-btn wd-btn-sm wd-btn-danger wd-timer-remove">${this._t("btn.remove_timer",{},"Delete")}</button></div>`,w(F),f.insertBefore(F,P)})});const B=t.getElementById("wd-status-prog");B&&B.addEventListener("change",()=>{const f=this._devices[this._selIdx];if(!f)return;const m=B.value;this._ws({type:`${y}/set_program`,entry_id:f.entry_id,program:m}).then(()=>(this._showToast(m==="auto_detect"?this._t("msg.toast_auto_detect_enabled",{},"Auto-detect enabled"):this._t("msg.toast_program_set",{program:m},`Program set: ${m}`)),this._fetchAll())).catch(x=>this._showToast(this._t("msg.toast_failed",{error:x.message||x},"Failed: "+(x.message||x)),"error"))}),t.querySelectorAll("[data-cid]").forEach(f=>f.addEventListener("click",m=>{if(m.target.tagName==="INPUT"||f.dataset.action)return;const x=f.dataset.cid;f.dataset.selmode==="1"?(this._cycleSel.has(x)?this._cycleSel.delete(x):this._cycleSel.add(x),this._render()):this._onAction({dataset:{action:"open-cycle",cid:x}})})),t.querySelectorAll(".wd-cyc-overlay").forEach(f=>f.addEventListener("change",()=>{const m=this._modal;if(!m||m.type!=="cycle-detail")return;this._snapshotCycleReviewForm(t);const x=new Set(m.overlays||[]);f.checked?x.add(f.value):x.delete(f.value),m.overlays=[...x];const L=this._devices[this._selIdx];f.checked&&L?this._ensureProfileEnvs(L.entry_id,[f.value]).then(()=>this._render()):this._render()})),t.querySelectorAll(".wd-compare-cyc").forEach(f=>f.addEventListener("change",()=>{const m=this._modal;if(!m||m.type!=="compare-cycles")return;const x=m.hidden instanceof Set?m.hidden:new Set(m.hidden||[]);f.checked?x.delete(f.value):x.add(f.value),m.hidden=x,this._render()})),t.querySelectorAll(".wd-compare-overlay").forEach(f=>f.addEventListener("change",()=>{const m=this._modal;if(!m||m.type!=="compare-cycles")return;const x=new Set(m.overlays||[]);f.checked?x.add(f.value):x.delete(f.value),m.overlays=[...x];const L=this._devices[this._selIdx];f.checked&&L?this._ensureProfileEnvs(L.entry_id,[f.value]).then(()=>this._render()):this._render()})),t.querySelectorAll(".wd-pg-mem").forEach(f=>f.addEventListener("change",()=>{const m=this._modal;if(!m||m.type!=="profile-group")return;const x=t.getElementById("wd-pg-name");x&&(m.name=x.value);const L=new Set(m.members||[]);f.checked?L.add(f.value):L.delete(f.value),m.members=[...L],this._render()}));const W=t.getElementById("wd-pg-name");W&&W.addEventListener("input",()=>{const f=this._modal;f&&f.type==="profile-group"&&(f.name=W.value)}),t.querySelectorAll(".wd-csel").forEach(f=>f.addEventListener("change",()=>{const m=f.closest("[data-cid]"),x=m&&m.dataset.cid;x&&(f.checked?this._cycleSel.add(x):this._cycleSel.delete(x),this._render())}));const O=t.getElementById("wd-merge-prof");O&&O.addEventListener("change",()=>{const f=t.getElementById("wd-merge-new");f&&(f.style.display=O.value==="__create_new__"?"":"none")}),t.querySelectorAll(".wd-log-filter").forEach(f=>{const m=f.dataset.logfilter,x={level:"_logLevel",device:"_logDevice",component:"_logComponent",search:"_logSearch"}[m];if(!x)return;const L=m==="search"?"input":"change";f.addEventListener(L,()=>{this[x]=f.value,this._refreshLogViews(),this._syncLogFilters(f)})});const D=t.querySelector(".wd-log-resize");D&&D.addEventListener("pointerdown",f=>{f.preventDefault(),D.setPointerCapture(f.pointerId),D.classList.add("dragging");const m=t.querySelector(".wd-log-drawer"),x=f.clientX,L=m.offsetWidth,j=I=>{const w=Math.max(280,Math.min(900,L+(x-I.clientX)));m.style.width=w+"px",this._logDrawerWidth=w};D.addEventListener("pointermove",j),D.addEventListener("pointerup",()=>{D.removeEventListener("pointermove",j),D.classList.remove("dragging");try{localStorage.setItem("wd-log-width",String(this._logDrawerWidth))}catch{}},{once:!0})});const A=t.getElementById("wd-hist-file");A&&A.addEventListener("change",()=>{const f=A.files&&A.files[0];if(!f)return;const m=this._modal,x=new FileReader;x.onload=()=>{const L=t.getElementById("wd-hist-csv"),j=String(x.result||"");L&&(L.value=j),m&&m.type==="history-import"&&(m.csvText=j)},x.onerror=()=>this._showToast(this._t("toast.file_read_failed",{},"Could not read that file"),"error"),x.readAsText(f)}),t.querySelectorAll("[data-hist-pick]").forEach(f=>f.addEventListener("change",()=>{const m=this._modal;if(!m||m.type!=="history-import")return;const x=parseInt(f.dataset.histPick,10);f.checked?m.accept.add(x):m.accept.delete(x),this._render(),requestAnimationFrame(()=>this._drawHistorySparklines())}));const E=t.getElementById("wd-import-file");E&&E.addEventListener("change",()=>{const f=E.files&&E.files[0];if(!f)return;const m=new FileReader;m.onload=()=>{const x=t.getElementById("wd-import-json");x&&(x.value=String(m.result||""))},m.readAsText(f)}),t.querySelectorAll("[data-stab]").forEach(f=>f.addEventListener("click",()=>{this._toolsSubtab=f.dataset.stab;const m=this._devices[this._selIdx];m&&(this._tabLoading=!0,this._render(),this._fetchToolsData(m.entry_id).then(()=>{this._tabLoading=!1,this._render()}))})),t.querySelectorAll("[data-proftab]").forEach(f=>f.addEventListener("click",async()=>{this._profSubtab=f.dataset.proftab;const m=this._devices[this._selIdx];m&&this._profSubtab==="phase-catalog"&&!this._phases.length&&(this._tabLoading=!0,this._render(),await this._fetchPhases(m.entry_id),this._tabLoading=!1),this._render()}));const M=t.getElementById("wd-settings-save");M&&M.addEventListener("click",()=>this._saveSettings());const H=t.getElementById("wd-ml-save");H&&H.addEventListener("click",()=>this._saveSettings());const q=t.getElementById("wd-settings-form");q&&(q.addEventListener("submit",f=>f.preventDefault()),q.addEventListener("input",()=>this._liveValidateSettings(t)),q.addEventListener("change",()=>this._liveValidateSettings(t)),q.addEventListener("click",f=>{const m=f.target.closest(".wd-conflict-fix");if(!m)return;const x=m.dataset.ckey,L=parseFloat(m.dataset.cval),j=q.querySelector(`[data-opt="${x}"]`);j&&!isNaN(L)&&(j.value=L,this._cascadeConflictFix(t,q,x))}),this._liveValidateSettings(t));const Q=t.getElementById("wd-settings-revert");Q&&Q.addEventListener("click",async()=>{if(!this._prevOpts)return;const f=this._devices[this._selIdx];f&&await this._busyRun("save-settings",async()=>{try{const m=this._prevOpts;await this._ws({type:`${y}/set_options`,entry_id:f.entry_id,options:m}),this._opts={...m},this._prevOpts=null,this._cascadePending={},this._preCascadeOpts=null,this._pendingSettings={},this._showToast(this._t("toast.settings_reverted",{},"Settings reverted; integration reloading")),this._render()}catch(m){this._showToast(this._t("msg.toast_revert_failed",{error:m.message||m},"Revert failed: "+(m.message||m)),"error")}})});const J=t.getElementById("wd-settings-reload");J&&J.addEventListener("click",async()=>{const f=this._devices[this._selIdx];if(f){this._prevOpts=null,this._cascadePending={},this._preCascadeOpts=null,this._pendingSettings={};const m=await this._ws({type:`${y}/get_options`,entry_id:f.entry_id});this._opts=m.options||{},this._optDefaults=m.defaults||{},await this._fetchSuggestions(f.entry_id),this._render()}});const G=t.getElementById("wd-pref-fontscale");if(G){const f=t.getElementById("wd-pref-fontscale-val");G.addEventListener("input",()=>{const m=parseFloat(G.value)||1;this._applyFontScale(m),f&&(f.textContent=Math.round(m*100)+"%")}),G.addEventListener("change",()=>{this._setPref("font_scale",parseFloat(G.value)||1)})}t.querySelectorAll("[data-action]").forEach(f=>f.addEventListener("click",m=>this._onAction(m.currentTarget))),t.querySelectorAll("[data-maction]").forEach(f=>f.addEventListener("click",m=>this._onModalAction(m.currentTarget.dataset.maction,m.currentTarget))),t.querySelectorAll("select[data-maction]").forEach(f=>f.addEventListener("change",m=>this._onModalAction(m.currentTarget.dataset.maction,m.currentTarget))),t.querySelectorAll("input[data-indeterminate]").forEach(f=>{f.indeterminate=!0});const X=t.querySelector("[data-toast-undo]");X&&X.addEventListener("click",()=>this._undoDelete(X.dataset.toastUndo)),t.querySelectorAll(".wd-create-cluster").forEach(f=>f.addEventListener("click",()=>{const m=f.dataset.name||"";this._modal={type:"create-profile",prefillName:m},this._render()})),t.querySelectorAll("[data-suglock]").forEach(f=>f.addEventListener("click",async()=>{const m=f.dataset.suglock,x=this._devices[this._selIdx],L=x&&x.entry_id;if(!(!L||!m))try{if(await this._ws({type:`${y}/set_suggestion_lock`,entry_id:L,key:m,locked:!0}),!this._isActiveEntry(L))return;this._suggestions=(this._suggestions||[]).filter(j=>j.key!==m),(this._lockedSuggestions||[]).includes(m)||(this._lockedSuggestions=this._lockedSuggestions||[]).push(m),this._showToast(this._t("msg.sug_muted",{},"Won't suggest this setting again"),"info"),this._render()}catch{this._showToast(this._t("msg.sug_mute_failed",{},"Could not mute suggestion"),"error")}})),t.querySelectorAll("[data-sugkey]").forEach(f=>f.addEventListener("click",()=>{const m=f.dataset.sugkey,x=f.dataset.sugval,L=parseFloat(x);this._pendingSettings[m]=isNaN(L)?x:L,this._stagedSuggestions=!0,this._suggestions=this._suggestions.filter(w=>w.key!==m),this._showToast(this._t("msg.sug_staged",{key:m,val:x},`Set ${m} = ${x}. Save to apply.`),"info"),this._render();const j=this.shadowRoot,I=j?.getElementById("wd-settings-form");I&&this._cascadeConflictFix(j,I,m)}));const ct=t.getElementById("wd-label-profile");ct&&ct.addEventListener("change",()=>{const f=t.getElementById("wd-new-profile-row"),m=ct.value==="__create_new__";if(f&&(f.style.display=m?"":"none"),!m){const x=t.getElementById("wd-new-profile-name");x&&(x.value="")}});const U=t.getElementById("wd-cp-cycle");if(U){const f=()=>{const m=t.getElementById("wd-cp-dur");if(!m)return;const x=!!U.value;m.disabled=x,m.style.opacity=x?"0.5":""};U.addEventListener("change",f),f()}const at=t.getElementById("wd-pr-mode");at&&at.addEventListener("change",()=>{const f=t.getElementById("wd-pr-profile"),m=t.getElementById("wd-pr-existing");if(!f||!m)return;const x=at.value==="existing_profile";f.style.display=x?"none":"",m.style.display=x?"":"none"}),this._wireCycleCanvas(t),this._wirePhaseInputs(t),this._wirePhaseCanvas(t),this._wireCleanup(t),this._wireSplitSegments(t)}_syncTrimInputs(){const t=this.shadowRoot,e=this._modal,s=(e.timeMode||"s")==="clock",n=t.getElementById("wd-trim-start"),a=t.getElementById("wd-trim-end");n&&(n.value=s?this._offsetToClock(e.trim.start):Math.round(e.trim.start)),a&&(a.value=s?this._offsetToClock(e.trim.end):Math.round(e.trim.end))}_snapTrimBounds(){const t=this._modal,e=t.curve&&t.curve.samples||[];if(e.length<2)return;const s=n=>e.reduce((a,r)=>Math.abs(r[0]-n)<Math.abs(a-n)?r[0]:a,e[0][0]);if(t.trim.start=s(t.trim.start),t.trim.end=s(t.trim.end),t.trim.end<=t.trim.start){const n=e.findIndex(a=>a[0]===t.trim.start);n>=0&&n+1<e.length?t.trim.end=e[n+1][0]:n>0&&(t.trim.start=e[n-1][0])}}_offsetToClock(t){const e=this._modal,s=e&&e.curve&&e.curve.start_time;if(!s)return"";const n=new Date(new Date(s).getTime()+(t||0)*1e3);return`${String(n.getHours()).padStart(2,"0")}:${String(n.getMinutes()).padStart(2,"0")}:${String(n.getSeconds()).padStart(2,"0")}`}_clockToOffset(t){const e=this._modal,s=e&&e.curve&&e.curve.start_time;if(!s||!t)return null;const n=String(t).split(":").map(Number);if(!n.length||n.some(l=>!Number.isFinite(l)))return null;const a=new Date(s),r=new Date(a);r.setHours(n[0]||0,n[1]||0,n[2]||0,0);let o=(r-a)/1e3;const i=e.curve&&e.curve.full_duration_s||0;return o<-1&&o+86400<=i&&(o+=86400),Math.max(0,Math.min(i,o))}_trimInputToOffset(t){if(t===""||t==null)return null;if(this._modal.timeMode==="clock")return this._clockToOffset(t);const e=Kt(t,NaN);return Number.isFinite(e)?e:null}_toggleSplit(t){const e=this._modal,s=e.curve&&e.curve.full_duration_s||0,n=Math.max(20,s*.025),a=e.split.offsets,r=a.findIndex(o=>Math.abs(o-t)<n);r>=0?a.splice(r,1):a.push(Math.round(t)),a.sort((o,i)=>o-i),e.split.profiles=[],this._render()}_wireCycleCanvas(t){const e=this._modal;if(!e||e.type!=="cycle-detail"||!e.loaded)return;const s=t.getElementById("wd-cyc-canvas");if(s)if(e.mode==="trim"){const n=t.getElementById("wd-trim-start"),a=t.getElementById("wd-trim-end");n&&n.addEventListener("input",()=>{const i=this._trimInputToOffset(n.value);i!==null&&(e.trim.start=Math.max(0,Math.min(i,e.trim.end-1)),this._drawCycleEditor())}),a&&a.addEventListener("input",()=>{const i=this._trimInputToOffset(a.value);i!==null&&(e.trim.end=Math.min(e.curve.full_duration_s,Math.max(i,e.trim.start+1)),this._drawCycleEditor())});const r=()=>{this._snapTrimBounds(),this._syncTrimInputs(),this._drawCycleEditor()};n&&n.addEventListener("change",r),a&&a.addEventListener("change",r),s.addEventListener("pointerdown",i=>{const l=s._wd;if(!l)return;const c=s.getBoundingClientRect(),_=i.clientX-c.left;e.drag=Math.abs(_-l.xToCss(e.trim.start))<=Math.abs(_-l.xToCss(e.trim.end))?"start":"end",s.setPointerCapture(i.pointerId)}),s.addEventListener("pointermove",i=>{if(!e.drag)return;const l=s._wd;if(!l)return;const c=s.getBoundingClientRect(),_=l.cssToX(i.clientX-c.left);e.drag==="start"?e.trim.start=Math.min(_,e.trim.end-1):e.trim.end=Math.max(_,e.trim.start+1),this._syncTrimInputs(),this._drawCycleEditor()});const o=()=>{e.drag&&(this._snapTrimBounds(),this._syncTrimInputs(),this._drawCycleEditor()),e.drag=null};s.addEventListener("pointerup",o),s.addEventListener("pointercancel",o)}else e.mode==="split"&&s.addEventListener("pointerdown",n=>{const a=s._wd;if(!a)return;const r=s.getBoundingClientRect();this._toggleSplit(a.cssToX(n.clientX-r.left))})}_wireSplitSegments(t){const e=this._modal;!e||e.type!=="cycle-detail"||e.mode!=="split"||t.querySelectorAll("[data-segidx]").forEach(s=>s.addEventListener("change",()=>{e.split.profiles[+s.dataset.segidx]=s.value||null}))}_wirePhaseInputs(t){const e=this._modal;!e||e.type!=="profile-panel"||e.tab!=="phases"||t.querySelectorAll("[data-phidx]").forEach(s=>{const n=()=>{const a=+s.dataset.phidx,r=s.dataset.phfield,o=e.phases[a];o&&(r==="name"?o.name=s.value:o[r]=Math.max(0,Kt(s.value,0)*60),this._drawPhaseEditor())};s.addEventListener("input",n),s.addEventListener("change",n)})}_wirePhaseCanvas(t){const e=this._modal;if(!e||e.type!=="profile-panel"||e.tab!=="phases"||!e.env||!(e.env.avg||[]).length)return;const s=t.getElementById("wd-phase-canvas");if(!s)return;const n=e.env.target_duration||e.env.avg[e.env.avg.length-1][0],a=Math.max(5,n*.01),r=i=>{const l=s._wd;if(!l)return null;let c=null,_=12;return(e.phases||[]).forEach((p,g)=>{[["start",p.start],["end",p.end]].forEach(([u,v])=>{const b=Math.abs(i-l.xToCss(v));b<_&&(_=b,c={idx:g,edge:u})})}),c};s.addEventListener("pointerdown",i=>{const l=s.getBoundingClientRect();e.phaseDrag=r(i.clientX-l.left),e.phaseDrag&&s.setPointerCapture(i.pointerId)}),s.addEventListener("pointermove",i=>{if(!e.phaseDrag)return;const l=s._wd;if(!l)return;const c=s.getBoundingClientRect(),_=l.cssToX(i.clientX-c.left),p=e.phases[e.phaseDrag.idx];p&&(e.phaseDrag.edge==="start"?p.start=Math.max(0,Math.min(_,p.end-a)):p.end=Math.min(n,Math.max(_,p.start+a)),this._syncPhaseInputs(e.phaseDrag.idx),this._drawPhaseEditor())});const o=()=>{e.phaseDrag=null};s.addEventListener("pointerup",o),s.addEventListener("pointercancel",o)}_syncPhaseInputs(t){const e=this.shadowRoot,s=this._modal.phases[t];if(!s)return;const n=e.querySelector(`[data-phidx="${t}"][data-phfield="start"]`),a=e.querySelector(`[data-phidx="${t}"][data-phfield="end"]`);n&&(n.value=(s.start/60).toFixed(1)),a&&(a.value=(s.end/60).toFixed(1))}_wireCleanup(t){const e=this._modal;if(!e||e.type!=="profile-panel"||e.tab!=="cleanup"||!e.cleanup)return;t.querySelectorAll("[data-cleanidx]").forEach(n=>n.addEventListener("change",()=>{const a=e.cleanup.cycles[+n.dataset.cleanidx];if(!a)return;n.checked?e.cleanup.selected.add(a.cycle_id):e.cleanup.selected.delete(a.cycle_id);const r=e.cleanup.selected,o=t.querySelector('[data-maction="pp-cleanup-del"]');o&&!this._busy.has("pp-cleanup-del")&&(o.disabled=r.size===0,o.textContent=this._t("btn.delete_selected",{n:r.size},`Delete selected (${r.size})`)),this._drawSpaghetti()})),t.querySelectorAll("tr[data-cid]").forEach(n=>{n.addEventListener("mouseenter",()=>{this._spagTableHoverCid=n.dataset.cid,this._drawSpaghetti()}),n.addEventListener("mouseleave",()=>{this._spagTableHoverCid=null,this._drawSpaghetti()})});const s=t.getElementById("wd-spag-canvas");s&&s.addEventListener("pointerdown",n=>{this._onGraphHoverInner(n.clientX,n.clientY,"wd-spag-canvas");const a=this._hoverNearest;if(a&&a.cid){const r=e.cleanup.selected;r.has(a.cid)?r.delete(a.cid):r.add(a.cid),this._render()}})}_onAction(t){const e=t.dataset.action,s=this.shadowRoot;if(e==="open-settings"){this._modal={type:"gear-settings",tab:this._gearTab||"prefs"},this._render();return}const n=this._devices[this._selIdx];if(!n)return;const a=n.entry_id;if(e.startsWith("sug-"))return this._onActSuggestions(e,t,n,a,s);if(e.startsWith("ml-"))return this._onActMl(e,t,n,a);if(e.startsWith("store-"))return this._onActStore(e,t,n,a,s);if(e.startsWith("auto-"))return this._onActAuto(e,t,n,a,s);if(e.startsWith("maint-"))return this._onActMaintenance(e,t,n,a,s);if(e.startsWith("pg-")&&e!=="pg-new"&&e!=="pg-edit"&&e!=="pg-suggest")return this._onActPlayground(e,t,n,a);if(e==="open-cycle"){const r=t.dataset.cid,o=t.dataset.mode==="review"?"review":"view";this._modal={type:"cycle-detail",entryId:a,cycleId:r,loaded:!1,mode:o,curve:null,ml:(this._mlById||{})[r]||null,trim:{start:0,end:0},split:{offsets:[],profiles:[]},drag:null},this._profiles.length||this._fetchProfiles(a),this._render(),this._ws({type:`${y}/get_cycle_power_data`,entry_id:a,cycle_id:r}).then(i=>{this._modal&&this._modal.cycleId===r&&(this._modal.curve=i,this._modal.loaded=!0,this._modal.trim={start:0,end:i.full_duration_s||0},this._render(),i.profile_name&&this._fetchCycleProfileEnv(a,i.profile_name))}).catch(i=>this._showToast(this._t("toast.could_not_load_cycle",{error:i.message||i},"Could not load cycle: "+(i.message||i)),"error"))}else if(e==="cleanup-edit-cycle"){const r=t.dataset.cid;this._prevModal=this._modal,this._modal={type:"cycle-detail",entryId:a,cycleId:r,loaded:!1,mode:"view",curve:null,ml:(this._mlById||{})[r]||null,trim:{start:0,end:0},split:{offsets:[],profiles:[]},drag:null},this._profiles.length||this._fetchProfiles(a),this._render(),this._ws({type:`${y}/get_cycle_power_data`,entry_id:a,cycle_id:r}).then(o=>{this._modal&&this._modal.cycleId===r&&(this._modal.curve=o,this._modal.loaded=!0,this._modal.trim={start:0,end:o.full_duration_s||0},this._render(),o.profile_name&&this._fetchCycleProfileEnv(a,o.profile_name))}).catch(o=>this._showToast(this._t("toast.could_not_load_cycle",{error:o.message||o},"Could not load cycle: "+(o.message||o)),"error"))}else if(e==="open-profile"){const r=t.dataset.pname;this._prevModal=null;const o=(this._profiles||[]).find(i=>i.name===r)||{name:r};this._modal={type:"profile-panel",name:r,tab:"stats",loaded:!1,stats:o,env:null,phases:[],catalog:[],cleanup:null},this._render(),Promise.all([this._ws({type:`${y}/get_profile_envelope`,entry_id:a,profile_name:r}).catch(()=>({envelope:null})),this._ws({type:`${y}/get_profile_phases`,entry_id:a,profile_name:r}).catch(()=>({phases:[]})),this._ws({type:`${y}/get_phase_catalog`,entry_id:a}).catch(()=>({phases:[]}))]).then(([i,l,c])=>{!this._modal||this._modal.name!==r||(this._modal.env=i.envelope,this._modal.phases=(l.phases||[]).map(_=>({name:_.name,start:_.start,end:_.end})),this._modal.catalog=(c.phases||[]).map(_=>_.name),this._modal.loaded=!0,this._render())})}else if(e==="create-profile")this._modal={type:"create-profile"},this._render();else if(e==="setup-cta"){const r=t.dataset.ctaAction||"";let o={};try{o=JSON.parse(t.dataset.ctaParams||"{}")}catch{}this._dispatchSetupCta(r,o)}else if(e==="setup-skip"){const r=t.dataset.step,o=t.dataset.snooze;if(r){let i;if(o==="never")i="never";else{const l=new Date;l.setDate(l.getDate()+14),i=l.toISOString()}this._setPref(r,i),this._reloadSetupStatus()}}else if(e==="hide-setup-card")this._setPref("setup_card_dismissed",!0),this._render();else if(e==="expand-setup")this._setPref("setup_card_dismissed",!1),this._render();else if(e==="set-settings-level"){const r=(t.type==="checkbox"?t.checked:t.dataset.slevel==="advanced")?"advanced":"basic";r!==this._pref("settings_level","basic")&&(this._snapshotFormToPending(s),this._setPref("settings_level",r),this._render())}else if(e==="pg-new"||e==="pg-edit"||e==="pg-suggest"){if(e==="pg-new")this._modal={type:"profile-group",orig:null,name:"",members:[]};else if(e==="pg-edit"){const r=t.dataset.gname,o=((this._profileGroups||{}).groups||[]).find(i=>i.name===r);this._modal={type:"profile-group",orig:r,name:r,members:o?[...o.members||[]]:[]}}else{const r=((this._profileGroups||{}).suggestions||[])[parseInt(t.dataset.idx,10)]||null;if(!r)return;this._modal={type:"profile-group",orig:r.existing_group||null,name:r.existing_group||"",members:[...r.members||[]]}}this._render(),this._ensureProfileEnvs(a,(this._profiles||[]).map(r=>r.name)).then(()=>{this._modal&&this._modal.type==="profile-group"&&this._render()})}else if(e==="rebuild-envelopes")this._kickAndTrack({type:`${y}/rebuild_envelopes`,entry_id:a},"rebuild-envelopes",async()=>{this._showToast(this._t("toast.envelopes_rebuilt",{},"Envelopes rebuilt")),await this._fetchProfiles(a)});else if(e==="rec-start")this._ws({type:`${y}/start_recording`,entry_id:a}).then(()=>(this._showToast(this._t("toast.recording_started",{},"Recording started")),this._fetchRecState(a))).then(()=>this._render()).catch(r=>this._showToast(this._t("toast.start_failed",{error:r.message||r},"Start failed: "+(r.message||r)),"error"));else if(e==="rec-stop")this._ws({type:`${y}/stop_recording`,entry_id:a}).then(()=>(this._showToast(this._t("toast.recording_stopped",{},"Recording stopped")),this._fetchRecState(a))).then(()=>this._render()).catch(r=>this._showToast(this._t("toast.stop_failed",{error:r.message||r},"Stop failed: "+(r.message||r)),"error"));else if(e==="rec-process-open")this._fetchProfiles(a).then(()=>{this._modal={type:"process-recording"},this._render()});else if(e==="rec-discard")this._modal={type:"confirm",title:this._t("modal.discard_recording_title",{},"Discard Recording"),message:this._t("modal.discard_recording_msg",{},"Discard the saved recording? This cannot be undone."),okLabel:this._t("btn.discard",{},"Discard"),onOk:async()=>{try{await this._ws({type:`${y}/discard_recording`,entry_id:a}),this._showToast(this._t("toast.recording_discarded",{},"Recording discarded")),await this._fetchRecState(a)}catch(r){this._showToast(this._t("toast.discard_failed",{error:r.message||r},"Discard failed: "+(r.message||r)),"error")}}},this._render();else if(e==="fb-confirm")this._ws({type:`${y}/resolve_feedback`,entry_id:a,cycle_id:t.dataset.cid,action:"confirm"}).then(()=>(this._showToast(this._t("toast.feedback_confirmed",{},"Feedback confirmed")),this._fetchFeedbacks(a))).then(()=>this._render()).catch(r=>this._showToast(this._t("msg.toast_error",{error:r.message||r},"Error: "+(r.message||r)),"error"));else if(e==="fb-ignore")this._ws({type:`${y}/resolve_feedback`,entry_id:a,cycle_id:t.dataset.cid,action:"ignore"}).then(()=>(this._showToast(this._t("toast.feedback_dismissed",{},"Feedback dismissed")),this._fetchFeedbacks(a))).then(()=>this._render()).catch(r=>this._showToast(this._t("msg.toast_error",{error:r.message||r},"Error: "+(r.message||r)),"error"));else if(e==="fb-correct")this._fetchProfiles(a).then(()=>{this._modal={type:"correct-feedback",cycleId:t.dataset.cid,detectedProfile:t.dataset.prof},this._render()});else if(e==="fb-dismiss-all")this._modal={type:"confirm",title:this._t("modal.dismiss_all_title",{},"Dismiss All Feedbacks"),message:this._t("modal.dismiss_all_msg",{count:this._feedbacks.length},`Dismiss all ${this._feedbacks.length} pending feedback requests?`),okLabel:this._t("modal.dismiss_all_ok",{},"Dismiss All"),onOk:async()=>{try{await this._ws({type:`${y}/dismiss_all_feedbacks`,entry_id:a}),this._showToast(this._t("toast.feedback_all_dismissed",{},"All feedbacks dismissed")),await this._fetchFeedbacks(a)}catch(r){this._showToast(this._t("msg.toast_error",{error:r.message||r},"Error: "+(r.message||r)),"error")}}},this._render();else if(e==="create-phase")this._modal={type:"create-phase",deviceType:t.dataset.dtype},this._render();else if(e==="edit-phase")this._modal={type:"edit-phase",phaseId:t.dataset.pid,phaseName:t.dataset.pname,phaseDesc:t.dataset.pdesc,isDefault:t.dataset.pisdefault==="true"},this._render();else if(e==="del-phase"){const r=t.dataset.pname,o=t.dataset.pid;this._modal={type:"confirm",title:this._t("modal.delete_phase_title",{},"Delete Phase"),message:this._t("modal.delete_phase_msg",{name:r},`Delete phase "${r}"?`),okLabel:this._t("btn.delete",{},"Delete"),onOk:async()=>{try{await this._ws({type:`${y}/delete_phase`,entry_id:a,phase_id:o}),this._showToast(this._t("toast.phase_deleted",{name:r},`Phase "${r}" deleted`)),await this._fetchPhases(a)}catch(i){this._showToast(this._t("msg.toast_delete_failed",{error:i.message||i},"Delete failed: "+(i.message||i)),"error")}}},this._render()}else if(e==="diag-refresh")this._fetchToolsData(a).then(()=>this._render());else if(e==="reprocess-history")this._modal={type:"confirm",title:this._t("modal.process_history_title",{},"Process History"),message:this._t("modal.process_history_msg",{},"Re-run matching, refresh suggestions, retrain ML (if enabled) and recompute cycle health across all stored cycles. This may take a while."),okLabel:this._t("modal.process_history_ok",{},"Process"),onOk:()=>this._kickAndTrack({type:`${y}/reprocess_history`,entry_id:a},"reprocess",async r=>{const o=r.count||0,i=[this._t("toast.processed_cycles",{n:o},o+" cycles")];r.suggestions!=null&&i.push(this._t("toast.processed_suggestions",{n:r.suggestions},r.suggestions+" suggestion(s)"));const l=r.ml_training&&r.ml_training.ok&&(r.ml_training.promoted||[]).length||0;l&&i.push(this._t("toast.processed_models",{n:l},l+" model(s) promoted")),this._showToast(this._t("toast.processed",{bits:i.join(", ")},"Processed "+i.join(", "))),await this._fetchToolsData(a)})},this._render();else if(e==="clear-debug")this._modal={type:"confirm",title:this._t("modal.clear_debug_title",{},"Clear Debug Data"),message:this._t("modal.clear_debug_msg",{},"Delete all stored debug traces?"),okLabel:this._t("status.clear",{},"Clear"),onOk:()=>this._busyRun("clear-debug",async()=>{try{const r=await this._ws({type:`${y}/clear_debug_data`,entry_id:a});this._showToast(this._t("toast.debug_cleared",{count:r.count||0},`Cleared ${r.count||0} debug traces`)),await this._fetchToolsData(a)}catch(r){this._showToast(this._t("msg.toast_error",{error:r.message||r},"Error: "+(r.message||r)),"error")}})},this._render();else if(e==="wipe-history")this._modal={type:"confirm",title:this._t("modal.wipe_all_title",{},"Wipe All Data"),message:this._t("modal.wipe_all_msg",{},"\u26A0\uFE0F This permanently deletes ALL cycles and profiles. This cannot be undone."),okLabel:this._t("modal.wipe_all_ok",{},"Wipe Everything"),onOk:()=>this._busyRun("wipe",async()=>{try{await this._ws({type:`${y}/wipe_history`,entry_id:a}),this._showToast(this._t("toast.all_wiped",{},"All data wiped")),this._cycles=[],this._profiles=[],await this._fetchToolsData(a)}catch(r){this._showToast(this._t("msg.toast_error",{error:r.message||r},"Error: "+(r.message||r)),"error")}})},this._render();else if(e==="export-config")this._ws({type:`${y}/export_config`,entry_id:a}).then(r=>{const o=new Blob([r.json_data],{type:"application/json"}),i=URL.createObjectURL(o),l=document.createElement("a");l.href=i,l.download=`washdata_export_${a.slice(0,8)}.json`,document.body.appendChild(l),l.click(),document.body.removeChild(l),URL.revokeObjectURL(i),this._showToast(this._t("toast.export_downloaded",{},"Export downloaded"))}).catch(r=>this._showToast(this._t("toast.export_failed",{error:r.message||r},"Export failed: "+(r.message||r)),"error"));else if(e==="export-select-open")this._modal={type:"export-select",inventory:null,loading:!0,sel:{cats:new Set,profiles:new Set,realIds:new Set,refIds:new Set},expanded:new Set},this._render(),(async()=>{let r=null;try{const o=await this._ws({type:`${y}/get_export_inventory`,entry_id:a});r=o&&o.manifest||null}catch(o){this._showToast(this._t("toast.export_failed",{error:o.message||o},"Export failed: "+(o.message||o)),"error")}if(!(!this._isActiveEntry(a)||!this._modal||this._modal.type!=="export-select")){if(!r){this._modal=null,this._render();return}this._modal.inventory=r,this._modal.sel=this._wizInitSel({categories:r},!1),this._modal.loading=!1,this._render()}})();else if(e==="cyc-select-toggle")this._selectMode=!this._selectMode,this._selectMode||this._cycleSel.clear(),this._render();else if(e==="cyc-auto-open")this._modal={type:"auto-label"},this._render();else if(e==="cyc-merge"){const r=Array.from(this._cycleSel);if(r.length<2)return;this._fetchProfiles(a).then(()=>{this._modal={type:"merge-cycles",ids:r},this._render()})}else if(e==="cyc-relabel"){const r=Array.from(this._cycleSel);if(!r.length)return;this._fetchProfiles(a).then(()=>{this._modal={type:"bulk-relabel",ids:r},this._render()})}else if(e==="cyc-load-more")this._busyRun("cyc-load-more",async()=>{try{await this._loadMoreCycles(a)}catch(r){this._showToast(this._t("toast.load_more_failed",{error:r.message||r},"Could not load more: "+(r.message||r)),"error")}});else if(e==="task-cancel"){const r=t.dataset.taskId;r&&(this._cancellingTasks.add(r),this._updateTaskPills(),this._ws({type:`${y}/cancel_task`,task_id:r}).catch(()=>{this._cancellingTasks.delete(r),this._updateTaskPills()}))}else if(e==="cyc-compare"){const r=Array.from(this._cycleSel);if(r.length<2)return;this._modal={type:"compare-cycles",ids:r,cycles:{},hidden:new Set,overlays:[],loaded:!1},this._profiles.length||this._fetchProfiles(a),this._render(),Promise.all(r.map(o=>this._ws({type:`${y}/get_cycle_power_data`,entry_id:a,cycle_id:o}).then(i=>({cid:o,r:i})).catch(()=>({cid:o,r:null})))).then(o=>{!this._modal||this._modal.type!=="compare-cycles"||(o.forEach(({cid:i,r:l})=>{l&&(this._modal.cycles[i]=l)}),this._modal.loaded=!0,this._render())})}else if(e==="cyc-bulk-del"){const r=Array.from(this._cycleSel);if(!r.length)return;this._deleteCyclesWithUndo(a,r)}else if(e==="retry-cycles")this._fetchCycles(a).then(()=>this._render());else if(e==="retry-profiles")Promise.all([this._fetchProfiles(a),this._fetchProfileGroups(a)]).then(()=>this._render());else if(e==="retry-suggestions")this._fetchSuggestions(a).then(()=>this._render());else if(e==="goto-suggestions")this._settingsSugOnly=!0,this._tab="settings",this._fetchTabData();else if(e==="goto-conflicts")this._tab="settings",this._fetchTabData();else if(e==="conf-goto-section"){const r=this._conflictKeysFromOpts();for(const o of mt)if((o.fields||(o.groups||[]).flatMap(l=>l.fields||[])).some(l=>r.has(l.key))){this._settingsSec=o.id,this._render();break}}else if(e==="toggle-settings-history")this._settingsHistoryOpen=!this._settingsHistoryOpen,this._render();else if(e==="settings-revert-key"){const r=t.dataset.key,o=JSON.parse(t.dataset.val);if(!r)return;const i=n.entry_id;this._ws({type:`${y}/set_options`,entry_id:i,options:{[r]:o}}).then(()=>this._ws({type:`${y}/get_options`,entry_id:i})).then(l=>(this._opts=l.options||{},this._optDefaults=l.defaults||{},this._fetchSettingsChangelog(i))).then(()=>{this._showToast(this._t("msg.toast_reverted",{key:this._t("setting."+r+".label",{},r)},"{key} reverted"),"success"),this._render()}).catch(l=>this._showToast(this._t("msg.toast_error",{error:l.message||l},"Error: "+(l.message||l)),"error"))}else if(e==="toggle-log-drawer"){this._logOpen=!this._logOpen;try{localStorage.setItem("wd-log-open",this._logOpen?"1":"0")}catch{}this._render(),this._logOpen&&this._fetchLogs().then(()=>{this._logOpen&&this._render()})}else if(e==="open-advanced"){const r=t.dataset.sub;r&&(this._panelSubtab=r),this._tab="advanced",this._render(),this._panelSubtab==="diagnostics"&&!this._diag?this._fetchToolsData(a).then(()=>{this._tab==="advanced"&&this._render()}):this._panelSubtab==="logs"?this._fetchLogs().then(()=>{this._tab==="advanced"&&this._render()}):this._panelSubtab==="maintenance"?this._fetchMaintenance(a).then(()=>{this._tab==="advanced"&&this._render()}):this._panelSubtab==="ml"&&this._fetchTabData()}else if(e==="add-device")this._navigate(`/config/integrations/integration/${y}`);else if(e==="goto-feedbacks")this._tab="history",this._cycleFilter={...this._cycleFilter,status:"needs_review"},this._fetchTabData();else if(e==="goto-recording")this._tab="status",this._fetchTabData();else if(e==="logs-refresh")this._fetchLogs().then(()=>this._render());else if(e==="logs-export")this._ws({type:`${y}/get_logs`,limit:500}).then(r=>{const o=(r.logs||[]).map(_=>`${new Date(_.ts*1e3).toISOString()} ${_.level} ${_.msg}`).join(`
`),i=new Blob([o],{type:"text/plain"}),l=URL.createObjectURL(i),c=document.createElement("a");c.href=l,c.download=`washdata_logs_${Date.now()}.txt`,document.body.appendChild(c),c.click(),document.body.removeChild(c),URL.revokeObjectURL(l),this._showToast(this._t("toast.logs_exported",{},"Logs exported"))}).catch(r=>this._showToast(this._t("toast.export_failed",{error:r.message||r},"Export failed: "+(r.message||r)),"error"));else if(e==="import-config-open")this._modal={type:"import-wizard",step:"input",jsonText:"",manifest:null,error:null,sel:{cats:new Set,profiles:new Set,realIds:new Set,refIds:new Set},expanded:new Set,mode:"merge",cycleDest:"reference",conflicts:{}},this._render();else if(e==="hist-import-open")this._modal={type:"history-import",step:"input",csvText:"",days:10,token:null,scanTaskId:null,applyTaskId:null,result:null,accept:new Set,done:null,error:null},this._render();else if(e==="import-config-raw")this._modal={type:"import-config"},this._render();else if(e==="save-prefs"){const r=s.getElementById("wd-pref-tab")?.value||"",o=!!s.getElementById("wd-pref-debug")?.checked,i=s.getElementById("wd-pref-expected")?!!s.getElementById("wd-pref-expected").checked:!0,l=!!s.getElementById("wd-pref-raw")?.checked,c=s.getElementById("wd-pref-datefmt")?.value||"relative",_=s.getElementById("wd-pref-lang")?.value||"",p=parseFloat(s.getElementById("wd-pref-fontscale")?.value)||1,g={default_tab:r,show_debug:o,show_expected:i,show_raw:l,date_format:c,lang_override:_,font_scale:p};this._busyRun("save-prefs",async()=>{try{await this._ws({type:`${y}/set_user_prefs`,prefs:g}),this._panelCfg&&(this._panelCfg.prefs={...this._panelCfg.prefs||{},...g});const u=_||this._hass&&this._hass.locale&&this._hass.locale.language;await this._loadPanelLang(u),this._render(),this._showToast(this._t("toast.preferences_saved",{},"Preferences saved"))}catch(u){this._showToast(this._t("toast.save_failed",{error:u.message||u},"Save failed: "+(u.message||u)),"error")}})}else if(e==="save-panel"){const r={default_tab:s.getElementById("wd-ps-deftab")?.value||"status",hidden_tabs:Array.from(s.querySelectorAll("[data-hidetab]")).filter(o=>o.checked).map(o=>o.dataset.hidetab)};this._busyRun("save-panel",async()=>{try{await this._ws({type:`${y}/set_panel_config`,panel:r}),this._panelCfg=await this._ws({type:`${y}/get_panel_config`}),this._tabInitialized=!0,this._applyPanelConfig(),this._showToast(this._t("toast.panel_settings_saved",{},"Panel settings saved"))}catch(o){this._showToast(this._t("msg.toast_save_failed",{error:o.message||o},"Save failed: "+(o.message||o)),"error")}})}else if(e==="pause-cycle")this._ws({type:`${y}/pause_cycle`,entry_id:a}).then(r=>{if(r&&r.ok===!1){this._showToast(this._t("toast.pause_no_cycle",{},"No active cycle to pause"),"error");return}return this._showToast(this._t("toast.cycle_paused",{},"Cycle paused")),this._fetchAll()}).catch(r=>this._showToast(this._t("toast.pause_failed",{error:r.message||r},"Pause failed: "+(r.message||r)),"error"));else if(e==="resume-cycle")this._ws({type:`${y}/resume_cycle`,entry_id:a}).then(r=>{if(r&&r.ok===!1){this._showToast(this._t("toast.resume_no_cycle",{},"No paused cycle to resume"),"error");return}return this._showToast(this._t("toast.cycle_resumed",{},"Cycle resumed")),this._fetchAll()}).catch(r=>this._showToast(this._t("msg.toast_resume_failed",{error:r.message||r},"Resume failed: "+(r.message||r)),"error"));else if(e==="terminate-cycle")this._modal={type:"confirm",title:this._t("modal.force_stop_title",{},"Force Stop Cycle"),message:this._t("modal.force_stop_msg",{},"Force-stop the active cycle now? The cycle will be saved as interrupted."),okLabel:this._t("btn.force_stop",{},"Force Stop"),onOk:async()=>{try{await this._ws({type:`${y}/terminate_cycle`,entry_id:a}),this._showToast(this._t("toast.cycle_force_stopped",{},"Cycle force-stopped")),await this._fetchAll()}catch(r){this._showToast(this._t("msg.toast_force_stop_failed",{error:r.message||r},"Force stop failed: "+(r.message||r)),"error")}}},this._render();else if(e==="save-rbac"){const r=!!s.getElementById("wd-rbac-enabled")?.checked,o=s.getElementById("wd-rbac-default")?.value||"none",i={};s.querySelectorAll("[data-rbacuser]").forEach(l=>{const c=l.dataset.rbacuser,_=l.dataset.rbacdev,p=l.value;i[c]||(i[c]={default:"none",devices:{}}),_==="__default__"?i[c].default=p:p&&p!=="inherit"&&(i[c].devices[_]=p)}),this._busyRun("save-rbac",async()=>{try{await this._ws({type:`${y}/set_panel_config`,rbac:{enabled:r,default_level:o,users:i}}),this._panelCfg=await this._ws({type:`${y}/get_panel_config`}),this._showToast(this._t("toast.access_saved",{},"Access control saved"))}catch(l){this._showToast(this._t("msg.toast_save_failed",{error:l.message||l},"Save failed: "+(l.message||l)),"error")}})}}_onActSuggestions(t,e,s,n,a){if(t==="sug-apply-all"){const r=this._suggestions.map(o=>o.key);this._busyRun("save-settings",async()=>{try{await this._ws({type:`${y}/apply_suggestions`,entry_id:n,keys:r}),this._showToast(this._t("toast.suggestions_applied",{},"Suggestions applied; integration reloading")),await this._fetchSuggestions(n);const o=await this._ws({type:`${y}/get_options`,entry_id:n});this._opts=o.options||{},this._optDefaults=o.defaults||{},this._prevOpts=null,this._cascadePending={},this._preCascadeOpts=null}catch(o){this._showToast(this._t("toast.apply_failed",{error:o.message||o},"Apply failed: "+(o.message||o)),"error")}})}else t==="sug-show-all"?(this._settingsSugOnly=!1,this._render()):t==="sug-dismiss"?(this._settingsSugOnly=!1,this._busyRun("save-settings",async()=>{try{await this._ws({type:`${y}/clear_suggestions`,entry_id:n}),this._suggestions=[],this._showToast(this._t("toast.suggestions_dismissed",{},"Suggestions dismissed"))}catch(r){this._showToast(this._t("toast.error",{error:r.message||r},"Error: "+(r.message||r)),"error")}})):t==="sug-unmute-all"?this._busyRun("save-settings",async()=>{try{const r=[...this._lockedSuggestions||[]],o=await Promise.allSettled(r.map(c=>this._ws({type:`${y}/set_suggestion_lock`,entry_id:n,key:c,locked:!1})));if(!this._isActiveEntry(n))return;const i=o.filter(c=>c.status==="rejected").length,l=o.slice().reverse().find(c=>c.status==="fulfilled");this._lockedSuggestions=l&&l.value&&l.value.locked_suggestions||[],await this._fetchSuggestions(n),i?this._showToast(this._t("toast.error",{error:`${i} suggestion(s) failed to unlock`},`${i} suggestion(s) failed to unlock`),"error"):this._showToast(this._t("msg.sug_unmuted_all",{},"Muted suggestions reset"),"success")}catch(r){this._showToast(this._t("toast.error",{error:r.message||r},"Error: "+(r.message||r)),"error")}}):t==="sug-analyze"&&this._busyRun("sug-analyze",async()=>{try{const r=await this._ws({type:`${y}/run_suggestion_analysis`,entry_id:n}),o=r&&r.count||0;this._showToast(o?this._t("toast.analysis_complete",{count:o},`Analysis complete: ${o} suggestion(s)`):this._t("toast.analysis_complete_none",{},"Analysis complete: no new suggestions")),await this._fetchSuggestions(n)}catch(r){this._showToast(this._t("toast.analysis_failed",{error:r.message||r},"Analysis failed: "+(r.message||r)),"error")}})}_onActMl(t,e,s,n){t==="ml-train-now"?this._kickAndTrack({type:`${y}/trigger_ml_training`,entry_id:n},"ml-train-now:"+n,async a=>{if(a&&a.ok){const r=(a.promoted||[]).length;this._showToast(r?this._t("toast.ml_training_promoted",{count:r},`Training complete: promoted ${r} model(s)`):this._t("toast.ml_training_no_improvement",{},"Training complete: baseline kept (no improvement)"))}else this._showToast(this._t("toast.ml_training_no_improvement",{},"Training complete: baseline kept (no improvement)"),"info");await this._loadMlTrainingStatus(n)}):t==="ml-revert-match"?this._busyRun("ml-revert-match",async()=>{try{await this._ws({type:`${y}/revert_matching_config`,entry_id:n}),this._showToast(this._t("toast.matching_reverted",{},"Matching weights reverted to defaults")),await this._loadMlTrainingStatus(n)}catch(a){this._showToast(this._t("toast.revert_failed",{error:a.message||a},"Revert failed: "+(a.message||a)),"error")}}):t==="ml-revert-models"&&this._busyRun("ml-revert-models",async()=>{try{await this._ws({type:`${y}/revert_ml_models`,entry_id:n}),this._showToast(this._t("toast.models_reverted",{},"On-device models reverted to baseline")),await this._loadMlTrainingStatus(n)}catch(a){this._showToast(this._t("msg.toast_revert_failed",{error:a.message||a},"Revert failed: "+(a.message||a)),"error")}})}_onActStore(t,e,s,n,a){if(t==="store-toggle-online"){const r=!!e.checked;this._busyRun("store-account",async()=>{try{const o=await this._ws({type:`${y}/store_set_online`,entry_id:n,enabled:r});this._constants.storeOnlineEnabled=!!(o&&o.enabled),this._constants.storeOnlineEnabled?(await this._loadStoreStatus(n),this._ensureStoreConnectListener()):(this._storeStatus={enabled:!1},this._storeConnected=!1)}catch(o){this._showToast(this._t("toast.store_error",{error:o.message||o},"Error: "+(o.message||o)),"error")}})}else if(t==="store-toggle-pref"){const r=e.dataset.pref,o=!!e.checked;if(!r)return;this._busyRun("store-account",async()=>{try{const i=await this._ws({type:`${y}/store_set_prefs`,entry_id:n,prefs:{[r]:o}});i&&i.prefs&&(this._constants={...this._constants,storePrefs:i.prefs})}catch(i){this._showToast(this._t("toast.store_error",{error:i.message||i},"Error: "+(i.message||i)),"error")}})}else if(t==="store-goto-identity")this._tab="settings",this._settingsSec="basic",this._fetchTabData(),requestAnimationFrame(()=>requestAnimationFrame(()=>{const r=this.shadowRoot&&this.shadowRoot.getElementById("wd-store-brand");r&&(r.focus(),r.scrollIntoView({block:"center"}))}));else if(t==="store-refresh-catalog")this._busyRun("store-refresh-catalog",async()=>{try{await this._ws({type:`${y}/store_refresh_catalog`,entry_id:n}),clearTimeout(this._brandSearchTimer),this._brandSearchTimer=null,this._catalog.brands=void 0,this._catalog.devices=void 0,this._catalog.forBrand=null,this._catalog.brandsFull=!1,this._catalog.brandPrefixes=[],this._catalogEntry=null,this._entityListCache&&(delete this._entityListCache.store_brand,delete this._entityListCache.store_model),this._showToast(this._t("toast.catalog_refreshed",{},"Community catalog refreshed"))}catch(r){this._showToast(this._t("toast.store_error",{error:r.message||r},"Error: "+(r.message||r)),"error")}});else if(t==="store-connect"){const r=this._constants.storeWebOrigin;if(!r){this._showToast(this._t("toast.store_unavailable",{},"The community store is not available."),"error");return}this._ensureStoreConnectListener(),window.open(r+"/connect.html?origin="+encodeURIComponent(location.origin),"washdata_connect","width=480,height=640")}else if(t==="store-disconnect")this._busyRun("store-account",async()=>{try{await this._ws({type:`${y}/store_disconnect`,entry_id:n}),await this._loadStoreStatus(n),this._showToast(this._t("toast.store_disconnected",{},"Disconnected from the community store"))}catch(r){this._showToast(this._t("toast.store_error",{error:r.message||r},"Error: "+(r.message||r)),"error")}});else if(t==="store-add-appliance"){const r=this._constants.storeWebOrigin;if(!r){this._showToast(this._t("toast.store_unavailable",{},"The community store is not available."),"error");return}this._ensureStoreConnectListener();const o=a.getElementById("wd-store-model"),i=a.getElementById("wd-store-brand"),l=new URLSearchParams({mode:"device",type:this._storeApplianceType(),brand:i&&i.value||this._opts.store_brand||"",model:o&&o.value||this._opts.store_model||"",origin:location.origin}).toString();window.open(r+"/create.html?"+l,"washdata_create","width=560,height=760")}else if(t==="store-add-brand"){const r=this._constants.storeWebOrigin;if(!r){this._showToast(this._t("toast.store_unavailable",{},"The community store is not available."),"error");return}this._ensureStoreConnectListener();const o=a.getElementById("wd-store-brand"),i=new URLSearchParams({mode:"brand",brand:o&&o.value||this._opts.store_brand||"",origin:location.origin}).toString();window.open(r+"/create.html?"+i,"washdata_create","width=560,height=760")}else if(t==="store-confirm-device"){const r=e.dataset.deviceId;this._busyRun("store-account",async()=>{try{const o=await this._ws({type:`${y}/store_confirm_device`,entry_id:n,device_id:r});if(o&&o.error){this._showToast(this._t("toast.store_error",{error:o.error},"Error: "+o.error),"error");return}const i=[...this._catalog.devices||[],...this._storeDevices||[],this._catalogEntry&&this._catalogEntry.device||null];for(const l of i)o&&l&&String(l.id)===String(r)&&(l.confirmCount=o.confirmCount,l.status=o.status);this._showToast(o&&o.status==="approved"?this._t("toast.device_approved",{},"Approved by the community"):this._t("toast.thanks_confirming",{},"Thanks for confirming")),this._render()}catch(o){this._showToast(this._t("toast.store_error",{error:o.message||o},"Error: "+(o.message||o)),"error")}})}else if(t==="store-rate-device"){const r=e.dataset.deviceId,o=parseInt(e.dataset.rating,10);if(!(o>=1&&o<=5))return;this._busyRun("store-account",async()=>{try{const i=await this._ws({type:`${y}/store_rate_device`,entry_id:n,device_id:r,rating:o});if(i&&i.error){this._showToast(this._t("toast.store_error",{error:i.error},"Error: "+i.error),"error");return}this._showToast(this._t("toast.rating_saved",{},"Quality rating saved"))}catch(i){this._showToast(this._t("toast.store_error",{error:i.message||i},"Error: "+(i.message||i)),"error")}})}else if(t==="store-search"){const r=a.getElementById("wd-store-q");this._storeSearch(r?r.value:"")}else if(t==="store-nav"){const r=e.dataset.view;r==="brands"?(this._storeView="brands",this._storeDevice=null,this._storeProfile=null,this._render()):r==="device"&&(this._storeView="device",this._storeProfile=null,this._render())}else if(t==="store-open-device"){const r=e.dataset.deviceId,o=(this._storeDevices||[]).find(i=>String(i.id)===String(r));if(!o)return;this._storeDevice=o,this._storeProfile=null,this._storeView="device",this._storeProfiles=[],this._storeCycles=[],this._storeLoading=!0,this._render(),this._ws({type:`${y}/store_get_profiles`,entry_id:n,device_id:o.id}).then(i=>{!this._isActiveEntry(n)||this._storeView!=="device"||(this._storeProfiles=i&&i.items||[])}).catch(()=>{this._isActiveEntry(n)&&(this._storeProfiles=[])}).finally(()=>{this._isActiveEntry(n)&&(this._storeLoading=!1,this._render())})}else if(t==="store-open-profile"){const r=e.dataset.profileId,o=(this._storeProfiles||[]).find(i=>String(i.id)===String(r));if(!o)return;this._storeProfile=o,this._storeView="profile",this._storeCycles=[],this._storeLoading=!0,this._render(),this._ws({type:`${y}/store_get_cycles`,entry_id:n,profile_id:o.id}).then(i=>{!this._isActiveEntry(n)||this._storeView!=="profile"||(this._storeCycles=i&&i.items||[])}).catch(()=>{this._isActiveEntry(n)&&(this._storeCycles=[])}).finally(()=>{this._isActiveEntry(n)&&(this._storeLoading=!1,this._render())})}else if(t==="store-onboard")this._storeQuery=(this._opts.store_brand||"").trim(),this._storeView="brands",this._storeDevice=null,this._storeProfile=null,this._tab="store",this._fetchTabData();else if(t==="store-toggle-dl-settings")this._dlSettings=!!e.checked;else if(t==="store-download-device"){const r=e.dataset.deviceId;if(!r)return;const o=!!this._dlSettings;this._busyRun("store-download-device",async()=>{try{const i=await this._ws({type:`${y}/store_download_device`,entry_id:n,device_id:r,include_settings:o});if(i&&(i.error||i.disabled)){const g=i.error||"unavailable";this._showToast(this._t("toast.store_download_failed",{error:g},"Download failed: "+g),"error");return}const l=i&&i.profiles_adopted||0,c=i&&i.cycles_imported||0,_=i&&i.settings_applied||0;if(!l&&!c&&!_){this._showToast(this._t("toast.store_download_nothing",{},"Nothing new to download - this setup is already on your device."),"info");return}await this._fetchProfiles(n),await this._fetchCycles(n);const p=i&&i.phases_applied||0;_?this._showToast(this._t("toast.store_device_downloaded_settings",{p:l,c,ph:p,s:_},`${l} program(s), ${c} recording(s), ${p} phase map(s), ${_} setting(s) added`)):p?this._showToast(this._t("toast.store_device_downloaded_phases",{p:l,c,ph:p},`${l} program(s), ${c} recording(s), ${p} phase map(s) added`)):this._showToast(this._t("toast.store_device_downloaded",{p:l,c},`${l} program(s), ${c} recording(s) added`))}catch(i){this._showToast(this._t("toast.store_download_failed",{error:i.message||i},"Download failed: "+(i.message||i)),"error")}})}else if(t==="store-import"){const r=e.dataset.cycleId,o=this._storeProfile&&this._storeProfile.program||"";this._modal={type:"store-import",cycleId:r,program:o,mode:"new"},this._render()}else if(t==="store-share-cycle"){const r=e.dataset.cid,o=e.dataset.prof||"";this._modal={type:"store-share",cycleId:r,program:o,profiles:null,deviceId:null},this._render(),this._loadShareProfiles()}else if(t==="store-share-device")this._modal={type:"store-share-device",selected:new Set,includePhases:new Set,includeSettings:!1,loading:!0},this._render(),(async()=>{try{const o=await this._ws({type:`${y}/get_shareable_cycles`,entry_id:n});this._shareableCycles=o&&o.items||[],this._sharePhasePrograms=o&&o.phase_programs||[],this._shareAllPrograms=o&&o.all_programs||[]}catch{this._shareableCycles=[],this._sharePhasePrograms=[],this._shareAllPrograms=[]}if(!this._isActiveEntry(n)||!this._modal||this._modal.type!=="store-share-device")return;const r=new Set;this._shareableByProgram().forEach(o=>o.cycles.forEach(i=>r.add(i.id))),this._modal.selected=r,this._modal.includePhases=new Set(this._sharePhasePrograms),this._modal.loading=!1,this._render()})();else if(t==="store-share-profile"){const r=e.dataset.prog||"";this._modal={type:"store-share-device",selected:new Set,includePhases:new Set,includeSettings:!1,loading:!0,focusProfile:r},this._render(),(async()=>{try{const i=await this._ws({type:`${y}/get_shareable_cycles`,entry_id:n});this._shareableCycles=i&&i.items||[],this._sharePhasePrograms=i&&i.phase_programs||[],this._shareAllPrograms=i&&i.all_programs||[]}catch{this._shareableCycles=[],this._sharePhasePrograms=[],this._shareAllPrograms=[]}if(!this._isActiveEntry(n)||!this._modal||this._modal.type!=="store-share-device")return;const o=new Set;this._shareableByProgram().filter(i=>i.program===r).forEach(i=>i.cycles.forEach(l=>o.add(l.id))),this._modal.selected=o,this._modal.includePhases=new Set((this._sharePhasePrograms||[]).filter(i=>i===r)),this._modal.loading=!1,this._render()})()}else if(t==="store-share-add-profile"){const r=this._modal,o=this._constants.storeWebOrigin;if(!o||!r){o||this._showToast(this._t("toast.store_unavailable",{},"The community store is not available."),"error");return}this._ensureStoreConnectListener();const i=new URLSearchParams({mode:"profile",device:r.deviceId||"",type:this._storeApplianceType(),brand:this._opts.store_brand||"",model:this._opts.store_model||"",origin:location.origin}).toString();window.open(o+"/create.html?"+i,"washdata_create","width=560,height=760")}}_onActAuto(t,e,s,n,a){if(t==="auto-new")this._navigate("/config/automation/edit/new");else if(t==="auto-new-started")this._newAutomationFromEvent("started");else if(t==="auto-new-finished")this._newAutomationFromEvent("finished");else if(t==="auto-delete"){const r=e.dataset.autoid,o=e.dataset.autoname||"this automation";this._modal={type:"confirm",title:this._t("modal.delete_automation_title",{},"Delete Automation"),message:this._t("modal.delete_automation_msg",{name:o},`Delete the automation "${o}" from Home Assistant? This cannot be undone.`),okLabel:this._t("btn.delete",{},"Delete"),onOk:async()=>{try{await this._hass.callApi("DELETE","config/automation/config/"+r),this._showToast(this._t("toast.automation_deleted",{},"Automation deleted")),await this._loadDeviceAutomations(n)}catch(i){this._showToast(this._t("toast.delete_failed",{error:i.message||i},"Delete failed: "+(i.message||i)),"error")}}},this._render()}else if(t==="auto-convert-legacy")this._convertLegacyActions();else if(t==="auto-remove-legacy")this._modal={type:"confirm",title:this._t("modal.remove_legacy_title",{},"Remove Legacy Actions"),message:this._t("modal.remove_legacy_msg",{},"Remove the legacy custom actions? They will stop firing on cycle events. This cannot be undone from the panel."),okLabel:this._t("btn.remove",{},"Remove"),onOk:async()=>{try{await this._ws({type:`${y}/set_options`,entry_id:n,options:{notify_actions:[]}}),this._opts={...this._opts,notify_actions:[]},this._showToast(this._t("toast.legacy_removed",{},"Legacy actions removed"))}catch(r){this._showToast(this._t("toast.delete_failed",{error:r.message||r},"Remove failed: "+(r.message||r)),"error")}}},this._render();else if(t==="auto-label"){const r=parseFloat(a.getElementById("wd-auto-label-threshold")?.value||"0.75");this._busyRun("auto-label",async()=>{try{await this._ws({type:`${y}/auto_label_cycles`,entry_id:n,confidence_threshold:r}),this._showToast(this._t("toast.auto_label_complete",{},"Auto-label complete")),await this._fetchCycles(n)}catch(o){this._showToast(this._t("toast.auto_label_failed",{error:o.message||o},"Auto-label failed: "+(o.message||o)),"error")}})}}_onActMaintenance(t,e,s,n,a){if(t==="maint-add"){const r=a.getElementById("wd-maint-type")?.value||"",o=a.getElementById("wd-maint-date")?.value||"",i=(a.getElementById("wd-maint-notes")?.value||"").trim();if(!r){this._showToast(this._t("toast.maint_add_failed",{error:this._t("lbl.event_type",{},"Event type")},"Could not add event: Event type"),"error");return}this._busyRun("maint-add",async()=>{try{const l={type:`${y}/add_maintenance_event`,entry_id:n,event_type:r};o&&(l.date=o),i&&(l.notes=i),await this._ws(l),await this._fetchMaintenance(n),this._showToast(this._t("toast.maint_added",{},"Maintenance event added")),this._render()}catch(l){this._showToast(this._t("toast.maint_add_failed",{error:l.message||l},"Could not add event: "+(l.message||l)),"error")}})}else if(t==="maint-delete"){const r=e.dataset.mid;this._modal={type:"confirm",title:this._t("modal.delete_maintenance_title",{},"Delete Maintenance Event"),message:this._t("modal.delete_maintenance_msg",{},"Delete this maintenance record? This cannot be undone."),okLabel:this._t("btn.delete",{},"Delete"),onOk:()=>this._busyRun("maint-delete",async()=>{try{await this._ws({type:`${y}/delete_maintenance_event`,entry_id:n,event_id:r}),await this._fetchMaintenance(n),this._showToast(this._t("toast.maint_deleted",{},"Maintenance event deleted"))}catch(o){this._showToast(this._t("toast.maint_delete_failed",{error:o.message||o},"Could not delete event: "+(o.message||o)),"error")}})},this._render()}else if(t==="maint-save-reminders"){const r={};a.querySelectorAll("[data-maint-rem]").forEach(o=>{const i=o.dataset.maintRem,l=parseInt(o.value,10);r[i]=!isNaN(l)&&l>0?l:0}),this._busyRun("maint-save-reminders",async()=>{try{await this._ws({type:`${y}/set_options`,entry_id:n,options:{maintenance_reminder_cycles:r}}),await this._fetchMaintenance(n),this._showToast(this._t("toast.reminders_saved",{},"Service reminders saved")),this._render()}catch(o){this._showToast(this._t("toast.reminders_save_failed",{error:o.message||o},"Could not save reminders: "+(o.message||o)),"error")}})}}_onActPlayground(t,e,s,n){if(t==="pg-analysis-tab"){const a=e.dataset.subtab||"history";a!==this._pgAnalysisTab&&(this._pgAnalysisTab=a,this._render(),requestAnimationFrame(()=>this._drawPlaygroundCanvases()))}else if(t==="pg-run-history")this._pgRunHistory();else if(t==="pg-batch-cancel"){this._pgBatchCancel=!0;const a=this._pgHistoryTaskId||this._pgSweepTaskId;a&&this._ws({type:`${y}/cancel_task`,task_id:a}).catch(()=>{})}else if(t==="pg-load-run"){const a=e.dataset.taskId;if(!a)return;const r=this._tasks[a];if(!r||r.kind!=="pg_history"&&r.kind!=="pg_sweep"){this._showToast(this._t("toast.pg_run_gone",{},"That run is no longer available."),"info");return}const o=r.entry_id;this._ws({type:`${y}/get_task_result`,task_id:a}).then(i=>{if(!this._isActiveEntry(o))return;const l=i&&i.result;if(!l){this._showToast(this._t("toast.pg_run_gone",{},"That run is no longer available."),"info");return}r.kind==="pg_history"?(this._pgHistory=l,this._pgAnalysisTab="history"):(this._pgSweepNew=l.error?null:l,this._pgAnalysisTab="sweep"),this._render()}).catch(()=>this._showToast(this._t("toast.pg_run_gone",{},"That run is no longer available."),"info"))}else if(t==="pg-open-cycle")this._pgSelectCycle(e.dataset.cid);else if(t==="pg-sweep-run2")this._pgRunSweep2();else if(t==="pg-sweep-apply2")this._pgApplySweepValue(e.dataset.val);else if(t==="pg-run"||t==="pg-load")this._pgLoad();else if(t==="pg-cancel-run")this._pgCancelRun();else if(t==="pg-reset-params")this._pgThreshStart=null,this._pgThreshStop=null,this._pgParamOverrides={},this._pgStressTail=!1,this._pgStressIdleW=null,this._render(),requestAnimationFrame(()=>this._pgDrawCanvas());else if(t==="pg-apply-settings")this._pgApplyToSettings();else if(t==="pg-load-live")this._pgLoadLive();else if(t==="pg-load-suggested")this._pgLoadSuggested("classic");else if(t==="pg-load-calibrated")this._pgLoadSuggested("ml");else if(t==="pg-preset-save")this._pgSavePreset();else if(t==="pg-preset-load"){const a=(this._pgPresets||[]).find(r=>r.name===this._pgPresetSel);a&&(this._pgApplyPresetValues(a.values),this._showToast(this._t("toast.pg_preset_loaded",{name:a.name},`Preset "${a.name}" loaded`)),this._render(),requestAnimationFrame(()=>this._pgDrawCanvas()))}else t==="pg-preset-delete"?this._pgDeletePreset():t==="pg-publish-one"&&this._pgPublishOne(e.dataset.pgkey)}async _onModalAction(t,e){const s=this.shadowRoot,n=this._devices[this._selIdx],a=n?n.entry_id:null,r=this._modal;if(t==="cancel"){if(r&&r.type==="cycle-detail"&&this._prevModal){const o=this._devices[this._selIdx];o?await this._closeCycleDetail(o.entry_id):(this._modal=null,this._render())}else this._modal=null,this._render();return}if(t==="ok"&&r&&r.onOk){const o=r.onOk;this._modal=null,this._render(),await o(),this._render();return}if(r&&r.type==="profile-group"){if(t==="pg-save"){const o=s.getElementById("wd-pg-name")?.value?.trim(),i=Array.from(s.querySelectorAll(".wd-pg-mem")).filter(l=>l.checked).map(l=>l.value);if(!o){this._showToast(this._t("toast.group_name_required",{},"Group name is required"),"error");return}if(i.length<2){this._showToast(this._t("toast.min_2_profiles",{},"Select at least 2 profiles for a group"),"error");return}await this._busyRun("pg-save",async()=>{try{r.orig&&r.orig!==o&&await this._ws({type:`${y}/rename_profile_group`,entry_id:a,name:r.orig,new_name:o}),await this._ws({type:`${y}/save_profile_group`,entry_id:a,name:o,members:i}),this._showToast(this._t("toast.group_saved",{},"Group saved")),this._modal=null,await this._fetchProfileGroups(a)}catch(l){this._showToast(this._t("msg.toast_save_failed",{error:l.message||l},"Save failed: "+(l.message||l)),"error")}});return}if(t==="pg-delete"&&r.orig){await this._busyRun("pg-save",async()=>{try{await this._ws({type:`${y}/delete_profile_group`,entry_id:a,name:r.orig}),this._showToast(this._t("toast.group_deleted",{},"Group deleted")),this._modal=null,await this._fetchProfileGroups(a)}catch(o){this._showToast(this._t("msg.toast_delete_failed",{error:o.message||o},"Delete failed: "+(o.message||o)),"error")}});return}}if(t.startsWith("store-import-")||t==="store-share-ok"||t.startsWith("wiz-")||t.startsWith("imp-")||t==="import-back"||t==="import-analyze"||t==="import-apply-ok")return this._onMActImport(t,e,r,a,s);if(t.startsWith("hist-"))return this._onMActHistoryImport(t,e,r,a,s);if(t.startsWith("sd-")||t==="store-share-device-ok")return this._onMActStoreShare(t,e,r,a);if(r&&r.type==="export-select"&&t==="export-generate"&&a){const o=this._wizSelectionPayload(r);await this._busyRun("export-select",async()=>{try{const i=await this._ws({type:`${y}/export_config_selective`,entry_id:a,selection:o}),l=new Blob([i.json_data],{type:"application/json"}),c=URL.createObjectURL(l),_=document.createElement("a");_.href=c,_.download=`washdata_export_${a.slice(0,8)}.json`,document.body.appendChild(_),_.click(),document.body.removeChild(_),URL.revokeObjectURL(c),this._modal=null,this._showToast(this._t("toast.export_selective_done",{},"Export downloaded"))}catch(i){this._showToast(this._t("toast.export_failed",{error:i.message||i},"Export failed: "+(i.message||i)),"error")}});return}if(t.startsWith("cyc-")||t.startsWith("trim-mode-"))return this._onMActCycleDetail(t,r,a,s);if(r&&r.type==="profile-panel"){if(t.indexOf("pp-tab-")===0){const o=t.slice(7);r.tab=o,this._render(),o==="cleanup"&&!r.cleanup&&this._ws({type:`${y}/get_profile_cycles`,entry_id:a,profile_name:r.name}).then(i=>{this._modal&&this._modal.name===r.name&&(this._modal.cleanup={cycles:i.cycles||[],selected:new Set},this._render())}).catch(()=>{this._modal&&(this._modal.cleanup={cycles:[],selected:new Set},this._render())});return}return this._onMActProfilePanel(t,e,r,a,s)}if(t==="label-ok"&&a){const o=s.getElementById("wd-label-profile"),i=o?o.value:"",l=i||null,c=i==="__create_new__"&&s.getElementById("wd-new-profile-name")?.value?.trim()||null;this._modal=null;try{await this._ws({type:`${y}/label_cycle`,entry_id:a,cycle_id:r.cycleId,profile_name:l||null,new_profile_name:c}),this._showToast(this._t("toast.cycle_labelled",{},"Cycle labelled")),await this._fetchCycles(a),await this._fetchProfiles(a),await this._fetchFeedbacks(a)}catch(_){this._showToast(this._t("toast.label_failed",{error:_.message||_},"Label failed: "+(_.message||_)),"error")}this._render()}else if(t==="create-profile-ok"&&a){const o=s.getElementById("wd-cp-name")?.value?.trim(),i=s.getElementById("wd-cp-cycle")?.value||null,l=parseFloat(s.getElementById("wd-cp-dur")?.value||0);if(this._modal=null,!o){this._showToast(this._t("toast.profile_name_required",{},"Profile name is required"),"error"),this._render();return}const c=!i&&l>0?l:null;try{await this._ws({type:`${y}/create_profile`,entry_id:a,name:o,reference_cycle:i||null,manual_duration_min:c}),this._showToast(this._t("toast.profile_created",{name:o},`Profile "${o}" created`)),await this._fetchProfiles(a)}catch(_){this._showToast(this._t("toast.create_failed",{error:_.message||_},"Create failed: "+(_.message||_)),"error")}this._render()}else if(t==="create-phase-ok"&&a){const o=s.getElementById("wd-ph-name")?.value?.trim(),i=s.getElementById("wd-ph-desc")?.value?.trim()||"";if(this._modal=null,!o){this._showToast(this._t("toast.phase_name_required",{},"Phase name is required"),"error"),this._render();return}try{await this._ws({type:`${y}/create_phase`,entry_id:a,device_type:r.deviceType||"",name:o,description:i}),this._showToast(this._t("toast.phase_created",{name:o},`Phase "${o}" created`)),await this._fetchPhases(a)}catch(l){this._showToast(this._t("msg.toast_create_failed",{error:l.message||l},"Create failed: "+(l.message||l)),"error")}this._render()}else if(t==="edit-phase-ok"&&a){const o=s.getElementById("wd-eph-name")?.value?.trim(),i=s.getElementById("wd-eph-desc")?.value?.trim()||"";if(this._modal=null,!o){this._showToast(this._t("toast.name_required",{},"Name required"),"error"),this._render();return}try{await this._ws({type:`${y}/update_phase`,entry_id:a,phase_id:r.phaseId,new_name:o,description:i}),this._showToast(this._t("toast.phase_updated",{},"Phase updated")),await this._fetchPhases(a)}catch(l){this._showToast(this._t("toast.update_failed",{error:l.message||l},"Update failed: "+(l.message||l)),"error")}this._render()}else if(t==="process-rec-ok"&&a){const o=s.getElementById("wd-pr-mode")?.value;let i=s.getElementById("wd-pr-profile")?.value?.trim();o==="existing_profile"&&(i=s.getElementById("wd-pr-profile-sel")?.value||i);const l=parseFloat(s.getElementById("wd-pr-head")?.value||0),c=parseFloat(s.getElementById("wd-pr-tail")?.value||0);if(this._modal=null,!i){this._showToast(this._t("msg.toast_profile_name_required",{},"Profile name is required"),"error"),this._render();return}try{await this._ws({type:`${y}/process_recording`,entry_id:a,profile_name:i,save_mode:o,head_trim:l,tail_trim:c}),this._showToast(this._t("toast.recording_saved",{},"Recording saved to profile")),await this._fetchRecState(a),await this._fetchProfiles(a)}catch(_){this._showToast(this._t("msg.toast_save_failed",{error:_.message||_},"Save failed: "+(_.message||_)),"error")}this._render()}else if(t==="correct-fb-ok"&&a){const o=s.getElementById("wd-fb-profile")?.value,i=parseFloat(s.getElementById("wd-fb-dur")?.value||0)||null;this._modal=null;try{await this._ws({type:`${y}/resolve_feedback`,entry_id:a,cycle_id:r.cycleId,action:"correct",corrected_profile:o,corrected_duration_min:i}),this._showToast(this._t("toast.correction_submitted",{},"Correction submitted")),await this._fetchFeedbacks(a)}catch(l){this._showToast(this._t("msg.toast_error",{error:l.message||l},"Error: "+(l.message||l)),"error")}this._render()}else if(t==="import-ok"&&a){const o=s.getElementById("wd-import-json")?.value;if(this._modal=null,!o?.trim()){this._showToast(this._t("toast.json_required",{},"JSON data is required"),"error"),this._render();return}try{await this._ws({type:`${y}/import_config`,entry_id:a,json_data:o}),this._showToast(this._t("toast.import_successful",{},"Import successful; integration reloading")),await this._fetchCycles(a)}catch(i){this._showToast(this._t("toast.import_failed",{error:i.message||i},"Import failed: "+(i.message||i)),"error")}this._render()}else if(t==="auto-run"&&a){const o=parseFloat(s.getElementById("wd-al-thr")?.value||"0.75");this._modal=null,this._render(),await this._busyRun("auto-label",async()=>{try{await this._ws({type:`${y}/auto_label_cycles`,entry_id:a,confidence_threshold:o}),this._showToast(this._t("msg.toast_auto_label_complete",{},"Auto-label complete")),await this._fetchCycles(a)}catch(i){this._showToast(this._t("msg.toast_auto_label_failed",{error:i.message||i},"Auto-label failed: "+(i.message||i)),"error")}})}else if(t==="merge-ok"&&a){const o=s.getElementById("wd-merge-prof")?.value||"",i=s.getElementById("wd-merge-newname")?.value?.trim()||null,l=r.ids||[];this._modal=null,this._render(),this._kickAndTrack({type:`${y}/apply_merge`,entry_id:a,cycle_ids:l,target_profile:o||null,new_profile_name:i},"cyc-merge",async()=>{this._showToast(this._t("toast.cycles_merged",{},"Cycles merged")),this._cycleSel.clear(),this._selectMode=!1,await this._fetchCycles(a),await this._fetchProfiles(a)})}else if(t==="bulk-relabel-ok"&&a){const o=s.getElementById("wd-relabel-profile")?.value||"",i=(r.ids||[]).slice();if(this._modal=null,this._render(),!i.length)return;await this._busyRun("cyc-relabel",async()=>{try{for(const l of i)await this._ws({type:`${y}/label_cycle`,entry_id:a,cycle_id:l,profile_name:o||null});this._showToast(this._t("toast.relabel_done",{count:i.length},`Relabelled ${i.length} cycle(s)`)),this._cycleSel.clear(),this._selectMode=!1,await this._fetchCycles(a),await this._fetchProfiles(a),await this._fetchFeedbacks(a)}catch(l){this._showToast(this._t("toast.relabel_failed",{error:l.message||l},"Relabel failed: "+(l.message||l)),"error")}})}}async _histUpload(t,e){const s=await this._ws({type:`${y}/history_import_begin`,entry_id:t}),n=Math.max(4096,s.chunk_bytes||512*1024);let a=0,r=0;for(;r<e.length;){let o=Math.min(e.length,r+n);if(o<e.length){const i=e.lastIndexOf(`
`,o);i>r&&(o=i+1)}await this._ws({type:`${y}/history_import_chunk`,entry_id:t,token:s.token,seq:a,text:e.slice(r,o)}),a+=1,r=o}return s.token}async _histStartScan(t,e,s){e.token=s,e.step="scan",e.error=null,this._render();const n=await this._ws({type:`${y}/start_history_import_scan`,entry_id:t,token:s});e.scanTaskId=n.task_id,this._addProvisionalTask(n.task_id,"history_import",t,0),this._tasksSubscribed||this._pollTaskGeneric(n.task_id),this._histAdopt(n.task_id)}async _histAdopt(t){if(!t)return;let e=(this._tasks||{})[t];if(!e||e.state==="running")try{e=await this._ws({type:`${y}/get_task_result`,task_id:t})}catch{return}e&&e.state&&e.state!=="running"&&this._histTaskFinished(e)}async _histTaskFinished(t){const e=this._modal;if(!(!e||e.type!=="history-import")&&!(t.id!==e.scanTaskId&&t.id!==e.applyTaskId)&&(this._histSettled=this._histSettled||new Set,!this._histSettled.has(t.id))){if(this._histSettled.add(t.id),t.id===e.scanTaskId){if(t.state==="error"){e.step="input",e.error=t.error||this._t("msg.hist_scan_failed",{},"Scanning failed."),this._render();return}try{const s=t.result?t:await this._ws({type:`${y}/get_task_result`,task_id:t.id});e.result=s.result||{},e.accept=new Set((e.result.segments||[]).filter(n=>n.accept).map(n=>n.index)),e.step="review",this._render(),requestAnimationFrame(()=>this._drawHistorySparklines())}catch(s){e.step="input",e.error=String(s&&s.message||s),this._render()}return}if(t.id===e.applyTaskId){if(t.state==="error"){e.error=t.error==="scan_expired"?this._t("msg.hist_scan_expired",{},"That scan is no longer available. Please scan again."):t.error||this._t("msg.hist_import_failed",{},"Import failed."),e.step="review",this._render();return}try{const n=t.result?t:await this._ws({type:`${y}/get_task_result`,task_id:t.id});e.done=n.result||{}}catch{e.done={}}e.step="done",this._render();const s=this._devices[this._selIdx];s&&(this._fetchCycles(s.entry_id),this._fetchProfiles(s.entry_id))}}}async _onMActHistoryImport(t,e,s,n,a){if(!(!s||s.type!=="history-import"||!n)){if(t==="hist-scan"){const r=a.getElementById("wd-hist-csv"),o=r?r.value:s.csvText||"";if(!o.trim()){this._showToast(this._t("toast.hist_csv_required",{},"Load a CSV file or paste its contents first"),"error");return}s.csvText=o,await this._busyRun("hist-import",async()=>{try{const i=await this._histUpload(n,o);await this._histStartScan(n,s,i)}catch(i){s.error=String(i&&i.message||i),this._render()}});return}if(t==="hist-recorder"){const r=a.getElementById("wd-hist-days"),o=Math.max(1,Math.min(14,parseInt(r&&r.value,10)||10));s.days=o,await this._busyRun("hist-import",async()=>{try{const i=await this._ws({type:`${y}/history_import_recorder`,entry_id:n,days:o});if(!i.rows){s.error=this._t("msg.hist_recorder_empty",{},"Home Assistant has no detailed history for this sensor in that window."),this._render();return}await this._histStartScan(n,s,i.token)}catch(i){s.error=String(i&&i.message||i),this._render()}});return}if(t==="hist-cancel-scan"){if(s.scanTaskId)try{await this._ws({type:`${y}/cancel_task`,task_id:s.scanTaskId})}catch{}s.step="input",s.scanTaskId=null,this._render();return}if(t==="hist-back"){s.step="input",s.result=null,s.error=null,this._render();return}if(t==="hist-toggle-all"){const r=s.result&&s.result.segments||[],o=r.every(i=>s.accept.has(i.index));s.accept=o?new Set:new Set(r.map(i=>i.index)),this._render(),requestAnimationFrame(()=>this._drawHistorySparklines());return}if(t==="hist-apply"){if(!s.accept||!s.accept.size)return;await this._busyRun("hist-apply",async()=>{try{const r=await this._ws({type:`${y}/apply_history_import`,entry_id:n,scan_task_id:s.scanTaskId,accept:[...s.accept].sort((o,i)=>o-i)});s.applyTaskId=r.task_id,this._addProvisionalTask(r.task_id,"history_import_apply",n,s.accept.size),this._tasksSubscribed||this._pollTaskGeneric(r.task_id),this._histAdopt(r.task_id)}catch(r){s.error=String(r&&r.message||r),this._render()}});return}if(t==="hist-goto-cycles"){this._modal=null,this._tab="history",this._cycleFilter={...this._cycleFilter||{},status:"imported"},this._fetchTabData();return}}}async _onMActImport(t,e,s,n,a){if(s&&s.type==="store-import"){if(t==="store-import-mode-new"){s.mode="new",this._render();return}if(t==="store-import-mode-merge"){s.mode="merge",this._render();return}if(t==="store-import-ok"){const r={type:`${y}/store_import_cycle`,entry_id:n,cycle_id:s.cycleId};if(s.mode==="merge"){const o=a.getElementById("wd-store-import-target")?.value||"";if(!o){this._showToast(this._t("toast.store_pick_profile",{},"Pick a profile to merge into"),"error");return}r.target_profile=o}else{const o=(a.getElementById("wd-store-import-name")?.value||"").trim()||s.program;if(!o){this._showToast(this._t("toast.store_name_required",{},"Enter a profile name"),"error");return}r.new_profile_name=o}await this._busyRun("store-import",async()=>{try{const o=await this._ws(r);if(o&&o.error){this._showToast(this._t("toast.store_import_failed",{error:o.error},"Import failed: "+o.error),"error");return}this._modal=null,this._showToast(this._t("toast.store_imported",{profile:o&&o.profile||""},`Imported into ${o&&o.profile||"profile"}`)),await this._fetchProfiles(n)}catch(o){this._showToast(this._t("toast.store_import_failed",{error:o.message||o},"Import failed: "+(o.message||o)),"error")}});return}}if(s&&s.type==="store-share"&&t==="store-share-ok"){const r=(a.getElementById("wd-store-share-prog")?.value||"").trim(),o=(a.getElementById("wd-store-share-desc")?.value||"").trim();if(!r){this._showToast(this._t("toast.store_pick_profile",{},"Pick a profile to share into"),"error");return}await this._busyRun("store-share",async()=>{try{const i=await this._ws({type:`${y}/store_upload_cycle`,entry_id:n,local_cycle_id:s.cycleId,program:r,description:o});if(i&&i.error){if(i.error==="no_appliance_declared")this._showToast(this._t("toast.store_no_appliance",{},"Set your appliance brand and model in Settings first."),"error");else{const l=i.detail?`${i.error} - ${i.detail}`:i.error;this._showToast(this._t("toast.store_share_failed",{error:l},"Share failed: "+l),"error")}return}this._modal=null,this._showToast(this._t("toast.store_shared",{},"Shared to the community store - pending review."))}catch(i){this._showToast(this._t("toast.store_share_failed",{error:i.message||i},"Share failed: "+(i.message||i)),"error")}});return}if(s&&(s.type==="export-select"||s.type==="import-wizard")){const r=s.type==="export-select"?{categories:s.inventory||{}}:s.manifest||{categories:{}};if(t==="wiz-toggle-all"){const o=r.categories||{},i=s.type==="import-wizard";let l=!0;this._wizCatOrder().filter(c=>o[c]&&o[c].present).forEach(c=>{i&&o[c].importable===!1||this._wizCatState(s,c,r).state!=="all"&&(l=!1)}),s.sel=l?{cats:new Set,profiles:new Set,realIds:new Set,refIds:new Set}:this._wizInitSel(r,i),this._render();return}if(t==="wiz-toggle-cat"){const o=e.dataset.cat,l=this._wizCatState(s,o,r).state!=="all";if(o==="profiles"){const c=r.categories.profiles&&r.categories.profiles.items||[];s.sel.profiles=new Set(l?c.map(_=>_.name):[])}else if(o==="real_cycles"||o==="reference_cycles"){const c=new Set;l&&(r.categories[o]&&r.categories[o].groups||[]).forEach(_=>_.cycles.forEach(p=>{p.id!=null&&c.add(String(p.id))})),o==="real_cycles"?s.sel.realIds=c:s.sel.refIds=c}else l?s.sel.cats.add(o):s.sel.cats.delete(o);this._render();return}if(t==="wiz-toggle-profile"){const o=e.dataset.name;s.sel.profiles.has(o)?s.sel.profiles.delete(o):s.sel.profiles.add(o),this._render();return}if(t==="wiz-toggle-cycgroup"){const o=e.dataset.cat,i=e.dataset.prof,l=o==="real_cycles"?s.sel.realIds:s.sel.refIds,c=this._wizGroupIds(r,o,i),_=c.length>0&&c.every(p=>l.has(p));c.forEach(p=>{_?l.delete(p):l.add(p)}),this._render();return}if(t==="wiz-toggle-cyc"){const o=e.dataset.cat,i=e.dataset.cid,l=o==="real_cycles"?s.sel.realIds:s.sel.refIds;l.has(i)?l.delete(i):l.add(i),this._render();return}if(t==="wiz-expand"){const o=e.dataset.key;s.expanded||(s.expanded=new Set),s.expanded.has(o)?s.expanded.delete(o):s.expanded.add(o),this._render();return}}if(s&&s.type==="import-wizard"){if(t==="import-back"){s.step="input",s.error=null,this._render();return}if(t==="imp-mode-merge"){s.mode="merge",this._render();return}if(t==="imp-mode-replace"){s.mode="replace",this._render();return}if(t==="imp-dest-reference"){s.cycleDest="reference",this._render();return}if(t==="imp-dest-real"){(s.manifest||{}).real_history_allowed!==!1&&(s.cycleDest="real_history",this._render());return}if(t==="imp-conflict"){s.conflicts[e.dataset.prof]=e.value;return}if(t==="import-analyze"&&n){const r=a.getElementById("wd-import-json"),o=r?r.value:s.jsonText||"";if(s.jsonText=o,!o.trim()){this._showToast(this._t("toast.json_required",{},"JSON data is required"),"error");return}s.step="analyze",s.error=null,this._render();try{const i=await this._ws({type:`${y}/analyze_import`,entry_id:n,json_data:o});if(!this._isActiveEntry(n)||!this._modal||this._modal.type!=="import-wizard")return;const l=i&&i.manifest||{};if(l.error){s.step="input",s.error=l.error,this._render();return}s.manifest=l,s.sel=this._wizInitSel(l,!0),s.conflicts={},(l.categories&&l.categories.profiles&&l.categories.profiles.items||[]).forEach(c=>{c.conflict&&(s.conflicts[c.name]="import_as_copy")}),s.step="select",this._render()}catch(i){if(!this._isActiveEntry(n)||!this._modal||this._modal.type!=="import-wizard")return;s.step="input",s.error=i&&i.message||String(i),this._render()}return}if(t==="import-apply-ok"&&n){const r=this._wizSelectionPayload(s);await this._busyRun("import-wizard",async()=>{try{const o=await this._ws({type:`${y}/import_config_selective`,entry_id:n,json_data:s.jsonText,selection:r,mode:s.mode,conflict_resolutions:s.conflicts,cycle_destination:s.cycleDest,apply_settings:!0}),i=o&&o.summary||{};this._modal=null,this._showToast(this._t("toast.import_selective_done",{profiles:i.profiles_imported||0,cycles:(i.real_cycles_imported||0)+(i.reference_cycles_imported||0)},`Imported ${i.profiles_imported||0} profile(s) and ${(i.real_cycles_imported||0)+(i.reference_cycles_imported||0)} cycle(s)`)),await this._fetchCycles(n),await this._fetchProfiles(n)}catch(o){this._showToast(this._t("toast.import_failed",{error:o.message||o},"Import failed: "+(o.message||o)),"error")}});return}}}async _onMActStoreShare(t,e,s,n){if(s&&s.type==="store-share-device"){if(t==="sd-toggle-cyc"){const a=e.dataset.cid;s.selected.has(a)?s.selected.delete(a):s.selected.add(a),this._render();return}if(t==="sd-toggle-prof"){const a=e.dataset.prog,r=this._shareableByProgram().find(o=>o.program===a);if(r){const o=r.cycles.every(i=>s.selected.has(i.id));r.cycles.forEach(i=>{o?s.selected.delete(i.id):s.selected.add(i.id)})}this._render();return}if(t==="sd-toggle-phases"){const a=e.dataset.prog;s.includePhases||(s.includePhases=new Set),s.includePhases.has(a)?s.includePhases.delete(a):s.includePhases.add(a),this._render();return}if(t==="sd-toggle-settings"){s.includeSettings=!s.includeSettings,this._render();return}if(t==="sd-toggle-consent"){s.consented=!s.consented,this._render();return}if(t==="sd-toggle-guide"){s.guideOpen=!s.guideOpen,this._render();return}if(t==="store-share-device-ok"){const a=new Map;(this._shareableCycles||[]).forEach(l=>a.set(l.id,(l.profile_name||"").trim()));const r=Array.from(s.selected).map(l=>({local_cycle_id:l,program:a.get(l)||""})).filter(l=>l.program);if(!r.length){this._showToast(this._t("toast.share_device_none_sel",{},"Select at least one cycle to share"),"error");return}const o=new Set(r.map(l=>l.program)),i=Array.from(s.includePhases||[]).filter(l=>o.has(l));await this._busyRun("store-share-device",async()=>{try{const l=await this._ws({type:`${y}/store_upload_device`,entry_id:n,items:r,include_phases:i,include_settings:!!s.includeSettings});if(l&&l.error){if(l.error==="no_appliance_declared")this._showToast(this._t("toast.store_no_appliance",{},"Set your appliance brand and model in Settings first."),"error");else{const u=l.detail?`${l.error} - ${l.detail}`:l.error;this._showToast(this._t("toast.store_share_failed",{error:u},"Share failed: "+u),"error")}return}const c=l&&l.cycle_ids&&l.cycle_ids.length||0,_=l&&l.errors&&l.errors.length||0,p=l&&l.duplicates||0,g=l&&l.created!=null?l.created:c;if(!c){const u=l&&l.errors&&l.errors[0]||l&&l.detail||"upload_failed";this._showToast(this._t("toast.store_share_failed",{error:u},"Share failed: "+u),"error");return}this._modal=null,_?this._showToast(this._t("toast.store_device_shared_partial",{n:c,failed:_},`Shared ${c} cycle(s); ${_} could not be uploaded.`),"info"):p&&!g?this._showToast(this._t("toast.store_device_shared_all_dup",{n:p},`All ${p} cycle(s) were already in the community store.`),"info"):p?this._showToast(this._t("toast.store_device_shared_some_dup",{created:g,dup:p},`Shared ${g} cycle(s); ${p} were already in the store.`)):this._showToast(this._t("toast.store_device_shared",{n:g},`Shared ${g} cycle(s) to the community store - pending review.`))}catch(l){this._showToast(this._t("toast.store_share_failed",{error:l.message||l},"Share failed: "+(l.message||l)),"error")}});return}}}async _onMActCycleDetail(t,e,s,n){if(e&&e.type==="cycle-detail"){if(t==="cyc-view"){e.mode="view",this._render();return}if(t==="cyc-trim"){e.mode="trim",(!e.trim||e.trim.end<=0)&&(e.trim={start:0,end:e.curve&&e.curve.full_duration_s||0}),this._render();return}if(t==="cyc-split"){e.mode="split",this._render();return}if(t==="cyc-review"){e.mode="review",this._render();return}if(t==="cyc-review-save"){const a=e.cycleId,r=n.getElementById("wd-cyc-rev-quality")?.value||"",o=!!n.getElementById("wd-cyc-rev-golden")?.checked,i=n.getElementById("wd-cyc-rev-notes")?.value||"",l=Array.from(n.querySelectorAll(".wd-cyc-rev-tag")).filter(p=>p.checked).map(p=>p.value),c=n.getElementById("wd-cyc-rev-label")?.value??"",_=e.curve&&e.curve.profile_name||"";await this._busyRun("cyc-review-save",async()=>{try{await this._ws({type:`${y}/set_ml_review`,entry_id:s,cycle_id:a,quality:r,golden:o,tags:l,notes:i}),c!==_&&await this._ws({type:`${y}/label_cycle`,entry_id:s,cycle_id:a,profile_name:c||null}),this._showToast(this._t("toast.review_saved",{},"Review saved")),await this._fetchCycles(s),c!==_&&await this._fetchFeedbacks(s),await this._loadMlIndex(s),this._modal&&this._modal.cycleId===a&&(this._modal.ml=(this._mlById||{})[a]||this._modal.ml)}catch(p){this._showToast(this._t("msg.toast_save_failed",{error:p.message||p},"Save failed: "+(p.message||p)),"error")}});return}if(t==="trim-mode-s"){e.timeMode="s",this._render();return}if(t==="trim-mode-clock"){e.timeMode="clock",this._render();return}if(t==="cyc-reset-trim"){e.trim={start:0,end:e.curve&&e.curve.full_duration_s||0},this._render();return}if(t==="cyc-clear-split"){e.split={offsets:[],profiles:[]},this._render();return}if(t==="cyc-label"){this._profiles.length||await this._fetchProfiles(s),this._modal={type:"label-cycle",cycleId:e.cycleId},this._render();return}if(t==="cyc-delete"){const a=e.cycleId;this._modal=null,this._render(),this._deleteCyclesWithUndo(s,[a]);return}if(t==="cyc-auto-split"){const a=parseInt(n.getElementById("wd-split-gap")?.value||"900",10);await this._busyRun("cyc-auto",async()=>{try{const r=await this._ws({type:`${y}/analyze_split`,entry_id:s,cycle_id:e.cycleId,gap_seconds:a});e.split.offsets=(r.split_offsets||[]).slice(),e.split.profiles=[],e.split.offsets.length||this._showToast(this._t("toast.no_split_found",{},"No idle gaps found to split on"),"info")}catch(r){this._showToast(this._t("toast.auto_detect_failed",{error:r.message||r},"Auto-detect failed: "+(r.message||r)),"error")}});return}if(t==="cyc-apply-trim"){const a=e.cycleId,r=e.trim.start,o=e.trim.end,i=e.curve&&e.curve.full_duration_s||0,l=i>0?Math.max(0,Math.round((o-r)/i*100)):100;if(l<50&&!confirm(this._t("msg.trim_destructive_confirm",{pct:l},`This keeps only ${l}% of the cycle and cannot be undone. Continue?`)))return;this._kickAndTrack({type:`${y}/trim_cycle`,entry_id:s,cycle_id:a,start_s:r,end_s:o},"cyc-trim-apply",async()=>{this._showToast(this._t("toast.cycle_trimmed",{},"Cycle trimmed")),await this._closeCycleDetail(s),await this._fetchCycles(s)});return}if(t==="cyc-apply-split"){const a=e.cycleId,r=e.split.offsets.slice(),o=e.split.profiles.slice();this._kickAndTrack({type:`${y}/apply_split`,entry_id:s,cycle_id:a,split_offsets:r,segment_profiles:o},"cyc-split-apply",async i=>{this._showToast(this._t("toast.split_complete",{count:(i.new_ids||[]).length},`Split into ${(i.new_ids||[]).length} cycles`)),await this._closeCycleDetail(s),await this._fetchCycles(s),await this._fetchProfiles(s)});return}}}async _onMActProfilePanel(t,e,s,n,a){if(s&&s.type==="profile-panel"){if(t==="pp-phase-add"){const r=s.env&&s.env.target_duration||(s.env&&s.env.avg&&s.env.avg.length?s.env.avg[s.env.avg.length-1][0]:600),o=s.phases.length?s.phases[s.phases.length-1].end:0,i=Math.min(o,r);s.phases.push({name:s.catalog[0]||"",start:i,end:Math.min(i+Math.max(60,r*.1),r)}),this._render();return}if(t==="pp-phase-rm"){const r=+(e&&e.dataset.idx||-1);r>=0&&(s.phases.splice(r,1),this._render());return}if(t==="pp-phase-save"){const r=s.phases.filter(o=>o.name).map(o=>({name:o.name,start:o.start,end:o.end}));await this._busyRun("pp-phase-save",async()=>{try{await this._ws({type:`${y}/set_profile_phases`,entry_id:n,profile_name:s.name,phases:r}),this._showToast(this._t("toast.phases_saved",{},"Phases saved"))}catch(o){this._showToast(this._t("msg.toast_save_failed",{error:o.message||o},"Save failed: "+(o.message||o)),"error")}});return}if(t==="pp-cleanup-del"){const r=s.cleanup?Array.from(s.cleanup.selected):[];if(!r.length)return;await this._busyRun("pp-cleanup-del",async()=>{try{for(const i of r)await this._ws({type:`${y}/delete_cycle`,entry_id:n,cycle_id:i});this._showToast(this._t("toast.cycles_deleted",{count:r.length},`Deleted ${r.length} cycle(s)`));const o=await this._ws({type:`${y}/get_profile_cycles`,entry_id:n,profile_name:s.name});this._modal&&(this._modal.cleanup={cycles:o.cycles||[],selected:new Set}),await this._fetchProfiles(n)}catch(o){this._showToast(this._t("msg.toast_delete_failed",{error:o.message||o},"Delete failed: "+(o.message||o)),"error")}});return}if(t==="pp-rename"){const r=a.getElementById("wd-pp-rename")?.value?.trim(),o=parseFloat(a.getElementById("wd-pp-dur")?.value||"0");if(!r){this._showToast(this._t("msg.toast_name_required",{},"Name required"),"error");return}try{await this._ws({type:`${y}/rename_profile`,entry_id:n,profile_name:s.name,new_name:r,manual_duration_min:o>0?o:null}),this._showToast(this._t("toast.profile_renamed",{},"Profile renamed")),s.name=r,await Promise.all([this._fetchProfiles(n),this._fetchProfileGroups(n)]),s.stats=(this._profiles||[]).find(i=>i.name===r)||s.stats,this._render()}catch(i){this._showToast(this._t("toast.rename_failed",{error:i.message||i},"Rename failed: "+(i.message||i)),"error")}return}if(t==="pp-rebuild"){this._kickAndTrack({type:`${y}/rebuild_envelopes`,entry_id:n},"pp-rebuild",async()=>{try{const r=await this._ws({type:`${y}/get_profile_envelope`,entry_id:n,profile_name:s.name});this._modal&&(this._modal.env=r.envelope)}catch{}this._showToast(this._t("toast.envelope_rebuilt",{},"Envelope rebuilt"))});return}if(t==="pp-delete"){this._deleteProfileWithUndo(n,s.name);return}}}_snapshotFormToPending(t){t&&t.querySelectorAll("#wd-settings-form [data-opt], #wd-ml-form [data-opt]").forEach(e=>{const s=e.dataset.opt,n=kt[s],a=n&&n.type||e.dataset.ftype||"text";if(e.type==="checkbox"){this._pendingSettings[s]=e.checked;return}if(a==="checkboxlist"){this._pendingSettings[s]=this._collectCheckboxlist(e,s);return}if(a==="entitylist"){this._pendingSettings[s]=Array.from(e.querySelectorAll(".wd-pill")).map(r=>r.dataset.val).filter(Boolean);return}if(a==="timerlist"){this._pendingSettings[s]=Array.from(e.querySelectorAll(".wd-timer-row")).map(r=>({offset_minutes:parseFloat(r.querySelector('[data-field="offset_minutes"]').value)||0,message:(r.querySelector('[data-field="message"]').value||"").trim(),auto_pause:r.querySelector('[data-field="auto_pause"]').checked})).filter(r=>r.offset_minutes>0);return}if(a==="number"){const r=String(e.value).trim();if(r===""){n&&n.clearable?this._pendingSettings[s]=null:delete this._pendingSettings[s];return}const o=parseFloat(r);isNaN(o)||(this._pendingSettings[s]=o);return}if(a==="list"){this._pendingSettings[s]=String(e.value).split(",").map(r=>r.trim()).filter(Boolean);return}if(a==="intlist"){this._pendingSettings[s]=Qt(e.value);return}if(a==="json"){const r=String(e.value).trim();if(!r){this._pendingSettings[s]=[];return}try{this._pendingSettings[s]=JSON.parse(r)}catch{}return}if(a==="entity"||a==="device"){const r=String(e.value).trim();this._pendingSettings[s]=r||null;return}this._pendingSettings[s]=e.value})}_collectCheckboxlist(t,e){const s=Array.from(t.querySelectorAll("[data-choice]")).filter(a=>a.checked).map(a=>a.dataset.choice);if(s.length)return s;const n=kt[e];return n&&Array.isArray(n.def)&&n.def.length?n.def.slice():s}_conflictKeysForOpts(t,e){const s=Object.assign({},e||{},t),n=new Set;for(const a of qt)if(a.check(s))for(const r of Object.keys(a.fieldErrors(s)))n.add(r);return n}_conflictCountForOpts(t,e){return this._conflictKeysForOpts(t,e).size}_conflictKeysFromOpts(){return this._conflictKeysForOpts(Object.assign({},this._opts,this._pendingSettings),this._optDefaults)}_readSettingsFormValues(t){const e=Object.assign({},this._optDefaults,this._opts,this._pendingSettings);return t&&t.querySelectorAll("#wd-settings-form [data-opt]").forEach(s=>{const n=s.dataset.opt;if(s.type==="checkbox"){e[n]=s.checked;return}if(s.dataset.ftype==="checkboxlist"){e[n]=this._collectCheckboxlist(s,n);return}const a=parseFloat(s.value);isNaN(a)?s.value!==""&&(e[n]=s.value):e[n]=a}),e}_liveValidateSettings(t){if(!t)return{};const e=this._readSettingsFormValues(t),s=t.getElementById("wd-settings-form");if(!s)return{};const n={};for(const r of this._suggestions||[])r.key!=null&&r.suggested!=null&&(n[r.key]=+r.suggested);const a={};for(const r of qt){if(!r.check(e))continue;const o=r.fieldErrors(e);for(const[i,l]of Object.entries(o)){const c=n[i],_=c!=null&&!r.check({...e,[i]:c})?{...l,suggFix:c}:l;(a[i]=a[i]||[]).push(_)}}return s.querySelectorAll("[data-cerr]").forEach(r=>{const o=r.dataset.cerr,i=a[o],l=s.querySelector(`.wd-field[data-field="${o}"]`);!i||!i.length?(r.hidden=!0,r.innerHTML="",l&&l.classList.remove("wd-has-conflict")):(r.hidden=!1,l&&l.classList.add("wd-has-conflict"),r.innerHTML=i.map(c=>{const _=this._t(c.msgKey,c.msgVars,c.msgFb);let p="";if(c.suggFix!=null){const g=+c.suggFix.toFixed(2);p=`<span class="wd-conflict-sug-note">${this._t("conflict.suggestion_resolves",{val:g},`Stage the pending suggestion (${g}) below to fix this`)}</span>`}else if(c.fixVal!=null&&!isNaN(+c.fixVal)){const g=Number.isInteger(c.fixVal)?c.fixVal:+c.fixVal.toFixed(2);p=`<button type="button" class="wd-conflict-fix" data-ckey="${o}" data-cval="${c.fixVal}">${this._t("conflict.use_fix",{val:g},`Use ${g}`)}</button>`}return`<div class="wd-conflict-row">\u26A0 ${d(_)}${p}</div>`}).join(""))}),a}_cascadeConflictFix(t,e,s){const n=new Set;for(let a=0;a<10;a++){const r=this._liveValidateSettings(t);let o=!1;for(const[i,l]of Object.entries(r)){if(i===s)continue;const c=l.find(p=>p.fixVal!=null&&!isNaN(+p.fixVal)&&+p.fixVal>0);if(!c)continue;const _=e.querySelector(`[data-opt="${i}"]`);_?_.value=c.fixVal:(this._preCascadeOpts==null&&(this._preCascadeOpts=JSON.parse(JSON.stringify(this._opts||{}))),this._opts={...this._opts,[i]:c.fixVal},(this._cascadePending??={})[i]=c.fixVal),n.add(i),o=!0;break}if(!o)break}if(this._liveValidateSettings(t),this._snapshotFormToPending(t),n.size>0){const a=n.size,r=a>1?"s":"";this._showToast(this._t("conflict.cascade_toast",{n:a},`Other settings adjusted for consistency: ${a}`),"success")}}async _saveSettings(){const t=this.shadowRoot,e=this._devices[this._selIdx];if(!e)return;const s=Object.assign({},this._pendingSettings,this._cascadePending);this._invalidJson=null,t.querySelectorAll("[data-opt]").forEach(o=>{const i=o.dataset.opt,l=kt[i],c=l&&l.type||o.dataset.ftype||"text";if(o.type==="checkbox"){s[i]=o.checked;return}if(c==="checkboxlist"){s[i]=this._collectCheckboxlist(o,i);return}if(c==="entitylist"){s[i]=Array.from(o.querySelectorAll(".wd-pill")).map(p=>p.dataset.val).filter(Boolean);return}if(c==="timerlist"){s[i]=Array.from(o.querySelectorAll(".wd-timer-row")).map(p=>({offset_minutes:parseFloat(p.querySelector('[data-field="offset_minutes"]').value)||0,message:(p.querySelector('[data-field="message"]').value||"").trim(),auto_pause:p.querySelector('[data-field="auto_pause"]').checked})).filter(p=>p.offset_minutes>0);return}const _=o.value;if(c==="number"){const p=String(_).trim();if(p===""){l&&l.clearable?s[i]=null:delete s[i];return}const g=parseFloat(p);isNaN(g)||(s[i]=g);return}if(c==="list"){s[i]=String(_).split(",").map(p=>p.trim()).filter(Boolean);return}if(c==="intlist"){s[i]=Qt(_);return}if(c==="json"){const p=String(_).trim();if(!p){s[i]=[];return}try{s[i]=JSON.parse(p)}catch{this._invalidJson=i}return}if(c==="entity"||c==="device"){const p=String(_).trim();s[i]=p||null;return}s[i]=_});const n=this._optDefaults||{};for(const o of Object.keys(n))!(o in this._opts)&&o in s&&JSON.stringify(s[o])===JSON.stringify(n[o])&&delete s[o];if(this._invalidJson){this._showToast(this._t("toast.invalid_json",{key:this._invalidJson},`"${this._invalidJson}" is not valid JSON - fix it or clear the field before saving.`),"error");return}const a=this._liveValidateSettings(t),r=new Set(Object.keys(a));if(r.size>0){const o={},i={};for(const[l,c]of Object.entries(s)){if(r.has(l)){i[l]=c;continue}JSON.stringify(c)!==JSON.stringify((this._opts||{})[l])&&(o[l]=c)}this._pendingSettings={...this._pendingSettings,...i},Object.keys(o).length?await this._busyRun("save-settings",async()=>{try{await this._ws({type:`${y}/set_options`,entry_id:e.entry_id,options:o}),this._opts={...this._opts,...o},this._showToast(this._t("toast.saved_except_conflicts",{},"Saved. Fix the highlighted conflicts to save the rest."),"info")}catch(l){this._showToast(this._t("msg.toast_save_failed",{error:l.message||l},"Save failed: "+(l.message||l)),"error")}}):this._showToast(this._t("toast.settings_conflicts",{},"Fix the highlighted setting conflicts before saving."),"error");return}await this._busyRun("save-settings",async()=>{try{const o=JSON.parse(JSON.stringify(this._preCascadeOpts||this._opts||{}));if(await this._ws({type:`${y}/set_options`,entry_id:e.entry_id,options:s}),this._opts={...this._opts,...s},this._prevOpts=o,this._cascadePending={},this._preCascadeOpts=null,this._pendingSettings={},this._stagedSuggestions){try{await this._ws({type:`${y}/clear_suggestions`,entry_id:e.entry_id})}catch{}this._stagedSuggestions=!1,this._suggestions=[]}this._showToast(this._t("toast.settings_saved",{},"Settings saved; integration reloading"))}catch(o){this._showToast(this._t("msg.toast_save_failed",{error:o.message||o},"Save failed: "+(o.message||o)),"error")}})}}customElements.get("ha-washdata-panel")||customElements.define("ha-washdata-panel",he);
