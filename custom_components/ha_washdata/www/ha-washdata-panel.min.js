...
`));b=`<div class="wd-sug"><span class="wd-sug-chip wd-sug-chip-obs">\u{1F4A1} ${d(g("suggestion.both_agree",{},"WashData recommends"))}</span><span class="wd-sug-val">${d(p)}${n}</span>${x(p)}${z}</div>`}else{const L=$?dt($):"",z=S?dt(S):"",R=g("suggestion.observed_label",{},"Observed"),I=g("suggestion.calibrated_label",{},"Calibrated");let C="",P="";if(!isNaN(v)&&!isNaN(E)){const U=E>v;P=g(`suggestion.impact.${s}.${U?"higher":"lower"}`,{},""),C=g(`suggestion.impact.${s}.${U?"lower":"higher"}`,{},"")}const M=C?`<div class="wd-sug-impact-line">${d(C)}</div>`:"",F=P?`<div class="wd-sug-impact-line">${d(P)}</div>`:"";b=`<div class="wd-sug wd-sug-split"><div class="wd-sug-opt"><span class="wd-sug-chip wd-sug-chip-obs">\u{1F4A1} ${d(R)}</span><span class="wd-sug-val">${d(p)}${n}</span>${x(p)}${L}${M}</div><div class="wd-sug-opt"><span class="wd-sug-chip wd-sug-chip-cal">\u{1F916} ${d(I)}</span><span class="wd-sug-val">${d(f)}${n}</span>${x(f)}${z}${F}</div></div>`}}else if(p!=null){const v=$?dt($):"",E=t!=null&&t!==""?` <span style="opacity:.6;font-size:.9em">(now ${d(t)}${n})</span>`:"";b=`<div class="wd-sug"><span class="wd-sug-chip wd-sug-chip-obs">\u{1F4A1} ${d(g("suggestion.observed_label",{},"Observed"))}</span><span class="wd-sug-val">${d(p)}${n}</span>${E}${x(p)}${v}</div>`}else if(f!=null){const v=S?dt(S):"";b=`<div class="wd-sug"><span class="wd-sug-chip wd-sug-chip-cal">\u{1F916} ${d(g("suggestion.calibrated_label",{},"Calibrated"))}</span><span class="wd-sug-val">${d(f)}${n}</span>${x(f)}${v}</div>`}if(b&&s){const v=d(g("btn.mute_suggestion",{},"Stop suggesting this setting")),E=`<button type="button" class="wd-sug-lock" data-suglock="${s}" title="${v}" aria-label="${v}">\u{1F515}</button>`;b=b.replace(/<\/div>\s*$/,E+"</div>")}return`<div class="wd-field" data-field="${s}"><div class="wd-label-row"><label style="margin:0">${d(r)}</label>${o}${a}</div>${i}${h.hint?`<div class="wd-field-hint">${d(h.hint)}</div>`:""}<div class="wd-conflict-err" data-cerr="${s}" hidden></div><div class="wd-setting-note" data-cnote="${s}" hidden></div>${b}</div>`}function Vt(h,t){if(h==null||t==null)return!1;const e=parseFloat(h),s=parseFloat(t);return!isNaN(e)&&!isNaN(s)?Math.abs(e-s)<1e-6:String(h)===String(t)}const Te={min_power:"min_power",off_delay:"off_delay",smoothing_window:"smoothing",start_threshold_w:"hysteresis",stop_threshold_w:"hysteresis",start_energy_threshold:"start_energy",running_dead_zone:"dead_zone",profile_duration_tolerance:"duration_tolerance",profile_match_min_duration_ratio:"match_ratios",profile_match_max_duration_ratio:"match_ratios",progress_reset_delay:"progress_reset",completion_min_seconds:"min_duration",min_off_gap:"min_off_gap",start_duration_threshold:"start_duration",end_energy_threshold:"end_energy_thresh",end_repeat_count:"end_repeat",profile_match_threshold:"confidence",profile_unmatch_threshold:"confidence",auto_label_confidence:"confidence",learning_confidence:"confidence",no_update_active_timeout:"watchdog_timeout",anti_wrinkle_enabled:"anti_wrinkle",anti_wrinkle_max_power:"anti_wrinkle",anti_wrinkle_max_duration:"anti_wrinkle",anti_wrinkle_exit_power:"anti_wrinkle",anti_wrinkle_idle_timeout:"anti_wrinkle",anti_crease_finalize_ratio:"anti_wrinkle",sampling_interval:"sampling"};function de(h){let t=0,e=window.innerWidth,s=0,r=window.innerHeight;for(let n=h.parentElement;n;n=n.parentElement){const a=getComputedStyle(n),o=a.overflowX!=="visible",i=a.overflowY!=="visible";if(!o&&!i)continue;const l=n.getBoundingClientRect();o&&(l.left>t&&(t=l.left),l.right<e&&(e=l.right)),i&&(l.top>s&&(s=l.top),l.bottom<r&&(r=l.bottom))}return{left:t,right:e,top:s,bottom:r}}function dt(h,t){return`<span class="wd-tip">i<span class="wd-tip-pop">${t?Ee(t):""}<span class="wd-tip-txt">${d(h)}</span></span></span>`}function Ut(h,t,e=""){return`<label class="wd-switch-lbl"><span class="wd-switch"><input type="checkbox" ${h}><span class="wd-switch-slider"></span></span><span class="wd-switch-text wd-switch-text--plain">${d(t)}</span></label>${e||""}`}function Mt(h,t,e="",s=""){return`<div class="wd-field wd-field-switch"><div class="wd-switch-row">${Ut(h,t,e)}</div>${s?`<div class="wd-field-hint">${d(s)}</div>`:""}</div>`}function Ee(h){const t=s=>`<svg class="wd-dg" viewBox="0 0 200 90" preserveAspectRatio="xMidYMid meet">${s}</svg>`,e='<line class="ax" x1="8" y1="78" x2="192" y2="78"/>';switch(h){case"smoothing":return t(`${e}
    ...
    `}_htmlDeviceBar(){const t=this._isAdmin()?`<button class="wd-devcard wd-devadd" data-action="add-device" title="${d(this._t("btn.add_device_tip",{},"Add another WashData device"))}">${this._t("btn.add_device",{},"+ Add device")}</button>`:"";return this._devices.length<=1?t?`<div class="wd-devbar">${t}</div>`:"":`<div class="wd-devbar">${this._devices.map((e,s)=>{const r=e.is_user_paused?"user_paused":e.detector_state||"unknown",n=Rt.includes(r),a=!!e.recording,o=a?"var(--error-color, #f44336)":this._stateColor(r),i=a?this._t("status.recording",{},"Recording"):this._stateLabel(r),l=[],c=this._conflictCountForOpts(e.options||{},e.option_defaults||{});c&&l.push(`<span class="wd-dbadge conf">\u26A0 ${c}</span>`);const _=this._sugCountsForDevice(e).total;return _&&l.push(`<span class="wd-dbadge sug">\u{1F4A1} ${_}</span>`),e.feedback_count&&l.push(`<span class="wd-dbadge fb">\u{1F4AC} ${e.feedback_count}</span>`),`<button class="wd-devcard ${s===this._selIdx?"active":""}" data-idx="${s}">
        ...
        ${l.join("")}
      </button>`}).join("")}${t}</div>`}_htmlStatus(){const t=this._devices[this._selIdx];if(!t)return`<div class="wd-empty">${this._t("msg.no_device_selected",{},"No device selected.")}</div>`;const e=!!t.is_user_paused,s=e?"user_paused":t.detector_state||"unknown",r=!!t.recording,n=r?"var(--error-color, #f44336)":this._stateColor(s),a=r?this._t("status.recording",{},"Recording"):this._stateLabel(s),o=r||Rt.includes(s),i=t.cycle_progress_pct,l=t.time_remaining_s,c=t.current_program,_=!!t.manual_program,p=c?null:t.armed_program||null,f=c||p||"auto_detect",g=(this._profiles||[]).map(k=>k.name);c&&!g.includes(c)&&g.unshift(c),p&&!g.includes(p)&&g.unshift(p);const $=g.map(k=>`<option value="${d(k)}" ${f===k?"selected":""}>${d(k)}</option>`).join(""),S=c?_?this._t("badge.manual",{},"(manually selected)"):this._t("badge.auto",{},"(auto-detected)"):p?this._t("badge.armed",{},"(applies to the next cycle)"):"",b=S?`<span class="wd-prog-tag ${c?_?"manual":"auto":"manual"}">${S}</span>`:"",v=`<div class="wd-prog-ctl"><label>${this._t("lbl.program",{},"Program")}</label>${dt(this._t("lbl.program_tip",{},"Override which profile is matched to the current cycle. Auto-detect lets the integration pick the best match automatically. Pin a specific program to force-match it when auto-detect is wrong or you know what is running. Pick one before starting the appliance and it is applied as soon as the next cycle begins."))}
          <select id="wd-status-prog">
            <option value="auto_detect" ${f==="auto_detect"?"selected":""}>${this._t("status.auto_detect",{},"Auto-detect")}</option>
            ${$}
          </select>${b}</div>`,E=[];t.recording&&this._canEdit()&&E.push(`<div class="wd-attn-card"><span class="wd-attn-icon">\u25CF</span><div class="wd-attn-body"><div class="wd-attn-title">${this._t("msg.recording_in_progress",{},"Recording in progress")}</div><div class="wd-attn-sub">${this._t("msg.see_recorder",{},"See recorder widget below")}</div></div></div>`),t.feedback_count&&this._canEdit()&&E.push(`<button class="wd-attn-card" type="button" data-action="goto-feedbacks"><span class="wd-attn-icon">\u{1F4AC}</span><div class="wd-attn-body"><div class="wd-attn-title">${this._t("msg.feedback_cycles_pending",{n:t.feedback_count},`To review: ${t.feedback_count}`)}</div><div class="wd-attn-sub">${this._t("msg.review_to_cycles",{},"Open the Cycles review queue")}</div></div></button>`);const D=this._conflictKeysFromOpts();if(D.size&&this._canEdit()){const k=D.size,B=k>1?"s":"";E.push(`<button class="wd-attn-card" type="button" style="border-color:var(--error-color,#b71c1c)" data-action="goto-conflicts"><span class="wd-attn-icon">\u26A0</span><div class="wd-attn-body"><div class="wd-attn-title" style="color:var(--error-color,#b71c1c)">${this._t("conflict.attn_title",{n:k},`Setting conflicts: ${k}`)}</div><div class="wd-attn-sub">${this._t("conflict.attn_sub",{},"Fix conflicts before saving")}</div></div></button>`)}const L=this._sugCountsForDevice(t);if(L.total&&this._canEdit()){const k=L.total,B=[];L.classic&&B.push(this._t("lbl.n_classic_suggestions",{n:L.classic},`${L.classic} classic`)),L.ml&&B.push(this._t("lbl.n_ml_suggestions",{n:L.ml},`${L.ml} ML`)),E.push(`<button class="wd-attn-card" type="button" data-action="goto-suggestions"><span class="wd-attn-icon">\u{1F4A1}</span><div class="wd-attn-body"><div class="wd-attn-title">${this._t("lbl.n_tuning_suggestions",{n:k},`${k} tuning suggestion${k>1?"s":""}`)}</div><div class="wd-attn-sub">${B.join(" \xB7 ")} \xB7 ${this._t("msg.review_in_settings",{},"Review in Settings")}</div></div></button>`)}const z=E.length?`<div class="wd-attn">${E.join("")}</div>`:"",R=t.envelope_position,I=R!=null?Math.round(R*100):null,C=I!=null?` <span style="opacity:.75" title="${d(this._t("lbl.envelope_position_tip",{pct:I},`Position on the matched program's own recorded curve, ${I}%. Measured by aligning this cycle against the profile instead of counting time, so it stays right when a run is longer or shorter than usual. It refreshes while the appliance is quiet, so it can lag behind during an active phase.`))}">${this._t("lbl.envelope_position",{pct:I},`curve ${I}%`)}</span>`:"",P=o&&i!=null?`
    ...
    `:"",F=((this._powerData||{}).live||[]).length>1,U=this._pref("show_expected",!0),st=this._pref("show_raw_active",!1),tt=`<div class="wd-leg">
      ...
      ${this._pref("show_raw",!1)?`<label class="wd-leg-i"><input type="checkbox" data-statustoggle="show_raw_active" ${st?"checked":""}><span class="wd-leg-sw" style="background:#9e9e9e"></span> ${this._t("lbl.raw_socket",{},"Raw socket")}</label>`:""}
        ...
        <div class="wd-kv" style="margin-bottom:12px">
          ...
          <div class="wd-kv-item"><div class="wd-kv-val" style="font-size:1em;color:${k.ambiguous?"var(--warning-color,#ff9800)":"var(--success-color,#4caf50)"}">${k.ambiguous?this._t("status.ambiguous",{},"Ambiguous"):this._t("status.clear",{},"Clear")}</div><div class="wd-kv-lbl">${this._t("lbl.label",{},"Match")}</div></div>
        </div>
        ${V?`<table class="wd-table"><thead><tr><th>Profile</th><th>Conf</th><th>MAE</th><th>Corr</th><th>Duration</th></tr></thead><tbody>${V}</tbody></table>`:`<p class="wd-info">${this._t("msg.no_match_yet",{},"No match attempt yet - this populates during a running cycle.")}</p>`}
      </div>`}const y=[];this._canEdit()&&y.push(`<button class="wd-attn-card" type="button" data-action="open-advanced" data-sub="diagnostics"><span class="wd-attn-icon">\u{1FA7A}</span><div class="wd-attn-body"><div class="wd-attn-title">${this._t("hdr.logs_diagnostics",{},"Diagnostics")}</div><div class="wd-attn-sub">${this._t("msg.storage_diagnostics",{},"Storage stats, maintenance, export/import")}</div></div></button>`),y.push(`<button class="wd-attn-card" type="button" data-action="open-settings"><span class="wd-attn-icon">\u2699\uFE0F</span><div class="wd-attn-body"><div class="wd-attn-title">${this._t("settings.gear.title",{},"Settings")}</div><div class="wd-attn-sub">${this._isAdmin()?this._t("msg.preferences_admin",{},"Preferences, panel & access control"):this._t("msg.preferences_adv",{},"Preferences")}</div></div></button>`);const m=`<div class="wd-card"><div class="wd-card-title">${this._t("hdr.tools_and_data",{},"Tools & Data")}</div><div class="wd-attn" style="margin-bottom:0;margin-top:12px">${y.join("")}</div></div>`,u=(()=>{if(!this._canEdit())return"";const k=Rt.includes(s),B=k&&!e,V=e,N=k||e;return!B&&!V&&!N?"":`<div class="wd-cycle-ctrl" style="margin-top:0">
        ${V?`<button class="wd-btn wd-btn-sm wd-btn-primary" data-action="resume-cycle" title="${d(this._t("btn.resume_cycle_tip",{},"Resume the paused cycle"))}">${this._t("btn.resume_cycle",{},"Resume")}</button>`:""}
    ...
    `}_htmlSetupCard(t,e=!1){if(!t)return"";const{phase:s,message_key:r,message_params:n,cta_label_key:a,cta_action:o,secondary_label_key:i,secondary_action:l,skippable:c,dismissible:_,step_key:p}=t;if(s==="phase4")return`
        ...
        </div>`;const f=this._t(r,n||{},""),g=this._t(a,{},"Continue"),$=i?this._t(i,{},""):null,S=c?`
    ...
    </div>`}_htmlHistory(){const t=this._cycles||[],e=this._refCycles||[],s=e.concat(t),r=this._canEdit(),n=this._selectMode&&r,a=this._cycleSel,{col:o,dir:i}=this._cycleSort,{text:l,status:c}=this._cycleFilter,_=this._mlById||{},p=new Set((this._feedbacks||[]).map(u=>u.cycle_id)),f=u=>_[u.id],g=u=>{const k=f(u);return!!(k&&k.ml_review&&k.ml_review.reviewed_at)},$=u=>{const k=f(u);return!!(k&&k.ml_review&&k.ml_review.golden)},S=u=>{if(p.has(u.id))return!0;if(g(u))return!1;const k=f(u),B=k&&k.ml_quality_label;return["uncertain","review"].includes(B)||["force_stopped","interrupted"].includes(u.status)},x=s.filter(S).length;let b=s;if(l){const u=l.toLowerCase();b=b.filter(k=>(k.profile_name||k.matched_profile||"").toLowerCase().includes(u))}c==="unlabelled"?b=b.filter(u=>!u.profile_name&&!u.matched_profile):c==="needs_review"?b=b.filter(S):c==="imported"?b=b.filter(u=>u.is_reference):c&&(b=b.filter(u=>(u.status||"completed")===c));const v={date:u=>u.start_time?new Date(u.start_time).getTime():0,confidence:u=>u.match_confidence,duration:u=>u.duration,energy:u=>u.energy_kwh!=null?u.energy_kwh:u.energy_wh!=null?u.energy_wh/1e3:null,cost:u=>u.cost!=null?u.cost:-1,status:u=>u.status||"completed",profile:u=>(u.profile_name||u.matched_profile||"\uFFFF").toLowerCase()};b=ae(b,v[o]||v.date,i);const E=u=>u==="completed"?"var(--success-color, #4caf50)":u==="interrupted"?"var(--error-color, #f44336)":u==="force_stopped"?"var(--warning-color, #ff9800)":"var(--secondary-text-color)",D=u=>{if(!u.is_reference)return"";const k=u.cycle_origin==="backfill",B=k?this._t("badge.backfilled_tip",{},"Detected in imported power history. Shapes program matching only, not counted in stats."):this._t("badge.imported_tip",{},"Imported from the community store. Used for matching only, not counted in stats.");return` <span title="${d(B)}" style="color:var(--info-color,#2196f3)">${k?"\u{1F557}":"\u{1F4E5}"}</span>`},L=u=>$(u)?' <span title="'+d(this._t("badge.golden_cycle",{},"Recorded reference cycle"))+'" style="color:var(--warning-color,#ff9800)">\u2B50</span>':p.has(u.id)?' <span title="'+d(this._t("badge.feedback_requested",{},"Feedback requested"))+'" style="color:var(--info-color,#2196f3)">\u{1F4AC}</span>':g(u)?' <span title="'+d(this._t("badge.reviewed",{},"Reviewed"))+'" style="color:var(--success-color,#4caf50)">\u2713</span>':S(u)?' <span title="'+d(this._t("badge.needs_review",{},"Needs review"))+'" style="color:var(--error-color,#f44336)">\u25CF</span>':"",z=u=>{if(u.anomaly!=="overrun")return"";const k=u.overrun_ratio?" "+this._t("badge.overrun_ratio",{x:Number(u.overrun_ratio).toFixed(1)},`(${Number(u.overrun_ratio).toFixed(1)}x expected)`):"";return` <span title="${d(this._t("badge.overrun",{},"Ran longer than usual"))}${d(k)}" style="color:var(--warning-color,#ff9800)">\u23F1</span>`},R=u=>{if(u.anomaly!=="underrun")return"";const k=u.underrun_ratio?" "+this._t("badge.underrun_ratio",{pct:Math.round(u.underrun_ratio*100)},`(${Math.round(u.underrun_ratio*100)}% of expected)`):"";return` <span title="${d(this._t("badge.underrun",{},"Finished faster than usual"))}${d(k)}" style="color:var(--info-color,#2196f3)">\u26A1</span>`},I=u=>{if(!u.energy_anomaly||u.energy_anomaly==="none")return"";const k=u.energy_anomaly==="energy_spike",B=u.energy_z_score!=null?` (${u.energy_z_score>0?"+":""}${Number(u.energy_z_score).toFixed(1)}\u03C3)`:"",V=k?"badge.energy_spike":"badge.energy_low",N=k?"Higher energy than usual":"Lower energy than usual",j=k?"\u{1F53A}":"\u{1F53B}",q=k?"var(--error-color,#f44336)":"var(--info-color,#2196f3)";return` <span title="${d(this._t(V,{},N))}${d(B)}" style="color:${q}">${j}</span>`},C=u=>{const k=Array.isArray(u.artifacts)?u.artifacts.length:0;return k?` <span title="${d(this._t("badge.artifact_tip",{n:k},`${k} anomal${k>1?"ies":"y"} detected (e.g. door opened mid-cycle) \u2014 open to see them on the graph`))}" style="color:var(--warning-color,#ff9800)">\u26A0</span>`:""},P=u=>{const k=Array.isArray(u.restart_gaps)?u.restart_gaps.length:0;return k?` <span title="${d(this._t("badge.restart_gap_tip",{n:k},`${k} HA restart gap${k>1?"s":""} during this cycle \u2014 power trace has a hole`))}" style="color:var(--info-color,#2196f3)">\u21BB</span>`:""},M=this._hass&&this._hass.config&&this._hass.config.currency||"",F=u=>{if(u.cost==null)return"-";const k=`${u.cost.toFixed(2)}${M?" "+M:""}`;if(u.energy_price_mode==="dynamic"&&u.energy_price!=null){const B=this._t("col.cost_dynamic_tip",{price:u.energy_price.toFixed(4),cur:M},`Time-weighted: charged at the price in force during the cycle (effective ${u.energy_price.toFixed(4)} ${M}/kWh).`);return`<span title="${d(B)}" style="border-bottom:1px dotted var(--secondary-text-color)">${k}</span>`}return k},U=b.map(u=>{const k=u.profile_name||u.matched_profile,B=u.match_confidence!=null?u.match_confidence*100:null,V=u.status||"completed",N=u.energy_kwh!=null?u.energy_kwh:u.energy_wh!=null?u.energy_wh/1e3:null,j=n,q=j?`<input type="checkbox" class="wd-csel" ${a.has(u.id)?"checked":""} style="width:auto;margin:0">`:`<span class="wd-devdot" style="background:${E(V)}" title="${d(V)}"></span>`,G={completed:this._t("status.completed",{},"Completed"),interrupted:this._t("status.interrupted",{},"Interrupted"),force_stopped:this._t("status.force_stopped",{},"Force stopped"),active:this._t("status.active",{},"Active")}[V]||V,A=`${D(u)}${L(u)}${z(u)}${R(u)}${I(u)}${C(u)}${P(u)}`.trim();return`<tr data-cid="${d(u.id)}" data-selmode="${j?1:0}" style="cursor:pointer">
        <td style="width:26px;padding:6px 4px 6px 8px">${q}</td>
        <td>${k?d(k):`<span style="color:var(--secondary-text-color)">${this._t("lbl.unlabelled",{},"Unlabelled")}</span>`}</td>
        <td class="wd-tc-flags">${A}</td>
        <td><span style="color:${E(V)};font-size:.9em">${d(G)}</span></td>
        <td class="wd-tc-date">${rt(u.start_time)}</td>
        <td class="wd-tc-num">${_t(u.duration)}</td>
        <td class="wd-tc-num">${N!=null?yt(N):"-"}</td>
        <td class="wd-tc-num">${F(u)}</td>
        <td class="wd-tc-num">${B!=null?B.toFixed(0)+"%":"-"}</td>
      </tr>`}).join(""),st=`<thead><tr>
      ...
      <div class="wd-card">
        ...
        ${pt}
      </div>`;return(this._cyclesError?`<div class="wd-error-state"><span>${this._t("msg.fetch_error",{},"Failed to load data.")}</span><button class="wd-btn" type="button" data-action="retry-cycles">${this._t("btn.retry",{},"Retry")}</button></div>`:"")+y}_trendIcon(t){return t==="up"?`<span title="${d(this._t("trend.up",{},"Trending up"))}" style="color:var(--warning-color,#ff9800)">\u2191</span>`:t==="down"?`<span title="${d(this._t("trend.down",{},"Trending down"))}" style="color:var(--info-color,#2196f3)">\u2193</span>`:""}_profileCardHtml(t){const e=t.avg_duration?this._t("lbl.duration_avg",{v:Math.round(t.avg_duration/60)},`~${Math.round(t.avg_duration/60)}m avg`):this._t("lbl.no_duration",{},"no duration"),s=t.avg_energy!=null?` \xB7 ${yt(t.avg_energy)}/cycle`:"",r=t.avg_energy!=null&&t.cycle_count?` \xB7 <strong>${yt(t.avg_energy*t.cycle_count)}</strong> total`:"",n=this._hass&&this._hass.config&&this._hass.config.currency||"",a=t.avg_cost!=null?` \xB7 ${this._t("lbl.avg_cost",{},"Avg")} ${t.avg_cost.toFixed(2)}${n?" "+n:""}/${this._t("lbl.per_cycle_short",{},"cycle")}`:"",o=(this._profileHealth||{})[t.name],i=(this._profileTrends||{})[t.name];let l="";o&&o.health_status==="poor"?l=`<span class="wd-badge" style="color:var(--error-color,#f44336);background:rgba(244,67,54,.12)" title="${d(this._t("badge.poor_fit_tip",{},"Inconsistent match history \u2014 consider rebuilding this profile"))}">\u26A0 ${this._t("badge.poor_fit",{},"poor fit")}</span>`:o&&o.health_status==="fair"&&(l=`<span class="wd-badge" style="color:var(--warning-color,#ff9800);background:rgba(255,152,0,.12)" title="${d(this._t("badge.fair_fit_tip",{},"Moderate match consistency \u2014 some cycles assigned to this profile have lower confidence scores. Label more cycles or re-record the profile to improve accuracy."))}">${this._t("badge.fair_fit",{},"fair fit")}</span>`);let c="";if(i){const L=this._trendIcon(i.duration_trend),z=i.energy_trend?this._trendIcon(i.energy_trend):"";if(i.duration_trend!=="stable"||i.energy_trend==="up"){const R=[];if(i.duration_trend!=="stable"){const C=`${i.duration_slope_pct>0?"+":""}${i.duration_slope_pct}`;R.push(i.duration_trend==="up"?this._t("msg.duration_trend_up_tip",{pct:C},`Duration up (${C}%/cycle)`):this._t("msg.duration_trend_down_tip",{pct:C},`Duration down (${C}%/cycle)`))}if(i.energy_trend&&i.energy_trend!=="stable"){const C=`${i.energy_slope_pct>0?"+":""}${i.energy_slope_pct}`;R.push(i.energy_trend==="up"?this._t("msg.energy_trend_up_tip",{pct:C},`Energy up (${C}%/cycle)`):this._t("msg.energy_trend_down_tip",{pct:C},`Energy down (${C}%/cycle)`))}const I=R.join(", ")||this._t("msg.performance_trending",{},"Performance trending");c=`<span class="wd-badge" style="color:var(--secondary-text-color,#888)" title="${d(I)}">${L}${z||""}</span>`}}const _=this._constants&&this._constants.PROFILE_MIN_WARMUP_CYCLES||5,p=o&&o.cycle_count||0,g=p<_&&!t.is_imported?`<span class="wd-badge" title="${d(this._t("msg.warmup_detail",{needed:_},`This profile needs ${_} labelled cycles before auto-matching begins. Every confirmed cycle helps it learn.`))}" style="background:var(--info-color,#2196f3);color:#fff">${this._t("msg.warmup_badge",{done:p,needed:_},`Still learning (${p}/${_} cycles)`)}</span>`:"",$=t.is_imported?`<span class="wd-badge" title="${d(this._t("badge.imported_tip",{},"Imported from the community store. Used for matching only, not counted in stats."))}" style="background:var(--info-color,#2196f3);color:#fff">\u{1F4E5} ${this._t("status.imported",{},"Imported")}</span>`:"",S=(this._profileAdvisories||[]).find(L=>L&&L.profile===t.name&&L.code==="unmatchable"),x=S?`<span class="wd-badge" style="color:var(--error-color,#f44336);background:rgba(244,67,54,.12)" title="${d(this._t(S.message_key,S.message_params,S.message))}">\u26A0 ${this._t("badge.unmatchable",{},"can't be matched")}</span>`:"",b=(this._profileTerminal||{})[t.name];let v="";if(b&&b.quiet_before_s>0&&b.seen_in>=2){const L=Math.max(1,Math.round(b.quiet_before_s/60)),z=Number(b.event_watts||0).toFixed(0),R=Math.round(b.event_seconds||0),I=this._t("badge.quiet_tail_tip",{mins:L,secs:R,watts:z,seen:b.seen_in,measured:b.measured},`Near the end this program goes quiet for about ${L} min, then draws about ${z} W for ${R} s before finishing. Seen in ${b.seen_in} of ${b.measured} measured cycles: the appliance does not do it every run.`);v=`<span class="wd-badge" style="color:var(--secondary-text-color,#888)" title="${d(I)}">${this._t("badge.quiet_tail",{mins:L},`~${L}m quiet tail`)}</span>`}const E=[x,l,c,v,g,$].filter(Boolean).join(" "),D=Array.isArray(t.signature_curve)&&t.signature_curve.length>=3?`<canvas class="wd-prof-spark" data-spark-prof="${d(t.name)}" width="64" height="20" aria-label="${d(this._t("lbl.sparkline",{name:t.name},"Average power curve"))}"></canvas>`:"";return`
    ...
    `}_htmlProfileGroupModal(t){const e=this._busy.has("pg-save"),s=this._profileEnvCache||{},r=p=>it[Math.max(0,this._profiles.findIndex(f=>f.name===p))%it.length],n=t.members||[],a=this._profiles.map(p=>{const f=n.includes(p.name),g=p.avg_duration?`~${Math.round(p.avg_duration/60)}m`:"",$=p.avg_energy!=null?` \xB7 ${yt(p.avg_energy)}`:"",S=f?`<span style="display:inline-block;width:10px;height:10px;border-radius:2px;background:${r(p.name)};margin:0 2px"></span>`:"";return`<label class="wd-rev-tag"><input type="checkbox" class="wd-pg-mem" value="${d(p.name)}" ${f?"checked":""}> ${S}${d(p.name)} <span style="color:var(--secondary-text-color);font-size:.85em">${g}${$}</span></label>`}).join(""),o=n.filter(p=>s[p]&&(s[p].avg||[]).length),i=o.length?`<div class="wd-leg">${o.map(p=>`<span class="wd-leg-i"><span class="wd-leg-sw" style="background:${r(p)}"></span> ${d(p)}</span>`).join("")}</div>`:"",l=o.length?`<div class="wd-canvas-wrap" style="margin-top:8px"><canvas id="wd-pgroup-canvas" role="img" aria-label="${d(this._t("lbl.aria_envelope_chart",{},"Profile power envelope chart"))}" style="height:150px"></canvas></div>${i}`:`<p class="wd-info">${this._t("msg.group_preview_hint",{},"Tick 2+ members to preview and compare their power curves.")}</p>`,c=((this._profileGroups||{}).groups||[]).find(p=>p.name===t.orig),_=c&&c.cohesion!=null?`<span class="wd-badge" style="color:${c.cohesive?"var(--success-color,#4caf50)":"var(--warning-color,#ff9800)"};background:${c.cohesive?"rgba(76,175,80,.14)":"rgba(255,152,0,.14)"}">${c.cohesive?this._t("lbl.cohesion_good",{pct:Math.round(c.cohesion*100)},"cohesion "+Math.round(c.cohesion*100)+"%"):this._t("lbl.cohesion_low",{pct:Math.round(c.cohesion*100)},"\u26A0 low cohesion "+Math.round(c.cohesion*100)+"%")}</span>`:"";return`<h2>${t.orig?this._t("modal.edit_group",{},"Edit profile group"):this._t("modal.new_group",{},"New profile group")}</h2>
    ...
    </label>`,S=r?`<p class="wd-info" style="margin:0 0 10px;font-size:.82em">${this._t("msg.settings_basic_note",{},"Showing essential settings. Switch to Advanced for the full list.")}</p>`:"",x=this._busy.has("save-settings"),b=l.size,v=b!==1?"s":"",E=b?`
      ...
      </div>`:"",D=this._suggestions.length,L=[...a].filter(X=>!n.has(X)).length,z=D+L,R=this._settingsSugOnly&&!this._settingsSearch,I=D?`<button class="wd-btn wd-btn-sm wd-btn-primary" data-action="sug-apply-all">${this._t("btn.apply_all",{},"Apply all")}</button>`:"",C=D?`<button class="wd-btn wd-btn-sm wd-btn-secondary" data-action="sug-dismiss">${this._t("btn.dismiss",{},"Dismiss")}</button>`:"",P=z?R?`
      <div class="wd-sug-banner">
        <span>\u{1F4A1} ${this._t("msg.showing_suggestions",{count:z},`Showing ${z} setting${z>1?"s":""} with suggestions.`)} <span style="text-decoration:underline;cursor:pointer" data-action="sug-show-all">${this._t("msg.show_all_settings",{},"Show all settings")}</span>.</span>
        ${I}
      </div>`:`
      <div class="wd-sug-banner">
        <span>\u{1F4A1} ${this._t("msg.tuning_suggestions_available",{count:z},`${z} tuning suggestion${z>1?"s":""} available from observed cycles. They appear beside the relevant fields.`)}</span>
    ...
    `}_htmlMlStatusSection(t,e){const s=e&&this._busy.has("ml-train-now:"+e)||t&&t.running,r=this._canEdit()?`<button class="wd-btn wd-btn-primary wd-btn-sm" data-action="ml-train-now" ${s?"disabled":""}>${s?`<span class="wd-spin"></span> ${this._t("status.training",{},"Training\u2026")}`:this._t("btn.train_now",{},"Train now")}</button>`:"";if(!t)return`<div class="wd-card"><div class="wd-card-title" style="margin:0 0 4px">${this._t("hdr.status",{},"Status")}</div><p class="wd-info" style="margin:0">${this._t("msg.loading",{},"Loading\u2026")}</p></div>`;const n=Object.keys(t.on_device_models||{}).length,a=n?`<span style="color:var(--success-color,#4caf50);font-weight:600">${this._t("ml.personalized",{},"\u25CF Personalized to this machine")}</span> <span style="color:var(--secondary-text-color)">${this._t("lbl.models_fine_tuned",{count:n},"(fine-tuned models: "+n+")")}</span>`:`<span style="color:var(--secondary-text-color)">${this._t("ml.builtin_models",{},"\u25CF Using built-in models")}</span>`,o=t.cycle_count||0,i=t.min_cycles||0,l=o>=i,c=i>0?Math.min(100,Math.round(o/i*100)):100,_=l?"var(--success-color,#4caf50)":"var(--warning-color,#ff9800)",p=Math.max(0,i-o),f=l?this._t("msg.enough_data",{current:o,min:i},`Enough data to learn from (${o}/${i} cycles).`):this._t("msg.collecting_data",{need:p,current:o,min:i},`Collecting data. Cycles still needed before fine-tuning can start: ${p} (${o}/${i}).`),g=`<div style="height:8px;border-radius:6px;background:var(--secondary-background-color);overflow:hidden;margin:8px 0"><div style="width:${c}%;height:100%;background:${_}"></div></div>`,$=t.last_trained?rt(t.last_trained):"never",S=s?`<span style="color:var(--info-color,#2196f3)"><span class="wd-spin"></span> ${this._t("status.fine_tuning",{},"fine-tuning now\u2026")}</span>`:t.enabled?this._t("lbl.auto_fine_tune_on",{hour:String(t.hour).padStart(2,"0")},`auto fine-tune on (around ${String(t.hour).padStart(2,"0")}:00)`):this._t("lbl.auto_fine_tune_off",{},"auto fine-tune off");return`<div class="wd-card">
      ...
      ${g}
      <p class="wd-info" style="margin:0">${this._t("lbl.last_checked",{},"Last checked:")} <strong>${d($)}</strong> \xB7 ${S}</p>
      ...
      ${r}
        ...
        </div>`,n=S);const P=this._pgStagedVal(p),M=P!=null&&!(v!==""&&v!==null&&v!==void 0&&this._pgSameVal(P,v)),F=this._canEdit()&&M&&this._pgIsPublishable(p)?`<button class="wd-pg-pub" data-action="pg-publish-one" data-pgkey="${d(p)}" aria-label="${d(this._t("btn.pg_publish_one",{label:b},"Publish "+b+" to the integration"))}" title="${d(this._t("btn.pg_publish_one",{label:b},"Publish "+b+" to the integration"))}">\u2191</button>`:'<span class="wd-pg-pub-slot" aria-hidden="true"></span>';return`${C}<div style="display:flex;align-items:flex-start;gap:6px;margin:0 0 6px 11px">
        <div style="flex:1;min-width:0">
          <div style="font-size:.82em;font-weight:600;margin-bottom:1px">${d(b)}${L?` <span style="color:${R};font-size:.85em" title="${d(this._t("lbl.pg_drag_hint",{},"Drag line on graph"))}">\u2195</span>`:""}</div>
          ${$?`<div style="font-size:.72em;color:var(--secondary-text-color);line-height:1.3">${d(this._t("pg_desc."+p,{},$))}</div>`:""}
        </div>
        <div style="display:flex;align-items:center;gap:4px;flex-shrink:0">
          ${D?`<input class="wd-pg-param-chk" type="checkbox" data-pgkey="${d(p)}" data-pgtype="bool" aria-label="${d(b)}" ${E?"checked":""} style="width:18px;height:18px;margin:1px 27px 0 27px">`:`<input class="wd-pg-param-inp" type="text" inputmode="decimal" data-pgkey="${d(p)}" value="${E!==""?d(String(E)):""}" placeholder="${v!==""?d(String(v)):""}" aria-label="${d(b)}" style="width:72px">`}
          ${z?`<span style="font-size:.75em;color:var(--secondary-text-color);min-width:14px">${d(z)}</span>`:""}
          ${F}
        </div>
      ...
      </div>`}const i=e.summary||{},l=`<div style="font-size:.82em;color:var(--secondary-text-color);margin-bottom:8px">${this._t("msg.pg_history_summary",{detected:i.detected,correct:i.match_correct,total:i.cycles},`${i.detected}/${i.cycles} detected \xB7 ${i.match_correct} matched correctly`)}</div>`,c=g=>(this._cycles||[]).find($=>$.id===g),_={};(e.baseline_rows||[]).forEach(g=>{_[g.cycle_id]=g});const p=e.rows.map(g=>{const $=c(g.cycle_id),S=$&&$.start_time?rt($.start_time):(g.cycle_id||"").slice(0,8),x=g.match_correct===!0?"\u2713":g.match_correct===!1?"\u2717":"\u2014",b=g.match_correct===!0?"var(--success-color,#4caf50)":g.match_correct===!1?"var(--error-color,#f44336)":"var(--secondary-text-color)",v=g.matched_profile||this._t("lbl.unlabelled",{},"Unlabelled"),E=g.duration_s?Math.round(g.duration_s/60)+"m":"\u2014",D=g.overrun_ratio!=null?` (${Math.round(g.overrun_ratio*100)}%)`:"",L=g.alerts&&g.alerts.length?` <span title="${d(g.alerts.join(", "))}" style="color:var(--warning-color,#ff9800)">\u26A0</span>`:"",z=_[g.cycle_id];let R="";return z&&(z.match_correct!==!0&&g.match_correct===!0?R=`<span style="color:var(--success-color,#4caf50)">\u25B2 ${this._t("lbl.pg_fixed",{},"fixed")}</span>`:z.match_correct===!0&&g.match_correct!==!0?R=`<span style="color:var(--error-color,#f44336)">\u25BC ${this._t("lbl.pg_broke",{},"broke")}</span>`:String(z.termination_reason)!==String(g.termination_reason)&&(R=`<span style="color:var(--warning-color,#ff9800)">${d(String(z.termination_reason||"\u2014"))}\u2192${d(String(g.termination_reason||"\u2014"))}</span>`)),`<tr class="wd-pg-hrow${g.cycle_id===this._pgCycleId?" selected":""}" data-action="pg-open-cycle" data-cid="${d(g.cycle_id)}" title="${d(this._t("msg.pg_row_load_hint",{},"Load this cycle in the graph above"))}">
        <td>${d(S)}</td>
        <td><span style="color:${b};font-weight:700">${x}</span> ${d(v)}</td>
        <td>${d(String(g.termination_reason||"\u2014"))}</td>
        <td style="font-variant-numeric:tabular-nums">${E}${D}${L}</td>
        ${e.diff?`<td>${R}</td>`:""}
    ...
    </div>`,o=t?this._htmlPgBatchBar():"";return`${n}${a}${this._htmlPgRecentRuns("pg_sweep")}${o}${this._htmlPgSweepResult()}`}_htmlPgSweepResult(){const t=this._pgSweepNew;if(!t||!Array.isArray(t.points)||!t.points.length)return"";const e=this._pgSweepObjectives().find(p=>p[0]===t.objective),s=e?e[2]:!1,r=t.points.filter(p=>p.metric!=null).map(p=>p.metric);if(!r.length)return`<div class="wd-empty" style="padding:16px">${this._t("msg.pg_sweep_no_metric",{},"Not enough data to score this objective.")}</div>`;const n=Math.min(...r),a=Math.max(...r),o=s?Math.min(...r):Math.max(...r),i=(t.points.find(p=>p.metric===o)||{}).value,l=p=>t.objective==="median_overrun"?Math.round(p*100)+"% off":Math.round(p*100)+"%",c=t.points.map(p=>{const f=a>n&&p.metric!=null?(p.metric-n)/(a-n):p.metric!=null?1:0,g=p.metric===o,$=t.current_value!=null&&Math.abs(p.value-t.current_value)<1e-6,S=g?"var(--success-color,#4caf50)":"var(--primary-color)";return`<div style="display:flex;align-items:center;gap:8px;margin:3px 0;font-size:.82em">
        <span style="flex:0 0 74px;text-align:right;font-variant-numeric:tabular-nums">${d(String(p.value))}${$?" \u25C0":""}</span>
        <div style="flex:1;height:16px;border-radius:4px;background:var(--secondary-background-color);overflow:hidden"><div style="height:100%;width:${Math.round((p.metric!=null?.15+.85*f:0)*100)}%;background:${S};border-radius:4px"></div></div>
    ...
    </div>`:"",S=s.map(v=>{const E=parseInt(a[v],10)||0;if(E<=0)return"";const D=Math.max(0,parseInt(o[v],10)||0),L=Math.max(0,Math.min(100,Math.round(D/E*100))),R=D>=E?"var(--warning-color,#ff9800)":"var(--primary-color)";return`<div style="margin-top:10px">
        <div style="display:flex;align-items:baseline;justify-content:space-between;gap:8px">
          <span style="font-weight:600">${d(this._maintLabel(v))}</span>
          <span class="wd-info">${this._t("lbl.cycles_since_service",{since:D,total:E},D+" / "+E+" cycles")}</span>
        </div>
        <div style="height:6px;margin-top:4px;border-radius:3px;background:var(--divider-color);overflow:hidden">
          <div style="height:100%;border-radius:3px;width:${L}%;background:${R}"></div>
        </div>
      ...
      ${S||`<p class="wd-info" style="margin-top:10px">${this._t("msg.no_reminders_set",{},"No service reminders are set yet.")}</p>`}
      ${x}
    </div>`;return`${c}
      ${b}
      ${f}
    ...
    ${r||`<div class="wd-card"><p class="wd-info">${this._t("msg.no_other_users",{},"No other Home Assistant users found.")}</p></div>`}`}_htmlStore(){if(!this._canEdit())return`<div class="wd-empty">${this._t("msg.no_device_selected",{},"No device selected.")}</div>`;const t=this._storeStatus,e=`<a href="https://3dg1luk43.github.io/washdata-store" target="_blank" rel="noopener noreferrer" style="font-size:.8em;font-weight:400;color:var(--primary-color);text-decoration:none;white-space:nowrap" title="${d(this._t("store.website_tip",{},"Open the community store website"))}">${this._t("store.website",{},"Store website \u2197")}</a>`,s=`<div style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap"><div class="wd-card-title" style="margin:0">${this._t("hdr.community_store",{},"Community Store")}</div>${e}</div>`;if(!this._onlineEnabled()||t&&t.enabled===!1)return`<div class="wd-card">${s}
      ...
      <div class="wd-log-drawer-body">
        ...
        <button class="wd-btn wd-btn-primary" data-maction="store-share-ok" ${this._busy.has("store-share")?"disabled":""}>${this._busy.has("store-share")?'<span class="wd-spin"></span> ':""}${this._t("btn.share",{},"Share")}</button></div>`}else t.type==="store-share-device"&&(e=this._htmlShareDeviceModal(t));return`<div class="wd-overlay"><div class="wd-modal" role="dialog" aria-modal="true" aria-labelledby="wd-modal-title" tabindex="-1">${e}</div></div>`}_htmlShareDeviceModal(t){const e=this._shareableByProgram(),s=this._busy.has("store-share-device"),r=t.selected||new Set,n=!!t.consented,a=e.reduce((g,$)=>g+$.cycles.filter(S=>r.has(S.id)).length,0),o=e.some(g=>!g.noCycles);let i;t.loading?i=`<div class="wd-empty" style="padding:24px"><div class="wd-icon">\u23F3</div>${this._t("msg.loading",{},"Loading\u2026")}</div>`:e.length?i=e.map(g=>{if(g.noCycles)return`<div class="wd-sd-group wd-sd-group-nocyc">
          ...
          </div>`;const $=g.cycles.every(E=>r.has(E.id)),S=!$&&g.cycles.some(E=>r.has(E.id)),x=g.cycles.map(E=>{const D=E.start_time?rt(E.start_time):"",L=E.duration!=null?_t(E.duration):"";return`<label class="wd-sd-cyc">
            <input type="checkbox" data-maction="sd-toggle-cyc" data-cid="${d(E.id)}" ${r.has(E.id)?"checked":""} ${s?"disabled":""}>
            <span class="wd-sd-cyc-meta">${d(D)} \xB7 ${d(L)} \u2B50</span>
          ...
          <label class="wd-sd-prof">
            <input type="checkbox" data-maction="sd-toggle-prof" data-prog="${d(g.program)}" ${$?"checked":""} ${S?'data-indeterminate="1"':""} ${s?"disabled":""}>
            <span class="wd-sd-prof-name">${d(g.program)}</span>
            <span class="wd-sd-count">${g.cycles.filter(E=>r.has(E.id)).length}/${g.cycles.length}</span>
          </label>
      ...
      <span class="wd-sd-prof-name"><strong>${this._t("btn.select_all",{},"Select all")}</strong></span>
    </label>`,c=a.map(_=>{const p=r[_],f=s.importableOnly&&p.importable===!1,g=this._wizCatState(t,_,e),S=["profiles","real_cycles","reference_cycles"].includes(_)?`${g.sel}/${p.count}`:"",x=`<label class="wd-sd-prof">
          ...
          <span class="wd-sd-cyc-meta">${d(v.name)} \xB7 ${(v.real_cycles||0)+(v.reference_cycles||0)} ${this._t("lbl.cycles_short",{},"cycles")}${n.has(v.name)?` <span class="wd-tag" style="color:var(--warning-color,#e6a700)">${this._t("badge.exists",{},"exists")}</span>`:""}</span>
        </label>`).join("")}</div>`;else if(!f&&(_==="real_cycles"||_==="reference_cycles")){const v=_==="real_cycles"?t.sel.realIds:t.sel.refIds;b=`<div class="wd-sd-cycles">${(p.groups||[]).map(E=>{const D=E.cycles.map(M=>String(M.id)),L=D.filter(M=>v.has(M)).length,z=L===D.length&&D.length>0,R=L>0&&!z,I=`${_}:${E.profile}`,C=t.expanded&&t.expanded.has(I),P=C?E.cycles.map(M=>`<label class="wd-sd-cyc" style="margin-left:18px">
            <input type="checkbox" data-maction="wiz-toggle-cyc" data-cat="${_}" data-cid="${d(String(M.id))}" ${v.has(String(M.id))?"checked":""}>
            <span class="wd-sd-cyc-meta">${d(M.date?rt(M.date):String(M.id))}${M.duration!=null?" \xB7 "+_t(M.duration):""}</span>
            ...
            <div class="wd-sd-cyc" style="display:flex;align-items:center">
              <label style="flex:1;display:flex;align-items:center;gap:8px;cursor:pointer">
                <input type="checkbox" data-maction="wiz-toggle-cycgroup" data-cat="${_}" data-prof="${d(E.profile)}" ${z?"checked":""} ${R?'data-indeterminate="1"':""}>
                <span class="wd-sd-cyc-meta"><strong>${d(E.profile||this._t("lbl.unlabelled",{},"Unlabelled"))}</strong> (${L}/${E.count})</span>
              </label>
              <button type="button" class="wd-linkbtn" data-maction="wiz-expand" data-key="${d(I)}" style="margin-left:auto;background:none;border:none;color:var(--primary-color);cursor:pointer">${C?"\u25BE":"\u25B8"}</button>
            </div>
            ${P}
      ...
      </div>`);const S=t.sel&&(t.sel.profiles.size||t.sel.realIds.size||t.sel.refIds.size||t.sel.cats.size);return`${s}
      ${i}
      ${l}
      ${c}
      ${p}
      ...
      <div class="wd-modal-actions">
        ...
        <button class="wd-btn wd-btn-primary" data-maction="import-apply-ok" ${e||!S?"disabled":""}>${e?'<span class="wd-spin"></span> ':""}${this._t("btn.import_selected",{},"Import selected")}</button>
      ...
      </tr>`}).join(""),S=n.every(x=>i.has(x.index));return`${e}
      ...
      <div style="margin-bottom:8px"><button class="wd-btn wd-btn-secondary wd-btn-sm" data-maction="hist-toggle-all">${S?this._t("btn.select_none",{},"Select none"):this._t("btn.select_all",{},"Select all")}</button></div>
      ...
      <div class="wd-modal-actions">
        ...
        <div class="wd-modal-actions"><button class="wd-btn wd-btn-secondary" data-maction="cancel">${this._t("btn.close",{},"Close")}</button></div>`;const e=t.curve||{},s=!!e.is_reference,r=e.labelable!==!1,n=e.editable!==!1,a=e.cycle_origin==="backfill",o=e.full_duration_s||e.duration||0,i=e.energy_kwh!=null?e.energy_kwh:null,l=t.ml||null;let c="";if(l&&l.ml_quality_score!=null){const P=l.ml_quality_label,M=P==="ok"?"var(--success-color,#4caf50)":P==="uncertain"?"var(--warning-color,#ff9800)":"var(--error-color,#f44336)",F=Math.round((1-l.ml_quality_score)*100);c=`<div class="wd-kv-item"><div class="wd-kv-val" style="font-size:.95em;color:${M}">${F}%</div><div class="wd-kv-lbl">${this._t("lbl.cycle_health",{},"Cycle health")}</div></div>`}const _=`<div class="wd-kv">
      ...
      ${c}
    </div>`,p=l&&l.ml_review||{},f=(this._feedbacks||[]).some(P=>P.cycle_id===t.cycleId),g=l&&l.ml_quality_label,$=!p.reviewed_at&&(f||["uncertain","review"].includes(g)||["force_stopped","interrupted"].includes(e.status)),S=$&&t.mode!=="review"?` <span title="${this._t("hdr.automation_needs_review",{},"This cycle needs review")}" style="color:var(--warning-color,#ff9800);font-size:1.1em;line-height:0">\u25CF</span>`:"",x=this._canEdit()&&n?`<div class="wd-mode-bar">
      ...
      <button class="wd-btn wd-btn-sm ${t.mode==="review"?"wd-btn-primary":"wd-btn-secondary"}" data-maction="cyc-review" title="${$?this._t("hdr.automation_needs_review",{},"This cycle needs review"):this._t("hdr.automation_review_this_cycle",{},"Review this cycle")}">${this._t("btn.review",{},"Review")}${S}</button>
        ...
        </div>`:"";let D="";if(t.mode==="view"){const P=!!(l&&l.ml_review&&l.ml_review.golden),F=this._canEdit()&&this._onlineEnabled()&&this._storeConnected&&P?`<button class="wd-btn wd-btn-secondary" data-action="store-share-cycle" data-cid="${d(t.cycleId)}" data-prof="${d(e.profile_name||"")}">${this._t("btn.share_to_store",{},"Share to store")}</button>`:"",U=this._canEdit()?`<button class="wd-btn wd-btn-danger" data-maction="cyc-delete">${this._t("btn.delete",{},"Delete")}</button>${r?`
        <button class="wd-btn wd-btn-primary" data-maction="cyc-label">${this._t("btn.label",{},"Label")}</button>`:""}`:"";D=`${E}<div class="wd-modal-actions">
        <button class="wd-btn wd-btn-secondary" data-maction="cancel">${this._t("btn.close",{},"Close")}</button>
        ${F}
        ${U}</div>`}else if(t.mode==="trim"){const P=this._busy.has("cyc-trim-apply"),M=t.timeMode||"s",F=M==="clock"?this._offsetToClock(t.trim.start):Math.round(t.trim.start),U=M==="clock"?this._offsetToClock(t.trim.end):Math.round(t.trim.end),st=M==="clock"?"time":"number",tt=M==="clock"?'step="1"':`min="0" max="${Math.ceil(o)}" step="1"`,Y=M==="clock"?"":" "+this._t("lbl.unit_s",{},"(s)");D=`<p class="wd-info" style="margin:4px 0 8px">${this._t("msg.trim_intro",{},"Drag the red handles, or enter values. Everything outside the window is removed.")}</p>
        ...
        <div class="wd-modal-actions">
          ...
          <button class="wd-btn wd-btn-primary" data-maction="cyc-apply-trim" ${P?"disabled":""}>${P?'<span class="wd-spin"></span> '+this._t("status.trimming",{},"Trimming\u2026"):this._t("btn.apply_trim",{},"Apply Trim")}</button>
        </div>`}else if(t.mode==="split"){const P=this._busy.has("cyc-split-apply"),M=(t.split.offsets||[]).slice().sort((st,tt)=>st-tt),F=[0,...M,o],U=F.slice(0,-1).map((st,tt)=>{const Y=F[tt+1];return`<div class="wd-seg-row"><span class="wd-swatch" style="background:${it[tt%it.length]}"></span>
        ...
        <div style="margin:10px 0">${M.length?U:`<p class="wd-info">${this._t("msg.no_split_points",{},"No split points yet.")}</p>`}</div>
        <div class="wd-modal-actions">
          <button class="wd-btn wd-btn-secondary" data-maction="cancel">${this._t("btn.close",{},"Close")}</button>
          <button class="wd-btn wd-btn-primary" data-maction="cyc-apply-split" ${P||!M.length?"disabled":""}>${P?'<span class="wd-spin"></span> '+this._t("status.splitting",{},"Splitting\u2026"):this._t("btn.apply_split",{},"Apply Split")}</button>
        </div>`}else if(t.mode==="review"){const P=l&&l.ml_review||{},M=this._busy.has("cyc-review-save"),F=(Z,ot)=>`<option value="${Z}" ${(P.quality||"")===Z?"selected":""}>${ot}</option>`,st=[["late_start",this._t("tag.late_start",{},"Late start")],["early_end",this._t("tag.early_end",{},"Early end")],["merged",this._t("tag.merged",{},"Merged cycles")],["split",this._t("tag.split",{},"Split cycle")],["noise",this._t("tag.noise",{},"Noise")],["wrong_profile",this._t("tag.wrong_profile",{},"Wrong profile")],["sensor_gap",this._t("tag.sensor_gap",{},"Sensor gap")]].map(([Z,ot])=>`<label class="wd-rev-tag"><input type="checkbox" class="wd-cyc-rev-tag" value="${Z}" ${(P.tags||[]).includes(Z)?"checked":""}> ${ot}</label>`).join(""),tt=P.reviewed_at?`<span style="font-size:.75em;color:var(--secondary-text-color)">${this._t("lbl.reviewed_on",{date:new Date(P.reviewed_at).toLocaleDateString()},`reviewed ${new Date(P.reviewed_at).toLocaleDateString()}`)}</span>`:"",Y=dt(this._t("msg.review_profile_tip",{},"The program this cycle is labelled as. If the auto-detected program was wrong, correct it here - labelling teaches matching for future cycles.")),Q=dt(this._t("msg.review_quality_tip",{},"How clean this cycle is. Good = a textbook example of this program; Bad = detected but noisy or atypical; Unusable = mis-detected (merged, truncated or spurious). Drives the health score and which cycles are allowed to train the model.")),wt=dt(this._t("msg.review_recorded_tip",{},'Mark this as a hand-picked reference cycle for its program - the same role as a manually recorded cycle. Reference cycles are always kept, seed the matching template, and are never dropped by cleanup. (This is the "golden"/recorded flag; both are the same thing.)')),X=dt(this._t("msg.review_tags_tip",{},"Optional flags describing what went wrong with this cycle, so training and cleanup can account for it.")),lt=dt(this._t("msg.review_notes_tip",{},"Free-text notes for your own reference. Not used by matching or training."));D=`
        ${E}
        ...
        <div style="display:flex;gap:16px;flex-wrap:wrap;align-items:center;margin:6px 0">
          ...
          <label style="display:inline-flex;align-items:center;gap:6px"><input type="checkbox" id="wd-cyc-rev-golden" ${P.golden?"checked":""}> ${this._t("badge.golden_cycle",{},"Recorded reference cycle")}${wt}</label>
          ${tt}
        </div>
        ...
        <textarea id="wd-cyc-rev-notes" class="wd-rev-notes" rows="3" placeholder="${this._t("msg.review_notes_placeholder",{},"Notes (optional)")}">${d(P.notes||"")}</textarea>
        ...
        </div>`}let L="";const z=t.mode==="view"||t.mode==="review"?e.artifacts||[]:[];if(z.length){const P=z.map(M=>{const F=M.detail_key?this._t(M.detail_key,M.detail_params||{},M.detail||""):M.detail||"";return`<li><b>${d(ne(M.type,(U,st,tt)=>this._t(U,st,tt)))}</b> ${this._t("lbl.at",{},"at")} ${zt(M.start_s)}\u2013${zt(M.end_s)} \u2014 ${d(F)}</li>`}).join("");L=`<div class="wd-card" style="margin:10px 0 0;padding:10px 12px;border-left:3px solid var(--warning-color,#ff9800)">
        <div style="font-weight:600;font-size:.9em">\u26A0 ${this._t("msg.artifact_header",{n:z.length},`${z.length} anomal${z.length>1?"ies":"y"} detected during this cycle`)}</div>
        <ul class="wd-info" style="margin:4px 0 0;padding-left:18px;font-size:.82em">${P}</ul>
      ...
      </div>`}let R="";const I=t.mode==="view"||t.mode==="review"?e.restart_gaps||[]:[];if(I.length){const P=I.map(M=>{const F=Math.round((M.gap_seconds||0)/60),U=F>=1?`${F}m`:`${Math.round(M.gap_seconds||0)}s`,st=M.match_confidence!=null?` \xB7 ${this._t("lbl.pct_match_confidence",{pct:Math.round(M.match_confidence*100)},`${Math.round(M.match_confidence*100)}% match confidence`)}`:"",tt=M.profile?` (${d(M.profile)})`:"";return`<li>${this._t("msg.restart_gap_item",{dur:U},`${U} gap`)}: ${this._t("lbl.ha_restarted",{},"HA restarted")}${tt}${st}</li>`}).join("");R=`<div class="wd-card" style="margin:10px 0 0;padding:10px 12px;border-left:3px solid var(--info-color,#2196f3)">
      ...
      </div>`}let C="";if(e.decimated){const P=(e.samples||[]).length,M=e.sample_count||P;C=`<div class="wd-info" style="margin:4px 0 0;font-size:.72em;color:var(--secondary-text-color)">${this._t("msg.samples_decimated",{shown:P,total:M},`Showing ${P} of ${M} samples (thinned for display; peaks kept). A wide gap here is thinning, not missing data.`)}</div>`}return`<h2>${this._t("lbl.cycle",{},"Cycle")} \xB7 ${d(rt(e.start_time))}</h2>
      ${_}${x}
      ...
      ${D}`}_htmlProfilePanel(t){const e=this._canEdit();t.tab==="danger"&&!e&&(t.tab="stats");const s=[["stats",this._t("tab.pp_overview",{},"Overview")],["phases",this._t("tab.pp_phases",{},"Phases")],["cleanup",this._t("tab.pp_cleanup",{},"Cleanup")]];e&&s.push(["danger",this._t("tab.pp_manage",{},"Manage")]);const r=s.map(([o,i])=>`<button class="wd-mini-tab ${t.tab===o?"active":""}" data-maction="pp-tab-${o}">${i}</button>`).join("");let n="";if(!t.loaded)n=`<div class="wd-empty" style="padding:32px"><div class="wd-icon">\u23F3</div>${this._t("msg.loading",{},"Loading\u2026")}</div>`;else if(t.tab==="stats"){const o=t.stats||{},i=t.env||{},l=this._hass&&this._hass.config&&this._hass.config.currency||"",c=o.avg_energy!=null&&o.cycle_count?o.avg_energy*o.cycle_count:null,_=S=>S?Math.round(S/60)+"m":"-",p=(this._profileHealth||{})[t.name],f=(this._profileTrends||{})[t.name],g=p&&p.health_status!=="unknown"?(()=>{const S={healthy:["var(--success-color,#4caf50)","rgba(76,175,80,.12)"],fair:["var(--warning-color,#ff9800)","rgba(255,152,0,.12)"],poor:["var(--error-color,#f44336)","rgba(244,67,54,.12)"]},[x,b]=S[p.health_status]||S.fair,v=Math.round((p.health_score||0)*100),E=p.duration_cv!=null?` \xB7 ${this._t("stat.duration_cv",{pct:Math.round(p.duration_cv*100)},`duration CV ${Math.round(p.duration_cv*100)}%`)}`:"",D=p.confidence_mean!=null?` \xB7 ${this._t("stat.avg_confidence",{pct:Math.round(p.confidence_mean*100)},`avg confidence ${Math.round(p.confidence_mean*100)}%`)}`:"";return`<div style="margin:8px 0 4px;padding:8px 12px;border-radius:6px;background:${b};border:1px solid color-mix(in srgb, ${x} 13%, transparent);display:flex;align-items:center;gap:8px;flex-wrap:wrap">
          ...
          <span style="font-size:.85em;opacity:.8">${this._t("stat.score",{pct:v},`score ${v}%`)}${E}${D}</span>
        ...
        </div>`})():"",$=f&&(f.duration_trend!=="stable"||f.energy_trend&&f.energy_trend!=="stable")?(()=>{const S=[];f.duration_trend==="up"?S.push(this._t("msg.trend_duration_longer",{pct:`${f.duration_slope_pct>0?"+":""}${f.duration_slope_pct}`,avg:`${Math.round(f.duration_recent_mean_s/60)}m`},`Duration trending longer (${f.duration_slope_pct>0?"+":""}${f.duration_slope_pct}%/cycle) \u2014 recent avg ${Math.round(f.duration_recent_mean_s/60)}m`)):f.duration_trend==="down"&&S.push(this._t("msg.trend_duration_shorter",{pct:`${f.duration_slope_pct}`,avg:`${Math.round(f.duration_recent_mean_s/60)}m`},`Duration trending shorter (${f.duration_slope_pct}%/cycle) \u2014 recent avg ${Math.round(f.duration_recent_mean_s/60)}m`)),f.energy_trend==="up"?S.push(this._t("msg.trend_energy_up",{pct:`${f.energy_slope_pct>0?"+":""}${f.energy_slope_pct}`,avg:yt(f.energy_recent_mean_wh)},`Energy trending up (${f.energy_slope_pct>0?"+":""}${f.energy_slope_pct}%/cycle) \u2014 recent avg ${yt(f.energy_recent_mean_wh)}`)):f.energy_trend==="down"&&S.push(this._t("msg.trend_energy_down",{pct:`${f.energy_slope_pct}`},`Energy trending down (${f.energy_slope_pct}%/cycle)`));const x=f.duration_trend==="up"||f.energy_trend==="up";return`<div style="margin:6px 0;padding:8px 12px;border-radius:6px;background:${x?"rgba(255,152,0,.10)":"rgba(33,150,243,.10)"};font-size:.88em">
        ...
        ${g}
        ${$}
        ${p&&p.shape_drift?(()=>{const S=p.shape_drift_correlation!=null?` (r=${Number(p.shape_drift_correlation).toFixed(2)})`:"";return`<div style="margin-top:8px;padding:8px 10px;background:color-mix(in srgb, var(--warning-color,#ff9800) 9%, transparent);border-radius:6px;border-left:3px solid var(--warning-color,#ff9800)">
            <span style="font-weight:600;color:var(--warning-color,#ff9800)">${this._t("msg.shape_drift_advisory",{},"\u26A0 Shape drifting")}${d(S)}</span>
        ...
        ${(()=>{const S=(this._profileAdvisories||[]).find(x=>x&&x.profile===t.name&&x.code==="phase_inconsistent");return S?`<div style="margin-top:8px;padding:8px 10px;background:color-mix(in srgb, var(--warning-color,#ff9800) 9%, transparent);border-radius:6px;border-left:3px solid var(--warning-color,#ff9800)">
            ...
            <span style="font-size:.82em;opacity:.85;display:block;margin-top:4px">${d(this._t(S.message_key,S.message_params,S.message))}</span>
          </div>`:""})()}
        ...
        ${e?`<div class="wd-mode-bar">
          ...
          <td style="width:26px;padding:6px 4px"><input type="checkbox" data-cleanidx="${E}" ${i.has(b.cycle_id)?"checked":""}></td>
          <td style="width:10px;padding:6px 2px"><span class="wd-swatch" style="background:${it[E%it.length]}"></span></td>
        ...
        ${e?"<th></th>":""}
      </tr></thead>`,$=this._busy.has("pp-cleanup-del"),S=this._busy.has("pp-cleanup-unlabel"),x=$||S;n=`<p class="wd-info" style="margin-bottom:10px">${this._t("msg.cleanup_intro",{},"Every labelled cycle overlaid. Tick outliers, then unlabel them (kept in history, but no longer shaping this profile) or delete them outright.")}</p>
