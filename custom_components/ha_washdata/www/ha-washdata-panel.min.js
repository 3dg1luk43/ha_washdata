...
`;function ot(h){if(h==null||h<0)return"-";const t=Math.floor(h/3600),e=Math.floor(h%3600/60),s=Math.floor(h%60);return t>0?`${t}h ${e}m`:e>0?`${e}m ${s}s`:`${s}s`}function Gt(h){return h==null?"-":h>=100?`${Math.round(h)} W`:`${h.toFixed(1)} W`}function gt(h){return h==null?"-":`${h.toFixed(2)} kWh`}let Kt="relative";const de=3700;function Xt(h){const t=e=>String(e).padStart(2,"0");return`${h.getFullYear()}-${t(h.getMonth()+1)}-${t(h.getDate())}`}function Yt(h){const t=new Date;return t.setDate(t.getDate()-h),Xt(t)}function ce(){return Xt(new Date)}function Ft(){return Yt(10)}function he(){return Yt(de-1)}function pe(h){const t=Math.round((h-Date.now())/1e3);let e;try{e=new Intl.RelativeTimeFormat(void 0,{numeric:"auto"})}catch{return Jt(h)}const s=Math.abs(t),n=[["year",31536e3],["month",2592e3],["week",604800],["day",86400],["hour",3600],["minute",60]];for(const[r,a]of n)if(s>=a)return e.format(Math.round(t/a),r);return e.format(t,"second")}function Jt(h){return new Date(h).toLocaleString(void 0,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}function st(h,t){if(!h)return"-";let e;if(typeof h=="number")e=h>=1e12?h:h*1e3;else{const s=String(h),n=/^(\d{4})-(\d{2})-(\d{2})$/.exec(s);e=n?new Date(+n[1],+n[2]-1,+n[3]).getTime():new Date(s).getTime()}return isNaN(e)?"-":(t||Kt)==="relative"?pe(e):Jt(e)}function Pt(h){if(h==null)return"unknown error";if(h instanceof Error)return`${h.name}: ${h.message}`;const t=h.code!=null?String(h.code):"",e=h.message!=null?String(h.message):"";if(!t&&!e)try{return JSON.stringify(h)}catch{return String(h)}const s=t==="unknown_command"?" (the integration is probably still starting up; this should stop on its own)":"";return`${t||"error"}${e?": "+e:""}${s}`}function d(h){return String(h??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function _e(h){const t=String(h??"").trim();return/^https?:\/\//i.test(t)?t:""}function St(h,t){const e=String(h??"").trim();let s=e.match(/^#([0-9a-fA-F]{3,8})$/);if(s){let n=s[1];if((n.length===3||n.length===4)&&(n=n.split("").map(r=>r+r).join("")),n.length===6||n.length===8){const r=parseInt(n.slice(0,2),16),a=parseInt(n.slice(2,4),16),o=parseInt(n.slice(4,6),16);return`rgba(${r}, ${a}, ${o}, ${t})`}}if(s=e.match(/^rgba?\(([^)]+)\)$/i),s){const n=s[1].split(",").map(r=>r.trim());if(n.length>=3)return`rgba(${n[0]}, ${n[1]}, ${n[2]}, ${t})`}return e}function Qt(h,t){const e=parseFloat(h);return isNaN(e)?t:e}function Ot(h){return h?Array.from(h.querySelectorAll('a[href],button:not([disabled]),input:not([disabled]),select:not([disabled]),textarea:not([disabled]),[tabindex]:not([tabindex="-1"])')).filter(e=>e.getClientRects().length>0):[]}function Tt(h){return h==null||h===""?"(none)":h===!0?"on":h===!1?"off":Array.isArray(h)?h.join(", ")||"(none)":String(h)}function Zt(h,t,e){return h.slice().sort((s,n)=>{const r=t(s),a=t(n);return r==null&&a==null?0:r==null?1:a==null?-1:(r<a?-1:r>a?1:0)*e})}function ft(h,t,e,s,n,r,a){const o=e?s===1?" \u25B2":" \u25BC":' <span style="opacity:.35">\u2195</span>',i=r==="right"?"text-align:right;":"",l=a?` title="${d(a)}"`:"";return`<th class="wd-th-sort" data-sortcol="${t}" data-sortact="${n}" style="cursor:pointer;user-select:none;${i}"${l}>${h}${o}</th>`}function Mt(h){h=Math.max(0,Math.round(h));const t=Math.floor(h/3600),e=Math.floor(h%3600/60),s=h%60;return t>0?`${t}:${String(e).padStart(2,"0")}:${String(s).padStart(2,"0")}`:`${e}:${String(s).padStart(2,"0")}`}function te(h,t){const e={pause:["lbl.artifact_interruption","Interruption"],dip:["lbl.artifact_low_power","Low power"],spike:["lbl.artifact_high_power","High power"]},[s,n]=e[h]||["lbl.artifact_anomaly","Anomaly"];return t?t(s,{},n):n}function ee(h){return h.toLowerCase().replace(/[\s&/\-]+/g,"_").replace(/^_+|_+$/g,"").replace(/_+/g,"_")}function se(h){const t=new Set;return String(h??"").split(",").forEach(e=>{const s=parseInt(e.trim(),10);Number.isFinite(s)&&s>0&&t.add(s)}),Array.from(t).sort((e,s)=>e-s)}function Dt(h,t){if(!h||!h.length)return null;if(t<=h[0][0])return h[0][1];if(t>=h[h.length-1][0])return h[h.length-1][1];for(let e=1;e<h.length;e++)if(h[e][0]>=t){const s=h[e-1],n=h[e],r=n[0]-s[0]||1;return s[1]+(n[1]-s[1])*((t-s[0])/r)}return h[h.length-1][1]}function ge(h,t,e){e=e||{};const s=h.key,n=h.unit?`${h.label} (${h.unit})`:h.label,r=h.unit?` ${h.unit}`:"",a=h.doc?it(h.doc,h.diagram||ue[s]):"",o=e.changed?`<span class="wd-chg-dot" title="${d(e.changed)}" aria-label="${d(e.changed)}"></span>`:"";if(h.type==="checkbox")return`<div class="wd-field wd-field-switch"><div class="wd-switch-row"><label class="wd-switch-lbl"><span class="wd-switch"><input type="checkbox" data-opt="${s}" ${t?"checked":""}><span class="wd-switch-slider"></span></span><span class="wd-switch-text">${d(h.label)}</span></label>${o}${a}</div>${h.hint?`<div class="wd-field-hint">${d(h.hint)}</div>`:""}</div>`;let i="";const l=t??"";if(h.type==="select"||h.type==="devicetype"||h.type==="device"){const E=(e.opts||[]).map(([R,B])=>`<option value="${d(R)}" ${String(l)===String(R)?"selected":""}>${d(B)}</option>`).join("");i=`<select data-opt="${s}" data-ftype="${h.type}">${E}</select>`}else if(h.type==="textarea")i=`<textarea data-opt="${s}" data-ftype="textarea">${d(l)}</textarea>`;else if(h.type==="json"){const S=l===""||l==null?"":typeof l=="string"?l:JSON.stringify(l,null,2);i=`<textarea data-opt="${s}" data-ftype="json" rows="3" placeholder='${d(e.t("placeholder.json_buttons",{},'[{"action":"ID","title":"Label"}]'))}'>${d(S)}</textarea>`}else if(h.type==="checkboxlist"){const S=Array.isArray(t)&&t.length?t.map(String):Array.isArray(h.def)?h.def.map(String):[];i=`<div data-opt="${s}" data-ftype="checkboxlist" class="wd-checkboxlist">`+(h.choices||[]).map(([E,R])=>`<label class="wd-switch-lbl" style="display:flex;align-items:center;gap:8px;margin:2px 0"><span class="wd-switch"><input type="checkbox" data-choice="${d(E)}" ${S.includes(String(E))?"checked":""}><span class="wd-switch-slider"></span></span><span class="wd-switch-text">${d(e.t?e.t("lbl.evidence_"+E,{},R):R)}</span></label>`).join("")+"</div>"}else if(h.type==="entitylist"){const E=(Array.isArray(t)?t:t?[t]:[]).map(R=>`<span class="wd-pill" data-val="${d(R)}">${d(R)}<button type="button" class="wd-pill-x" aria-label="${d(e.t?e.t("btn.remove",{},"Remove"):"Remove")}">\xD7</button></span>`).join("");i=`<div class="wd-pillbox" data-opt="${s}" data-ftype="entitylist">${E}<div class="wd-combo wd-combo-pill"><input type="text" class="wd-pill-add" autocomplete="off" spellcheck="false" placeholder="${d(e.t("placeholder."+(h.domain||"add"),{},h.placeholder||"add\u2026"))}"><div class="wd-combo-drop" hidden></div></div></div>`}else if(h.type==="timerlist"){const S=Array.isArray(l)?l:[],E=e.t?e.t("lbl.timer_min",{},"min"):"min",R=e.t?e.t("lbl.timer_msg_placeholder",{},"Message (optional, {device}/{program}/{minutes})"):"Message (optional, {device}/{program}/{minutes})",B=e.t?e.t("lbl.timer_auto_pause",{},"Auto-pause"):"Auto-pause",W=e.t?e.t("btn.remove_timer",{},"Delete"):"Delete",O=e.t?e.t("btn.add_timer",{},"+ Add timer"):"+ Add timer",D=(C,M)=>{const H=C&&C.offset_minutes?String(C.offset_minutes):"",q=C&&C.message?d(C.message):"",Q=C&&C.auto_pause?" checked":"";return`<div class="wd-timer-row" data-tidx="${M}"><div class="wd-timer-top"><input type="number" min="1" placeholder="${d(E)}" data-field="offset_minutes" value="${d(H)}"><textarea placeholder="${d(R)}" data-field="message">${q}</textarea></div><div class="wd-timer-footer"><label class="wd-switch-lbl"><span class="wd-switch"><input type="checkbox" data-field="auto_pause"${Q}><span class="wd-switch-slider"></span></span><span class="wd-switch-text">${d(B)}</span></label><button type="button" class="wd-btn wd-btn-sm wd-btn-danger wd-timer-remove">${d(W)}</button></div></div>`},A=S.map((C,M)=>D(C,M)).join("");i=`<div class="wd-timerlist" data-opt="${s}" data-ftype="timerlist">${A}<button type="button" class="wd-btn wd-btn-sm wd-btn-secondary wd-timer-add">${d(O)}</button></div>`}else if(h.type==="entity"){const S=h.placeholder?` placeholder="${d(h.placeholder)}"`:"";i=`<div class="wd-combo"><input type="text" class="wd-combo-inp" data-opt="${s}" data-ftype="entity" value="${d(l)}" autocomplete="off" spellcheck="false"${S}><div class="wd-combo-drop" hidden></div></div>`}else if(h.type==="list"){const S=Array.isArray(l)?l.join(", "):d(l);i=`<input type="text" data-opt="${s}" data-ftype="list" value="${d(S)}" placeholder="${d(e.t("placeholder.notify_services_list",{},"notify.mobile_app_phone, ..."))}">`}else if(h.type==="intlist"){const S=Array.isArray(l)?l.join(", "):String(l??""),E=h.placeholder?` placeholder="${d(h.placeholder)}"`:"";i=`<input type="text" data-opt="${s}" data-ftype="intlist" value="${d(S)}"${E}>`}else{const S=h.type==="number"?"number":"text",E=e.datalistId?` list="${e.datalistId}"`:"",R=h.step!=null?` step="${h.step}"`:"",B=h.min!=null?` min="${h.min}"`:"",W=h.max!=null?` max="${h.max}"`:"",O=h.placeholder?` placeholder="${d(h.placeholder)}"`:"";i=`<input type="${S}" data-opt="${s}" data-ftype="${h.type}" value="${d(l)}"${R}${B}${W}${E}${O}>${e.datalist||""}`}const c=e.suggestion,_=e.mlSuggestion,p=c&&c.suggested!=null&&!Nt(c.suggested,t)?c.suggested:null,g=_&&_.value!=null&&!Nt(_.value,t)?_.value:null,u=e.t,b=c?c.reason_key?u(c.reason_key,c.reason_params||{},c.reason||""):c.reason||"":"",k=_?_.reason_key?u(_.reason_key,_.reason_params||{},_.reason||""):_.reason||"":"",$=S=>`<button type="button" class="wd-sug-use" data-sugkey="${s}" data-sugval="${d(S)}">${e.useBtnLabel||"Use"}</button>`;let x="";if(p!=null&&g!=null){const S=parseFloat(p),E=parseFloat(g);if((!isNaN(S)&&!isNaN(E)?Math.abs(S-E)/Math.max(Math.abs(S),Math.abs(E),1e-9):1)<.05){const B=u("suggestion.calibrated_label",{},"Calibrated"),W=it([b,k?`${B}: ${k}`:""].filter(Boolean).join(`

`));x=`<div class="wd-sug"><span class="wd-sug-chip wd-sug-chip-obs">\u{1F4A1} ${d(u("suggestion.both_agree",{},"WashData recommends"))}</span><span class="wd-sug-val">${d(p)}${r}</span>${$(p)}${W}</div>`}else{const B=b?it(b):"",W=k?it(k):"",O=u("suggestion.observed_label",{},"Observed"),D=u("suggestion.calibrated_label",{},"Calibrated");let A="",C="";if(!isNaN(S)&&!isNaN(E)){const q=E>S;C=u(`suggestion.impact.${s}.${q?"higher":"lower"}`,{},""),A=u(`suggestion.impact.${s}.${q?"lower":"higher"}`,{},"")}const M=A?`<div class="wd-sug-impact-line">${d(A)}</div>`:"",H=C?`<div class="wd-sug-impact-line">${d(C)}</div>`:"";x=`<div class="wd-sug wd-sug-split"><div class="wd-sug-opt"><span class="wd-sug-chip wd-sug-chip-obs">\u{1F4A1} ${d(O)}</span><span class="wd-sug-val">${d(p)}${r}</span>${$(p)}${B}${M}</div><div class="wd-sug-opt"><span class="wd-sug-chip wd-sug-chip-cal">\u{1F916} ${d(D)}</span><span class="wd-sug-val">${d(g)}${r}</span>${$(g)}${W}${H}</div></div>`}}else if(p!=null){const S=b?it(b):"",E=t!=null&&t!==""?` <span style="opacity:.6;font-size:.9em">(now ${d(t)}${r})</span>`:"";x=`<div class="wd-sug"><span class="wd-sug-chip wd-sug-chip-obs">\u{1F4A1} ${d(u("suggestion.observed_label",{},"Observed"))}</span><span class="wd-sug-val">${d(p)}${r}</span>${E}${$(p)}${S}</div>`}else if(g!=null){const S=k?it(k):"";x=`<div class="wd-sug"><span class="wd-sug-chip wd-sug-chip-cal">\u{1F916} ${d(u("suggestion.calibrated_label",{},"Calibrated"))}</span><span class="wd-sug-val">${d(g)}${r}</span>${$(g)}${S}</div>`}if(x&&s){const S=d(u("btn.mute_suggestion",{},"Stop suggesting this setting")),E=`<button type="button" class="wd-sug-lock" data-suglock="${s}" title="${S}" aria-label="${S}">\u{1F515}</button>`;x=x.replace(/<\/div>\s*$/,E+"</div>")}return`<div class="wd-field" data-field="${s}"><div class="wd-label-row"><label style="margin:0">${d(n)}</label>${o}${a}</div>${i}${h.hint?`<div class="wd-field-hint">${d(h.hint)}</div>`:""}<div class="wd-conflict-err" data-cerr="${s}" hidden></div>${x}</div>`}function Nt(h,t){if(h==null||t==null)return!1;const e=parseFloat(h),s=parseFloat(t);return!isNaN(e)&&!isNaN(s)?Math.abs(e-s)<1e-6:String(h)===String(t)}const ue={min_power:"min_power",off_delay:"off_delay",smoothing_window:"smoothing",start_threshold_w:"hysteresis",stop_threshold_w:"hysteresis",start_energy_threshold:"start_energy",running_dead_zone:"dead_zone",profile_duration_tolerance:"duration_tolerance",profile_match_min_duration_ratio:"match_ratios",profile_match_max_duration_ratio:"match_ratios",progress_reset_delay:"progress_reset",completion_min_seconds:"min_duration",min_off_gap:"min_off_gap",start_duration_threshold:"start_duration",end_energy_threshold:"end_energy_thresh",end_repeat_count:"end_repeat",profile_match_threshold:"confidence",profile_unmatch_threshold:"confidence",auto_label_confidence:"confidence",learning_confidence:"confidence",no_update_active_timeout:"watchdog_timeout",anti_wrinkle_enabled:"anti_wrinkle",anti_wrinkle_max_power:"anti_wrinkle",anti_wrinkle_max_duration:"anti_wrinkle",anti_wrinkle_exit_power:"anti_wrinkle",anti_wrinkle_idle_timeout:"anti_wrinkle",sampling_interval:"sampling"};function fe(h){let t=0,e=window.innerWidth;for(let s=h.parentElement;s;s=s.parentElement){if(getComputedStyle(s).overflowX==="visible")continue;const n=s.getBoundingClientRect();n.left>t&&(t=n.left),n.right<e&&(e=n.right)}return{left:t,right:e}}function it(h,t){return`<span class="wd-tip">i<span class="wd-tip-pop">${t?me(t):""}<span class="wd-tip-txt">${d(h)}</span></span></span>`}function jt(h,t,e=""){return`<label class="wd-switch-lbl"><span class="wd-switch"><input type="checkbox" ${h}><span class="wd-switch-slider"></span></span><span class="wd-switch-text wd-switch-text--plain">${d(t)}</span></label>${e||""}`}function Et(h,t,e="",s=""){return`<div class="wd-field wd-field-switch"><div class="wd-switch-row">${jt(h,t,e)}</div>${s?`<div class="wd-field-hint">${d(s)}</div>`:""}</div>`}function me(h){const t=s=>`<svg class="wd-dg" viewBox="0 0 200 90" preserveAspectRatio="xMidYMid meet">${s}</svg>`,e='<line class="ax" x1="8" y1="78" x2="192" y2="78"/>';switch(h){case"smoothing":return t(`${e}
    ...
    `}_htmlDeviceBar(){const t=this._isAdmin()?`<button class="wd-devcard wd-devadd" data-action="add-device" title="${d(this._t("btn.add_device_tip",{},"Add another WashData device"))}">${this._t("btn.add_device",{},"+ Add device")}</button>`:"";return this._devices.length<=1?t?`<div class="wd-devbar">${t}</div>`:"":`<div class="wd-devbar">${this._devices.map((e,s)=>{const n=e.is_user_paused?"user_paused":e.detector_state||"unknown",r=Bt.includes(n),a=!!e.recording,o=a?"var(--error-color, #f44336)":this._stateColor(n),i=a?this._t("status.recording",{},"Recording"):this._stateLabel(n),l=[],c=this._conflictCountForOpts(e.options||{},e.option_defaults||{});c&&l.push(`<span class="wd-dbadge conf">\u26A0 ${c}</span>`);const _=this._sugCountsForDevice(e).total;return _&&l.push(`<span class="wd-dbadge sug">\u{1F4A1} ${_}</span>`),e.feedback_count&&l.push(`<span class="wd-dbadge fb">\u{1F4AC} ${e.feedback_count}</span>`),`<button class="wd-devcard ${s===this._selIdx?"active":""}" data-idx="${s}">
        ...
        ${l.join("")}
      </button>`}).join("")}${t}</div>`}_htmlStatus(){const t=this._devices[this._selIdx];if(!t)return`<div class="wd-empty">${this._t("msg.no_device_selected",{},"No device selected.")}</div>`;const e=!!t.is_user_paused,s=e?"user_paused":t.detector_state||"unknown",n=!!t.recording,r=n?"var(--error-color, #f44336)":this._stateColor(s),a=n?this._t("status.recording",{},"Recording"):this._stateLabel(s),o=n||Bt.includes(s),i=t.cycle_progress_pct,l=t.time_remaining_s,c=t.current_program,_=!!t.manual_program,p=c||"auto_detect",g=(this._profiles||[]).map(v=>v.name);c&&!g.includes(c)&&g.unshift(c);const u=g.map(v=>`<option value="${d(v)}" ${p===v?"selected":""}>${d(v)}</option>`).join(""),b=c?_?this._t("badge.manual",{},"(manually selected)"):this._t("badge.auto",{},"(auto-detected)"):"",k=b?`<span class="wd-prog-tag ${_?"manual":"auto"}">${b}</span>`:"",$=`<div class="wd-prog-ctl"><label>${this._t("lbl.program",{},"Program")}</label>${it(this._t("lbl.program_tip",{},"Override which profile is matched to the current cycle. Auto-detect lets the integration pick the best match automatically. Pin a specific program to force-match it when auto-detect is wrong or you know what is running."))}
          <select id="wd-status-prog">
            <option value="auto_detect" ${p==="auto_detect"?"selected":""}>${this._t("status.auto_detect",{},"Auto-detect")}</option>
            ${u}
          </select>${k}</div>`,x=[];t.recording&&this._canEdit()&&x.push(`<div class="wd-attn-card"><span class="wd-attn-icon">\u25CF</span><div class="wd-attn-body"><div class="wd-attn-title">${this._t("msg.recording_in_progress",{},"Recording in progress")}</div><div class="wd-attn-sub">${this._t("msg.see_recorder",{},"See recorder widget below")}</div></div></div>`),t.feedback_count&&this._canEdit()&&x.push(`<button class="wd-attn-card" type="button" data-action="goto-feedbacks"><span class="wd-attn-icon">\u{1F4AC}</span><div class="wd-attn-body"><div class="wd-attn-title">${this._t("msg.feedback_cycles_pending",{n:t.feedback_count},`To review: ${t.feedback_count}`)}</div><div class="wd-attn-sub">${this._t("msg.review_to_cycles",{},"Open the Cycles review queue")}</div></div></button>`);const S=this._conflictKeysFromOpts();if(S.size&&this._canEdit()){const v=S.size,L=v>1?"s":"";x.push(`<button class="wd-attn-card" type="button" style="border-color:var(--error-color,#b71c1c)" data-action="goto-conflicts"><span class="wd-attn-icon">\u26A0</span><div class="wd-attn-body"><div class="wd-attn-title" style="color:var(--error-color,#b71c1c)">${this._t("conflict.attn_title",{n:v},`Setting conflicts: ${v}`)}</div><div class="wd-attn-sub">${this._t("conflict.attn_sub",{},"Fix conflicts before saving")}</div></div></button>`)}const E=this._sugCountsForDevice(t);if(E.total&&this._canEdit()){const v=E.total,L=[];E.classic&&L.push(this._t("lbl.n_classic_suggestions",{n:E.classic},`${E.classic} classic`)),E.ml&&L.push(this._t("lbl.n_ml_suggestions",{n:E.ml},`${E.ml} ML`)),x.push(`<button class="wd-attn-card" type="button" data-action="goto-suggestions"><span class="wd-attn-icon">\u{1F4A1}</span><div class="wd-attn-body"><div class="wd-attn-title">${this._t("lbl.n_tuning_suggestions",{n:v},`${v} tuning suggestion${v>1?"s":""}`)}</div><div class="wd-attn-sub">${L.join(" \xB7 ")} \xB7 ${this._t("msg.review_in_settings",{},"Review in Settings")}</div></div></button>`)}const R=x.length?`<div class="wd-attn">${x.join("")}</div>`:"",B=o&&i!=null?`
    ...
    `:"",O=((this._powerData||{}).live||[]).length>1,D=this._pref("show_expected",!0),A=this._pref("show_raw_active",!1),C=`<div class="wd-leg">
      ...
      ${this._pref("show_raw",!1)?`<label class="wd-leg-i"><input type="checkbox" data-statustoggle="show_raw_active" ${A?"checked":""}><span class="wd-leg-sw" style="background:#9e9e9e"></span> ${this._t("lbl.raw_socket",{},"Raw socket")}</label>`:""}
        ...
        <div class="wd-kv" style="margin-bottom:12px">
          ...
          <div class="wd-kv-item"><div class="wd-kv-val" style="font-size:1em;color:${v.ambiguous?"var(--warning-color,#ff9800)":"var(--success-color,#4caf50)"}">${v.ambiguous?this._t("status.ambiguous",{},"Ambiguous"):this._t("status.clear",{},"Clear")}</div><div class="wd-kv-lbl">${this._t("lbl.label",{},"Match")}</div></div>
        </div>
      ...
      <div class="wd-card">
        ...
        ${$}
        <div class="wd-stats">
          <div class="wd-stat"><div class="wd-stat-val">${Gt(t.current_power_w)}</div><div class="wd-stat-lbl">${this._t("lbl.power",{},"Power")}</div></div>
      ...
      ${U}
      ${f}
    `}_htmlSetupCard(t,e=!1){if(!t)return"";const{phase:s,message_key:n,message_params:r,cta_label_key:a,cta_action:o,secondary_label_key:i,secondary_action:l,skippable:c,dismissible:_,step_key:p}=t;if(s==="phase4")return`
        ...
        </div>`;const g=this._t(n,r||{},""),u=this._t(a,{},"Continue"),b=i?this._t(i,{},""):null,k=c?`
      ...
      <div class="wd-setup-card" data-phase="${d(s)}">
        ...
        <div style="display:flex;flex-wrap:wrap;gap:8px;align-items:center">
          ...
          ${b?`<a class="wd-link" data-action="setup-cta"
                           data-cta-action="${d(l||"")}"
                           data-cta-params="${d(JSON.stringify(r||{}))}">
            ${d(b)}
          </a>`:""}
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;flex-wrap:wrap">
    ...
    </div>`}_htmlHistory(){const t=this._cycles||[],e=this._refCycles||[],s=e.concat(t),n=this._canEdit(),r=this._selectMode&&n,a=this._cycleSel,{col:o,dir:i}=this._cycleSort,{text:l,status:c}=this._cycleFilter,_=this._mlById||{},p=new Set((this._feedbacks||[]).map(w=>w.cycle_id)),g=w=>_[w.id],u=w=>{const P=g(w);return!!(P&&P.ml_review&&P.ml_review.reviewed_at)},b=w=>{const P=g(w);return!!(P&&P.ml_review&&P.ml_review.golden)},k=w=>{if(p.has(w.id))return!0;if(u(w))return!1;const P=g(w),z=P&&P.ml_quality_label;return["uncertain","review"].includes(z)||["force_stopped","interrupted"].includes(w.status)},$=s.filter(k).length;let x=s;if(l){const w=l.toLowerCase();x=x.filter(P=>(P.profile_name||P.matched_profile||"").toLowerCase().includes(w))}c==="unlabelled"?x=x.filter(w=>!w.profile_name&&!w.matched_profile):c==="needs_review"?x=x.filter(k):c==="imported"?x=x.filter(w=>w.is_reference):c&&(x=x.filter(w=>(w.status||"completed")===c));const S={date:w=>w.start_time?new Date(w.start_time).getTime():0,confidence:w=>w.match_confidence,duration:w=>w.duration,energy:w=>w.energy_kwh!=null?w.energy_kwh:w.energy_wh!=null?w.energy_wh/1e3:null,cost:w=>w.cost!=null?w.cost:-1,status:w=>w.status||"completed",profile:w=>(w.profile_name||w.matched_profile||"\uFFFF").toLowerCase()};x=Zt(x,S[o]||S.date,i);const E=w=>w==="completed"?"var(--success-color, #4caf50)":w==="interrupted"?"var(--error-color, #f44336)":w==="force_stopped"?"var(--warning-color, #ff9800)":"var(--secondary-text-color)",R=w=>{if(!w.is_reference)return"";const P=w.cycle_origin==="backfill",z=P?this._t("badge.backfilled_tip",{},"Detected in imported power history. Shapes program matching only, not counted in stats."):this._t("badge.imported_tip",{},"Imported from the community store. Used for matching only, not counted in stats.");return` <span title="${d(z)}" style="color:var(--info-color,#2196f3)">${P?"\u{1F557}":"\u{1F4E5}"}</span>`},B=w=>b(w)?' <span title="'+d(this._t("badge.golden_cycle",{},"Recorded reference cycle"))+'" style="color:var(--warning-color,#ff9800)">\u2B50</span>':p.has(w.id)?' <span title="'+d(this._t("badge.feedback_requested",{},"Feedback requested"))+'" style="color:var(--info-color,#2196f3)">\u{1F4AC}</span>':u(w)?' <span title="'+d(this._t("badge.reviewed",{},"Reviewed"))+'" style="color:var(--success-color,#4caf50)">\u2713</span>':k(w)?' <span title="'+d(this._t("badge.needs_review",{},"Needs review"))+'" style="color:var(--error-color,#f44336)">\u25CF</span>':"",W=w=>{if(w.anomaly!=="overrun")return"";const P=w.overrun_ratio?" "+this._t("badge.overrun_ratio",{x:Number(w.overrun_ratio).toFixed(1)},`(${Number(w.overrun_ratio).toFixed(1)}x expected)`):"";return` <span title="${d(this._t("badge.overrun",{},"Ran longer than usual"))}${d(P)}" style="color:var(--warning-color,#ff9800)">\u23F1</span>`},O=w=>{if(w.anomaly!=="underrun")return"";const P=w.underrun_ratio?" "+this._t("badge.underrun_ratio",{pct:Math.round(w.underrun_ratio*100)},`(${Math.round(w.underrun_ratio*100)}% of expected)`):"";return` <span title="${d(this._t("badge.underrun",{},"Finished faster than usual"))}${d(P)}" style="color:var(--info-color,#2196f3)">\u26A1</span>`},D=w=>{if(!w.energy_anomaly||w.energy_anomaly==="none")return"";const P=w.energy_anomaly==="energy_spike",z=w.energy_z_score!=null?` (${w.energy_z_score>0?"+":""}${Number(w.energy_z_score).toFixed(1)}\u03C3)`:"",F=P?"badge.energy_spike":"badge.energy_low",K=P?"Higher energy than usual":"Lower energy than usual",Z=P?"\u{1F53A}":"\u{1F53B}",rt=P?"var(--error-color,#f44336)":"var(--info-color,#2196f3)";return` <span title="${d(this._t(F,{},K))}${d(z)}" style="color:${rt}">${Z}</span>`},A=w=>{const P=Array.isArray(w.artifacts)?w.artifacts.length:0;return P?` <span title="${d(this._t("badge.artifact_tip",{n:P},`${P} anomal${P>1?"ies":"y"} detected (e.g. door opened mid-cycle) \u2014 open to see them on the graph`))}" style="color:var(--warning-color,#ff9800)">\u26A0</span>`:""},C=w=>{const P=Array.isArray(w.restart_gaps)?w.restart_gaps.length:0;return P?` <span title="${d(this._t("badge.restart_gap_tip",{n:P},`${P} HA restart gap${P>1?"s":""} during this cycle \u2014 power trace has a hole`))}" style="color:var(--info-color,#2196f3)">\u21BB</span>`:""},M=this._hass&&this._hass.config&&this._hass.config.currency||"",H=w=>w.cost!=null?`${w.cost.toFixed(2)}${M?" "+M:""}`:"-",q=x.map(w=>{const P=w.profile_name||w.matched_profile,z=w.match_confidence!=null?w.match_confidence*100:null,F=w.status||"completed",K=w.energy_kwh!=null?w.energy_kwh:w.energy_wh!=null?w.energy_wh/1e3:null,Z=r,rt=Z?`<input type="checkbox" class="wd-csel" ${a.has(w.id)?"checked":""} style="width:auto;margin:0">`:`<span class="wd-devdot" style="background:${E(F)}" title="${d(F)}"></span>`,ht={completed:this._t("status.completed",{},"Completed"),interrupted:this._t("status.interrupted",{},"Interrupted"),force_stopped:this._t("status.force_stopped",{},"Force stopped"),active:this._t("status.active",{},"Active")}[F]||F,nt=`${R(w)}${B(w)}${W(w)}${O(w)}${D(w)}${A(w)}${C(w)}`.trim();return`<tr data-cid="${d(w.id)}" data-selmode="${Z?1:0}" style="cursor:pointer">
        ...
        <td><span style="color:${E(F)};font-size:.9em">${d(ht)}</span></td>
    ...
    </div>`,G=x.length!==s.length?this._t("lbl.n_shown",{n:x.length},`, ${x.length} shown`):"",X=e.length?this._t("lbl.n_imported_note",{n:e.length},`, ${e.length} imported`):"",ct=this._t("lbl.cycles_title",{n:`${t.length}${X}${G}`},`Cycles (${t.length}${X}${G})`),U=n?`<div class="wd-card-actions" style="margin:0 0 4px;justify-content:flex-end">
    ...
    </div>`:"",at=new Set(e.map(w=>w.id)),f=[...a].some(w=>at.has(w)),m=r?`<div class="wd-card-actions" style="margin:0 0 10px">
    ...
    </div>`:"",v=this._busy.has("cyc-load-more"),L=this._cyclesHasMore?`<div style="text-align:center;margin-top:12px">
      <button class="wd-btn wd-btn-secondary wd-btn-sm" data-action="cyc-load-more" ${v?"disabled":""}>${v?'<span class="wd-spin"></span> ':""}${this._t("btn.load_more",{},"Load more")}</button>
    </div>`:"",j=`
      <div class="wd-card">
        <div class="wd-card-title">${ct}</div>
        ${J}
        ${U}${m}
        ${x.length===0?`<div class="wd-empty" style="padding:24px"><div class="wd-icon">\u{1F4CB}</div>${s.length?this._t("msg.no_cycles_match",{},"No cycles match the current filter."):this._t("msg.no_cycles_yet",{},"No cycles recorded yet.")}</div>`:`<div class="wd-table-wrap"><table class="wd-table">${Q}<tbody>${q}</tbody></table></div>`}
        ${L}
      </div>`;return(this._cyclesError?`<div class="wd-error-state"><span>${this._t("msg.fetch_error",{},"Failed to load data.")}</span><button class="wd-btn" type="button" data-action="retry-cycles">${this._t("btn.retry",{},"Retry")}</button></div>`:"")+j}_trendIcon(t){return t==="up"?`<span title="${d(this._t("trend.up",{},"Trending up"))}" style="color:var(--warning-color,#ff9800)">\u2191</span>`:t==="down"?`<span title="${d(this._t("trend.down",{},"Trending down"))}" style="color:var(--info-color,#2196f3)">\u2193</span>`:""}_profileCardHtml(t){const e=t.avg_duration?this._t("lbl.duration_avg",{v:Math.round(t.avg_duration/60)},`~${Math.round(t.avg_duration/60)}m avg`):this._t("lbl.no_duration",{},"no duration"),s=t.avg_energy!=null?` \xB7 ${gt(t.avg_energy)}/cycle`:"",n=t.avg_energy!=null&&t.cycle_count?` \xB7 <strong>${gt(t.avg_energy*t.cycle_count)}</strong> total`:"",r=this._hass&&this._hass.config&&this._hass.config.currency||"",a=t.avg_cost!=null?` \xB7 ${this._t("lbl.avg_cost",{},"Avg")} ${t.avg_cost.toFixed(2)}${r?" "+r:""}/${this._t("lbl.per_cycle_short",{},"cycle")}`:"",o=(this._profileHealth||{})[t.name],i=(this._profileTrends||{})[t.name];let l="";o&&o.health_status==="poor"?l=`<span class="wd-badge" style="color:var(--error-color,#f44336);background:rgba(244,67,54,.12)" title="${d(this._t("badge.poor_fit_tip",{},"Inconsistent match history \u2014 consider rebuilding this profile"))}">\u26A0 ${this._t("badge.poor_fit",{},"poor fit")}</span>`:o&&o.health_status==="fair"&&(l=`<span class="wd-badge" style="color:var(--warning-color,#ff9800);background:rgba(255,152,0,.12)" title="${d(this._t("badge.fair_fit_tip",{},"Moderate match consistency \u2014 some cycles assigned to this profile have lower confidence scores. Label more cycles or re-record the profile to improve accuracy."))}">${this._t("badge.fair_fit",{},"fair fit")}</span>`);let c="";if(i){const x=this._trendIcon(i.duration_trend),S=i.energy_trend?this._trendIcon(i.energy_trend):"";if(i.duration_trend!=="stable"||i.energy_trend==="up"){const E=[];if(i.duration_trend!=="stable"){const B=`${i.duration_slope_pct>0?"+":""}${i.duration_slope_pct}`;E.push(i.duration_trend==="up"?this._t("msg.duration_trend_up_tip",{pct:B},`Duration up (${B}%/cycle)`):this._t("msg.duration_trend_down_tip",{pct:B},`Duration down (${B}%/cycle)`))}if(i.energy_trend&&i.energy_trend!=="stable"){const B=`${i.energy_slope_pct>0?"+":""}${i.energy_slope_pct}`;E.push(i.energy_trend==="up"?this._t("msg.energy_trend_up_tip",{pct:B},`Energy up (${B}%/cycle)`):this._t("msg.energy_trend_down_tip",{pct:B},`Energy down (${B}%/cycle)`))}const R=E.join(", ")||this._t("msg.performance_trending",{},"Performance trending");c=`<span class="wd-badge" style="color:var(--secondary-text-color,#888)" title="${d(R)}">${x}${S||""}</span>`}}const _=this._constants&&this._constants.PROFILE_MIN_WARMUP_CYCLES||5,p=o&&o.cycle_count||0,u=p<_&&!t.is_imported?`<span class="wd-badge" title="${d(this._t("msg.warmup_detail",{needed:_},`This profile needs ${_} labelled cycles before auto-matching begins. Every confirmed cycle helps it learn.`))}" style="background:var(--info-color,#2196f3);color:#fff">${this._t("msg.warmup_badge",{done:p,needed:_},`Still learning (${p}/${_} cycles)`)}</span>`:"",b=t.is_imported?`<span class="wd-badge" title="${d(this._t("badge.imported_tip",{},"Imported from the community store. Used for matching only, not counted in stats."))}" style="background:var(--info-color,#2196f3);color:#fff">\u{1F4E5} ${this._t("status.imported",{},"Imported")}</span>`:"",k=[l,c,u,b].filter(Boolean).join(" "),$=Array.isArray(t.signature_curve)&&t.signature_curve.length>=3?`<canvas class="wd-prof-spark" data-spark-prof="${d(t.name)}" width="64" height="20" aria-label="${d(this._t("lbl.sparkline",{name:t.name},"Average power curve"))}"></canvas>`:"";return`
      <div class="wd-prof-wrap">
        <button class="wd-profile-card" type="button" data-action="open-profile" data-pname="${d(t.name)}">
          ...
          ${k?`<div class="wd-profile-badges">${k}</div>`:""}
          <div class="wd-profile-meta">${t.cycle_count||0} cycles \xB7 ${e}${s}${n}${a}</div>
        </button>
      </div>`}_paintSparkline(t,e){if(!t||!Array.isArray(e)||e.length<3)return;const s=(getComputedStyle(this).getPropertyValue("--primary-color")||"#03a9f4").trim()||"#03a9f4",n=window.devicePixelRatio||1,r=t.getBoundingClientRect(),a=t.width=Math.max(1,Math.round((r.width||64)*n)),o=t.height=Math.max(1,Math.round((r.height||20)*n)),i=t.getContext("2d");i.clearRect(0,0,a,o);const l=Math.max(...e,1),c=2*n,_=g=>c+(e.length===1?0:g/(e.length-1)*(a-2*c)),p=g=>o-c-Math.max(0,g)/l*(o-2*c);i.beginPath(),e.forEach((g,u)=>{const b=_(u),k=p(g);u===0?i.moveTo(b,k):i.lineTo(b,k)}),i.strokeStyle=s,i.lineWidth=1.5*n,i.lineJoin="round",i.lineCap="round",i.stroke()}_drawProfileSparklines(){const t=this.shadowRoot;if(!t)return;const e=t.querySelectorAll("canvas[data-spark-prof]");if(!e.length)return;const s={};for(const n of this._profiles||[])s[n.name]=n;e.forEach(n=>{const r=s[n.dataset.sparkProf];this._paintSparkline(n,r&&r.signature_curve||[])})}_htmlProfiles(){const t=this._busy.has("rebuild-envelopes"),e=this._canEdit(),s={};this._profiles.forEach(u=>{s[u.name]=u});const n=this._profileGroups||{groups:[],suggestions:[]},r=new Set;n.groups.forEach(u=>(u.members||[]).forEach(b=>r.add(b)));const a=n.groups.map(u=>{const b=(u.members||[]).map(E=>s[E]?this._profileCardHtml(s[E]):"").join(""),k=Math.round((u.cohesion!=null?u.cohesion:1)*100),$=u.cohesive?`<span class="wd-badge" style="color:var(--success-color,#4caf50);background:rgba(76,175,80,.14);margin-bottom:0">${this._t("lbl.cohesion_good",{pct:k},"cohesion "+k+"%")}</span>`:`<span class="wd-badge" style="color:var(--warning-color,#ff9800);background:rgba(255,152,0,.14);margin-bottom:0">${this._t("lbl.cohesion_low",{pct:k},"\u26A0 low cohesion "+k+"%")}</span>`,x=u.cohesive?"":`<p class="wd-info" style="margin:0 0 8px;color:var(--warning-color,#ff9800)">${this._t("msg.group_not_cohesive",{},"These profiles aren't similar enough to group reliably, so matching treats them individually until you remove the outlier or split the group.")}</p>`;return`<div class="wd-card" style="margin-bottom:12px">
        ...
        ${x}
        <div class="wd-profiles-grid">${b}</div>
      </div>`}).join(""),o=this._profiles.filter(u=>!r.has(u.name)),i=o.map(u=>this._profileCardHtml(u)).join(""),l=this._profiles.length===0&&!(this._cycles||[]).length&&!(this._refCycles||[]).length,_=(e&&this._onlineEnabled()&&l?`
    ...
    `}_htmlProfileGroupModal(t){const e=this._busy.has("pg-save"),s=this._profileEnvCache||{},n=p=>et[Math.max(0,this._profiles.findIndex(g=>g.name===p))%et.length],r=t.members||[],a=this._profiles.map(p=>{const g=r.includes(p.name),u=p.avg_duration?`~${Math.round(p.avg_duration/60)}m`:"",b=p.avg_energy!=null?` \xB7 ${gt(p.avg_energy)}`:"",k=g?`<span style="display:inline-block;width:10px;height:10px;border-radius:2px;background:${n(p.name)};margin:0 2px"></span>`:"";return`<label class="wd-rev-tag"><input type="checkbox" class="wd-pg-mem" value="${d(p.name)}" ${g?"checked":""}> ${k}${d(p.name)} <span style="color:var(--secondary-text-color);font-size:.85em">${u}${b}</span></label>`}).join(""),o=r.filter(p=>s[p]&&(s[p].avg||[]).length),i=o.length?`<div class="wd-leg">${o.map(p=>`<span class="wd-leg-i"><span class="wd-leg-sw" style="background:${n(p)}"></span> ${d(p)}</span>`).join("")}</div>`:"",l=o.length?`<div class="wd-canvas-wrap" style="margin-top:8px"><canvas id="wd-pgroup-canvas" role="img" aria-label="${d(this._t("lbl.aria_envelope_chart",{},"Profile power envelope chart"))}" style="height:150px"></canvas></div>${i}`:`<p class="wd-info">${this._t("msg.group_preview_hint",{},"Tick 2+ members to preview and compare their power curves.")}</p>`,c=((this._profileGroups||{}).groups||[]).find(p=>p.name===t.orig),_=c&&c.cohesion!=null?`<span class="wd-badge" style="color:${c.cohesive?"var(--success-color,#4caf50)":"var(--warning-color,#ff9800)"};background:${c.cohesive?"rgba(76,175,80,.14)":"rgba(255,152,0,.14)"}">${c.cohesive?this._t("lbl.cohesion_good",{pct:Math.round(c.cohesion*100)},"cohesion "+Math.round(c.cohesion*100)+"%"):this._t("lbl.cohesion_low",{pct:Math.round(c.cohesion*100)},"\u26A0 low cohesion "+Math.round(c.cohesion*100)+"%")}</span>`:"";return`<h2>${t.orig?this._t("modal.edit_group",{},"Edit profile group"):this._t("modal.new_group",{},"New profile group")}</h2>
    ...
    </label>`,k=n?`<p class="wd-info" style="margin:0 0 10px;font-size:.82em">${this._t("msg.settings_basic_note",{},"Showing essential settings. Switch to Advanced for the full list.")}</p>`:"",$=this._busy.has("save-settings"),x=l.size,S=x!==1?"s":"",E=x?`
      <div class="wd-sug-banner" style="background:rgba(183,28,28,.10);border-color:rgba(183,28,28,.4);color:var(--error-color,#b71c1c)">
        <span>\u26A0 ${this._t("conflict.settings_banner",{n:x},`Setting conflicts: ${x}. Check the highlighted sections and fix them before saving.`)}</span>
      ...
      </div>`:"",R=this._suggestions.length,B=[...a].filter(U=>!r.has(U)).length,W=R+B,O=this._settingsSugOnly&&!this._settingsSearch,D=R?`<button class="wd-btn wd-btn-sm wd-btn-primary" data-action="sug-apply-all">${this._t("btn.apply_all",{},"Apply all")}</button>`:"",A=R?`<button class="wd-btn wd-btn-sm wd-btn-secondary" data-action="sug-dismiss">${this._t("btn.dismiss",{},"Dismiss")}</button>`:"",C=W?O?`
    ...
    `}_htmlSettingsHistory(){const t=this._settingsChangelog||[];if(!t.length)return"";const e=this._settingsHistoryOpen,s=this._canEdit(),n=t.slice(0,100).map(r=>{const a=s?`<button class="wd-btn wd-btn-secondary wd-btn-xs" data-action="settings-revert-key" data-key="${d(r.key)}" data-val="${d(JSON.stringify(r.old))}" title="${d(this._t("btn.revert_to_previous",{},"Restore this setting to its previous value"))}">${this._t("btn.revert",{},"Revert")}</button>`:"";return`<tr>
        ...
        <td>${d(Tt(r.old))} \u2192 ${d(Tt(r.new))}</td>
        <td class="wd-tc-date">${st(r.timestamp)}</td>
      ...
      <div class="wd-combo-row">
        ...
        <div class="wd-field-hint">${d(this._t("msg.pick_brand_first",{},"Pick an appliance brand first."))}</div></div>`;this._ensureCatalogEntry(),this._catalog.forBrand!==o&&(this._catalog.forBrand=o,this._catalog.devices=void 0);const i=Array.isArray(this._catalog.devices)?this._catalog.devices:[];this._entityListCache=this._entityListCache||{},this._entityListCache[t]=i.map(u=>u.model).filter(Boolean);const l=this._catalogEntryFor(s,"device"),c=this._statusTag(l),_=this._catalog.devices===null?` <span class="wd-info" style="font-size:.85em">${this._t("msg.loading",{},"Loading\u2026")}</span>`:"";let p="";if(l){const u=[],b=_e(l.manualUrl);b&&u.push(`<a href="${d(b)}" target="_blank" rel="noopener noreferrer nofollow">${this._t("link.manual",{},"Manual \u2197")}</a>`),(this._constants&&this._constants.storePrefs||{}).show_contributor!==!1&&u.push(this._t("store.contributed_by",{name:d(l.createdByName||this._t("lbl.anonymous",{},"Anonymous"))},`by ${d(l.createdByName||"Anonymous")}`));const k=!!(this._storeStatus&&this._storeStatus.connected);let $="";if(k){const x=[1,2,3,4,5].map(S=>`<button type="button" class="wd-star-btn" data-action="store-rate-device" data-device-id="${d(l.id)}" data-rating="${S}" aria-label="${this._t("lbl.rate_n",{n:S},`${S} stars`)}">\u2605</button>`).join("");$=`<button type="button" class="wd-btn wd-btn-secondary wd-btn-sm" data-action="store-confirm-device" data-device-id="${d(l.id)}" ${l.status!=="pending"?"disabled":""}>${l.status==="pending"?this._t("btn.confirm_appliance",{},"Confirm this appliance"):this._t("btn.confirmed",{},"Confirmed")}</button>
          <span class="wd-star-row" title="${d(this._t("lbl.rate_quality",{},"Rate quality"))}">${x}</span>`}else $=`<span class="wd-info" style="font-size:.85em">${this._t("msg.connect_to_confirm",{},"Connect in the settings gear to confirm or rate.")}</span>`;p=`<div class="wd-store-picker-detail">${u.join(" \xB7 ")}<div class="wd-store-picker-actions">${$}</div></div>`}const g=this._onlineEnabled()&&this._storeDeviceDeclared()?`<div class="wd-store-actions">
    ...
    </div>`:"";return`<div class="wd-field"><label>${d(n)} ${r?it(r):""}${c}${_}</label>
      <div class="wd-combo-row">
        <div class="wd-combo">
    ...
    `}_htmlMlStatusSection(t,e){const s=e&&this._busy.has("ml-train-now:"+e)||t&&t.running,n=this._canEdit()?`<button class="wd-btn wd-btn-primary wd-btn-sm" data-action="ml-train-now" ${s?"disabled":""}>${s?`<span class="wd-spin"></span> ${this._t("status.training",{},"Training\u2026")}`:this._t("btn.train_now",{},"Train now")}</button>`:"";if(!t)return`<div class="wd-card"><div class="wd-card-title" style="margin:0 0 4px">${this._t("hdr.status",{},"Status")}</div><p class="wd-info" style="margin:0">${this._t("msg.loading",{},"Loading\u2026")}</p></div>`;const r=Object.keys(t.on_device_models||{}).length,a=r?`<span style="color:var(--success-color,#4caf50);font-weight:600">${this._t("ml.personalized",{},"\u25CF Personalized to this machine")}</span> <span style="color:var(--secondary-text-color)">${this._t("lbl.models_fine_tuned",{count:r},"(fine-tuned models: "+r+")")}</span>`:`<span style="color:var(--secondary-text-color)">${this._t("ml.builtin_models",{},"\u25CF Using built-in models")}</span>`,o=t.cycle_count||0,i=t.min_cycles||0,l=o>=i,c=i>0?Math.min(100,Math.round(o/i*100)):100,_=l?"var(--success-color,#4caf50)":"var(--warning-color,#ff9800)",p=Math.max(0,i-o),g=l?this._t("msg.enough_data",{current:o,min:i},`Enough data to learn from (${o}/${i} cycles).`):this._t("msg.collecting_data",{need:p,current:o,min:i},`Collecting data. Cycles still needed before fine-tuning can start: ${p} (${o}/${i}).`),u=`<div style="height:8px;border-radius:6px;background:var(--secondary-background-color);overflow:hidden;margin:8px 0"><div style="width:${c}%;height:100%;background:${_}"></div></div>`,b=t.last_trained?st(t.last_trained):"never",k=s?`<span style="color:var(--info-color,#2196f3)"><span class="wd-spin"></span> ${this._t("status.fine_tuning",{},"fine-tuning now\u2026")}</span>`:t.enabled?this._t("lbl.auto_fine_tune_on",{hour:String(t.hour).padStart(2,"0")},`auto fine-tune on (around ${String(t.hour).padStart(2,"0")}:00)`):this._t("lbl.auto_fine_tune_off",{},"auto fine-tune off");return`<div class="wd-card">
      ...
      ${u}
      <p class="wd-info" style="margin:0">${this._t("lbl.last_checked",{},"Last checked:")} <strong>${d(b)}</strong> \xB7 ${k}</p>
      ...
      ${n}
        ...
        </div>`,r=k);const C=this._pgStagedVal(p),M=C!=null&&!(S!==""&&S!==null&&S!==void 0&&this._pgSameVal(C,S)),H=this._canEdit()&&M&&this._pgIsPublishable(p)?`<button class="wd-pg-pub" data-action="pg-publish-one" data-pgkey="${d(p)}" aria-label="${d(this._t("btn.pg_publish_one",{label:x},"Publish "+x+" to the integration"))}" title="${d(this._t("btn.pg_publish_one",{label:x},"Publish "+x+" to the integration"))}">\u2191</button>`:'<span class="wd-pg-pub-slot" aria-hidden="true"></span>';return`${A}<div style="display:flex;align-items:flex-start;gap:6px;margin:0 0 6px 11px">
        <div style="flex:1;min-width:0">
          <div style="font-size:.82em;font-weight:600;margin-bottom:1px">${d(x)}${B?` <span style="color:${O};font-size:.85em" title="${d(this._t("lbl.pg_drag_hint",{},"Drag line on graph"))}">\u2195</span>`:""}</div>
          ${b?`<div style="font-size:.72em;color:var(--secondary-text-color);line-height:1.3">${d(this._t("pg_desc."+p,{},b))}</div>`:""}
        </div>
        <div style="display:flex;align-items:center;gap:4px;flex-shrink:0">
          ${R?`<input class="wd-pg-param-chk" type="checkbox" data-pgkey="${d(p)}" data-pgtype="bool" aria-label="${d(x)}" ${E?"checked":""} style="width:18px;height:18px;margin:1px 27px 0 27px">`:`<input class="wd-pg-param-inp" type="text" inputmode="decimal" data-pgkey="${d(p)}" value="${E!==""?d(String(E)):""}" placeholder="${S!==""?d(String(S)):""}" aria-label="${d(x)}" style="width:72px">`}
    ...
    </div>`,r=t?this._htmlPgBatchBar():"",a=`<p class="wd-sec-intro" style="margin:0 0 8px">${this._t("msg.pg_history_intro2",{},"Replay your recent cycles through the real detector and matcher with the settings above. Click any row to load that cycle in the graph; edit a setting to see a before/after comparison.")}</p>`;if(!e||!Array.isArray(e.rows))return`${a}${n}${this._htmlPgRecentRuns("pg_history")}${r}${t?"":`<div class="wd-empty" style="padding:24px">${this._t("msg.pg_history_empty",{},"Press Run to replay your cycles.")}</div>`}`;let o="";if(e.diff){const u=(b,k,$)=>`<span class="wd-pg-diffbadge" style="background:${k}22;color:${k}">${b} ${d($)}</span>`;o=`<div style="margin:0 0 10px">
    ...
    </tr></thead><tbody>${p}</tbody></table>`;return`${a}${n}${this._htmlPgRecentRuns("pg_history")}${o}${l}${g}`}_htmlPgBatchBar(){const t=this._pgBatchProgress;return t?`<div class="wd-pg-batchrow">
    ...
    </div>`,o=t?this._htmlPgBatchBar():"";return`${r}${a}${this._htmlPgRecentRuns("pg_sweep")}${o}${this._htmlPgSweepResult()}`}_htmlPgSweepResult(){const t=this._pgSweepNew;if(!t||!Array.isArray(t.points)||!t.points.length)return"";const e=this._pgSweepObjectives().find(p=>p[0]===t.objective),s=e?e[2]:!1,n=t.points.filter(p=>p.metric!=null).map(p=>p.metric);if(!n.length)return`<div class="wd-empty" style="padding:16px">${this._t("msg.pg_sweep_no_metric",{},"Not enough data to score this objective.")}</div>`;const r=Math.min(...n),a=Math.max(...n),o=s?Math.min(...n):Math.max(...n),i=(t.points.find(p=>p.metric===o)||{}).value,l=p=>t.objective==="median_overrun"?Math.round(p*100)+"% off":Math.round(p*100)+"%",c=t.points.map(p=>{const g=a>r&&p.metric!=null?(p.metric-r)/(a-r):p.metric!=null?1:0,u=p.metric===o,b=t.current_value!=null&&Math.abs(p.value-t.current_value)<1e-6,k=u?"var(--success-color,#4caf50)":"var(--primary-color)";return`<div style="display:flex;align-items:center;gap:8px;margin:3px 0;font-size:.82em">
        <span style="flex:0 0 74px;text-align:right;font-variant-numeric:tabular-nums">${d(String(p.value))}${b?" \u25C0":""}</span>
        <div style="flex:1;height:16px;border-radius:4px;background:var(--secondary-background-color);overflow:hidden"><div style="height:100%;width:${Math.round((p.metric!=null?.15+.85*g:0)*100)}%;background:${k};border-radius:4px"></div></div>
      ...
      </div>`};let s="";if(t&&(t.stage2||t.stage4)){const i=t.stage2||{},l=t.stage4||{},c=t.dtw||{},_=l.final_score??c.blended_score??i.score,p=this._pgDetail&&this._pgDetail.outcome&&this._pgDetail.outcome.confidence,g=p??_;let u="\u2014",b="var(--secondary-text-color)";g!=null&&(g>=.7?(u="\u2705 "+this._t("lbl.pg_strong_match",{},"Strong match"),b="var(--success-color, #4caf50)"):g>=.4?(u="\u26A0 "+this._t("lbl.pg_weak_match",{},"Weak match"),b="var(--warning-color, #ff9800)"):(u="\u274C "+this._t("lbl.pg_poor_match",{},"Poor match"),b="var(--error-color, #f44336)"));const k=t.profile_name||this._pgProfileName||"\u2014";s+=`<div style="font-weight:700;font-size:.9em;color:${b};margin-bottom:8px">${u}</div>`,k!=="\u2014"&&(s+=`<div style="font-size:.82em;color:var(--secondary-text-color);margin-bottom:6px">${d(k)} \xB7 ${d(this._t("lbl.score",{},"score"))} ${_!=null?_.toFixed(3):"\u2014"}</div>`),s+=e(this._t("lbl.correlation",{},"Correlation"),i.correlation,1,"#42a5f5",i.correlation!=null?i.correlation.toFixed(2):null),c.blended_score!=null&&(s+=e(this._t("lbl.pg_dtw",{},"DTW"),c.blended_score,1,"#ab47bc",c.blended_score.toFixed(2))),l.duration_agreement!=null&&(s+=e(this._t("lbl.duration",{},"Duration"),l.duration_agreement,1,"#66bb6a",l.duration_agreement.toFixed(2))),l.energy_agreement!=null&&(s+=e(this._t("lbl.energy",{},"Energy"),l.energy_agreement,1,"#ffa726",l.energy_agreement.toFixed(2))),s+='<div style="height:1px;background:var(--divider-color,rgba(127,127,127,.2));margin:8px 0"></div>'}else t||(s+=`<p class="wd-info" style="margin:0 0 8px">${this._t("msg.pg_analysis_empty2",{},"Press Run to see match analysis.")}</p>`);const n=this._pgDetail&&this._pgDetail.outcome,r=n&&n.matched_profile||t&&t.profile_name||this._pgProfileName||(this._cycles||[]).find(i=>i.id===this._pgCycleId)?.profile_name||"",a=n&&n.confidence;if(r&&a!=null){const i=Math.round(Math.max(0,Math.min(1,a))*100);s+=`<div class="wd-subhead" style="margin:0 0 4px">${d(this._t("lbl.pg_match_confidence",{},"Match confidence"))}</div>`,s+=`<div class="wd-pg-cand-row">
    ...
    </div>`:"",_=r.length?r.map(g=>{const u=t?`<button class="wd-btn wd-btn-danger wd-btn-sm" data-action="maint-delete" data-mid="${d(g.id)}">${this._t("btn.delete",{},"Delete")}</button>`:"",b=g.notes?`<div class="wd-info" style="margin-top:2px">${d(g.notes)}</div>`:"";return`<div class="wd-card" style="background:var(--secondary-background-color);padding:10px 12px">
    ...
    ${n||`<div class="wd-card"><p class="wd-info">${this._t("msg.no_other_users",{},"No other Home Assistant users found.")}</p></div>`}`}_htmlStore(){if(!this._canEdit())return`<div class="wd-empty">${this._t("msg.no_device_selected",{},"No device selected.")}</div>`;const t=this._storeStatus,e=`<a href="https://3dg1luk43.github.io/washdata-store" target="_blank" rel="noopener noreferrer" style="font-size:.8em;font-weight:400;color:var(--primary-color);text-decoration:none;white-space:nowrap" title="${d(this._t("store.website_tip",{},"Open the community store website"))}">${this._t("store.website",{},"Store website \u2197")}</a>`,s=`<div style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap"><div class="wd-card-title" style="margin:0">${this._t("hdr.community_store",{},"Community Store")}</div>${e}</div>`;if(!this._onlineEnabled()||t&&t.enabled===!1)return`<div class="wd-card">${s}
      ...
      ${jt(`data-action="store-toggle-dl-settings" ${this._dlSettings?"checked":""} ${r?"disabled":""}`,this._t("lbl.adopt_settings",{},"Also adopt settings"),it(this._t("msg.adopt_settings_hint",{},"Overwrite this device's detection & matching thresholds with the shared ones. Your notifications, entities and energy price are never changed.")))}
    ...
    </div>`:"")+s}_htmlStoreProfile(){const t=this._storeCycles||[],e=t.map(n=>{const r=n.stats||{},a=this._storeSparkline(n.trace&&n.trace.points||[]),o=r.duration!=null?ot(r.duration):"-",i=r.energy_wh!=null?gt(r.energy_wh/1e3):"-",l=r.peak_w!=null?Gt(r.peak_w):"-",c=d(n.uploaderName||this._t("store.anon",{},"anonymous")),_=this._statusTag(n),p=n.rating||{},g=p.avg!=null&&p.count?this._t("store.rating_summary",{avg:Number(p.avg).toFixed(1),n:p.count},`\u2605 ${Number(p.avg).toFixed(1)} (${p.count})`):this._t("store.no_ratings",{},"No ratings yet");return`<div class="wd-card">
        <div class="wd-store-cycle-top">
          ${a}
          <div class="wd-store-cycle-stats">
            <div><b>${o}</b> \xB7 ${i} \xB7 ${l} ${_}</div>
            <div class="wd-info">${this._t("store.uploaded_by",{name:c},`Shared by ${c}`)} \xB7 \u2B07 ${n.downloads||0} \xB7 ${g}</div>
          </div>
      ...
      ${Et(`data-action="store-toggle-online" ${t?"checked":""} ${n?"disabled":""}`,this._t("lbl.enable_online",{},"Enable online features"))}
      ${t?this._htmlStorePrefs(n):""}
      ${t?r:""}
    </div>`}_htmlStorePrefs(t){const e=this._constants&&this._constants.storePrefs||{},s=oe.map(r=>{const a=e[r.key]!==!1;return Et(`data-action="store-toggle-pref" data-pref="${d(r.key)}" ${a?"checked":""} ${t?"disabled":""}`,this._t(r.labelKey,{},r.labelFb),it(this._t(r.docKey,{},r.docFb)))}).join(""),n=this._busy.has("store-refresh-catalog");return s+`
      ...
      <div class="wd-log-drawer-body">
    ...
    </div>`}_drawCurves(t,e){const s=this.shadowRoot&&this.shadowRoot.getElementById(t);if(!s)return null;const n=s.getBoundingClientRect(),r=window.devicePixelRatio||1,a=Math.max(1,Math.round(n.width*r)),o=Math.max(1,Math.round((n.height||240)*r));(s.width!==a||s.height!==o)&&(s.width=a,s.height=o);const i=s.getContext("2d"),l=getComputedStyle(this),c=(l.getPropertyValue("--primary-color")||"#03a9f4").trim()||"#03a9f4",_=(l.getPropertyValue("--divider-color")||"rgba(127,127,127,.3)").trim()||"rgba(127,127,127,.3)",p=(l.getPropertyValue("--secondary-text-color")||"#888").trim()||"#888",g=44*r,u=12*r,b=12*r,k=22*r,$=e.series||[];let x=e.xMax||0;x||($.forEach(A=>(A.points||[]).forEach(C=>{C[0]>x&&(x=C[0])})),e.band&&(e.band.max||[]).forEach(A=>{A[0]>x&&(x=A[0])})),x=x||1;let S=e.yMax||0;if(!S){const A=C=>(C||[]).forEach(M=>{M[1]>S&&(S=M[1])});$.forEach(C=>{C.noScale||A(C.points)}),e.band&&A(e.band.max),S=(S||10)*1.08}const E=this._canvasZoom&&this._canvasZoom[t],R=E?E.xMin:0,B=E?E.xMax:x,W=a-g-u,O=A=>g+(A-R)/(B-R)*W,D=A=>o-k-A/S*(o-b-k);i.clearRect(0,0,a,o),i.strokeStyle=_,i.lineWidth=r,i.fillStyle=p,i.font=`${11*r}px sans-serif`,i.textAlign="right",i.textBaseline="middle";for(let A=0;A<=2;A++){const C=b+A/2*(o-b-k);i.beginPath(),i.moveTo(g,C),i.lineTo(a-u,C),i.stroke(),i.fillText(Math.round(S*(1-A/2))+"W",g-4*r,C)}if(i.save(),i.beginPath(),i.rect(g,b,W,o-b),i.clip(),(e.bands||[]).forEach(A=>{i.fillStyle=A.fill;const C=O(A.x0),M=O(A.x1);i.fillRect(Math.min(C,M),b,Math.abs(M-C),o-b-k)}),e.band&&(e.band.min||[]).length&&(e.band.max||[]).length){i.beginPath(),e.band.max.forEach((A,C)=>C?i.lineTo(O(A[0]),D(A[1])):i.moveTo(O(A[0]),D(A[1])));for(let A=e.band.min.length-1;A>=0;A--)i.lineTo(O(e.band.min[A][0]),D(e.band.min[A][1]));i.closePath(),i.fillStyle=e.band.fill||St(c,.13),i.fill()}return $.forEach(A=>{const C=A.points||[];if(!C.length)return;const M=A.stroke==="primary"?c:A.stroke;if(A.fill){i.beginPath(),C.forEach((q,Q)=>Q?i.lineTo(O(q[0]),D(q[1])):i.moveTo(O(q[0]),D(q[1]))),i.lineTo(O(C[C.length-1][0]),D(0)),i.lineTo(O(C[0][0]),D(0)),i.closePath();const H=i.createLinearGradient(0,b,0,o-k);H.addColorStop(0,St(M,.33)),H.addColorStop(1,St(M,.03)),i.fillStyle=H,i.fill()}i.beginPath(),C.forEach((H,q)=>q?i.lineTo(O(H[0]),D(H[1])):i.moveTo(O(H[0]),D(H[1]))),i.strokeStyle=M,i.lineWidth=(A.width||1.5)*r,i.lineJoin="round",A.dash&&i.setLineDash([6*r,4*r]),i.globalAlpha=A.alpha!=null?A.alpha:1,i.stroke(),i.globalAlpha=1,A.dash&&i.setLineDash([])}),i.textAlign="center",i.textBaseline="top",(e.vlines||[]).forEach(A=>{const C=O(A.x);i.beginPath(),i.moveTo(C,b),i.lineTo(C,o-k),i.strokeStyle=A.color,i.lineWidth=2*r,i.setLineDash([4*r,3*r]),i.stroke(),i.setLineDash([]),A.handle&&(i.fillStyle=A.color,i.beginPath(),i.arc(C,b+4*r,4.5*r,0,Math.PI*2),i.fill()),A.label&&(i.fillStyle=A.color,i.fillText(A.label,C,b+(A.handle?12*r:2*r)))}),i.restore(),i.fillStyle=p,i.font=`${11*r}px sans-serif`,i.textBaseline="bottom",E&&(i.textAlign="left",i.fillText((R/60).toFixed(1)+" min",g,o-2*r)),i.textAlign="right",i.fillText((B/60).toFixed(0)+" min",a-u,o-2*r),s._wd={xMax:x,xMin:R,xViewMax:B,yMax:S,dpr:r,padT:b,padB:k,ch:o,primary:c,Xpx:O,Ypx:D,xToCss:A=>O(A)/r,cssToX:A=>Math.max(R,Math.min(B,R+(A*r-g)/W*(B-R))),series:(e.series||[]).map(A=>({points:A.points,stroke:A.stroke,name:A.name,cid:A.cid})),band:e.band||null,artifacts:e.artifacts||null,_opts:e},s._wd}_drawModalCanvas(){const t=this._modal;t&&(t.type==="cycle-detail"?this._drawCycleEditor():t.type==="compare-cycles"?this._drawCompareCanvas():t.type==="profile-group"?this._drawGroupCanvas():t.type==="profile-panel"&&(t.tab==="stats"?this._drawProfileEnvelope():t.tab==="phases"?this._drawPhaseEditor():t.tab==="cleanup"&&this._drawSpaghetti()))}_redrawCanvas(t){if(t==="wd-status-canvas")this._drawStatusCurve();else if(t==="wd-cyc-canvas")this._drawCycleEditor();else if(t==="wd-compare-canvas")this._drawCompareCanvas();else if(t==="wd-env-canvas")this._drawProfileEnvelope();else if(t==="wd-phase-canvas")this._drawPhaseEditor();else if(t==="wd-spag-canvas")this._drawSpaghetti();else if(t==="wd-pgroup-canvas")this._drawGroupCanvas();else{const e=this.shadowRoot&&this.shadowRoot.getElementById(t);e&&e._wd&&e._wd._opts&&this._drawCurves(t,e._wd._opts)}}_pref(t,e){const s=this._panelCfg&&this._panelCfg.prefs||{};return s[t]===void 0?e:s[t]}_setPref(t,e){this._panelCfg||(this._panelCfg={}),this._panelCfg.prefs={...this._panelCfg.prefs||{},[t]:e},this._ws({type:`${y}/set_user_prefs`,prefs:{[t]:e}}).catch(()=>{})}_drawStatusCurve(){const t=this._powerData||{},e=t.live||[];if(e.length<2)return;const s=this._statusEnv,n=this._pref("show_expected",!0),r=this._pref("show_raw_active",!1),a=[];let o=e[e.length-1][0];if(t.cycle_active&&s&&(s.avg||[]).length&&n){const l=s.target_duration||s.avg[s.avg.length-1][0];a.push({points:s.avg,stroke:"#ff9800",width:2,alpha:.4,name:this._t("lbl.expected",{},"Expected")}),o=Math.max(o,l)}a.push({points:e,stroke:"primary",fill:!0,width:2,name:this._t("lbl.power",{},"Power")}),r&&(t.raw||[]).length>1&&a.push({points:t.raw,stroke:"#9e9e9e",width:1,alpha:.65,name:this._t("lbl.raw_socket",{},"Raw socket"),noScale:!0});const i=[];(t.restart_gaps||[]).forEach(l=>{if(!e.length)return;const c=t.cycle_start_iso;if(!c)return;const _=new Date(c).getTime(),p=Math.max(0,(new Date(l.start_ts).getTime()-_)/1e3),g=Math.max(p+1,(new Date(l.end_ts).getTime()-_)/1e3);i.push({x0:p,x1:g,fill:"rgba(96,125,139,.20)"})}),this._drawCurves("wd-status-canvas",{series:a,xMax:o,bands:i})}_attachHover(t){const e=this.shadowRoot,s=e&&e.getElementById(t);s&&(s.addEventListener("pointermove",n=>this._onGraphHover(n,t)),s.addEventListener("pointerleave",()=>this._hideGraphTip()),s.addEventListener("wheel",n=>{const r=s._wd;if(!r)return;n.preventDefault();const a=s.getBoundingClientRect(),o=r.cssToX(n.clientX-a.left),i=this._canvasZoom[t],l=r.xMax,c=i?i.xMin:0,p=(i?i.xMax:l)-c,g=p*(n.deltaY>0?1.3:.75);if(g>=l*.99)delete this._canvasZoom[t];else{const u=(o-c)/p,b=Math.min(l,o-u*g+g),k=Math.max(0,b-g);this._canvasZoom[t]={xMin:k,xMax:Math.min(l,k+g)}}this._redrawCanvas(t)},{passive:!1}),s.addEventListener("dblclick",()=>{delete this._canvasZoom[t],this._redrawCanvas(t)}))}_onGraphHover(t,e){const s=t.clientX,n=t.clientY;if(this._hoverRafId){this._hoverPending={px:s,py:n,id:e};return}this._hoverPending={px:s,py:n,id:e},this._hoverRafId=requestAnimationFrame(()=>{this._hoverRafId=null;const{px:r,py:a,id:o}=this._hoverPending||{};o&&this._onGraphHoverInner(r,a,o)})}_onGraphHoverInner(t,e,s){const n=this.shadowRoot&&this.shadowRoot.getElementById(s),r=n&&n._wd;if(!r)return;const a=n.getBoundingClientRect(),o=r.cssToX(t-a.left),i=(e-a.top)*r.dpr;this._redrawCanvas(s);const l=n.getContext("2d"),c=r.Xpx(o);l.save(),l.strokeStyle="rgba(140,140,140,.75)",l.lineWidth=r.dpr,l.setLineDash([3*r.dpr,3*r.dpr]),l.beginPath(),l.moveTo(c,r.padT),l.lineTo(c,r.ch-r.padB),l.stroke(),l.setLineDash([]);const _=b=>b.stroke==="primary"?r.primary:b.stroke,p=(b,k)=>{l.fillStyle=k,l.beginPath(),l.arc(c,r.Ypx(b),3.4*r.dpr,0,6.2832),l.fill()},g=[`${this._t("lbl.from_start",{},"From start")}: <b>${Mt(o)}</b>`,`${this._t("lbl.to_end",{},"To end")}: <b>${Mt(Math.max(0,r.xMax-o))}</b>`],u=r.series||[];if(this._hoverNearest=null,u.length>4){let b=null,k=1/0;if(u.forEach($=>{const x=Dt($.points,o);if(x==null)return;const S=Math.abs(r.Ypx(x)-i);S<k&&(k=S,b={s:$,v:x})}),b){const $=_(b.s);l.strokeStyle=$,l.lineWidth=3*r.dpr,l.beginPath(),(b.s.points||[]).forEach((x,S)=>S?l.lineTo(r.Xpx(x[0]),r.Ypx(x[1])):l.moveTo(r.Xpx(x[0]),r.Ypx(x[1]))),l.stroke(),p(b.v,$),g.push(`${d(b.s.name||"")}: <b>${b.v.toFixed(b.v<100?1:0)} W</b>`),b.s.cid&&(g.push(`<span style="opacity:.7">${this._t("lbl.click_to_select",{},"click to select")}</span>`),this._hoverNearest={id:s,cid:b.s.cid})}}else u.forEach(b=>{const k=Dt(b.points,o);k!=null&&(p(k,_(b)),g.push(`${d(b.name||this._t("lbl.power",{},"Power"))}: <b>${k.toFixed(k<100?1:0)} W</b>`))});if(r.band){const b=Dt(r.band.min,o),k=Dt(r.band.max,o);b!=null&&k!=null&&g.push(`${this._t("lbl.envelope",{},"Envelope")}: ${b.toFixed(0)}\u2013${k.toFixed(0)} W`)}(r.artifacts||[]).forEach(b=>{if(o>=b.start_s&&o<=b.end_s){const k=b.detail_key?this._t(b.detail_key,b.detail_params||{},b.detail||""):b.detail||"";g.push(`<span style="color:var(--warning-color,#ff9800)">\u26A0 ${d(te(b.type,($,x,S)=>this._t($,x,S)))}</span>: ${d(k)}`)}}),this._canvasZoom[s]&&g.push(`<span style="opacity:.45">${this._t("lbl.zoom_hint",{},"scroll to zoom \xB7 dblclick to reset")}</span>`),l.restore(),this._showGraphTip(t,e,g),this._syncSpagRowHighlight(this._hoverNearest?this._hoverNearest.cid:null)}_showGraphTip(t,e,s){const n=this._gtip;if(!n)return;n.innerHTML=s.join("<br>"),n.style.display="block";const r=n.offsetWidth,a=n.offsetHeight,o=16;let i=t+o,l=e+o;i+r>window.innerWidth-6&&(i=t-r-o),l+a>window.innerHeight-6&&(l=e-a-o),n.style.left=Math.max(6,i)+"px",n.style.top=Math.max(6,l)+"px"}_hideGraphTip(){this._hoverRafId&&(cancelAnimationFrame(this._hoverRafId),this._hoverRafId=null,this._hoverPending=null),this._gtip&&(this._gtip.style.display="none"),this._syncSpagRowHighlight(null)}_positionTip(t){const e=t.querySelector(".wd-tip-pop");if(!e)return;e.style.transform="";const s=!e.offsetWidth,n=s?e.style.cssText:"";s&&(e.style.visibility="hidden",e.style.display="block");const r=e.getBoundingClientRect();if(s&&(e.style.cssText=n),!r.width)return;const a=fe(t),o=6,i=a.left+o,l=a.right-o-r.width,c=l<i?i:Math.min(Math.max(r.left,i),l),_=Math.round(c-r.left);_&&(e.style.transform=`translateX(calc(-50% + ${_}px))`)}_syncSpagRowHighlight(t){if(t===this._spagHoverCid)return;this._spagHoverCid=t;const e=this.shadowRoot;e&&e.querySelectorAll("tr[data-cid]").forEach(s=>{s.style.backgroundColor=t&&s.dataset.cid===t?"var(--secondary-background-color,rgba(0,0,0,.06))":""})}_showToast(t,e="success",s={}){this._toastTimer&&clearTimeout(this._toastTimer);const n=s.duration||3500;this._toast={msg:t,cls:`wd-toast-${e}`,actionLabel:s.actionLabel||null,actionToken:s.actionToken||null},this._render(),this._toastTimer=setTimeout(()=>{this._toast=null,this._render()},n)}_profileOptions(t){return(this._profiles||[]).map(e=>`<option value="${d(e.name)}" ${String(t)===String(e.name)?"selected":""}>${d(e.name)}</option>`).join("")}_htmlModal(){const t=this._modal;if(t.type==="cycle-detail")return`<div class="wd-overlay"><div class="wd-modal wd-modal-lg" role="dialog" aria-modal="true" aria-labelledby="wd-modal-title" tabindex="-1">${this._htmlCycleModal(t)}</div></div>`;if(t.type==="profile-panel")return`<div class="wd-overlay"><div class="wd-modal wd-modal-lg" role="dialog" aria-modal="true" aria-labelledby="wd-modal-title" tabindex="-1">${this._htmlProfilePanel(t)}</div></div>`;if(t.type==="profile-group")return`<div class="wd-overlay"><div class="wd-modal wd-modal-lg" role="dialog" aria-modal="true" aria-labelledby="wd-modal-title" tabindex="-1">${this._htmlProfileGroupModal(t)}</div></div>`;if(t.type==="compare-cycles")return`<div class="wd-overlay"><div class="wd-modal wd-modal-lg" role="dialog" aria-modal="true" aria-labelledby="wd-modal-title" tabindex="-1">${this._htmlCompareModal(t)}</div></div>`;if(t.type==="gear-settings")return`<div class="wd-overlay"><div class="wd-modal wd-modal-lg" role="dialog" aria-modal="true" aria-labelledby="wd-modal-title" tabindex="-1">${this._htmlGearModal(t)}</div></div>`;if(t.type==="export-select")return`<div class="wd-overlay"><div class="wd-modal wd-modal-lg" role="dialog" aria-modal="true" aria-labelledby="wd-modal-title" tabindex="-1">${this._htmlExportSelectModal(t)}</div></div>`;if(t.type==="import-wizard")return`<div class="wd-overlay"><div class="wd-modal wd-modal-lg" role="dialog" aria-modal="true" aria-labelledby="wd-modal-title" tabindex="-1">${this._htmlImportWizardModal(t)}</div></div>`;if(t.type==="history-import")return`<div class="wd-overlay"><div class="wd-modal wd-modal-lg" role="dialog" aria-modal="true" aria-labelledby="wd-modal-title" tabindex="-1">${this._htmlHistoryImportModal(t)}</div></div>`;let e="";if(t.type==="confirm")e=`<h2>${d(t.title)}</h2><p class="wd-info">${d(t.message)}</p>
        ...
        <button class="wd-btn wd-btn-primary" data-maction="store-share-ok" ${this._busy.has("store-share")?"disabled":""}>${this._busy.has("store-share")?'<span class="wd-spin"></span> ':""}${this._t("btn.share",{},"Share")}</button></div>`}else t.type==="store-share-device"&&(e=this._htmlShareDeviceModal(t));return`<div class="wd-overlay"><div class="wd-modal" role="dialog" aria-modal="true" aria-labelledby="wd-modal-title" tabindex="-1">${e}</div></div>`}_htmlShareDeviceModal(t){const e=this._shareableByProgram(),s=this._busy.has("store-share-device"),n=t.selected||new Set,r=!!t.consented,a=e.reduce((u,b)=>u+b.cycles.filter(k=>n.has(k.id)).length,0),o=e.some(u=>!u.noCycles);let i;t.loading?i=`<div class="wd-empty" style="padding:24px"><div class="wd-icon">\u23F3</div>${this._t("msg.loading",{},"Loading\u2026")}</div>`:e.length?i=e.map(u=>{if(u.noCycles)return`<div class="wd-sd-group wd-sd-group-nocyc">
          ...
          </div>`;const b=u.cycles.every(E=>n.has(E.id)),k=!b&&u.cycles.some(E=>n.has(E.id)),$=u.cycles.map(E=>{const R=E.start_time?st(E.start_time):"",B=E.duration!=null?ot(E.duration):"";return`<label class="wd-sd-cyc">
            <input type="checkbox" data-maction="sd-toggle-cyc" data-cid="${d(E.id)}" ${n.has(E.id)?"checked":""} ${s?"disabled":""}>
          ...
          <label class="wd-sd-prof">
            <input type="checkbox" data-maction="sd-toggle-prof" data-prog="${d(u.program)}" ${b?"checked":""} ${k?'data-indeterminate="1"':""} ${s?"disabled":""}>
            <span class="wd-sd-prof-name">${d(u.program)}</span>
            <span class="wd-sd-count">${u.cycles.filter(E=>n.has(E.id)).length}/${u.cycles.length}</span>
          </label>
    ...
    </label>`,c=a.map(_=>{const p=n[_],g=s.importableOnly&&p.importable===!1,u=this._wizCatState(t,_,e),k=["profiles","real_cycles","reference_cycles"].includes(_)?`${u.sel}/${p.count}`:"",$=`<label class="wd-sd-prof">
      ...
      </label>`;let x="";if(!g&&_==="profiles")x=`<div class="wd-sd-cycles">${(p.items||[]).map(S=>`<label class="wd-sd-cyc">
          <input type="checkbox" data-maction="wiz-toggle-profile" data-name="${d(S.name)}" ${t.sel.profiles.has(S.name)?"checked":""}>
          <span class="wd-sd-cyc-meta">${d(S.name)} \xB7 ${(S.real_cycles||0)+(S.reference_cycles||0)} ${this._t("lbl.cycles_short",{},"cycles")}${r.has(S.name)?` <span class="wd-tag" style="color:var(--warning-color,#e6a700)">${this._t("badge.exists",{},"exists")}</span>`:""}</span>
        </label>`).join("")}</div>`;else if(!g&&(_==="real_cycles"||_==="reference_cycles")){const S=_==="real_cycles"?t.sel.realIds:t.sel.refIds;x=`<div class="wd-sd-cycles">${(p.groups||[]).map(E=>{const R=E.cycles.map(M=>String(M.id)),B=R.filter(M=>S.has(M)).length,W=B===R.length&&R.length>0,O=B>0&&!W,D=`${_}:${E.profile}`,A=t.expanded&&t.expanded.has(D),C=A?E.cycles.map(M=>`<label class="wd-sd-cyc" style="margin-left:18px">
            ...
            <div class="wd-sd-cyc" style="display:flex;align-items:center">
              <label style="flex:1;display:flex;align-items:center;gap:8px;cursor:pointer">
                <input type="checkbox" data-maction="wiz-toggle-cycgroup" data-cat="${_}" data-prof="${d(E.profile)}" ${W?"checked":""} ${O?'data-indeterminate="1"':""}>
                <span class="wd-sd-cyc-meta"><strong>${d(E.profile||this._t("lbl.unlabelled",{},"Unlabelled"))}</strong> (${B}/${E.count})</span>
              </label>
    ...
    </div>`,_=n.real_history_allowed!==!1,p=`<div class="wd-field"><label>${this._t("lbl.cycle_destination",{},"Where should imported cycles go?")}</label>
      <div class="wd-mode-bar" style="display:flex;gap:8px">
    ...
    </div>`;let g="";const b=(n.categories&&n.categories.profiles&&n.categories.profiles.items||[]).filter($=>$.conflict&&t.sel.profiles.has($.name));t.mode==="merge"&&b.length&&(g=`<div class="wd-field"><label>${this._t("lbl.conflict_resolution",{},"Name clashes")}</label>
        ${b.map($=>`<div style="display:flex;align-items:center;gap:8px;margin:4px 0">
      ...
      <div class="wd-modal-actions">
        ...
        <div class="wd-field"><label>${this._t("lbl.load_from_file",{},"Load from file")}</label><input type="file" id="wd-hist-file" accept=".csv,text/csv,text/plain" ${x}></div>
        <div class="wd-field"><label>${this._t("lbl.hist_csv_data",{},"CSV data")}</label><textarea id="wd-hist-csv" style="min-height:96px;font-family:monospace;font-size:.78em" placeholder="entity_id,state,last_changed" ${x}>${d(t.csvText||"")}</textarea></div>
        <div class="wd-field">
          ...
          <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
            ...
            <input type="date" id="wd-hist-since" value="${d(t.since||Ft())}" min="${d(he())}" max="${d(ce())}" ${x}>
            <button class="wd-btn wd-btn-secondary" data-maction="hist-recorder" ${x}>${this._t("btn.hist_read_recorder",{},"Read from Home Assistant")}</button>
          </div>
        ...
        ${s}
        <div class="wd-modal-actions">
          <button class="wd-btn wd-btn-secondary" data-maction="cancel" ${x}>${this._t("btn.cancel",{},"Cancel")}</button>
          <button class="wd-btn wd-btn-primary" data-maction="hist-scan" ${x}>${$?'<span class="wd-spin"></span> ':""}${this._t("btn.hist_scan",{},"Scan for cycles")}</button>
        </div>`}if(t.step==="scan"){const $=t.scanTaskId?(this._tasks||{})[t.scanTaskId]:null,x=$&&$.total>0?Math.round($.done/$.total*100):null;return`${e}
        ...
        <div class="wd-prog-bg" style="margin-bottom:8px"><div class="wd-prog-fill" style="width:${x??0}%"></div></div>
        <p class="wd-info">${x==null?this._t("status.preparing",{},"Preparing\u2026"):`${x}%`}</p>
        ${s}
        <div class="wd-modal-actions">
          <button class="wd-btn wd-btn-secondary" data-maction="hist-cancel-scan">${this._t("btn.cancel",{},"Cancel")}</button>
        </div>`}if(t.step==="done"){const $=t.done||{},x=[this._t("msg.hist_imported_count",{n:$.imported||0},`${$.imported||0} cycles imported.`),$.duplicates?this._t("msg.hist_duplicates",{n:$.duplicates},`${$.duplicates} were already imported and were skipped.`):"",$.capped?this._t("msg.hist_capped",{},"The per-device limit for imported cycles was reached; the rest were not stored."):""].filter(Boolean);return`${e}
        ${x.map(S=>`<p class="wd-info">${d(S)}</p>`).join("")}
        ...
        </div>`;const b=r.map($=>{const x=i.has($.index),S=this._histSegReason($.reason);return`<tr data-hist-row="${$.index}" style="${x?"":"opacity:.55"}">
        <td><input type="checkbox" data-hist-pick="${$.index}" ${x?"checked":""} aria-label="${d(this._t("lbl.hist_keep",{},"Keep this cycle"))}"></td>
      ...
      </tr>`}).join(""),k=r.every($=>i.has($.index));return`${e}
      ...
      <div class="wd-modal-actions">
        ...
        <div class="wd-modal-actions"><button class="wd-btn wd-btn-secondary" data-maction="cancel">${this._t("btn.close",{},"Close")}</button></div>`;const e=t.curve||{},s=!!e.is_reference,n=e.labelable!==!1,r=e.editable!==!1,a=e.cycle_origin==="backfill",o=e.full_duration_s||e.duration||0,i=e.energy_kwh!=null?e.energy_kwh:null,l=t.ml||null;let c="";if(l&&l.ml_quality_score!=null){const C=l.ml_quality_label,M=C==="ok"?"var(--success-color,#4caf50)":C==="uncertain"?"var(--warning-color,#ff9800)":"var(--error-color,#f44336)",H=Math.round((1-l.ml_quality_score)*100);c=`<div class="wd-kv-item"><div class="wd-kv-val" style="font-size:.95em;color:${M}">${H}%</div><div class="wd-kv-lbl">${this._t("lbl.cycle_health",{},"Cycle health")}</div></div>`}const _=`<div class="wd-kv">
      ...
      ${c}
    </div>`,p=l&&l.ml_review||{},g=(this._feedbacks||[]).some(C=>C.cycle_id===t.cycleId),u=l&&l.ml_quality_label,b=!p.reviewed_at&&(g||["uncertain","review"].includes(u)||["force_stopped","interrupted"].includes(e.status)),k=b&&t.mode!=="review"?` <span title="${this._t("hdr.automation_needs_review",{},"This cycle needs review")}" style="color:var(--warning-color,#ff9800);font-size:1.1em;line-height:0">\u25CF</span>`:"",$=this._canEdit()&&r?`<div class="wd-mode-bar">
      ...
      <button class="wd-btn wd-btn-sm ${t.mode==="review"?"wd-btn-primary":"wd-btn-secondary"}" data-maction="cyc-review" title="${b?this._t("hdr.automation_needs_review",{},"This cycle needs review"):this._t("hdr.automation_review_this_cycle",{},"Review this cycle")}">${this._t("btn.review",{},"Review")}${k}</button>
    </div>`:s?`<div class="wd-info" style="margin:0 0 8px"><span style="color:var(--info-color,#2196f3)">\u{1F4E5}</span> ${a?this._t("msg.imported_history_readonly",{},"Detected in imported power history. It shapes program matching but is not counted in your statistics, and cannot be trimmed or split. Label it to name the program."):this._t("msg.imported_readonly",{},"Imported from the community store. Shown for reference and matching. It is not counted in your stats and cannot be edited.")}</div>`:"",x=this._canEdit()?(this._feedbacks||[]).find(C=>C.cycle_id===t.cycleId):null,S=x?x.detected_profile||x.profile_name||this._t("lbl.unknown",{},"Unknown"):"",E=x?`
        ...
        </div>`}else if(t.mode==="split"){const C=this._busy.has("cyc-split-apply"),M=(t.split.offsets||[]).slice().sort((Q,J)=>Q-J),H=[0,...M,o],q=H.slice(0,-1).map((Q,J)=>{const G=H[J+1];return`<div class="wd-seg-row"><span class="wd-swatch" style="background:${et[J%et.length]}"></span>
        ...
        <div style="display:flex;gap:16px;flex-wrap:wrap;align-items:center;margin:6px 0">
          ...
          <label style="display:inline-flex;align-items:center;gap:6px"><input type="checkbox" id="wd-cyc-rev-golden" ${C.golden?"checked":""}> ${this._t("badge.golden_cycle",{},"Recorded reference cycle")}${ct}</label>
          ${J}
        </div>
        ...
        <div class="wd-rev-tags">${(this._profiles||[]).map(f=>{const m=(t.overlays||[]).includes(f.name),v=m?`<span style="display:inline-block;width:10px;height:10px;border-radius:2px;background:${et[Math.max(0,this._profiles.findIndex(L=>L.name===f.name))%et.length]};margin:0 2px"></span>`:"";return`<label class="wd-rev-tag"><input type="checkbox" class="wd-cyc-overlay" value="${d(f.name)}" ${m?"checked":""}> ${v}${d(f.name)}</label>`}).join("")||`<span class="wd-info">${this._t("msg.no_profiles_compare",{},"No profiles to compare.")}</span>`}</div>
        ...
        </div>`}let B="";const W=t.mode==="view"||t.mode==="review"?e.artifacts||[]:[];if(W.length){const C=W.map(M=>{const H=M.detail_key?this._t(M.detail_key,M.detail_params||{},M.detail||""):M.detail||"";return`<li><b>${d(te(M.type,(q,Q,J)=>this._t(q,Q,J)))}</b> ${this._t("lbl.at",{},"at")} ${Mt(M.start_s)}\u2013${Mt(M.end_s)} \u2014 ${d(H)}</li>`}).join("");B=`<div class="wd-card" style="margin:10px 0 0;padding:10px 12px;border-left:3px solid var(--warning-color,#ff9800)">
      ...
      </div>`}let O="";const D=t.mode==="view"||t.mode==="review"?e.restart_gaps||[]:[];if(D.length){const C=D.map(M=>{const H=Math.round((M.gap_seconds||0)/60),q=H>=1?`${H}m`:`${Math.round(M.gap_seconds||0)}s`,Q=M.match_confidence!=null?` \xB7 ${this._t("lbl.pct_match_confidence",{pct:Math.round(M.match_confidence*100)},`${Math.round(M.match_confidence*100)}% match confidence`)}`:"",J=M.profile?` (${d(M.profile)})`:"";return`<li>${this._t("msg.restart_gap_item",{dur:q},`${q} gap`)}: ${this._t("lbl.ha_restarted",{},"HA restarted")}${J}${Q}</li>`}).join("");O=`<div class="wd-card" style="margin:10px 0 0;padding:10px 12px;border-left:3px solid var(--info-color,#2196f3)">
      ...
      ${R}`}_htmlProfilePanel(t){const e=this._canEdit();t.tab==="danger"&&!e&&(t.tab="stats");const s=[["stats",this._t("tab.pp_overview",{},"Overview")],["phases",this._t("tab.pp_phases",{},"Phases")],["cleanup",this._t("tab.pp_cleanup",{},"Cleanup")]];e&&s.push(["danger",this._t("tab.pp_manage",{},"Manage")]);const n=s.map(([o,i])=>`<button class="wd-mini-tab ${t.tab===o?"active":""}" data-maction="pp-tab-${o}">${i}</button>`).join("");let r="";if(!t.loaded)r=`<div class="wd-empty" style="padding:32px"><div class="wd-icon">\u23F3</div>${this._t("msg.loading",{},"Loading\u2026")}</div>`;else if(t.tab==="stats"){const o=t.stats||{},i=t.env||{},l=this._hass&&this._hass.config&&this._hass.config.currency||"",c=o.avg_energy!=null&&o.cycle_count?o.avg_energy*o.cycle_count:null,_=k=>k?Math.round(k/60)+"m":"-",p=(this._profileHealth||{})[t.name],g=(this._profileTrends||{})[t.name],u=p&&p.health_status!=="unknown"?(()=>{const k={healthy:["var(--success-color,#4caf50)","rgba(76,175,80,.12)"],fair:["var(--warning-color,#ff9800)","rgba(255,152,0,.12)"],poor:["var(--error-color,#f44336)","rgba(244,67,54,.12)"]},[$,x]=k[p.health_status]||k.fair,S=Math.round((p.health_score||0)*100),E=p.duration_cv!=null?` \xB7 ${this._t("stat.duration_cv",{pct:Math.round(p.duration_cv*100)},`duration CV ${Math.round(p.duration_cv*100)}%`)}`:"",R=p.confidence_mean!=null?` \xB7 ${this._t("stat.avg_confidence",{pct:Math.round(p.confidence_mean*100)},`avg confidence ${Math.round(p.confidence_mean*100)}%`)}`:"";return`<div style="margin:8px 0 4px;padding:8px 12px;border-radius:6px;background:${x};border:1px solid color-mix(in srgb, ${$} 13%, transparent);display:flex;align-items:center;gap:8px;flex-wrap:wrap">
          ...
          <span style="font-size:.85em;opacity:.8">${this._t("stat.score",{pct:S},`score ${S}%`)}${E}${R}</span>
        ...
        </div>`})():"",b=g&&(g.duration_trend!=="stable"||g.energy_trend&&g.energy_trend!=="stable")?(()=>{const k=[];g.duration_trend==="up"?k.push(this._t("msg.trend_duration_longer",{pct:`${g.duration_slope_pct>0?"+":""}${g.duration_slope_pct}`,avg:`${Math.round(g.duration_recent_mean_s/60)}m`},`Duration trending longer (${g.duration_slope_pct>0?"+":""}${g.duration_slope_pct}%/cycle) \u2014 recent avg ${Math.round(g.duration_recent_mean_s/60)}m`)):g.duration_trend==="down"&&k.push(this._t("msg.trend_duration_shorter",{pct:`${g.duration_slope_pct}`,avg:`${Math.round(g.duration_recent_mean_s/60)}m`},`Duration trending shorter (${g.duration_slope_pct}%/cycle) \u2014 recent avg ${Math.round(g.duration_recent_mean_s/60)}m`)),g.energy_trend==="up"?k.push(this._t("msg.trend_energy_up",{pct:`${g.energy_slope_pct>0?"+":""}${g.energy_slope_pct}`,avg:gt(g.energy_recent_mean_wh)},`Energy trending up (${g.energy_slope_pct>0?"+":""}${g.energy_slope_pct}%/cycle) \u2014 recent avg ${gt(g.energy_recent_mean_wh)}`)):g.energy_trend==="down"&&k.push(this._t("msg.trend_energy_down",{pct:`${g.energy_slope_pct}`},`Energy trending down (${g.energy_slope_pct}%/cycle)`));const $=g.duration_trend==="up"||g.energy_trend==="up";return`<div style="margin:6px 0;padding:8px 12px;border-radius:6px;background:${$?"rgba(255,152,0,.10)":"rgba(33,150,243,.10)"};font-size:.88em">
        ...
        ${(()=>{const k=(this._profileAdvisories||[]).find($=>$&&$.profile===t.name&&$.code==="phase_inconsistent");return k?`<div style="margin-top:8px;padding:8px 10px;background:color-mix(in srgb, var(--warning-color,#ff9800) 9%, transparent);border-radius:6px;border-left:3px solid var(--warning-color,#ff9800)">
        ...
        ${i.avg&&i.avg.length?`<div class="wd-canvas-wrap"><canvas id="wd-env-canvas" role="img" aria-label="${d(this._t("lbl.aria_envelope_chart",{},"Profile power envelope chart"))}"></canvas></div>`:`<p class="wd-info">${this._t("msg.no_envelope",{},"No envelope yet - rebuild after labelling cycles.")}</p>`}`}else if(t.tab==="phases"){const o=t.catalog||[],i=(t.phases||[]).map((c,_)=>{const p=o.map(g=>`<option value="${d(g)}" ${c.name===g?"selected":""}>${d(g)}</option>`).join("");return`<div class="wd-phase-row"><span class="wd-swatch" style="background:${et[_%et.length]}"></span>
          ...
          <button class="wd-btn wd-btn-danger wd-btn-sm" data-maction="pp-phase-rm" data-idx="${_}">\u2715</button></div>`}).join(""),l=this._busy.has("pp-phase-save");r=`<p class="wd-info" style="margin-bottom:10px">${this._t("msg.phase_ranges_intro",{},"Phase ranges (minutes from cycle start) overlaid on the average curve. Edit values to preview live.")}</p>
        ...
        </div>`:""}`}else if(t.tab==="cleanup"){const o=t.cleanup&&t.cleanup.cycles||[],i=t.cleanup&&t.cleanup.selected||new Set,{col:l,dir:c}=this._cleanupSort,_={date:x=>x.start_time?new Date(x.start_time).getTime():0,duration:x=>x.duration,energy:x=>x.energy_kwh,status:x=>x.status||""},g=Zt(o,_[l]||_.date,c).map((x,S)=>{const E=o.indexOf(x),R=e?`<td style="padding:4px 6px 4px 2px;white-space:nowrap">
          <button class="wd-btn wd-btn-secondary wd-btn-sm" data-action="cleanup-edit-cycle" data-cid="${d(x.cycle_id)}">${this._t("btn.trim_split",{},"Trim / Split")}</button>
        </td>`:"";return`<tr data-cid="${d(x.cycle_id)}" style="cursor:pointer">
          <td style="width:26px;padding:6px 4px"><input type="checkbox" data-cleanidx="${E}" ${i.has(x.cycle_id)?"checked":""}></td>
          <td style="width:10px;padding:6px 2px"><span class="wd-swatch" style="background:${et[E%et.length]}"></span></td>
          <td class="wd-tc-date">${st(x.start_time)}</td>
          <td class="wd-tc-num">${ot(x.duration)}</td>
          <td class="wd-tc-num">${x.energy_kwh!=null?gt(x.energy_kwh):"-"}</td>
          <td><span class="wd-pill">${d(x.status||"completed")}</span></td>
          ${R}
        </tr>`}).join(""),u=`<thead><tr>
        ...
        ${e?"<th></th>":""}
      </tr></thead>`,b=this._busy.has("pp-cleanup-del"),k=this._busy.has("pp-cleanup-unlabel"),$=b||k;r=`<p class="wd-info" style="margin-bottom:10px">${this._t("msg.cleanup_intro",{},"Every labelled cycle overlaid. Tick outliers, then unlabel them (kept in history, but no longer shaping this profile) or delete them outright.")}</p>
        ...
        ${e?`<div class="wd-modal-actions">
          <button class="wd-btn wd-btn-secondary" data-maction="pp-cleanup-unlabel" ${$||i.size===0?"disabled":""} title="${d(this._t("btn.unlabel_selected_tip",{},"Remove the profile label from the selected cycles. They stay in your history and can be labelled again later - they just stop shaping this profile."))}">${k?'<span class="wd-spin"></span> '+this._t("status.unlabelling",{},"Unlabelling\u2026"):this._t("btn.unlabel_selected",{n:i.size},`Unlabel selected (${i.size})`)}</button>
          <button class="wd-btn wd-btn-danger" data-maction="pp-cleanup-del" ${$||i.size===0?"disabled":""}>${b?'<span class="wd-spin"></span> '+this._t("status.deleting",{},"Deleting\u2026"):this._t("btn.delete_selected",{n:i.size},`Delete selected (${i.size})`)}</button>
      ...
      <div class="wd-modal-actions" style="margin-top:14px">
        <button class="wd-btn wd-btn-secondary" data-maction="cancel">${this._t("btn.close",{},"Close")}</button>
        ${a}
      </div>`}_drawCycleEditor(){const t=this._modal;if(!t||t.type!=="cycle-detail"||!t.loaded)return;const e=t.curve||{},s=e.samples||[];if(!s.length)return;let n=e.full_duration_s||s[s.length-1][0]||1;const r=[],a=t.profileEnv;if((t.mode==="view"||t.mode==="review")&&a&&(a.avg||[]).length&&(r.push({points:a.avg,stroke:"#ff9800",width:2,alpha:.45,name:`${this._t("lbl.expected",{},"Expected")} (${e.profile_name||"profile"})`}),n=Math.max(n,a.target_duration||a.avg[a.avg.length-1][0]||0)),t.mode==="review"&&(t.overlays||[]).length){const c=this._profileEnvCache||{};(t.overlays||[]).forEach(_=>{const p=c[_];if(!p||!(p.avg||[]).length)return;const g=et[Math.max(0,(this._profiles||[]).findIndex(b=>b.name===_))%et.length];r.push({points:p.avg,stroke:g,width:1.6,alpha:.7,name:_});const u=p.avg[p.avg.length-1];n=Math.max(n,p.target_duration||(u?u[0]:0))})}r.push({points:s,stroke:"primary",fill:!0,width:2,name:this._t("lbl.power",{},"Power")});const o=[],i=[];let l=[];if(t.mode==="trim"){const c=t.trim.start,_=t.trim.end;o.push({x0:0,x1:c,fill:"rgba(244,67,54,.18)"}),o.push({x0:_,x1:n,fill:"rgba(244,67,54,.18)"}),i.push({x:c,color:"#f44336",label:"S"},{x:_,color:"#f44336",label:"E"})}else if(t.mode==="split")(t.split.offsets||[]).slice().sort((c,_)=>c-_).forEach((c,_)=>i.push({x:c,color:"#ff9800",label:"#"+(_+1)}));else{l=e.artifacts||[];const c={pause:"rgba(255,152,0,.22)",dip:"rgba(33,150,243,.18)",spike:"rgba(244,67,54,.18)"};l.forEach(p=>o.push({x0:p.start_s,x1:Math.max(p.end_s,p.start_s+1),fill:c[p.type]||"rgba(158,158,158,.18)"}));const _=e.start_time;(e.restart_gaps||[]).forEach(p=>{if(!_)return;const g=new Date(_).getTime(),u=Math.max(0,(new Date(p.start_ts).getTime()-g)/1e3),b=Math.max(u+1,(new Date(p.end_ts).getTime()-g)/1e3);o.push({x0:u,x1:b,fill:"rgba(96,125,139,.20)",label:"\u21BB"})})}this._drawCurves("wd-cyc-canvas",{series:r,xMax:n,bands:o,vlines:i,artifacts:l})}_htmlCompareModal(t){const e=t.ids||[],s={};(this._cycles||[]).forEach(o=>{s[o.id]=o});const n=t.hidden||new Set,r=e.map((o,i)=>{const l=s[o]||{},c=et[i%et.length],_=!n.has(o),p=!!(t.cycles&&t.cycles[o]),g=`${st(l.start_time)||String(o).slice(0,8)} \xB7 ${Math.round((l.duration||0)/60)}m \xB7 ${d(l.profile_name||this._t("lbl.unlabelled",{},"Unlabelled"))}`;return`<label class="wd-rev-tag"><input type="checkbox" class="wd-compare-cyc" value="${d(o)}" ${_?"checked":""} ${p?"":"disabled"}><span style="display:inline-block;width:10px;height:10px;border-radius:2px;background:${c};margin:0 2px"></span>${g}${p?"":` <span class="wd-info">${this._t("lbl.loading_paren",{},"(loading\u2026)")}</span>`}</label>`}).join(""),a=(this._profiles||[]).map(o=>{const i=(t.overlays||[]).includes(o.name),l=et[Math.max(0,(this._profiles||[]).findIndex(_=>_.name===o.name))%et.length],c=i?`<span style="display:inline-block;width:10px;height:10px;border-radius:2px;background:${l};margin:0 2px;opacity:.55"></span>`:"";return`<label class="wd-rev-tag"><input type="checkbox" class="wd-compare-overlay" value="${d(o.name)}" ${i?"checked":""}> ${c}${d(o.name)}</label>`}).join("")||`<span class="wd-info">${this._t("msg.no_profiles_overlay",{},"No profiles to overlay.")}</span>`;return`<h2>${this._t("msg.compare_cycles_title",{count:e.length},`Compare ${e.length} cycles`)}</h2>
