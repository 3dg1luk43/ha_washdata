...
`;function _t(h){if(h==null||h<0)return"-";const t=Math.floor(h/3600),e=Math.floor(h%3600/60),s=Math.floor(h%60);return t>0?`${t}h ${e}m`:e>0?`${e}m ${s}s`:`${s}s`}function Zt(h){return h==null?"-":h>=100?`${Math.round(h)} W`:`${h.toFixed(1)} W`}function yt(h){return h==null?"-":`${h.toFixed(2)} kWh`}let Qt="relative";const be=3700;function te(h){const t=e=>String(e).padStart(2,"0");return`${h.getFullYear()}-${t(h.getMonth()+1)}-${t(h.getDate())}`}function ee(h){const t=new Date;return t.setDate(t.getDate()-h),te(t)}function ve(){return te(new Date)}function jt(){return ee(10)}function xe(){return ee(be-1)}function $e(h){const t=Math.round((h-Date.now())/1e3);let e;try{e=new Intl.RelativeTimeFormat(void 0,{numeric:"auto"})}catch{return se(h)}const s=Math.abs(t),r=[["year",31536e3],["month",2592e3],["week",604800],["day",86400],["hour",3600],["minute",60]];for(const[n,a]of r)if(s>=a)return e.format(Math.round(t/a),n);return e.format(t,"second")}function se(h){return new Date(h).toLocaleString(void 0,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}function rt(h,t){if(!h)return"-";let e;if(typeof h=="number")e=h>=1e12?h:h*1e3;else{const s=String(h),r=/^(\d{4})-(\d{2})-(\d{2})$/.exec(s);e=r?new Date(+r[1],+r[2]-1,+r[3]).getTime():new Date(s).getTime()}return isNaN(e)?"-":(t||Qt)==="relative"?$e(e):se(e)}function Lt(h){if(h==null)return"unknown error";if(h instanceof Error)return`${h.name}: ${h.message}`;const t=h.code!=null?String(h.code):"",e=h.message!=null?String(h.message):"";if(!t&&!e)try{return JSON.stringify(h)}catch{return String(h)}const s=t==="unknown_command"?" (the integration is probably still starting up; this should stop on its own)":"";return`${t||"error"}${e?": "+e:""}${s}`}function d(h){return String(h??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function ke(h){const t=String(h??"").trim();return/^https?:\/\//i.test(t)?t:""}function Pt(h,t){const e=String(h??"").trim();let s=e.match(/^#([0-9a-fA-F]{3,8})$/);if(s){let r=s[1];if((r.length===3||r.length===4)&&(r=r.split("").map(n=>n+n).join("")),r.length===6||r.length===8){const n=parseInt(r.slice(0,2),16),a=parseInt(r.slice(2,4),16),o=parseInt(r.slice(4,6),16);return`rgba(${n}, ${a}, ${o}, ${t})`}}if(s=e.match(/^rgba?\(([^)]+)\)$/i),s){const r=s[1].split(",").map(n=>n.trim());if(r.length>=3)return`rgba(${r[0]}, ${r[1]}, ${r[2]}, ${t})`}return e}function ie(h,t){const e=parseFloat(h);return isNaN(e)?t:e}function qt(h){return h?Array.from(h.querySelectorAll('a[href],button:not([disabled]),input:not([disabled]),select:not([disabled]),textarea:not([disabled]),[tabindex]:not([tabindex="-1"])')).filter(e=>e.getClientRects().length>0):[]}function At(h){return h==null||h===""?"(none)":h===!0?"on":h===!1?"off":Array.isArray(h)?h.join(", ")||"(none)":String(h)}function ae(h,t,e){return h.slice().sort((s,r)=>{const n=t(s),a=t(r);return n==null&&a==null?0:n==null?1:a==null?-1:(n<a?-1:n>a?1:0)*e})}function bt(h,t,e,s,r,n,a){const o=e?s===1?" \u25B2":" \u25BC":' <span style="opacity:.35">\u2195</span>',i=n==="right"?"text-align:right;":"",l=a?` title="${d(a)}"`:"";return`<th class="wd-th-sort" data-sortcol="${t}" data-sortact="${r}" style="cursor:pointer;user-select:none;${i}"${l}>${h}${o}</th>`}function zt(h){h=Math.max(0,Math.round(h));const t=Math.floor(h/3600),e=Math.floor(h%3600/60),s=h%60;return t>0?`${t}:${String(e).padStart(2,"0")}:${String(s).padStart(2,"0")}`:`${e}:${String(s).padStart(2,"0")}`}function ne(h,t){const e={pause:["lbl.artifact_interruption","Interruption"],dip:["lbl.artifact_low_power","Low power"],spike:["lbl.artifact_high_power","High power"]},[s,r]=e[h]||["lbl.artifact_anomaly","Anomaly"];return t?t(s,{},r):r}function re(h){return h.toLowerCase().replace(/[\s&/\-]+/g,"_").replace(/^_+|_+$/g,"").replace(/_+/g,"_")}function oe(h){let t=String(h??"").trim().toLowerCase();try{t=t.normalize("NFKD").replace(/[\u0300-\u036f]/g,"")}catch{}return t.replace(/[^\p{L}\p{N}]+/gu,"-").replace(/^-+|-+$/g,"")}function le(h){const t=new Set;return String(h??"").split(",").forEach(e=>{const s=parseInt(e.trim(),10);Number.isFinite(s)&&s>0&&t.add(s)}),Array.from(t).sort((e,s)=>e-s)}function Ot(h,t){if(!h||!h.length)return null;if(t<=h[0][0])return h[0][1];if(t>=h[h.length-1][0])return h[h.length-1][1];for(let e=1;e<h.length;e++)if(h[e][0]>=t){const s=h[e-1],r=h[e],n=r[0]-s[0]||1;return s[1]+(r[1]-s[1])*((t-s[0])/n)}return h[h.length-1][1]}function Se(h,t,e){e=e||{};const s=h.key,r=h.unit?`${h.label} (${h.unit})`:h.label,n=h.unit?` ${h.unit}`:"",a=h.doc?lt(h.doc,h.diagram||Te[s]):"",o=e.changed?`<span class="wd-chg-dot" title="${d(e.changed)}" aria-label="${d(e.changed)}"></span>`:"";if(h.type==="checkbox")return`<div class="wd-field wd-field-switch"><div class="wd-switch-row"><label class="wd-switch-lbl"><span class="wd-switch"><input type="checkbox" data-opt="${s}" ${t?"checked":""}><span class="wd-switch-slider"></span></span><span class="wd-switch-text">${d(h.label)}</span></label>${o}${a}</div>${h.hint?`<div class="wd-field-hint">${d(h.hint)}</div>`:""}</div>`;let i="";const l=t??"";if(h.type==="select"||h.type==="devicetype"||h.type==="device"){const S=(e.opts||[]).map(([D,z])=>`<option value="${d(D)}" ${String(l)===String(D)?"selected":""}>${d(z)}</option>`).join("");i=`<select data-opt="${s}" data-ftype="${h.type}">${S}</select>`}else if(h.type==="textarea")i=`<textarea data-opt="${s}" data-ftype="textarea">${d(l)}</textarea>`;else if(h.type==="json"){const v=l===""||l==null?"":typeof l=="string"?l:JSON.stringify(l,null,2);i=`<textarea data-opt="${s}" data-ftype="json" rows="3" placeholder='${d(e.t("placeholder.json_buttons",{},'[{"action":"ID","title":"Label"}]'))}'>${d(v)}</textarea>`}else if(h.type==="checkboxlist"){const v=Array.isArray(t)&&t.length?t.map(String):Array.isArray(h.def)?h.def.map(String):[];i=`<div data-opt="${s}" data-ftype="checkboxlist" class="wd-checkboxlist">`+(h.choices||[]).map(([S,D])=>`<label class="wd-switch-lbl" style="display:flex;align-items:center;gap:8px;margin:2px 0"><span class="wd-switch"><input type="checkbox" data-choice="${d(S)}" ${v.includes(String(S))?"checked":""}><span class="wd-switch-slider"></span></span><span class="wd-switch-text">${d(e.t?e.t("lbl.evidence_"+S,{},D):D)}</span></label>`).join("")+"</div>"}else if(h.type==="entitylist"){const S=(Array.isArray(t)?t:t?[t]:[]).map(D=>`<span class="wd-pill" data-val="${d(D)}">${d(D)}<button type="button" class="wd-pill-x" aria-label="${d(e.t?e.t("btn.remove",{},"Remove"):"Remove")}">\xD7</button></span>`).join("");i=`<div class="wd-pillbox" data-opt="${s}" data-ftype="entitylist">${S}<div class="wd-combo wd-combo-pill"><input type="text" class="wd-pill-add" autocomplete="off" spellcheck="false" placeholder="${d(e.t("placeholder."+(h.domain||"add"),{},h.placeholder||"add\u2026"))}"><div class="wd-combo-drop" hidden></div></div></div>`}else if(h.type==="timerlist"){const v=Array.isArray(l)?l:[],S=e.t?e.t("lbl.timer_min",{},"min"):"min",D=e.t?e.t("lbl.timer_msg_placeholder",{},"Message (optional, {device}/{program}/{minutes})"):"Message (optional, {device}/{program}/{minutes})",z=e.t?e.t("lbl.timer_auto_pause",{},"Auto-pause"):"Auto-pause",O=e.t?e.t("btn.remove_timer",{},"Delete"):"Delete",R=e.t?e.t("btn.add_timer",{},"+ Add timer"):"+ Add timer",I=(C,A)=>{const F=C&&C.offset_minutes?String(C.offset_minutes):"",U=C&&C.message?d(C.message):"",st=C&&C.auto_pause?" checked":"";return`<div class="wd-timer-row" data-tidx="${A}"><div class="wd-timer-top"><input type="number" min="1" placeholder="${d(S)}" data-field="offset_minutes" value="${d(F)}"><textarea placeholder="${d(D)}" data-field="message">${U}</textarea></div><div class="wd-timer-footer"><label class="wd-switch-lbl"><span class="wd-switch"><input type="checkbox" data-field="auto_pause"${st}><span class="wd-switch-slider"></span></span><span class="wd-switch-text">${d(z)}</span></label><button type="button" class="wd-btn wd-btn-sm wd-btn-danger wd-timer-remove">${d(O)}</button></div></div>`},P=v.map((C,A)=>I(C,A)).join("");i=`<div class="wd-timerlist" data-opt="${s}" data-ftype="timerlist">${P}<button type="button" class="wd-btn wd-btn-sm wd-btn-secondary wd-timer-add">${d(R)}</button></div>`}else if(h.type==="entity"){const v=h.placeholder?` placeholder="${d(h.placeholder)}"`:"";i=`<div class="wd-combo"><input type="text" class="wd-combo-inp" data-opt="${s}" data-ftype="entity" value="${d(l)}" autocomplete="off" spellcheck="false"${v}><div class="wd-combo-drop" hidden></div></div>`}else if(h.type==="list"){const v=Array.isArray(l)?l.join(", "):d(l);i=`<input type="text" data-opt="${s}" data-ftype="list" value="${d(v)}" placeholder="${d(e.t("placeholder.notify_services_list",{},"notify.mobile_app_phone, ..."))}">`}else if(h.type==="intlist"){const v=Array.isArray(l)?l.join(", "):String(l??""),S=h.placeholder?` placeholder="${d(h.placeholder)}"`:"";i=`<input type="text" data-opt="${s}" data-ftype="intlist" value="${d(v)}"${S}>`}else{const v=h.type==="number"?"number":"text",S=e.datalistId?` list="${e.datalistId}"`:"",D=h.step!=null?` step="${h.step}"`:"",z=h.min!=null?` min="${h.min}"`:"",O=h.max!=null?` max="${h.max}"`:"",R=h.placeholder?` placeholder="${d(h.placeholder)}"`:"";i=`<input type="${v}" data-opt="${s}" data-ftype="${h.type}" value="${d(l)}"${D}${z}${O}${S}${R}>${e.datalist||""}`}const c=e.suggestion,_=e.mlSuggestion,p=c&&c.suggested!=null&&!Vt(c.suggested,t)?c.suggested:null,f=_&&_.value!=null&&!Vt(_.value,t)?_.value:null,g=e.t,$=c?c.reason_key?g(c.reason_key,c.reason_params||{},c.reason||""):c.reason||"":"",k=_?_.reason_key?g(_.reason_key,_.reason_params||{},_.reason||""):_.reason||"":"",x=v=>`<button type="button" class="wd-sug-use" data-sugkey="${s}" data-sugval="${d(v)}">${e.useBtnLabel||"Use"}</button>`;let b="";if(p!=null&&f!=null){const v=parseFloat(p),S=parseFloat(f);if((!isNaN(v)&&!isNaN(S)?Math.abs(v-S)/Math.max(Math.abs(v),Math.abs(S),1e-9):1)<.05){const z=g("suggestion.calibrated_label",{},"Calibrated"),O=lt([$,k?`${z}: ${k}`:""].filter(Boolean).join(`

`));b=`<div class="wd-sug"><span class="wd-sug-chip wd-sug-chip-obs">\u{1F4A1} ${d(g("suggestion.both_agree",{},"WashData recommends"))}</span><span class="wd-sug-val">${d(p)}${n}</span>${x(p)}${O}</div>`}else{const z=$?lt($):"",O=k?lt(k):"",R=g("suggestion.observed_label",{},"Observed"),I=g("suggestion.calibrated_label",{},"Calibrated");let P="",C="";if(!isNaN(v)&&!isNaN(S)){const U=S>v;C=g(`suggestion.impact.${s}.${U?"higher":"lower"}`,{},""),P=g(`suggestion.impact.${s}.${U?"lower":"higher"}`,{},"")}const A=P?`<div class="wd-sug-impact-line">${d(P)}</div>`:"",F=C?`<div class="wd-sug-impact-line">${d(C)}</div>`:"";b=`<div class="wd-sug wd-sug-split"><div class="wd-sug-opt"><span class="wd-sug-chip wd-sug-chip-obs">\u{1F4A1} ${d(R)}</span><span class="wd-sug-val">${d(p)}${n}</span>${x(p)}${z}${A}</div><div class="wd-sug-opt"><span class="wd-sug-chip wd-sug-chip-cal">\u{1F916} ${d(I)}</span><span class="wd-sug-val">${d(f)}${n}</span>${x(f)}${O}${F}</div></div>`}}else if(p!=null){const v=$?lt($):"",S=t!=null&&t!==""?` <span style="opacity:.6;font-size:.9em">(now ${d(t)}${n})</span>`:"";b=`<div class="wd-sug"><span class="wd-sug-chip wd-sug-chip-obs">\u{1F4A1} ${d(g("suggestion.observed_label",{},"Observed"))}</span><span class="wd-sug-val">${d(p)}${n}</span>${S}${x(p)}${v}</div>`}else if(f!=null){const v=k?lt(k):"";b=`<div class="wd-sug"><span class="wd-sug-chip wd-sug-chip-cal">\u{1F916} ${d(g("suggestion.calibrated_label",{},"Calibrated"))}</span><span class="wd-sug-val">${d(f)}${n}</span>${x(f)}${v}</div>`}if(b&&s){const v=d(g("btn.mute_suggestion",{},"Stop suggesting this setting")),S=`<button type="button" class="wd-sug-lock" data-suglock="${s}" title="${v}" aria-label="${v}">\u{1F515}</button>`;b=b.replace(/<\/div>\s*$/,S+"</div>")}return`<div class="wd-field" data-field="${s}"><div class="wd-label-row"><label style="margin:0">${d(r)}</label>${o}${a}</div>${i}${h.hint?`<div class="wd-field-hint">${d(h.hint)}</div>`:""}<div class="wd-conflict-err" data-cerr="${s}" hidden></div><div class="wd-setting-note" data-cnote="${s}" hidden></div>${b}</div>`}function Vt(h,t){if(h==null||t==null)return!1;const e=parseFloat(h),s=parseFloat(t);return!isNaN(e)&&!isNaN(s)?Math.abs(e-s)<1e-6:String(h)===String(t)}const Te={min_power:"min_power",off_delay:"off_delay",smoothing_window:"smoothing",start_threshold_w:"hysteresis",stop_threshold_w:"hysteresis",start_energy_threshold:"start_energy",running_dead_zone:"dead_zone",profile_duration_tolerance:"duration_tolerance",profile_match_min_duration_ratio:"match_ratios",profile_match_max_duration_ratio:"match_ratios",progress_reset_delay:"progress_reset",completion_min_seconds:"min_duration",min_off_gap:"min_off_gap",start_duration_threshold:"start_duration",end_energy_threshold:"end_energy_thresh",end_repeat_count:"end_repeat",profile_match_threshold:"confidence",profile_unmatch_threshold:"confidence",auto_label_confidence:"confidence",learning_confidence:"confidence",no_update_active_timeout:"watchdog_timeout",anti_wrinkle_enabled:"anti_wrinkle",anti_wrinkle_max_power:"anti_wrinkle",anti_wrinkle_max_duration:"anti_wrinkle",anti_wrinkle_exit_power:"anti_wrinkle",anti_wrinkle_idle_timeout:"anti_wrinkle",anti_crease_finalize_ratio:"anti_wrinkle",sampling_interval:"sampling"};function de(h){let t=0,e=window.innerWidth,s=0,r=window.innerHeight;for(let n=h.parentElement;n;n=n.parentElement){const a=getComputedStyle(n),o=a.overflowX!=="visible",i=a.overflowY!=="visible";if(!o&&!i)continue;const l=n.getBoundingClientRect();o&&(l.left>t&&(t=l.left),l.right<e&&(e=l.right)),i&&(l.top>s&&(s=l.top),l.bottom<r&&(r=l.bottom))}return{left:t,right:e,top:s,bottom:r}}function lt(h,t){return`<span class="wd-tip">i<span class="wd-tip-pop">${t?Ee(t):""}<span class="wd-tip-txt">${d(h)}</span></span></span>`}function Ut(h,t,e=""){return`<label class="wd-switch-lbl"><span class="wd-switch"><input type="checkbox" ${h}><span class="wd-switch-slider"></span></span><span class="wd-switch-text wd-switch-text--plain">${d(t)}</span></label>${e||""}`}function Mt(h,t,e="",s=""){return`<div class="wd-field wd-field-switch"><div class="wd-switch-row">${Ut(h,t,e)}</div>${s?`<div class="wd-field-hint">${d(s)}</div>`:""}</div>`}function Ee(h){const t=s=>`<svg class="wd-dg" viewBox="0 0 200 90" preserveAspectRatio="xMidYMid meet">${s}</svg>`,e='<line class="ax" x1="8" y1="78" x2="192" y2="78"/>';switch(h){case"smoothing":return t(`${e}
      ...
      <input class="wd-log-filter wd-log-search" data-logfilter="search" data-ctx="${t}" type="search" value="${d(this._logSearch)}" placeholder="${d(this._t("log.search_ph",{},"Search logs\u2026"))}" style="font-size:.8em;padding:2px 6px;min-width:120px">`}_refreshLogViews(){const t=this.shadowRoot;if(!t)return;const e=this._logLinesHtml();["wd-log-lines-drawer","wd-log-lines-page"].forEach(s=>{const r=t.getElementById(s);r&&(r.innerHTML=e)})}_syncLogFilters(t){const e=this.shadowRoot;e&&e.querySelectorAll(`.wd-log-filter[data-logfilter="${t.dataset.logfilter}"]`).forEach(s=>{s!==t&&s.value!==t.value&&(s.value=t.value)})}async _fetchRecState(t){try{this._recState=await this._ws({type:`${w}/get_recording_state`,entry_id:t})}catch{}}async _fetchFeedbacks(t){try{const e=await this._ws({type:`${w}/get_feedbacks`,entry_id:t});this._feedbacks=e.feedbacks||[]}catch{}}async _fetchPhases(t){try{const e=await this._ws({type:`${w}/get_phase_catalog`,entry_id:t});this._phases=e.phases||[]}catch{}}_localize(t,e){try{const s=this._hass&&this._hass.localize?this._hass.localize(t):"";return s&&s!==t?s:e}catch{return e}}_tLookup(t,e){const s=this._panelTrans&&(this._panelTrans[e]||this._panelTrans.en);if(!s)return null;const r=t.split(".").reduce((n,a)=>n&&n[a]!==void 0?n[a]:null,s);return r&&typeof r=="string"?r:null}_t(t,e={},s=""){let r;const a=this._panelCfg&&this._panelCfg.prefs&&this._panelCfg.prefs.lang_override||this._hass&&this._hass.locale&&this._hass.locale.language;this._panelTrans?r=a&&this._tLookup(t,a)||this._tLookup(t,"en")||s:r=this._localize(`component.${w}.panel.${t}`,s);for(const[o,i]of Object.entries(e))r=r.replace(new RegExp(`\\{${o}\\}`,"g"),String(i));return r}_stateColor(t){const e=this._constants.stateColors||{};return e[t]||e.unknown||"var(--disabled-color, #bdbdbd)"}_stateLabel(t){const e=(t||"unknown").replace(/_/g," ").replace(/\b\w/g,s=>s.toUpperCase());return this._localize(`component.${w}.entity.sensor.washer_state.state.${t}`,e)}_deviceTypeLabel(t){const e=(this._constants.deviceTypes||[]).find(r=>r.id===t),s=e?e.label:(t||"").replace(/_/g," ");return this._localize(`component.${w}.selector.device_type.options.${t}`,s)}_deviceTypeOpts(t){const e=(this._constants.deviceTypes||[]).map(s=>[s.id,this._deviceTypeLabel(s.id)]);return t&&!e.some(([s])=>String(s)===String(t))&&e.push([t,this._deviceTypeLabel(t)]),e}_deviceOpts(t){const e=[["","- None -"]],s=this._hass&&this._hass.devices?this._hass.devices:{},r=this._ownDeviceId();return Object.values(s).forEach(n=>{if(r&&String(n.id)===r)return;const a=n.name_by_user||n.name||n.id;e.push([n.id,a])}),t&&String(t)!==r&&!e.some(([n])=>String(n)===String(t))&&e.push([t,this._t("lbl.device_unresolved",{id:t},`Unavailable device (${t})`)]),e}_ownDeviceId(){const t=this._devices&&this._devices[this._selIdx],e=t&&t.entry_id;if(!e)return"";const s=this._hass&&this._hass.devices?this._hass.devices:{};for(const r of Object.values(s)){if(!r||!r.id)continue;if((Array.isArray(r.identifiers)?r.identifiers:[]).some(a=>Array.isArray(a)&&a[0]===w&&String(a[1])===String(e))||Array.isArray(r.config_entries)&&r.config_entries.map(String).includes(String(e)))return String(r.id)}return""}_applyFontScale(t){let e=parseFloat(t);isFinite(e)||(e=1),e=Math.max(.7,Math.min(2,e)),this.style.fontSize=e===1?"":Math.round(e*1e3)/10+"%"}_applyPanelConfig(){const t=this._panelCfg;if(!t)return;this._applyFontScale(t.prefs&&t.prefs.font_scale||1);const e=t.prefs&&t.prefs.lang_override;e&&!(this._panelTrans&&this._panelTrans[e])&&this._loadPanelLang(e).then(()=>this._render()).catch(()=>{});const s=t.panel||{};if(!this._tabInitialized){const r=t.prefs&&t.prefs.default_tab||s.default_tab;r&&["status","history","profiles","settings","playground"].includes(r)&&(this._tab=r),this._tabInitialized=!0}}_isAdmin(){return!!(this._panelCfg&&this._panelCfg.is_admin)}_curPerm(){const t=this._devices[this._selIdx];return t&&t.perm||"full"}_canEdit(){const t=this._curPerm();return this._isAdmin()||t==="edit"||t==="full"}_canFull(){const t=this._curPerm();return this._isAdmin()||t==="full"}_onlineEnabled(){return!!(this._constants&&this._constants.storeOnlineAvailable&&this._constants.storeOnlineEnabled)}_visibleTabIds(){const t=this._isAdmin(),e=!t&&this._panelCfg&&this._panelCfg.panel&&this._panelCfg.panel.hidden_tabs||[],s=["status","history","profiles"];return this._canEdit()&&s.push("settings"),this._canEdit()&&s.push("playground"),this._canEdit()&&this._onlineEnabled()&&s.push("store"),s.push("advanced"),s.filter(r=>t||!e.includes(r))}async _busyRun(t,e){this._busy.add(t),this._render();try{return await e()}finally{this._busy.delete(t),this._render()}}async _closeCycleDetail(t){const e=this._prevModal;if(this._prevModal=null,e&&e.type==="profile-panel"){if(this._modal=e,this._render(),e.tab==="cleanup")try{const s=await this._ws({type:`${w}/get_profile_cycles`,entry_id:t,profile_name:e.name});this._modal&&this._modal.type==="profile-panel"&&this._modal.name===e.name&&(this._modal.cleanup={cycles:s.cycles||[],selected:new Set},this._render(),this._drawSpaghetti())}catch{}}else this._modal=null,this._render()}_SCROLLERS=[".wd-main",".wd-modal",".wd-logs",".wd-log-drawer-body",".wd-table-wrap",".wd-sd-tree"];_SCROLLERS_CHROME=[".wd-tabs"];_MODAL_KEY_PREFIX="modal:";_eachScroller(t,e){const s=this.shadowRoot;if(s)for(const r of t){const n=Array.from(s.querySelectorAll(r)),a=o=>r!==".wd-modal"&&!!o.closest(".wd-modal");for(const o of[!1,!0]){const i=n.filter(c=>a(c)===o),l=o?this._MODAL_KEY_PREFIX:"";i.forEach((c,_)=>e(c,`${l}${r}|${_}/${i.length}`))}}}_captureScroll(){if(!this.shadowRoot)return null;const t={};return this._eachScroller([...this._SCROLLERS,...this._SCROLLERS_CHROME],(e,s)=>{(e.scrollTop||e.scrollLeft)&&(t[s]={top:e.scrollTop,left:e.scrollLeft})}),t}_scrollChromeOnly(t){if(!t)return t;const e={};for(const[s,r]of Object.entries(t))this._SCROLLERS_CHROME.some(n=>s.startsWith(`${n}|`))&&(e[s]=r);return e}_restoreScroll(t){!t||!this.shadowRoot||this._eachScroller([...this._SCROLLERS,...this._SCROLLERS_CHROME],(e,s)=>{const r=t[s];if(!r)return;const n=Math.max(0,e.scrollHeight-e.clientHeight),a=Math.max(0,e.scrollWidth-e.clientWidth);e.scrollTop=Math.min(r.top,n),e.scrollLeft=Math.min(r.left,a)})}_navKey(){return[this._tab,this._selIdx,this._panelSubtab,this._profSubtab,this._toolsSubtab,this._settingsSec,this._storeView].join("|")}_modalNavKey(){const t=this._modal;if(!t)return"";const e=t.type==="profile-group"?t.orig||"":t.name||t.cycleId||"";return[t.type,e,t.tab||t.mode||t.step||""].join("|")}_render(){if(!this._container)return;Qt=this._pref("date_format","relative");const t=this.shadowRoot,e=t&&(t.activeElement||this.getRootNode()&&this.getRootNode().activeElement)||null;this._hideGraphTip();const s=this._navKey(),r=this._scrollNavKey!=null&&this._scrollNavKey!==s;this._scrollNavKey=s;let n=this._scrollCarry||this._captureScroll();r&&(n=this._scrollChromeOnly(n));const a=this._modalNavKey();this._scrollModalKey!==void 0&&a!==this._scrollModalKey&&n&&(n=Object.fromEntries(Object.entries(n).filter(([o])=>!o.startsWith(".wd-modal|")&&!o.startsWith(this._MODAL_KEY_PREFIX)))),this._scrollModalKey=a,this._container.innerHTML=this._buildHtml(),this._wire(),this._tabLoading?(this._scrollCarry=n,this._restoreScroll(this._scrollChromeOnly(n))):(this._scrollCarry=null,this._restoreScroll(n)),this._drawStatusCurve(),this._drawModalCanvas(),this._drawProfileSparklines(),this._drawHistorySparklines(),this._drawPlaygroundCanvases(),["wd-status-canvas","wd-cyc-canvas","wd-compare-canvas","wd-env-canvas","wd-phase-canvas","wd-spag-canvas","wd-pgroup-canvas"].forEach(o=>this._attachGraphGestures(o)),this._syncModalFocus(e),requestAnimationFrame(()=>this._resizeLogsPage())}_resizeLogsPage(){const t=this.shadowRoot&&this.shadowRoot.getElementById("wd-log-lines-page");if(!t)return;const e=t.getBoundingClientRect();e.top<=0||(t.style.height=window.innerHeight-e.top-24+"px")}_syncModalFocus(t=null){const e=this.shadowRoot;if(!e)return;const s=e.querySelector('.wd-modal[role="dialog"]');if(s){const r=s.querySelector("h2");if(r&&!r.id&&(r.id="wd-modal-title"),this._modalFocusActive){const n=e.activeElement||this.getRootNode()&&this.getRootNode().activeElement||null;if(!n||!s.contains(n)){const a=qt(s);try{(a[0]||s).focus({preventScroll:!0})}catch{}}}else{this._modalFocusActive=!0;const n=t||null;n&&!s.contains(n)&&(this._modalReturnFocus=n);const a=qt(s);try{(a[0]||s).focus()}catch{}}}else if(this._modalFocusActive){this._modalFocusActive=!1;const r=this._modalReturnFocus;if(this._modalReturnFocus=null,r&&r.isConnected&&typeof r.focus=="function")try{r.focus()}catch{}}}_renderPreservingFormEdits(){this._snapshotFormToPending(this.shadowRoot),this._render()}_snapshotCycleReviewForm(t){const e=this._modal;if(!t||!e||e.type!=="cycle-detail"||e.mode!=="review")return;const s=t.getElementById("wd-cyc-rev-quality"),r=t.getElementById("wd-cyc-rev-golden"),n=t.getElementById("wd-cyc-rev-notes"),a=t.getElementById("wd-cyc-rev-label");if(!s&&!r&&!n&&!a)return;e.ml||(e.ml={}),e.ml.ml_review||(e.ml.ml_review={});const o=e.ml.ml_review;s&&(o.quality=s.value||""),r&&(o.golden=!!r.checked),n&&(o.notes=n.value||""),o.tags=Array.from(t.querySelectorAll(".wd-cyc-rev-tag")).filter(i=>i.checked).map(i=>i.value),a&&e.curve&&(e.curve.profile_name=a.value||"")}_buildHtml(){const t=this._toast?`<div class="wd-toast ${this._toast.cls}" role="${this._toast.cls.includes("error")?"alert":"status"}" aria-live="${this._toast.cls.includes("error")?"assertive":"polite"}"><span>${d(this._toast.msg)}</span>${this._toast.actionLabel?`<button type="button" class="wd-toast-action" data-toast-undo="${d(this._toast.actionToken||"")}">${d(this._toast.actionLabel)}</button>`:""}</div>`:"";return`
      <div class="wd-shell">
        ${this._htmlHeader()}
    ...
    `}_htmlDeviceBar(){const t=this._isAdmin()?`<button class="wd-devcard wd-devadd" data-action="add-device" title="${d(this._t("btn.add_device_tip",{},"Add another WashData device"))}">${this._t("btn.add_device",{},"+ Add device")}</button>`:"";return this._devices.length<=1?t?`<div class="wd-devbar">${t}</div>`:"":`<div class="wd-devbar">${this._devices.map((e,s)=>{const r=e.is_user_paused?"user_paused":e.detector_state||"unknown",n=Rt.includes(r),a=!!e.recording,o=a?"var(--error-color, #f44336)":this._stateColor(r),i=a?this._t("status.recording",{},"Recording"):this._stateLabel(r),l=[],c=this._conflictCountForOpts(e.options||{},e.option_defaults||{});c&&l.push(`<span class="wd-dbadge conf">\u26A0 ${c}</span>`);const _=this._sugCountsForDevice(e).total;return _&&l.push(`<span class="wd-dbadge sug">\u{1F4A1} ${_}</span>`),e.feedback_count&&l.push(`<span class="wd-dbadge fb">\u{1F4AC} ${e.feedback_count}</span>`),`<button class="wd-devcard ${s===this._selIdx?"active":""}" data-idx="${s}">
        ...
        ${l.join("")}
      </button>`}).join("")}${t}</div>`}_htmlStatus(){const t=this._devices[this._selIdx];if(!t)return`<div class="wd-empty">${this._t("msg.no_device_selected",{},"No device selected.")}</div>`;const e=!!t.is_user_paused,s=e?"user_paused":t.detector_state||"unknown",r=!!t.recording,n=r?"var(--error-color, #f44336)":this._stateColor(s),a=r?this._t("status.recording",{},"Recording"):this._stateLabel(s),o=r||Rt.includes(s),i=t.cycle_progress_pct,l=t.time_remaining_s,c=t.current_program,_=!!t.manual_program,p=c?null:t.armed_program||null,f=c||p||"auto_detect",g=(this._profiles||[]).map(L=>L.name);c&&!g.includes(c)&&g.unshift(c),p&&!g.includes(p)&&g.unshift(p);const $=g.map(L=>`<option value="${d(L)}" ${f===L?"selected":""}>${d(L)}</option>`).join(""),k=c?_?this._t("badge.manual",{},"(manually selected)"):this._t("badge.auto",{},"(auto-detected)"):p?this._t("badge.armed",{},"(applies to the next cycle)"):"",b=k?`<span class="wd-prog-tag ${c?_?"manual":"auto":"manual"}">${k}</span>`:"",v=`<div class="wd-prog-ctl"><label>${this._t("lbl.program",{},"Program")}</label>${lt(this._t("lbl.program_tip",{},"Override which profile is matched to the current cycle. Auto-detect lets the integration pick the best match automatically. Pin a specific program to force-match it when auto-detect is wrong or you know what is running. Pick one before starting the appliance and it is applied as soon as the next cycle begins."))}
          <select id="wd-status-prog">
            <option value="auto_detect" ${f==="auto_detect"?"selected":""}>${this._t("status.auto_detect",{},"Auto-detect")}</option>
            ${$}
          </select>${b}</div>`,S=[];t.recording&&this._canEdit()&&S.push(`<div class="wd-attn-card"><span class="wd-attn-icon">\u25CF</span><div class="wd-attn-body"><div class="wd-attn-title">${this._t("msg.recording_in_progress",{},"Recording in progress")}</div><div class="wd-attn-sub">${this._t("msg.see_recorder",{},"See recorder widget below")}</div></div></div>`),t.feedback_count&&this._canEdit()&&S.push(`<button class="wd-attn-card" type="button" data-action="goto-feedbacks"><span class="wd-attn-icon">\u{1F4AC}</span><div class="wd-attn-body"><div class="wd-attn-title">${this._t("msg.feedback_cycles_pending",{n:t.feedback_count},`To review: ${t.feedback_count}`)}</div><div class="wd-attn-sub">${this._t("msg.review_to_cycles",{},"Open the Cycles review queue")}</div></div></button>`);const D=this._conflictKeysFromOpts();if(D.size&&this._canEdit()){const L=D.size,W=L>1?"s":"";S.push(`<button class="wd-attn-card" type="button" style="border-color:var(--error-color,#b71c1c)" data-action="goto-conflicts"><span class="wd-attn-icon">\u26A0</span><div class="wd-attn-body"><div class="wd-attn-title" style="color:var(--error-color,#b71c1c)">${this._t("conflict.attn_title",{n:L},`Setting conflicts: ${L}`)}</div><div class="wd-attn-sub">${this._t("conflict.attn_sub",{},"Fix conflicts before saving")}</div></div></button>`)}const z=this._sugCountsForDevice(t);if(z.total&&this._canEdit()){const L=z.total,W=[];z.classic&&W.push(this._t("lbl.n_classic_suggestions",{n:z.classic},`${z.classic} classic`)),z.ml&&W.push(this._t("lbl.n_ml_suggestions",{n:z.ml},`${z.ml} ML`)),S.push(`<button class="wd-attn-card" type="button" data-action="goto-suggestions"><span class="wd-attn-icon">\u{1F4A1}</span><div class="wd-attn-body"><div class="wd-attn-title">${this._t("lbl.n_tuning_suggestions",{n:L},`${L} tuning suggestion${L>1?"s":""}`)}</div><div class="wd-attn-sub">${W.join(" \xB7 ")} \xB7 ${this._t("msg.review_in_settings",{},"Review in Settings")}</div></div></button>`)}const O=t.standby_above_stop;O&&this._canEdit()&&S.push(`<button class="wd-attn-card" type="button" style="border-color:var(--warning-color,#ff9800)" data-action="goto-standby"><span class="wd-attn-icon">\u26A0</span><div class="wd-attn-body"><div class="wd-attn-title" style="color:var(--warning-color,#ff9800)">${this._t("msg.standby_above_stop_title",{idle:O.idle_w,stop:O.stop_threshold_w},`Still drawing ${O.idle_w} W when the cycle ended`)}</div><div class="wd-attn-sub">${this._t("msg.standby_above_stop_sub",{n:O.cycles_above,total:O.cycles_checked,stop:O.stop_threshold_w},`${O.cycles_above} of the last ${O.cycles_checked} cycles. The off delay only starts below the Stop Threshold (${O.stop_threshold_w} W), so cycles finish late.`)}</div></div></button>`);const R=S.length?`<div class="wd-attn">${S.join("")}</div>`:"",I=t.envelope_position,P=I!=null?Math.round(I*100):null,C=P!=null?` <span style="opacity:.75" title="${d(this._t("lbl.envelope_position_tip",{pct:P},`Position on the matched program's own recorded curve, ${P}%. Measured by aligning this cycle against the profile instead of counting time, so it stays right when a run is longer or shorter than usual. It refreshes while the appliance is quiet, so it can lag behind during an active phase.`))}">${this._t("lbl.envelope_position",{pct:P},`curve ${P}%`)}</span>`:"",A=o&&i!=null?`
      <div class="wd-prog-bg"><div class="wd-prog-fill" style="width:${Math.min(100,i)}%"></div></div>
      <div class="wd-prog-row"><span>${i.toFixed(1)}%${C}</span>${l!=null?`<span>${this._t("lbl.time_remaining",{v:_t(l)},`~${_t(l)} remaining`)}</span>`:""}</div>
    `:"",U=((this._powerData||{}).live||[]).length>1,st=this._pref("show_expected",!0),tt=this._pref("show_raw_active",!1),X=`<div class="wd-leg">
      <span class="wd-leg-i"><span class="wd-leg-sw" style="background:var(--primary-color)"></span> ${this._t("lbl.power",{},"Power")}</span>
      ${this._statusEnv?`<label class="wd-leg-i"><input type="checkbox" data-statustoggle="show_expected" ${st?"checked":""}><span class="wd-leg-sw" style="background:#ff9800"></span> ${this._t("lbl.expected",{},"Expected")}</label>`:""}
      ${this._pref("show_raw",!1)?`<label class="wd-leg-i"><input type="checkbox" data-statustoggle="show_raw_active" ${tt?"checked":""}><span class="wd-leg-sw" style="background:#9e9e9e"></span> ${this._t("lbl.raw_socket",{},"Raw socket")}</label>`:""}
    </div>`,Q=this._cyclesTotal||0,vt=(this._profiles||[]).length,Y=this._pref("setup_card_dismissed",!1),dt=this._setupStatus,J=dt&&!U,ot=J?this._htmlSetupCard(dt,Y):"",ht=U?`<div class="wd-canvas-wrap" style="margin-top:14px"><canvas id="wd-status-canvas" role="img" aria-label="${d(this._t("lbl.aria_power_chart",{},"Power consumption chart"))}" style="height:160px"></canvas></div>${X}`:J?ot:`<p class="wd-info" style="margin-top:12px">${this._t("msg.live_chart_loading",{},"Live power chart appears as readings arrive.")}</p>`,pt=this._pref("show_debug",!1);let y="";if(pt){const L=this._matchDebug||{},W=L.confidence!=null?`${(L.confidence*100).toFixed(1)}%`:"-",H=(L.candidates||[]).map(N=>`<tr><td>${d(N.profile_name)}</td><td>${N.confidence_pct}%</td><td>${N.mae}</td><td>${N.correlation}</td><td>${N.duration_ratio>=0?"+":""}${N.duration_ratio}%</td></tr>`).join("");y=`<div class="wd-card">
        <div class="wd-card-title">Live Match Debug ${lt("Confidence: how closely the current power curve matches the top candidate profile (0-100%). Ambiguous: the two best candidates score within 5% of each other - the label is uncertain until the cycle finishes.")}</div>
        <div class="wd-kv" style="margin-bottom:12px">
          ...
          <div class="wd-kv-item"><div class="wd-kv-val" style="font-size:1em;color:${L.ambiguous?"var(--warning-color,#ff9800)":"var(--success-color,#4caf50)"}">${L.ambiguous?this._t("status.ambiguous",{},"Ambiguous"):this._t("status.clear",{},"Clear")}</div><div class="wd-kv-lbl">${this._t("lbl.label",{},"Match")}</div></div>
        </div>
        ${H?`<table class="wd-table"><thead><tr><th>Profile</th><th>Conf</th><th>MAE</th><th>Corr</th><th>Duration</th></tr></thead><tbody>${H}</tbody></table>`:`<p class="wd-info">${this._t("msg.no_match_yet",{},"No match attempt yet - this populates during a running cycle.")}</p>`}
      </div>`}const m=[];this._canEdit()&&m.push(`<button class="wd-attn-card" type="button" data-action="open-advanced" data-sub="diagnostics"><span class="wd-attn-icon">\u{1FA7A}</span><div class="wd-attn-body"><div class="wd-attn-title">${this._t("hdr.logs_diagnostics",{},"Diagnostics")}</div><div class="wd-attn-sub">${this._t("msg.storage_diagnostics",{},"Storage stats, maintenance, export/import")}</div></div></button>`),m.push(`<button class="wd-attn-card" type="button" data-action="open-settings"><span class="wd-attn-icon">\u2699\uFE0F</span><div class="wd-attn-body"><div class="wd-attn-title">${this._t("settings.gear.title",{},"Settings")}</div><div class="wd-attn-sub">${this._isAdmin()?this._t("msg.preferences_admin",{},"Preferences, panel & access control"):this._t("msg.preferences_adv",{},"Preferences")}</div></div></button>`);const u=`<div class="wd-card"><div class="wd-card-title">${this._t("hdr.tools_and_data",{},"Tools & Data")}</div><div class="wd-attn" style="margin-bottom:0;margin-top:12px">${m.join("")}</div></div>`,E=(()=>{if(!this._canEdit())return"";const L=Rt.includes(s),W=L&&!e,H=e,N=L||e;return!W&&!H&&!N?"":`<div class="wd-cycle-ctrl" style="margin-top:0">
        ${H?`<button class="wd-btn wd-btn-sm wd-btn-primary" data-action="resume-cycle" title="${d(this._t("btn.resume_cycle_tip",{},"Resume the paused cycle"))}">${this._t("btn.resume_cycle",{},"Resume")}</button>`:""}
        ${W?`<button class="wd-btn wd-btn-sm" data-action="pause-cycle" title="${d(this._t("btn.pause_cycle_tip",{},"Pause the running cycle \u2014 the appliance will resume where it left off"))}">${this._t("btn.pause_cycle",{},"Pause")}</button>`:""}
        ${N?`<button class="wd-btn wd-btn-sm wd-btn-danger" data-action="terminate-cycle" title="${d(this._t("btn.force_stop_tip",{},"Immediately end the current cycle and mark it as force-stopped"))}">${this._t("btn.force_stop",{},"Force Stop")}</button>`:""}
      </div>`})();return`
      ${R}
      <div class="wd-card">
        ...
        ${v}
        <div class="wd-stats">
          <div class="wd-stat"><div class="wd-stat-val">${Zt(t.current_power_w)}</div><div class="wd-stat-lbl">${this._t("lbl.power",{},"Power")}</div></div>
    ...
    `}_htmlSetupCard(t,e=!1){if(!t)return"";const{phase:s,message_key:r,message_params:n,cta_label_key:a,cta_action:o,secondary_label_key:i,secondary_action:l,skippable:c,dismissible:_,step_key:p}=t;if(s==="phase4")return`
        ...
        </div>`;const f=this._t(r,n||{},""),g=this._t(a,{},"Continue"),$=i?this._t(i,{},""):null,k=c?`
      ...
      <div class="wd-setup-card" data-phase="${d(s)}">
        ...
        <p style="margin:6px 0 12px">${d(f)}</p>
        <div style="display:flex;flex-wrap:wrap;gap:8px;align-items:center">
          ...
          ${$?`<a class="wd-link" data-action="setup-cta"
                           data-cta-action="${d(l||"")}"
                           data-cta-params="${d(JSON.stringify(n||{}))}">
            ${d($)}
          </a>`:""}
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;flex-wrap:wrap">
        <div style="display:flex;align-items:center;gap:8px">
          ...
          <div><strong>${this._t("hdr.manual_recording",{},"Manual Recording")}</strong>${lt(this._t("hdr.manual_recording_tip",{},"Run a cycle intentionally while WashData records the power trace. Start just before the appliance starts, Stop when it finishes, then Process to save it as a named profile."))}${o?`<span class="wd-field-hint" style="margin-left:8px">${o}</span>`:""}</div>
        </div>
        <div style="display:flex;gap:6px;flex-wrap:wrap">${i}</div>
      </div>
    </div>`}_htmlHistory(){const t=this._cycles||[],e=this._refCycles||[],s=e.concat(t),r=this._canEdit(),n=this._selectMode&&r,a=this._cycleSel,{col:o,dir:i}=this._cycleSort,{text:l,status:c}=this._cycleFilter,_=this._mlById||{},p=new Set((this._feedbacks||[]).map(u=>u.cycle_id)),f=u=>_[u.id],g=u=>{const E=f(u);return!!(E&&E.ml_review&&E.ml_review.reviewed_at)},$=u=>{const E=f(u);return!!(E&&E.ml_review&&E.ml_review.golden)},k=u=>{if(p.has(u.id))return!0;if(g(u))return!1;const E=f(u),L=E&&E.ml_quality_label;return["uncertain","review"].includes(L)||["force_stopped","interrupted"].includes(u.status)},x=s.filter(k).length;let b=s;if(l){const u=l.toLowerCase();b=b.filter(E=>(E.profile_name||E.matched_profile||"").toLowerCase().includes(u))}c==="unlabelled"?b=b.filter(u=>!u.profile_name&&!u.matched_profile):c==="needs_review"?b=b.filter(k):c==="imported"?b=b.filter(u=>u.is_reference):c&&(b=b.filter(u=>(u.status||"completed")===c));const v={date:u=>u.start_time?new Date(u.start_time).getTime():0,confidence:u=>u.match_confidence,duration:u=>u.duration,energy:u=>u.energy_kwh!=null?u.energy_kwh:u.energy_wh!=null?u.energy_wh/1e3:null,cost:u=>u.cost!=null?u.cost:-1,status:u=>u.status||"completed",profile:u=>(u.profile_name||u.matched_profile||"\uFFFF").toLowerCase()};b=ae(b,v[o]||v.date,i);const S=u=>u==="completed"?"var(--success-color, #4caf50)":u==="interrupted"?"var(--error-color, #f44336)":u==="force_stopped"?"var(--warning-color, #ff9800)":"var(--secondary-text-color)",D=u=>{if(!u.is_reference)return"";const E=u.cycle_origin==="backfill",L=E?this._t("badge.backfilled_tip",{},"Detected in imported power history. Shapes program matching only, not counted in stats."):this._t("badge.imported_tip",{},"Imported from the community store. Used for matching only, not counted in stats.");return` <span title="${d(L)}" style="color:var(--info-color,#2196f3)">${E?"\u{1F557}":"\u{1F4E5}"}</span>`},z=u=>$(u)?' <span title="'+d(this._t("badge.golden_cycle",{},"Recorded reference cycle"))+'" style="color:var(--warning-color,#ff9800)">\u2B50</span>':p.has(u.id)?' <span title="'+d(this._t("badge.feedback_requested",{},"Feedback requested"))+'" style="color:var(--info-color,#2196f3)">\u{1F4AC}</span>':g(u)?' <span title="'+d(this._t("badge.reviewed",{},"Reviewed"))+'" style="color:var(--success-color,#4caf50)">\u2713</span>':k(u)?' <span title="'+d(this._t("badge.needs_review",{},"Needs review"))+'" style="color:var(--error-color,#f44336)">\u25CF</span>':"",O=u=>{if(u.anomaly!=="overrun")return"";const E=u.overrun_ratio?" "+this._t("badge.overrun_ratio",{x:Number(u.overrun_ratio).toFixed(1)},`(${Number(u.overrun_ratio).toFixed(1)}x expected)`):"";return` <span title="${d(this._t("badge.overrun",{},"Ran longer than usual"))}${d(E)}" style="color:var(--warning-color,#ff9800)">\u23F1</span>`},R=u=>{if(u.anomaly!=="underrun")return"";const E=u.underrun_ratio?" "+this._t("badge.underrun_ratio",{pct:Math.round(u.underrun_ratio*100)},`(${Math.round(u.underrun_ratio*100)}% of expected)`):"";return` <span title="${d(this._t("badge.underrun",{},"Finished faster than usual"))}${d(E)}" style="color:var(--info-color,#2196f3)">\u26A1</span>`},I=u=>{if(!u.energy_anomaly||u.energy_anomaly==="none")return"";const E=u.energy_anomaly==="energy_spike",L=u.energy_z_score!=null?` (${u.energy_z_score>0?"+":""}${Number(u.energy_z_score).toFixed(1)}\u03C3)`:"",W=E?"badge.energy_spike":"badge.energy_low",H=E?"Higher energy than usual":"Lower energy than usual",N=E?"\u{1F53A}":"\u{1F53B}",V=E?"var(--error-color,#f44336)":"var(--info-color,#2196f3)";return` <span title="${d(this._t(W,{},H))}${d(L)}" style="color:${V}">${N}</span>`},P=u=>{const E=Array.isArray(u.artifacts)?u.artifacts.length:0;return E?` <span title="${d(this._t("badge.artifact_tip",{n:E},`${E} anomal${E>1?"ies":"y"} detected (e.g. door opened mid-cycle) \u2014 open to see them on the graph`))}" style="color:var(--warning-color,#ff9800)">\u26A0</span>`:""},C=u=>{const E=Array.isArray(u.restart_gaps)?u.restart_gaps.length:0;return E?` <span title="${d(this._t("badge.restart_gap_tip",{n:E},`${E} HA restart gap${E>1?"s":""} during this cycle \u2014 power trace has a hole`))}" style="color:var(--info-color,#2196f3)">\u21BB</span>`:""},A=this._hass&&this._hass.config&&this._hass.config.currency||"",F=u=>{if(u.cost==null)return"-";const E=`${u.cost.toFixed(2)}${A?" "+A:""}`;if(u.energy_price_mode==="dynamic"&&u.energy_price!=null){const L=this._t("col.cost_dynamic_tip",{price:u.energy_price.toFixed(4),cur:A},`Time-weighted: charged at the price in force during the cycle (effective ${u.energy_price.toFixed(4)} ${A}/kWh).`);return`<span title="${d(L)}" style="border-bottom:1px dotted var(--secondary-text-color)">${E}</span>`}return E},U=b.map(u=>{const E=u.profile_name||u.matched_profile,L=u.match_confidence!=null?u.match_confidence*100:null,W=u.status||"completed",H=u.energy_kwh!=null?u.energy_kwh:u.energy_wh!=null?u.energy_wh/1e3:null,N=n,V=N?`<input type="checkbox" class="wd-csel" ${a.has(u.id)?"checked":""} style="width:auto;margin:0">`:`<span class="wd-devdot" style="background:${S(W)}" title="${d(W)}"></span>`,G={completed:this._t("status.completed",{},"Completed"),interrupted:this._t("status.interrupted",{},"Interrupted"),force_stopped:this._t("status.force_stopped",{},"Force stopped"),active:this._t("status.active",{},"Active")}[W]||W,M=`${D(u)}${z(u)}${O(u)}${R(u)}${I(u)}${P(u)}${C(u)}`.trim();return`<tr data-cid="${d(u.id)}" data-selmode="${N?1:0}" style="cursor:pointer">
        <td style="width:26px;padding:6px 4px 6px 8px">${V}</td>
        <td>${E?d(E):`<span style="color:var(--secondary-text-color)">${this._t("lbl.unlabelled",{},"Unlabelled")}</span>`}</td>
        <td class="wd-tc-flags">${M}</td>
        <td><span style="color:${S(W)};font-size:.9em">${d(G)}</span></td>
        <td class="wd-tc-date">${rt(u.start_time)}</td>
        <td class="wd-tc-num">${_t(u.duration)}</td>
        <td class="wd-tc-num">${H!=null?yt(H):"-"}</td>
        <td class="wd-tc-num">${F(u)}</td>
        <td class="wd-tc-num">${L!=null?L.toFixed(0)+"%":"-"}</td>
      </tr>`}).join(""),st=`<thead><tr>
      ...
      ${bt(this._t("lbl.cost",{},"Cost"),"cost",o==="cost",i,"cycsort","right",this._t("col.cost_tip",{},"Energy cost for this cycle, frozen at completion. With Time-Weighted Cost on, the power trace is charged at the price in force at each moment; otherwise energy x the single price in effect at the end. Set a price under Settings to populate it."))}
    ...
    </div>`,X=b.length!==s.length?this._t("lbl.n_shown",{n:b.length},`, ${b.length} shown`):"",Q=e.length?this._t("lbl.n_imported_note",{n:e.length},`, ${e.length} imported`):"",vt=this._t("lbl.cycles_title",{n:`${t.length}${Q}${X}`},`Cycles (${t.length}${Q}${X})`),Y=r?`<div class="wd-card-actions" style="margin:0 0 4px;justify-content:flex-end">
    ...
    </div>`:"",dt=new Set(e.map(u=>u.id)),J=[...a].some(u=>dt.has(u)),ot=n?`<div class="wd-card-actions" style="margin:0 0 10px">
    ...
    </div>`:"",ht=this._busy.has("cyc-load-more"),pt=this._cyclesHasMore?`<div style="text-align:center;margin-top:12px">
      <button class="wd-btn wd-btn-secondary wd-btn-sm" data-action="cyc-load-more" ${ht?"disabled":""}>${ht?'<span class="wd-spin"></span> ':""}${this._t("btn.load_more",{},"Load more")}</button>
    </div>`:"",y=`
      <div class="wd-card">
        <div class="wd-card-title">${vt}</div>
        ${tt}
        ${Y}${ot}
        ${b.length===0?`<div class="wd-empty" style="padding:24px"><div class="wd-icon">\u{1F4CB}</div>${s.length?this._t("msg.no_cycles_match",{},"No cycles match the current filter."):this._t("msg.no_cycles_yet",{},"No cycles recorded yet.")}</div>`:`<div class="wd-table-wrap"><table class="wd-table">${st}<tbody>${U}</tbody></table></div>`}
        ${pt}
      </div>`;return(this._cyclesError?`<div class="wd-error-state"><span>${this._t("msg.fetch_error",{},"Failed to load data.")}</span><button class="wd-btn" type="button" data-action="retry-cycles">${this._t("btn.retry",{},"Retry")}</button></div>`:"")+y}_trendIcon(t){return t==="up"?`<span title="${d(this._t("trend.up",{},"Trending up"))}" style="color:var(--warning-color,#ff9800)">\u2191</span>`:t==="down"?`<span title="${d(this._t("trend.down",{},"Trending down"))}" style="color:var(--info-color,#2196f3)">\u2193</span>`:""}_profileCardHtml(t){const e=t.avg_duration?this._t("lbl.duration_avg",{v:Math.round(t.avg_duration/60)},`~${Math.round(t.avg_duration/60)}m avg`):this._t("lbl.no_duration",{},"no duration"),s=t.avg_energy!=null?` \xB7 ${yt(t.avg_energy)}/cycle`:"",r=t.avg_energy!=null&&t.cycle_count?` \xB7 <strong>${yt(t.avg_energy*t.cycle_count)}</strong> total`:"",n=this._hass&&this._hass.config&&this._hass.config.currency||"",a=t.avg_cost!=null?` \xB7 ${this._t("lbl.avg_cost",{},"Avg")} ${t.avg_cost.toFixed(2)}${n?" "+n:""}/${this._t("lbl.per_cycle_short",{},"cycle")}`:"",o=(this._profileHealth||{})[t.name],i=(this._profileTrends||{})[t.name];let l="";o&&o.health_status==="poor"?l=`<span class="wd-badge" style="color:var(--error-color,#f44336);background:rgba(244,67,54,.12)" title="${d(this._t("badge.poor_fit_tip",{},"Inconsistent match history \u2014 consider rebuilding this profile"))}">\u26A0 ${this._t("badge.poor_fit",{},"poor fit")}</span>`:o&&o.health_status==="fair"&&(l=`<span class="wd-badge" style="color:var(--warning-color,#ff9800);background:rgba(255,152,0,.12)" title="${d(this._t("badge.fair_fit_tip",{},"Moderate match consistency \u2014 some cycles assigned to this profile have lower confidence scores. Label more cycles or re-record the profile to improve accuracy."))}">${this._t("badge.fair_fit",{},"fair fit")}</span>`);let c="";if(i){const R=this._trendIcon(i.duration_trend),I=i.energy_trend?this._trendIcon(i.energy_trend):"";if(i.duration_trend!=="stable"||i.energy_trend==="up"){const P=[];if(i.duration_trend!=="stable"){const A=`${i.duration_slope_pct>0?"+":""}${i.duration_slope_pct}`;P.push(i.duration_trend==="up"?this._t("msg.duration_trend_up_tip",{pct:A},`Duration up (${A}%/cycle)`):this._t("msg.duration_trend_down_tip",{pct:A},`Duration down (${A}%/cycle)`))}if(i.energy_trend&&i.energy_trend!=="stable"){const A=`${i.energy_slope_pct>0?"+":""}${i.energy_slope_pct}`;P.push(i.energy_trend==="up"?this._t("msg.energy_trend_up_tip",{pct:A},`Energy up (${A}%/cycle)`):this._t("msg.energy_trend_down_tip",{pct:A},`Energy down (${A}%/cycle)`))}const C=P.join(", ")||this._t("msg.performance_trending",{},"Performance trending");c=`<span class="wd-badge" style="color:var(--secondary-text-color,#888)" title="${d(C)}">${R}${I||""}</span>`}}const _=this._constants&&this._constants.PROFILE_MIN_WARMUP_CYCLES||5,p=o&&o.cycle_count||0,g=p<_&&!t.is_imported?`<span class="wd-badge" title="${d(this._t("msg.warmup_detail",{needed:_},`This profile needs ${_} labelled cycles before auto-matching begins. Every confirmed cycle helps it learn.`))}" style="background:var(--info-color,#2196f3);color:#fff">${this._t("msg.warmup_badge",{done:p,needed:_},`Still learning (${p}/${_} cycles)`)}</span>`:"",$=t.is_imported?`<span class="wd-badge" title="${d(this._t("badge.imported_tip",{},"Imported from the community store. Used for matching only, not counted in stats."))}" style="background:var(--info-color,#2196f3);color:#fff">\u{1F4E5} ${this._t("status.imported",{},"Imported")}</span>`:"",k=(this._profileAdvisories||[]).find(R=>R&&R.profile===t.name&&R.code==="unmatchable"),x=k?`<span class="wd-badge" style="color:var(--error-color,#f44336);background:rgba(244,67,54,.12)" title="${d(this._t(k.message_key,k.message_params,k.message))}">\u26A0 ${this._t("badge.unmatchable",{},"can't be matched")}</span>`:"",b=(this._profileTerminal||{})[t.name];let v="";if(b&&b.quiet_before_s>0&&b.seen_in>=2){const R=Math.max(1,Math.round(b.quiet_before_s/60)),I=Number(b.event_watts||0).toFixed(0),P=Math.round(b.event_seconds||0),C=this._t("badge.quiet_tail_tip",{mins:R,secs:P,watts:I,seen:b.seen_in,measured:b.measured},`Near the end this program goes quiet for about ${R} min, then draws about ${I} W for ${P} s before finishing. Seen in ${b.seen_in} of ${b.measured} measured cycles: the appliance does not do it every run.`);v=`<span class="wd-badge" style="color:var(--secondary-text-color,#888)" title="${d(C)}">${this._t("badge.quiet_tail",{mins:R},`~${R}m quiet tail`)}</span>`}const S=(this._profileAdvisories||[]).find(R=>R&&R.profile===t.name&&R.code==="duration_outlier"),D=S?`<span class="wd-badge" style="color:var(--warning-color,#ff9800);background:rgba(255,152,0,.12)" title="${d(this._t(S.message_key,S.message_params,S.message))}">\u26A0 ${this._t("badge.duration_outlier",{n:(S.message_params||{}).n||1},`${(S.message_params||{}).n||1} odd-length cycle(s)`)}</span>`:"",z=[x,D,l,c,v,g,$].filter(Boolean).join(" "),O=Array.isArray(t.signature_curve)&&t.signature_curve.length>=3?`<canvas class="wd-prof-spark" data-spark-prof="${d(t.name)}" width="64" height="20" aria-label="${d(this._t("lbl.sparkline",{name:t.name},"Average power curve"))}"></canvas>`:"";return`
      <div class="wd-prof-wrap">
        ...
        ${b}
        <div class="wd-profiles-grid">${$}</div>
      </div>`}).join(""),o=this._profiles.filter(g=>!n.has(g.name)),i=o.map(g=>this._profileCardHtml(g)).join(""),l=this._profiles.length===0&&!(this._cycles||[]).length&&!(this._refCycles||[]).length,_=(e&&this._onlineEnabled()&&l?`
    ...
    `}_htmlProfileGroupModal(t){const e=this._busy.has("pg-save"),s=this._profileEnvCache||{},r=p=>it[Math.max(0,this._profiles.findIndex(f=>f.name===p))%it.length],n=t.members||[],a=this._profiles.map(p=>{const f=n.includes(p.name),g=p.avg_duration?`~${Math.round(p.avg_duration/60)}m`:"",$=p.avg_energy!=null?` \xB7 ${yt(p.avg_energy)}`:"",k=f?`<span style="display:inline-block;width:10px;height:10px;border-radius:2px;background:${r(p.name)};margin:0 2px"></span>`:"";return`<label class="wd-rev-tag"><input type="checkbox" class="wd-pg-mem" value="${d(p.name)}" ${f?"checked":""}> ${k}${d(p.name)} <span style="color:var(--secondary-text-color);font-size:.85em">${g}${$}</span></label>`}).join(""),o=n.filter(p=>s[p]&&(s[p].avg||[]).length),i=o.length?`<div class="wd-leg">${o.map(p=>`<span class="wd-leg-i"><span class="wd-leg-sw" style="background:${r(p)}"></span> ${d(p)}</span>`).join("")}</div>`:"",l=o.length?`<div class="wd-canvas-wrap" style="margin-top:8px"><canvas id="wd-pgroup-canvas" role="img" aria-label="${d(this._t("lbl.aria_envelope_chart",{},"Profile power envelope chart"))}" style="height:150px"></canvas></div>${i}`:`<p class="wd-info">${this._t("msg.group_preview_hint",{},"Tick 2+ members to preview and compare their power curves.")}</p>`,c=((this._profileGroups||{}).groups||[]).find(p=>p.name===t.orig),_=c&&c.cohesion!=null?`<span class="wd-badge" style="color:${c.cohesive?"var(--success-color,#4caf50)":"var(--warning-color,#ff9800)"};background:${c.cohesive?"rgba(76,175,80,.14)":"rgba(255,152,0,.14)"}">${c.cohesive?this._t("lbl.cohesion_good",{pct:Math.round(c.cohesion*100)},"cohesion "+Math.round(c.cohesion*100)+"%"):this._t("lbl.cohesion_low",{pct:Math.round(c.cohesion*100)},"\u26A0 low cohesion "+Math.round(c.cohesion*100)+"%")}</span>`:"";return`<h2>${t.orig?this._t("modal.edit_group",{},"Edit profile group"):this._t("modal.new_group",{},"New profile group")}</h2>
      ...
      </div>`}_drawGroupCanvas(){const t=this._modal;if(!t||t.type!=="profile-group")return;const e=this._profileEnvCache||{},s=a=>it[Math.max(0,this._profiles.findIndex(o=>o.name===a))%it.length];let r=0;const n=(t.members||[]).filter(a=>e[a]&&(e[a].avg||[]).length).map(a=>{const o=e[a],i=o.avg[o.avg.length-1];return r=Math.max(r,o.target_duration||(i?i[0]:0)),{points:o.avg,stroke:s(a),width:2,alpha:.9,name:a}});n.length&&this._drawCurves("wd-pgroup-canvas",{series:n,xMax:r})}_settingsLevel(){return this._pref("settings_level","basic")==="advanced"?"advanced":"basic"}_settingFieldVisible(t){return this._settingsLevel()==="advanced"||!!t.basic}_secHasBasicFields(t){return(t.fields||(t.groups||[]).flatMap(s=>s.fields||[])).some(s=>s.basic)}_htmlSettings(){const t=this._editedOpts();if(!Object.keys(t).length)return`<div class="wd-empty"><div class="wd-icon">\u2699\uFE0F</div>${this._t("msg.loading_settings",{},"Loading settings\u2026")}</div>`;const e=this._suggestionsError?`<div class="wd-error-state"><span>${this._t("msg.fetch_error",{},"Failed to load data.")}</span><button class="wd-btn" type="button" data-action="retry-suggestions">${this._t("btn.retry",{},"Retry")}</button></div>`:"",r=this._settingsLevel()==="basic",n=new Set((this._suggestions||[]).map(Y=>Y.key)),a=this._mlSugKeys(t),o=new Set([...n,...a]),i=Y=>(Y.fields||(Y.groups||[]).flatMap(J=>J.fields||[])).some(J=>o.has(J.key)),l=this._conflictKeysFromOpts(),c=Y=>(Y.fields||(Y.groups||[]).flatMap(J=>J.fields||[])).some(J=>l.has(J.key)),_=this._opts&&this._opts.device_type||"",p=xt.filter(Y=>!(Y.id==="ml_training"||_&&Y.notDeviceTypes&&Y.notDeviceTypes.includes(_)||_&&Y.onlyDeviceTypes&&!Y.onlyDeviceTypes.includes(_)||r&&!this._secHasBasicFields(Y))),f=(p.find(Y=>Y.id===this._settingsSec)||p[0]||{}).id,g=p.map(Y=>{const dt=i(Y),J=c(Y);return`<button class="wd-sec-btn ${f===Y.id?"active":""}" data-sec="${Y.id}">${d(this._t("section."+Y.id+".label",{},Y.label))}${J?'<span class="wd-sec-conf-dot"></span>':dt?'<span class="wd-sec-sug-dot"></span>':""}</button>`}).join(""),$=`<label class="wd-mode-switch" title="${d(this._t("lbl.settings_detail_level",{},"Settings detail level"))}">
    ...
    </label>`,k=r?`<p class="wd-info" style="margin:0 0 10px;font-size:.82em">${this._t("msg.settings_basic_note",{},"Showing essential settings. Switch to Advanced for the full list.")}</p>`:"",x=this._busy.has("save-settings"),b=l.size,v=b!==1?"s":"",S=b?`
      ...
      </div>`:"",D=this._suggestions.length,z=[...a].filter(Y=>!n.has(Y)).length,O=D+z,R=this._settingsSugOnly&&!this._settingsSearch,I=D?`<button class="wd-btn wd-btn-sm wd-btn-primary" data-action="sug-apply-all">${this._t("btn.apply_all",{},"Apply all")}</button>`:"",P=D?`<button class="wd-btn wd-btn-sm wd-btn-secondary" data-action="sug-dismiss">${this._t("btn.dismiss",{},"Dismiss")}</button>`:"",C=O?R?`
      <div class="wd-sug-banner">
        <span>\u{1F4A1} ${this._t("msg.showing_suggestions",{count:O},`Showing ${O} setting${O>1?"s":""} with suggestions.`)} <span style="text-decoration:underline;cursor:pointer" data-action="sug-show-all">${this._t("msg.show_all_settings",{},"Show all settings")}</span>.</span>
        ${I}
      </div>`:`
      <div class="wd-sug-banner">
        <span>\u{1F4A1} ${this._t("msg.tuning_suggestions_available",{count:O},`${O} tuning suggestion${O>1?"s":""} available from observed cycles. They appear beside the relevant fields.`)}</span>
        <button class="wd-btn wd-btn-sm wd-btn-secondary" data-action="goto-suggestions">${this._t("btn.show_only",{},"Show only")}</button>
        ${I}
        ${P}
      </div>`:"",A=this._busy.has("sug-analyze"),F=`<button class="wd-btn wd-btn-secondary wd-btn-sm" data-action="sug-analyze" ${A?"disabled":""} title="${d(this._t("btn.run_analysis_tip",{},"Analyze your recorded cycles now and refresh tuning suggestions"))}">${A?'<span class="wd-spin"></span> '+this._t("status.analyzing",{},"Analyzing\u2026"):this._t("btn.run_analysis",{},"\u{1F50D} Run suggestion analysis")}</button>`,U=this._settingsSearch||"",st=U.trim().toLowerCase(),tt=`<input type="text" id="wd-settings-search" class="wd-filter-input" placeholder="${this._t("msg.search_placeholder",{},"Search settings\u2026")}" value="${d(U)}" autocomplete="off" style="flex:0 0 auto;width:200px;max-width:40%">`,X=st?this._htmlSettingsSearch(t,st):R?this._htmlSettingsSugOnly(t):this._htmlSettingsSection(t),Q=(this._lockedSuggestions||[]).length,vt=Q?`
    ...
    `}_htmlSettingsHistory(){const t=this._settingsChangelog||[];if(!t.length)return"";const e=this._settingsHistoryOpen,s=this._canEdit(),r=t.slice(0,100).map(n=>{const a=s?`<button class="wd-btn wd-btn-secondary wd-btn-xs" data-action="settings-revert-key" data-key="${d(n.key)}" data-val="${d(JSON.stringify(n.old))}" title="${d(this._t("btn.revert_to_previous",{},"Restore this setting to its previous value"))}">${this._t("btn.revert",{},"Revert")}</button>`:"";return`<tr>
        ...
        <td>${d(At(n.old))} \u2192 ${d(At(n.new))}</td>
        <td class="wd-tc-date">${rt(n.timestamp)}</td>
      ...
      <div class="wd-combo-row">
        ...
        <div class="wd-field-hint">${d(this._t("msg.pick_brand_first",{},"Pick an appliance brand first."))}</div></div>`;this._ensureCatalogEntry(),this._catalog.forBrand!==o&&(this._catalog.forBrand=o,this._catalog.devices=void 0);const i=Array.isArray(this._catalog.devices)?this._catalog.devices:[];this._entityListCache=this._entityListCache||{},this._entityListCache[t]=i.map(g=>g.model).filter(Boolean);const l=this._catalogEntryFor(s,"device"),c=this._statusTag(l),_=this._catalog.devices===null?` <span class="wd-info" style="font-size:.85em">${this._t("msg.loading",{},"Loading\u2026")}</span>`:"";let p="";if(l){const g=[],$=ke(l.manualUrl);$&&g.push(`<a href="${d($)}" target="_blank" rel="noopener noreferrer nofollow">${this._t("link.manual",{},"Manual \u2197")}</a>`),(this._constants&&this._constants.storePrefs||{}).show_contributor!==!1&&g.push(this._t("store.contributed_by",{name:d(l.createdByName||this._t("lbl.anonymous",{},"Anonymous"))},`by ${d(l.createdByName||"Anonymous")}`));const k=!!(this._storeStatus&&this._storeStatus.connected);let x="";if(k){const b=[1,2,3,4,5].map(v=>`<button type="button" class="wd-star-btn" data-action="store-rate-device" data-device-id="${d(l.id)}" data-rating="${v}" aria-label="${this._t("lbl.rate_n",{n:v},`${v} stars`)}">\u2605</button>`).join("");x=`<button type="button" class="wd-btn wd-btn-secondary wd-btn-sm" data-action="store-confirm-device" data-device-id="${d(l.id)}" ${l.status!=="pending"?"disabled":""}>${l.status==="pending"?this._t("btn.confirm_appliance",{},"Confirm this appliance"):this._t("btn.confirmed",{},"Confirmed")}</button>
          <span class="wd-star-row" title="${d(this._t("lbl.rate_quality",{},"Rate quality"))}">${b}</span>`}else x=`<span class="wd-info" style="font-size:.85em">${this._t("msg.connect_to_confirm",{},"Connect in the settings gear to confirm or rate.")}</span>`;p=`<div class="wd-store-picker-detail">${g.join(" \xB7 ")}<div class="wd-store-picker-actions">${x}</div></div>`}const f=this._onlineEnabled()&&this._storeDeviceDeclared()?`<div class="wd-store-actions">
    ...
    </div>`:"";return`<div class="wd-field"><label>${d(r)} ${n?lt(n):""}${c}${_}</label>
      <div class="wd-combo-row">
        <div class="wd-combo">
      ...
      ${p}${f}</div>`}_storeApplianceType(){return{washing_machine:"washer",washer:"washer",dryer:"dryer",dishwasher:"dishwasher",washer_dryer:"washer_dryer"}[this._opts.device_type]||""}_storeDeviceDeclared(){return!!((this._opts.store_brand||"").trim()&&(this._opts.store_model||"").trim())}_shareableByProgram(){const t=new Map;for(const s of this._shareableCycles||[]){const r=(s.profile_name||"").trim();r&&(t.has(r)||t.set(r,[]),t.get(r).push(s))}const e=Array.from(t.entries()).map(([s,r])=>({program:s,cycles:r})).sort((s,r)=>s.program.localeCompare(r.program));for(const s of this._shareAllPrograms||[])t.has(s)||e.push({program:s,cycles:[],noCycles:!0});return e}_editedOpts(){return Object.assign({},this._opts,this._pendingSettings)}_catalogEntryKey(){const t=this._editedOpts();return[(t.store_brand||"").trim().toLowerCase(),(t.store_model||"").trim().toLowerCase(),t.device_type||""].join("|")}_ensureCatalogEntry(){const t=this._catalogEntryKey(),e=this._catalogEntry;e&&e.key===t||(this._catalogEntry={key:t,brand:null,device:null,deviceId:null,loading:!0},this._loadCatalogEntry(t))}_catalogEntryFor(t,e){const s=this._catalogEntry;if(!s||s.key!==this._catalogEntryKey())return null;const r=e==="brand"?s.brand:s.device;if(!r)return null;const n=e==="brand"?r.brand:r.model;return String(n||"").toLowerCase()===String(t||"").trim().toLowerCase()?r:null}async _loadCatalogEntry(t){const e=this._devices[this._selIdx],s=this._editedOpts(),r=(s.store_brand||"").trim(),n=(s.store_model||"").trim();if(!e||!this._onlineEnabled()||!r||!n){this._catalogEntry&&this._catalogEntry.key===t&&(this._catalogEntry.loading=!1);return}let a=null;try{a=await this._ws({type:`${w}/store_get_catalog_entry`,entry_id:e.entry_id,brand:r,model:n,appliance_type:s.device_type||""})}catch{}!this._catalogEntry||this._catalogEntry.key!==t||(this._catalogEntry={key:t,loading:!1,brand:a&&a.brand||null,device:a&&a.device||null,deviceId:a&&a.device_id||null},this._isActiveEntry(e.entry_id)&&this._renderPreservingFormEdits())}_refreshComboAfterLoad(t,e,s=!0){const r=this.shadowRoot;for(const n of Array.isArray(t)?t:[t]){const a=r&&r.getElementById(n);if(a&&r.activeElement===a){if(s){a._wdSyntheticInput=!0;try{a.dispatchEvent(new Event("input",{bubbles:!0}))}finally{a._wdSyntheticInput=!1}}return}}this._isActiveEntry(e)&&this._render()}_ensureCatalogList(t,e,s=!1){if(this._onlineEnabled()){if(t==="store_brand"){this._ensureBrandCandidates(e);return}if(t==="store_search"){this._ensureBrandCandidates(e),s&&String(e||"").trim().length>=2&&this._ensureStoreTypeDevices();return}if(t==="store_model"){const r=String(this._editedOpts().store_brand||"").trim();if(!r)return;(this._catalog.forBrand!==r||this._catalog.devices===void 0)&&(this._catalog.forBrand=r,this._catalog.devices=null,this._loadCatalogDevices(r))}}}_ensureBrandCandidates(t){const e=String(t||"").trim().toLowerCase();if(this._catalog.brandsFull)return;if(!e){clearTimeout(this._brandSearchTimer),this._brandSearchTimer=null,this._catalog.brands===void 0&&(this._catalog.brands=null,this._loadCatalogBrands(""));return}(this._catalog.brandPrefixes||(this._catalog.brandPrefixes=[])).some(r=>e.startsWith(r))||(clearTimeout(this._brandSearchTimer),this._brandSearchTimer=setTimeout(()=>this._loadCatalogBrands(e),250))}_mergeBrandCandidates(t){const e=new Map((Array.isArray(this._catalog.brands)?this._catalog.brands:[]).map(s=>[String(s.id!=null?s.id:s.brand),s]));for(const s of t||[])e.set(String(s.id!=null?s.id:s.brand),s);this._catalog.brands=Array.from(e.values()).sort((s,r)=>String(s.brand||"").localeCompare(String(r.brand||"")))}async _loadCatalogBrands(t=""){this._entityListCache=this._entityListCache||{};let e=!1;const s=this._devices[this._selIdx];if(!s||!this._onlineEnabled()){this._catalog.brands=[],this._entityListCache.store_brand=[];return}if(!(t&&this._catalog.brandsFull)){this._catalog.brands===void 0&&(this._catalog.brands=null);try{const r=await this._ws({type:`${w}/store_list_brands`,entry_id:s.entry_id,query:t||null,include_pending:!this._catalog.approvedOnly}),n=r&&r.items||[];t?(this._mergeBrandCandidates(n),(this._catalog.brandPrefixes||(this._catalog.brandPrefixes=[])).push(t)):(this._catalog.brands=n,this._catalog.brandsFull=!0)}catch{Array.isArray(this._catalog.brands)||(this._catalog.brands=[]),e=!0}this._entityListCache.store_brand=(this._catalog.brands||[]).map(r=>r.brand).filter(Boolean),this._syncStoreSearchCandidates(),this._refreshComboAfterLoad(["wd-store-brand","wd-store-q"],s.entry_id,!e)}}_syncStoreSearchCandidates(){this._entityListCache=this._entityListCache||{};const t=new Map,e=[];for(const r of this._catalog.typeDevices||[]){const n=String(r&&r.model||"").trim();if(!n)continue;const a=r.brand?`${n} \xB7 ${r.brand}`:n;t.has(a)||(t.set(a,r),e.push(a))}this._catalog.modelIndex=t;const s=(this._catalog.brands||[]).map(r=>r.brand).filter(Boolean);this._entityListCache.store_search=s.concat(e)}_dropModelCandidates(){this._catalog.typeDevices=void 0,this._catalog.typeDevicesFor=null,this._catalog.modelIndex=new Map,this._entityListCache&&delete this._entityListCache.store_search}_ensureStoreTypeDevices(){const t=this._devices[this._selIdx];if(!t||!this._onlineEnabled())return;const e=this._storeApplianceType();this._catalog.typeDevicesFor===e&&this._catalog.typeDevices!==void 0||(this._catalog.typeDevicesFor=e,this._catalog.typeDevices=null,this._ws({type:`${w}/store_search_devices`,entry_id:t.entry_id,query:null,appliance_type:e,include_pending:!0}).then(s=>{this._catalog.typeDevicesFor===e&&(this._catalog.typeDevices=s&&s.items||[])}).catch(()=>{this._catalog.typeDevicesFor===e&&(this._catalog.typeDevices=[])}).finally(()=>{this._syncStoreSearchCandidates(),this._refreshComboAfterLoad(["wd-store-q"],t.entry_id,!!(this._catalog.typeDevices||[]).length)}))}async _loadCatalogDevices(t){this._entityListCache=this._entityListCache||{};const e=this._devices[this._selIdx];if(!e||!this._onlineEnabled()||!t){this._catalog.devices=[],this._entityListCache.store_model=[];return}try{const s=await this._ws({type:`${w}/store_search_devices`,entry_id:e.entry_id,query:t,appliance_type:this._storeApplianceType(),include_pending:!this._catalog.approvedOnly});this._catalog.forBrand===t&&(this._catalog.devices=s&&s.items||[])}catch{this._catalog.forBrand===t&&(this._catalog.devices=[])}this._entityListCache.store_model=(this._catalog.devices||[]).map(s=>s.model).filter(Boolean),this._refreshComboAfterLoad("wd-store-model",e.entry_id)}async _loadShareProfiles(){const t=this._devices[this._selIdx],e=this._modal;if(!t||!e||e.type!=="store-share")return;const s=(this._opts.store_brand||"").trim(),r=(this._opts.store_model||"").trim();if(!s||!r){e.profiles=[],this._modal===e&&this._render();return}try{const n=await this._ws({type:`${w}/store_get_device_profiles`,entry_id:t.entry_id,brand:s,model:r,appliance_type:this._opts.device_type||""});if(this._modal!==e)return;e.profiles=n&&n.items||[],e.deviceId=n&&n.device_id||null}catch{this._modal===e&&(e.profiles=[])}this._modal===e&&this._render()}_htmlAutomations(){const t=this._deviceAutomations||[],e=t.length?t.map(n=>`<span class="wd-auto-pill"><a class="wd-auto-pill-link" href="/config/automation/edit/${encodeURIComponent(n.id)}" target="_top" title="${this._t("hdr.automation_open",{},"Open in the automation editor")}">\u{1F517} ${d(n.name)}${n.enabled?"":' <span style="opacity:.6">(off)</span>'}</a><button type="button" class="wd-auto-pill-x" data-action="auto-delete" data-autoid="${d(n.id)}" data-autoname="${d(n.name)}" title="${this._t("hdr.automation_delete",{},"Delete this automation")}">\xD7</button></span>`).join(""):`<span class="wd-info" style="margin:0">${this._autoLoading?this._t("msg.loading",{},"Loading\u2026"):this._t("hdr.no_automations",{},"No automations reference this device yet.")}</span>`,s=Array.isArray(this._opts.notify_actions)?this._opts.notify_actions:[],r=s.length?`
    ...
    `}_htmlMlStatusSection(t,e){const s=e&&this._busy.has("ml-train-now:"+e)||t&&t.running,r=this._canEdit()?`<button class="wd-btn wd-btn-primary wd-btn-sm" data-action="ml-train-now" ${s?"disabled":""}>${s?`<span class="wd-spin"></span> ${this._t("status.training",{},"Training\u2026")}`:this._t("btn.train_now",{},"Train now")}</button>`:"";if(!t)return`<div class="wd-card"><div class="wd-card-title" style="margin:0 0 4px">${this._t("hdr.status",{},"Status")}</div><p class="wd-info" style="margin:0">${this._t("msg.loading",{},"Loading\u2026")}</p></div>`;const n=Object.keys(t.on_device_models||{}).length,a=n?`<span style="color:var(--success-color,#4caf50);font-weight:600">${this._t("ml.personalized",{},"\u25CF Personalized to this machine")}</span> <span style="color:var(--secondary-text-color)">${this._t("lbl.models_fine_tuned",{count:n},"(fine-tuned models: "+n+")")}</span>`:`<span style="color:var(--secondary-text-color)">${this._t("ml.builtin_models",{},"\u25CF Using built-in models")}</span>`,o=t.cycle_count||0,i=t.min_cycles||0,l=o>=i,c=i>0?Math.min(100,Math.round(o/i*100)):100,_=l?"var(--success-color,#4caf50)":"var(--warning-color,#ff9800)",p=Math.max(0,i-o),f=l?this._t("msg.enough_data",{current:o,min:i},`Enough data to learn from (${o}/${i} cycles).`):this._t("msg.collecting_data",{need:p,current:o,min:i},`Collecting data. Cycles still needed before fine-tuning can start: ${p} (${o}/${i}).`),g=`<div style="height:8px;border-radius:6px;background:var(--secondary-background-color);overflow:hidden;margin:8px 0"><div style="width:${c}%;height:100%;background:${_}"></div></div>`,$=t.last_trained?rt(t.last_trained):"never",k=s?`<span style="color:var(--info-color,#2196f3)"><span class="wd-spin"></span> ${this._t("status.fine_tuning",{},"fine-tuning now\u2026")}</span>`:t.enabled?this._t("lbl.auto_fine_tune_on",{hour:String(t.hour).padStart(2,"0")},`auto fine-tune on (around ${String(t.hour).padStart(2,"0")}:00)`):this._t("lbl.auto_fine_tune_off",{},"auto fine-tune off");return`<div class="wd-card">
      ...
      <p class="wd-info" style="margin:0">${f}</p>
      ${g}
      <p class="wd-info" style="margin:0">${this._t("lbl.last_checked",{},"Last checked:")} <strong>${d($)}</strong> \xB7 ${k}</p>
    </div>`}_htmlMlLearnedSection(t){if(!t)return"";const e=t.on_device_models||{},s=Object.keys(e),r=this._busy.has("ml-revert-models");let n;if(!s.length)n=`<p class="wd-info" style="margin:0">${this._t("msg.no_fine_tuned",{},"Nothing fine-tuned yet \u2014 WashData is using its built-in models.")}</p>`;else{const a=s.map(i=>{const l=e[i]||{},c=l.trained_at?rt(l.trained_at):"unknown";return`<div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid var(--divider-color)">
    ...
    </div>`:"",_=e&&r.length?`<button class="wd-btn wd-btn-sm wd-btn-primary" data-action="pg-apply-settings" title="${d(this._t("btn.pg_apply_to_settings_tip",{},"Copy the values you edited here into this device's live settings"))}">${this._t("btn.pg_publish_all",{n:r.length},"Publish "+r.length+" to integration")}</button>`:"",p=Object.keys(this._pgSuggClassic||{}).length,f=Object.keys(this._pgSuggMl||{}).length,g=p>0?`<button class="wd-btn wd-btn-sm" data-action="pg-load-suggested" title="${d(this._t("btn.pg_load_suggested_tip",{},"Stage the auto-tuner's current suggestions as Playground overrides"))}">\u2193 ${this._t("btn.pg_load_suggested",{n:p},"Load suggested ("+p+")")}</button>`:"",$=this._pgMlSuggEnabled&&f>0?`<button class="wd-btn wd-btn-sm" data-action="pg-load-calibrated" title="${d(this._t("btn.pg_load_calibrated_tip",{},"Stage ML-calibrated suggestions as Playground overrides"))}">\u2193 ${this._t("btn.pg_load_calibrated",{n:f},"Load Calibrated (ML) ("+f+")")}</button>`:"";return`<div class="wd-pg-ctrl">
      ...
      <div style="display:flex;gap:6px;align-items:flex-end;padding-bottom:2px">
        ...
        </div>`:"",c=`${i}<div class="wd-pg-canvas-wrap" style="position:relative"><canvas id="wd-pg-canvas" role="img" aria-label="${d(this._t("lbl.aria_playground_chart3",{},"Interactive cycle power graph: hover to read time/power, scroll or pinch to zoom, drag to pan"))}"></canvas>${l}</div>`,_=this._htmlPgStrip(),p=this._pgNeedsRestart?`<p class="wd-info" style="color:var(--warning-color,#ff9800);margin:4px 0">\u26A0 ${this._t("msg.pg_restart_note",{},"Restart Home Assistant to enable simulation tools.")}</p>`:"",f=`${o}${c}${_}
      ...
      ${r}
        ...
        </div>`,n=k);const C=this._pgStagedVal(p),A=C!=null&&!(v!==""&&v!==null&&v!==void 0&&this._pgSameVal(C,v)),F=this._canEdit()&&A&&this._pgIsPublishable(p)?`<button class="wd-pg-pub" data-action="pg-publish-one" data-pgkey="${d(p)}" aria-label="${d(this._t("btn.pg_publish_one",{label:b},"Publish "+b+" to the integration"))}" title="${d(this._t("btn.pg_publish_one",{label:b},"Publish "+b+" to the integration"))}">\u2191</button>`:'<span class="wd-pg-pub-slot" aria-hidden="true"></span>';return`${P}<div style="display:flex;align-items:flex-start;gap:6px;margin:0 0 6px 11px">
        <div style="flex:1;min-width:0">
          <div style="font-size:.82em;font-weight:600;margin-bottom:1px">${d(b)}${z?` <span style="color:${R};font-size:.85em" title="${d(this._t("lbl.pg_drag_hint",{},"Drag line on graph"))}">\u2195</span>`:""}</div>
          ${$?`<div style="font-size:.72em;color:var(--secondary-text-color);line-height:1.3">${d(this._t("pg_desc."+p,{},$))}</div>`:""}
        </div>
        <div style="display:flex;align-items:center;gap:4px;flex-shrink:0">
          ${D?`<input class="wd-pg-param-chk" type="checkbox" data-pgkey="${d(p)}" data-pgtype="bool" aria-label="${d(b)}" ${S?"checked":""} style="width:18px;height:18px;margin:1px 27px 0 27px">`:`<input class="wd-pg-param-inp" type="text" inputmode="decimal" data-pgkey="${d(p)}" value="${S!==""?d(String(S)):""}" placeholder="${v!==""?d(String(v)):""}" aria-label="${d(b)}" style="width:72px">`}
          ${O?`<span style="font-size:.75em;color:var(--secondary-text-color);min-width:14px">${d(O)}</span>`:""}
          ${F}
        </div>
      ...
      </div>`}const i=e.summary||{},l=`<div style="font-size:.82em;color:var(--secondary-text-color);margin-bottom:8px">${this._t("msg.pg_history_summary",{detected:i.detected,correct:i.match_correct,total:i.cycles},`${i.detected}/${i.cycles} detected \xB7 ${i.match_correct} matched correctly`)}</div>`,c=g=>(this._cycles||[]).find($=>$.id===g),_={};(e.baseline_rows||[]).forEach(g=>{_[g.cycle_id]=g});const p=e.rows.map(g=>{const $=c(g.cycle_id),k=$&&$.start_time?rt($.start_time):(g.cycle_id||"").slice(0,8),x=g.match_correct===!0?"\u2713":g.match_correct===!1?"\u2717":"\u2014",b=g.match_correct===!0?"var(--success-color,#4caf50)":g.match_correct===!1?"var(--error-color,#f44336)":"var(--secondary-text-color)",v=g.matched_profile||this._t("lbl.unlabelled",{},"Unlabelled"),S=g.duration_s?Math.round(g.duration_s/60)+"m":"\u2014",D=g.overrun_ratio!=null?` (${Math.round(g.overrun_ratio*100)}%)`:"",z=g.alerts&&g.alerts.length?` <span title="${d(g.alerts.join(", "))}" style="color:var(--warning-color,#ff9800)">\u26A0</span>`:"",O=_[g.cycle_id];let R="";return O&&(O.match_correct!==!0&&g.match_correct===!0?R=`<span style="color:var(--success-color,#4caf50)">\u25B2 ${this._t("lbl.pg_fixed",{},"fixed")}</span>`:O.match_correct===!0&&g.match_correct!==!0?R=`<span style="color:var(--error-color,#f44336)">\u25BC ${this._t("lbl.pg_broke",{},"broke")}</span>`:String(O.termination_reason)!==String(g.termination_reason)&&(R=`<span style="color:var(--warning-color,#ff9800)">${d(String(O.termination_reason||"\u2014"))}\u2192${d(String(g.termination_reason||"\u2014"))}</span>`)),`<tr class="wd-pg-hrow${g.cycle_id===this._pgCycleId?" selected":""}" data-action="pg-open-cycle" data-cid="${d(g.cycle_id)}" title="${d(this._t("msg.pg_row_load_hint",{},"Load this cycle in the graph above"))}">
        <td>${d(k)}</td>
        <td><span style="color:${b};font-weight:700">${x}</span> ${d(v)}</td>
        <td>${d(String(g.termination_reason||"\u2014"))}</td>
        <td style="font-variant-numeric:tabular-nums">${S}${D}${z}</td>
        ${e.diff?`<td>${R}</td>`:""}
      </tr>`}).join(""),f=`<table class="wd-pg-htable"><thead><tr>
    ...
    </tr></thead><tbody>${p}</tbody></table>`;return`${a}${r}${this._htmlPgRecentRuns("pg_history")}${o}${l}${f}`}_htmlPgBatchBar(){const t=this._pgBatchProgress;return t?`<div class="wd-pg-batchrow">
    ...
    </div>`,o=t?this._htmlPgBatchBar():"";return`${n}${a}${this._htmlPgRecentRuns("pg_sweep")}${o}${this._htmlPgSweepResult()}`}_htmlPgSweepResult(){const t=this._pgSweepNew;if(!t||!Array.isArray(t.points)||!t.points.length)return"";const e=this._pgSweepObjectives().find(p=>p[0]===t.objective),s=e?e[2]:!1,r=t.points.filter(p=>p.metric!=null).map(p=>p.metric);if(!r.length)return`<div class="wd-empty" style="padding:16px">${this._t("msg.pg_sweep_no_metric",{},"Not enough data to score this objective.")}</div>`;const n=Math.min(...r),a=Math.max(...r),o=s?Math.min(...r):Math.max(...r),i=(t.points.find(p=>p.metric===o)||{}).value,l=p=>t.objective==="median_overrun"?Math.round(p*100)+"% off":Math.round(p*100)+"%",c=t.points.map(p=>{const f=a>n&&p.metric!=null?(p.metric-n)/(a-n):p.metric!=null?1:0,g=p.metric===o,$=t.current_value!=null&&Math.abs(p.value-t.current_value)<1e-6,k=g?"var(--success-color,#4caf50)":"var(--primary-color)";return`<div style="display:flex;align-items:center;gap:8px;margin:3px 0;font-size:.82em">
        <span style="flex:0 0 74px;text-align:right;font-variant-numeric:tabular-nums">${d(String(p.value))}${$?" \u25C0":""}</span>
        <div style="flex:1;height:16px;border-radius:4px;background:var(--secondary-background-color);overflow:hidden"><div style="height:100%;width:${Math.round((p.metric!=null?.15+.85*f:0)*100)}%;background:${k};border-radius:4px"></div></div>
      ...
      ${c}
      <div style="margin-top:8px">${_}</div>`}async _pgRunSweep2(){const t=this._devices[this._selIdx];if(!t)return;const e=parseFloat(this._pgSweepFrom),s=parseFloat(this._pgSweepTo);if(isNaN(e)||isNaN(s)||e===s){this._showToast(this._t("msg.toast_name_required",{},"Set a valid From/To range."),"error");return}const r=Math.max(2,Math.min(12,this._pgSweepSteps||5)),n=Array.from({length:r},(l,c)=>+(e+(s-e)*c/(r-1)).toFixed(3)),a=this._pgSweepParam||"off_delay",o=this._pgSweepObjective||"match_accuracy",i={type:`${w}/start_playground_sweep`,entry_id:t.entry_id,param:a,values:n,objective:o};this._pgBatchProgress={done:0,total:n.length},this._busy.add("pg-sweep"),this._render();try{const l=await this._ws(i);if(this._pgSweepTaskId=l&&l.task_id,this._pgNeedsRestart=!1,!this._pgSweepTaskId)throw new Error("no task id");this._addProvisionalTask(this._pgSweepTaskId,"pg_sweep",t.entry_id,n.length),this._tasksSubscribed||this._pgPollTask(this._pgSweepTaskId)}catch(l){this._busy.delete("pg-sweep"),this._pgBatchProgress=null,this._pgIsUnknownCmd(l)?this._pgNeedsRestart=!0:this._showToast(this._t("msg.toast_error",{error:l.message||l},"Error: "+(l.message||l)),"error"),this._render()}}async _pgApplyToSettings(){const t=this._devices[this._selIdx];if(!t||!this._canEdit())return;const e={};for(const n of this._pgChangedKeys())this._pgIsPublishable(n)&&(e[n]=this._pgStagedVal(n));const s=Object.keys(e);if(!s.length)return;const r=s.map(n=>this._t("setting."+n+".label",{},n)).join(", ");confirm(this._t("msg.pg_apply_settings_confirm",{n:s.length,list:r},`Save these ${s.length} setting(s) to this device? ${r}`))&&(await this._busyRun("pg-apply-settings",async()=>{try{if(await this._ws({type:`${w}/set_options`,entry_id:t.entry_id,options:e}),!this._isActiveEntry(t.entry_id))return;this._opts={...this._opts,...e},this._pgEffective&&(this._pgEffective={...this._pgEffective,...e});for(const n of s)this._pgClearStaged(n);this._showToast(this._t("toast.settings_saved",{},"Settings saved; integration reloading"))}catch(n){this._showToast(this._t("msg.toast_save_failed",{error:n.message||n},"Save failed: "+(n.message||n)),"error")}}),this._render())}async _pgApplySweepValue(t){const e=this._devices[this._selIdx];if(!e||!this._canEdit()||t==null)return;const s=this._pgSweepParam,r=this._t("setting."+s+".label",{},s);confirm(this._t("msg.pg_apply_confirm",{label:r,value:t},"Apply best value: "+r+" = "+t+"?"))&&await this._busyRun("pg-sweep-apply",async()=>{try{await this._ws({type:`${w}/set_options`,entry_id:e.entry_id,options:{[s]:+t}}),this._opts={...this._opts,[s]:+t},s==="start_threshold_w"?this._pgThreshStart=+t:s==="stop_threshold_w"?this._pgThreshStop=+t:this._pgParamOverrides[s]=+t,this._showToast(this._t("toast.settings_saved",{},"Settings saved; integration reloading")),this._render()}catch(n){this._showToast(this._t("msg.toast_save_failed",{error:n.message||n},"Save failed: "+(n.message||n)),"error")}})}_htmlPgStrip(){return`<div class="wd-pg-strip" id="wd-pg-strip">
      ...
      <span style="font-size:.75em;color:var(--secondary-text-color);text-transform:uppercase">${this._t("lbl.phase",{},"Phase")} <span id="wd-pg-phase" style="color:var(--primary-text-color,inherit)">\u2014</span></span>
    </div>`}_htmlPgAnalysis(){const t=this._pgDtwData,e=(i,l,c,_,p)=>{const f=c&&l!=null?Math.max(0,Math.min(1,l/c)):l!=null?Math.max(0,Math.min(1,l)):0;return`<div class="wd-pg-score-bar-row">
        <span class="wd-pg-score-bar-lbl">${d(i)}</span>
        <div class="wd-pg-score-bar-track"><div class="wd-pg-score-bar-fill" style="width:${Math.round(f*100)}%;background:${_}"></div></div>
        <span class="wd-pg-score-bar-val">${p!=null?d(String(p)):"\u2014"}</span>
      </div>`};let s="";if(t&&(t.stage2||t.stage4)){const i=t.stage2||{},l=t.stage4||{},c=t.dtw||{},_=l.final_score??c.blended_score??i.score,p=this._pgDetail&&this._pgDetail.outcome&&this._pgDetail.outcome.confidence,f=p??_;let g="\u2014",$="var(--secondary-text-color)";f!=null&&(f>=.7?(g="\u2705 "+this._t("lbl.pg_strong_match",{},"Strong match"),$="var(--success-color, #4caf50)"):f>=.4?(g="\u26A0 "+this._t("lbl.pg_weak_match",{},"Weak match"),$="var(--warning-color, #ff9800)"):(g="\u274C "+this._t("lbl.pg_poor_match",{},"Poor match"),$="var(--error-color, #f44336)"));const k=t.profile_name||this._pgProfileName||"\u2014";s+=`<div style="font-weight:700;font-size:.9em;color:${$};margin-bottom:8px">${g}</div>`,k!=="\u2014"&&(s+=`<div style="font-size:.82em;color:var(--secondary-text-color);margin-bottom:6px">${d(k)} \xB7 ${d(this._t("lbl.score",{},"score"))} ${_!=null?_.toFixed(3):"\u2014"}</div>`),s+=e(this._t("lbl.correlation",{},"Correlation"),i.correlation,1,"#42a5f5",i.correlation!=null?i.correlation.toFixed(2):null),c.blended_score!=null&&(s+=e(this._t("lbl.pg_dtw",{},"DTW"),c.blended_score,1,"#ab47bc",c.blended_score.toFixed(2))),l.duration_agreement!=null&&(s+=e(this._t("lbl.duration",{},"Duration"),l.duration_agreement,1,"#66bb6a",l.duration_agreement.toFixed(2))),l.energy_agreement!=null&&(s+=e(this._t("lbl.energy",{},"Energy"),l.energy_agreement,1,"#ffa726",l.energy_agreement.toFixed(2))),s+='<div style="height:1px;background:var(--divider-color,rgba(127,127,127,.2));margin:8px 0"></div>'}else t||(s+=`<p class="wd-info" style="margin:0 0 8px">${this._t("msg.pg_analysis_empty2",{},"Press Run to see match analysis.")}</p>`);const r=this._pgDetail&&this._pgDetail.outcome,n=r&&r.matched_profile||t&&t.profile_name||this._pgProfileName||(this._cycles||[]).find(i=>i.id===this._pgCycleId)?.profile_name||"",a=r&&r.confidence;if(n&&a!=null){const i=Math.round(Math.max(0,Math.min(1,a))*100);s+=`<div class="wd-subhead" style="margin:0 0 4px">${d(this._t("lbl.pg_match_confidence",{},"Match confidence"))}</div>`,s+=`<div class="wd-pg-cand-row">
        ...
        <span class="wd-pg-cand-pct">${i}%</span>
      </div>`}return`<div>${s||`<p class="wd-info" style="margin:0">${this._t("msg.pg_analysis_hint2",{},"Pick a cycle and press Run to load match analysis.")}</p>`}</div>`}async _pgLoad(){const t=this._devices[this._selIdx];if(!t||this._pgLoading)return;const e=this._pgCycleId||this._cycles?.[0]?.id||"";if(!e)return;this._pgCycleId=e,this._pgLoading=!0,this._pgView=null,this._pgHoverT=null,this._pgPowerPts=null,this._pgDtwData=null,this._pgEnvData=null,this._pgDetail=null;const s=++this._pgLoadSeq;this._render();try{const r=await this._ws({type:`${w}/get_cycle_power_data`,entry_id:t.entry_id,cycle_id:e}),n=r.samples||[],a=[];for(const l of n){if(!Array.isArray(l)||l.length<2)continue;const c=+l[0],_=+l[1];!isNaN(c)&&!isNaN(_)&&a.push({t:c,w:_})}if(this._pgPowerPts=a.length?a:null,typeof r.full_duration_s=="number"&&r.full_duration_s>0){const l=(this._cycles||[]).find(c=>c.id===e);l&&(l._pg_duration=r.full_duration_s)}const o=this._pgProfileName||(this._cycles||[]).find(l=>l.id===e)?.profile_name||"";if(a.length)try{const l={type:`${w}/get_dtw_debug`,entry_id:t.entry_id,cycle_id:e};o&&(l.profile_name=o),this._pgDtwData=await this._ws(l),this._pgNeedsRestart=!1}catch(l){this._pgIsUnknownCmd(l)&&(this._pgNeedsRestart=!0),this._pgDtwData=null}const i=this._pgDtwData?.profile_name||o;if(i)try{const l=await this._ws({type:`${w}/get_profile_envelope`,entry_id:t.entry_id,profile_name:i});this._pgEnvData=l.envelope||null}catch{this._pgEnvData=null}if(s!==this._pgLoadSeq)return;await this._pgLoadDetail(t.entry_id,e)}catch(r){this._showToast(this._t("msg.toast_error",{error:r.message||r},"Error: "+(r.message||r)),"error")}s===this._pgLoadSeq&&(this._pgLoading=!1,this._render(),requestAnimationFrame(()=>{this._pgDrawCanvas(),this._pgUpdateStripAt(null)}),this._pgNeedsRestart&&this._tab==="playground"?(this._pgRestartRetries||0)<5&&!this._pgRestartRetryTimer&&(this._pgRestartRetries=(this._pgRestartRetries||0)+1,this._pgRestartRetryTimer=setTimeout(()=>{this._pgRestartRetryTimer=null,this._pgNeedsRestart&&this._tab==="playground"&&this._pgLoad()},3e3)):this._pgRestartRetries=0)}_pgCancelRun(){this._pgLoadSeq++,this._pgLoading=!1,this._render()}_pgSelectCycle(t){t&&(this._pgCycleId=t,this._pgProfileName="",this._pgView=null,this._pgHoverT=null,this._pgLoading=!1,this._pgLoad(),requestAnimationFrame(()=>{const e=this.shadowRoot&&this.shadowRoot.getElementById("wd-pg-canvas");e&&typeof e.scrollIntoView=="function"&&e.scrollIntoView({behavior:"smooth",block:"center"})}))}async _pgLoadDetail(t,e){const s={...this._pgParamOverrides};this._pgThreshStart!=null&&(s.start_threshold_w=this._pgThreshStart),this._pgThreshStop!=null&&(s.stop_threshold_w=this._pgThreshStop);try{const r=await this._ws({type:`${w}/start_playground_cycle_detail`,entry_id:t,cycle_id:e,settings_override:s,stress_tail:this._pgStressTail,stress_idle_w:this._pgStressIdleW!=null?parseFloat(this._pgStressIdleW):null}),n=r&&r.task_id;if(!n)throw new Error("no task id");this._addProvisionalTask(n,"pg_detail",t,0);const a=await new Promise(o=>{this._taskCallbacks[n]=async l=>{if(l.state==="done"||l.state==="cancelled")try{const c=await this._ws({type:`${w}/get_task_result`,task_id:l.id});o(c&&c.result)}catch{o(null)}else o(null)};const i=this._tasks[n];i&&i.state!=="running"?this._settleTaskCallback(i):this._tasksSubscribed||this._pollTaskGeneric(n)});if(!this._isActiveEntry(t))return;this._pgDetail=a&&!a.error?a:null,this._pgNeedsRestart=!1}catch(r){this._pgIsUnknownCmd(r)&&(this._pgNeedsRestart=!0),this._pgDetail=null}}_pgRerunDetail(){const t=this._devices[this._selIdx];!t||!this._pgCycleId||(clearTimeout(this._pgDetailDebounceTimer),this._pgDetailDebounceTimer=setTimeout(async()=>{this._pgDetailBusy=!0,this._render(),await this._pgLoadDetail(t.entry_id,this._pgCycleId),this._pgDetailBusy=!1,this._render(),requestAnimationFrame(()=>this._pgDrawCanvas())},220))}_pgMapState(t){return t==="running"||t==="paused"?"running":t==="ending"?"ending":t==="starting"?"detecting":t==="anti_wrinkle"?"anti_wrinkle":"idle"}_pgSeriesAt(t){const e=this._pgDetail&&this._pgDetail.series;if(!e||!e.length)return null;let s=e[0];for(const r of e)if(r.t<=t)s=r;else break;return s}_pgStateSegsFromSeries(t){const e=this._pgDetail&&this._pgDetail.series;if(!e||!e.length)return[];const s=[];let r=null;for(const n of e){const a=this._pgMapState(n.state);!r||r.state!==a?(r={start:n.t,end:n.t,state:a},s.push(r)):r.end=n.t}return s.length&&(s[s.length-1].end=t),s}_pgDrawCanvas(){if(this._tab!=="playground")return;const t=this.shadowRoot,e=t&&t.getElementById("wd-pg-canvas");if(!e)return;const s=this._pgPowerPts,r=e.getBoundingClientRect(),n=window.devicePixelRatio||1,a=Math.max(1,Math.round(r.width*n)),o=Math.max(1,Math.round((r.height||280)*n));(e.width!==a||e.height!==o)&&(e.width=a,e.height=o);const i=e.getContext("2d"),l=getComputedStyle(this),c=(l.getPropertyValue("--primary-color")||"#03a9f4").trim(),_=(l.getPropertyValue("--divider-color")||"rgba(127,127,127,.2)").trim(),p=(l.getPropertyValue("--secondary-text-color")||"#888").trim(),f=(l.getPropertyValue("--secondary-background-color")||"#1a1a1a").trim();i.clearRect(0,0,a,o);const g=34*n,$=14*n,k=Ht*n,x=44*n,b=8*n,v=k+8*n,S=g+$+4*n,D=o-v-S;if(!s||!s.length){this._pgLoading&&(i.fillStyle=p,i.font=`${12*n}px sans-serif`,i.textAlign="center",i.textBaseline="middle",i.fillText(this._t("msg.loading",{},"Loading\u2026"),a/2,o/2));return}const z=this._pgDetail&&this._pgDetail.outcome&&this._pgDetail.outcome.stress,O=z&&z.enabled?z.synthetic_from_s:null,R=O!=null&&this._pgDetail&&this._pgDetail.series?this._pgDetail.series.filter(T=>T.t>=O):[];let I=(this._cycles||[]).find(T=>T.id===this._pgCycleId)?._pg_duration||s[s.length-1].t||1;z&&z.terminated&&z.terminated_after_s!=null&&O!=null?I=Math.max(I,O+z.terminated_after_s):R.length&&(I=Math.max(I,R[R.length-1].t));const P=Math.max(...s.map(T=>T.w),...R.map(T=>T.power||0),1),C=this._pgThreshStart??this._pgFieldVal("start_threshold_w",{})??50,A=this._pgThreshStop??this._pgFieldVal("stop_threshold_w",{})??5;let F=0,U=I;this._pgView&&this._pgView.max-this._pgView.min>1&&(F=Math.max(0,this._pgView.min),U=Math.min(I,this._pgView.max),U-F<=1&&(F=0,U=I));const st=Math.max(1e-6,U-F),tt=a-x-b,X=T=>x+(T-F)/st*tt,Q=T=>v+(1-Math.max(0,T)/P)*D;this._pgMap={vMin:F,vMax:U,totalDur:I,padLpx:x/n,plotWpx:tt/n},i.strokeStyle="rgba(127,127,127,0.12)",i.lineWidth=n,i.setLineDash([]),[.25,.5,.75,1].map(T=>Math.round(T*P/100)*100||Math.round(T*P)).forEach(T=>{const B=Q(T);B<v||B>v+D||(i.beginPath(),i.moveTo(x,B),i.lineTo(a-b,B),i.stroke(),i.fillStyle=p,i.font=`${9*n}px sans-serif`,i.textAlign="right",i.textBaseline="middle",i.fillText(T+"W",x-4*n,B))}),i.save(),i.beginPath(),i.rect(x,v,tt,D),i.clip();const Y=this._pgEnvData,dt=Y&&Array.isArray(Y.avg)?Y.avg.filter(T=>Array.isArray(T)&&T.length>=2):[];if(dt.length){const T=Math.max(...dt.map(j=>j[0]))||1,B=j=>X(j/T*I),Z=Array.isArray(Y.min)?Y.min.filter(j=>Array.isArray(j)&&j.length>=2):[],q=Array.isArray(Y.max)?Y.max.filter(j=>Array.isArray(j)&&j.length>=2):[];if(Z.length&&q.length){i.beginPath(),q.forEach((j,gt)=>gt?i.lineTo(B(j[0]),Q(j[1])):i.moveTo(B(j[0]),Q(j[1])));for(let j=Z.length-1;j>=0;j--)i.lineTo(B(Z[j][0]),Q(Z[j][1]));i.closePath(),i.fillStyle="#eda10012",i.fill()}else i.beginPath(),i.strokeStyle="#eda100",i.lineWidth=2*n,i.setLineDash([5*n,4*n]),dt.forEach((j,gt)=>gt?i.lineTo(B(j[0]),Q(j[1])):i.moveTo(B(j[0]),Q(j[1]))),i.stroke(),i.setLineDash([])}const J=this._pgDtwData,ot=J&&Array.isArray(J.profile_trace)?J.profile_trace.filter(T=>Array.isArray(T)&&T.length>=2):[],ht=J&&Array.isArray(J.cycle_trace)?J.cycle_trace.filter(T=>Array.isArray(T)&&T.length>=2):[],pt=J&&Array.isArray(J.warp_path)?J.warp_path:[];if(ht.length&&ot.length&&pt.length){const T=Math.max(...ot.map(q=>q[0]))||1,B=Math.max(...ht.map(q=>q[0]))||1,Z=Math.max(1,Math.floor(pt.length/25));i.save(),i.globalAlpha=.13,i.strokeStyle="#fff",i.lineWidth=n;for(let q=0;q<pt.length;q+=Z){const j=pt[q];if(!Array.isArray(j)||j.length<2)continue;const gt=Math.min(j[0],ht.length-1),ft=Math.min(j[1],ot.length-1),St=X(ht[gt][0]/B*I),mt=Q(ht[gt][1]),$t=X(ot[ft][0]/T*I),ut=Q(ot[ft][1]);i.beginPath(),i.moveTo(St,mt),i.lineTo($t,ut),i.stroke()}i.restore()}if(ot.length){const T=Math.max(...ot.map(B=>B[0]))||1;i.beginPath(),i.strokeStyle="#eda100",i.lineWidth=2*n,i.setLineDash([6*n,4*n]),ot.forEach((B,Z)=>{const q=X(B[0]/T*I),j=Q(B[1]);Z?i.lineTo(q,j):i.moveTo(q,j)}),i.stroke(),i.setLineDash([])}if(i.beginPath(),i.moveTo(X(0),Q(0)),s.forEach(T=>i.lineTo(X(T.t),Q(T.w))),i.lineTo(X(s[s.length-1].t),Q(0)),i.closePath(),i.fillStyle=Pt(c,.1),i.fill(),i.beginPath(),i.strokeStyle=c,i.lineWidth=2*n,s.forEach((T,B)=>B?i.lineTo(X(T.t),Q(T.w)):i.moveTo(X(T.t),Q(T.w))),i.stroke(),R.length){const T=s[s.length-1];i.beginPath(),i.moveTo(X(T.t),Q(T.w)),R.forEach(B=>i.lineTo(X(B.t),Q(B.power))),i.lineTo(X(R[R.length-1].t),Q(0)),i.lineTo(X(T.t),Q(0)),i.closePath(),i.fillStyle=Pt(c,.06),i.fill(),i.beginPath(),i.strokeStyle=c,i.lineWidth=1.5*n,i.setLineDash([4*n,4*n]),i.moveTo(X(T.t),Q(T.w)),R.forEach(B=>i.lineTo(X(B.t),Q(B.power))),i.stroke(),i.setLineDash([])}if(O!=null&&O<U){const T=X(O);i.fillStyle="rgba(192,57,43,0.07)",i.fillRect(T,v,Math.max(0,x+tt-T),D),i.save(),i.strokeStyle="rgba(192,57,43,0.5)",i.lineWidth=n,i.setLineDash([4*n,4*n]),i.beginPath(),i.moveTo(T,v),i.lineTo(T,v+D),i.stroke(),i.restore()}i.restore();const y=(T,B,Z)=>{const q=Q(T);q<v-2||q>v+D+2||(i.save(),i.strokeStyle=B,i.lineWidth=2*n,i.setLineDash([8*n,4*n]),i.beginPath(),i.moveTo(x,q),i.lineTo(a-b,q),i.stroke(),i.setLineDash([]),i.fillStyle=B,i.beginPath(),i.arc(x+(a-x-b)*.06,q,5*n,0,Math.PI*2),i.fill(),i.fillStyle=B,i.font=`bold ${9*n}px sans-serif`,i.textAlign="left",i.textBaseline="bottom",i.fillText(Z+" "+Math.round(T)+"W",x+14*n,q-2*n),i.restore())};y(+C,"#2a78d6",this._t("lbl.start",{},"Start")),y(+A,"#e34948",this._t("btn.stop",{},"Stop"));const m={idle:f,detecting:"#42a5f566",running:"#66bb6a66",ending:"#ef535066",anti_wrinkle:"#ab47bc66"},u={idle:this._t("lbl.pg_idle",{},"Idle"),detecting:this._t("lbl.pg_detecting",{},"Detecting"),running:this._t("lbl.pg_ev_running",{},"Running"),ending:this._t("lbl.pg_ev_ending",{},"Ending"),anti_wrinkle:this._t("lbl.pg_anti_wrinkle",{},"Anti-wrinkle")},E=o-g-$;i.fillStyle=f,i.fillRect(x,E,a-x-b,g),i.save(),i.beginPath(),i.rect(x,E,tt,g),i.clip(),this._pgStateSegsFromSeries(I).forEach(T=>{const B=X(T.start),Z=X(T.end);i.fillStyle=m[T.state]||_,i.fillRect(B,E,Math.max(1,Z-B),g),Z-B>50*n&&(i.fillStyle=p,i.font=`${8*n}px sans-serif`,i.textAlign="center",i.textBaseline="middle",i.fillText((u[T.state]||T.state).toUpperCase(),(B+Z)/2,E+g/2))}),i.restore();const W=(this._cycles||[]).find(T=>T.id===this._pgCycleId),H=this._pgDtwData?.profile_name||this._pgProfileName||W?.profile_name,N=H?(this._profiles||[]).find(T=>T.name===H):null,V=o-$;N&&Array.isArray(N.phases)&&N.phases.length&&(i.save(),i.beginPath(),i.rect(x,V,tt,$),i.clip(),N.phases.forEach((T,B)=>{const Z=X((T.start||0)*I),q=X((T.end||1)*I),j=B*47%360;i.fillStyle=`hsla(${j},60%,55%,0.55)`,i.fillRect(Z,V,Math.max(1,q-Z),$),q-Z>40*n&&(i.fillStyle="#fff",i.font=`${7*n}px sans-serif`,i.textAlign="center",i.textBaseline="middle",i.fillText(T.name,(Z+q)/2,V+$/2))}),i.restore()),this._pgEventHits=[];const G=this._pgDetail&&Array.isArray(this._pgDetail.events)?this._pgDetail.events.filter(T=>T.type!=="state"&&T.t>=F&&T.t<=U).slice().sort((T,B)=>T.t-B.t):[],M=11*n,K=v-k/2-2*n,et=M*2+3*n;let nt=-1/0;const at=this._pgHoverEvent;if(G.forEach(T=>{const B=this._pgEventMeta(T.type),Z=X(T.t);let q=Math.max(Z,nt+et);q=Math.max(x+M,Math.min(a-b-M,q)),nt=q;const j=at&&at.t===T.t&&at.type===T.type;i.save(),i.strokeStyle=B.color,i.globalAlpha=j?.9:.5,i.lineWidth=(j?2:1)*n,i.beginPath(),i.moveTo(q,K+M),i.lineTo(Z,v),i.stroke(),j&&(i.globalAlpha=.3,i.setLineDash([2*n,3*n]),i.beginPath(),i.moveTo(Z,v),i.lineTo(Z,o-$),i.stroke(),i.setLineDash([])),i.restore(),i.save(),i.beginPath(),i.arc(q,K,M,0,Math.PI*2),i.fillStyle=j?B.color:f||"#1a1a1a",i.fill(),i.lineWidth=(j?2:1.5)*n,i.strokeStyle=B.color,i.stroke(),i.fillStyle=j?"#fff":B.color,i.font=`${14*n}px sans-serif`,i.textAlign="center",i.textBaseline="middle",i.fillText(B.glyph,q,K+n),i.restore(),this._pgEventHits.push({cx:q/n,cy:K/n,r:M/n,type:T.type,label:B.label,detail:T.detail,t:T.t})}),at){const T=this._pgEventHits.find(B=>B.t===at.t&&B.type===at.type);if(T){const B=this._pgEventMeta(T.type),Z=wt=>`${Math.floor(wt/60)}:${String(Math.round(wt%60)).padStart(2,"0")}`,q=this._pgEventDescription(T.type),j=[{t:T.label,bold:!0,col:B.color},...q?[{t:q}]:[],...T.detail?[{t:T.detail,dim:!0}]:[],{t:`${this._t("lbl.from_start",{},"From start")} ${Z(T.t)}`,dim:!0}];i.save(),i.font=`${10*n}px sans-serif`;const gt=Math.min(220*n,a-2*x),ft=[];j.forEach(wt=>{const It=String(wt.t).split(" ");let Tt="";It.forEach(Ft=>{const Gt=Tt?Tt+" "+Ft:Ft;i.measureText(Gt).width>gt&&Tt?(ft.push({...wt,t:Tt}),Tt=Ft):Tt=Gt}),ft.push({...wt,t:Tt})});const St=14*n,mt=Math.min(gt+14*n,a-8*n),$t=ft.length*St+10*n;let kt=T.cx*n-mt/2;kt=Math.max(4*n,Math.min(a-mt-4*n,kt));const Bt=K+M+6*n;i.fillStyle=f,i.globalAlpha=.97,i.fillRect(kt,Bt,mt,$t),i.globalAlpha=1,i.strokeStyle=B.color,i.lineWidth=n,i.strokeRect(kt,Bt,mt,$t),i.textAlign="left",i.textBaseline="top",ft.forEach((wt,It)=>{i.font=`${wt.bold?"600 ":""}${10*n}px sans-serif`,i.fillStyle=wt.col||(wt.dim?p:(l.getPropertyValue("--primary-text-color")||"#ddd").trim()),i.fillText(wt.t,kt+7*n,Bt+5*n+It*St)}),i.restore()}}if(this._pgHoverT!=null&&this._pgHoverT>=F&&this._pgHoverT<=U){const T=X(this._pgHoverT);i.save(),i.strokeStyle="#e34948",i.lineWidth=1.5*n,i.setLineDash([4*n,3*n]),i.beginPath(),i.moveTo(T,v),i.lineTo(T,o-$),i.stroke(),i.setLineDash([]);const B=this._pgInterpPower(s,this._pgHoverT),Z=Q(B);i.fillStyle="#e34948",i.beginPath(),i.arc(T,Z,3.5*n,0,Math.PI*2),i.fill();const q=ut=>`${Math.floor(ut/60)}:${String(Math.round(ut%60)).padStart(2,"0")}`,j=ut=>ut>=1e3?(ut/1e3).toFixed(2)+" kW":Math.round(ut)+" W",gt=[`${this._t("lbl.from_start",{},"From start")} ${q(this._pgHoverT)}`,`${this._t("lbl.to_end",{},"To end")} ${q(Math.max(0,I-this._pgHoverT))}`,`${this._t("lbl.power",{},"Power")} ${j(B)}`];i.font=`${9.5*n}px sans-serif`;const ft=Math.max(...gt.map(ut=>i.measureText(ut).width))+12*n,St=gt.length*13*n+8*n;let mt=T+8*n;mt+ft>a-b&&(mt=T-ft-8*n);const $t=v+4*n;i.fillStyle=f,i.globalAlpha=.95,i.fillRect(mt,$t,ft,St),i.globalAlpha=1,i.strokeStyle=_,i.lineWidth=n,i.strokeRect(mt,$t,ft,St),i.fillStyle=p,i.textAlign="left",i.textBaseline="top",gt.forEach((ut,kt)=>i.fillText(ut,mt+6*n,$t+5*n+kt*13*n)),i.restore()}i.fillStyle=p,i.font=`${9*n}px sans-serif`,i.textAlign="center",i.textBaseline="top";const ct=T=>`${Math.floor(T/60)}:${String(Math.round(T%60)).padStart(2,"0")}`;for(let T=0;T<=4;T++){const B=F+st*T/4,Z=X(B);i.textAlign=T===0?"left":T===4?"right":"center",i.fillText(ct(B),Math.max(x,Math.min(a-b,Z)),E+g+1*n)}st<I-1&&(i.fillStyle=p,i.font=`${8*n}px sans-serif`,i.textAlign="right",i.textBaseline="top",i.fillText(this._t("lbl.pg_zoom_hint",{},"double-click to reset zoom"),a-b,v))}_pgEventMeta(t){const s={detected:["\u25CF","#42a5f5",this._t("lbl.pg_ev_detected",{},"Detected")],match_commit:["\u25C6","#66bb6a",this._t("lbl.pg_ev_match",{},"Match committed")],match_changed:["\u25C7","#eda100",this._t("lbl.pg_ev_match_changed",{},"Match changed")],match_ambiguous:["\u25C8","#eda100",this._t("lbl.pg_ev_ambiguous",{},"Ambiguous")],unmatched:["\u25CB","#9e9e9e",this._t("lbl.pg_ev_unmatched",{},"Unmatched")],notify_start:["\u{1F514}","#2a78d6",this._t("lbl.pg_ev_notify_start",{},"Start notification")],notify_pre_complete:["\u{1F514}","#2a78d6",this._t("lbl.pg_ev_notify_pre",{},"Almost-done notification")],notify_finish:["\u{1F514}","#2a78d6",this._t("lbl.pg_ev_notify_finish",{},"Finish notification")],notify_milestone:["\u{1F3C6}","#2a78d6",this._t("lbl.pg_ev_notify_milestone",{},"Milestone notification")],notify_held:["\u{1F319}","#7e57c2",this._t("lbl.pg_ev_notify_held",{},"Notification held (quiet hours)")],finished:["\u2713","#4caf50",this._t("lbl.pg_ev_finished",{},"Finished")]}[t]||["\u2022","var(--secondary-text-color)",t];return{glyph:s[0],color:s[1],label:s[2]}}_pgEventDescription(t){return{detected:this._t("pg_evd.detected",{},"The detector recognized a cycle had started."),match_commit:this._t("pg_evd.match_commit",{},"The matcher committed to a program with enough confidence."),match_changed:this._t("pg_evd.match_changed",{},"The leading program changed as more of the cycle was seen."),match_ambiguous:this._t("pg_evd.match_ambiguous",{},"Two programs scored close together, so the match was uncertain."),unmatched:this._t("pg_evd.unmatched",{},"No saved program fit this cycle."),notify_start:this._t("pg_evd.notify_start",{},"A start notification would be sent."),notify_pre_complete:this._t("pg_evd.notify_pre_complete",{},"An almost-done notification would be sent."),notify_finish:this._t("pg_evd.notify_finish",{},"A finish notification would be sent."),notify_milestone:this._t("pg_evd.notify_milestone",{},"A milestone notification would be sent."),notify_held:this._t("pg_evd.notify_held",{},"A notification was held back for quiet hours."),finished:this._t("pg_evd.finished",{},"The cycle reached a terminal state and ended.")}[t]||""}_pgUpdateParamInput(t,e){const s=this.shadowRoot,r=s&&s.querySelector(`input[data-pgkey="${t}"]`);r&&(r.dataset.pgtype==="bool"?r.checked=!!e:r.value=typeof e=="number"?Math.round(e):e)}_pgUpdateStripAt(t){const e=this._pgPowerPts;if(!e?.length)return;const r=(this._cycles||[]).find(I=>I.id===this._pgCycleId)?._pg_duration||e[e.length-1].t||1,n=t??r,a=this._pgInterpPower(e,n),o=this._pgSeriesAt(n),i=o?this._pgMapState(o.state):"idle",l={idle:[this._t("lbl.pg_idle",{},"Idle"),"var(--secondary-background-color)"],detecting:[this._t("lbl.pg_detecting",{},"Detecting"),"#42a5f5"],running:[this._t("lbl.pg_ev_running",{},"Running"),"#66bb6a"],ending:[this._t("lbl.pg_ev_ending",{},"Ending"),"#ef5350"],anti_wrinkle:[this._t("lbl.pg_anti_wrinkle",{},"Anti-wrinkle"),"#ab47bc"]},[c,_]=l[i]||l.idle,p=o&&o.progress!=null?Math.round(o.progress):null,f=o&&o.remaining_s!=null?o.remaining_s:null,g=o&&o.energy_wh!=null?o.energy_wh:this._pgTrapEnergy(e,n),$=o&&o.confidence!=null?Math.round(o.confidence*100)+"%":"\u2014",k=o&&o.phase?o.phase:"\u2014",x=I=>Math.floor(I/60)+":"+String(Math.round(I%60)).padStart(2,"0"),b=I=>I>=1e3?(I/1e3).toFixed(2)+" kWh":I.toFixed(0)+" Wh",v=I=>I>=1e3?(I/1e3).toFixed(1)+" kW":Math.round(I)+" W",S=this.shadowRoot,D=I=>S&&S.getElementById(I),z=(I,P)=>{const C=D(I);C&&(C.textContent=P)},O=(I,P,C)=>{const A=D(I);A&&(A.style[P]=C)},R=D("wd-pg-state-badge");R&&(R.textContent=c,R.style.background=_,R.style.color=_.includes("var")?"":"#fff"),z("wd-pg-power",v(a)),z("wd-pg-pct",p!=null?p+"%":"\u2014%"),O("wd-pg-pbar","width",(p??0)+"%"),z("wd-pg-rem",f!=null?x(f):"\u2014"),z("wd-pg-energy",b(g)),z("wd-pg-conf",$),z("wd-pg-phase",k)}_pgIsUnknownCmd(t){const e=t&&(t.code||t.error&&t.error.code)||"",s=(t&&(t.message||t.error)||"").toString().toLowerCase();return e==="unknown_command"||s.includes("unknown command")||s.includes("unknown_command")}_pgInterpPower(t,e){if(!t.length)return 0;if(e<=t[0].t)return t[0].w;for(let s=1;s<t.length;s++)if(e<=t[s].t){const r=t[s].t-t[s-1].t;if(r<=0)return t[s].w;const n=(e-t[s-1].t)/r;return t[s-1].w+n*(t[s].w-t[s-1].w)}return t[t.length-1].w}_pgTrapEnergy(t,e){let s=0;for(let r=1;r<t.length;r++){const n=t[r-1].t,a=t[r-1].w,o=Math.min(t[r].t,e),i=this._pgInterpPower(t,o);if(!(o<=n)&&(s+=(a+i)/2*(o-n)/3600,o>=e))break}return s}_drawPlaygroundCanvases(){this._tab==="playground"&&this._pgDrawCanvas()}_htmlPhases(){const t=this._devices[this._selIdx],e=t&&t.options.device_type||"washing_machine",s=this._canEdit(),r=this._phases.map(o=>{const i=o.is_default,l=o.translation_key?this._t(o.translation_key,{},o.description||""):o.description||"",c=s?`
        <td>
    ...
    </div>`:"",k=s.map(v=>{const S=parseInt(a[v],10)||0;if(S<=0)return"";const D=Math.max(0,parseInt(o[v],10)||0),z=Math.max(0,Math.min(100,Math.round(D/S*100))),R=D>=S?"var(--warning-color,#ff9800)":"var(--primary-color)";return`<div style="margin-top:10px">
        <div style="display:flex;align-items:baseline;justify-content:space-between;gap:8px">
          <span style="font-weight:600">${d(this._maintLabel(v))}</span>
          <span class="wd-info">${this._t("lbl.cycles_since_service",{since:D,total:S},D+" / "+S+" cycles")}</span>
        </div>
        <div style="height:6px;margin-top:4px;border-radius:3px;background:var(--divider-color);overflow:hidden">
          <div style="height:100%;border-radius:3px;width:${z}%;background:${R}"></div>
        </div>
    ...
    ${r||`<div class="wd-card"><p class="wd-info">${this._t("msg.no_other_users",{},"No other Home Assistant users found.")}</p></div>`}`}_htmlStore(){if(!this._canEdit())return`<div class="wd-empty">${this._t("msg.no_device_selected",{},"No device selected.")}</div>`;const t=this._storeStatus,e=`<a href="https://3dg1luk43.github.io/washdata-store" target="_blank" rel="noopener noreferrer" style="font-size:.8em;font-weight:400;color:var(--primary-color);text-decoration:none;white-space:nowrap" title="${d(this._t("store.website_tip",{},"Open the community store website"))}">${this._t("store.website",{},"Store website \u2197")}</a>`,s=`<div style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap"><div class="wd-card-title" style="margin:0">${this._t("hdr.community_store",{},"Community Store")}</div>${e}</div>`;if(!this._onlineEnabled()||t&&t.enabled===!1)return`<div class="wd-card">${s}
      ...
      ${Ut(`data-action="store-toggle-dl-settings" ${this._dlSettings?"checked":""} ${n?"disabled":""}`,this._t("lbl.adopt_settings",{},"Also adopt settings"),lt(this._t("msg.adopt_settings_hint",{},"Overwrite this device's detection & matching thresholds with the shared ones. Your notifications, entities and energy price are never changed.")))}
    ...
    </div>`:"")+s}_htmlStoreProfile(){const t=this._storeCycles||[],e=t.map(r=>{const n=r.stats||{},a=this._storeSparkline(r.trace&&r.trace.points||[]),o=n.duration!=null?_t(n.duration):"-",i=n.energy_wh!=null?yt(n.energy_wh/1e3):"-",l=n.peak_w!=null?Zt(n.peak_w):"-",c=d(r.uploaderName||this._t("store.anon",{},"anonymous")),_=this._statusTag(r),p=r.rating||{},f=p.avg!=null&&p.count?this._t("store.rating_summary",{avg:Number(p.avg).toFixed(1),n:p.count},`\u2605 ${Number(p.avg).toFixed(1)} (${p.count})`):this._t("store.no_ratings",{},"No ratings yet");return`<div class="wd-card">
        <div class="wd-store-cycle-top">
          ${a}
          <div class="wd-store-cycle-stats">
            <div><b>${o}</b> \xB7 ${i} \xB7 ${l} ${_}</div>
            <div class="wd-info">${this._t("store.uploaded_by",{name:c},`Shared by ${c}`)} \xB7 \u2B07 ${r.downloads||0} \xB7 ${f}</div>
          </div>
      ...
      ${Mt(`data-action="store-toggle-online" ${t?"checked":""} ${r?"disabled":""}`,this._t("lbl.enable_online",{},"Enable online features"))}
      ${t?this._htmlStorePrefs(r):""}
      ${t?n:""}
    </div>`}_htmlStorePrefs(t){const e=this._constants&&this._constants.storePrefs||{},s=pe.map(n=>{const a=e[n.key]!==!1;return Mt(`data-action="store-toggle-pref" data-pref="${d(n.key)}" ${a?"checked":""} ${t?"disabled":""}`,this._t(n.labelKey,{},n.labelFb),lt(this._t(n.docKey,{},n.docFb)))}).join(""),r=this._busy.has("store-refresh-catalog");return s+`
      ...
      </div>`}async _loadStoreStatus(t){if(!this._onlineEnabled()){this._storeStatus={enabled:!1},this._storeConnected=!1;return}try{const e=await this._ws({type:`${w}/store_status`,entry_id:t});if(!this._isActiveEntry(t))return;this._storeStatus=e||null,this._storeConnected=!!(e&&e.connected)}catch{}}_storeBrandScope(){return String(this._storeQuery||(this._opts||{}).store_brand||"").trim()}_renderKeepingStoreQFocus(){const t=this.shadowRoot,e=t&&t.getElementById("wd-store-q"),s=!!e&&t.activeElement===e,r=s?e.selectionStart:0;if(this._render(),!s)return;const n=t.getElementById("wd-store-q");if(!n)return;n.focus();try{n.setSelectionRange(r,r)}catch{}const a=n.parentElement&&n.parentElement.querySelector(".wd-combo-drop");a&&(a.hidden=!0)}_storeOpenDevice(t){const e=this._devices[this._selIdx];if(!e||!t)return;const s=e.entry_id;this._storeDevice=t,this._storeProfile=null,this._storeView="device",this._storeProfiles=[],this._storeCycles=[],this._storeLoading=!0,this._render(),this._ws({type:`${w}/store_get_profiles`,entry_id:s,device_id:t.id}).then(r=>{!this._isActiveEntry(s)||this._storeView!=="device"||(this._storeProfiles=r&&r.items||[])}).catch(()=>{this._isActiveEntry(s)&&(this._storeProfiles=[])}).finally(()=>{this._isActiveEntry(s)&&(this._storeLoading=!1,this._render())})}async _storeOpenModel(t){if(!t)return;await this._storeSearch(String(t.brand||""));const e=(this._storeDevices||[]).find(s=>String(s.id)===String(t.id))||t;this._storeOpenDevice(e)}async _storeSearch(t){const e=this._devices[this._selIdx];if(!e)return;const s=e.entry_id;this._storeQuery=t||"",this._storeView="brands",this._storeDevice=null,this._storeProfile=null,this._storeProfiles=[],this._storeCycles=[];const r=this._storeBrandScope();if(!r){this._storeDevices=[],this._storeLoading=!1,this._renderKeepingStoreQFocus();return}this._storeLoading=!0,this._renderKeepingStoreQFocus();try{const n=await this._ws({type:`${w}/store_search_devices`,entry_id:s,query:r,appliance_type:this._storeApplianceType(),include_pending:!0});if(!this._isActiveEntry(s))return;n&&n.disabled?(this._storeStatus={enabled:!1},this._storeDevices=[]):this._storeDevices=this._sortStoreDevices(n&&n.items||[])}catch(n){this._isActiveEntry(s)&&(this._storeDevices=[],this._showToast(this._t("toast.store_search_failed",{error:n.message||n},"Search failed: "+(n.message||n)),"error"))}finally{this._isActiveEntry(s)&&(this._storeLoading=!1,this._renderKeepingStoreQFocus())}}_sortStoreDevices(t){const e=String((this._opts||{}).store_model||"").trim().toLowerCase(),s=r=>e&&String(r.model||"").toLowerCase()===e?0:this._storeItemHasContent(r)?1:2;return t.map((r,n)=>({d:r,i:n})).sort((r,n)=>s(r.d)-s(n.d)||r.i-n.i).map(r=>r.d)}_storeItemHasContent(t){return(Number(t&&t.profileCount)||0)>0||(Number(t&&t.cycleCount)||0)>0}_ensureStoreConnectListener(){if(this._storeConnectListener)return;const t=this._constants.storeWebOrigin;if(!t)return;let e;try{e=new URL(t).origin}catch{return}this._storeConnectListener=async s=>{if(s.origin!==e)return;const r=s.data;if(!r)return;const n=this._devices[this._selIdx];if(!n)return;const a=n.entry_id;if(r.type==="washdata-device-created"){const o={};r.brand&&(o.store_brand=r.brand),r.model&&(o.store_model=r.model),this._catalog.brands=void 0,this._catalog.devices=void 0,this._catalog.forBrand=null,this._catalog.brandsFull=!1,this._catalog.brandPrefixes=[],this._dropModelCandidates(),this._catalogEntry=null;const i=this._storeContribEid||a,l=Object.keys(o).length>0,c=this._isActiveEntry(i);this._showToast(l&&!c?this._t("toast.contrib_device_changed",{},"Added to the community store, but the device changed while the form was open. Select it in Settings."):this._t("toast.appliance_added",{},"Appliance added - awaiting approval")),l&&c&&await this._saveStoreOptions(o,{silent:!0}),this._render();return}if(r.type==="washdata-brand-created"){this._catalog.brands=void 0,this._catalog.brandsFull=!1,this._catalog.brandPrefixes=[],this._dropModelCandidates(),this._catalogEntry=null;const o=this._isActiveEntry(this._storeContribEid||a);this._showToast(r.brand&&!o?this._t("toast.contrib_device_changed",{},"Added to the community store, but the device changed while the form was open. Select it in Settings."):this._t("toast.brand_added",{},"Brand added - awaiting approval")),r.brand&&o&&await this._saveStoreOptions({store_brand:r.brand,store_model:""},{silent:!0}),this._render();return}if(r.type==="washdata-profile-created"){const o=this._modal;o&&o.type==="store-share"&&(o.program=r.program||o.program,this._loadShareProfiles()),this._showToast(this._t("toast.profile_added",{},"Profile added - awaiting approval"));return}if(r.type==="washdata-connect")try{const o=await this._ws({type:`${w}/store_connect`,entry_id:a,refresh_token:r.refreshToken,uid:r.uid,name:r.displayName});if(o&&o.error){this._showToast(this._t("toast.store_connect_failed",{error:o.error},"Connect failed: "+o.error),"error");return}if(await this._loadStoreStatus(a),!this._isActiveEntry(a))return;this._showToast(this._t("toast.store_connected",{},"Connected to the community store")),this._render()}catch(o){this._showToast(this._t("toast.store_connect_failed",{error:o.message||o},"Connect failed: "+(o.message||o)),"error")}},window.addEventListener("message",this._storeConnectListener)}async _saveStoreOptions(t,{silent:e=!1}={}){const s=this._devices[this._selIdx];if(!s)return!1;const r=s.entry_id;try{if(await this._ws({type:`${w}/set_options`,entry_id:r,options:t}),!this._isActiveEntry(r))return!0;this._opts={...this._opts,...t};for(const n of Object.keys(t))delete this._pendingSettings[n],this._dirtyOptKeys.delete(n);return e||this._showToast(this._t("toast.settings_saved",{},"Settings saved; integration reloading")),!0}catch(n){return this._showToast(this._t("msg.toast_save_failed",{error:n.message||n},"Save failed: "+(n.message||n)),"error"),!1}}_htmlLogDrawer(){return`<div class="wd-log-drawer open" style="width:${this._logDrawerWidth}px">
      ...
      <div class="wd-log-drawer-body">
    ...
    </div>`}_drawCurves(t,e){const s=this.shadowRoot&&this.shadowRoot.getElementById(t);if(!s)return null;const r=s.getBoundingClientRect(),n=window.devicePixelRatio||1,a=Math.max(1,Math.round(r.width*n)),o=Math.max(1,Math.round((r.height||240)*n));(s.width!==a||s.height!==o)&&(s.width=a,s.height=o);const i=s.getContext("2d"),l=getComputedStyle(this),c=(l.getPropertyValue("--primary-color")||"#03a9f4").trim()||"#03a9f4",_=(l.getPropertyValue("--divider-color")||"rgba(127,127,127,.3)").trim()||"rgba(127,127,127,.3)",p=(l.getPropertyValue("--secondary-text-color")||"#888").trim()||"#888",f=44*n,g=12*n,$=12*n,k=22*n,x=e.series||[];let b=e.xMax||0;b||(x.forEach(P=>(P.points||[]).forEach(C=>{C[0]>b&&(b=C[0])})),e.band&&(e.band.max||[]).forEach(P=>{P[0]>b&&(b=P[0])})),b=b||1;const v=this._canvasZoom&&this._canvasZoom[t],S=v?v.xMin:0,D=v?v.xMax:b;let z=e.yMax||0;if(!z){const P=C=>{const A=C||[];for(let F=0;F<A.length;F++){const U=A[F][0];U<S&&!(F+1<A.length&&A[F+1][0]>=S)||U>D&&!(F>0&&A[F-1][0]<=D)||A[F][1]>z&&(z=A[F][1])}};x.forEach(C=>{C.noScale||P(C.points)}),e.band&&P(e.band.max),z=(z||10)*1.08}const O=a-f-g,R=P=>f+(P-S)/(D-S)*O,I=P=>o-k-P/z*(o-$-k);i.clearRect(0,0,a,o),i.strokeStyle=_,i.lineWidth=n,i.fillStyle=p,i.font=`${11*n}px sans-serif`,i.textAlign="right",i.textBaseline="middle";for(let P=0;P<=2;P++){const C=$+P/2*(o-$-k);i.beginPath(),i.moveTo(f,C),i.lineTo(a-g,C),i.stroke(),i.fillText(Math.round(z*(1-P/2))+"W",f-4*n,C)}if(i.save(),i.beginPath(),i.rect(f,$,O,o-$),i.clip(),(e.bands||[]).forEach(P=>{i.fillStyle=P.fill;const C=R(P.x0),A=R(P.x1);i.fillRect(Math.min(C,A),$,Math.abs(A-C),o-$-k)}),e.band&&(e.band.min||[]).length&&(e.band.max||[]).length){i.beginPath(),e.band.max.forEach((P,C)=>C?i.lineTo(R(P[0]),I(P[1])):i.moveTo(R(P[0]),I(P[1])));for(let P=e.band.min.length-1;P>=0;P--)i.lineTo(R(e.band.min[P][0]),I(e.band.min[P][1]));i.closePath(),i.fillStyle=e.band.fill||Pt(c,.13),i.fill()}return x.forEach(P=>{const C=P.points||[];if(!C.length)return;const A=P.stroke==="primary"?c:P.stroke;if(P.fill){i.beginPath(),C.forEach((U,st)=>st?i.lineTo(R(U[0]),I(U[1])):i.moveTo(R(U[0]),I(U[1]))),i.lineTo(R(C[C.length-1][0]),I(0)),i.lineTo(R(C[0][0]),I(0)),i.closePath();const F=i.createLinearGradient(0,$,0,o-k);F.addColorStop(0,Pt(A,.33)),F.addColorStop(1,Pt(A,.03)),i.fillStyle=F,i.fill()}i.beginPath(),C.forEach((F,U)=>U?i.lineTo(R(F[0]),I(F[1])):i.moveTo(R(F[0]),I(F[1]))),i.strokeStyle=A,i.lineWidth=(P.width||1.5)*n,i.lineJoin="round",P.dash&&i.setLineDash([6*n,4*n]),i.globalAlpha=P.alpha!=null?P.alpha:1,i.stroke(),i.globalAlpha=1,P.dash&&i.setLineDash([])}),i.textAlign="center",i.textBaseline="top",(e.vlines||[]).forEach(P=>{const C=R(P.x);i.beginPath(),i.moveTo(C,$),i.lineTo(C,o-k),i.strokeStyle=P.color,i.lineWidth=2*n,i.setLineDash([4*n,3*n]),i.stroke(),i.setLineDash([]),P.handle&&(i.fillStyle=P.color,i.beginPath(),i.arc(C,$+4*n,4.5*n,0,Math.PI*2),i.fill()),P.label&&(i.fillStyle=P.color,i.fillText(P.label,C,$+(P.handle?12*n:2*n)))}),i.restore(),i.fillStyle=p,i.font=`${11*n}px sans-serif`,i.textBaseline="bottom",v&&(i.textAlign="left",i.fillText((S/60).toFixed(1)+" min",f,o-2*n)),i.textAlign="right",i.fillText((D/60).toFixed(0)+" min",a-g,o-2*n),s._wd={xMax:b,xMin:S,xViewMax:D,yMax:z,dpr:n,padT:$,padB:k,padL:f,ch:o,primary:c,plotWcss:O/n,Xpx:R,Ypx:I,xToCss:P=>R(P)/n,cssToX:P=>Math.max(S,Math.min(D,S+(P*n-f)/O*(D-S))),series:(e.series||[]).map(P=>({points:P.points,stroke:P.stroke,name:P.name,cid:P.cid})),band:e.band||null,artifacts:e.artifacts||null,_opts:e},s._wd}_drawModalCanvas(){const t=this._modal;t&&(t.type==="cycle-detail"?this._drawCycleEditor():t.type==="compare-cycles"?this._drawCompareCanvas():t.type==="profile-group"?this._drawGroupCanvas():t.type==="profile-panel"&&(t.tab==="stats"?this._drawProfileEnvelope():t.tab==="phases"?this._drawPhaseEditor():t.tab==="cleanup"&&this._drawSpaghetti()))}_redrawCanvas(t){if(t==="wd-status-canvas")this._drawStatusCurve();else if(t==="wd-cyc-canvas")this._drawCycleEditor();else if(t==="wd-compare-canvas")this._drawCompareCanvas();else if(t==="wd-env-canvas")this._drawProfileEnvelope();else if(t==="wd-phase-canvas")this._drawPhaseEditor();else if(t==="wd-spag-canvas")this._drawSpaghetti();else if(t==="wd-pgroup-canvas")this._drawGroupCanvas();else{const e=this.shadowRoot&&this.shadowRoot.getElementById(t);e&&e._wd&&e._wd._opts&&this._drawCurves(t,e._wd._opts)}}_pref(t,e){const s=this._panelCfg&&this._panelCfg.prefs||{};return s[t]===void 0?e:s[t]}_setPref(t,e){this._panelCfg||(this._panelCfg={}),this._panelCfg.prefs={...this._panelCfg.prefs||{},[t]:e},this._ws({type:`${w}/set_user_prefs`,prefs:{[t]:e}}).catch(()=>{})}_drawStatusCurve(){const t=this._powerData||{},e=t.live||[];if(e.length<2)return;const s=this._statusEnv,r=this._pref("show_expected",!0),n=this._pref("show_raw_active",!1),a=[];let o=e[e.length-1][0];if(t.cycle_active&&s&&(s.avg||[]).length&&r){const l=s.target_duration||s.avg[s.avg.length-1][0];a.push({points:s.avg,stroke:"#ff9800",width:2,alpha:.4,name:this._t("lbl.expected",{},"Expected")}),o=Math.max(o,l)}a.push({points:e,stroke:"primary",fill:!0,width:2,name:this._t("lbl.power",{},"Power")}),n&&(t.raw||[]).length>1&&a.push({points:t.raw,stroke:"#9e9e9e",width:1,alpha:.65,name:this._t("lbl.raw_socket",{},"Raw socket"),noScale:!0});const i=[];(t.restart_gaps||[]).forEach(l=>{if(!e.length)return;const c=t.cycle_start_iso;if(!c)return;const _=new Date(c).getTime(),p=Math.max(0,(new Date(l.start_ts).getTime()-_)/1e3),f=Math.max(p+1,(new Date(l.end_ts).getTime()-_)/1e3);i.push({x0:p,x1:f,fill:"rgba(96,125,139,.20)"})}),this._drawCurves("wd-status-canvas",{series:a,xMax:o,bands:i})}_attachGraphGestures(t){const e=this.shadowRoot,s=e&&e.getElementById(t);if(!s)return;this._syncZoomReset(t),this._attachTouchOwnershipGuard(s);const r=new Map;let n=null;const a=i=>i-s.getBoundingClientRect().left;s.addEventListener("pointerdown",i=>{if(r.set(i.pointerId,{x:i.clientX,y:i.clientY}),r.size===1){if(i.pointerType==="touch"&&this._unpinGraphTip(),i.pointerType==="touch"&&this._onAxisHandle(s,t,i)){s._wdOwnGesture=!0;try{s.setPointerCapture(i.pointerId)}catch{}}return}if(r.size!==2)return;s._wdPinching=!0,s._wdOwnGesture=!1,s._wdAbortEdit&&s._wdAbortEdit(),this._hideGraphTip();const[l,c]=[...r.values()];n={dist:Math.max(1,Math.abs(l.x-c.x)),centerCss:a((l.x+c.x)/2)}}),s.addEventListener("pointermove",i=>{if(r.has(i.pointerId)&&r.set(i.pointerId,{x:i.clientX,y:i.clientY}),s._wdPinching){n&&r.size>=2&&this._onGraphPinch(s,t,r,n,a);return}this._onGraphHover(i,t)});const o=i=>{r.delete(i.pointerId),r.size<2&&(n=null,s._wdPinching=!1),r.size===0&&(s._wdOwnGesture=!1)};s.addEventListener("pointerup",i=>{const l=s._wdPinching;o(i),i.pointerType==="touch"&&!l&&this._pinGraphTip()}),s.addEventListener("pointercancel",i=>{o(i),this._hideGraphTip()}),s.addEventListener("pointerleave",i=>{i.pointerType!=="touch"&&this._hideGraphTip()}),s.addEventListener("wheel",i=>{const l=s._wd;if(!l)return;i.preventDefault();const c=i.ctrlKey?Math.min(4,Math.max(.25,Math.exp(i.deltaY*.01))):i.deltaY>0?1.3:.75;this._zoomCanvasAbout(t,l.cssToX(a(i.clientX)),c)},{passive:!1}),s.addEventListener("dblclick",i=>{const l=s._wd;if(l){if(this._canvasZoom[t]){this._resetCanvasZoom(t),this._hideGraphTip();return}this._zoomCanvasAbout(t,l.cssToX(a(i.clientX)),Xt)}})}_attachTouchOwnershipGuard(t){t._wdTouchGuard||(t._wdTouchGuard=!0,t.addEventListener("touchmove",e=>{(e.touches.length>=2||t._wdOwnGesture)&&e.preventDefault()},{passive:!1}))}_onAxisHandle(t,e,s){const r=this._gAxisHandle?this._gAxisHandle[e]:null,n=t._wd;if(r==null||!n)return!1;const a=t.getBoundingClientRect(),o=(n.ch-n.padB)/n.dpr;return Math.abs(s.clientX-a.left-r)<=Dt&&Math.abs(s.clientY-a.top-o)<=Dt}_onGraphPinch(t,e,s,r,n){const a=t._wd;if(!a)return;const[o,i]=[...s.values()],l=Math.max(1,Math.abs(o.x-i.x)),c=n((o.x+i.x)/2),_=this._gPinchPending&&this._gPinchPending.id===e?this._gPinchPending:null,p=_?{xMin:_.lo,xMax:_.hi}:this._canvasZoom[e],f=p?p.xMin:0,g=(p?p.xMax:a.xMax||1)-f,$=Math.max(1,a.plotWcss),k=Math.max(0,Math.min(1,c/$)),x=f+k*g,b=g*(r.dist/l),v=x-k*b-(c-r.centerCss)/$*b;r.dist=l,r.centerCss=c,this._gPinchPending={id:e,lo:v,hi:v+b},!this._gPinchRaf&&(this._gPinchRaf=requestAnimationFrame(()=>{this._gPinchRaf=null;const S=this._gPinchPending;this._gPinchPending=null,S&&this._setCanvasViewport(S.id,S.lo,S.hi)}))}_setCanvasViewport(t,e,s){const r=this.shadowRoot&&this.shadowRoot.getElementById(t),n=r&&r._wd;if(!n)return;const a=n.xMax||1,o=Math.min(a,Math.max(a/_e,ge)),i=Math.min(a,Math.max(o,s-e));if(i>=a*.99){this._resetCanvasZoom(t);return}const l=Math.max(0,Math.min(a-i,e)),c=this._canvasZoom[t];c&&Math.abs(c.xMin-l)<1e-6&&Math.abs(c.xMax-l-i)<1e-6||(this._canvasZoom[t]={xMin:l,xMax:l+i},this._redrawCanvas(t),this._syncZoomReset(t))}_zoomCanvasAbout(t,e,s){const r=this.shadowRoot&&this.shadowRoot.getElementById(t),n=r&&r._wd;if(!n)return;const a=n.xMax||1,o=this._canvasZoom[t],i=o?o.xMin:0,l=Math.max(1e-6,(o?o.xMax:a)-i),c=Math.max(0,Math.min(1,(e-i)/l)),_=l*s;this._setCanvasViewport(t,e-c*_,e+(1-c)*_)}_resetCanvasZoom(t){if(!this._canvasZoom[t]){this._syncZoomReset(t);return}delete this._canvasZoom[t],this._redrawCanvas(t),this._syncZoomReset(t)}_ensureZoomReset(t,e){const s=e.parentElement;if(!s)return null;let r=s.querySelector(".wd-zoom-reset");r||(r=document.createElement("button"),r.type="button",r.className="wd-zoom-reset",r.textContent="\u21BB",r.addEventListener("click",a=>{a.stopPropagation(),this._resetCanvasZoom(t)}),s.appendChild(r));const n=this._t("btn.reset_zoom",{},"Reset zoom");return r.title!==n&&(r.title=n,r.setAttribute("aria-label",n)),r}_syncZoomReset(t){const e=this.shadowRoot&&this.shadowRoot.getElementById(t);if(!e)return;const s=this._ensureZoomReset(t,e);s&&s.classList.toggle("wd-zoom-reset--on",!!this._canvasZoom[t])}_onGraphHover(t,e){const s=t.clientX,r=t.clientY,n=t.pointerType==="touch";if(this._hoverRafId){this._hoverPending={px:s,py:r,id:e,touch:n};return}this._hoverPending={px:s,py:r,id:e,touch:n},this._hoverRafId=requestAnimationFrame(()=>{this._hoverRafId=null;const{px:a,py:o,id:i,touch:l}=this._hoverPending||{};i&&this._onGraphHoverInner(a,o,i,{touch:l})})}_onGraphHoverInner(t,e,s,r={}){const n=this.shadowRoot&&this.shadowRoot.getElementById(s),a=n&&n._wd;if(!a)return;const o=n.getBoundingClientRect(),i=a.cssToX(t-o.left),l=(e-o.top)*a.dpr;this._redrawCanvas(s);const c=n.getContext("2d"),_=a.Xpx(i);c.save(),c.strokeStyle="rgba(140,140,140,.75)",c.lineWidth=a.dpr,c.setLineDash([3*a.dpr,3*a.dpr]),c.beginPath(),c.moveTo(_,a.padT),c.lineTo(_,a.ch-a.padB),c.stroke(),c.setLineDash([]);const p=b=>b.stroke==="primary"?a.primary:b.stroke,f=(b,v)=>{c.fillStyle=v,c.beginPath(),c.arc(_,a.Ypx(b),3.4*a.dpr,0,6.2832),c.fill()},g=[`${this._t("lbl.from_start",{},"From start")}: <b>${zt(i)}</b>`,`${this._t("lbl.to_end",{},"To end")}: <b>${zt(Math.max(0,a.xMax-i))}</b>`],$=a.series||[];if(this._hoverNearest=null,$.length>4){let b=null,v=1/0;if($.forEach(S=>{const D=Ot(S.points,i);if(D==null)return;const z=Math.abs(a.Ypx(D)-l);z<v&&(v=z,b={s:S,v:D})}),b){const S=p(b.s);c.strokeStyle=S,c.lineWidth=3*a.dpr,c.beginPath(),(b.s.points||[]).forEach((D,z)=>z?c.lineTo(a.Xpx(D[0]),a.Ypx(D[1])):c.moveTo(a.Xpx(D[0]),a.Ypx(D[1]))),c.stroke(),f(b.v,S),g.push(`${d(b.s.name||"")}: <b>${b.v.toFixed(b.v<100?1:0)} W</b>`),b.s.cid&&(g.push(`<span style="opacity:.7">${this._t("lbl.click_to_select",{},"click to select")}</span>`),this._hoverNearest={id:s,cid:b.s.cid})}}else $.forEach(b=>{const v=Ot(b.points,i);v!=null&&(f(v,p(b)),g.push(`${d(b.name||this._t("lbl.power",{},"Power"))}: <b>${v.toFixed(v<100?1:0)} W</b>`))});if(a.band){const b=Ot(a.band.min,i),v=Ot(a.band.max,i);b!=null&&v!=null&&g.push(`${this._t("lbl.envelope",{},"Envelope")}: ${b.toFixed(0)}\u2013${v.toFixed(0)} W`)}(a.artifacts||[]).forEach(b=>{if(i>=b.start_s&&i<=b.end_s){const v=b.detail_key?this._t(b.detail_key,b.detail_params||{},b.detail||""):b.detail||"";g.push(`<span style="color:var(--warning-color,#ff9800)">\u26A0 ${d(ne(b.type,(S,D,z)=>this._t(S,D,z)))}</span>: ${d(v)}`)}});const k=!!this._canvasZoom[s];let x=null;if(r.touch?x=k?this._t("lbl.zoom_hint_touch_zoomed",{},"pinch to zoom \xB7 double-tap to reset"):this._t("lbl.zoom_hint_touch",{},"pinch to zoom"):k&&(x=this._t("lbl.zoom_hint",{},"scroll to zoom \xB7 dblclick to reset")),x&&g.push(`<span style="opacity:.45">${d(x)}</span>`),r.touch){const b=a.ch-a.padB;c.beginPath(),c.arc(_,b,Dt*.41*a.dpr,0,6.2832),c.fillStyle=a.primary,c.fill(),c.strokeStyle="rgba(255,255,255,.92)",c.lineWidth=1.6*a.dpr,c.stroke(),this._gAxisHandle||(this._gAxisHandle={}),this._gAxisHandle[s]=a.xToCss(i)}else this._gAxisHandle&&delete this._gAxisHandle[s];c.restore(),this._showGraphTip(t,e,g,{touch:r.touch,rect:o}),this._syncSpagRowHighlight(this._hoverNearest?this._hoverNearest.cid:null)}_showGraphTip(t,e,s,r={}){const n=this._gtip;if(!n)return;this._gtipLines=s,n.innerHTML=s.join("<br>"),n.classList.remove("wd-gtip--pinned"),this._gtipPinned=!1,n.style.display="block";const a=n.offsetWidth,o=n.offsetHeight,i=16;let l,c;if(r.touch&&r.rect){const _=r.rect,p=8;_.top-o-p>=6?c=_.top-o-p:_.bottom+p+o<=window.innerHeight-6?c=_.bottom+p:c=e>_.top+_.height*.5?_.top+6:_.bottom-o-6,l=t-a/2}else l=t+i,c=e+i,c+o>window.innerHeight-6&&(c=e-o-i),l+a>window.innerWidth-6&&(l=t-a-i);n.style.left=Math.max(6,Math.min(l,window.innerWidth-a-6))+"px",n.style.top=Math.max(6,Math.min(c,window.innerHeight-o-6))+"px"}_pinGraphTip(){const t=this._gtip;if(!t||t.style.display==="none"||this._gtipPinned)return;this._gtipPinned=!0,t.classList.add("wd-gtip--pinned");const e=this._t("lbl.tap_to_dismiss",{},"tap the chart to dismiss");t.innerHTML=(this._gtipLines||[]).join("<br>")+`<span class="wd-gtip-dismiss">${d(e)}</span>`}_maybeDismissGraphTip(t){if(!this._gtipPinned)return;const e=t.target;e&&e.closest&&e.closest(".wd-canvas-wrap")||this._unpinGraphTip()}_unpinGraphTip(){this._gtipPinned&&(this._gtipPinned=!1,this._gtip&&(this._gtip.classList.remove("wd-gtip--pinned"),this._gtip.style.display="none"))}_hideGraphTip(){this._hoverRafId&&(cancelAnimationFrame(this._hoverRafId),this._hoverRafId=null,this._hoverPending=null),this._gtipPinned=!1,this._gtip&&(this._gtip.classList.remove("wd-gtip--pinned"),this._gtip.style.display="none"),this._syncSpagRowHighlight(null)}_positionTip(t){const e=t.querySelector(".wd-tip-pop");if(!e)return;e.style.transform="";const s=!e.offsetWidth,r=s?e.style.cssText:"";s&&(e.style.visibility="hidden",e.style.display="block");const n=e.getBoundingClientRect();if(s&&(e.style.cssText=r),!n.width)return;const a=de(t),o=6,i=a.left+o,l=a.right-o-n.width,c=l<i?i:Math.min(Math.max(n.left,i),l),_=Math.round(c-n.left);_&&(e.style.transform=`translateX(calc(-50% + ${_}px))`)}_positionComboDrop(t,e){const s=t.getBoundingClientRect();if(!s.height)return;const r=de(t),n=8,a=r.bottom-s.bottom-n,o=s.top-r.top-n,i=a<120&&o>a,l=Math.max(64,Math.floor(i?o:a));e.classList.toggle("wd-drop-up",i),e.style.maxHeight=`${Math.min(220,l)}px`}_syncSpagRowHighlight(t){if(t===this._spagHoverCid)return;this._spagHoverCid=t;const e=this.shadowRoot;e&&e.querySelectorAll("tr[data-cid]").forEach(s=>{s.style.backgroundColor=t&&s.dataset.cid===t?"var(--secondary-background-color,rgba(0,0,0,.06))":""})}_showToast(t,e="success",s={}){this._toastTimer&&clearTimeout(this._toastTimer);const r=s.duration||3500;this._toast={msg:t,cls:`wd-toast-${e}`,actionLabel:s.actionLabel||null,actionToken:s.actionToken||null},this._render(),this._toastTimer=setTimeout(()=>{this._toast=null,this._render()},r)}_profileOptions(t){return(this._profiles||[]).map(e=>`<option value="${d(e.name)}" ${String(t)===String(e.name)?"selected":""}>${d(e.name)}</option>`).join("")}_htmlModal(){const t=this._modal;if(t.type==="cycle-detail")return`<div class="wd-overlay"><div class="wd-modal wd-modal-lg" role="dialog" aria-modal="true" aria-labelledby="wd-modal-title" tabindex="-1">${this._htmlCycleModal(t)}</div></div>`;if(t.type==="profile-panel")return`<div class="wd-overlay"><div class="wd-modal wd-modal-lg" role="dialog" aria-modal="true" aria-labelledby="wd-modal-title" tabindex="-1">${this._htmlProfilePanel(t)}</div></div>`;if(t.type==="profile-group")return`<div class="wd-overlay"><div class="wd-modal wd-modal-lg" role="dialog" aria-modal="true" aria-labelledby="wd-modal-title" tabindex="-1">${this._htmlProfileGroupModal(t)}</div></div>`;if(t.type==="compare-cycles")return`<div class="wd-overlay"><div class="wd-modal wd-modal-lg" role="dialog" aria-modal="true" aria-labelledby="wd-modal-title" tabindex="-1">${this._htmlCompareModal(t)}</div></div>`;if(t.type==="gear-settings")return`<div class="wd-overlay"><div class="wd-modal wd-modal-lg" role="dialog" aria-modal="true" aria-labelledby="wd-modal-title" tabindex="-1">${this._htmlGearModal(t)}</div></div>`;if(t.type==="export-select")return`<div class="wd-overlay"><div class="wd-modal wd-modal-lg" role="dialog" aria-modal="true" aria-labelledby="wd-modal-title" tabindex="-1">${this._htmlExportSelectModal(t)}</div></div>`;if(t.type==="import-wizard")return`<div class="wd-overlay"><div class="wd-modal wd-modal-lg" role="dialog" aria-modal="true" aria-labelledby="wd-modal-title" tabindex="-1">${this._htmlImportWizardModal(t)}</div></div>`;if(t.type==="history-import")return`<div class="wd-overlay"><div class="wd-modal wd-modal-lg" role="dialog" aria-modal="true" aria-labelledby="wd-modal-title" tabindex="-1">${this._htmlHistoryImportModal(t)}</div></div>`;let e="";if(t.type==="confirm")e=`<h2>${d(t.title)}</h2><p class="wd-info">${d(t.message)}</p>
        ...
        <div class="wd-modal-actions"><button class="wd-btn wd-btn-secondary" data-maction="cancel">${this._t("btn.cancel",{},"Cancel")}</button>
        <button class="wd-btn wd-btn-primary" data-maction="label-ok">${this._t("btn.apply_label",{},"Apply Label")}</button></div>`;else if(t.type==="create-profile"){const s=(this._cycles||[]).slice(0,40).map(r=>`<option value="${d(r.id)}">${rt(r.start_time)} - ${Math.round((r.duration||0)/60)}m - ${d(r.profile_name||this._t("lbl.unlabelled",{},"Unlabelled"))}</option>`).join("");e=`<h2>${this._t("modal.create_profile",{},"Create Profile")}</h2>
        ...
        <button class="wd-btn wd-btn-primary" data-maction="store-share-ok" ${this._busy.has("store-share")?"disabled":""}>${this._busy.has("store-share")?'<span class="wd-spin"></span> ':""}${this._t("btn.share",{},"Share")}</button></div>`}else t.type==="store-share-device"&&(e=this._htmlShareDeviceModal(t));return`<div class="wd-overlay"><div class="wd-modal" role="dialog" aria-modal="true" aria-labelledby="wd-modal-title" tabindex="-1">${e}</div></div>`}_htmlShareDeviceModal(t){const e=this._shareableByProgram(),s=this._busy.has("store-share-device"),r=t.selected||new Set,n=!!t.consented,a=e.reduce((g,$)=>g+$.cycles.filter(k=>r.has(k.id)).length,0),o=e.some(g=>!g.noCycles);let i;t.loading?i=`<div class="wd-empty" style="padding:24px"><div class="wd-icon">\u23F3</div>${this._t("msg.loading",{},"Loading\u2026")}</div>`:e.length?i=e.map(g=>{if(g.noCycles)return`<div class="wd-sd-group wd-sd-group-nocyc">
          ...
          </div>`;const $=g.cycles.every(S=>r.has(S.id)),k=!$&&g.cycles.some(S=>r.has(S.id)),x=g.cycles.map(S=>{const D=S.start_time?rt(S.start_time):"",z=S.duration!=null?_t(S.duration):"";return`<label class="wd-sd-cyc">
            <input type="checkbox" data-maction="sd-toggle-cyc" data-cid="${d(S.id)}" ${r.has(S.id)?"checked":""} ${s?"disabled":""}>
            <span class="wd-sd-cyc-meta">${d(D)} \xB7 ${d(z)} \u2B50</span>
          ...
          <label class="wd-sd-prof">
            <input type="checkbox" data-maction="sd-toggle-prof" data-prog="${d(g.program)}" ${$?"checked":""} ${k?'data-indeterminate="1"':""} ${s?"disabled":""}>
            <span class="wd-sd-prof-name">${d(g.program)}</span>
            <span class="wd-sd-count">${g.cycles.filter(S=>r.has(S.id)).length}/${g.cycles.length}</span>
          </label>
        ...
        ${lt(this._t("msg.include_settings_hint",{},"Share this device's recognition and matching thresholds (not your notifications, entities or energy price). Adopters choose whether to apply them."))}
    ...
    </details>`,f=o?`<label class="wd-sd-consent">
    ...
    </label>`,c=a.map(_=>{const p=r[_],f=s.importableOnly&&p.importable===!1,g=this._wizCatState(t,_,e),k=["profiles","real_cycles","reference_cycles"].includes(_)?`${g.sel}/${p.count}`:"",x=`<label class="wd-sd-prof">
        <input type="checkbox" data-maction="wiz-toggle-cat" data-cat="${_}" ${g.state==="all"?"checked":""} ${g.state==="some"?'data-indeterminate="1"':""} ${f?"disabled":""}>
        <span class="wd-sd-prof-name">${d(this._wizCatLabel(_))}${f?` <span class="wd-tag">${this._t("badge.not_importable",{},"n/a here")}</span>`:""}</span>
        <span class="wd-sd-count">${k}</span>
      </label>`;let b="";if(!f&&_==="profiles")b=`<div class="wd-sd-cycles">${(p.items||[]).map(v=>`<label class="wd-sd-cyc">
          <input type="checkbox" data-maction="wiz-toggle-profile" data-name="${d(v.name)}" ${t.sel.profiles.has(v.name)?"checked":""}>
          <span class="wd-sd-cyc-meta">${d(v.name)} \xB7 ${(v.real_cycles||0)+(v.reference_cycles||0)} ${this._t("lbl.cycles_short",{},"cycles")}${n.has(v.name)?` <span class="wd-tag" style="color:var(--warning-color,#e6a700)">${this._t("badge.exists",{},"exists")}</span>`:""}</span>
        </label>`).join("")}</div>`;else if(!f&&(_==="real_cycles"||_==="reference_cycles")){const v=_==="real_cycles"?t.sel.realIds:t.sel.refIds;b=`<div class="wd-sd-cycles">${(p.groups||[]).map(S=>{const D=S.cycles.map(A=>String(A.id)),z=D.filter(A=>v.has(A)).length,O=z===D.length&&D.length>0,R=z>0&&!O,I=`${_}:${S.profile}`,P=t.expanded&&t.expanded.has(I),C=P?S.cycles.map(A=>`<label class="wd-sd-cyc" style="margin-left:18px">
            <input type="checkbox" data-maction="wiz-toggle-cyc" data-cat="${_}" data-cid="${d(String(A.id))}" ${v.has(String(A.id))?"checked":""}>
            <span class="wd-sd-cyc-meta">${d(A.date?rt(A.date):String(A.id))}${A.duration!=null?" \xB7 "+_t(A.duration):""}</span>
            ...
            <div class="wd-sd-cyc" style="display:flex;align-items:center">
              <label style="flex:1;display:flex;align-items:center;gap:8px;cursor:pointer">
                <input type="checkbox" data-maction="wiz-toggle-cycgroup" data-cat="${_}" data-prof="${d(S.profile)}" ${O?"checked":""} ${R?'data-indeterminate="1"':""}>
                <span class="wd-sd-cyc-meta"><strong>${d(S.profile||this._t("lbl.unlabelled",{},"Unlabelled"))}</strong> (${z}/${S.count})</span>
              </label>
    ...
    </div>`;let f="";const $=(r.categories&&r.categories.profiles&&r.categories.profiles.items||[]).filter(x=>x.conflict&&t.sel.profiles.has(x.name));t.mode==="merge"&&$.length&&(f=`<div class="wd-field"><label>${this._t("lbl.conflict_resolution",{},"Name clashes")}</label>
        ${$.map(x=>`<div style="display:flex;align-items:center;gap:8px;margin:4px 0">
      ...
      <div class="wd-modal-actions">
        ...
        <div class="wd-field">
          ...
          <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
            ...
            <input type="date" id="wd-hist-since" value="${d(t.since||jt())}" min="${d(xe())}" max="${d(ve())}" ${b}>
        ...
        </div>`}const r=t.result||{},n=r.segments||[],a=r.parse||{},o=this._busy.has("hist-apply"),i=t.accept||new Set,l=[];a.rows_total&&l.push(this._t("msg.hist_rows_read",{n:a.rows_total},`${a.rows_total} readings read`)),a.first&&a.last&&l.push(`${rt(a.first)} \u2013 ${rt(a.last)}`),a.breaks&&l.push(this._t("msg.hist_breaks",{n:a.breaks},`${a.breaks} gaps where the sensor was unavailable`)),a.rows_other_entity&&l.push(this._t("msg.hist_other_entity",{n:a.rows_other_entity},`${a.rows_other_entity} readings for other entities ignored`)),a.entity_substituted_from&&l.push(this._t("msg.hist_entity_substituted",{used:a.entity_id||"?",wanted:a.entity_substituted_from},`read ${a.entity_id||"?"} (this device is configured for ${a.entity_substituted_from})`));const c=r.skipped||[],_={};c.forEach(x=>{_[x.reason]=(_[x.reason]||0)+1});const p=Object.entries(_).map(([x,b])=>`${b} \xD7 ${this._histSkipReason(x)}`).join(", "),f=r.settings||{},g=f.min_power!=null?this._t("msg.hist_settings_used",{w:f.min_power,s:f.off_delay},`Detected using this device's current settings (minimum power ${f.min_power} W, off delay ${f.off_delay} s).`):"";if(!n.length)return`${e}
        ...
        </div>`;const $=n.map(x=>{const b=i.has(x.index),v=this._histSegReason(x.reason);return`<tr data-hist-row="${x.index}" style="${b?"":"opacity:.55"}">
        <td><input type="checkbox" data-hist-pick="${x.index}" ${b?"checked":""} aria-label="${d(this._t("lbl.hist_keep",{},"Keep this cycle"))}"></td>
        <td>${d(rt(x.start_time))}</td>
      ...
      </tr>`}).join(""),k=n.every(x=>i.has(x.index));return`${e}
      ...
      <div class="wd-modal-actions">
        ...
        <div class="wd-modal-actions"><button class="wd-btn wd-btn-secondary" data-maction="cancel">${this._t("btn.close",{},"Close")}</button></div>`;const e=t.curve||{},s=!!e.is_reference,r=e.labelable!==!1,n=e.editable!==!1,a=e.cycle_origin==="backfill",o=e.full_duration_s||e.duration||0,i=e.energy_kwh!=null?e.energy_kwh:null,l=t.ml||null;let c="";if(l&&l.ml_quality_score!=null){const C=l.ml_quality_label,A=C==="ok"?"var(--success-color,#4caf50)":C==="uncertain"?"var(--warning-color,#ff9800)":"var(--error-color,#f44336)",F=Math.round((1-l.ml_quality_score)*100);c=`<div class="wd-kv-item"><div class="wd-kv-val" style="font-size:.95em;color:${A}">${F}%</div><div class="wd-kv-lbl">${this._t("lbl.cycle_health",{},"Cycle health")}</div></div>`}const _=`<div class="wd-kv">
      ...
      ${c}
    </div>`,p=l&&l.ml_review||{},f=(this._feedbacks||[]).some(C=>C.cycle_id===t.cycleId),g=l&&l.ml_quality_label,$=!p.reviewed_at&&(f||["uncertain","review"].includes(g)||["force_stopped","interrupted"].includes(e.status)),k=$&&t.mode!=="review"?` <span title="${this._t("hdr.automation_needs_review",{},"This cycle needs review")}" style="color:var(--warning-color,#ff9800);font-size:1.1em;line-height:0">\u25CF</span>`:"",x=this._canEdit()&&n?`<div class="wd-mode-bar">
      ...
      <button class="wd-btn wd-btn-sm ${t.mode==="review"?"wd-btn-primary":"wd-btn-secondary"}" data-maction="cyc-review" title="${$?this._t("hdr.automation_needs_review",{},"This cycle needs review"):this._t("hdr.automation_review_this_cycle",{},"Review this cycle")}">${this._t("btn.review",{},"Review")}${k}</button>
        ...
        </div>`:"";let D="";if(t.mode==="view"){const C=!!(l&&l.ml_review&&l.ml_review.golden),F=this._canEdit()&&this._onlineEnabled()&&this._storeConnected&&C?`<button class="wd-btn wd-btn-secondary" data-action="store-share-cycle" data-cid="${d(t.cycleId)}" data-prof="${d(e.profile_name||"")}">${this._t("btn.share_to_store",{},"Share to store")}</button>`:"",U=this._canEdit()?`<button class="wd-btn wd-btn-danger" data-maction="cyc-delete">${this._t("btn.delete",{},"Delete")}</button>${r?`
        <button class="wd-btn wd-btn-primary" data-maction="cyc-label">${this._t("btn.label",{},"Label")}</button>`:""}`:"";D=`${S}<div class="wd-modal-actions">
        <button class="wd-btn wd-btn-secondary" data-maction="cancel">${this._t("btn.close",{},"Close")}</button>
        ${F}
        ${U}</div>`}else if(t.mode==="trim"){const C=this._busy.has("cyc-trim-apply"),A=t.timeMode||"s",F=A==="clock"?this._offsetToClock(t.trim.start):Math.round(t.trim.start),U=A==="clock"?this._offsetToClock(t.trim.end):Math.round(t.trim.end),st=A==="clock"?"time":"number",tt=A==="clock"?'step="1"':`min="0" max="${Math.ceil(o)}" step="1"`,X=A==="clock"?"":" "+this._t("lbl.unit_s",{},"(s)");D=`<p class="wd-info" style="margin:4px 0 8px">${this._t("msg.trim_intro",{},"Drag the red handles, or enter values. Everything outside the window is removed.")}</p>
        ...
        <div class="wd-form-grid">
          <div class="wd-field"><label>${this._t("lbl.start",{},"Start")}${X}</label><input type="${st}" id="wd-trim-start" ${tt} value="${F}"></div>
          <div class="wd-field"><label>${this._t("lbl.end",{},"End")}${X}</label><input type="${st}" id="wd-trim-end" ${tt} value="${U}"></div>
        </div>
        <div class="wd-modal-actions">
          <button class="wd-btn wd-btn-secondary" data-maction="cancel">${this._t("btn.close",{},"Close")}</button>
        ...
        </div>`}else if(t.mode==="split"){const C=this._busy.has("cyc-split-apply"),A=(t.split.offsets||[]).slice().sort((st,tt)=>st-tt),F=[0,...A,o],U=F.slice(0,-1).map((st,tt)=>{const X=F[tt+1];return`<div class="wd-seg-row"><span class="wd-swatch" style="background:${it[tt%it.length]}"></span>
          <span style="min-width:120px">${_t(st)} \u2013 ${_t(X)}</span>
          <select data-segidx="${tt}"><option value="">${this._t("lbl.unlabelled_paren",{},"(unlabelled)")}</option>${this._profileOptions(t.split.profiles[tt])}</select></div>`}).join("");D=`<p class="wd-info" style="margin:4px 0 8px">${this._t("msg.split_intro",{},"Click the graph to add or remove a split point, or auto-detect by idle gaps. Each resulting segment can get its own profile.")}</p>
        <div class="wd-mode-bar">
        ...
        <div style="margin:10px 0">${A.length?U:`<p class="wd-info">${this._t("msg.no_split_points",{},"No split points yet.")}</p>`}</div>
        <div class="wd-modal-actions">
          <button class="wd-btn wd-btn-secondary" data-maction="cancel">${this._t("btn.close",{},"Close")}</button>
          <button class="wd-btn wd-btn-primary" data-maction="cyc-apply-split" ${C||!A.length?"disabled":""}>${C?'<span class="wd-spin"></span> '+this._t("status.splitting",{},"Splitting\u2026"):this._t("btn.apply_split",{},"Apply Split")}</button>
        </div>`}else if(t.mode==="review"){const C=l&&l.ml_review||{},A=this._busy.has("cyc-review-save"),F=(J,ot)=>`<option value="${J}" ${(C.quality||"")===J?"selected":""}>${ot}</option>`,st=[["late_start",this._t("tag.late_start",{},"Late start")],["early_end",this._t("tag.early_end",{},"Early end")],["merged",this._t("tag.merged",{},"Merged cycles")],["split",this._t("tag.split",{},"Split cycle")],["noise",this._t("tag.noise",{},"Noise")],["wrong_profile",this._t("tag.wrong_profile",{},"Wrong profile")],["sensor_gap",this._t("tag.sensor_gap",{},"Sensor gap")]].map(([J,ot])=>`<label class="wd-rev-tag"><input type="checkbox" class="wd-cyc-rev-tag" value="${J}" ${(C.tags||[]).includes(J)?"checked":""}> ${ot}</label>`).join(""),tt=C.reviewed_at?`<span style="font-size:.75em;color:var(--secondary-text-color)">${this._t("lbl.reviewed_on",{date:new Date(C.reviewed_at).toLocaleDateString()},`reviewed ${new Date(C.reviewed_at).toLocaleDateString()}`)}</span>`:"",X=lt(this._t("msg.review_profile_tip",{},"The program this cycle is labelled as. If the auto-detected program was wrong, correct it here - labelling teaches matching for future cycles.")),Q=lt(this._t("msg.review_quality_tip",{},"How clean this cycle is. Good = a textbook example of this program; Bad = detected but noisy or atypical; Unusable = mis-detected (merged, truncated or spurious). Drives the health score and which cycles are allowed to train the model.")),vt=lt(this._t("msg.review_recorded_tip",{},'Mark this as a hand-picked reference cycle for its program - the same role as a manually recorded cycle. Reference cycles are always kept, seed the matching template, and are never dropped by cleanup. (This is the "golden"/recorded flag; both are the same thing.)')),Y=lt(this._t("msg.review_tags_tip",{},"Optional flags describing what went wrong with this cycle, so training and cleanup can account for it.")),dt=lt(this._t("msg.review_notes_tip",{},"Free-text notes for your own reference. Not used by matching or training."));D=`
        ${S}
        ...
        <div style="display:flex;gap:16px;flex-wrap:wrap;align-items:center;margin:6px 0">
          ...
          <label style="display:inline-flex;align-items:center;gap:6px"><input type="checkbox" id="wd-cyc-rev-golden" ${C.golden?"checked":""}> ${this._t("badge.golden_cycle",{},"Recorded reference cycle")}${vt}</label>
          ${tt}
        </div>
        <div class="wd-rev-sub">${this._t("lbl.compare_profiles",{},"Compare with profiles")}${lt(this._t("msg.compare_profiles_tip",{},"Overlay other profile envelopes on the chart above to see which one best fits this cycle."))}</div>
        <div class="wd-rev-tags">${(this._profiles||[]).map(J=>{const ot=(t.overlays||[]).includes(J.name),ht=ot?`<span style="display:inline-block;width:10px;height:10px;border-radius:2px;background:${it[Math.max(0,this._profiles.findIndex(pt=>pt.name===J.name))%it.length]};margin:0 2px"></span>`:"";return`<label class="wd-rev-tag"><input type="checkbox" class="wd-cyc-overlay" value="${d(J.name)}" ${ot?"checked":""}> ${ht}${d(J.name)}</label>`}).join("")||`<span class="wd-info">${this._t("msg.no_profiles_compare",{},"No profiles to compare.")}</span>`}</div>
        <div class="wd-rev-sub">${this._t("lbl.tags",{},"Tags")}${Y}</div>
        <div class="wd-rev-tags">${st}</div>
        <div class="wd-rev-sub">${this._t("lbl.notes",{},"Notes")}${dt}</div>
        ...
        </div>`}let z="";const O=t.mode==="view"||t.mode==="review"?e.artifacts||[]:[];if(O.length){const C=O.map(A=>{const F=A.detail_key?this._t(A.detail_key,A.detail_params||{},A.detail||""):A.detail||"";return`<li><b>${d(ne(A.type,(U,st,tt)=>this._t(U,st,tt)))}</b> ${this._t("lbl.at",{},"at")} ${zt(A.start_s)}\u2013${zt(A.end_s)} \u2014 ${d(F)}</li>`}).join("");z=`<div class="wd-card" style="margin:10px 0 0;padding:10px 12px;border-left:3px solid var(--warning-color,#ff9800)">
        <div style="font-weight:600;font-size:.9em">\u26A0 ${this._t("msg.artifact_header",{n:O.length},`${O.length} anomal${O.length>1?"ies":"y"} detected during this cycle`)}</div>
      ...
      </div>`}let R="";const I=t.mode==="view"||t.mode==="review"?e.restart_gaps||[]:[];if(I.length){const C=I.map(A=>{const F=Math.round((A.gap_seconds||0)/60),U=F>=1?`${F}m`:`${Math.round(A.gap_seconds||0)}s`,st=A.match_confidence!=null?` \xB7 ${this._t("lbl.pct_match_confidence",{pct:Math.round(A.match_confidence*100)},`${Math.round(A.match_confidence*100)}% match confidence`)}`:"",tt=A.profile?` (${d(A.profile)})`:"";return`<li>${this._t("msg.restart_gap_item",{dur:U},`${U} gap`)}: ${this._t("lbl.ha_restarted",{},"HA restarted")}${tt}${st}</li>`}).join("");R=`<div class="wd-card" style="margin:10px 0 0;padding:10px 12px;border-left:3px solid var(--info-color,#2196f3)">
      ...
      </div>`}let P="";if(e.decimated){const C=(e.samples||[]).length,A=e.sample_count||C;P=`<div class="wd-info" style="margin:4px 0 0;font-size:.72em;color:var(--secondary-text-color)">${this._t("msg.samples_decimated",{shown:C,total:A},`Showing ${C} of ${A} samples (thinned for display; peaks kept). A wide gap here is thinning, not missing data.`)}</div>`}return`<h2>${this._t("lbl.cycle",{},"Cycle")} \xB7 ${d(rt(e.start_time))}</h2>
      ${_}${x}
      ...
      ${z}
      ${R}
      ${D}`}_htmlProfilePanel(t){const e=this._canEdit();t.tab==="danger"&&!e&&(t.tab="stats");const s=[["stats",this._t("tab.pp_overview",{},"Overview")],["phases",this._t("tab.pp_phases",{},"Phases")],["cleanup",this._t("tab.pp_cleanup",{},"Cleanup")]];e&&s.push(["danger",this._t("tab.pp_manage",{},"Manage")]);const r=s.map(([o,i])=>`<button class="wd-mini-tab ${t.tab===o?"active":""}" data-maction="pp-tab-${o}">${i}</button>`).join("");let n="";if(!t.loaded)n=`<div class="wd-empty" style="padding:32px"><div class="wd-icon">\u23F3</div>${this._t("msg.loading",{},"Loading\u2026")}</div>`;else if(t.tab==="stats"){const o=t.stats||{},i=t.env||{},l=this._hass&&this._hass.config&&this._hass.config.currency||"",c=o.avg_energy!=null&&o.cycle_count?o.avg_energy*o.cycle_count:null,_=k=>k?Math.round(k/60)+"m":"-",p=(this._profileHealth||{})[t.name],f=(this._profileTrends||{})[t.name],g=p&&p.health_status!=="unknown"?(()=>{const k={healthy:["var(--success-color,#4caf50)","rgba(76,175,80,.12)"],fair:["var(--warning-color,#ff9800)","rgba(255,152,0,.12)"],poor:["var(--error-color,#f44336)","rgba(244,67,54,.12)"]},[x,b]=k[p.health_status]||k.fair,v=Math.round((p.health_score||0)*100),S=p.duration_cv!=null?` \xB7 ${this._t("stat.duration_cv",{pct:Math.round(p.duration_cv*100)},`duration CV ${Math.round(p.duration_cv*100)}%`)}`:"",D=p.confidence_mean!=null?` \xB7 ${this._t("stat.avg_confidence",{pct:Math.round(p.confidence_mean*100)},`avg confidence ${Math.round(p.confidence_mean*100)}%`)}`:"";return`<div style="margin:8px 0 4px;padding:8px 12px;border-radius:6px;background:${b};border:1px solid color-mix(in srgb, ${x} 13%, transparent);display:flex;align-items:center;gap:8px;flex-wrap:wrap">
          <span style="font-weight:600;color:${x}">${p.health_status==="poor"?this._t("health.poor",{},"\u26A0 Poor match fit"):p.health_status==="fair"?this._t("health.fair",{},"Fair match fit"):this._t("health.good",{},"\u2713 Good match fit")}</span>
          <span style="font-size:.85em;opacity:.8">${this._t("stat.score",{pct:v},`score ${v}%`)}${S}${D}</span>
        ...
        </div>`})():"",$=f&&(f.duration_trend!=="stable"||f.energy_trend&&f.energy_trend!=="stable")?(()=>{const k=[];f.duration_trend==="up"?k.push(this._t("msg.trend_duration_longer",{pct:`${f.duration_slope_pct>0?"+":""}${f.duration_slope_pct}`,avg:`${Math.round(f.duration_recent_mean_s/60)}m`},`Duration trending longer (${f.duration_slope_pct>0?"+":""}${f.duration_slope_pct}%/cycle) \u2014 recent avg ${Math.round(f.duration_recent_mean_s/60)}m`)):f.duration_trend==="down"&&k.push(this._t("msg.trend_duration_shorter",{pct:`${f.duration_slope_pct}`,avg:`${Math.round(f.duration_recent_mean_s/60)}m`},`Duration trending shorter (${f.duration_slope_pct}%/cycle) \u2014 recent avg ${Math.round(f.duration_recent_mean_s/60)}m`)),f.energy_trend==="up"?k.push(this._t("msg.trend_energy_up",{pct:`${f.energy_slope_pct>0?"+":""}${f.energy_slope_pct}`,avg:yt(f.energy_recent_mean_wh)},`Energy trending up (${f.energy_slope_pct>0?"+":""}${f.energy_slope_pct}%/cycle) \u2014 recent avg ${yt(f.energy_recent_mean_wh)}`)):f.energy_trend==="down"&&k.push(this._t("msg.trend_energy_down",{pct:`${f.energy_slope_pct}`},`Energy trending down (${f.energy_slope_pct}%/cycle)`));const x=f.duration_trend==="up"||f.energy_trend==="up";return`<div style="margin:6px 0;padding:8px 12px;border-radius:6px;background:${x?"rgba(255,152,0,.10)":"rgba(33,150,243,.10)"};font-size:.88em">
          <span style="font-weight:600;color:${x?"var(--warning-color,#ff9800)":"var(--info-color,#2196f3)"}">${this._t("msg.performance_trend",{n:f.cycle_count},`Performance trend (${f.cycle_count} cycles)`)}</span><br>
          ${k.map(S=>`<span>${S}</span>`).join("<br>")}
          ...
          <div class="wd-sg">
            ...
            <div class="wd-sg-sub">${this._t("stat.last_run",{v:o.last_run?rt(o.last_run):"-"},`last run ${o.last_run?rt(o.last_run):"-"}`)}</div>
          </div>
        </div>
        ${g}
        ${$}
        ${p&&p.shape_drift?(()=>{const k=p.shape_drift_correlation!=null?` (r=${Number(p.shape_drift_correlation).toFixed(2)})`:"";return`<div style="margin-top:8px;padding:8px 10px;background:color-mix(in srgb, var(--warning-color,#ff9800) 9%, transparent);border-radius:6px;border-left:3px solid var(--warning-color,#ff9800)">
        ...
        ${i.avg&&i.avg.length?`<div class="wd-canvas-wrap"><canvas id="wd-env-canvas" role="img" aria-label="${d(this._t("lbl.aria_envelope_chart",{},"Profile power envelope chart"))}"></canvas></div>`:`<p class="wd-info">${this._t("msg.no_envelope",{},"No envelope yet - rebuild after labelling cycles.")}</p>`}`}else if(t.tab==="phases"){const o=t.catalog||[],i=(t.phases||[]).map((c,_)=>{const p=o.map(f=>`<option value="${d(f)}" ${c.name===f?"selected":""}>${d(f)}</option>`).join("");return`<div class="wd-phase-row"><span class="wd-swatch" style="background:${it[_%it.length]}"></span>
        ...
        ${e?`<div class="wd-mode-bar">
          ...
          <td style="width:26px;padding:6px 4px"><input type="checkbox" data-cleanidx="${S}" ${i.has(b.cycle_id)?"checked":""}></td>
          <td style="width:10px;padding:6px 2px"><span class="wd-swatch" style="background:${it[S%it.length]}"></span></td>
          <td class="wd-tc-date">${rt(b.start_time)}</td>
        ...
        ${e?"<th></th>":""}
      </tr></thead>`,$=this._busy.has("pp-cleanup-del"),k=this._busy.has("pp-cleanup-unlabel"),x=$||k;n=`<p class="wd-info" style="margin-bottom:10px">${this._t("msg.cleanup_intro",{},"Every labelled cycle overlaid. Tick outliers, then unlabel them (kept in history, but no longer shaping this profile) or delete them outright.")}</p>
        ...
        ${o.length?`<div class="wd-table-wrap" style="max-height:420px;overflow:auto;margin:10px 0"><table class="wd-table">${g}<tbody>${f}</tbody></table></div>`:""}
        ${e?`<div class="wd-modal-actions">
          ...
          <button class="wd-btn wd-btn-danger" data-maction="pp-cleanup-del" ${x||i.size===0?"disabled":""}>${$?'<span class="wd-spin"></span> '+this._t("status.deleting",{},"Deleting\u2026"):this._t("btn.delete_selected",{n:i.size},`Delete selected (${i.size})`)}</button>
        </div>`:""}`}else if(t.tab==="danger"){const o=this._busy.has("pp-rebuild"),i=t.stats&&t.stats.avg_duration?Math.round(t.stats.avg_duration/60):0;n=`<div class="wd-field"><label>${this._t("lbl.rename_profile",{},"Rename Profile")}</label><input type="text" id="wd-pp-rename" value="${d(t.name)}"></div>
      ...
      <div class="wd-modal-actions" style="margin-top:14px">
        <button class="wd-btn wd-btn-secondary" data-maction="cancel">${this._t("btn.close",{},"Close")}</button>
        ${a}
      </div>`}_drawCycleEditor(){const t=this._modal;if(!t||t.type!=="cycle-detail"||!t.loaded)return;const e=t.curve||{},s=e.samples||[];if(!s.length)return;let r=e.full_duration_s||s[s.length-1][0]||1;const n=[],a=t.profileEnv;if((t.mode==="view"||t.mode==="review")&&(e.expected||[]).length>1?n.push({points:e.expected,stroke:"#ff9800",width:2,alpha:.45,name:`${this._t("lbl.expected",{},"Expected")} (${e.profile_name||"profile"})`}):(t.mode==="view"||t.mode==="review")&&a&&(a.avg||[]).length&&(n.push({points:a.avg,stroke:"#ff9800",width:2,alpha:.45,name:`${this._t("lbl.expected",{},"Expected")} (${e.profile_name||"profile"})`}),r=Math.max(r,a.target_duration||a.avg[a.avg.length-1][0]||0)),t.mode==="review"&&(t.overlays||[]).length){const c=this._profileEnvCache||{};(t.overlays||[]).forEach(_=>{const p=c[_];if(!p||!(p.avg||[]).length)return;const f=it[Math.max(0,(this._profiles||[]).findIndex($=>$.name===_))%it.length];n.push({points:p.avg,stroke:f,width:1.6,alpha:.7,name:_});const g=p.avg[p.avg.length-1];r=Math.max(r,p.target_duration||(g?g[0]:0))})}n.push({points:s,stroke:"primary",fill:!0,width:2,name:this._t("lbl.power",{},"Power")});const o=[],i=[];let l=[];if(t.mode==="trim"){const c=t.trim.start,_=t.trim.end;o.push({x0:0,x1:c,fill:"rgba(244,67,54,.18)"}),o.push({x0:_,x1:r,fill:"rgba(244,67,54,.18)"}),i.push({x:c,color:"#f44336",label:"S"},{x:_,color:"#f44336",label:"E"})}else if(t.mode==="split")(t.split.offsets||[]).slice().sort((c,_)=>c-_).forEach((c,_)=>i.push({x:c,color:"#ff9800",label:"#"+(_+1)}));else{l=e.artifacts||[];const c={pause:"rgba(255,152,0,.22)",dip:"rgba(33,150,243,.18)",spike:"rgba(244,67,54,.18)"};l.forEach(p=>o.push({x0:p.start_s,x1:Math.max(p.end_s,p.start_s+1),fill:c[p.type]||"rgba(158,158,158,.18)"}));const _=e.start_time;(e.restart_gaps||[]).forEach(p=>{if(!_)return;const f=new Date(_).getTime(),g=Math.max(0,(new Date(p.start_ts).getTime()-f)/1e3),$=Math.max(g+1,(new Date(p.end_ts).getTime()-f)/1e3);o.push({x0:g,x1:$,fill:"rgba(96,125,139,.20)",label:"\u21BB"})})}this._drawCurves("wd-cyc-canvas",{series:n,xMax:r,bands:o,vlines:i,artifacts:l})}_htmlCompareModal(t){const e=t.ids||[],s={};(this._cycles||[]).forEach(o=>{s[o.id]=o});const r=t.hidden||new Set,n=e.map((o,i)=>{const l=s[o]||{},c=it[i%it.length],_=!r.has(o),p=!!(t.cycles&&t.cycles[o]),f=`${rt(l.start_time)||String(o).slice(0,8)} \xB7 ${Math.round((l.duration||0)/60)}m \xB7 ${d(l.profile_name||this._t("lbl.unlabelled",{},"Unlabelled"))}`;return`<label class="wd-rev-tag"><input type="checkbox" class="wd-compare-cyc" value="${d(o)}" ${_?"checked":""} ${p?"":"disabled"}><span style="display:inline-block;width:10px;height:10px;border-radius:2px;background:${c};margin:0 2px"></span>${f}${p?"":` <span class="wd-info">${this._t("lbl.loading_paren",{},"(loading\u2026)")}</span>`}</label>`}).join(""),a=(this._profiles||[]).map(o=>{const i=(t.overlays||[]).includes(o.name),l=it[Math.max(0,(this._profiles||[]).findIndex(_=>_.name===o.name))%it.length],c=i?`<span style="display:inline-block;width:10px;height:10px;border-radius:2px;background:${l};margin:0 2px;opacity:.55"></span>`:"";return`<label class="wd-rev-tag"><input type="checkbox" class="wd-compare-overlay" value="${d(o.name)}" ${i?"checked":""}> ${c}${d(o.name)}</label>`}).join("")||`<span class="wd-info">${this._t("msg.no_profiles_overlay",{},"No profiles to overlay.")}</span>`;return`<h2>${this._t("msg.compare_cycles_title",{count:e.length},`Compare ${e.length} cycles`)}</h2>
      ...
      <div class="wd-rev-sub">${this._t("msg.compare_overlay_profiles",{},"Overlay profiles (faint)")}${lt(this._t("msg.compare_overlay_tip",{},"Overlay learned profile envelopes to see which program each cycle resembles."))}</div>
      <div class="wd-rev-tags">${a}</div>
      <div class="wd-modal-actions" style="margin-top:16px">
        <button class="wd-btn wd-btn-secondary" data-maction="cancel">${this._t("btn.close",{},"Close")}</button>
      </div>`}_drawCompareCanvas(){const t=this._modal;if(!t||t.type!=="compare-cycles")return;const e=t.ids||[],s={};(this._cycles||[]).forEach(i=>{s[i.id]=i});const r=t.hidden||new Set,n=[];let a=0;const o=this._profileEnvCache||{};(t.overlays||[]).forEach(i=>{const l=o[i];if(!l||!(l.avg||[]).length)return;const c=it[Math.max(0,(this._profiles||[]).findIndex(p=>p.name===i))%it.length];n.push({points:l.avg,stroke:c,width:2,alpha:.4,name:i});const _=l.avg[l.avg.length-1];a=Math.max(a,l.target_duration||(_?_[0]:0))}),e.forEach((i,l)=>{if(r.has(i))return;const c=t.cycles&&t.cycles[i],_=c&&c.samples;if(!_||!_.length)return;const p=it[l%it.length],f=s[i]||{};n.push({points:_,stroke:p,width:1.8,alpha:.9,name:rt(f.start_time)||String(i).slice(0,8)}),a=Math.max(a,c.full_duration_s||_[_.length-1][0]||0)}),this._drawCurves("wd-compare-canvas",{series:n,xMax:a||1})}_drawProfileEnvelope(){const t=this._modal;if(!t||!t.env||!(t.env.avg||[]).length)return;const e=t.env;this._drawCurves("wd-env-canvas",{series:[{points:e.avg,stroke:"primary",width:2,name:"Average"}],band:{min:e.min,max:e.max},xMax:e.target_duration||e.avg[e.avg.length-1][0]})}_drawPhaseEditor(){const t=this._modal;if(!t||!t.env||!(t.env.avg||[]).length)return;const e=t.env,s=e.target_duration||e.avg[e.avg.length-1][0],r=(t.phases||[]).map((a,o)=>({x0:a.start,x1:a.end,fill:Pt(it[o%it.length],.2)})),n=[];(t.phases||[]).forEach((a,o)=>{const i=it[o%it.length];n.push({x:a.start,color:i,label:a.name?a.name.slice(0,7):"",handle:!0}),n.push({x:a.end,color:i,handle:!0})}),this._drawCurves("wd-phase-canvas",{series:[{points:e.avg,stroke:"primary",width:2,name:"Average"}],band:{min:e.min,max:e.max},bands:r,vlines:n,xMax:s})}_drawSpaghetti(){const t=this._modal;if(!t||!t.cleanup||!(t.cleanup.cycles||[]).length)return;const e=t.cleanup.cycles,s=t.cleanup.selected||new Set,r=this._spagTableHoverCid||null;let n=1;e.forEach(o=>{const i=o.samples||[];i.length&&i[i.length-1][0]>n&&(n=i[i.length-1][0])});const a=e.map((o,i)=>{const l=s.has(o.cycle_id),c=r===o.cycle_id;return{points:o.samples||[],stroke:it[i%it.length],width:l||c?2.6:1,alpha:s.size?l?1:.22:r?c?1:.22:.7,name:rt(o.start_time),cid:o.cycle_id}});this._drawCurves("wd-spag-canvas",{series:a,xMax:n})}_wire(){const t=this.shadowRoot;if(!t)return;const e=t.getElementById("wd-burger");e&&e.addEventListener("click",()=>{this.dispatchEvent(new CustomEvent("hass-toggle-menu",{bubbles:!0,composed:!0}))}),t.querySelectorAll(".wd-devcard[data-idx]").forEach(y=>y.addEventListener("click",()=>this._selectDevice(parseInt(y.dataset.idx,10)))),t.querySelectorAll("[data-tab]").forEach(y=>y.addEventListener("click",()=>{y.dataset.tab!=="settings"&&(this._pendingSettings={},this._dirtyOptKeys=new Set),this._tab=y.dataset.tab,this._fetchTabData()})),t.querySelectorAll("[data-sec]").forEach(y=>y.addEventListener("click",()=>{this._snapshotFormToPending(t),this._settingsSec=y.dataset.sec,this._settingsSearch="",this._settingsSugOnly=!1,this._render()})),t.querySelectorAll("[data-ptab]").forEach(y=>y.addEventListener("click",()=>{const m=this._panelSubtab=y.dataset.ptab;this._render();const u=this._devices[this._selIdx];u&&(m==="diagnostics"&&!this._diag?this._fetchToolsData(u.entry_id).then(()=>{this._panelSubtab==="diagnostics"&&this._render()}):m==="logs"?this._fetchLogs().then(()=>{this._panelSubtab==="logs"&&this._render()}):m==="maintenance"?this._fetchMaintenance(u.entry_id).then(()=>{this._panelSubtab==="maintenance"&&this._render()}):m==="ml"&&this._fetchTabData())})),t.querySelectorAll("[data-gtab]").forEach(y=>y.addEventListener("click",()=>{this._gearTab=y.dataset.gtab,this._modal&&this._modal.type==="gear-settings"&&(this._modal.tab=this._gearTab);const m=this._devices[this._selIdx];this._gearTab==="online"&&m&&this._onlineEnabled()&&(this._ensureStoreConnectListener(),this._loadStoreStatus(m.entry_id).then(()=>{this._modal&&this._modal.type==="gear-settings"&&this._render()})),this._render()}));const s=(y,m)=>{this._pendingSettings[y]=m,this._dirtyOptKeys.add(y)},r=t.getElementById("wd-store-brand");r&&r.addEventListener("change",()=>{const y=r.value.trim();if(y!==String(this._editedOpts().store_brand||"").trim()){s("store_model","");const m=t.getElementById("wd-store-model");m&&(m.value="")}s("store_brand",y),this._catalog.forBrand=y,this._catalog.devices=void 0,this._renderPreservingFormEdits()});const n=t.getElementById("wd-store-model");n&&n.addEventListener("change",()=>{s("store_model",n.value.trim()),this._renderPreservingFormEdits()});const a=t.getElementById("wd-pg-canvas");if(a){const y=()=>{const M=a.getBoundingClientRect(),K=M.height||330,et=Ht+8,nt=K-et-52;return{rect:M,padT:et,powerH:nt}},m=(M,K)=>{const et=a.getBoundingClientRect(),nt=M-et.left,at=K-et.top;if(at>Ht+4)return null;const ct=this._pgEventHits||[];let T=null,B=1/0;for(const Z of ct){const q=(nt-Z.cx)**2+(at-Z.cy)**2;q<=(Z.r+4)**2&&q<B&&(T=Z,B=q)}return T},u=M=>{const{rect:K,padT:et,powerH:nt}=y(),at=this._pgPowerPts;if(!at?.length)return 0;const ct=Math.max(...at.map(T=>T.w),1);return Math.max(0,(1-Math.max(0,M-K.top-et)/nt)*ct)},E=M=>{const{rect:K,padT:et,powerH:nt}=y(),at=this._pgPowerPts;if(!at?.length)return 0;const ct=Math.max(...at.map(T=>T.w),1);return K.top+et+(1-Math.max(0,+M)/ct)*nt},L=M=>{const K=this._pgMap;if(!K)return null;const{rect:et}=y(),nt=(M-et.left-K.padLpx)/Math.max(1,K.plotWpx);return K.vMin+Math.max(0,Math.min(1,nt))*(K.vMax-K.vMin)};a.addEventListener("pointermove",M=>{if(H.has(M.pointerId)&&H.set(M.pointerId,{x:M.clientX,y:M.clientY}),a._wdPinching){G();return}if(this._pgDragging==="start_thr"){this._pgThreshStart=Math.max(0,u(M.clientY)),this._pgUpdateParamInput("start_threshold_w",this._pgThreshStart),this._pgDrawCanvas();return}if(this._pgDragging==="stop_thr"){this._pgThreshStop=Math.max(0,u(M.clientY)),this._pgUpdateParamInput("stop_threshold_w",this._pgThreshStop),this._pgDrawCanvas();return}if(this._pgDragging==="pan"&&this._pgPanStart){const T=this._pgMap,B=(M.clientX-this._pgPanStart.clientX)/Math.max(1,T.plotWpx),Z=this._pgPanStart.vMax-this._pgPanStart.vMin;let q=this._pgPanStart.vMin-B*Z;q=Math.max(0,Math.min(this._pgPanStart.totalDur-Z,q)),this._pgView={min:q,max:q+Z},this._pgDrawCanvas();return}if(!this._pgPowerPts?.length)return;const K=m(M.clientX,M.clientY);if(K){a.style.cursor="pointer",this._pgHoverEvent={t:K.t,type:K.type},this._pgHoverT=null,this._pgDrawCanvas();return}this._pgHoverEvent&&(this._pgHoverEvent=null,this._pgDrawCanvas());const et=this._pgThreshStart??this._pgFieldVal("start_threshold_w",{})??50,nt=this._pgThreshStop??this._pgFieldVal("stop_threshold_w",{})??5,at=Math.abs(M.clientY-E(et))<8||Math.abs(M.clientY-E(nt))<8;a.style.cursor=at?"ns-resize":"crosshair";const ct=L(M.clientX);ct!=null&&(this._pgHoverT=ct,this._pgUpdateStripAt(ct),this._pgDrawCanvas())});const W=(M,K)=>{const et=this._pgMap;if(!et||M==null)return;const nt=et.vMax-et.vMin,at=Math.max(30,Math.min(et.totalDur,nt*K)),ct=nt>0?Math.max(0,Math.min(1,(M-et.vMin)/nt)):.5;let T=M-ct*at;T=Math.max(0,Math.min(et.totalDur-at,T)),this._pgView=at>=et.totalDur-1?null:{min:T,max:T+at},this._pgDrawCanvas()},H=new Map;let N=null;a.addEventListener("pointerdown",M=>{if(!this._pgPowerPts?.length)return;if(H.set(M.pointerId,{x:M.clientX,y:M.clientY}),H.size===2&&this._pgMap){a._wdPinching=!0,a._wdOwnGesture=!1,this._pgDragging=null,this._pgPanStart=null,a.classList.remove("wd-pg-panning");const[at,ct]=[...H.values()];N={dist:Math.max(1,Math.abs(at.x-ct.x)),centerX:(at.x+ct.x)/2};return}if(H.size!==1||m(M.clientX,M.clientY))return;a.setPointerCapture(M.pointerId);const K=this._pgThreshStart??this._pgFieldVal("start_threshold_w",{})??50,et=this._pgThreshStop??this._pgFieldVal("stop_threshold_w",{})??5,nt=M.pointerType==="touch"?18:10;Math.abs(M.clientY-E(K))<nt?this._pgDragging="start_thr":Math.abs(M.clientY-E(et))<nt?this._pgDragging="stop_thr":this._pgMap&&(this._pgDragging="pan",this._pgPanStart={clientX:M.clientX,vMin:this._pgMap.vMin,vMax:this._pgMap.vMax,totalDur:this._pgMap.totalDur},a.classList.add("wd-pg-panning")),a._wdOwnGesture=this._pgDragging==="start_thr"||this._pgDragging==="stop_thr"});const V=M=>{try{a.releasePointerCapture(M.pointerId)}catch{}H.delete(M.pointerId),H.size<2&&(N=null,a._wdPinching=!1),H.size===0&&(a._wdOwnGesture=!1);const K=this._pgDragging==="start_thr"||this._pgDragging==="stop_thr";return this._pgDragging=null,this._pgPanStart=null,a.classList.remove("wd-pg-panning"),K};a.addEventListener("pointerup",M=>{V(M)&&this._pgDrawCanvas()}),a.addEventListener("pointercancel",M=>{V(M)&&this._pgDrawCanvas(),this._pgHoverT=null,this._pgHoverEvent=null,this._pgUpdateStripAt(null),this._pgDrawCanvas()}),a.addEventListener("pointerleave",M=>{this._pgDragging||M.pointerType==="touch"||(this._pgHoverT=null,this._pgHoverEvent=null,this._pgUpdateStripAt(null),this._pgDrawCanvas())}),a.addEventListener("wheel",M=>{if(!this._pgPowerPts?.length||!this._pgMap)return;M.preventDefault();const K=M.ctrlKey?Math.min(4,Math.max(.25,Math.exp(M.deltaY*.01))):M.deltaY>0?1.25:.8;W(L(M.clientX),K)},{passive:!1}),a.addEventListener("dblclick",M=>{if(this._pgView){this._pgView=null,this._pgDrawCanvas();return}W(L(M.clientX),Xt)});const G=()=>{const M=this._pgMap;if(!M||!N||H.size<2)return;const[K,et]=[...H.values()],nt=Math.max(1,Math.abs(K.x-et.x)),at=(K.x+et.x)/2,ct=M.vMax-M.vMin,T=Math.max(30,Math.min(M.totalDur,ct*(N.dist/nt))),B=L(at),Z=(at-N.centerX)/Math.max(1,M.plotWpx)*T;if(N.dist=nt,N.centerX=at,B==null)return;const q=ct>0?Math.max(0,Math.min(1,(B-M.vMin)/ct)):.5;let j=B-q*T-Z;j=Math.max(0,Math.min(M.totalDur-T,j)),this._pgView=T>=M.totalDur-1?null:{min:j,max:j+T},this._pgDrawCanvas()};this._attachTouchOwnershipGuard(a)}t.querySelectorAll("input[data-pgkey]").forEach(y=>{y.addEventListener("input",()=>{const m=y.dataset.pgkey;if(y.dataset.pgtype==="bool"){const H=y.checked;m==="start_threshold_w"?this._pgThreshStart=H:m==="stop_threshold_w"?this._pgThreshStop=H:this._pgParamOverrides[m]=H,this._render();const N=t.querySelector(`input[data-pgkey="${m}"]`);N&&N.focus({preventScroll:!0}),requestAnimationFrame(()=>this._pgDrawCanvas());return}const u=y.value.trim();if(!u)m==="start_threshold_w"?this._pgThreshStart=null:m==="stop_threshold_w"?this._pgThreshStop=null:delete this._pgParamOverrides[m];else{const H=parseFloat(u);isNaN(H)||(m==="start_threshold_w"?this._pgThreshStart=H:m==="stop_threshold_w"?this._pgThreshStop=H:this._pgParamOverrides[m]=H)}const E=y.selectionStart,L=y.value;this._render();const W=t.querySelector(`input[data-pgkey="${m}"]`);if(W){W.value=L,W.focus({preventScroll:!0});try{W.setSelectionRange(E,E)}catch{}}requestAnimationFrame(()=>this._pgDrawCanvas())})});const o=t.getElementById("wd-pg-stress-toggle");o&&o.addEventListener("change",()=>{this._pgStressTail=o.checked,this._pgStressIdleW=null,this._render()});const i=t.getElementById("wd-pg-stress-idle-w");i&&i.addEventListener("input",()=>{const y=parseFloat(i.value);this._pgStressIdleW=Number.isFinite(y)&&y>=0?y:null});const l=t.getElementById("wd-pg-cyc-sel");l&&l.addEventListener("change",()=>this._pgSelectCycle(l.value));const c=t.getElementById("wd-pg-prof-sel");c&&c.addEventListener("change",()=>{this._pgProfileName=c.value,this._pgLoad()});const _=t.getElementById("wd-pg-preset-sel");_&&_.addEventListener("change",()=>{this._pgPresetSel=_.value,this._render()});const p=t.getElementById("wd-pg-preset-name");p&&p.addEventListener("input",()=>{this._pgPresetName=p.value;const y=(this._pgPresetName||"").trim(),m=this._pgPresetLimit>0&&(this._pgPresets||[]).length>=this._pgPresetLimit&&!(this._pgPresets||[]).some(L=>L.name===y),u=t.querySelector('[data-action="pg-preset-save"]');u&&(u.disabled=!(y&&!m));const E=t.getElementById("wd-pg-preset-limit-note");E&&(E.style.display=m?"":"none")});const f=t.getElementById("wd-pg-simn");f&&f.addEventListener("input",()=>{this._pgSimCycles=Math.max(1,Math.min(200,parseInt(f.value,10)||20))});const g=t.getElementById("wd-pg-sw-param");g&&g.addEventListener("change",()=>{this._pgSweepParam=g.value,this._pgSweepNew=null,this._render()});const $=t.getElementById("wd-pg-sw-obj");$&&$.addEventListener("change",()=>{this._pgSweepObjective=$.value,this._pgSweepNew=null,this._render()});const k=t.getElementById("wd-pg-sw-paramy"),x=t.getElementById("wd-pg-sw-from");x&&x.addEventListener("input",()=>{this._pgSweepFrom=x.value});const b=t.getElementById("wd-pg-sw-to");b&&b.addEventListener("input",()=>{this._pgSweepTo=b.value});const v=t.getElementById("wd-pg-sw-steps");v&&v.addEventListener("input",()=>{this._pgSweepSteps=parseInt(v.value,10)||5}),t.querySelectorAll("[data-statustoggle]").forEach(y=>y.addEventListener("change",async()=>{const m=y.dataset.statustoggle,u=y.checked;this._panelCfg||(this._panelCfg={}),this._panelCfg.prefs={...this._panelCfg.prefs||{},[m]:u},this._ws({type:`${w}/set_user_prefs`,prefs:{[m]:u}}).catch(()=>{});const E=this._devices[this._selIdx];if(E&&this._tab==="status")try{this._powerData=await this._ws({type:`${w}/get_power_history`,entry_id:E.entry_id,with_raw:this._pref("show_raw_active",!1)})}catch{}this._render()})),t.querySelectorAll("[data-sortact]").forEach(y=>y.addEventListener("click",()=>{const m=y.dataset.sortact,u=y.dataset.sortcol,E=L=>{L.col===u?L.dir*=-1:(L.col=u,L.dir=u==="date"?-1:1)};m==="cycsort"?E(this._cycleSort):m==="cleanupsort"&&E(this._cleanupSort),this._render()}));const S=t.getElementById("wd-cyc-filter-text");S&&S.addEventListener("input",y=>{const m=y.target.selectionStart;this._cycleFilter.text=S.value,this._render();const u=this.shadowRoot.getElementById("wd-cyc-filter-text");u&&(u.focus({preventScroll:!0}),u.setSelectionRange(m,m))});const D=t.getElementById("wd-settings-search");D&&D.addEventListener("input",y=>{const m=y.target.selectionStart;this._settingsSearch=D.value,D.value.trim()&&(this._settingsSugOnly=!1),this._render();const u=this.shadowRoot.getElementById("wd-settings-search");u&&(u.focus({preventScroll:!0}),u.setSelectionRange(m,m))});const z=t.getElementById("wd-cyc-filter-status");z&&z.addEventListener("change",()=>{this._cycleFilter.status=z.value,this._render()}),t.querySelectorAll(".wd-pillbox").forEach(y=>{const m=y.querySelector(".wd-pill-add"),u=L=>{const W=document.createElement("span");W.className="wd-pill",W.dataset.val=L,W.appendChild(document.createTextNode(L));const H=document.createElement("button");return H.type="button",H.className="wd-pill-x",H.setAttribute("aria-label","Remove"),H.textContent="\xD7",H.addEventListener("click",()=>W.remove()),W.appendChild(H),W},E=L=>{const W=String(L||"").trim();if(!W)return;Array.from(y.querySelectorAll(".wd-pill")).some(N=>N.dataset.val===W)||y.insertBefore(u(W),m.closest(".wd-combo")||m),m&&(m.value="")};y.querySelectorAll(".wd-pill-x").forEach(L=>L.addEventListener("click",()=>L.closest(".wd-pill")?.remove())),m&&(m.addEventListener("change",()=>E(m.value)),m.addEventListener("keydown",L=>{L.key==="Enter"&&(L.preventDefault(),E(m.value))}),m.addEventListener("blur",()=>E(m.value)))}),t.querySelectorAll(".wd-combo").forEach(y=>{const m=y.querySelector(".wd-combo-inp, .wd-pill-add"),u=y.querySelector(".wd-combo-drop");if(!m||!u)return;const E=y.classList.contains("wd-combo-pill"),L=m.dataset.catalog||m.dataset.opt||y.closest("[data-opt]")?.dataset.opt,W=(N,V=!1)=>{this._ensureCatalogList(L,N,V);const G=(this._entityListCache||{})[L]||[],M=(N||"").toLowerCase(),K=M?G.filter(et=>et.toLowerCase().includes(M)).slice(0,40):G.slice(0,20);if(!K.length){u.hidden=!0;return}u.innerHTML=K.map(et=>`<div class="wd-combo-item" data-val="${d(et)}">${d(et)}</div>`).join(""),u._kbd=-1,u.hidden=!1,this._positionComboDrop(m,u)},H=N=>{if(N){if(E){const V=y.closest(".wd-pillbox");if(V&&!Array.from(V.querySelectorAll(".wd-pill")).some(G=>G.dataset.val===N)){const G=document.createElement("span");G.className="wd-pill",G.dataset.val=N,G.appendChild(document.createTextNode(N));const M=document.createElement("button");M.type="button",M.className="wd-pill-x",M.setAttribute("aria-label","Remove"),M.textContent="\xD7",M.addEventListener("click",()=>G.remove()),G.appendChild(M),V.insertBefore(G,y)}m.value=""}else m.value=N,m.dispatchEvent(new Event("change",{bubbles:!0}));u.hidden=!0}};m.addEventListener("focus",()=>W(m.value)),m.addEventListener("input",()=>W(m.value,!m._wdSyntheticInput)),m.addEventListener("blur",()=>setTimeout(()=>{u.hidden=!0},150)),m.addEventListener("keydown",N=>{if(u.hidden&&N.key!=="ArrowDown")return;const V=u.querySelectorAll(".wd-combo-item");let G=u._kbd||-1;if(N.key==="ArrowDown"){if(N.preventDefault(),u.hidden){W(m.value);return}G=Math.min(G+1,V.length-1)}else if(N.key==="ArrowUp")N.preventDefault(),G=Math.max(G-1,0);else if(N.key==="Enter"&&!u.hidden){N.preventDefault(),G>=0?H(V[G].dataset.val):E&&m.value.trim()&&H(m.value.trim());return}else if(N.key==="Escape"){u.hidden=!0;return}else return;u._kbd=G,V.forEach((M,K)=>M.classList.toggle("kbd",K===G)),V[G]?.scrollIntoView({block:"nearest"})}),u.addEventListener("mousedown",N=>{const V=N.target.closest(".wd-combo-item");V&&(N.preventDefault(),H(V.dataset.val))})});const O=t.getElementById("wd-store-q");if(O){const y=()=>{const m=String(O.value||"").trim(),u=(this._catalog.modelIndex||new Map).get(m);if(u){this._storeOpenModel(u);return}m.toLowerCase()!==String(this._storeQuery||"").trim().toLowerCase()&&this._storeSearch(m)};O.addEventListener("change",y),O.addEventListener("keydown",m=>{if(m.key!=="Enter")return;const u=O.parentElement&&O.parentElement.querySelector(".wd-combo-drop");u&&!u.hidden&&u._kbd>=0||(m.preventDefault(),y())}),O.addEventListener("input",()=>{O._wdSyntheticInput||(clearTimeout(this._storeQTimer),this._storeQTimer=setTimeout(()=>{const m=String(O.value||"").trim();if(!m)return;((this._entityListCache||{}).store_brand||[]).some(E=>String(E).toLowerCase()===m.toLowerCase())&&y()},450))})}t.querySelectorAll(".wd-timerlist").forEach(y=>{const m=y.dataset.opt,u=()=>{if(!Array.isArray(this._pendingSettings[m])){const V=Array.isArray(this._opts&&this._opts[m])?this._opts[m]:[];this._pendingSettings[m]=V.map(G=>({...G}))}return this._pendingSettings[m]},E=V=>({offset_minutes:parseFloat(V.querySelector('[data-field="offset_minutes"]').value)||0,message:(V.querySelector('[data-field="message"]').value||"").trim(),auto_pause:V.querySelector('[data-field="auto_pause"]').checked}),L=V=>{const G=parseInt(V.dataset.tidx,10);if(isNaN(G))return;const M=u();M[G]=E(V)},W=V=>{const G=parseInt(V.dataset.tidx,10);isNaN(G)||(this._pendingSettings[m]=u().filter((M,K)=>K!==G)),V.remove(),y.querySelectorAll(".wd-timer-row").forEach((M,K)=>{M.dataset.tidx=K})},H=V=>{const G=V.querySelector(".wd-timer-remove");G&&G.addEventListener("click",()=>W(V)),V.querySelector('[data-field="auto_pause"]')?.addEventListener("change",()=>L(V)),V.querySelectorAll('[data-field="offset_minutes"],[data-field="message"]').forEach(M=>M.addEventListener("input",()=>L(V)))};y.querySelectorAll(".wd-timer-row").forEach(V=>H(V));const N=y.querySelector(".wd-timer-add");N&&N.addEventListener("click",()=>{const V=y.querySelectorAll(".wd-timer-row").length;u().push({offset_minutes:0,message:"",auto_pause:!1});const G=document.createElement("div");G.className="wd-timer-row",G.dataset.tidx=V,G.innerHTML=`<div class="wd-timer-top"><input type="number" min="1" placeholder="${this._t("lbl.timer_min",{},"min")}" data-field="offset_minutes"><textarea placeholder="${this._t("lbl.timer_msg_placeholder",{},"Message (optional, {device}/{program}/{minutes})")}" data-field="message"></textarea></div><div class="wd-timer-footer"><label class="wd-switch-lbl"><span class="wd-switch"><input type="checkbox" data-field="auto_pause"><span class="wd-switch-slider"></span></span><span class="wd-switch-text">${this._t("lbl.timer_auto_pause",{},"Auto-pause")}</span></label><button type="button" class="wd-btn wd-btn-sm wd-btn-danger wd-timer-remove">${this._t("btn.remove_timer",{},"Delete")}</button></div>`,H(G),y.insertBefore(G,N)})});const R=t.getElementById("wd-status-prog");R&&R.addEventListener("change",()=>{const y=this._devices[this._selIdx];if(!y)return;const m=R.value,u=!Rt.includes(y.detector_state||"");this._ws({type:`${w}/set_program`,entry_id:y.entry_id,program:m}).then(()=>(this._showToast(m==="auto_detect"?this._t("msg.toast_auto_detect_enabled",{},"Auto-detect enabled"):u?this._t("msg.toast_program_armed",{program:m},`Program armed for the next cycle: ${m}`):this._t("msg.toast_program_set",{program:m},`Program set: ${m}`)),this._fetchAll())).catch(E=>this._showToast(this._t("msg.toast_failed",{error:E.message||E},"Failed: "+(E.message||E)),"error"))}),t.querySelectorAll("[data-cid]").forEach(y=>y.addEventListener("click",m=>{if(m.target.tagName==="INPUT"||y.dataset.action)return;const u=y.dataset.cid;y.dataset.selmode==="1"?(this._cycleSel.has(u)?this._cycleSel.delete(u):this._cycleSel.add(u),this._render()):this._onAction({dataset:{action:"open-cycle",cid:u}})})),t.querySelectorAll(".wd-cyc-overlay").forEach(y=>y.addEventListener("change",()=>{const m=this._modal;if(!m||m.type!=="cycle-detail")return;this._snapshotCycleReviewForm(t);const u=new Set(m.overlays||[]);y.checked?u.add(y.value):u.delete(y.value),m.overlays=[...u];const E=this._devices[this._selIdx];y.checked&&E?this._ensureProfileEnvs(E.entry_id,[y.value]).then(()=>this._render()):this._render()})),t.querySelectorAll(".wd-compare-cyc").forEach(y=>y.addEventListener("change",()=>{const m=this._modal;if(!m||m.type!=="compare-cycles")return;const u=m.hidden instanceof Set?m.hidden:new Set(m.hidden||[]);y.checked?u.delete(y.value):u.add(y.value),m.hidden=u,this._render()})),t.querySelectorAll(".wd-compare-overlay").forEach(y=>y.addEventListener("change",()=>{const m=this._modal;if(!m||m.type!=="compare-cycles")return;const u=new Set(m.overlays||[]);y.checked?u.add(y.value):u.delete(y.value),m.overlays=[...u];const E=this._devices[this._selIdx];y.checked&&E?this._ensureProfileEnvs(E.entry_id,[y.value]).then(()=>this._render()):this._render()})),t.querySelectorAll(".wd-pg-mem").forEach(y=>y.addEventListener("change",()=>{const m=this._modal;if(!m||m.type!=="profile-group")return;const u=t.getElementById("wd-pg-name");u&&(m.name=u.value);const E=new Set(m.members||[]);y.checked?E.add(y.value):E.delete(y.value),m.members=[...E],this._render()}));const I=t.getElementById("wd-pg-name");I&&I.addEventListener("input",()=>{const y=this._modal;y&&y.type==="profile-group"&&(y.name=I.value)}),t.querySelectorAll(".wd-csel").forEach(y=>y.addEventListener("change",()=>{const m=y.closest("[data-cid]"),u=m&&m.dataset.cid;u&&(y.checked?this._cycleSel.add(u):this._cycleSel.delete(u),this._render())}));const P=t.getElementById("wd-merge-prof");P&&P.addEventListener("change",()=>{const y=t.getElementById("wd-merge-new");y&&(y.style.display=P.value==="__create_new__"?"":"none")}),t.querySelectorAll(".wd-log-filter").forEach(y=>{const m=y.dataset.logfilter,u={level:"_logLevel",device:"_logDevice",component:"_logComponent",search:"_logSearch"}[m];if(!u)return;const E=m==="search"?"input":"change";y.addEventListener(E,()=>{this[u]=y.value,this._refreshLogViews(),this._syncLogFilters(y)})});const C=t.querySelector(".wd-log-resize");C&&C.addEventListener("pointerdown",y=>{y.preventDefault(),C.setPointerCapture(y.pointerId),C.classList.add("dragging");const m=t.querySelector(".wd-log-drawer"),u=y.clientX,E=m.offsetWidth,L=W=>{const H=Math.max(280,Math.min(900,E+(u-W.clientX)));m.style.width=H+"px",this._logDrawerWidth=H};C.addEventListener("pointermove",L),C.addEventListener("pointerup",()=>{C.removeEventListener("pointermove",L),C.classList.remove("dragging");try{localStorage.setItem("wd-log-width",String(this._logDrawerWidth))}catch{}},{once:!0})});const A=t.getElementById("wd-hist-file");A&&A.addEventListener("change",()=>{const y=A.files&&A.files[0];if(!y)return;const m=this._modal,u=new FileReader;u.onload=()=>{const E=t.getElementById("wd-hist-csv"),L=String(u.result||"");E&&(E.value=L),m&&m.type==="history-import"&&(m.csvText=L)},u.onerror=()=>this._showToast(this._t("toast.file_read_failed",{},"Could not read that file"),"error"),u.readAsText(y)}),t.querySelectorAll("[data-hist-pick]").forEach(y=>y.addEventListener("change",()=>{const m=this._modal;if(!m||m.type!=="history-import")return;const u=parseInt(y.dataset.histPick,10);y.checked?m.accept.add(u):m.accept.delete(u),this._render(),requestAnimationFrame(()=>this._drawHistorySparklines())}));const F=t.getElementById("wd-import-file");F&&F.addEventListener("change",()=>{const y=F.files&&F.files[0];if(!y)return;const m=new FileReader;m.onload=()=>{const u=t.getElementById("wd-import-json");u&&(u.value=String(m.result||""))},m.readAsText(y)}),t.querySelectorAll("[data-stab]").forEach(y=>y.addEventListener("click",()=>{this._toolsSubtab=y.dataset.stab;const m=this._devices[this._selIdx];m&&(this._tabLoading=!0,this._render(),this._fetchToolsData(m.entry_id).then(()=>{this._tabLoading=!1,this._render()}))})),t.querySelectorAll("[data-proftab]").forEach(y=>y.addEventListener("click",async()=>{this._profSubtab=y.dataset.proftab;const m=this._devices[this._selIdx];m&&this._profSubtab==="phase-catalog"&&!this._phases.length&&(this._tabLoading=!0,this._render(),await this._fetchPhases(m.entry_id),this._tabLoading=!1),this._render()}));const U=t.getElementById("wd-settings-save");U&&U.addEventListener("click",()=>this._saveSettings());const st=t.getElementById("wd-ml-save");st&&st.addEventListener("click",()=>this._saveSettings());const tt=t.getElementById("wd-settings-form"),X=y=>{const m=y.target&&y.target.closest?y.target.closest("[data-opt]"):null;m&&m.dataset.opt&&this._dirtyOptKeys.add(m.dataset.opt)};tt&&(tt.addEventListener("submit",y=>y.preventDefault()),tt.addEventListener("input",y=>{X(y),this._liveValidateSettings(t)}),tt.addEventListener("change",y=>{X(y),this._liveValidateSettings(t)}),tt.addEventListener("click",y=>{const m=y.target.closest(".wd-conflict-fix");if(!m)return;const u=m.dataset.ckey,E=parseFloat(m.dataset.cval),L=tt.querySelector(`[data-opt="${u}"]`);L&&!isNaN(E)&&(L.value=E,this._dirtyOptKeys.add(u),this._cascadeConflictFix(t,tt,u))}),this._liveValidateSettings(t));const Q=t.getElementById("wd-ml-form");Q&&(Q.addEventListener("input",X),Q.addEventListener("change",X));const vt=t.getElementById("wd-settings-revert");vt&&vt.addEventListener("click",async()=>{if(!this._prevOpts)return;const y=this._devices[this._selIdx];y&&await this._busyRun("save-settings",async()=>{try{const m=this._prevOpts;await this._ws({type:`${w}/set_options`,entry_id:y.entry_id,options:m}),this._opts={...m},this._prevOpts=null,this._cascadePending={},this._preCascadeOpts=null,this._pendingSettings={},this._dirtyOptKeys=new Set,this._showToast(this._t("toast.settings_reverted",{},"Settings reverted; integration reloading")),this._render()}catch(m){this._showToast(this._t("msg.toast_revert_failed",{error:m.message||m},"Revert failed: "+(m.message||m)),"error")}})});const Y=t.getElementById("wd-settings-reload");Y&&Y.addEventListener("click",async()=>{const y=this._devices[this._selIdx];if(y){this._prevOpts=null,this._cascadePending={},this._preCascadeOpts=null,this._pendingSettings={},this._dirtyOptKeys=new Set;const m=await this._ws({type:`${w}/get_options`,entry_id:y.entry_id});this._opts=m.options||{},this._optDefaults=m.defaults||{},await this._fetchSuggestions(y.entry_id),this._render()}});const dt=t.getElementById("wd-pref-fontscale");if(dt){const y=t.getElementById("wd-pref-fontscale-val");dt.addEventListener("input",()=>{const m=parseFloat(dt.value)||1;this._applyFontScale(m),y&&(y.textContent=Math.round(m*100)+"%")}),dt.addEventListener("change",()=>{this._setPref("font_scale",parseFloat(dt.value)||1)})}t.querySelectorAll("[data-action]").forEach(y=>y.addEventListener("click",m=>this._onAction(m.currentTarget))),t.querySelectorAll("[data-maction]").forEach(y=>y.addEventListener("click",m=>this._onModalAction(m.currentTarget.dataset.maction,m.currentTarget))),t.querySelectorAll("select[data-maction]").forEach(y=>y.addEventListener("change",m=>this._onModalAction(m.currentTarget.dataset.maction,m.currentTarget))),t.querySelectorAll("input[data-indeterminate]").forEach(y=>{y.indeterminate=!0});const J=t.querySelector("[data-toast-undo]");J&&J.addEventListener("click",()=>this._undoDelete(J.dataset.toastUndo)),t.querySelectorAll(".wd-create-cluster").forEach(y=>y.addEventListener("click",()=>{const m=y.dataset.name||"";this._modal={type:"create-profile",prefillName:m},this._render()})),t.querySelectorAll("[data-suglock]").forEach(y=>y.addEventListener("click",async()=>{const m=y.dataset.suglock,u=this._devices[this._selIdx],E=u&&u.entry_id;if(!(!E||!m))try{if(await this._ws({type:`${w}/set_suggestion_lock`,entry_id:E,key:m,locked:!0}),!this._isActiveEntry(E))return;this._suggestions=(this._suggestions||[]).filter(L=>L.key!==m),(this._lockedSuggestions||[]).includes(m)||(this._lockedSuggestions=this._lockedSuggestions||[]).push(m),this._showToast(this._t("msg.sug_muted",{},"Won't suggest this setting again"),"info"),this._render()}catch{this._showToast(this._t("msg.sug_mute_failed",{},"Could not mute suggestion"),"error")}})),t.querySelectorAll("[data-sugkey]").forEach(y=>y.addEventListener("click",()=>{const m=y.dataset.sugkey,u=y.dataset.sugval,E=parseFloat(u);this._pendingSettings[m]=isNaN(E)?u:E,this._dirtyOptKeys.add(m),this._stagedSuggestions=!0,this._suggestions=this._suggestions.filter(H=>H.key!==m),this._showToast(this._t("msg.sug_staged",{key:m,val:u},`Set ${m} = ${u}. Save to apply.`),"info"),this._render();const L=this.shadowRoot,W=L?.getElementById("wd-settings-form");W&&this._cascadeConflictFix(L,W,m)}));const ot=t.getElementById("wd-label-profile");ot&&ot.addEventListener("change",()=>{const y=t.getElementById("wd-new-profile-row"),m=ot.value==="__create_new__";if(y&&(y.style.display=m?"":"none"),!m){const u=t.getElementById("wd-new-profile-name");u&&(u.value="")}});const ht=t.getElementById("wd-cp-cycle");if(ht){const y=()=>{const m=t.getElementById("wd-cp-dur");if(!m)return;const u=!!ht.value;m.disabled=u,m.style.opacity=u?"0.5":""};ht.addEventListener("change",y),y()}const pt=t.getElementById("wd-pr-mode");pt&&pt.addEventListener("change",()=>{const y=t.getElementById("wd-pr-profile"),m=t.getElementById("wd-pr-existing");if(!y||!m)return;const u=pt.value==="existing_profile";y.style.display=u?"none":"",m.style.display=u?"":"none"}),this._wireCycleCanvas(t),this._wirePhaseInputs(t),this._wirePhaseCanvas(t),this._wireCleanup(t),this._wireSplitSegments(t)}_syncTrimInputs(){const t=this.shadowRoot,e=this._modal,s=(e.timeMode||"s")==="clock",r=t.getElementById("wd-trim-start"),n=t.getElementById("wd-trim-end");r&&(r.value=s?this._offsetToClock(e.trim.start):Math.round(e.trim.start)),n&&(n.value=s?this._offsetToClock(e.trim.end):Math.round(e.trim.end))}_snapTrimBounds(){const t=this._modal,e=t.curve&&t.curve.samples||[];if(e.length<2)return;const s=r=>e.reduce((n,a)=>Math.abs(a[0]-r)<Math.abs(n-r)?a[0]:n,e[0][0]);if(t.trim.start=s(t.trim.start),t.trim.end=s(t.trim.end),t.trim.end<=t.trim.start){const r=e.findIndex(n=>n[0]===t.trim.start);r>=0&&r+1<e.length?t.trim.end=e[r+1][0]:r>0&&(t.trim.start=e[r-1][0])}}_offsetToClock(t){const e=this._modal,s=e&&e.curve&&e.curve.start_time;if(!s)return"";const r=new Date(new Date(s).getTime()+(t||0)*1e3);return`${String(r.getHours()).padStart(2,"0")}:${String(r.getMinutes()).padStart(2,"0")}:${String(r.getSeconds()).padStart(2,"0")}`}_clockToOffset(t){const e=this._modal,s=e&&e.curve&&e.curve.start_time;if(!s||!t)return null;const r=String(t).split(":").map(Number);if(!r.length||r.some(l=>!Number.isFinite(l)))return null;const n=new Date(s),a=new Date(n);a.setHours(r[0]||0,r[1]||0,r[2]||0,0);let o=(a-n)/1e3;const i=e.curve&&e.curve.full_duration_s||0;return o<-1&&o+86400<=i&&(o+=86400),Math.max(0,Math.min(i,o))}_trimInputToOffset(t){if(t===""||t==null)return null;if(this._modal.timeMode==="clock")return this._clockToOffset(t);const e=ie(t,NaN);return Number.isFinite(e)?e:null}_toggleSplit(t){const e=this._modal,s=e.curve&&e.curve.full_duration_s||0,r=Math.max(20,s*.025),n=e.split.offsets,a=n.findIndex(o=>Math.abs(o-t)<r);a>=0?n.splice(a,1):n.push(Math.round(t)),n.sort((o,i)=>o-i),e.split.profiles=[],this._render()}_wireCycleCanvas(t){const e=this._modal;if(!e||e.type!=="cycle-detail"||!e.loaded)return;const s=t.getElementById("wd-cyc-canvas");if(s)if(e.mode==="trim"){const r=t.getElementById("wd-trim-start"),n=t.getElementById("wd-trim-end");r&&r.addEventListener("input",()=>{const i=this._trimInputToOffset(r.value);i!==null&&(e.trim.start=Math.max(0,Math.min(i,e.trim.end-1)),this._drawCycleEditor())}),n&&n.addEventListener("input",()=>{const i=this._trimInputToOffset(n.value);i!==null&&(e.trim.end=Math.min(e.curve.full_duration_s,Math.max(i,e.trim.start+1)),this._drawCycleEditor())});const a=()=>{this._snapTrimBounds(),this._syncTrimInputs(),this._drawCycleEditor()};r&&r.addEventListener("change",a),n&&n.addEventListener("change",a);const o=()=>{e.drag&&(this._snapTrimBounds(),this._syncTrimInputs(),this._drawCycleEditor()),e.drag=null,s._wdOwnGesture=!1};s._wdAbortEdit=()=>{e.drag=null,s._wdOwnGesture=!1},s.addEventListener("pointerdown",i=>{const l=s._wd;if(!l)return;const c=s.getBoundingClientRect(),_=i.clientX-c.left,p=Math.abs(_-l.xToCss(e.trim.start)),f=Math.abs(_-l.xToCss(e.trim.end));if(i.pointerType==="touch"&&Math.min(p,f)>Dt){e.drag=null;return}e.drag=p<=f?"start":"end",s._wdOwnGesture=!0,s.setPointerCapture(i.pointerId)}),s.addEventListener("pointermove",i=>{if(!e.drag||s._wdPinching)return;const l=s._wd;if(!l)return;const c=s.getBoundingClientRect(),_=l.cssToX(i.clientX-c.left);e.drag==="start"?e.trim.start=Math.min(_,e.trim.end-1):e.trim.end=Math.max(_,e.trim.start+1),this._syncTrimInputs(),this._drawCycleEditor()}),s.addEventListener("pointerup",o),s.addEventListener("pointercancel",o)}else e.mode==="split"&&s.addEventListener("pointerdown",r=>{const n=s._wd;if(!n||s._wdPinching)return;const a=s.getBoundingClientRect();this._toggleSplit(n.cssToX(r.clientX-a.left))})}_wireSplitSegments(t){const e=this._modal;!e||e.type!=="cycle-detail"||e.mode!=="split"||t.querySelectorAll("[data-segidx]").forEach(s=>s.addEventListener("change",()=>{e.split.profiles[+s.dataset.segidx]=s.value||null}))}_wirePhaseInputs(t){const e=this._modal;!e||e.type!=="profile-panel"||e.tab!=="phases"||t.querySelectorAll("[data-phidx]").forEach(s=>{const r=()=>{const n=+s.dataset.phidx,a=s.dataset.phfield,o=e.phases[n];o&&(a==="name"?o.name=s.value:o[a]=Math.max(0,ie(s.value,0)*60),this._drawPhaseEditor())};s.addEventListener("input",r),s.addEventListener("change",r)})}_wirePhaseCanvas(t){const e=this._modal;if(!e||e.type!=="profile-panel"||e.tab!=="phases"||!e.env||!(e.env.avg||[]).length)return;const s=t.getElementById("wd-phase-canvas");if(!s)return;const r=e.env.target_duration||e.env.avg[e.env.avg.length-1][0],n=Math.max(5,r*.01),a=(i,l)=>{const c=s._wd;if(!c)return null;let _=null,p=l==="touch"?Dt:12;return(e.phases||[]).forEach((f,g)=>{[["start",f.start],["end",f.end]].forEach(([$,k])=>{const x=Math.abs(i-c.xToCss(k));x<p&&(p=x,_={idx:g,edge:$})})}),_},o=()=>{e.phaseDrag=null,s._wdOwnGesture=!1};s._wdAbortEdit=o,s.addEventListener("pointerdown",i=>{const l=s.getBoundingClientRect();e.phaseDrag=a(i.clientX-l.left,i.pointerType),e.phaseDrag&&(s._wdOwnGesture=!0,s.setPointerCapture(i.pointerId))}),s.addEventListener("pointermove",i=>{if(!e.phaseDrag||s._wdPinching)return;const l=s._wd;if(!l)return;const c=s.getBoundingClientRect(),_=l.cssToX(i.clientX-c.left),p=e.phases[e.phaseDrag.idx];p&&(e.phaseDrag.edge==="start"?p.start=Math.max(0,Math.min(_,p.end-n)):p.end=Math.min(r,Math.max(_,p.start+n)),this._syncPhaseInputs(e.phaseDrag.idx),this._drawPhaseEditor())}),s.addEventListener("pointerup",o),s.addEventListener("pointercancel",o)}_syncPhaseInputs(t){const e=this.shadowRoot,s=this._modal.phases[t];if(!s)return;const r=e.querySelector(`[data-phidx="${t}"][data-phfield="start"]`),n=e.querySelector(`[data-phidx="${t}"][data-phfield="end"]`);r&&(r.value=(s.start/60).toFixed(1)),n&&(n.value=(s.end/60).toFixed(1))}_wireCleanup(t){const e=this._modal;if(!e||e.type!=="profile-panel"||e.tab!=="cleanup"||!e.cleanup)return;t.querySelectorAll("[data-cleanidx]").forEach(r=>r.addEventListener("change",()=>{const n=e.cleanup.cycles[+r.dataset.cleanidx];if(!n)return;r.checked?e.cleanup.selected.add(n.cycle_id):e.cleanup.selected.delete(n.cycle_id);const a=e.cleanup.selected;if(!this._busy.has("pp-cleanup-del")&&!this._busy.has("pp-cleanup-unlabel")){const o=[["pp-cleanup-unlabel","btn.unlabel_selected",`Unlabel selected (${a.size})`],["pp-cleanup-del","btn.delete_selected",`Delete selected (${a.size})`]];for(const[i,l,c]of o){const _=t.querySelector(`[data-maction="${i}"]`);_&&(_.disabled=a.size===0,_.textContent=this._t(l,{n:a.size},c))}}this._drawSpaghetti()})),t.querySelectorAll("tr[data-cid]").forEach(r=>{r.addEventListener("mouseenter",()=>{this._spagTableHoverCid=r.dataset.cid,this._drawSpaghetti()}),r.addEventListener("mouseleave",()=>{this._spagTableHoverCid=null,this._drawSpaghetti()})});const s=t.getElementById("wd-spag-canvas");s&&s.addEventListener("pointerdown",r=>{if(s._wdPinching)return;this._onGraphHoverInner(r.clientX,r.clientY,"wd-spag-canvas",{touch:r.pointerType==="touch"});const n=this._hoverNearest;if(n&&n.cid){const a=e.cleanup.selected;a.has(n.cid)?a.delete(n.cid):a.add(n.cid),this._render()}})}_onAction(t){const e=t.dataset.action,s=this.shadowRoot;if(e==="open-settings"){this._modal={type:"gear-settings",tab:this._gearTab||"prefs"},this._render();return}const r=this._devices[this._selIdx];if(!r)return;const n=r.entry_id;if(e.startsWith("sug-"))return this._onActSuggestions(e,t,r,n,s);if(e.startsWith("ml-"))return this._onActMl(e,t,r,n);if(e.startsWith("store-"))return this._onActStore(e,t,r,n,s);if(e.startsWith("auto-"))return this._onActAuto(e,t,r,n,s);if(e.startsWith("maint-"))return this._onActMaintenance(e,t,r,n,s);if(e.startsWith("pg-")&&e!=="pg-new"&&e!=="pg-edit"&&e!=="pg-suggest")return this._onActPlayground(e,t,r,n);if(e==="open-cycle"){const a=t.dataset.cid,o=t.dataset.mode==="review"?"review":"view";this._modal={type:"cycle-detail",entryId:n,cycleId:a,loaded:!1,mode:o,curve:null,ml:(this._mlById||{})[a]||null,trim:{start:0,end:0},split:{offsets:[],profiles:[]},drag:null},this._profiles.length||this._fetchProfiles(n),this._render(),this._ws({type:`${w}/get_cycle_power_data`,entry_id:n,cycle_id:a}).then(i=>{this._modal&&this._modal.cycleId===a&&(this._modal.curve=i,this._modal.loaded=!0,this._modal.trim={start:0,end:i.full_duration_s||0},this._render(),i.profile_name&&this._fetchCycleProfileEnv(n,i.profile_name))}).catch(i=>this._showToast(this._t("toast.could_not_load_cycle",{error:i.message||i},"Could not load cycle: "+(i.message||i)),"error"))}else if(e==="cleanup-edit-cycle"){const a=t.dataset.cid;this._prevModal=this._modal,this._modal={type:"cycle-detail",entryId:n,cycleId:a,loaded:!1,mode:"view",curve:null,ml:(this._mlById||{})[a]||null,trim:{start:0,end:0},split:{offsets:[],profiles:[]},drag:null},this._profiles.length||this._fetchProfiles(n),this._render(),this._ws({type:`${w}/get_cycle_power_data`,entry_id:n,cycle_id:a}).then(o=>{this._modal&&this._modal.cycleId===a&&(this._modal.curve=o,this._modal.loaded=!0,this._modal.trim={start:0,end:o.full_duration_s||0},this._render(),o.profile_name&&this._fetchCycleProfileEnv(n,o.profile_name))}).catch(o=>this._showToast(this._t("toast.could_not_load_cycle",{error:o.message||o},"Could not load cycle: "+(o.message||o)),"error"))}else if(e==="open-profile"){const a=t.dataset.pname;this._prevModal=null;const o=(this._profiles||[]).find(i=>i.name===a)||{name:a};this._modal={type:"profile-panel",name:a,tab:"stats",loaded:!1,stats:o,env:null,phases:[],catalog:[],cleanup:null},this._render(),Promise.all([this._ws({type:`${w}/get_profile_envelope`,entry_id:n,profile_name:a}).catch(()=>({envelope:null})),this._ws({type:`${w}/get_profile_phases`,entry_id:n,profile_name:a}).catch(()=>({phases:[]})),this._ws({type:`${w}/get_phase_catalog`,entry_id:n}).catch(()=>({phases:[]}))]).then(([i,l,c])=>{!this._modal||this._modal.name!==a||(this._modal.env=i.envelope,this._modal.phases=(l.phases||[]).map(_=>({name:_.name,start:_.start,end:_.end})),this._modal.catalog=(c.phases||[]).map(_=>_.name),this._modal.loaded=!0,this._render())})}else if(e==="create-profile")this._modal={type:"create-profile"},this._render();else if(e==="setup-cta"){const a=t.dataset.ctaAction||"";let o={};try{o=JSON.parse(t.dataset.ctaParams||"{}")}catch{}this._dispatchSetupCta(a,o)}else if(e==="setup-skip"){const a=t.dataset.step,o=t.dataset.snooze;if(a){let i;if(o==="never")i="never";else{const l=new Date;l.setDate(l.getDate()+14),i=l.toISOString()}this._setPref(a,i),this._reloadSetupStatus()}}else if(e==="hide-setup-card")this._setPref("setup_card_dismissed",!0),this._render();else if(e==="expand-setup")this._setPref("setup_card_dismissed",!1),this._render();else if(e==="set-settings-level"){const a=(t.type==="checkbox"?t.checked:t.dataset.slevel==="advanced")?"advanced":"basic";a!==this._pref("settings_level","basic")&&(this._snapshotFormToPending(s),this._setPref("settings_level",a),this._render())}else if(e==="pg-new"||e==="pg-edit"||e==="pg-suggest"){if(e==="pg-new")this._modal={type:"profile-group",orig:null,name:"",members:[]};else if(e==="pg-edit"){const a=t.dataset.gname,o=((this._profileGroups||{}).groups||[]).find(i=>i.name===a);this._modal={type:"profile-group",orig:a,name:a,members:o?[...o.members||[]]:[]}}else{const a=((this._profileGroups||{}).suggestions||[])[parseInt(t.dataset.idx,10)]||null;if(!a)return;this._modal={type:"profile-group",orig:a.existing_group||null,name:a.existing_group||"",members:[...a.members||[]]}}this._render(),this._ensureProfileEnvs(n,(this._profiles||[]).map(a=>a.name)).then(()=>{this._modal&&this._modal.type==="profile-group"&&this._render()})}else if(e==="rebuild-envelopes")this._kickAndTrack({type:`${w}/rebuild_envelopes`,entry_id:n},"rebuild-envelopes",async()=>{this._showToast(this._t("toast.envelopes_rebuilt",{},"Envelopes rebuilt")),await this._fetchProfiles(n)});else if(e==="rec-start")this._ws({type:`${w}/start_recording`,entry_id:n}).then(()=>(this._showToast(this._t("toast.recording_started",{},"Recording started")),this._fetchRecState(n))).then(()=>this._render()).catch(a=>this._showToast(this._t("toast.start_failed",{error:a.message||a},"Start failed: "+(a.message||a)),"error"));else if(e==="rec-stop")this._ws({type:`${w}/stop_recording`,entry_id:n}).then(()=>(this._showToast(this._t("toast.recording_stopped",{},"Recording stopped")),this._fetchRecState(n))).then(()=>this._render()).catch(a=>this._showToast(this._t("toast.stop_failed",{error:a.message||a},"Stop failed: "+(a.message||a)),"error"));else if(e==="rec-process-open")this._fetchProfiles(n).then(()=>{this._modal={type:"process-recording"},this._render()});else if(e==="rec-discard")this._modal={type:"confirm",title:this._t("modal.discard_recording_title",{},"Discard Recording"),message:this._t("modal.discard_recording_msg",{},"Discard the saved recording? This cannot be undone."),okLabel:this._t("btn.discard",{},"Discard"),onOk:async()=>{try{await this._ws({type:`${w}/discard_recording`,entry_id:n}),this._showToast(this._t("toast.recording_discarded",{},"Recording discarded")),await this._fetchRecState(n)}catch(a){this._showToast(this._t("toast.discard_failed",{error:a.message||a},"Discard failed: "+(a.message||a)),"error")}}},this._render();else if(e==="fb-confirm")this._ws({type:`${w}/resolve_feedback`,entry_id:n,cycle_id:t.dataset.cid,action:"confirm"}).then(()=>(this._showToast(this._t("toast.feedback_confirmed",{},"Feedback confirmed")),this._fetchFeedbacks(n))).then(()=>this._render()).catch(a=>this._showToast(this._t("msg.toast_error",{error:a.message||a},"Error: "+(a.message||a)),"error"));else if(e==="fb-ignore")this._ws({type:`${w}/resolve_feedback`,entry_id:n,cycle_id:t.dataset.cid,action:"ignore"}).then(()=>(this._showToast(this._t("toast.feedback_dismissed",{},"Feedback dismissed")),this._fetchFeedbacks(n))).then(()=>this._render()).catch(a=>this._showToast(this._t("msg.toast_error",{error:a.message||a},"Error: "+(a.message||a)),"error"));else if(e==="fb-correct")this._fetchProfiles(n).then(()=>{this._modal={type:"correct-feedback",cycleId:t.dataset.cid,detectedProfile:t.dataset.prof},this._render()});else if(e==="fb-dismiss-all")this._modal={type:"confirm",title:this._t("modal.dismiss_all_title",{},"Dismiss All Feedbacks"),message:this._t("modal.dismiss_all_msg",{count:this._feedbacks.length},`Dismiss all ${this._feedbacks.length} pending feedback requests?`),okLabel:this._t("modal.dismiss_all_ok",{},"Dismiss All"),onOk:async()=>{try{await this._ws({type:`${w}/dismiss_all_feedbacks`,entry_id:n}),this._showToast(this._t("toast.feedback_all_dismissed",{},"All feedbacks dismissed")),await this._fetchFeedbacks(n)}catch(a){this._showToast(this._t("msg.toast_error",{error:a.message||a},"Error: "+(a.message||a)),"error")}}},this._render();else if(e==="create-phase")this._modal={type:"create-phase",deviceType:t.dataset.dtype},this._render();else if(e==="edit-phase")this._modal={type:"edit-phase",phaseId:t.dataset.pid,phaseName:t.dataset.pname,phaseDesc:t.dataset.pdesc,isDefault:t.dataset.pisdefault==="true"},this._render();else if(e==="del-phase"){const a=t.dataset.pname,o=t.dataset.pid;this._modal={type:"confirm",title:this._t("modal.delete_phase_title",{},"Delete Phase"),message:this._t("modal.delete_phase_msg",{name:a},`Delete phase "${a}"?`),okLabel:this._t("btn.delete",{},"Delete"),onOk:async()=>{try{await this._ws({type:`${w}/delete_phase`,entry_id:n,phase_id:o}),this._showToast(this._t("toast.phase_deleted",{name:a},`Phase "${a}" deleted`)),await this._fetchPhases(n)}catch(i){this._showToast(this._t("msg.toast_delete_failed",{error:i.message||i},"Delete failed: "+(i.message||i)),"error")}}},this._render()}else if(e==="diag-refresh")this._fetchToolsData(n).then(()=>this._render());else if(e==="reprocess-history")this._modal={type:"confirm",title:this._t("modal.process_history_title",{},"Process History"),message:this._t("modal.process_history_msg",{},"Re-run matching, refresh suggestions, retrain ML (if enabled), recost cycles against recorded energy prices and recompute cycle health across all stored cycles. This may take a while."),okLabel:this._t("modal.process_history_ok",{},"Process"),onOk:()=>this._kickAndTrack({type:`${w}/reprocess_history`,entry_id:n},"reprocess",async a=>{const o=a.count||0,i=[this._t("toast.processed_cycles",{n:o},o+" cycles")];a.suggestions!=null&&i.push(this._t("toast.processed_suggestions",{n:a.suggestions},a.suggestions+" suggestion(s)"));const l=a.ml_training&&a.ml_training.ok&&(a.ml_training.promoted||[]).length||0;l&&i.push(this._t("toast.processed_models",{n:l},l+" model(s) promoted")),this._showToast(this._t("toast.processed",{bits:i.join(", ")},"Processed "+i.join(", "))),await this._fetchToolsData(n)})},this._render();else if(e==="clear-debug")this._modal={type:"confirm",title:this._t("modal.clear_debug_title",{},"Clear Debug Data"),message:this._t("modal.clear_debug_msg",{},"Delete all stored debug traces?"),okLabel:this._t("status.clear",{},"Clear"),onOk:()=>this._busyRun("clear-debug",async()=>{try{const a=await this._ws({type:`${w}/clear_debug_data`,entry_id:n});this._showToast(this._t("toast.debug_cleared",{count:a.count||0},`Cleared ${a.count||0} debug traces`)),await this._fetchToolsData(n)}catch(a){this._showToast(this._t("msg.toast_error",{error:a.message||a},"Error: "+(a.message||a)),"error")}})},this._render();else if(e==="wipe-history")this._modal={type:"confirm",title:this._t("modal.wipe_all_title",{},"Wipe All Data"),message:this._t("modal.wipe_all_msg",{},"\u26A0\uFE0F This permanently deletes ALL cycles and profiles. This cannot be undone."),okLabel:this._t("modal.wipe_all_ok",{},"Wipe Everything"),onOk:()=>this._busyRun("wipe",async()=>{try{await this._ws({type:`${w}/wipe_history`,entry_id:n}),this._showToast(this._t("toast.all_wiped",{},"All data wiped")),this._cycles=[],this._profiles=[],await this._fetchToolsData(n)}catch(a){this._showToast(this._t("msg.toast_error",{error:a.message||a},"Error: "+(a.message||a)),"error")}})},this._render();else if(e==="export-config")this._ws({type:`${w}/export_config`,entry_id:n}).then(a=>{const o=new Blob([a.json_data],{type:"application/json"}),i=URL.createObjectURL(o),l=document.createElement("a");l.href=i,l.download=`washdata_export_${n.slice(0,8)}.json`,document.body.appendChild(l),l.click(),document.body.removeChild(l),URL.revokeObjectURL(i),this._showToast(this._t("toast.export_downloaded",{},"Export downloaded"))}).catch(a=>this._showToast(this._t("toast.export_failed",{error:a.message||a},"Export failed: "+(a.message||a)),"error"));else if(e==="export-select-open")this._modal={type:"export-select",inventory:null,loading:!0,sel:{cats:new Set,profiles:new Set,realIds:new Set,refIds:new Set},expanded:new Set},this._render(),(async()=>{let a=null;try{const o=await this._ws({type:`${w}/get_export_inventory`,entry_id:n});a=o&&o.manifest||null}catch(o){this._showToast(this._t("toast.export_failed",{error:o.message||o},"Export failed: "+(o.message||o)),"error")}if(!(!this._isActiveEntry(n)||!this._modal||this._modal.type!=="export-select")){if(!a){this._modal=null,this._render();return}this._modal.inventory=a,this._modal.sel=this._wizInitSel({categories:a},!1),this._modal.loading=!1,this._render()}})();else if(e==="cyc-select-toggle")this._selectMode=!this._selectMode,this._selectMode||this._cycleSel.clear(),this._render();else if(e==="cyc-auto-open")this._modal={type:"auto-label"},this._render();else if(e==="cyc-merge"){const a=Array.from(this._cycleSel);if(a.length<2)return;this._fetchProfiles(n).then(()=>{this._modal={type:"merge-cycles",ids:a},this._render()})}else if(e==="cyc-relabel"){const a=Array.from(this._cycleSel);if(!a.length)return;this._fetchProfiles(n).then(()=>{this._modal={type:"bulk-relabel",ids:a},this._render()})}else if(e==="cyc-load-more")this._busyRun("cyc-load-more",async()=>{try{await this._loadMoreCycles(n)}catch(a){this._showToast(this._t("toast.load_more_failed",{error:a.message||a},"Could not load more: "+(a.message||a)),"error")}});else if(e==="task-cancel"){const a=t.dataset.taskId;a&&(this._cancellingTasks.add(a),this._updateTaskPills(),this._ws({type:`${w}/cancel_task`,task_id:a}).catch(()=>{this._cancellingTasks.delete(a),this._updateTaskPills()}))}else if(e==="cyc-compare"){const a=Array.from(this._cycleSel);if(a.length<2)return;this._modal={type:"compare-cycles",ids:a,cycles:{},hidden:new Set,overlays:[],loaded:!1},this._profiles.length||this._fetchProfiles(n),this._render(),Promise.all(a.map(o=>this._ws({type:`${w}/get_cycle_power_data`,entry_id:n,cycle_id:o}).then(i=>({cid:o,r:i})).catch(()=>({cid:o,r:null})))).then(o=>{!this._modal||this._modal.type!=="compare-cycles"||(o.forEach(({cid:i,r:l})=>{l&&(this._modal.cycles[i]=l)}),this._modal.loaded=!0,this._render())})}else if(e==="cyc-bulk-del"){const a=Array.from(this._cycleSel);if(!a.length)return;this._deleteCyclesWithUndo(n,a)}else if(e==="retry-cycles")this._fetchCycles(n).then(()=>this._render());else if(e==="retry-profiles")Promise.all([this._fetchProfiles(n),this._fetchProfileGroups(n)]).then(()=>this._render());else if(e==="retry-suggestions")this._fetchSuggestions(n).then(()=>this._render());else if(e==="goto-suggestions")this._settingsSugOnly=!0,this._tab="settings",this._fetchTabData();else if(e==="goto-conflicts")this._tab="settings",this._fetchTabData();else if(e==="goto-standby")this._settingsSec="detection",this._settingsSearch="",this._settingsSugOnly=!1,this._tab="settings",this._fetchTabData();else if(e==="conf-goto-section"){const a=this._conflictKeysFromOpts();for(const o of xt)if((o.fields||(o.groups||[]).flatMap(l=>l.fields||[])).some(l=>a.has(l.key))){this._settingsSec=o.id,this._render();break}}else if(e==="toggle-settings-history")this._settingsHistoryOpen=!this._settingsHistoryOpen,this._render();else if(e==="settings-revert-key"){const a=t.dataset.key,o=JSON.parse(t.dataset.val);if(!a)return;const i=r.entry_id;this._ws({type:`${w}/set_options`,entry_id:i,options:{[a]:o}}).then(()=>this._ws({type:`${w}/get_options`,entry_id:i})).then(l=>(this._opts=l.options||{},this._optDefaults=l.defaults||{},this._fetchSettingsChangelog(i))).then(()=>{this._showToast(this._t("msg.toast_reverted",{key:this._t("setting."+a+".label",{},a)},"{key} reverted"),"success"),this._render()}).catch(l=>this._showToast(this._t("msg.toast_error",{error:l.message||l},"Error: "+(l.message||l)),"error"))}else if(e==="toggle-log-drawer"){this._logOpen=!this._logOpen;try{localStorage.setItem("wd-log-open",this._logOpen?"1":"0")}catch{}this._render(),this._logOpen&&this._fetchLogs().then(()=>{this._logOpen&&this._render()})}else if(e==="open-advanced"){const a=t.dataset.sub;a&&(this._panelSubtab=a),this._tab="advanced",this._render(),this._panelSubtab==="diagnostics"&&!this._diag?this._fetchToolsData(n).then(()=>{this._tab==="advanced"&&this._render()}):this._panelSubtab==="logs"?this._fetchLogs().then(()=>{this._tab==="advanced"&&this._render()}):this._panelSubtab==="maintenance"?this._fetchMaintenance(n).then(()=>{this._tab==="advanced"&&this._render()}):this._panelSubtab==="ml"&&this._fetchTabData()}else if(e==="add-device")this._navigate(`/config/integrations/integration/${w}`);else if(e==="goto-feedbacks")this._tab="history",this._cycleFilter={...this._cycleFilter,status:"needs_review"},this._fetchTabData();else if(e==="goto-recording")this._tab="status",this._fetchTabData();else if(e==="logs-refresh")this._fetchLogs().then(()=>this._render());else if(e==="logs-export")this._ws({type:`${w}/get_logs`,limit:500}).then(a=>{const o=(a.logs||[]).map(_=>`${new Date(_.ts*1e3).toISOString()} ${_.level} ${_.msg}`).join(`
`),i=new Blob([o],{type:"text/plain"}),l=URL.createObjectURL(i),c=document.createElement("a");c.href=l,c.download=`washdata_logs_${Date.now()}.txt`,document.body.appendChild(c),c.click(),document.body.removeChild(c),URL.revokeObjectURL(l),this._showToast(this._t("toast.logs_exported",{},"Logs exported"))}).catch(a=>this._showToast(this._t("toast.export_failed",{error:a.message||a},"Export failed: "+(a.message||a)),"error"));else if(e==="import-config-open")this._modal={type:"import-wizard",step:"input",jsonText:"",manifest:null,error:null,sel:{cats:new Set,profiles:new Set,realIds:new Set,refIds:new Set},expanded:new Set,mode:"merge",cycleDest:"reference",conflicts:{}},this._render();else if(e==="hist-import-open")this._modal={type:"history-import",step:"input",csvText:"",since:jt(),token:null,scanTaskId:null,applyTaskId:null,result:null,accept:new Set,done:null,error:null},this._render();else if(e==="import-config-raw")this._modal={type:"import-config"},this._render();else if(e==="save-prefs"){const a=s.getElementById("wd-pref-tab")?.value||"",o=!!s.getElementById("wd-pref-debug")?.checked,i=s.getElementById("wd-pref-expected")?!!s.getElementById("wd-pref-expected").checked:!0,l=!!s.getElementById("wd-pref-raw")?.checked,c=s.getElementById("wd-pref-datefmt")?.value||"relative",_=s.getElementById("wd-pref-lang")?.value||"",p=parseFloat(s.getElementById("wd-pref-fontscale")?.value)||1,f={default_tab:a,show_debug:o,show_expected:i,show_raw:l,date_format:c,lang_override:_,font_scale:p};this._busyRun("save-prefs",async()=>{try{await this._ws({type:`${w}/set_user_prefs`,prefs:f}),this._panelCfg&&(this._panelCfg.prefs={...this._panelCfg.prefs||{},...f});const g=_||this._hass&&this._hass.locale&&this._hass.locale.language;await this._loadPanelLang(g),this._render(),this._showToast(this._t("toast.preferences_saved",{},"Preferences saved"))}catch(g){this._showToast(this._t("toast.save_failed",{error:g.message||g},"Save failed: "+(g.message||g)),"error")}})}else if(e==="save-panel"){const a={default_tab:s.getElementById("wd-ps-deftab")?.value||"status",hidden_tabs:Array.from(s.querySelectorAll("[data-hidetab]")).filter(o=>o.checked).map(o=>o.dataset.hidetab)};this._busyRun("save-panel",async()=>{try{await this._ws({type:`${w}/set_panel_config`,panel:a}),this._panelCfg=await this._ws({type:`${w}/get_panel_config`}),this._tabInitialized=!0,this._applyPanelConfig(),this._showToast(this._t("toast.panel_settings_saved",{},"Panel settings saved"))}catch(o){this._showToast(this._t("msg.toast_save_failed",{error:o.message||o},"Save failed: "+(o.message||o)),"error")}})}else if(e==="pause-cycle")this._ws({type:`${w}/pause_cycle`,entry_id:n}).then(a=>{if(a&&a.ok===!1){this._showToast(this._t("toast.pause_no_cycle",{},"No active cycle to pause"),"error");return}return this._showToast(this._t("toast.cycle_paused",{},"Cycle paused")),this._fetchAll()}).catch(a=>this._showToast(this._t("toast.pause_failed",{error:a.message||a},"Pause failed: "+(a.message||a)),"error"));else if(e==="resume-cycle")this._ws({type:`${w}/resume_cycle`,entry_id:n}).then(a=>{if(a&&a.ok===!1){this._showToast(this._t("toast.resume_no_cycle",{},"No paused cycle to resume"),"error");return}return this._showToast(this._t("toast.cycle_resumed",{},"Cycle resumed")),this._fetchAll()}).catch(a=>this._showToast(this._t("msg.toast_resume_failed",{error:a.message||a},"Resume failed: "+(a.message||a)),"error"));else if(e==="terminate-cycle")this._modal={type:"confirm",title:this._t("modal.force_stop_title",{},"Force Stop Cycle"),message:this._t("modal.force_stop_msg",{},"Force-stop the active cycle now? The cycle will be saved as interrupted."),okLabel:this._t("btn.force_stop",{},"Force Stop"),onOk:async()=>{try{await this._ws({type:`${w}/terminate_cycle`,entry_id:n}),this._showToast(this._t("toast.cycle_force_stopped",{},"Cycle force-stopped")),await this._fetchAll()}catch(a){this._showToast(this._t("msg.toast_force_stop_failed",{error:a.message||a},"Force stop failed: "+(a.message||a)),"error")}}},this._render();else if(e==="save-rbac"){const a=!!s.getElementById("wd-rbac-enabled")?.checked,o=s.getElementById("wd-rbac-default")?.value||"none",i={};s.querySelectorAll("[data-rbacuser]").forEach(l=>{const c=l.dataset.rbacuser,_=l.dataset.rbacdev,p=l.value;i[c]||(i[c]={default:"none",devices:{}}),_==="__default__"?i[c].default=p:p&&p!=="inherit"&&(i[c].devices[_]=p)}),this._busyRun("save-rbac",async()=>{try{await this._ws({type:`${w}/set_panel_config`,rbac:{enabled:a,default_level:o,users:i}}),this._panelCfg=await this._ws({type:`${w}/get_panel_config`}),this._showToast(this._t("toast.access_saved",{},"Access control saved"))}catch(l){this._showToast(this._t("msg.toast_save_failed",{error:l.message||l},"Save failed: "+(l.message||l)),"error")}})}}_onActSuggestions(t,e,s,r,n){if(t==="sug-apply-all"){const a=this._suggestions.map(o=>o.key);this._busyRun("save-settings",async()=>{try{await this._ws({type:`${w}/apply_suggestions`,entry_id:r,keys:a}),this._showToast(this._t("toast.suggestions_applied",{},"Suggestions applied; integration reloading")),await this._fetchSuggestions(r);const o=await this._ws({type:`${w}/get_options`,entry_id:r});this._opts=o.options||{},this._optDefaults=o.defaults||{},this._prevOpts=null,this._cascadePending={},this._preCascadeOpts=null}catch(o){this._showToast(this._t("toast.apply_failed",{error:o.message||o},"Apply failed: "+(o.message||o)),"error")}})}else t==="sug-show-all"?(this._settingsSugOnly=!1,this._render()):t==="sug-dismiss"?(this._settingsSugOnly=!1,this._busyRun("save-settings",async()=>{try{await this._ws({type:`${w}/clear_suggestions`,entry_id:r}),this._suggestions=[],this._showToast(this._t("toast.suggestions_dismissed",{},"Suggestions dismissed"))}catch(a){this._showToast(this._t("toast.error",{error:a.message||a},"Error: "+(a.message||a)),"error")}})):t==="sug-unmute-all"?this._busyRun("save-settings",async()=>{try{const a=[...this._lockedSuggestions||[]],o=await Promise.allSettled(a.map(c=>this._ws({type:`${w}/set_suggestion_lock`,entry_id:r,key:c,locked:!1})));if(!this._isActiveEntry(r))return;const i=o.filter(c=>c.status==="rejected").length,l=o.slice().reverse().find(c=>c.status==="fulfilled");this._lockedSuggestions=l&&l.value&&l.value.locked_suggestions||[],await this._fetchSuggestions(r),i?this._showToast(this._t("toast.error",{error:`${i} suggestion(s) failed to unlock`},`${i} suggestion(s) failed to unlock`),"error"):this._showToast(this._t("msg.sug_unmuted_all",{},"Muted suggestions reset"),"success")}catch(a){this._showToast(this._t("toast.error",{error:a.message||a},"Error: "+(a.message||a)),"error")}}):t==="sug-analyze"&&this._busyRun("sug-analyze",async()=>{try{const a=await this._ws({type:`${w}/run_suggestion_analysis`,entry_id:r}),o=a&&a.count||0;this._showToast(o?this._t("toast.analysis_complete",{count:o},`Analysis complete: ${o} suggestion(s)`):this._t("toast.analysis_complete_none",{},"Analysis complete: no new suggestions")),await this._fetchSuggestions(r)}catch(a){this._showToast(this._t("toast.analysis_failed",{error:a.message||a},"Analysis failed: "+(a.message||a)),"error")}})}_onActMl(t,e,s,r){t==="ml-train-now"?this._kickAndTrack({type:`${w}/trigger_ml_training`,entry_id:r},"ml-train-now:"+r,async n=>{if(n&&n.ok){const a=(n.promoted||[]).length;this._showToast(a?this._t("toast.ml_training_promoted",{count:a},`Training complete: promoted ${a} model(s)`):this._t("toast.ml_training_no_improvement",{},"Training complete: baseline kept (no improvement)"))}else this._showToast(this._t("toast.ml_training_no_improvement",{},"Training complete: baseline kept (no improvement)"),"info");await this._loadMlTrainingStatus(r)}):t==="ml-revert-match"?this._busyRun("ml-revert-match",async()=>{try{await this._ws({type:`${w}/revert_matching_config`,entry_id:r}),this._showToast(this._t("toast.matching_reverted",{},"Matching weights reverted to defaults")),await this._loadMlTrainingStatus(r)}catch(n){this._showToast(this._t("toast.revert_failed",{error:n.message||n},"Revert failed: "+(n.message||n)),"error")}}):t==="ml-revert-models"&&this._busyRun("ml-revert-models",async()=>{try{await this._ws({type:`${w}/revert_ml_models`,entry_id:r}),this._showToast(this._t("toast.models_reverted",{},"On-device models reverted to baseline")),await this._loadMlTrainingStatus(r)}catch(n){this._showToast(this._t("msg.toast_revert_failed",{error:n.message||n},"Revert failed: "+(n.message||n)),"error")}})}_onActStore(t,e,s,r,n){if(t==="store-toggle-online"){const a=!!e.checked;this._busyRun("store-account",async()=>{try{const o=await this._ws({type:`${w}/store_set_online`,entry_id:r,enabled:a});this._constants.storeOnlineEnabled=!!(o&&o.enabled),this._constants.storeOnlineEnabled?(await this._loadStoreStatus(r),this._ensureStoreConnectListener()):(this._storeStatus={enabled:!1},this._storeConnected=!1)}catch(o){this._showToast(this._t("toast.store_error",{error:o.message||o},"Error: "+(o.message||o)),"error")}})}else if(t==="store-toggle-pref"){const a=e.dataset.pref,o=!!e.checked;if(!a)return;this._busyRun("store-account",async()=>{try{const i=await this._ws({type:`${w}/store_set_prefs`,entry_id:r,prefs:{[a]:o}});i&&i.prefs&&(this._constants={...this._constants,storePrefs:i.prefs})}catch(i){this._showToast(this._t("toast.store_error",{error:i.message||i},"Error: "+(i.message||i)),"error")}})}else if(t==="store-goto-identity")this._tab="settings",this._settingsSec="basic",this._fetchTabData(),requestAnimationFrame(()=>requestAnimationFrame(()=>{const a=this.shadowRoot&&this.shadowRoot.getElementById("wd-store-brand");a&&(a.focus(),a.scrollIntoView({block:"center"}))}));else if(t==="store-refresh-catalog")this._busyRun("store-refresh-catalog",async()=>{try{await this._ws({type:`${w}/store_refresh_catalog`,entry_id:r}),clearTimeout(this._brandSearchTimer),this._brandSearchTimer=null,this._catalog.brands=void 0,this._catalog.devices=void 0,this._catalog.forBrand=null,this._catalog.brandsFull=!1,this._catalog.brandPrefixes=[],this._dropModelCandidates(),this._catalogEntry=null,this._entityListCache&&(delete this._entityListCache.store_brand,delete this._entityListCache.store_model),this._showToast(this._t("toast.catalog_refreshed",{},"Community catalog refreshed"))}catch(a){this._showToast(this._t("toast.store_error",{error:a.message||a},"Error: "+(a.message||a)),"error")}});else if(t==="store-connect"){const a=this._constants.storeWebOrigin;if(!a){this._showToast(this._t("toast.store_unavailable",{},"The community store is not available."),"error");return}this._ensureStoreConnectListener(),window.open(a+"/connect.html?origin="+encodeURIComponent(location.origin),"washdata_connect","width=480,height=640")}else if(t==="store-disconnect")this._busyRun("store-account",async()=>{try{await this._ws({type:`${w}/store_disconnect`,entry_id:r}),await this._loadStoreStatus(r),this._showToast(this._t("toast.store_disconnected",{},"Disconnected from the community store"))}catch(a){this._showToast(this._t("toast.store_error",{error:a.message||a},"Error: "+(a.message||a)),"error")}});else if(t==="store-add-appliance"){const a=this._constants.storeWebOrigin;if(!a){this._showToast(this._t("toast.store_unavailable",{},"The community store is not available."),"error");return}this._ensureStoreConnectListener();const o=n.getElementById("wd-store-model"),i=n.getElementById("wd-store-brand"),l=new URLSearchParams({mode:"device",type:this._storeApplianceType(),brand:i&&i.value||this._opts.store_brand||"",model:o&&o.value||this._opts.store_model||"",origin:location.origin}).toString();this._storeContribEid=r,window.open(a+"/create.html?"+l,"washdata_create","width=560,height=760")}else if(t==="store-add-brand"){const a=this._constants.storeWebOrigin;if(!a){this._showToast(this._t("toast.store_unavailable",{},"The community store is not available."),"error");return}this._ensureStoreConnectListener();const o=n.getElementById("wd-store-brand"),i=new URLSearchParams({mode:"brand",brand:o&&o.value||this._opts.store_brand||"",origin:location.origin}).toString();this._storeContribEid=r,window.open(a+"/create.html?"+i,"washdata_create","width=560,height=760")}else if(t==="store-confirm-device"){const a=e.dataset.deviceId;this._busyRun("store-account",async()=>{try{const o=await this._ws({type:`${w}/store_confirm_device`,entry_id:r,device_id:a});if(o&&o.error){this._showToast(this._t("toast.store_error",{error:o.error},"Error: "+o.error),"error");return}const i=[...this._catalog.devices||[],...this._storeDevices||[],this._catalogEntry&&this._catalogEntry.device||null];for(const l of i)o&&l&&String(l.id)===String(a)&&(l.confirmCount=o.confirmCount,l.status=o.status);this._showToast(o&&o.status==="approved"?this._t("toast.device_approved",{},"Approved by the community"):this._t("toast.thanks_confirming",{},"Thanks for confirming")),this._render()}catch(o){this._showToast(this._t("toast.store_error",{error:o.message||o},"Error: "+(o.message||o)),"error")}})}else if(t==="store-rate-device"){const a=e.dataset.deviceId,o=parseInt(e.dataset.rating,10);if(!(o>=1&&o<=5))return;this._busyRun("store-account",async()=>{try{const i=await this._ws({type:`${w}/store_rate_device`,entry_id:r,device_id:a,rating:o});if(i&&i.error){this._showToast(this._t("toast.store_error",{error:i.error},"Error: "+i.error),"error");return}this._showToast(this._t("toast.rating_saved",{},"Quality rating saved"))}catch(i){this._showToast(this._t("toast.store_error",{error:i.message||i},"Error: "+(i.message||i)),"error")}})}else if(t==="store-nav"){const a=e.dataset.view;a==="brands"?(this._storeView="brands",this._storeDevice=null,this._storeProfile=null,this._render()):a==="device"&&(this._storeView="device",this._storeProfile=null,this._render())}else if(t==="store-open-device"){const a=e.dataset.deviceId,o=(this._storeDevices||[]).find(i=>String(i.id)===String(a));o&&this._storeOpenDevice(o)}else if(t==="store-open-profile"){const a=e.dataset.profileId,o=(this._storeProfiles||[]).find(i=>String(i.id)===String(a));if(!o)return;this._storeProfile=o,this._storeView="profile",this._storeCycles=[],this._storeLoading=!0,this._render(),this._ws({type:`${w}/store_get_cycles`,entry_id:r,profile_id:o.id}).then(i=>{!this._isActiveEntry(r)||this._storeView!=="profile"||(this._storeCycles=i&&i.items||[])}).catch(()=>{this._isActiveEntry(r)&&(this._storeCycles=[])}).finally(()=>{this._isActiveEntry(r)&&(this._storeLoading=!1,this._render())})}else if(t==="store-onboard")this._storeQuery=(this._opts.store_brand||"").trim(),this._storeView="brands",this._storeDevice=null,this._storeProfile=null,this._tab="store",this._fetchTabData();else if(t==="store-toggle-dl-settings")this._dlSettings=!!e.checked;else if(t==="store-download-device"){const a=e.dataset.deviceId;if(!a)return;const o=!!this._dlSettings;this._busyRun("store-download-device",async()=>{try{const i=await this._ws({type:`${w}/store_download_device`,entry_id:r,device_id:a,include_settings:o});if(i&&(i.error||i.disabled)){const f=i.error||"unavailable";this._showToast(this._t("toast.store_download_failed",{error:f},"Download failed: "+f),"error");return}const l=i&&i.profiles_adopted||0,c=i&&i.cycles_imported||0,_=i&&i.settings_applied||0;if(!l&&!c&&!_){this._showToast(this._t("toast.store_download_nothing",{},"Nothing new to download - this setup is already on your device."),"info");return}await this._fetchProfiles(r),await this._fetchCycles(r);const p=i&&i.phases_applied||0;_?this._showToast(this._t("toast.store_device_downloaded_settings",{p:l,c,ph:p,s:_},`${l} program(s), ${c} recording(s), ${p} phase map(s), ${_} setting(s) added`)):p?this._showToast(this._t("toast.store_device_downloaded_phases",{p:l,c,ph:p},`${l} program(s), ${c} recording(s), ${p} phase map(s) added`)):this._showToast(this._t("toast.store_device_downloaded",{p:l,c},`${l} program(s), ${c} recording(s) added`))}catch(i){this._showToast(this._t("toast.store_download_failed",{error:i.message||i},"Download failed: "+(i.message||i)),"error")}})}else if(t==="store-import"){const a=e.dataset.cycleId,o=this._storeProfile&&this._storeProfile.program||"";this._modal={type:"store-import",cycleId:a,program:o,mode:"new"},this._render()}else if(t==="store-share-cycle"){const a=e.dataset.cid,o=e.dataset.prof||"";this._modal={type:"store-share",cycleId:a,program:o,profiles:null,deviceId:null},this._render(),this._loadShareProfiles()}else if(t==="store-share-device")this._modal={type:"store-share-device",selected:new Set,includePhases:new Set,includeSettings:!1,loading:!0},this._render(),(async()=>{try{const o=await this._ws({type:`${w}/get_shareable_cycles`,entry_id:r});this._shareableCycles=o&&o.items||[],this._sharePhasePrograms=o&&o.phase_programs||[],this._shareAllPrograms=o&&o.all_programs||[]}catch{this._shareableCycles=[],this._sharePhasePrograms=[],this._shareAllPrograms=[]}if(!this._isActiveEntry(r)||!this._modal||this._modal.type!=="store-share-device")return;const a=new Set;this._shareableByProgram().forEach(o=>o.cycles.forEach(i=>a.add(i.id))),this._modal.selected=a,this._modal.includePhases=new Set(this._sharePhasePrograms),this._modal.loading=!1,this._render()})();else if(t==="store-share-profile"){const a=e.dataset.prog||"";this._modal={type:"store-share-device",selected:new Set,includePhases:new Set,includeSettings:!1,loading:!0,focusProfile:a},this._render(),(async()=>{try{const i=await this._ws({type:`${w}/get_shareable_cycles`,entry_id:r});this._shareableCycles=i&&i.items||[],this._sharePhasePrograms=i&&i.phase_programs||[],this._shareAllPrograms=i&&i.all_programs||[]}catch{this._shareableCycles=[],this._sharePhasePrograms=[],this._shareAllPrograms=[]}if(!this._isActiveEntry(r)||!this._modal||this._modal.type!=="store-share-device")return;const o=new Set;this._shareableByProgram().filter(i=>i.program===a).forEach(i=>i.cycles.forEach(l=>o.add(l.id))),this._modal.selected=o,this._modal.includePhases=new Set((this._sharePhasePrograms||[]).filter(i=>i===a)),this._modal.loading=!1,this._render()})()}else if(t==="store-share-add-profile"){const a=this._modal,o=this._constants.storeWebOrigin;if(!o||!a){o||this._showToast(this._t("toast.store_unavailable",{},"The community store is not available."),"error");return}this._ensureStoreConnectListener();const i=new URLSearchParams({mode:"profile",device:a.deviceId||"",type:this._storeApplianceType(),brand:this._opts.store_brand||"",model:this._opts.store_model||"",origin:location.origin}).toString();window.open(o+"/create.html?"+i,"washdata_create","width=560,height=760")}}_onActAuto(t,e,s,r,n){if(t==="auto-new")this._navigate("/config/automation/edit/new");else if(t==="auto-new-started")this._newAutomationFromEvent("started");else if(t==="auto-new-finished")this._newAutomationFromEvent("finished");else if(t==="auto-delete"){const a=e.dataset.autoid,o=e.dataset.autoname||"this automation";this._modal={type:"confirm",title:this._t("modal.delete_automation_title",{},"Delete Automation"),message:this._t("modal.delete_automation_msg",{name:o},`Delete the automation "${o}" from Home Assistant? This cannot be undone.`),okLabel:this._t("btn.delete",{},"Delete"),onOk:async()=>{try{await this._hass.callApi("DELETE","config/automation/config/"+a),this._showToast(this._t("toast.automation_deleted",{},"Automation deleted")),await this._loadDeviceAutomations(r)}catch(i){this._showToast(this._t("toast.delete_failed",{error:i.message||i},"Delete failed: "+(i.message||i)),"error")}}},this._render()}else if(t==="auto-convert-legacy")this._convertLegacyActions();else if(t==="auto-remove-legacy")this._modal={type:"confirm",title:this._t("modal.remove_legacy_title",{},"Remove Legacy Actions"),message:this._t("modal.remove_legacy_msg",{},"Remove the legacy custom actions? They will stop firing on cycle events. This cannot be undone from the panel."),okLabel:this._t("btn.remove",{},"Remove"),onOk:async()=>{try{await this._ws({type:`${w}/set_options`,entry_id:r,options:{notify_actions:[]}}),this._opts={...this._opts,notify_actions:[]},this._showToast(this._t("toast.legacy_removed",{},"Legacy actions removed"))}catch(a){this._showToast(this._t("toast.delete_failed",{error:a.message||a},"Remove failed: "+(a.message||a)),"error")}}},this._render();else if(t==="auto-label"){const a=parseFloat(n.getElementById("wd-auto-label-threshold")?.value||"0.75");this._busyRun("auto-label",async()=>{try{await this._ws({type:`${w}/auto_label_cycles`,entry_id:r,confidence_threshold:a}),this._showToast(this._t("toast.auto_label_complete",{},"Auto-label complete")),await this._fetchCycles(r)}catch(o){this._showToast(this._t("toast.auto_label_failed",{error:o.message||o},"Auto-label failed: "+(o.message||o)),"error")}})}}_onActMaintenance(t,e,s,r,n){if(t==="maint-add"){const a=n.getElementById("wd-maint-type")?.value||"",o=n.getElementById("wd-maint-date")?.value||"",i=(n.getElementById("wd-maint-notes")?.value||"").trim();if(!a){this._showToast(this._t("toast.maint_add_failed",{error:this._t("lbl.event_type",{},"Event type")},"Could not add event: Event type"),"error");return}this._busyRun("maint-add",async()=>{try{const l={type:`${w}/add_maintenance_event`,entry_id:r,event_type:a};o&&(l.date=o),i&&(l.notes=i),await this._ws(l),await this._fetchMaintenance(r),this._showToast(this._t("toast.maint_added",{},"Maintenance event added")),this._render()}catch(l){this._showToast(this._t("toast.maint_add_failed",{error:l.message||l},"Could not add event: "+(l.message||l)),"error")}})}else if(t==="maint-delete"){const a=e.dataset.mid;this._modal={type:"confirm",title:this._t("modal.delete_maintenance_title",{},"Delete Maintenance Event"),message:this._t("modal.delete_maintenance_msg",{},"Delete this maintenance record? This cannot be undone."),okLabel:this._t("btn.delete",{},"Delete"),onOk:()=>this._busyRun("maint-delete",async()=>{try{await this._ws({type:`${w}/delete_maintenance_event`,entry_id:r,event_id:a}),await this._fetchMaintenance(r),this._showToast(this._t("toast.maint_deleted",{},"Maintenance event deleted"))}catch(o){this._showToast(this._t("toast.maint_delete_failed",{error:o.message||o},"Could not delete event: "+(o.message||o)),"error")}})},this._render()}else if(t==="maint-save-odometer"){const a=n.getElementById("wd-maint-odometer")?.value,o=parseInt(a,10);if(isNaN(o)||o<0){this._showToast(this._t("toast.odometer_save_failed",{error:this._t("lbl.total_cycles_run",{},"cycles run in total")},"Could not save total: cycles run in total"),"error");return}this._busyRun("maint-save-odometer",async()=>{try{await this._ws({type:`${w}/set_lifetime_cycle_count`,entry_id:r,count:o}),await this._fetchMaintenance(r),this._showToast(this._t("toast.odometer_saved",{},"Total cycles saved")),this._render()}catch(i){this._showToast(this._t("toast.odometer_save_failed",{error:i.message||i},"Could not save total: "+(i.message||i)),"error")}})}else if(t==="maint-save-reminders"){const a={};n.querySelectorAll("[data-maint-rem]").forEach(o=>{const i=o.dataset.maintRem,l=parseInt(o.value,10);a[i]=!isNaN(l)&&l>0?l:0}),this._busyRun("maint-save-reminders",async()=>{try{await this._ws({type:`${w}/set_options`,entry_id:r,options:{maintenance_reminder_cycles:a}}),await this._fetchMaintenance(r),this._showToast(this._t("toast.reminders_saved",{},"Service reminders saved")),this._render()}catch(o){this._showToast(this._t("toast.reminders_save_failed",{error:o.message||o},"Could not save reminders: "+(o.message||o)),"error")}})}}_onActPlayground(t,e,s,r){if(t==="pg-analysis-tab"){const n=e.dataset.subtab||"history";n!==this._pgAnalysisTab&&(this._pgAnalysisTab=n,this._render(),requestAnimationFrame(()=>this._drawPlaygroundCanvases()))}else if(t==="pg-run-history")this._pgRunHistory();else if(t==="pg-batch-cancel"){this._pgBatchCancel=!0;const n=this._pgHistoryTaskId||this._pgSweepTaskId;n&&this._ws({type:`${w}/cancel_task`,task_id:n}).catch(()=>{})}else if(t==="pg-load-run"){const n=e.dataset.taskId;if(!n)return;const a=this._tasks[n];if(!a||a.kind!=="pg_history"&&a.kind!=="pg_sweep"){this._showToast(this._t("toast.pg_run_gone",{},"That run is no longer available."),"info");return}const o=a.entry_id;this._ws({type:`${w}/get_task_result`,task_id:n}).then(i=>{if(!this._isActiveEntry(o))return;const l=i&&i.result;if(!l){this._showToast(this._t("toast.pg_run_gone",{},"That run is no longer available."),"info");return}a.kind==="pg_history"?(this._pgHistory=l,this._pgAnalysisTab="history"):(this._pgSweepNew=l.error?null:l,this._pgAnalysisTab="sweep"),this._render()}).catch(()=>this._showToast(this._t("toast.pg_run_gone",{},"That run is no longer available."),"info"))}else if(t==="pg-open-cycle")this._pgSelectCycle(e.dataset.cid);else if(t==="pg-sweep-run2")this._pgRunSweep2();else if(t==="pg-sweep-apply2")this._pgApplySweepValue(e.dataset.val);else if(t==="pg-run"||t==="pg-load")this._pgLoad();else if(t==="pg-cancel-run")this._pgCancelRun();else if(t==="pg-reset-params")this._pgThreshStart=null,this._pgThreshStop=null,this._pgParamOverrides={},this._pgStressTail=!1,this._pgStressIdleW=null,this._render(),requestAnimationFrame(()=>this._pgDrawCanvas());else if(t==="pg-apply-settings")this._pgApplyToSettings();else if(t==="pg-load-live")this._pgLoadLive();else if(t==="pg-load-suggested")this._pgLoadSuggested("classic");else if(t==="pg-load-calibrated")this._pgLoadSuggested("ml");else if(t==="pg-preset-save")this._pgSavePreset();else if(t==="pg-preset-load"){const n=(this._pgPresets||[]).find(a=>a.name===this._pgPresetSel);n&&(this._pgApplyPresetValues(n.values),this._showToast(this._t("toast.pg_preset_loaded",{name:n.name},`Preset "${n.name}" loaded`)),this._render(),requestAnimationFrame(()=>this._pgDrawCanvas()))}else t==="pg-preset-delete"?this._pgDeletePreset():t==="pg-publish-one"&&this._pgPublishOne(e.dataset.pgkey)}async _onModalAction(t,e){const s=this.shadowRoot,r=this._devices[this._selIdx],n=r?r.entry_id:null,a=this._modal;if(t==="cancel"){if(a&&a.type==="cycle-detail"&&this._prevModal){const o=this._devices[this._selIdx];o?await this._closeCycleDetail(o.entry_id):(this._modal=null,this._render())}else this._modal=null,this._render();return}if(t==="ok"&&a&&a.onOk){const o=a.onOk;this._modal=null,this._render(),await o(),this._render();return}if(a&&a.type==="profile-group"){if(t==="pg-save"){const o=s.getElementById("wd-pg-name")?.value?.trim(),i=Array.from(s.querySelectorAll(".wd-pg-mem")).filter(l=>l.checked).map(l=>l.value);if(!o){this._showToast(this._t("toast.group_name_required",{},"Group name is required"),"error");return}if(i.length<2){this._showToast(this._t("toast.min_2_profiles",{},"Select at least 2 profiles for a group"),"error");return}await this._busyRun("pg-save",async()=>{try{a.orig&&a.orig!==o&&await this._ws({type:`${w}/rename_profile_group`,entry_id:n,name:a.orig,new_name:o}),await this._ws({type:`${w}/save_profile_group`,entry_id:n,name:o,members:i}),this._showToast(this._t("toast.group_saved",{},"Group saved")),this._modal=null,await this._fetchProfileGroups(n)}catch(l){this._showToast(this._t("msg.toast_save_failed",{error:l.message||l},"Save failed: "+(l.message||l)),"error")}});return}if(t==="pg-delete"&&a.orig){await this._busyRun("pg-save",async()=>{try{await this._ws({type:`${w}/delete_profile_group`,entry_id:n,name:a.orig}),this._showToast(this._t("toast.group_deleted",{},"Group deleted")),this._modal=null,await this._fetchProfileGroups(n)}catch(o){this._showToast(this._t("msg.toast_delete_failed",{error:o.message||o},"Delete failed: "+(o.message||o)),"error")}});return}}if(t.startsWith("store-import-")||t==="store-share-ok"||t.startsWith("wiz-")||t.startsWith("imp-")||t==="import-back"||t==="import-analyze"||t==="import-apply-ok")return this._onMActImport(t,e,a,n,s);if(t.startsWith("hist-"))return this._onMActHistoryImport(t,e,a,n,s);if(t.startsWith("sd-")||t==="store-share-device-ok")return this._onMActStoreShare(t,e,a,n);if(a&&a.type==="export-select"&&t==="export-generate"&&n){const o=this._wizSelectionPayload(a);await this._busyRun("export-select",async()=>{try{const i=await this._ws({type:`${w}/export_config_selective`,entry_id:n,selection:o}),l=new Blob([i.json_data],{type:"application/json"}),c=URL.createObjectURL(l),_=document.createElement("a");_.href=c,_.download=`washdata_export_${n.slice(0,8)}.json`,document.body.appendChild(_),_.click(),document.body.removeChild(_),URL.revokeObjectURL(c),this._modal=null,this._showToast(this._t("toast.export_selective_done",{},"Export downloaded"))}catch(i){this._showToast(this._t("toast.export_failed",{error:i.message||i},"Export failed: "+(i.message||i)),"error")}});return}if(t.startsWith("cyc-")||t.startsWith("trim-mode-"))return this._onMActCycleDetail(t,a,n,s);if(a&&a.type==="profile-panel"){if(t.indexOf("pp-tab-")===0){const o=t.slice(7);a.tab=o,this._render(),o==="cleanup"&&!a.cleanup&&this._ws({type:`${w}/get_profile_cycles`,entry_id:n,profile_name:a.name}).then(i=>{this._modal&&this._modal.name===a.name&&(this._modal.cleanup={cycles:i.cycles||[],selected:new Set},this._render())}).catch(()=>{this._modal&&(this._modal.cleanup={cycles:[],selected:new Set},this._render())});return}return this._onMActProfilePanel(t,e,a,n,s)}if(t==="label-ok"&&n){const o=s.getElementById("wd-label-profile"),i=o?o.value:"",l=i||null,c=i==="__create_new__"&&s.getElementById("wd-new-profile-name")?.value?.trim()||null;this._modal=null;try{await this._ws({type:`${w}/label_cycle`,entry_id:n,cycle_id:a.cycleId,profile_name:l||null,new_profile_name:c}),this._showToast(this._t("toast.cycle_labelled",{},"Cycle labelled")),await this._fetchCycles(n),await this._fetchProfiles(n),await this._fetchFeedbacks(n)}catch(_){this._showToast(this._t("toast.label_failed",{error:_.message||_},"Label failed: "+(_.message||_)),"error")}this._render()}else if(t==="create-profile-ok"&&n){const o=s.getElementById("wd-cp-name")?.value?.trim(),i=s.getElementById("wd-cp-cycle")?.value||null,l=parseFloat(s.getElementById("wd-cp-dur")?.value||0);if(this._modal=null,!o){this._showToast(this._t("toast.profile_name_required",{},"Profile name is required"),"error"),this._render();return}const c=!i&&l>0?l:null;try{await this._ws({type:`${w}/create_profile`,entry_id:n,name:o,reference_cycle:i||null,manual_duration_min:c}),this._showToast(this._t("toast.profile_created",{name:o},`Profile "${o}" created`)),await this._fetchProfiles(n)}catch(_){this._showToast(this._t("toast.create_failed",{error:_.message||_},"Create failed: "+(_.message||_)),"error")}this._render()}else if(t==="create-phase-ok"&&n){const o=s.getElementById("wd-ph-name")?.value?.trim(),i=s.getElementById("wd-ph-desc")?.value?.trim()||"";if(this._modal=null,!o){this._showToast(this._t("toast.phase_name_required",{},"Phase name is required"),"error"),this._render();return}try{await this._ws({type:`${w}/create_phase`,entry_id:n,device_type:a.deviceType||"",name:o,description:i}),this._showToast(this._t("toast.phase_created",{name:o},`Phase "${o}" created`)),await this._fetchPhases(n)}catch(l){this._showToast(this._t("msg.toast_create_failed",{error:l.message||l},"Create failed: "+(l.message||l)),"error")}this._render()}else if(t==="edit-phase-ok"&&n){const o=s.getElementById("wd-eph-name")?.value?.trim(),i=s.getElementById("wd-eph-desc")?.value?.trim()||"";if(this._modal=null,!o){this._showToast(this._t("toast.name_required",{},"Name required"),"error"),this._render();return}try{await this._ws({type:`${w}/update_phase`,entry_id:n,phase_id:a.phaseId,new_name:o,description:i}),this._showToast(this._t("toast.phase_updated",{},"Phase updated")),await this._fetchPhases(n)}catch(l){this._showToast(this._t("toast.update_failed",{error:l.message||l},"Update failed: "+(l.message||l)),"error")}this._render()}else if(t==="process-rec-ok"&&n){const o=s.getElementById("wd-pr-mode")?.value;let i=s.getElementById("wd-pr-profile")?.value?.trim();o==="existing_profile"&&(i=s.getElementById("wd-pr-profile-sel")?.value||i);const l=parseFloat(s.getElementById("wd-pr-head")?.value||0),c=parseFloat(s.getElementById("wd-pr-tail")?.value||0);if(this._modal=null,!i){this._showToast(this._t("msg.toast_profile_name_required",{},"Profile name is required"),"error"),this._render();return}try{await this._ws({type:`${w}/process_recording`,entry_id:n,profile_name:i,save_mode:o,head_trim:l,tail_trim:c}),this._showToast(this._t("toast.recording_saved",{},"Recording saved to profile")),await this._fetchRecState(n),await this._fetchProfiles(n)}catch(_){this._showToast(this._t("msg.toast_save_failed",{error:_.message||_},"Save failed: "+(_.message||_)),"error")}this._render()}else if(t==="correct-fb-ok"&&n){const o=s.getElementById("wd-fb-profile")?.value,i=parseFloat(s.getElementById("wd-fb-dur")?.value||0)||null;this._modal=null;try{await this._ws({type:`${w}/resolve_feedback`,entry_id:n,cycle_id:a.cycleId,action:"correct",corrected_profile:o,corrected_duration_min:i}),this._showToast(this._t("toast.correction_submitted",{},"Correction submitted")),await this._fetchFeedbacks(n)}catch(l){this._showToast(this._t("msg.toast_error",{error:l.message||l},"Error: "+(l.message||l)),"error")}this._render()}else if(t==="import-ok"&&n){const o=s.getElementById("wd-import-json")?.value;if(this._modal=null,!o?.trim()){this._showToast(this._t("toast.json_required",{},"JSON data is required"),"error"),this._render();return}try{await this._ws({type:`${w}/import_config`,entry_id:n,json_data:o}),this._showToast(this._t("toast.import_successful",{},"Import successful; integration reloading")),await this._fetchCycles(n)}catch(i){this._showToast(this._t("toast.import_failed",{error:i.message||i},"Import failed: "+(i.message||i)),"error")}this._render()}else if(t==="auto-run"&&n){const o=parseFloat(s.getElementById("wd-al-thr")?.value||"0.75");this._modal=null,this._render(),await this._busyRun("auto-label",async()=>{try{await this._ws({type:`${w}/auto_label_cycles`,entry_id:n,confidence_threshold:o}),this._showToast(this._t("msg.toast_auto_label_complete",{},"Auto-label complete")),await this._fetchCycles(n)}catch(i){this._showToast(this._t("msg.toast_auto_label_failed",{error:i.message||i},"Auto-label failed: "+(i.message||i)),"error")}})}else if(t==="merge-ok"&&n){const o=s.getElementById("wd-merge-prof")?.value||"",i=s.getElementById("wd-merge-newname")?.value?.trim()||null,l=a.ids||[];this._modal=null,this._render(),this._kickAndTrack({type:`${w}/apply_merge`,entry_id:n,cycle_ids:l,target_profile:o||null,new_profile_name:i},"cyc-merge",async()=>{this._showToast(this._t("toast.cycles_merged",{},"Cycles merged")),this._cycleSel.clear(),this._selectMode=!1,await this._fetchCycles(n),await this._fetchProfiles(n)})}else if(t==="bulk-relabel-ok"&&n){const o=s.getElementById("wd-relabel-profile")?.value||"",i=(a.ids||[]).slice();if(this._modal=null,this._render(),!i.length)return;await this._busyRun("cyc-relabel",async()=>{try{for(const l of i)await this._ws({type:`${w}/label_cycle`,entry_id:n,cycle_id:l,profile_name:o||null});this._showToast(this._t("toast.relabel_done",{count:i.length},`Relabelled ${i.length} cycle(s)`)),this._cycleSel.clear(),this._selectMode=!1,await this._fetchCycles(n),await this._fetchProfiles(n),await this._fetchFeedbacks(n)}catch(l){this._showToast(this._t("toast.relabel_failed",{error:l.message||l},"Relabel failed: "+(l.message||l)),"error")}})}}async _histUpload(t,e){const s=await this._ws({type:`${w}/history_import_begin`,entry_id:t}),r=Math.max(4096,s.chunk_bytes||512*1024);let n=0,a=0;for(;a<e.length;){let o=Math.min(e.length,a+r);if(o<e.length){const i=e.lastIndexOf(`
`,o);i>a&&(o=i+1)}await this._ws({type:`${w}/history_import_chunk`,entry_id:t,token:s.token,seq:n,text:e.slice(a,o)}),n+=1,a=o}return s.token}async _histStartScan(t,e,s){e.token=s,e.step="scan",e.error=null,this._render();const r=await this._ws({type:`${w}/start_history_import_scan`,entry_id:t,token:s});e.scanTaskId=r.task_id,this._addProvisionalTask(r.task_id,"history_import",t,0),this._tasksSubscribed||this._pollTaskGeneric(r.task_id),this._histAdopt(r.task_id)}async _histAdopt(t){if(!t)return;let e=(this._tasks||{})[t];if(!e||e.state==="running")try{e=await this._ws({type:`${w}/get_task_result`,task_id:t})}catch{return}e&&e.state&&e.state!=="running"&&this._histTaskFinished(e)}async _histTaskFinished(t){const e=this._modal;if(!(!e||e.type!=="history-import")&&!(t.id!==e.scanTaskId&&t.id!==e.applyTaskId)&&(this._histSettled=this._histSettled||new Set,!this._histSettled.has(t.id))){if(this._histSettled.add(t.id),t.id===e.scanTaskId){if(t.state==="error"){e.step="input",e.error=t.error||this._t("msg.hist_scan_failed",{},"Scanning failed."),this._render();return}try{const s=t.result?t:await this._ws({type:`${w}/get_task_result`,task_id:t.id});e.result=s.result||{},e.accept=new Set((e.result.segments||[]).filter(r=>r.accept).map(r=>r.index)),e.step="review",this._render(),requestAnimationFrame(()=>this._drawHistorySparklines())}catch(s){e.step="input",e.error=String(s&&s.message||s),this._render()}return}if(t.id===e.applyTaskId){if(t.state==="error"){e.error=t.error==="scan_expired"?this._t("msg.hist_scan_expired",{},"That scan is no longer available. Please scan again."):t.error||this._t("msg.hist_import_failed",{},"Import failed."),e.step="review",this._render();return}try{const r=t.result?t:await this._ws({type:`${w}/get_task_result`,task_id:t.id});e.done=r.result||{}}catch{e.done={}}e.step="done",this._render();const s=this._devices[this._selIdx];s&&(this._fetchCycles(s.entry_id),this._fetchProfiles(s.entry_id))}}}async _onMActHistoryImport(t,e,s,r,n){if(!(!s||s.type!=="history-import"||!r)){if(t==="hist-scan"){const a=n.getElementById("wd-hist-csv"),o=a?a.value:s.csvText||"";if(!o.trim()){this._showToast(this._t("toast.hist_csv_required",{},"Load a CSV file or paste its contents first"),"error");return}s.csvText=o,await this._busyRun("hist-import",async()=>{try{const i=await this._histUpload(r,o);await this._histStartScan(r,s,i)}catch(i){s.error=String(i&&i.message||i),this._render()}});return}if(t==="hist-recorder"){const a=n.getElementById("wd-hist-since"),o=a&&a.value||jt();s.since=o,await this._busyRun("hist-import",async()=>{try{const i=await this._ws({type:`${w}/history_import_recorder`,entry_id:r,start_date:o});if(!i.rows){s.error=this._t("msg.hist_recorder_empty",{},"Home Assistant has no detailed history for this sensor in that window."),this._render();return}await this._histStartScan(r,s,i.token)}catch(i){s.error=String(i&&i.message||i),this._render()}});return}if(t==="hist-cancel-scan"){if(s.scanTaskId)try{await this._ws({type:`${w}/cancel_task`,task_id:s.scanTaskId})}catch{}s.step="input",s.scanTaskId=null,this._render();return}if(t==="hist-back"){s.step="input",s.result=null,s.error=null,this._render();return}if(t==="hist-toggle-all"){const a=s.result&&s.result.segments||[],o=a.every(i=>s.accept.has(i.index));s.accept=o?new Set:new Set(a.map(i=>i.index)),this._render(),requestAnimationFrame(()=>this._drawHistorySparklines());return}if(t==="hist-apply"){if(!s.accept||!s.accept.size)return;await this._busyRun("hist-apply",async()=>{try{const a=await this._ws({type:`${w}/apply_history_import`,entry_id:r,scan_task_id:s.scanTaskId,accept:[...s.accept].sort((o,i)=>o-i)});s.applyTaskId=a.task_id,this._addProvisionalTask(a.task_id,"history_import_apply",r,s.accept.size),this._tasksSubscribed||this._pollTaskGeneric(a.task_id),this._histAdopt(a.task_id)}catch(a){s.error=String(a&&a.message||a),this._render()}});return}if(t==="hist-goto-cycles"){this._modal=null,this._tab="history",this._cycleFilter={...this._cycleFilter||{},status:"imported"},this._fetchTabData();return}}}async _onMActImport(t,e,s,r,n){if(s&&s.type==="store-import"){if(t==="store-import-mode-new"){s.mode="new",this._render();return}if(t==="store-import-mode-merge"){s.mode="merge",this._render();return}if(t==="store-import-ok"){const a={type:`${w}/store_import_cycle`,entry_id:r,cycle_id:s.cycleId};if(s.mode==="merge"){const o=n.getElementById("wd-store-import-target")?.value||"";if(!o){this._showToast(this._t("toast.store_pick_profile",{},"Pick a profile to merge into"),"error");return}a.target_profile=o}else{const o=(n.getElementById("wd-store-import-name")?.value||"").trim()||s.program;if(!o){this._showToast(this._t("toast.store_name_required",{},"Enter a profile name"),"error");return}a.new_profile_name=o}await this._busyRun("store-import",async()=>{try{const o=await this._ws(a);if(o&&o.error){this._showToast(this._t("toast.store_import_failed",{error:o.error},"Import failed: "+o.error),"error");return}this._modal=null,this._showToast(this._t("toast.store_imported",{profile:o&&o.profile||""},`Imported into ${o&&o.profile||"profile"}`)),await this._fetchProfiles(r)}catch(o){this._showToast(this._t("toast.store_import_failed",{error:o.message||o},"Import failed: "+(o.message||o)),"error")}});return}}if(s&&s.type==="store-share"&&t==="store-share-ok"){const a=(n.getElementById("wd-store-share-prog")?.value||"").trim(),o=(n.getElementById("wd-store-share-desc")?.value||"").trim();if(!a){this._showToast(this._t("toast.store_pick_profile",{},"Pick a profile to share into"),"error");return}await this._busyRun("store-share",async()=>{try{const i=await this._ws({type:`${w}/store_upload_cycle`,entry_id:r,local_cycle_id:s.cycleId,program:a,description:o});if(i&&i.error){if(i.error==="no_appliance_declared")this._showToast(this._t("toast.store_no_appliance",{},"Set your appliance brand and model in Settings first."),"error");else{const l=i.detail?`${i.error} - ${i.detail}`:i.error;this._showToast(this._t("toast.store_share_failed",{error:l},"Share failed: "+l),"error")}return}this._modal=null,this._showToast(this._t("toast.store_shared",{},"Shared to the community store - pending review."))}catch(i){this._showToast(this._t("toast.store_share_failed",{error:i.message||i},"Share failed: "+(i.message||i)),"error")}});return}if(s&&(s.type==="export-select"||s.type==="import-wizard")){const a=s.type==="export-select"?{categories:s.inventory||{}}:s.manifest||{categories:{}};if(t==="wiz-toggle-all"){const o=a.categories||{},i=s.type==="import-wizard";let l=!0;this._wizCatOrder().filter(c=>o[c]&&o[c].present).forEach(c=>{i&&o[c].importable===!1||this._wizCatState(s,c,a).state!=="all"&&(l=!1)}),s.sel=l?{cats:new Set,profiles:new Set,realIds:new Set,refIds:new Set}:this._wizInitSel(a,i),this._render();return}if(t==="wiz-toggle-cat"){const o=e.dataset.cat,l=this._wizCatState(s,o,a).state!=="all";if(o==="profiles"){const c=a.categories.profiles&&a.categories.profiles.items||[];s.sel.profiles=new Set(l?c.map(_=>_.name):[])}else if(o==="real_cycles"||o==="reference_cycles"){const c=new Set;l&&(a.categories[o]&&a.categories[o].groups||[]).forEach(_=>_.cycles.forEach(p=>{p.id!=null&&c.add(String(p.id))})),o==="real_cycles"?s.sel.realIds=c:s.sel.refIds=c}else l?s.sel.cats.add(o):s.sel.cats.delete(o);this._render();return}if(t==="wiz-toggle-profile"){const o=e.dataset.name;s.sel.profiles.has(o)?s.sel.profiles.delete(o):s.sel.profiles.add(o),this._render();return}if(t==="wiz-toggle-cycgroup"){const o=e.dataset.cat,i=e.dataset.prof,l=o==="real_cycles"?s.sel.realIds:s.sel.refIds,c=this._wizGroupIds(a,o,i),_=c.length>0&&c.every(p=>l.has(p));c.forEach(p=>{_?l.delete(p):l.add(p)}),this._render();return}if(t==="wiz-toggle-cyc"){const o=e.dataset.cat,i=e.dataset.cid,l=o==="real_cycles"?s.sel.realIds:s.sel.refIds;l.has(i)?l.delete(i):l.add(i),this._render();return}if(t==="wiz-expand"){const o=e.dataset.key;s.expanded||(s.expanded=new Set),s.expanded.has(o)?s.expanded.delete(o):s.expanded.add(o),this._render();return}}if(s&&s.type==="import-wizard"){if(t==="import-back"){s.step="input",s.error=null,this._render();return}if(t==="imp-mode-merge"){s.mode="merge",this._render();return}if(t==="imp-mode-replace"){s.mode="replace",this._render();return}if(t==="imp-dest-reference"){s.cycleDest="reference",this._render();return}if(t==="imp-dest-real"){(s.manifest||{}).real_history_allowed!==!1&&(s.cycleDest="real_history",this._render());return}if(t==="imp-conflict"){s.conflicts[e.dataset.prof]=e.value;return}if(t==="import-analyze"&&r){const a=n.getElementById("wd-import-json"),o=a?a.value:s.jsonText||"";if(s.jsonText=o,!o.trim()){this._showToast(this._t("toast.json_required",{},"JSON data is required"),"error");return}s.step="analyze",s.error=null,this._render();try{const i=await this._ws({type:`${w}/analyze_import`,entry_id:r,json_data:o});if(!this._isActiveEntry(r)||!this._modal||this._modal.type!=="import-wizard")return;const l=i&&i.manifest||{};if(l.error){s.step="input",s.error=l.error,this._render();return}s.manifest=l,s.sel=this._wizInitSel(l,!0),s.conflicts={},(l.categories&&l.categories.profiles&&l.categories.profiles.items||[]).forEach(c=>{c.conflict&&(s.conflicts[c.name]="import_as_copy")}),s.step="select",this._render()}catch(i){if(!this._isActiveEntry(r)||!this._modal||this._modal.type!=="import-wizard")return;s.step="input",s.error=i&&i.message||String(i),this._render()}return}if(t==="import-apply-ok"&&r){const a=this._wizSelectionPayload(s);await this._busyRun("import-wizard",async()=>{try{const o=await this._ws({type:`${w}/import_config_selective`,entry_id:r,json_data:s.jsonText,selection:a,mode:s.mode,conflict_resolutions:s.conflicts,cycle_destination:s.cycleDest,apply_settings:!0}),i=o&&o.summary||{};this._modal=null,this._showToast(this._t("toast.import_selective_done",{profiles:i.profiles_imported||0,cycles:(i.real_cycles_imported||0)+(i.reference_cycles_imported||0)},`Imported ${i.profiles_imported||0} profile(s) and ${(i.real_cycles_imported||0)+(i.reference_cycles_imported||0)} cycle(s)`)),await this._fetchCycles(r),await this._fetchProfiles(r)}catch(o){this._showToast(this._t("toast.import_failed",{error:o.message||o},"Import failed: "+(o.message||o)),"error")}});return}}}async _onMActStoreShare(t,e,s,r){if(s&&s.type==="store-share-device"){if(t==="sd-toggle-cyc"){const n=e.dataset.cid;s.selected.has(n)?s.selected.delete(n):s.selected.add(n),this._render();return}if(t==="sd-toggle-prof"){const n=e.dataset.prog,a=this._shareableByProgram().find(o=>o.program===n);if(a){const o=a.cycles.every(i=>s.selected.has(i.id));a.cycles.forEach(i=>{o?s.selected.delete(i.id):s.selected.add(i.id)})}this._render();return}if(t==="sd-toggle-phases"){const n=e.dataset.prog;s.includePhases||(s.includePhases=new Set),s.includePhases.has(n)?s.includePhases.delete(n):s.includePhases.add(n),this._render();return}if(t==="sd-toggle-settings"){s.includeSettings=!s.includeSettings,this._render();return}if(t==="sd-toggle-consent"){s.consented=!s.consented,this._render();return}if(t==="sd-toggle-guide"){s.guideOpen=!s.guideOpen,this._render();return}if(t==="store-share-device-ok"){const n=new Map;(this._shareableCycles||[]).forEach(l=>n.set(l.id,(l.profile_name||"").trim()));const a=Array.from(s.selected).map(l=>({local_cycle_id:l,program:n.get(l)||""})).filter(l=>l.program);if(!a.length){this._showToast(this._t("toast.share_device_none_sel",{},"Select at least one cycle to share"),"error");return}const o=new Set(a.map(l=>l.program)),i=Array.from(s.includePhases||[]).filter(l=>o.has(l));await this._busyRun("store-share-device",async()=>{try{const l=await this._ws({type:`${w}/store_upload_device`,entry_id:r,items:a,include_phases:i,include_settings:!!s.includeSettings});if(l&&l.error){if(l.error==="no_appliance_declared")this._showToast(this._t("toast.store_no_appliance",{},"Set your appliance brand and model in Settings first."),"error");else{const g=l.detail?`${l.error} - ${l.detail}`:l.error;this._showToast(this._t("toast.store_share_failed",{error:g},"Share failed: "+g),"error")}return}const c=l&&l.cycle_ids&&l.cycle_ids.length||0,_=l&&l.errors&&l.errors.length||0,p=l&&l.duplicates||0,f=l&&l.created!=null?l.created:c;if(!c){const g=l&&l.errors&&l.errors[0]||l&&l.detail||"upload_failed";this._showToast(this._t("toast.store_share_failed",{error:g},"Share failed: "+g),"error");return}this._modal=null,_?this._showToast(this._t("toast.store_device_shared_partial",{n:c,failed:_},`Shared ${c} cycle(s); ${_} could not be uploaded.`),"info"):p&&!f?this._showToast(this._t("toast.store_device_shared_all_dup",{n:p},`All ${p} cycle(s) were already in the community store.`),"info"):p?this._showToast(this._t("toast.store_device_shared_some_dup",{created:f,dup:p},`Shared ${f} cycle(s); ${p} were already in the store.`)):this._showToast(this._t("toast.store_device_shared",{n:f},`Shared ${f} cycle(s) to the community store - pending review.`))}catch(l){this._showToast(this._t("toast.store_share_failed",{error:l.message||l},"Share failed: "+(l.message||l)),"error")}});return}}}async _onMActCycleDetail(t,e,s,r){if(e&&e.type==="cycle-detail"){if(t==="cyc-view"){e.mode="view",this._render();return}if(t==="cyc-trim"){e.mode="trim",(!e.trim||e.trim.end<=0)&&(e.trim={start:0,end:e.curve&&e.curve.full_duration_s||0}),this._render();return}if(t==="cyc-split"){e.mode="split",this._render();return}if(t==="cyc-review"){e.mode="review",this._render();return}if(t==="cyc-review-save"){const n=e.cycleId,a=r.getElementById("wd-cyc-rev-quality")?.value||"",o=!!r.getElementById("wd-cyc-rev-golden")?.checked,i=r.getElementById("wd-cyc-rev-notes")?.value||"",l=Array.from(r.querySelectorAll(".wd-cyc-rev-tag")).filter(p=>p.checked).map(p=>p.value),c=r.getElementById("wd-cyc-rev-label")?.value??"",_=e.curve&&e.curve.profile_name||"";await this._busyRun("cyc-review-save",async()=>{try{await this._ws({type:`${w}/set_ml_review`,entry_id:s,cycle_id:n,quality:a,golden:o,tags:l,notes:i}),c!==_&&await this._ws({type:`${w}/label_cycle`,entry_id:s,cycle_id:n,profile_name:c||null}),this._showToast(this._t("toast.review_saved",{},"Review saved")),await this._fetchCycles(s),c!==_&&await this._fetchFeedbacks(s),await this._loadMlIndex(s),this._modal&&this._modal.cycleId===n&&(this._modal.ml=(this._mlById||{})[n]||this._modal.ml)}catch(p){this._showToast(this._t("msg.toast_save_failed",{error:p.message||p},"Save failed: "+(p.message||p)),"error")}});return}if(t==="trim-mode-s"){e.timeMode="s",this._render();return}if(t==="trim-mode-clock"){e.timeMode="clock",this._render();return}if(t==="cyc-reset-trim"){e.trim={start:0,end:e.curve&&e.curve.full_duration_s||0},this._render();return}if(t==="cyc-clear-split"){e.split={offsets:[],profiles:[]},this._render();return}if(t==="cyc-label"){this._profiles.length||await this._fetchProfiles(s),this._modal={type:"label-cycle",cycleId:e.cycleId},this._render();return}if(t==="cyc-delete"){const n=e.cycleId;this._modal=null,this._render(),this._deleteCyclesWithUndo(s,[n]);return}if(t==="cyc-auto-split"){const n=parseInt(r.getElementById("wd-split-gap")?.value||"900",10);await this._busyRun("cyc-auto",async()=>{try{const a=await this._ws({type:`${w}/analyze_split`,entry_id:s,cycle_id:e.cycleId,gap_seconds:n});e.split.offsets=(a.split_offsets||[]).slice(),e.split.profiles=[],e.split.offsets.length||this._showToast(this._t("toast.no_split_found",{},"No idle gaps found to split on"),"info")}catch(a){this._showToast(this._t("toast.auto_detect_failed",{error:a.message||a},"Auto-detect failed: "+(a.message||a)),"error")}});return}if(t==="cyc-apply-trim"){const n=e.cycleId,a=e.trim.start,o=e.trim.end,i=e.curve&&e.curve.full_duration_s||0,l=i>0?Math.max(0,Math.round((o-a)/i*100)):100;if(l<50&&!confirm(this._t("msg.trim_destructive_confirm",{pct:l},`This keeps only ${l}% of the cycle and cannot be undone. Continue?`)))return;this._kickAndTrack({type:`${w}/trim_cycle`,entry_id:s,cycle_id:n,start_s:a,end_s:o},"cyc-trim-apply",async()=>{this._showToast(this._t("toast.cycle_trimmed",{},"Cycle trimmed")),await this._closeCycleDetail(s),await this._fetchCycles(s)});return}if(t==="cyc-apply-split"){const n=e.cycleId,a=e.split.offsets.slice(),o=e.split.profiles.slice();this._kickAndTrack({type:`${w}/apply_split`,entry_id:s,cycle_id:n,split_offsets:a,segment_profiles:o},"cyc-split-apply",async i=>{this._showToast(this._t("toast.split_complete",{count:(i.new_ids||[]).length},`Split into ${(i.new_ids||[]).length} cycles`)),await this._closeCycleDetail(s),await this._fetchCycles(s),await this._fetchProfiles(s)});return}}}async _onMActProfilePanel(t,e,s,r,n){if(s&&s.type==="profile-panel"){if(t==="pp-phase-add"){const a=s.env&&s.env.target_duration||(s.env&&s.env.avg&&s.env.avg.length?s.env.avg[s.env.avg.length-1][0]:600),o=s.phases.length?s.phases[s.phases.length-1].end:0,i=Math.min(o,a);s.phases.push({name:s.catalog[0]||"",start:i,end:Math.min(i+Math.max(60,a*.1),a)}),this._render();return}if(t==="pp-phase-rm"){const a=+(e&&e.dataset.idx||-1);a>=0&&(s.phases.splice(a,1),this._render());return}if(t==="pp-phase-save"){const a=s.phases.filter(o=>o.name).map(o=>({name:o.name,start:o.start,end:o.end}));await this._busyRun("pp-phase-save",async()=>{try{await this._ws({type:`${w}/set_profile_phases`,entry_id:r,profile_name:s.name,phases:a}),this._showToast(this._t("toast.phases_saved",{},"Phases saved"))}catch(o){this._showToast(this._t("msg.toast_save_failed",{error:o.message||o},"Save failed: "+(o.message||o)),"error")}});return}if(t==="pp-cleanup-del"){const a=s.cleanup?Array.from(s.cleanup.selected):[];if(!a.length)return;await this._busyRun("pp-cleanup-del",async()=>{try{for(const i of a)await this._ws({type:`${w}/delete_cycle`,entry_id:r,cycle_id:i});this._showToast(this._t("toast.cycles_deleted",{count:a.length},`Deleted ${a.length} cycle(s)`));const o=await this._ws({type:`${w}/get_profile_cycles`,entry_id:r,profile_name:s.name});this._modal&&(this._modal.cleanup={cycles:o.cycles||[],selected:new Set}),await this._fetchProfiles(r)}catch(o){this._showToast(this._t("msg.toast_delete_failed",{error:o.message||o},"Delete failed: "+(o.message||o)),"error")}});return}if(t==="pp-cleanup-unlabel"){const a=s.cleanup?Array.from(s.cleanup.selected):[];if(!a.length)return;await this._busyRun("pp-cleanup-unlabel",async()=>{try{for(const i of a)await this._ws({type:`${w}/label_cycle`,entry_id:r,cycle_id:i,profile_name:null});this._showToast(this._t("toast.unlabel_done",{count:a.length},`Unlabelled ${a.length} cycle(s)`));const o=await this._ws({type:`${w}/get_profile_cycles`,entry_id:r,profile_name:s.name});this._modal===s&&(s.cleanup={cycles:o.cycles||[],selected:new Set}),await this._fetchProfiles(r),await this._fetchCycles(r),await this._fetchFeedbacks(r)}catch(o){this._showToast(this._t("toast.unlabel_failed",{error:o.message||o},"Unlabel failed: "+(o.message||o)),"error")}});return}if(t==="pp-rename"){const a=n.getElementById("wd-pp-rename")?.value?.trim(),o=parseFloat(n.getElementById("wd-pp-dur")?.value||"0");if(!a){this._showToast(this._t("msg.toast_name_required",{},"Name required"),"error");return}try{await this._ws({type:`${w}/rename_profile`,entry_id:r,profile_name:s.name,new_name:a,manual_duration_min:o>0?o:null}),this._showToast(this._t("toast.profile_renamed",{},"Profile renamed")),s.name=a,await Promise.all([this._fetchProfiles(r),this._fetchProfileGroups(r)]),s.stats=(this._profiles||[]).find(i=>i.name===a)||s.stats,this._render()}catch(i){this._showToast(this._t("toast.rename_failed",{error:i.message||i},"Rename failed: "+(i.message||i)),"error")}return}if(t==="pp-rebuild"){this._kickAndTrack({type:`${w}/rebuild_envelopes`,entry_id:r},"pp-rebuild",async()=>{try{const a=await this._ws({type:`${w}/get_profile_envelope`,entry_id:r,profile_name:s.name});this._modal&&(this._modal.env=a.envelope)}catch{}this._showToast(this._t("toast.envelope_rebuilt",{},"Envelope rebuilt"))});return}if(t==="pp-delete"){this._deleteProfileWithUndo(r,s.name);return}}}_snapshotFormToPending(t){t&&t.querySelectorAll("#wd-settings-form [data-opt], #wd-ml-form [data-opt]").forEach(e=>{const s=e.dataset.opt;if(!this._dirtyOptKeys.has(s))return;const r=Ct[s],n=r&&r.type||e.dataset.ftype||"text";if(e.type==="checkbox"){this._pendingSettings[s]=e.checked;return}if(n==="checkboxlist"){this._pendingSettings[s]=this._collectCheckboxlist(e,s);return}if(n==="entitylist"){this._pendingSettings[s]=Array.from(e.querySelectorAll(".wd-pill")).map(a=>a.dataset.val).filter(Boolean);return}if(n==="timerlist"){this._pendingSettings[s]=Array.from(e.querySelectorAll(".wd-timer-row")).map(a=>({offset_minutes:parseFloat(a.querySelector('[data-field="offset_minutes"]').value)||0,message:(a.querySelector('[data-field="message"]').value||"").trim(),auto_pause:a.querySelector('[data-field="auto_pause"]').checked})).filter(a=>a.offset_minutes>0);return}if(n==="number"){const a=String(e.value).trim();if(a===""){r&&r.clearable?this._pendingSettings[s]=null:delete this._pendingSettings[s];return}const o=parseFloat(a);isNaN(o)||(this._pendingSettings[s]=o);return}if(n==="list"){this._pendingSettings[s]=String(e.value).split(",").map(a=>a.trim()).filter(Boolean);return}if(n==="intlist"){this._pendingSettings[s]=le(e.value);return}if(n==="json"){const a=String(e.value).trim();if(!a){this._pendingSettings[s]=[];return}try{this._pendingSettings[s]=JSON.parse(a)}catch{}return}if(n==="entity"||n==="device"){const a=String(e.value).trim();this._pendingSettings[s]=a||null;return}this._pendingSettings[s]=e.value})}_collectCheckboxlist(t,e){const s=Array.from(t.querySelectorAll("[data-choice]")).filter(n=>n.checked).map(n=>n.dataset.choice);if(s.length)return s;const r=Ct[e];return r&&Array.isArray(r.def)&&r.def.length?r.def.slice():s}_conflictKeysForOpts(t,e){const s=Object.assign({},e||{},t),r=this._conflictCtx(),n=new Set;for(const a of Jt)if(a.check(s,r))for(const o of Object.keys(a.fieldErrors(s,r)))n.add(o);return n}_conflictCtx(){const t=this._hass&&this._hass.states?this._hass.states:{};return{stateOf:e=>e&&t[e]||null}}_conflictCountForOpts(t,e){return this._conflictKeysForOpts(t,e).size}_conflictKeysFromOpts(){return this._conflictKeysForOpts(this._editedOpts(),this._optDefaults)}_readSettingsFormValues(t){const e=Object.assign({},this._optDefaults,this._opts,this._pendingSettings);return t&&t.querySelectorAll("#wd-settings-form [data-opt]").forEach(s=>{const r=s.dataset.opt;if(s.type==="checkbox"){e[r]=s.checked;return}if(s.dataset.ftype==="checkboxlist"){e[r]=this._collectCheckboxlist(s,r);return}if(s.dataset.ftype==="entity"||s.dataset.ftype==="device"){e[r]=String(s.value).trim()||null;return}const n=parseFloat(s.value);isNaN(n)?s.value!==""&&(e[r]=s.value):e[r]=n}),e}_liveValidateSettings(t){if(!t)return{};const e=this._readSettingsFormValues(t),s=t.getElementById("wd-settings-form");if(!s)return{};const r={};for(const i of this._suggestions||[])i.key!=null&&i.suggested!=null&&(r[i.key]=+i.suggested);const n={},a=this._conflictCtx();for(const i of Jt){if(!i.check(e,a))continue;const l=i.fieldErrors(e,a);for(const[c,_]of Object.entries(l)){const p=r[c],f=p!=null&&!i.check({...e,[c]:p},a)?{..._,suggFix:p}:_;(n[c]=n[c]||[]).push(f)}}const o={};for(const i of fe)if(i.check(e,a))for(const[l,c]of Object.entries(i.fieldNotes(e,a)))(o[l]=o[l]||[]).push(c);return s.querySelectorAll("[data-cnote]").forEach(i=>{const l=o[i.dataset.cnote];if(!l||!l.length){i.hidden=!0,i.innerHTML="";return}i.hidden=!1,i.innerHTML=l.map(c=>d(this._t(c.msgKey,c.msgVars,c.msgFb))).join("<br>")}),s.querySelectorAll("[data-cerr]").forEach(i=>{const l=i.dataset.cerr,c=n[l],_=s.querySelector(`.wd-field[data-field="${l}"]`);!c||!c.length?(i.hidden=!0,i.innerHTML="",_&&_.classList.remove("wd-has-conflict")):(i.hidden=!1,_&&_.classList.add("wd-has-conflict"),i.innerHTML=c.map(p=>{const f=this._t(p.msgKey,p.msgVars,p.msgFb);let g="";if(p.suggFix!=null){const $=+p.suggFix.toFixed(2);g=`<span class="wd-conflict-sug-note">${this._t("conflict.suggestion_resolves",{val:$},`Stage the pending suggestion (${$}) below to fix this`)}</span>`}else if(p.fixVal!=null&&!isNaN(+p.fixVal)){const $=Number.isInteger(p.fixVal)?p.fixVal:+p.fixVal.toFixed(2);g=`<button type="button" class="wd-conflict-fix" data-ckey="${l}" data-cval="${p.fixVal}">${this._t("conflict.use_fix",{val:$},`Use ${$}`)}</button>`}return`<div class="wd-conflict-row">\u26A0 ${d(f)}${g}</div>`}).join(""))}),n}_cascadeConflictFix(t,e,s){const r=new Set;for(let n=0;n<10;n++){const a=this._liveValidateSettings(t);let o=!1;for(const[i,l]of Object.entries(a)){if(i===s)continue;const c=l.find(p=>p.fixVal!=null&&!isNaN(+p.fixVal)&&+p.fixVal>0);if(!c)continue;const _=e.querySelector(`[data-opt="${i}"]`);_?(_.value=c.fixVal,this._dirtyOptKeys.add(i)):(this._preCascadeOpts==null&&(this._preCascadeOpts=JSON.parse(JSON.stringify(this._opts||{}))),this._opts={...this._opts,[i]:c.fixVal},(this._cascadePending??={})[i]=c.fixVal),r.add(i),o=!0;break}if(!o)break}if(this._liveValidateSettings(t),this._snapshotFormToPending(t),r.size>0){const n=r.size,a=n>1?"s":"";this._showToast(this._t("conflict.cascade_toast",{n},`Other settings adjusted for consistency: ${n}`),"success")}}_changedOptions(t){const e=this._preCascadeOpts||this._opts||{},s={};for(const[r,n]of Object.entries(t))JSON.stringify(n)!==JSON.stringify(e[r])&&(!(r in e)&&(n===""||n===null)||(s[r]=n));return s}async _saveSettings(){const t=this.shadowRoot,e=this._devices[this._selIdx];if(!e)return;const s=Object.assign({},this._pendingSettings,this._cascadePending);this._invalidJson=null,t.querySelectorAll("[data-opt]").forEach(i=>{const l=i.dataset.opt,c=Ct[l],_=c&&c.type||i.dataset.ftype||"text";if(i.type==="checkbox"){s[l]=i.checked;return}if(_==="checkboxlist"){s[l]=this._collectCheckboxlist(i,l);return}if(_==="entitylist"){s[l]=Array.from(i.querySelectorAll(".wd-pill")).map(f=>f.dataset.val).filter(Boolean);return}if(_==="timerlist"){s[l]=Array.from(i.querySelectorAll(".wd-timer-row")).map(f=>({offset_minutes:parseFloat(f.querySelector('[data-field="offset_minutes"]').value)||0,message:(f.querySelector('[data-field="message"]').value||"").trim(),auto_pause:f.querySelector('[data-field="auto_pause"]').checked})).filter(f=>f.offset_minutes>0);return}const p=i.value;if(_==="number"){const f=String(p).trim();if(f===""){c&&c.clearable?s[l]=null:delete s[l];return}const g=parseFloat(f);isNaN(g)||(s[l]=g);return}if(_==="list"){s[l]=String(p).split(",").map(f=>f.trim()).filter(Boolean);return}if(_==="intlist"){s[l]=le(p);return}if(_==="json"){const f=String(p).trim();if(!f){s[l]=[];return}try{s[l]=JSON.parse(f)}catch{this._invalidJson=l}return}if(_==="entity"||_==="device"){const f=String(p).trim();s[l]=f||null;return}s[l]=p});const r=this._optDefaults||{};for(const i of Object.keys(r))!(i in this._opts)&&i in s&&JSON.stringify(s[i])===JSON.stringify(r[i])&&delete s[i];if(this._invalidJson){this._showToast(this._t("toast.invalid_json",{key:this._invalidJson},`"${this._invalidJson}" is not valid JSON - fix it or clear the field before saving.`),"error");return}const n=this._liveValidateSettings(t),a=new Set(Object.keys(n));if(a.size>0){const i={},l={};for(const[_,p]of Object.entries(s))a.has(_)?l[_]=p:i[_]=p;const c=this._changedOptions(i);this._pendingSettings={...this._pendingSettings,...l},Object.keys(c).length?await this._busyRun("save-settings",async()=>{try{await this._ws({type:`${w}/set_options`,entry_id:e.entry_id,options:c}),this._opts={...this._opts,...c},this._showToast(this._t("toast.saved_except_conflicts",{},"Saved. Fix the highlighted conflicts to save the rest."),"info")}catch(_){this._showToast(this._t("msg.toast_save_failed",{error:_.message||_},"Save failed: "+(_.message||_)),"error")}}):this._showToast(this._t("toast.settings_conflicts",{},"Fix the highlighted setting conflicts before saving."),"error");return}const o=this._changedOptions(s);if(!Object.keys(o).length){this._pendingSettings={},this._dirtyOptKeys=new Set,this._showToast(this._t("toast.settings_no_changes",{},"No changes to save"),"info");return}await this._busyRun("save-settings",async()=>{try{const i=JSON.parse(JSON.stringify(this._preCascadeOpts||this._opts||{}));if(await this._ws({type:`${w}/set_options`,entry_id:e.entry_id,options:o}),this._opts={...this._opts,...o},this._prevOpts=i,this._cascadePending={},this._preCascadeOpts=null,this._pendingSettings={},this._dirtyOptKeys=new Set,this._stagedSuggestions){try{await this._ws({type:`${w}/clear_suggestions`,entry_id:e.entry_id})}catch{}this._stagedSuggestions=!1,this._suggestions=[]}this._showToast(this._t("toast.settings_saved",{},"Settings saved; integration reloading"))}catch(i){this._showToast(this._t("msg.toast_save_failed",{error:i.message||i},"Save failed: "+(i.message||i)),"error")}})}}customElements.get("ha-washdata-panel")||customElements.define("ha-washdata-panel",Ce);
