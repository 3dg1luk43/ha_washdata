...
`));v=`<div class="wd-sug"><span class="wd-sug-chip wd-sug-chip-obs">\u{1F4A1} ${d(u("suggestion.both_agree",{},"WashData recommends"))}</span><span class="wd-sug-val">${d(p)}${r}</span>${$(p)}${W}</div>`}else{const B=b?it(b):"",W=k?it(k):"",O=u("suggestion.observed_label",{},"Observed"),D=u("suggestion.calibrated_label",{},"Calibrated");let A="",C="";if(!isNaN(S)&&!isNaN(E)){const q=E>S;C=u(`suggestion.impact.${s}.${q?"higher":"lower"}`,{},""),A=u(`suggestion.impact.${s}.${q?"lower":"higher"}`,{},"")}const M=A?`<div class="wd-sug-impact-line">${d(A)}</div>`:"",H=C?`<div class="wd-sug-impact-line">${d(C)}</div>`:"";v=`<div class="wd-sug wd-sug-split"><div class="wd-sug-opt"><span class="wd-sug-chip wd-sug-chip-obs">\u{1F4A1} ${d(O)}</span><span class="wd-sug-val">${d(p)}${r}</span>${$(p)}${B}${M}</div><div class="wd-sug-opt"><span class="wd-sug-chip wd-sug-chip-cal">\u{1F916} ${d(D)}</span><span class="wd-sug-val">${d(g)}${r}</span>${$(g)}${W}${H}</div></div>`}}else if(p!=null){const S=b?it(b):"",E=t!=null&&t!==""?` <span style="opacity:.6;font-size:.9em">(now ${d(t)}${r})</span>`:"";v=`<div class="wd-sug"><span class="wd-sug-chip wd-sug-chip-obs">\u{1F4A1} ${d(u("suggestion.observed_label",{},"Observed"))}</span><span class="wd-sug-val">${d(p)}${r}</span>${E}${$(p)}${S}</div>`}else if(g!=null){const S=k?it(k):"";v=`<div class="wd-sug"><span class="wd-sug-chip wd-sug-chip-cal">\u{1F916} ${d(u("suggestion.calibrated_label",{},"Calibrated"))}</span><span class="wd-sug-val">${d(g)}${r}</span>${$(g)}${S}</div>`}if(v&&s){const S=d(u("btn.mute_suggestion",{},"Stop suggesting this setting")),E=`<button type="button" class="wd-sug-lock" data-suglock="${s}" title="${S}" aria-label="${S}">\u{1F515}</button>`;v=v.replace(/<\/div>\s*$/,E+"</div>")}return`<div class="wd-field" data-field="${s}"><div class="wd-label-row"><label style="margin:0">${d(n)}</label>${o}${a}</div>${i}${h.hint?`<div class="wd-field-hint">${d(h.hint)}</div>`:""}<div class="wd-conflict-err" data-cerr="${s}" hidden></div>${v}</div>`}function Ot(h,t){if(h==null||t==null)return!1;const e=parseFloat(h),s=parseFloat(t);return!isNaN(e)&&!isNaN(s)?Math.abs(e-s)<1e-6:String(h)===String(t)}const ge={min_power:"min_power",off_delay:"off_delay",smoothing_window:"smoothing",start_threshold_w:"hysteresis",stop_threshold_w:"hysteresis",start_energy_threshold:"start_energy",running_dead_zone:"dead_zone",profile_duration_tolerance:"duration_tolerance",profile_match_min_duration_ratio:"match_ratios",profile_match_max_duration_ratio:"match_ratios",progress_reset_delay:"progress_reset",completion_min_seconds:"min_duration",min_off_gap:"min_off_gap",start_duration_threshold:"start_duration",end_energy_threshold:"end_energy_thresh",end_repeat_count:"end_repeat",profile_match_threshold:"confidence",profile_unmatch_threshold:"confidence",auto_label_confidence:"confidence",learning_confidence:"confidence",no_update_active_timeout:"watchdog_timeout",anti_wrinkle_enabled:"anti_wrinkle",anti_wrinkle_max_power:"anti_wrinkle",anti_wrinkle_max_duration:"anti_wrinkle",anti_wrinkle_exit_power:"anti_wrinkle",anti_wrinkle_idle_timeout:"anti_wrinkle",sampling_interval:"sampling"};function ue(h){let t=0,e=window.innerWidth;for(let s=h.parentElement;s;s=s.parentElement){if(getComputedStyle(s).overflowX==="visible")continue;const n=s.getBoundingClientRect();n.left>t&&(t=n.left),n.right<e&&(e=n.right)}return{left:t,right:e}}function it(h,t){return`<span class="wd-tip">i<span class="wd-tip-pop">${t?fe(t):""}<span class="wd-tip-txt">${d(h)}</span></span></span>`}function Nt(h,t,e=""){return`<label class="wd-switch-lbl"><span class="wd-switch"><input type="checkbox" ${h}><span class="wd-switch-slider"></span></span><span class="wd-switch-text wd-switch-text--plain">${d(t)}</span></label>${e||""}`}function Et(h,t,e="",s=""){return`<div class="wd-field wd-field-switch"><div class="wd-switch-row">${Nt(h,t,e)}</div>${s?`<div class="wd-field-hint">${d(s)}</div>`:""}</div>`}function fe(h){const t=s=>`<svg class="wd-dg" viewBox="0 0 200 90" preserveAspectRatio="xMidYMid meet">${s}</svg>`,e='<line class="ax" x1="8" y1="78" x2="192" y2="78"/>';switch(h){case"smoothing":return t(`${e}
    ...
    `}_htmlDeviceBar(){const t=this._isAdmin()?`<button class="wd-devcard wd-devadd" data-action="add-device" title="${d(this._t("btn.add_device_tip",{},"Add another WashData device"))}">${this._t("btn.add_device",{},"+ Add device")}</button>`:"";return this._devices.length<=1?t?`<div class="wd-devbar">${t}</div>`:"":`<div class="wd-devbar">${this._devices.map((e,s)=>{const n=e.is_user_paused?"user_paused":e.detector_state||"unknown",r=["running","starting","paused","user_paused","ending","anti_wrinkle","rinse"].includes(n),a=!!e.recording,o=a?"var(--error-color, #f44336)":this._stateColor(n),i=a?this._t("status.recording",{},"Recording"):this._stateLabel(n),l=[],c=this._conflictCountForOpts(e.options||{},e.option_defaults||{});c&&l.push(`<span class="wd-dbadge conf">\u26A0 ${c}</span>`);const _=this._sugCountsForDevice(e).total;return _&&l.push(`<span class="wd-dbadge sug">\u{1F4A1} ${_}</span>`),e.feedback_count&&l.push(`<span class="wd-dbadge fb">\u{1F4AC} ${e.feedback_count}</span>`),`<button class="wd-devcard ${s===this._selIdx?"active":""}" data-idx="${s}">
        ...
        ${l.join("")}
      </button>`}).join("")}${t}</div>`}_htmlStatus(){const t=this._devices[this._selIdx];if(!t)return`<div class="wd-empty">${this._t("msg.no_device_selected",{},"No device selected.")}</div>`;const e=!!t.is_user_paused,s=e?"user_paused":t.detector_state||"unknown",n=!!t.recording,r=n?"var(--error-color, #f44336)":this._stateColor(s),a=n?this._t("status.recording",{},"Recording"):this._stateLabel(s),o=n||["running","starting","paused","user_paused","ending","anti_wrinkle","rinse"].includes(s),i=t.cycle_progress_pct,l=t.time_remaining_s,c=t.current_program,_=!!t.manual_program,p=c||"auto_detect",g=(this._profiles||[]).map(x=>x.name);c&&!g.includes(c)&&g.unshift(c);const u=g.map(x=>`<option value="${d(x)}" ${p===x?"selected":""}>${d(x)}</option>`).join(""),b=c?_?this._t("badge.manual",{},"(manually selected)"):this._t("badge.auto",{},"(auto-detected)"):"",k=b?`<span class="wd-prog-tag ${_?"manual":"auto"}">${b}</span>`:"",$=`<div class="wd-prog-ctl"><label>${this._t("lbl.program",{},"Program")}</label>${it(this._t("lbl.program_tip",{},"Override which profile is matched to the current cycle. Auto-detect lets the integration pick the best match automatically. Pin a specific program to force-match it when auto-detect is wrong or you know what is running."))}
          <select id="wd-status-prog">
            <option value="auto_detect" ${p==="auto_detect"?"selected":""}>${this._t("status.auto_detect",{},"Auto-detect")}</option>
            ${u}
          </select>${k}</div>`,v=[];t.recording&&this._canEdit()&&v.push(`<div class="wd-attn-card"><span class="wd-attn-icon">\u25CF</span><div class="wd-attn-body"><div class="wd-attn-title">${this._t("msg.recording_in_progress",{},"Recording in progress")}</div><div class="wd-attn-sub">${this._t("msg.see_recorder",{},"See recorder widget below")}</div></div></div>`),t.feedback_count&&this._canEdit()&&v.push(`<button class="wd-attn-card" type="button" data-action="goto-feedbacks"><span class="wd-attn-icon">\u{1F4AC}</span><div class="wd-attn-body"><div class="wd-attn-title">${this._t("msg.feedback_cycles_pending",{n:t.feedback_count},`To review: ${t.feedback_count}`)}</div><div class="wd-attn-sub">${this._t("msg.review_to_cycles",{},"Open the Cycles review queue")}</div></div></button>`);const S=this._conflictKeysFromOpts();if(S.size&&this._canEdit()){const x=S.size,L=x>1?"s":"";v.push(`<button class="wd-attn-card" type="button" style="border-color:var(--error-color,#b71c1c)" data-action="goto-conflicts"><span class="wd-attn-icon">\u26A0</span><div class="wd-attn-body"><div class="wd-attn-title" style="color:var(--error-color,#b71c1c)">${this._t("conflict.attn_title",{n:x},`Setting conflicts: ${x}`)}</div><div class="wd-attn-sub">${this._t("conflict.attn_sub",{},"Fix conflicts before saving")}</div></div></button>`)}const E=this._sugCountsForDevice(t);if(E.total&&this._canEdit()){const x=E.total,L=[];E.classic&&L.push(this._t("lbl.n_classic_suggestions",{n:E.classic},`${E.classic} classic`)),E.ml&&L.push(this._t("lbl.n_ml_suggestions",{n:E.ml},`${E.ml} ML`)),v.push(`<button class="wd-attn-card" type="button" data-action="goto-suggestions"><span class="wd-attn-icon">\u{1F4A1}</span><div class="wd-attn-body"><div class="wd-attn-title">${this._t("lbl.n_tuning_suggestions",{n:x},`${x} tuning suggestion${x>1?"s":""}`)}</div><div class="wd-attn-sub">${L.join(" \xB7 ")} \xB7 ${this._t("msg.review_in_settings",{},"Review in Settings")}</div></div></button>`)}const R=v.length?`<div class="wd-attn">${v.join("")}</div>`:"",B=o&&i!=null?`
    ...
    </div>`,G=v.length!==s.length?this._t("lbl.n_shown",{n:v.length},`, ${v.length} shown`):"",X=e.length?this._t("lbl.n_imported_note",{n:e.length},`, ${e.length} imported`):"",ct=this._t("lbl.cycles_title",{n:`${t.length}${X}${G}`},`Cycles (${t.length}${X}${G})`),U=n?`<div class="wd-card-actions" style="margin:0 0 4px;justify-content:flex-end">
    ...
    </label>`,k=n?`<p class="wd-info" style="margin:0 0 10px;font-size:.82em">${this._t("msg.settings_basic_note",{},"Showing essential settings. Switch to Advanced for the full list.")}</p>`:"",$=this._busy.has("save-settings"),v=l.size,S=v!==1?"s":"",E=v?`
      <div class="wd-sug-banner" style="background:rgba(183,28,28,.10);border-color:rgba(183,28,28,.4);color:var(--error-color,#b71c1c)">
        <span>\u26A0 ${this._t("conflict.settings_banner",{n:v},`Setting conflicts: ${v}. Check the highlighted sections and fix them before saving.`)}</span>
    ...
    `}_htmlSettingsHistory(){const t=this._settingsChangelog||[];if(!t.length)return"";const e=this._settingsHistoryOpen,s=this._canEdit(),n=t.slice(0,100).map(r=>{const a=s?`<button class="wd-btn wd-btn-secondary wd-btn-xs" data-action="settings-revert-key" data-key="${d(r.key)}" data-val="${d(JSON.stringify(r.old))}" title="${d(this._t("btn.revert_to_previous",{},"Restore this setting to its previous value"))}">${this._t("btn.revert",{},"Revert")}</button>`:"";return`<tr>
      ...
      <div class="wd-combo-row">
        ...
        <div class="wd-field-hint">${d(this._t("msg.pick_brand_first",{},"Pick an appliance brand first."))}</div></div>`;this._ensureCatalogEntry(),this._catalog.forBrand!==o&&(this._catalog.forBrand=o,this._catalog.devices=void 0);const i=Array.isArray(this._catalog.devices)?this._catalog.devices:[];this._entityListCache=this._entityListCache||{},this._entityListCache[t]=i.map(u=>u.model).filter(Boolean);const l=this._catalogEntryFor(s,"device"),c=this._statusTag(l),_=this._catalog.devices===null?` <span class="wd-info" style="font-size:.85em">${this._t("msg.loading",{},"Loading\u2026")}</span>`:"";let p="";if(l){const u=[],b=pe(l.manualUrl);b&&u.push(`<a href="${d(b)}" target="_blank" rel="noopener noreferrer nofollow">${this._t("link.manual",{},"Manual \u2197")}</a>`),(this._constants&&this._constants.storePrefs||{}).show_contributor!==!1&&u.push(this._t("store.contributed_by",{name:d(l.createdByName||this._t("lbl.anonymous",{},"Anonymous"))},`by ${d(l.createdByName||"Anonymous")}`));const k=!!(this._storeStatus&&this._storeStatus.connected);let $="";if(k){const v=[1,2,3,4,5].map(S=>`<button type="button" class="wd-star-btn" data-action="store-rate-device" data-device-id="${d(l.id)}" data-rating="${S}" aria-label="${this._t("lbl.rate_n",{n:S},`${S} stars`)}">\u2605</button>`).join("");$=`<button type="button" class="wd-btn wd-btn-secondary wd-btn-sm" data-action="store-confirm-device" data-device-id="${d(l.id)}" ${l.status!=="pending"?"disabled":""}>${l.status==="pending"?this._t("btn.confirm_appliance",{},"Confirm this appliance"):this._t("btn.confirmed",{},"Confirmed")}</button>
          <span class="wd-star-row" title="${d(this._t("lbl.rate_quality",{},"Rate quality"))}">${v}</span>`}else $=`<span class="wd-info" style="font-size:.85em">${this._t("msg.connect_to_confirm",{},"Connect in the settings gear to confirm or rate.")}</span>`;p=`<div class="wd-store-picker-detail">${u.join(" \xB7 ")}<div class="wd-store-picker-actions">${$}</div></div>`}const g=this._onlineEnabled()&&this._storeDeviceDeclared()?`<div class="wd-store-actions">
    ...
    `}_htmlMlStatusSection(t,e){const s=e&&this._busy.has("ml-train-now:"+e)||t&&t.running,n=this._canEdit()?`<button class="wd-btn wd-btn-primary wd-btn-sm" data-action="ml-train-now" ${s?"disabled":""}>${s?`<span class="wd-spin"></span> ${this._t("status.training",{},"Training\u2026")}`:this._t("btn.train_now",{},"Train now")}</button>`:"";if(!t)return`<div class="wd-card"><div class="wd-card-title" style="margin:0 0 4px">${this._t("hdr.status",{},"Status")}</div><p class="wd-info" style="margin:0">${this._t("msg.loading",{},"Loading\u2026")}</p></div>`;const r=Object.keys(t.on_device_models||{}).length,a=r?`<span style="color:var(--success-color,#4caf50);font-weight:600">${this._t("ml.personalized",{},"\u25CF Personalized to this machine")}</span> <span style="color:var(--secondary-text-color)">${this._t("lbl.models_fine_tuned",{count:r},"(fine-tuned models: "+r+")")}</span>`:`<span style="color:var(--secondary-text-color)">${this._t("ml.builtin_models",{},"\u25CF Using built-in models")}</span>`,o=t.cycle_count||0,i=t.min_cycles||0,l=o>=i,c=i>0?Math.min(100,Math.round(o/i*100)):100,_=l?"var(--success-color,#4caf50)":"var(--warning-color,#ff9800)",p=Math.max(0,i-o),g=l?this._t("msg.enough_data",{current:o,min:i},`Enough data to learn from (${o}/${i} cycles).`):this._t("msg.collecting_data",{need:p,current:o,min:i},`Collecting data. Cycles still needed before fine-tuning can start: ${p} (${o}/${i}).`),u=`<div style="height:8px;border-radius:6px;background:var(--secondary-background-color);overflow:hidden;margin:8px 0"><div style="width:${c}%;height:100%;background:${_}"></div></div>`,b=t.last_trained?st(t.last_trained):"never",k=s?`<span style="color:var(--info-color,#2196f3)"><span class="wd-spin"></span> ${this._t("status.fine_tuning",{},"fine-tuning now\u2026")}</span>`:t.enabled?this._t("lbl.auto_fine_tune_on",{hour:String(t.hour).padStart(2,"0")},`auto fine-tune on (around ${String(t.hour).padStart(2,"0")}:00)`):this._t("lbl.auto_fine_tune_off",{},"auto fine-tune off");return`<div class="wd-card">
      ...
      ${n}
        ...
        </div>`,r=k);const C=this._pgStagedVal(p),M=C!=null&&!(S!==""&&S!==null&&S!==void 0&&this._pgSameVal(C,S)),H=this._canEdit()&&M&&this._pgIsPublishable(p)?`<button class="wd-pg-pub" data-action="pg-publish-one" data-pgkey="${d(p)}" aria-label="${d(this._t("btn.pg_publish_one",{label:v},"Publish "+v+" to the integration"))}" title="${d(this._t("btn.pg_publish_one",{label:v},"Publish "+v+" to the integration"))}">\u2191</button>`:'<span class="wd-pg-pub-slot" aria-hidden="true"></span>';return`${A}<div style="display:flex;align-items:flex-start;gap:6px;margin:0 0 6px 11px">
        <div style="flex:1;min-width:0">
          <div style="font-size:.82em;font-weight:600;margin-bottom:1px">${d(v)}${B?` <span style="color:${O};font-size:.85em" title="${d(this._t("lbl.pg_drag_hint",{},"Drag line on graph"))}">\u2195</span>`:""}</div>
          ${b?`<div style="font-size:.72em;color:var(--secondary-text-color);line-height:1.3">${d(this._t("pg_desc."+p,{},b))}</div>`:""}
        </div>
        <div style="display:flex;align-items:center;gap:4px;flex-shrink:0">
          ${R?`<input class="wd-pg-param-chk" type="checkbox" data-pgkey="${d(p)}" data-pgtype="bool" aria-label="${d(v)}" ${E?"checked":""} style="width:18px;height:18px;margin:1px 27px 0 27px">`:`<input class="wd-pg-param-inp" type="text" inputmode="decimal" data-pgkey="${d(p)}" value="${E!==""?d(String(E)):""}" placeholder="${S!==""?d(String(S)):""}" aria-label="${d(v)}" style="width:72px">`}
    ...
    ${n||`<div class="wd-card"><p class="wd-info">${this._t("msg.no_other_users",{},"No other Home Assistant users found.")}</p></div>`}`}_htmlStore(){if(!this._canEdit())return`<div class="wd-empty">${this._t("msg.no_device_selected",{},"No device selected.")}</div>`;const t=this._storeStatus,e=`<a href="https://3dg1luk43.github.io/washdata-store" target="_blank" rel="noopener noreferrer" style="font-size:.8em;font-weight:400;color:var(--primary-color);text-decoration:none;white-space:nowrap" title="${d(this._t("store.website_tip",{},"Open the community store website"))}">${this._t("store.website",{},"Store website \u2197")}</a>`,s=`<div style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap"><div class="wd-card-title" style="margin:0">${this._t("hdr.community_store",{},"Community Store")}</div>${e}</div>`;if(!this._onlineEnabled()||t&&t.enabled===!1)return`<div class="wd-card">${s}
      ...
      ${Nt(`data-action="store-toggle-dl-settings" ${this._dlSettings?"checked":""} ${r?"disabled":""}`,this._t("lbl.adopt_settings",{},"Also adopt settings"),it(this._t("msg.adopt_settings_hint",{},"Overwrite this device's detection & matching thresholds with the shared ones. Your notifications, entities and energy price are never changed.")))}
      ...
      ${t?r:""}
    </div>`}_htmlStorePrefs(t){const e=this._constants&&this._constants.storePrefs||{},s=re.map(r=>{const a=e[r.key]!==!1;return Et(`data-action="store-toggle-pref" data-pref="${d(r.key)}" ${a?"checked":""} ${t?"disabled":""}`,this._t(r.labelKey,{},r.labelFb),it(this._t(r.docKey,{},r.docFb)))}).join(""),n=this._busy.has("store-refresh-catalog");return s+`
      ...
      <span class="wd-sd-prof-name"><strong>${this._t("btn.select_all",{},"Select all")}</strong></span>
      ...
      </label>`;let v="";if(!g&&_==="profiles")v=`<div class="wd-sd-cycles">${(p.items||[]).map(S=>`<label class="wd-sd-cyc">
        ...
        </label>`).join("")}</div>`;else if(!g&&(_==="real_cycles"||_==="reference_cycles")){const S=_==="real_cycles"?t.sel.realIds:t.sel.refIds;v=`<div class="wd-sd-cycles">${(p.groups||[]).map(E=>{const R=E.cycles.map(M=>String(M.id)),B=R.filter(M=>S.has(M)).length,W=B===R.length&&R.length>0,O=B>0&&!W,D=`${_}:${E.profile}`,A=t.expanded&&t.expanded.has(D),C=A?E.cycles.map(M=>`<label class="wd-sd-cyc" style="margin-left:18px">
      ...
      <div class="wd-modal-actions">
        ...
        <div class="wd-field"><label>${this._t("lbl.load_from_file",{},"Load from file")}</label><input type="file" id="wd-hist-file" accept=".csv,text/csv,text/plain" ${v}></div>
        <div class="wd-field"><label>${this._t("lbl.hist_csv_data",{},"CSV data")}</label><textarea id="wd-hist-csv" style="min-height:96px;font-family:monospace;font-size:.78em" placeholder="entity_id,state,last_changed" ${v}>${d(t.csvText||"")}</textarea></div>
        <div class="wd-field">
          ...
          <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
            ...
            <input type="date" id="wd-hist-since" value="${d(t.since||zt())}" min="${d(ce())}" max="${d(de())}" ${v}>
            <button class="wd-btn wd-btn-secondary" data-maction="hist-recorder" ${v}>${this._t("btn.hist_read_recorder",{},"Read from Home Assistant")}</button>
          </div>
        ...
        ${s}
        <div class="wd-modal-actions">
          <button class="wd-btn wd-btn-secondary" data-maction="cancel" ${v}>${this._t("btn.cancel",{},"Cancel")}</button>
          <button class="wd-btn wd-btn-primary" data-maction="hist-scan" ${v}>${$?'<span class="wd-spin"></span> ':""}${this._t("btn.hist_scan",{},"Scan for cycles")}</button>
        </div>`}if(t.step==="scan"){const $=t.scanTaskId?(this._tasks||{})[t.scanTaskId]:null,v=$&&$.total>0?Math.round($.done/$.total*100):null;return`${e}
        ...
        <div class="wd-prog-bg" style="margin-bottom:8px"><div class="wd-prog-fill" style="width:${v??0}%"></div></div>
        <p class="wd-info">${v==null?this._t("status.preparing",{},"Preparing\u2026"):`${v}%`}</p>
        ${s}
        <div class="wd-modal-actions">
          <button class="wd-btn wd-btn-secondary" data-maction="hist-cancel-scan">${this._t("btn.cancel",{},"Cancel")}</button>
        </div>`}if(t.step==="done"){const $=t.done||{},v=[this._t("msg.hist_imported_count",{n:$.imported||0},`${$.imported||0} cycles imported.`),$.duplicates?this._t("msg.hist_duplicates",{n:$.duplicates},`${$.duplicates} were already imported and were skipped.`):"",$.capped?this._t("msg.hist_capped",{},"The per-device limit for imported cycles was reached; the rest were not stored."):""].filter(Boolean);return`${e}
        ${v.map(S=>`<p class="wd-info">${d(S)}</p>`).join("")}
        ...
        </div>`;const b=r.map($=>{const v=i.has($.index),S=this._histSegReason($.reason);return`<tr data-hist-row="${$.index}" style="${v?"":"opacity:.55"}">
        <td><input type="checkbox" data-hist-pick="${$.index}" ${v?"checked":""} aria-label="${d(this._t("lbl.hist_keep",{},"Keep this cycle"))}"></td>
      ...
      <button class="wd-btn wd-btn-sm ${t.mode==="review"?"wd-btn-primary":"wd-btn-secondary"}" data-maction="cyc-review" title="${b?this._t("hdr.automation_needs_review",{},"This cycle needs review"):this._t("hdr.automation_review_this_cycle",{},"Review this cycle")}">${this._t("btn.review",{},"Review")}${k}</button>
    </div>`:s?`<div class="wd-info" style="margin:0 0 8px"><span style="color:var(--info-color,#2196f3)">\u{1F4E5}</span> ${a?this._t("msg.imported_history_readonly",{},"Detected in imported power history. It shapes program matching but is not counted in your statistics, and cannot be trimmed or split. Label it to name the program."):this._t("msg.imported_readonly",{},"Imported from the community store. Shown for reference and matching. It is not counted in your stats and cannot be edited.")}</div>`:"",v=this._canEdit()?(this._feedbacks||[]).find(C=>C.cycle_id===t.cycleId):null,S=v?v.detected_profile||v.profile_name||this._t("lbl.unknown",{},"Unknown"):"",E=v?`
        ...
        </div>`}let B="";const W=t.mode==="view"||t.mode==="review"?e.artifacts||[]:[];if(W.length){const C=W.map(M=>{const H=M.detail_key?this._t(M.detail_key,M.detail_params||{},M.detail||""):M.detail||"";return`<li><b>${d(Zt(M.type,(q,Q,J)=>this._t(q,Q,J)))}</b> ${this._t("lbl.at",{},"at")} ${Mt(M.start_s)}\u2013${Mt(M.end_s)} \u2014 ${d(H)}</li>`}).join("");B=`<div class="wd-card" style="margin:10px 0 0;padding:10px 12px;border-left:3px solid var(--warning-color,#ff9800)">
      ...
      ${R}`}_htmlProfilePanel(t){const e=this._canEdit();t.tab==="danger"&&!e&&(t.tab="stats");const s=[["stats",this._t("tab.pp_overview",{},"Overview")],["phases",this._t("tab.pp_phases",{},"Phases")],["cleanup",this._t("tab.pp_cleanup",{},"Cleanup")]];e&&s.push(["danger",this._t("tab.pp_manage",{},"Manage")]);const n=s.map(([o,i])=>`<button class="wd-mini-tab ${t.tab===o?"active":""}" data-maction="pp-tab-${o}">${i}</button>`).join("");let r="";if(!t.loaded)r=`<div class="wd-empty" style="padding:32px"><div class="wd-icon">\u23F3</div>${this._t("msg.loading",{},"Loading\u2026")}</div>`;else if(t.tab==="stats"){const o=t.stats||{},i=t.env||{},l=this._hass&&this._hass.config&&this._hass.config.currency||"",c=o.avg_energy!=null&&o.cycle_count?o.avg_energy*o.cycle_count:null,_=k=>k?Math.round(k/60)+"m":"-",p=(this._profileHealth||{})[t.name],g=(this._profileTrends||{})[t.name],u=p&&p.health_status!=="unknown"?(()=>{const k={healthy:["var(--success-color,#4caf50)","rgba(76,175,80,.12)"],fair:["var(--warning-color,#ff9800)","rgba(255,152,0,.12)"],poor:["var(--error-color,#f44336)","rgba(244,67,54,.12)"]},[$,v]=k[p.health_status]||k.fair,S=Math.round((p.health_score||0)*100),E=p.duration_cv!=null?` \xB7 ${this._t("stat.duration_cv",{pct:Math.round(p.duration_cv*100)},`duration CV ${Math.round(p.duration_cv*100)}%`)}`:"",R=p.confidence_mean!=null?` \xB7 ${this._t("stat.avg_confidence",{pct:Math.round(p.confidence_mean*100)},`avg confidence ${Math.round(p.confidence_mean*100)}%`)}`:"";return`<div style="margin:8px 0 4px;padding:8px 12px;border-radius:6px;background:${v};border:1px solid color-mix(in srgb, ${$} 13%, transparent);display:flex;align-items:center;gap:8px;flex-wrap:wrap">
        ...
        ${e?`<div class="wd-mode-bar">
        ...
        </div>`:""}`}else if(t.tab==="cleanup"){const o=t.cleanup&&t.cleanup.cycles||[],i=t.cleanup&&t.cleanup.selected||new Set,{col:l,dir:c}=this._cleanupSort,_={date:v=>v.start_time?new Date(v.start_time).getTime():0,duration:v=>v.duration,energy:v=>v.energy_kwh,status:v=>v.status||""},g=Qt(o,_[l]||_.date,c).map((v,S)=>{const E=o.indexOf(v),R=e?`<td style="padding:4px 6px 4px 2px;white-space:nowrap">
          ...
          <td style="width:26px;padding:6px 4px"><input type="checkbox" data-cleanidx="${E}" ${i.has(v.cycle_id)?"checked":""}></td>
          <td style="width:10px;padding:6px 2px"><span class="wd-swatch" style="background:${et[E%et.length]}"></span></td>
          <td class="wd-tc-date">${st(v.start_time)}</td>
          <td class="wd-tc-num">${ot(v.duration)}</td>
          <td class="wd-tc-num">${v.energy_kwh!=null?gt(v.energy_kwh):"-"}</td>
