...
`;function dt(h){if(h==null||h<0)return"-";const t=Math.floor(h/3600),e=Math.floor(h%3600/60),s=Math.floor(h%60);return t>0?`${t}h ${e}m`:e>0?`${e}m ${s}s`:`${s}s`}function Vt(h){return h==null?"-":h>=100?`${Math.round(h)} W`:`${h.toFixed(1)} W`}function _t(h){return h==null?"-":`${h.toFixed(2)} kWh`}let Ut="relative";function ne(h){const t=Math.round((h-Date.now())/1e3);let e;try{e=new Intl.RelativeTimeFormat(void 0,{numeric:"auto"})}catch{return Gt(h)}const s=Math.abs(t),r=[["year",31536e3],["month",2592e3],["week",604800],["day",86400],["hour",3600],["minute",60]];for(const[n,a]of r)if(s>=a)return e.format(Math.round(t/a),n);return e.format(t,"second")}function Gt(h){return new Date(h).toLocaleString(void 0,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}function et(h,t){if(!h)return"-";let e;if(typeof h=="number")e=h>=1e12?h:h*1e3;else{const s=String(h),r=/^(\d{4})-(\d{2})-(\d{2})$/.exec(s);e=r?new Date(+r[1],+r[2]-1,+r[3]).getTime():new Date(s).getTime()}return isNaN(e)?"-":(t||Ut)==="relative"?ne(e):Gt(e)}function Pt(h){if(h==null)return"unknown error";if(h instanceof Error)return`${h.name}: ${h.message}`;const t=h.code!=null?String(h.code):"",e=h.message!=null?String(h.message):"";if(!t&&!e)try{return JSON.stringify(h)}catch{return String(h)}const s=t==="unknown_command"?" (the integration is probably still starting up; this should stop on its own)":"";return`${t||"error"}${e?": "+e:""}${s}`}function d(h){return String(h??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function re(h){const t=String(h??"").trim();return/^https?:\/\//i.test(t)?t:""}function kt(h,t){const e=String(h??"").trim();let s=e.match(/^#([0-9a-fA-F]{3,8})$/);if(s){let r=s[1];if((r.length===3||r.length===4)&&(r=r.split("").map(n=>n+n).join("")),r.length===6||r.length===8){const n=parseInt(r.slice(0,2),16),a=parseInt(r.slice(2,4),16),o=parseInt(r.slice(4,6),16);return`rgba(${n}, ${a}, ${o}, ${t})`}}if(s=e.match(/^rgba?\(([^)]+)\)$/i),s){const r=s[1].split(",").map(n=>n.trim());if(r.length>=3)return`rgba(${r[0]}, ${r[1]}, ${r[2]}, ${t})`}return e}function Kt(h,t){const e=parseFloat(h);return isNaN(e)?t:e}function zt(h){return h?Array.from(h.querySelectorAll('a[href],button:not([disabled]),input:not([disabled]),select:not([disabled]),textarea:not([disabled]),[tabindex]:not([tabindex="-1"])')).filter(e=>e.getClientRects().length>0):[]}function St(h){return h==null||h===""?"(none)":h===!0?"on":h===!1?"off":Array.isArray(h)?h.join(", ")||"(none)":String(h)}function Xt(h,t,e){return h.slice().sort((s,r)=>{const n=t(s),a=t(r);return n==null&&a==null?0:n==null?1:a==null?-1:(n<a?-1:n>a?1:0)*e})}function ut(h,t,e,s,r,n,a){const o=e?s===1?" \u25B2":" \u25BC":' <span style="opacity:.35">\u2195</span>',i=n==="right"?"text-align:right;":"",l=a?` title="${d(a)}"`:"";return`<th class="wd-th-sort" data-sortcol="${t}" data-sortact="${r}" style="cursor:pointer;user-select:none;${i}"${l}>${h}${o}</th>`}function Mt(h){h=Math.max(0,Math.round(h));const t=Math.floor(h/3600),e=Math.floor(h%3600/60),s=h%60;return t>0?`${t}:${String(e).padStart(2,"0")}:${String(s).padStart(2,"0")}`:`${e}:${String(s).padStart(2,"0")}`}function Yt(h,t){const e={pause:["lbl.artifact_interruption","Interruption"],dip:["lbl.artifact_low_power","Low power"],spike:["lbl.artifact_high_power","High power"]},[s,r]=e[h]||["lbl.artifact_anomaly","Anomaly"];return t?t(s,{},r):r}function Jt(h){return h.toLowerCase().replace(/[\s&/\-]+/g,"_").replace(/^_+|_+$/g,"").replace(/_+/g,"_")}function Qt(h){const t=new Set;return String(h??"").split(",").forEach(e=>{const s=parseInt(e.trim(),10);Number.isFinite(s)&&s>0&&t.add(s)}),Array.from(t).sort((e,s)=>e-s)}function Dt(h,t){if(!h||!h.length)return null;if(t<=h[0][0])return h[0][1];if(t>=h[h.length-1][0])return h[h.length-1][1];for(let e=1;e<h.length;e++)if(h[e][0]>=t){const s=h[e-1],r=h[e],n=r[0]-s[0]||1;return s[1]+(r[1]-s[1])*((t-s[0])/n)}return h[h.length-1][1]}function oe(h,t,e){e=e||{};const s=h.key,r=h.unit?`${h.label} (${h.unit})`:h.label,n=h.unit?` ${h.unit}`:"",a=h.doc?st(h.doc,h.diagram||le[s]):"",o=e.changed?`<span class="wd-chg-dot" title="${d(e.changed)}" aria-label="${d(e.changed)}"></span>`:"";if(h.type==="checkbox")return`<div class="wd-field wd-field-switch"><div class="wd-switch-row"><label class="wd-switch-lbl"><span class="wd-switch"><input type="checkbox" data-opt="${s}" ${t?"checked":""}><span class="wd-switch-slider"></span></span><span class="wd-switch-text">${d(h.label)}</span></label>${o}${a}</div>${h.hint?`<div class="wd-field-hint">${d(h.hint)}</div>`:""}</div>`;let i="";const l=t??"";if(h.type==="select"||h.type==="devicetype"||h.type==="device"){const C=(e.opts||[]).map(([j,z])=>`<option value="${d(j)}" ${String(l)===String(j)?"selected":""}>${d(z)}</option>`).join("");i=`<select data-opt="${s}" data-ftype="${h.type}">${C}</select>`}else if(h.type==="textarea")i=`<textarea data-opt="${s}" data-ftype="textarea">${d(l)}</textarea>`;else if(h.type==="json"){const $=l===""||l==null?"":typeof l=="string"?l:JSON.stringify(l,null,2);i=`<textarea data-opt="${s}" data-ftype="json" rows="3" placeholder='${d(e.t("placeholder.json_buttons",{},'[{"action":"ID","title":"Label"}]'))}'>${d($)}</textarea>`}else if(h.type==="entitylist"){const C=(Array.isArray(t)?t:t?[t]:[]).map(j=>`<span class="wd-pill" data-val="${d(j)}">${d(j)}<button type="button" class="wd-pill-x" aria-label="${d(e.t?e.t("btn.remove",{},"Remove"):"Remove")}">\xD7</button></span>`).join("");i=`<div class="wd-pillbox" data-opt="${s}" data-ftype="entitylist">${C}<div class="wd-combo wd-combo-pill"><input type="text" class="wd-pill-add" autocomplete="off" spellcheck="false" placeholder="${d(e.t("placeholder."+(h.domain||"add"),{},h.placeholder||"add\u2026"))}"><div class="wd-combo-drop" hidden></div></div></div>`}else if(h.type==="timerlist"){const $=Array.isArray(l)?l:[],C=e.t?e.t("lbl.timer_min",{},"min"):"min",j=e.t?e.t("lbl.timer_msg_placeholder",{},"Message (optional, {device}/{program}/{minutes})"):"Message (optional, {device}/{program}/{minutes})",z=e.t?e.t("lbl.timer_auto_pause",{},"Auto-pause"):"Auto-pause",I=e.t?e.t("btn.remove_timer",{},"Delete"):"Delete",A=e.t?e.t("btn.add_timer",{},"+ Add timer"):"+ Add timer",M=(L,F)=>{const V=L&&L.offset_minutes?String(L.offset_minutes):"",Y=L&&L.message?d(L.message):"",it=L&&L.auto_pause?" checked":"";return`<div class="wd-timer-row" data-tidx="${F}"><div class="wd-timer-top"><input type="number" min="1" placeholder="${d(C)}" data-field="offset_minutes" value="${d(V)}"><textarea placeholder="${d(j)}" data-field="message">${Y}</textarea></div><div class="wd-timer-footer"><label class="wd-switch-lbl"><span class="wd-switch"><input type="checkbox" data-field="auto_pause"${it}><span class="wd-switch-slider"></span></span><span class="wd-switch-text">${d(z)}</span></label><button type="button" class="wd-btn wd-btn-sm wd-btn-danger wd-timer-remove">${d(I)}</button></div></div>`},E=$.map((L,F)=>M(L,F)).join("");i=`<div class="wd-timerlist" data-opt="${s}" data-ftype="timerlist">${E}<button type="button" class="wd-btn wd-btn-sm wd-btn-secondary wd-timer-add">${d(A)}</button></div>`}else if(h.type==="entity"){const $=h.placeholder?` placeholder="${d(h.placeholder)}"`:"";i=`<div class="wd-combo"><input type="text" class="wd-combo-inp" data-opt="${s}" data-ftype="entity" value="${d(l)}" autocomplete="off" spellcheck="false"${$}><div class="wd-combo-drop" hidden></div></div>`}else if(h.type==="list"){const $=Array.isArray(l)?l.join(", "):d(l);i=`<input type="text" data-opt="${s}" data-ftype="list" value="${d($)}" placeholder="${d(e.t("placeholder.notify_services_list",{},"notify.mobile_app_phone, ..."))}">`}else if(h.type==="intlist"){const $=Array.isArray(l)?l.join(", "):String(l??""),C=h.placeholder?` placeholder="${d(h.placeholder)}"`:"";i=`<input type="text" data-opt="${s}" data-ftype="intlist" value="${d($)}"${C}>`}else{const $=h.type==="number"?"number":"text",C=e.datalistId?` list="${e.datalistId}"`:"",j=h.step!=null?` step="${h.step}"`:"",z=h.min!=null?` min="${h.min}"`:"",I=h.max!=null?` max="${h.max}"`:"",A=h.placeholder?` placeholder="${d(h.placeholder)}"`:"";i=`<input type="${$}" data-opt="${s}" data-ftype="${h.type}" value="${d(l)}"${j}${z}${I}${C}${A}>${e.datalist||""}`}const c=e.suggestion,_=e.mlSuggestion,p=c&&c.suggested!=null&&!Ft(c.suggested,t)?c.suggested:null,u=_&&_.value!=null&&!Ft(_.value,t)?_.value:null,g=e.t,b=c?c.reason_key?g(c.reason_key,c.reason_params||{},c.reason||""):c.reason||"":"",v=_?_.reason_key?g(_.reason_key,_.reason_params||{},_.reason||""):_.reason||"":"",S=$=>`<button type="button" class="wd-sug-use" data-sugkey="${s}" data-sugval="${d($)}">${e.useBtnLabel||"Use"}</button>`;let k="";if(p!=null&&u!=null){const $=parseFloat(p),C=parseFloat(u);if((!isNaN($)&&!isNaN(C)?Math.abs($-C)/Math.max(Math.abs($),Math.abs(C),1e-9):1)<.05){const z=g("suggestion.calibrated_label",{},"Calibrated"),I=st([b,v?`${z}: ${v}`:""].filter(Boolean).join(`

`));k=`<div class="wd-sug"><span class="wd-sug-chip wd-sug-chip-obs">\u{1F4A1} ${d(g("suggestion.both_agree",{},"WashData recommends"))}</span><span class="wd-sug-val">${d(p)}${n}</span>${S(p)}${I}</div>`}else{const z=b?st(b):"",I=v?st(v):"",A=g("suggestion.observed_label",{},"Observed"),M=g("suggestion.calibrated_label",{},"Calibrated");let E="",L="";if(!isNaN($)&&!isNaN(C)){const Y=C>$;L=g(`suggestion.impact.${s}.${Y?"higher":"lower"}`,{},""),E=g(`suggestion.impact.${s}.${Y?"lower":"higher"}`,{},"")}const F=E?`<div class="wd-sug-impact-line">${d(E)}</div>`:"",V=L?`<div class="wd-sug-impact-line">${d(L)}</div>`:"";k=`<div class="wd-sug wd-sug-split"><div class="wd-sug-opt"><span class="wd-sug-chip wd-sug-chip-obs">\u{1F4A1} ${d(A)}</span><span class="wd-sug-val">${d(p)}${n}</span>${S(p)}${z}${F}</div><div class="wd-sug-opt"><span class="wd-sug-chip wd-sug-chip-cal">\u{1F916} ${d(M)}</span><span class="wd-sug-val">${d(u)}${n}</span>${S(u)}${I}${V}</div></div>`}}else if(p!=null){const $=b?st(b):"",C=t!=null&&t!==""?` <span style="opacity:.6;font-size:.9em">(now ${d(t)}${n})</span>`:"";k=`<div class="wd-sug"><span class="wd-sug-chip wd-sug-chip-obs">\u{1F4A1} ${d(g("suggestion.observed_label",{},"Observed"))}</span><span class="wd-sug-val">${d(p)}${n}</span>${C}${S(p)}${$}</div>`}else if(u!=null){const $=v?st(v):"";k=`<div class="wd-sug"><span class="wd-sug-chip wd-sug-chip-cal">\u{1F916} ${d(g("suggestion.calibrated_label",{},"Calibrated"))}</span><span class="wd-sug-val">${d(u)}${n}</span>${S(u)}${$}</div>`}if(k&&s){const $=d(g("btn.mute_suggestion",{},"Stop suggesting this setting")),C=`<button type="button" class="wd-sug-lock" data-suglock="${s}" title="${$}" aria-label="${$}">\u{1F515}</button>`;k=k.replace(/<\/div>\s*$/,C+"</div>")}return`<div class="wd-field" data-field="${s}"><div class="wd-label-row"><label style="margin:0">${d(r)}</label>${o}${a}</div>${i}${h.hint?`<div class="wd-field-hint">${d(h.hint)}</div>`:""}<div class="wd-conflict-err" data-cerr="${s}" hidden></div>${k}</div>`}function Ft(h,t){if(h==null||t==null)return!1;const e=parseFloat(h),s=parseFloat(t);return!isNaN(e)&&!isNaN(s)?Math.abs(e-s)<1e-6:String(h)===String(t)}const le={min_power:"min_power",off_delay:"off_delay",smoothing_window:"smoothing",start_threshold_w:"hysteresis",stop_threshold_w:"hysteresis",start_energy_threshold:"start_energy",running_dead_zone:"dead_zone",profile_duration_tolerance:"duration_tolerance",profile_match_min_duration_ratio:"match_ratios",profile_match_max_duration_ratio:"match_ratios",progress_reset_delay:"progress_reset",completion_min_seconds:"min_duration",min_off_gap:"min_off_gap",start_duration_threshold:"start_duration",end_energy_threshold:"end_energy_thresh",end_repeat_count:"end_repeat",profile_match_threshold:"confidence",profile_unmatch_threshold:"confidence",auto_label_confidence:"confidence",learning_confidence:"confidence",no_update_active_timeout:"watchdog_timeout",anti_wrinkle_enabled:"anti_wrinkle",anti_wrinkle_max_power:"anti_wrinkle",anti_wrinkle_max_duration:"anti_wrinkle",anti_wrinkle_exit_power:"anti_wrinkle",anti_wrinkle_idle_timeout:"anti_wrinkle",sampling_interval:"sampling"};function de(h){let t=0,e=window.innerWidth;for(let s=h.parentElement;s;s=s.parentElement){if(getComputedStyle(s).overflowX==="visible")continue;const r=s.getBoundingClientRect();r.left>t&&(t=r.left),r.right<e&&(e=r.right)}return{left:t,right:e}}function st(h,t){return`<span class="wd-tip">i<span class="wd-tip-pop">${t?ce(t):""}<span class="wd-tip-txt">${d(h)}</span></span></span>`}function Ot(h,t,e=""){return`<label class="wd-switch-lbl"><span class="wd-switch"><input type="checkbox" ${h}><span class="wd-switch-slider"></span></span><span class="wd-switch-text wd-switch-text--plain">${d(t)}</span></label>${e||""}`}function Tt(h,t,e="",s=""){return`<div class="wd-field wd-field-switch"><div class="wd-switch-row">${Ot(h,t,e)}</div>${s?`<div class="wd-field-hint">${d(s)}</div>`:""}</div>`}function ce(h){const t=s=>`<svg class="wd-dg" viewBox="0 0 200 90" preserveAspectRatio="xMidYMid meet">${s}</svg>`,e='<line class="ax" x1="8" y1="78" x2="192" y2="78"/>';switch(h){case"smoothing":return t(`${e}
    ...
    `}_htmlSetupCard(t,e=!1){if(!t)return"";const{phase:s,message_key:r,message_params:n,cta_label_key:a,cta_action:o,secondary_label_key:i,secondary_action:l,skippable:c,dismissible:_,step_key:p}=t;if(s==="phase4")return`
        ...
        </div>`;const u=this._t(r,n||{},""),g=this._t(a,{},"Continue"),b=i?this._t(i,{},""):null,v=c?`
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;flex-wrap:wrap">
    ...
    </div>`}_htmlHistory(){const t=this._cycles||[],e=this._refCycles||[],s=e.concat(t),r=this._canEdit(),n=this._selectMode&&r,a=this._cycleSel,{col:o,dir:i}=this._cycleSort,{text:l,status:c}=this._cycleFilter,_=this._mlById||{},p=new Set((this._feedbacks||[]).map(w=>w.cycle_id)),u=w=>_[w.id],g=w=>{const P=u(w);return!!(P&&P.ml_review&&P.ml_review.reviewed_at)},b=w=>{const P=u(w);return!!(P&&P.ml_review&&P.ml_review.golden)},v=w=>{if(p.has(w.id))return!0;if(g(w))return!1;const P=u(w),O=P&&P.ml_quality_label;return["uncertain","review"].includes(O)||["force_stopped","interrupted"].includes(w.status)},S=s.filter(v).length;let k=s;if(l){const w=l.toLowerCase();k=k.filter(P=>(P.profile_name||P.matched_profile||"").toLowerCase().includes(w))}c==="unlabelled"?k=k.filter(w=>!w.profile_name&&!w.matched_profile):c==="needs_review"?k=k.filter(v):c==="imported"?k=k.filter(w=>w.is_reference):c&&(k=k.filter(w=>(w.status||"completed")===c));const $={date:w=>w.start_time?new Date(w.start_time).getTime():0,confidence:w=>w.match_confidence,duration:w=>w.duration,energy:w=>w.energy_kwh!=null?w.energy_kwh:w.energy_wh!=null?w.energy_wh/1e3:null,cost:w=>w.cost!=null?w.cost:-1,status:w=>w.status||"completed",profile:w=>(w.profile_name||w.matched_profile||"\uFFFF").toLowerCase()};k=Xt(k,$[o]||$.date,i);const C=w=>w==="completed"?"var(--success-color, #4caf50)":w==="interrupted"?"var(--error-color, #f44336)":w==="force_stopped"?"var(--warning-color, #ff9800)":"var(--secondary-text-color)",j=w=>w.is_reference?` <span title="${d(this._t("badge.imported_tip",{},"Imported from the community store. Used for matching only, not counted in stats."))}" style="color:var(--info-color,#2196f3)">\u{1F4E5}</span>`:"",z=w=>b(w)?' <span title="'+d(this._t("badge.golden_cycle",{},"Recorded reference cycle"))+'" style="color:var(--warning-color,#ff9800)">\u2B50</span>':p.has(w.id)?' <span title="'+d(this._t("badge.feedback_requested",{},"Feedback requested"))+'" style="color:var(--info-color,#2196f3)">\u{1F4AC}</span>':g(w)?' <span title="'+d(this._t("badge.reviewed",{},"Reviewed"))+'" style="color:var(--success-color,#4caf50)">\u2713</span>':v(w)?' <span title="'+d(this._t("badge.needs_review",{},"Needs review"))+'" style="color:var(--error-color,#f44336)">\u25CF</span>':"",I=w=>{if(w.anomaly!=="overrun")return"";const P=w.overrun_ratio?" "+this._t("badge.overrun_ratio",{x:Number(w.overrun_ratio).toFixed(1)},`(${Number(w.overrun_ratio).toFixed(1)}x expected)`):"";return` <span title="${d(this._t("badge.overrun",{},"Ran longer than usual"))}${d(P)}" style="color:var(--warning-color,#ff9800)">\u23F1</span>`},A=w=>{if(w.anomaly!=="underrun")return"";const P=w.underrun_ratio?" "+this._t("badge.underrun_ratio",{pct:Math.round(w.underrun_ratio*100)},`(${Math.round(w.underrun_ratio*100)}% of expected)`):"";return` <span title="${d(this._t("badge.underrun",{},"Finished faster than usual"))}${d(P)}" style="color:var(--info-color,#2196f3)">\u26A1</span>`},M=w=>{if(!w.energy_anomaly||w.energy_anomaly==="none")return"";const P=w.energy_anomaly==="energy_spike",O=w.energy_z_score!=null?` (${w.energy_z_score>0?"+":""}${Number(w.energy_z_score).toFixed(1)}\u03C3)`:"",G=P?"badge.energy_spike":"badge.energy_low",Z=P?"Higher energy than usual":"Lower energy than usual",at=P?"\u{1F53A}":"\u{1F53B}",rt=P?"var(--error-color,#f44336)":"var(--info-color,#2196f3)";return` <span title="${d(this._t(G,{},Z))}${d(O)}" style="color:${rt}">${at}</span>`},E=w=>{const P=Array.isArray(w.artifacts)?w.artifacts.length:0;return P?` <span title="${d(this._t("badge.artifact_tip",{n:P},`${P} anomal${P>1?"ies":"y"} detected (e.g. door opened mid-cycle) \u2014 open to see them on the graph`))}" style="color:var(--warning-color,#ff9800)">\u26A0</span>`:""},L=w=>{const P=Array.isArray(w.restart_gaps)?w.restart_gaps.length:0;return P?` <span title="${d(this._t("badge.restart_gap_tip",{n:P},`${P} HA restart gap${P>1?"s":""} during this cycle \u2014 power trace has a hole`))}" style="color:var(--info-color,#2196f3)">\u21BB</span>`:""},F=this._hass&&this._hass.config&&this._hass.config.currency||"",V=w=>w.cost!=null?`${w.cost.toFixed(2)}${F?" "+F:""}`:"-",Y=k.map(w=>{const P=w.profile_name||w.matched_profile,O=w.match_confidence!=null?w.match_confidence*100:null,G=w.status||"completed",Z=w.energy_kwh!=null?w.energy_kwh:w.energy_wh!=null?w.energy_wh/1e3:null,at=n,rt=at?`<input type="checkbox" class="wd-csel" ${a.has(w.id)?"checked":""} style="width:auto;margin:0">`:`<span class="wd-devdot" style="background:${C(G)}" title="${d(G)}"></span>`,ct={completed:this._t("status.completed",{},"Completed"),interrupted:this._t("status.interrupted",{},"Interrupted"),force_stopped:this._t("status.force_stopped",{},"Force stopped"),active:this._t("status.active",{},"Active")}[G]||G,ft=`${j(w)}${z(w)}${I(w)}${A(w)}${M(w)}${E(w)}${L(w)}`.trim();return`<tr data-cid="${d(w.id)}" data-selmode="${at?1:0}" style="cursor:pointer">
        ...
        <td class="wd-tc-flags">${ft}</td>
        <td><span style="color:${C(G)};font-size:.9em">${d(ct)}</span></td>
        <td class="wd-tc-date">${et(w.start_time)}</td>
        <td class="wd-tc-num">${dt(w.duration)}</td>
        <td class="wd-tc-num">${Z!=null?_t(Z):"-"}</td>
        <td class="wd-tc-num">${V(w)}</td>
        <td class="wd-tc-num">${O!=null?O.toFixed(0)+"%":"-"}</td>
      </tr>`}).join(""),it=`<thead><tr>
      <th style="width:26px;padding:6px 4px 6px 8px"></th>
      ${ut(this._t("lbl.profile",{},"Profile"),"profile",o==="profile",i,"cycsort","",this._t("col.profile_tip",{},"Matched program name. Unlabelled means no profile matched at end of cycle."))}
      ...
      ${ut(this._t("lbl.status",{},"Status"),"status",o==="status",i,"cycsort","",this._t("col.status_tip",{},"Cycle outcome: Completed (natural end), Interrupted (abrupt power drop), Force Stopped (manual), or Needs Review (feedback pending)."))}
      ${ut(this._t("lbl.date",{},"Date"),"date",o==="date",i,"cycsort","",this._t("col.date_tip",{},"Date and time the cycle started."))}
      ${ut(this._t("lbl.duration",{},"Duration"),"duration",o==="duration",i,"cycsort","right",this._t("col.duration_tip",{},"Total cycle run time from start to end."))}
      ${ut(this._t("lbl.energy",{},"Energy"),"energy",o==="energy",i,"cycsort","right",this._t("col.energy_tip",{},"Total energy consumed (kWh). Computed by integrating power over time."))}
      ${ut(this._t("lbl.cost",{},"Cost"),"cost",o==="cost",i,"cycsort","right",this._t("col.cost_tip",{},"Energy cost for this cycle, frozen at completion using the price in effect then (energy x price per kWh). Set a price under Settings to populate it."))}
      ${ut(this._t("lbl.confidence",{},"Confidence"),"confidence",o==="confidence",i,"cycsort","right",this._t("col.confidence_tip",{},"Profile match confidence (0-100%). How closely the cycle power curve matched the identified program."))}
    ...
    </div>`,X=k.length!==s.length?this._t("lbl.n_shown",{n:k.length},`, ${k.length} shown`):"",H=e.length?this._t("lbl.n_imported_note",{n:e.length},`, ${e.length} imported`):"",nt=this._t("lbl.cycles_title",{n:`${t.length}${H}${X}`},`Cycles (${t.length}${H}${X})`),U=r?`<div class="wd-card-actions" style="margin:0 0 4px;justify-content:flex-end">
    ...
    </div>`:"",m=new Set(e.map(w=>w.id)),f=[...a].some(w=>m.has(w)),x=n?`<div class="wd-card-actions" style="margin:0 0 10px">
      ...
      <div class="wd-card">
        ...
        ${tt}
        ${U}${x}
        ${k.length===0?`<div class="wd-empty" style="padding:24px"><div class="wd-icon">\u{1F4CB}</div>${s.length?this._t("msg.no_cycles_match",{},"No cycles match the current filter."):this._t("msg.no_cycles_yet",{},"No cycles recorded yet.")}</div>`:`<div class="wd-table-wrap"><table class="wd-table">${it}<tbody>${Y}</tbody></table></div>`}
        ${W}
      </div>`;return(this._cyclesError?`<div class="wd-error-state"><span>${this._t("msg.fetch_error",{},"Failed to load data.")}</span><button class="wd-btn" type="button" data-action="retry-cycles">${this._t("btn.retry",{},"Retry")}</button></div>`:"")+B}_trendIcon(t){return t==="up"?`<span title="${d(this._t("trend.up",{},"Trending up"))}" style="color:var(--warning-color,#ff9800)">\u2191</span>`:t==="down"?`<span title="${d(this._t("trend.down",{},"Trending down"))}" style="color:var(--info-color,#2196f3)">\u2193</span>`:""}_profileCardHtml(t){const e=t.avg_duration?this._t("lbl.duration_avg",{v:Math.round(t.avg_duration/60)},`~${Math.round(t.avg_duration/60)}m avg`):this._t("lbl.no_duration",{},"no duration"),s=t.avg_energy!=null?` \xB7 ${_t(t.avg_energy)}/cycle`:"",r=t.avg_energy!=null&&t.cycle_count?` \xB7 <strong>${_t(t.avg_energy*t.cycle_count)}</strong> total`:"",n=this._hass&&this._hass.config&&this._hass.config.currency||"",a=t.avg_cost!=null?` \xB7 ${this._t("lbl.avg_cost",{},"Avg")} ${t.avg_cost.toFixed(2)}${n?" "+n:""}/${this._t("lbl.per_cycle_short",{},"cycle")}`:"",o=(this._profileHealth||{})[t.name],i=(this._profileTrends||{})[t.name];let l="";o&&o.health_status==="poor"?l=`<span class="wd-badge" style="color:var(--error-color,#f44336);background:rgba(244,67,54,.12)" title="${d(this._t("badge.poor_fit_tip",{},"Inconsistent match history \u2014 consider rebuilding this profile"))}">\u26A0 ${this._t("badge.poor_fit",{},"poor fit")}</span>`:o&&o.health_status==="fair"&&(l=`<span class="wd-badge" style="color:var(--warning-color,#ff9800);background:rgba(255,152,0,.12)" title="${d(this._t("badge.fair_fit_tip",{},"Moderate match consistency \u2014 some cycles assigned to this profile have lower confidence scores. Label more cycles or re-record the profile to improve accuracy."))}">${this._t("badge.fair_fit",{},"fair fit")}</span>`);let c="";if(i){const k=this._trendIcon(i.duration_trend),$=i.energy_trend?this._trendIcon(i.energy_trend):"";if(i.duration_trend!=="stable"||i.energy_trend==="up"){const C=[];if(i.duration_trend!=="stable"){const z=`${i.duration_slope_pct>0?"+":""}${i.duration_slope_pct}`;C.push(i.duration_trend==="up"?this._t("msg.duration_trend_up_tip",{pct:z},`Duration up (${z}%/cycle)`):this._t("msg.duration_trend_down_tip",{pct:z},`Duration down (${z}%/cycle)`))}if(i.energy_trend&&i.energy_trend!=="stable"){const z=`${i.energy_slope_pct>0?"+":""}${i.energy_slope_pct}`;C.push(i.energy_trend==="up"?this._t("msg.energy_trend_up_tip",{pct:z},`Energy up (${z}%/cycle)`):this._t("msg.energy_trend_down_tip",{pct:z},`Energy down (${z}%/cycle)`))}const j=C.join(", ")||this._t("msg.performance_trending",{},"Performance trending");c=`<span class="wd-badge" style="color:var(--secondary-text-color,#888)" title="${d(j)}">${k}${$||""}</span>`}}const _=this._constants&&this._constants.PROFILE_MIN_WARMUP_CYCLES||5,p=o&&o.cycle_count||0,g=p<_&&!t.is_imported?`<span class="wd-badge" title="${d(this._t("msg.warmup_detail",{needed:_},`This profile needs ${_} labelled cycles before auto-matching begins. Every confirmed cycle helps it learn.`))}" style="background:var(--info-color,#2196f3);color:#fff">${this._t("msg.warmup_badge",{done:p,needed:_},`Still learning (${p}/${_} cycles)`)}</span>`:"",b=t.is_imported?`<span class="wd-badge" title="${d(this._t("badge.imported_tip",{},"Imported from the community store. Used for matching only, not counted in stats."))}" style="background:var(--info-color,#2196f3);color:#fff">\u{1F4E5} ${this._t("status.imported",{},"Imported")}</span>`:"",v=[l,c,g,b].filter(Boolean).join(" "),S=Array.isArray(t.signature_curve)&&t.signature_curve.length>=3?`<canvas class="wd-prof-spark" data-spark-prof="${d(t.name)}" width="64" height="20" aria-label="${d(this._t("lbl.sparkline",{name:t.name},"Average power curve"))}"></canvas>`:"";return`
      ...
      </div>`}_drawProfileSparklines(){const t=this.shadowRoot;if(!t)return;const e=t.querySelectorAll("canvas[data-spark-prof]");if(!e.length)return;const s=(getComputedStyle(this).getPropertyValue("--primary-color")||"#03a9f4").trim()||"#03a9f4",r={};for(const n of this._profiles||[])r[n.name]=n;e.forEach(n=>{const a=n.dataset.sparkProf,o=r[a]&&r[a].signature_curve||[];if(!Array.isArray(o)||o.length<3)return;const i=window.devicePixelRatio||1,l=n.getBoundingClientRect(),c=n.width=Math.max(1,Math.round((l.width||64)*i)),_=n.height=Math.max(1,Math.round((l.height||20)*i)),p=n.getContext("2d");p.clearRect(0,0,c,_);const u=Math.max(...o,1),g=2*i,b=S=>g+(o.length===1?0:S/(o.length-1)*(c-2*g)),v=S=>_-g-Math.max(0,S)/u*(_-2*g);p.beginPath(),o.forEach((S,k)=>{const $=b(k),C=v(S);k===0?p.moveTo($,C):p.lineTo($,C)}),p.strokeStyle=s,p.lineWidth=1.5*i,p.lineJoin="round",p.lineCap="round",p.stroke()})}_htmlProfiles(){const t=this._busy.has("rebuild-envelopes"),e=this._canEdit(),s={};this._profiles.forEach(g=>{s[g.name]=g});const r=this._profileGroups||{groups:[],suggestions:[]},n=new Set;r.groups.forEach(g=>(g.members||[]).forEach(b=>n.add(b)));const a=r.groups.map(g=>{const b=(g.members||[]).map(C=>s[C]?this._profileCardHtml(s[C]):"").join(""),v=Math.round((g.cohesion!=null?g.cohesion:1)*100),S=g.cohesive?`<span class="wd-badge" style="color:var(--success-color,#4caf50);background:rgba(76,175,80,.14);margin-bottom:0">${this._t("lbl.cohesion_good",{pct:v},"cohesion "+v+"%")}</span>`:`<span class="wd-badge" style="color:var(--warning-color,#ff9800);background:rgba(255,152,0,.14);margin-bottom:0">${this._t("lbl.cohesion_low",{pct:v},"\u26A0 low cohesion "+v+"%")}</span>`,k=g.cohesive?"":`<p class="wd-info" style="margin:0 0 8px;color:var(--warning-color,#ff9800)">${this._t("msg.group_not_cohesive",{},"These profiles aren't similar enough to group reliably, so matching treats them individually until you remove the outlier or split the group.")}</p>`;return`<div class="wd-card" style="margin-bottom:12px">
    ...
    `}_htmlProfileGroupModal(t){const e=this._busy.has("pg-save"),s=this._profileEnvCache||{},r=p=>Q[Math.max(0,this._profiles.findIndex(u=>u.name===p))%Q.length],n=t.members||[],a=this._profiles.map(p=>{const u=n.includes(p.name),g=p.avg_duration?`~${Math.round(p.avg_duration/60)}m`:"",b=p.avg_energy!=null?` \xB7 ${_t(p.avg_energy)}`:"",v=u?`<span style="display:inline-block;width:10px;height:10px;border-radius:2px;background:${r(p.name)};margin:0 2px"></span>`:"";return`<label class="wd-rev-tag"><input type="checkbox" class="wd-pg-mem" value="${d(p.name)}" ${u?"checked":""}> ${v}${d(p.name)} <span style="color:var(--secondary-text-color);font-size:.85em">${g}${b}</span></label>`}).join(""),o=n.filter(p=>s[p]&&(s[p].avg||[]).length),i=o.length?`<div class="wd-leg">${o.map(p=>`<span class="wd-leg-i"><span class="wd-leg-sw" style="background:${r(p)}"></span> ${d(p)}</span>`).join("")}</div>`:"",l=o.length?`<div class="wd-canvas-wrap" style="margin-top:8px"><canvas id="wd-pgroup-canvas" role="img" aria-label="${d(this._t("lbl.aria_envelope_chart",{},"Profile power envelope chart"))}" style="height:150px"></canvas></div>${i}`:`<p class="wd-info">${this._t("msg.group_preview_hint",{},"Tick 2+ members to preview and compare their power curves.")}</p>`,c=((this._profileGroups||{}).groups||[]).find(p=>p.name===t.orig),_=c&&c.cohesion!=null?`<span class="wd-badge" style="color:${c.cohesive?"var(--success-color,#4caf50)":"var(--warning-color,#ff9800)"};background:${c.cohesive?"rgba(76,175,80,.14)":"rgba(255,152,0,.14)"}">${c.cohesive?this._t("lbl.cohesion_good",{pct:Math.round(c.cohesion*100)},"cohesion "+Math.round(c.cohesion*100)+"%"):this._t("lbl.cohesion_low",{pct:Math.round(c.cohesion*100)},"\u26A0 low cohesion "+Math.round(c.cohesion*100)+"%")}</span>`:"";return`<h2>${t.orig?this._t("modal.edit_group",{},"Edit profile group"):this._t("modal.new_group",{},"New profile group")}</h2>
    ...
    </label>`,v=r?`<p class="wd-info" style="margin:0 0 10px;font-size:.82em">${this._t("msg.settings_basic_note",{},"Showing essential settings. Switch to Advanced for the full list.")}</p>`:"",S=this._busy.has("save-settings"),k=l.size,$=k!==1?"s":"",C=k?`
      ...
      </div>`:"",j=this._suggestions.length,z=[...a].filter(U=>!n.has(U)).length,I=j+z,A=this._settingsSugOnly&&!this._settingsSearch,M=j?`<button class="wd-btn wd-btn-sm wd-btn-primary" data-action="sug-apply-all">${this._t("btn.apply_all",{},"Apply all")}</button>`:"",E=j?`<button class="wd-btn wd-btn-sm wd-btn-secondary" data-action="sug-dismiss">${this._t("btn.dismiss",{},"Dismiss")}</button>`:"",L=I?A?`
    ...
    `}_htmlSettingsHistory(){const t=this._settingsChangelog||[];if(!t.length)return"";const e=this._settingsHistoryOpen,s=this._canEdit(),r=t.slice(0,100).map(n=>{const a=s?`<button class="wd-btn wd-btn-secondary wd-btn-xs" data-action="settings-revert-key" data-key="${d(n.key)}" data-val="${d(JSON.stringify(n.old))}" title="${d(this._t("btn.revert_to_previous",{},"Restore this setting to its previous value"))}">${this._t("btn.revert",{},"Revert")}</button>`:"";return`<tr>
      ...
      <div class="wd-combo-row">
        ...
        <div class="wd-field-hint">${d(this._t("msg.pick_brand_first",{},"Pick an appliance brand first."))}</div></div>`;this._ensureCatalogEntry(),this._catalog.forBrand!==o&&(this._catalog.forBrand=o,this._catalog.devices=void 0);const i=Array.isArray(this._catalog.devices)?this._catalog.devices:[];this._entityListCache=this._entityListCache||{},this._entityListCache[t]=i.map(g=>g.model).filter(Boolean);const l=this._catalogEntryFor(s,"device"),c=this._statusTag(l),_=this._catalog.devices===null?` <span class="wd-info" style="font-size:.85em">${this._t("msg.loading",{},"Loading\u2026")}</span>`:"";let p="";if(l){const g=[],b=re(l.manualUrl);b&&g.push(`<a href="${d(b)}" target="_blank" rel="noopener noreferrer nofollow">${this._t("link.manual",{},"Manual \u2197")}</a>`),(this._constants&&this._constants.storePrefs||{}).show_contributor!==!1&&g.push(this._t("store.contributed_by",{name:d(l.createdByName||this._t("lbl.anonymous",{},"Anonymous"))},`by ${d(l.createdByName||"Anonymous")}`));const v=!!(this._storeStatus&&this._storeStatus.connected);let S="";if(v){const k=[1,2,3,4,5].map($=>`<button type="button" class="wd-star-btn" data-action="store-rate-device" data-device-id="${d(l.id)}" data-rating="${$}" aria-label="${this._t("lbl.rate_n",{n:$},`${$} stars`)}">\u2605</button>`).join("");S=`<button type="button" class="wd-btn wd-btn-secondary wd-btn-sm" data-action="store-confirm-device" data-device-id="${d(l.id)}" ${l.status!=="pending"?"disabled":""}>${l.status==="pending"?this._t("btn.confirm_appliance",{},"Confirm this appliance"):this._t("btn.confirmed",{},"Confirmed")}</button>
          <span class="wd-star-row" title="${d(this._t("lbl.rate_quality",{},"Rate quality"))}">${k}</span>`}else S=`<span class="wd-info" style="font-size:.85em">${this._t("msg.connect_to_confirm",{},"Connect in the settings gear to confirm or rate.")}</span>`;p=`<div class="wd-store-picker-detail">${g.join(" \xB7 ")}<div class="wd-store-picker-actions">${S}</div></div>`}const u=this._onlineEnabled()&&this._storeDeviceDeclared()?`<div class="wd-store-actions">
    ...
    </div>`:"";return`<div class="wd-field"><label>${d(r)} ${n?st(n):""}${c}${_}</label>
      <div class="wd-combo-row">
        <div class="wd-combo">
    ...
    `}_htmlMlStatusSection(t,e){const s=e&&this._busy.has("ml-train-now:"+e)||t&&t.running,r=this._canEdit()?`<button class="wd-btn wd-btn-primary wd-btn-sm" data-action="ml-train-now" ${s?"disabled":""}>${s?`<span class="wd-spin"></span> ${this._t("status.training",{},"Training\u2026")}`:this._t("btn.train_now",{},"Train now")}</button>`:"";if(!t)return`<div class="wd-card"><div class="wd-card-title" style="margin:0 0 4px">${this._t("hdr.status",{},"Status")}</div><p class="wd-info" style="margin:0">${this._t("msg.loading",{},"Loading\u2026")}</p></div>`;const n=Object.keys(t.on_device_models||{}).length,a=n?`<span style="color:var(--success-color,#4caf50);font-weight:600">${this._t("ml.personalized",{},"\u25CF Personalized to this machine")}</span> <span style="color:var(--secondary-text-color)">${this._t("lbl.models_fine_tuned",{count:n},"(fine-tuned models: "+n+")")}</span>`:`<span style="color:var(--secondary-text-color)">${this._t("ml.builtin_models",{},"\u25CF Using built-in models")}</span>`,o=t.cycle_count||0,i=t.min_cycles||0,l=o>=i,c=i>0?Math.min(100,Math.round(o/i*100)):100,_=l?"var(--success-color,#4caf50)":"var(--warning-color,#ff9800)",p=Math.max(0,i-o),u=l?this._t("msg.enough_data",{current:o,min:i},`Enough data to learn from (${o}/${i} cycles).`):this._t("msg.collecting_data",{need:p,current:o,min:i},`Collecting data. Cycles still needed before fine-tuning can start: ${p} (${o}/${i}).`),g=`<div style="height:8px;border-radius:6px;background:var(--secondary-background-color);overflow:hidden;margin:8px 0"><div style="width:${c}%;height:100%;background:${_}"></div></div>`,b=t.last_trained?et(t.last_trained):"never",v=s?`<span style="color:var(--info-color,#2196f3)"><span class="wd-spin"></span> ${this._t("status.fine_tuning",{},"fine-tuning now\u2026")}</span>`:t.enabled?this._t("lbl.auto_fine_tune_on",{hour:String(t.hour).padStart(2,"0")},`auto fine-tune on (around ${String(t.hour).padStart(2,"0")}:00)`):this._t("lbl.auto_fine_tune_off",{},"auto fine-tune off");return`<div class="wd-card">
      ...
      ${g}
      <p class="wd-info" style="margin:0">${this._t("lbl.last_checked",{},"Last checked:")} <strong>${d(b)}</strong> \xB7 ${v}</p>
    </div>`}_htmlMlLearnedSection(t){if(!t)return"";const e=t.on_device_models||{},s=Object.keys(e),r=this._busy.has("ml-revert-models");let n;if(!s.length)n=`<p class="wd-info" style="margin:0">${this._t("msg.no_fine_tuned",{},"Nothing fine-tuned yet \u2014 WashData is using its built-in models.")}</p>`;else{const a=s.map(i=>{const l=e[i]||{},c=l.trained_at?et(l.trained_at):"unknown";return`<div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid var(--divider-color)">
      ...
      ${r}
        ...
        </div>`,n=v);const L=this._pgStagedVal(p),F=L!=null&&!($!==""&&$!==null&&$!==void 0&&this._pgSameVal(L,$)),V=this._canEdit()&&F&&this._pgIsPublishable(p)?`<button class="wd-pg-pub" data-action="pg-publish-one" data-pgkey="${d(p)}" aria-label="${d(this._t("btn.pg_publish_one",{label:k},"Publish "+k+" to the integration"))}" title="${d(this._t("btn.pg_publish_one",{label:k},"Publish "+k+" to the integration"))}">\u2191</button>`:'<span class="wd-pg-pub-slot" aria-hidden="true"></span>';return`${E}<div style="display:flex;align-items:flex-start;gap:6px;margin:0 0 6px 11px">
        <div style="flex:1;min-width:0">
          <div style="font-size:.82em;font-weight:600;margin-bottom:1px">${d(k)}${z?` <span style="color:${A};font-size:.85em" title="${d(this._t("lbl.pg_drag_hint",{},"Drag line on graph"))}">\u2195</span>`:""}</div>
          ${b?`<div style="font-size:.72em;color:var(--secondary-text-color);line-height:1.3">${d(this._t("pg_desc."+p,{},b))}</div>`:""}
        </div>
        <div style="display:flex;align-items:center;gap:4px;flex-shrink:0">
          ${j?`<input class="wd-pg-param-chk" type="checkbox" data-pgkey="${d(p)}" data-pgtype="bool" aria-label="${d(k)}" ${C?"checked":""} style="width:18px;height:18px;margin:1px 27px 0 27px">`:`<input class="wd-pg-param-inp" type="text" inputmode="decimal" data-pgkey="${d(p)}" value="${C!==""?d(String(C)):""}" placeholder="${$!==""?d(String($)):""}" aria-label="${d(k)}" style="width:72px">`}
      ...
      </div>`}const i=e.summary||{},l=`<div style="font-size:.82em;color:var(--secondary-text-color);margin-bottom:8px">${this._t("msg.pg_history_summary",{detected:i.detected,correct:i.match_correct,total:i.cycles},`${i.detected}/${i.cycles} detected \xB7 ${i.match_correct} matched correctly`)}</div>`,c=g=>(this._cycles||[]).find(b=>b.id===g),_={};(e.baseline_rows||[]).forEach(g=>{_[g.cycle_id]=g});const p=e.rows.map(g=>{const b=c(g.cycle_id),v=b&&b.start_time?et(b.start_time):(g.cycle_id||"").slice(0,8),S=g.match_correct===!0?"\u2713":g.match_correct===!1?"\u2717":"\u2014",k=g.match_correct===!0?"var(--success-color,#4caf50)":g.match_correct===!1?"var(--error-color,#f44336)":"var(--secondary-text-color)",$=g.matched_profile||this._t("lbl.unlabelled",{},"Unlabelled"),C=g.duration_s?Math.round(g.duration_s/60)+"m":"\u2014",j=g.overrun_ratio!=null?` (${Math.round(g.overrun_ratio*100)}%)`:"",z=g.alerts&&g.alerts.length?` <span title="${d(g.alerts.join(", "))}" style="color:var(--warning-color,#ff9800)">\u26A0</span>`:"",I=_[g.cycle_id];let A="";return I&&(I.match_correct!==!0&&g.match_correct===!0?A=`<span style="color:var(--success-color,#4caf50)">\u25B2 ${this._t("lbl.pg_fixed",{},"fixed")}</span>`:I.match_correct===!0&&g.match_correct!==!0?A=`<span style="color:var(--error-color,#f44336)">\u25BC ${this._t("lbl.pg_broke",{},"broke")}</span>`:String(I.termination_reason)!==String(g.termination_reason)&&(A=`<span style="color:var(--warning-color,#ff9800)">${d(String(I.termination_reason||"\u2014"))}\u2192${d(String(g.termination_reason||"\u2014"))}</span>`)),`<tr class="wd-pg-hrow${g.cycle_id===this._pgCycleId?" selected":""}" data-action="pg-open-cycle" data-cid="${d(g.cycle_id)}" title="${d(this._t("msg.pg_row_load_hint",{},"Load this cycle in the graph above"))}">
        <td>${d(v)}</td>
        <td><span style="color:${k};font-weight:700">${S}</span> ${d($)}</td>
        <td>${d(String(g.termination_reason||"\u2014"))}</td>
        <td style="font-variant-numeric:tabular-nums">${C}${j}${z}</td>
        ${e.diff?`<td>${A}</td>`:""}
    ...
    </div>`,o=t?this._htmlPgBatchBar():"";return`${n}${a}${this._htmlPgRecentRuns("pg_sweep")}${o}${this._htmlPgSweepResult()}`}_htmlPgSweepResult(){const t=this._pgSweepNew;if(!t||!Array.isArray(t.points)||!t.points.length)return"";const e=this._pgSweepObjectives().find(p=>p[0]===t.objective),s=e?e[2]:!1,r=t.points.filter(p=>p.metric!=null).map(p=>p.metric);if(!r.length)return`<div class="wd-empty" style="padding:16px">${this._t("msg.pg_sweep_no_metric",{},"Not enough data to score this objective.")}</div>`;const n=Math.min(...r),a=Math.max(...r),o=s?Math.min(...r):Math.max(...r),i=(t.points.find(p=>p.metric===o)||{}).value,l=p=>t.objective==="median_overrun"?Math.round(p*100)+"% off":Math.round(p*100)+"%",c=t.points.map(p=>{const u=a>n&&p.metric!=null?(p.metric-n)/(a-n):p.metric!=null?1:0,g=p.metric===o,b=t.current_value!=null&&Math.abs(p.value-t.current_value)<1e-6,v=g?"var(--success-color,#4caf50)":"var(--primary-color)";return`<div style="display:flex;align-items:center;gap:8px;margin:3px 0;font-size:.82em">
        <span style="flex:0 0 74px;text-align:right;font-variant-numeric:tabular-nums">${d(String(p.value))}${b?" \u25C0":""}</span>
        <div style="flex:1;height:16px;border-radius:4px;background:var(--secondary-background-color);overflow:hidden"><div style="height:100%;width:${Math.round((p.metric!=null?.15+.85*u:0)*100)}%;background:${v};border-radius:4px"></div></div>
      ...
      <span style="font-size:.75em;color:var(--secondary-text-color);text-transform:uppercase">${this._t("lbl.phase",{},"Phase")} <span id="wd-pg-phase" style="color:var(--primary-text-color,inherit)">\u2014</span></span>
      ...
      </div>`};let s="";if(t&&(t.stage2||t.stage4)){const i=t.stage2||{},l=t.stage4||{},c=t.dtw||{},_=l.final_score??c.blended_score??i.score,p=this._pgDetail&&this._pgDetail.outcome&&this._pgDetail.outcome.confidence,u=p??_;let g="\u2014",b="var(--secondary-text-color)";u!=null&&(u>=.7?(g="\u2705 "+this._t("lbl.pg_strong_match",{},"Strong match"),b="var(--success-color, #4caf50)"):u>=.4?(g="\u26A0 "+this._t("lbl.pg_weak_match",{},"Weak match"),b="var(--warning-color, #ff9800)"):(g="\u274C "+this._t("lbl.pg_poor_match",{},"Poor match"),b="var(--error-color, #f44336)"));const v=t.profile_name||this._pgProfileName||"\u2014";s+=`<div style="font-weight:700;font-size:.9em;color:${b};margin-bottom:8px">${g}</div>`,v!=="\u2014"&&(s+=`<div style="font-size:.82em;color:var(--secondary-text-color);margin-bottom:6px">${d(v)} \xB7 ${d(this._t("lbl.score",{},"score"))} ${_!=null?_.toFixed(3):"\u2014"}</div>`),s+=e(this._t("lbl.correlation",{},"Correlation"),i.correlation,1,"#42a5f5",i.correlation!=null?i.correlation.toFixed(2):null),c.blended_score!=null&&(s+=e(this._t("lbl.pg_dtw",{},"DTW"),c.blended_score,1,"#ab47bc",c.blended_score.toFixed(2))),l.duration_agreement!=null&&(s+=e(this._t("lbl.duration",{},"Duration"),l.duration_agreement,1,"#66bb6a",l.duration_agreement.toFixed(2))),l.energy_agreement!=null&&(s+=e(this._t("lbl.energy",{},"Energy"),l.energy_agreement,1,"#ffa726",l.energy_agreement.toFixed(2))),s+='<div style="height:1px;background:var(--divider-color,rgba(127,127,127,.2));margin:8px 0"></div>'}else t||(s+=`<p class="wd-info" style="margin:0 0 8px">${this._t("msg.pg_analysis_empty2",{},"Press Run to see match analysis.")}</p>`);const r=this._pgDetail&&this._pgDetail.outcome,n=r&&r.matched_profile||t&&t.profile_name||this._pgProfileName||(this._cycles||[]).find(i=>i.id===this._pgCycleId)?.profile_name||"",a=r&&r.confidence;if(n&&a!=null){const i=Math.round(Math.max(0,Math.min(1,a))*100);s+=`<div class="wd-subhead" style="margin:0 0 4px">${d(this._t("lbl.pg_match_confidence",{},"Match confidence"))}</div>`,s+=`<div class="wd-pg-cand-row">
        ...
        <span class="wd-pg-cand-pct">${i}%</span>
      </div>`}return`<div>${s||`<p class="wd-info" style="margin:0">${this._t("msg.pg_analysis_hint2",{},"Pick a cycle and press Run to load match analysis.")}</p>`}</div>`}async _pgLoad(){const t=this._devices[this._selIdx];if(!t||this._pgLoading)return;const e=this._pgCycleId||this._cycles?.[0]?.id||"";if(!e)return;this._pgCycleId=e,this._pgLoading=!0,this._pgView=null,this._pgHoverT=null,this._pgPowerPts=null,this._pgDtwData=null,this._pgEnvData=null,this._pgDetail=null;const s=++this._pgLoadSeq;this._render();try{const r=await this._ws({type:`${y}/get_cycle_power_data`,entry_id:t.entry_id,cycle_id:e}),n=r.samples||[],a=[];for(const l of n){if(!Array.isArray(l)||l.length<2)continue;const c=+l[0],_=+l[1];!isNaN(c)&&!isNaN(_)&&a.push({t:c,w:_})}if(this._pgPowerPts=a.length?a:null,typeof r.full_duration_s=="number"&&r.full_duration_s>0){const l=(this._cycles||[]).find(c=>c.id===e);l&&(l._pg_duration=r.full_duration_s)}const o=this._pgProfileName||(this._cycles||[]).find(l=>l.id===e)?.profile_name||"";if(a.length)try{const l={type:`${y}/get_dtw_debug`,entry_id:t.entry_id,cycle_id:e};o&&(l.profile_name=o),this._pgDtwData=await this._ws(l),this._pgNeedsRestart=!1}catch(l){this._pgIsUnknownCmd(l)&&(this._pgNeedsRestart=!0),this._pgDtwData=null}const i=this._pgDtwData?.profile_name||o;if(i)try{const l=await this._ws({type:`${y}/get_profile_envelope`,entry_id:t.entry_id,profile_name:i});this._pgEnvData=l.envelope||null}catch{this._pgEnvData=null}if(s!==this._pgLoadSeq)return;await this._pgLoadDetail(t.entry_id,e)}catch(r){this._showToast(this._t("msg.toast_error",{error:r.message||r},"Error: "+(r.message||r)),"error")}s===this._pgLoadSeq&&(this._pgLoading=!1,this._render(),requestAnimationFrame(()=>{this._pgDrawCanvas(),this._pgUpdateStripAt(null)}),this._pgNeedsRestart&&this._tab==="playground"?(this._pgRestartRetries||0)<5&&!this._pgRestartRetryTimer&&(this._pgRestartRetries=(this._pgRestartRetries||0)+1,this._pgRestartRetryTimer=setTimeout(()=>{this._pgRestartRetryTimer=null,this._pgNeedsRestart&&this._tab==="playground"&&this._pgLoad()},3e3)):this._pgRestartRetries=0)}_pgCancelRun(){this._pgLoadSeq++,this._pgLoading=!1,this._render()}_pgSelectCycle(t){t&&(this._pgCycleId=t,this._pgProfileName="",this._pgView=null,this._pgHoverT=null,this._pgLoading=!1,this._pgLoad(),requestAnimationFrame(()=>{const e=this.shadowRoot&&this.shadowRoot.getElementById("wd-pg-canvas");e&&typeof e.scrollIntoView=="function"&&e.scrollIntoView({behavior:"smooth",block:"center"})}))}async _pgLoadDetail(t,e){const s={...this._pgParamOverrides};this._pgThreshStart!=null&&(s.start_threshold_w=this._pgThreshStart),this._pgThreshStop!=null&&(s.stop_threshold_w=this._pgThreshStop);try{const r=await this._ws({type:`${y}/start_playground_cycle_detail`,entry_id:t,cycle_id:e,settings_override:s,stress_tail:this._pgStressTail,stress_idle_w:this._pgStressIdleW!=null?parseFloat(this._pgStressIdleW):null}),n=r&&r.task_id;if(!n)throw new Error("no task id");this._addProvisionalTask(n,"pg_detail",t,0);const a=await new Promise(o=>{this._taskCallbacks[n]=async l=>{if(l.state==="done"||l.state==="cancelled")try{const c=await this._ws({type:`${y}/get_task_result`,task_id:l.id});o(c&&c.result)}catch{o(null)}else o(null)};const i=this._tasks[n];i&&i.state!=="running"?this._settleTaskCallback(i):this._tasksSubscribed||this._pollTaskGeneric(n)});if(!this._isActiveEntry(t))return;this._pgDetail=a&&!a.error?a:null,this._pgNeedsRestart=!1}catch(r){this._pgIsUnknownCmd(r)&&(this._pgNeedsRestart=!0),this._pgDetail=null}}_pgRerunDetail(){const t=this._devices[this._selIdx];!t||!this._pgCycleId||(clearTimeout(this._pgDetailDebounceTimer),this._pgDetailDebounceTimer=setTimeout(async()=>{this._pgDetailBusy=!0,this._render(),await this._pgLoadDetail(t.entry_id,this._pgCycleId),this._pgDetailBusy=!1,this._render(),requestAnimationFrame(()=>this._pgDrawCanvas())},220))}_pgMapState(t){return t==="running"||t==="paused"?"running":t==="ending"?"ending":t==="starting"?"detecting":t==="anti_wrinkle"?"anti_wrinkle":"idle"}_pgSeriesAt(t){const e=this._pgDetail&&this._pgDetail.series;if(!e||!e.length)return null;let s=e[0];for(const r of e)if(r.t<=t)s=r;else break;return s}_pgStateSegsFromSeries(t){const e=this._pgDetail&&this._pgDetail.series;if(!e||!e.length)return[];const s=[];let r=null;for(const n of e){const a=this._pgMapState(n.state);!r||r.state!==a?(r={start:n.t,end:n.t,state:a},s.push(r)):r.end=n.t}return s.length&&(s[s.length-1].end=t),s}_pgDrawCanvas(){if(this._tab!=="playground")return;const t=this.shadowRoot,e=t&&t.getElementById("wd-pg-canvas");if(!e)return;const s=this._pgPowerPts,r=e.getBoundingClientRect(),n=window.devicePixelRatio||1,a=Math.max(1,Math.round(r.width*n)),o=Math.max(1,Math.round((r.height||280)*n));(e.width!==a||e.height!==o)&&(e.width=a,e.height=o);const i=e.getContext("2d"),l=getComputedStyle(this),c=(l.getPropertyValue("--primary-color")||"#03a9f4").trim(),_=(l.getPropertyValue("--divider-color")||"rgba(127,127,127,.2)").trim(),p=(l.getPropertyValue("--secondary-text-color")||"#888").trim(),u=(l.getPropertyValue("--secondary-background-color")||"#1a1a1a").trim();i.clearRect(0,0,a,o);const g=34*n,b=14*n,v=Bt*n,S=44*n,k=8*n,$=v+8*n,C=g+b+4*n,j=o-$-C;if(!s||!s.length){this._pgLoading&&(i.fillStyle=p,i.font=`${12*n}px sans-serif`,i.textAlign="center",i.textBaseline="middle",i.fillText(this._t("msg.loading",{},"Loading\u2026"),a/2,o/2));return}const z=this._pgDetail&&this._pgDetail.outcome&&this._pgDetail.outcome.stress,I=z&&z.enabled?z.synthetic_from_s:null,A=I!=null&&this._pgDetail&&this._pgDetail.series?this._pgDetail.series.filter(T=>T.t>=I):[];let M=(this._cycles||[]).find(T=>T.id===this._pgCycleId)?._pg_duration||s[s.length-1].t||1;z&&z.terminated&&z.terminated_after_s!=null&&I!=null?M=Math.max(M,I+z.terminated_after_s):A.length&&(M=Math.max(M,A[A.length-1].t));const E=Math.max(...s.map(T=>T.w),...A.map(T=>T.power||0),1),L=this._pgThreshStart??this._pgFieldVal("start_threshold_w",{})??50,F=this._pgThreshStop??this._pgFieldVal("stop_threshold_w",{})??5;let V=0,Y=M;this._pgView&&this._pgView.max-this._pgView.min>1&&(V=Math.max(0,this._pgView.min),Y=Math.min(M,this._pgView.max),Y-V<=1&&(V=0,Y=M));const it=Math.max(1e-6,Y-V),tt=a-S-k,X=T=>S+(T-V)/it*tt,H=T=>$+(1-Math.max(0,T)/E)*j;this._pgMap={vMin:V,vMax:Y,totalDur:M,padLpx:S/n,plotWpx:tt/n},i.strokeStyle="rgba(127,127,127,0.12)",i.lineWidth=n,i.setLineDash([]),[.25,.5,.75,1].map(T=>Math.round(T*E/100)*100||Math.round(T*E)).forEach(T=>{const N=H(T);N<$||N>$+j||(i.beginPath(),i.moveTo(S,N),i.lineTo(a-k,N),i.stroke(),i.fillStyle=p,i.font=`${9*n}px sans-serif`,i.textAlign="right",i.textBaseline="middle",i.fillText(T+"W",S-4*n,N))}),i.save(),i.beginPath(),i.rect(S,$,tt,j),i.clip();const U=this._pgEnvData,m=U&&Array.isArray(U.avg)?U.avg.filter(T=>Array.isArray(T)&&T.length>=2):[];if(m.length){const T=Math.max(...m.map(q=>q[0]))||1,N=q=>X(q/T*M),J=Array.isArray(U.min)?U.min.filter(q=>Array.isArray(q)&&q.length>=2):[],K=Array.isArray(U.max)?U.max.filter(q=>Array.isArray(q)&&q.length>=2):[];if(J.length&&K.length){i.beginPath(),K.forEach((q,ot)=>ot?i.lineTo(N(q[0]),H(q[1])):i.moveTo(N(q[0]),H(q[1])));for(let q=J.length-1;q>=0;q--)i.lineTo(N(J[q][0]),H(J[q][1]));i.closePath(),i.fillStyle="#eda10012",i.fill()}else i.beginPath(),i.strokeStyle="#eda100",i.lineWidth=2*n,i.setLineDash([5*n,4*n]),m.forEach((q,ot)=>ot?i.lineTo(N(q[0]),H(q[1])):i.moveTo(N(q[0]),H(q[1]))),i.stroke(),i.setLineDash([])}const f=this._pgDtwData,x=f&&Array.isArray(f.profile_trace)?f.profile_trace.filter(T=>Array.isArray(T)&&T.length>=2):[],D=f&&Array.isArray(f.cycle_trace)?f.cycle_trace.filter(T=>Array.isArray(T)&&T.length>=2):[],W=f&&Array.isArray(f.warp_path)?f.warp_path:[];if(D.length&&x.length&&W.length){const T=Math.max(...x.map(K=>K[0]))||1,N=Math.max(...D.map(K=>K[0]))||1,J=Math.max(1,Math.floor(W.length/25));i.save(),i.globalAlpha=.13,i.strokeStyle="#fff",i.lineWidth=n;for(let K=0;K<W.length;K+=J){const q=W[K];if(!Array.isArray(q)||q.length<2)continue;const ot=Math.min(q[0],D.length-1),ht=Math.min(q[1],x.length-1),bt=X(D[ot][0]/N*M),pt=H(D[ot][1]),yt=X(x[ht][0]/T*M),lt=H(x[ht][1]);i.beginPath(),i.moveTo(bt,pt),i.lineTo(yt,lt),i.stroke()}i.restore()}if(x.length){const T=Math.max(...x.map(N=>N[0]))||1;i.beginPath(),i.strokeStyle="#eda100",i.lineWidth=2*n,i.setLineDash([6*n,4*n]),x.forEach((N,J)=>{const K=X(N[0]/T*M),q=H(N[1]);J?i.lineTo(K,q):i.moveTo(K,q)}),i.stroke(),i.setLineDash([])}if(i.beginPath(),i.moveTo(X(0),H(0)),s.forEach(T=>i.lineTo(X(T.t),H(T.w))),i.lineTo(X(s[s.length-1].t),H(0)),i.closePath(),i.fillStyle=kt(c,.1),i.fill(),i.beginPath(),i.strokeStyle=c,i.lineWidth=2*n,s.forEach((T,N)=>N?i.lineTo(X(T.t),H(T.w)):i.moveTo(X(T.t),H(T.w))),i.stroke(),A.length){const T=s[s.length-1];i.beginPath(),i.moveTo(X(T.t),H(T.w)),A.forEach(N=>i.lineTo(X(N.t),H(N.power))),i.lineTo(X(A[A.length-1].t),H(0)),i.lineTo(X(T.t),H(0)),i.closePath(),i.fillStyle=kt(c,.06),i.fill(),i.beginPath(),i.strokeStyle=c,i.lineWidth=1.5*n,i.setLineDash([4*n,4*n]),i.moveTo(X(T.t),H(T.w)),A.forEach(N=>i.lineTo(X(N.t),H(N.power))),i.stroke(),i.setLineDash([])}if(I!=null&&I<Y){const T=X(I);i.fillStyle="rgba(192,57,43,0.07)",i.fillRect(T,$,Math.max(0,S+tt-T),j),i.save(),i.strokeStyle="rgba(192,57,43,0.5)",i.lineWidth=n,i.setLineDash([4*n,4*n]),i.beginPath(),i.moveTo(T,$),i.lineTo(T,$+j),i.stroke(),i.restore()}i.restore();const B=(T,N,J)=>{const K=H(T);K<$-2||K>$+j+2||(i.save(),i.strokeStyle=N,i.lineWidth=2*n,i.setLineDash([8*n,4*n]),i.beginPath(),i.moveTo(S,K),i.lineTo(a-k,K),i.stroke(),i.setLineDash([]),i.fillStyle=N,i.beginPath(),i.arc(S+(a-S-k)*.06,K,5*n,0,Math.PI*2),i.fill(),i.fillStyle=N,i.font=`bold ${9*n}px sans-serif`,i.textAlign="left",i.textBaseline="bottom",i.fillText(J+" "+Math.round(T)+"W",S+14*n,K-2*n),i.restore())};B(+L,"#2a78d6",this._t("lbl.start",{},"Start")),B(+F,"#e34948",this._t("btn.stop",{},"Stop"));const R={idle:u,detecting:"#42a5f566",running:"#66bb6a66",ending:"#ef535066",anti_wrinkle:"#ab47bc66"},w={idle:this._t("lbl.pg_idle",{},"Idle"),detecting:this._t("lbl.pg_detecting",{},"Detecting"),running:this._t("lbl.pg_ev_running",{},"Running"),ending:this._t("lbl.pg_ev_ending",{},"Ending"),anti_wrinkle:this._t("lbl.pg_anti_wrinkle",{},"Anti-wrinkle")},P=o-g-b;i.fillStyle=u,i.fillRect(S,P,a-S-k,g),i.save(),i.beginPath(),i.rect(S,P,tt,g),i.clip(),this._pgStateSegsFromSeries(M).forEach(T=>{const N=X(T.start),J=X(T.end);i.fillStyle=R[T.state]||_,i.fillRect(N,P,Math.max(1,J-N),g),J-N>50*n&&(i.fillStyle=p,i.font=`${8*n}px sans-serif`,i.textAlign="center",i.textBaseline="middle",i.fillText((w[T.state]||T.state).toUpperCase(),(N+J)/2,P+g/2))}),i.restore();const G=(this._cycles||[]).find(T=>T.id===this._pgCycleId),Z=this._pgDtwData?.profile_name||this._pgProfileName||G?.profile_name,at=Z?(this._profiles||[]).find(T=>T.name===Z):null,rt=o-b;at&&Array.isArray(at.phases)&&at.phases.length&&(i.save(),i.beginPath(),i.rect(S,rt,tt,b),i.clip(),at.phases.forEach((T,N)=>{const J=X((T.start||0)*M),K=X((T.end||1)*M),q=N*47%360;i.fillStyle=`hsla(${q},60%,55%,0.55)`,i.fillRect(J,rt,Math.max(1,K-J),b),K-J>40*n&&(i.fillStyle="#fff",i.font=`${7*n}px sans-serif`,i.textAlign="center",i.textBaseline="middle",i.fillText(T.name,(J+K)/2,rt+b/2))}),i.restore()),this._pgEventHits=[];const ct=this._pgDetail&&Array.isArray(this._pgDetail.events)?this._pgDetail.events.filter(T=>T.type!=="state"&&T.t>=V&&T.t<=Y).slice().sort((T,N)=>T.t-N.t):[],ft=11*n,Et=$-v/2-2*n,Zt=ft*2+3*n;let Nt=-1/0;const xt=this._pgHoverEvent;if(ct.forEach(T=>{const N=this._pgEventMeta(T.type),J=X(T.t);let K=Math.max(J,Nt+Zt);K=Math.max(S+ft,Math.min(a-k-ft,K)),Nt=K;const q=xt&&xt.t===T.t&&xt.type===T.type;i.save(),i.strokeStyle=N.color,i.globalAlpha=q?.9:.5,i.lineWidth=(q?2:1)*n,i.beginPath(),i.moveTo(K,Et+ft),i.lineTo(J,$),i.stroke(),q&&(i.globalAlpha=.3,i.setLineDash([2*n,3*n]),i.beginPath(),i.moveTo(J,$),i.lineTo(J,o-b),i.stroke(),i.setLineDash([])),i.restore(),i.save(),i.beginPath(),i.arc(K,Et,ft,0,Math.PI*2),i.fillStyle=q?N.color:u||"#1a1a1a",i.fill(),i.lineWidth=(q?2:1.5)*n,i.strokeStyle=N.color,i.stroke(),i.fillStyle=q?"#fff":N.color,i.font=`${14*n}px sans-serif`,i.textAlign="center",i.textBaseline="middle",i.fillText(N.glyph,K,Et+n),i.restore(),this._pgEventHits.push({cx:K/n,cy:Et/n,r:ft/n,type:T.type,label:N.label,detail:T.detail,t:T.t})}),xt){const T=this._pgEventHits.find(N=>N.t===xt.t&&N.type===xt.type);if(T){const N=this._pgEventMeta(T.type),J=gt=>`${Math.floor(gt/60)}:${String(Math.round(gt%60)).padStart(2,"0")}`,K=this._pgEventDescription(T.type),q=[{t:T.label,bold:!0,col:N.color},...K?[{t:K}]:[],...T.detail?[{t:T.detail,dim:!0}]:[],{t:`${this._t("lbl.from_start",{},"From start")} ${J(T.t)}`,dim:!0}];i.save(),i.font=`${10*n}px sans-serif`;const ot=Math.min(220*n,a-2*S),ht=[];q.forEach(gt=>{const At=String(gt.t).split(" ");let vt="";At.forEach(It=>{const jt=vt?vt+" "+It:It;i.measureText(jt).width>ot&&vt?(ht.push({...gt,t:vt}),vt=It):vt=jt}),ht.push({...gt,t:vt})});const bt=14*n,pt=Math.min(ot+14*n,a-8*n),yt=ht.length*bt+10*n;let wt=T.cx*n-pt/2;wt=Math.max(4*n,Math.min(a-pt-4*n,wt));const Lt=Et+ft+6*n;i.fillStyle=u,i.globalAlpha=.97,i.fillRect(wt,Lt,pt,yt),i.globalAlpha=1,i.strokeStyle=N.color,i.lineWidth=n,i.strokeRect(wt,Lt,pt,yt),i.textAlign="left",i.textBaseline="top",ht.forEach((gt,At)=>{i.font=`${gt.bold?"600 ":""}${10*n}px sans-serif`,i.fillStyle=gt.col||(gt.dim?p:(l.getPropertyValue("--primary-text-color")||"#ddd").trim()),i.fillText(gt.t,wt+7*n,Lt+5*n+At*bt)}),i.restore()}}if(this._pgHoverT!=null&&this._pgHoverT>=V&&this._pgHoverT<=Y){const T=X(this._pgHoverT);i.save(),i.strokeStyle="#e34948",i.lineWidth=1.5*n,i.setLineDash([4*n,3*n]),i.beginPath(),i.moveTo(T,$),i.lineTo(T,o-b),i.stroke(),i.setLineDash([]);const N=this._pgInterpPower(s,this._pgHoverT),J=H(N);i.fillStyle="#e34948",i.beginPath(),i.arc(T,J,3.5*n,0,Math.PI*2),i.fill();const K=lt=>`${Math.floor(lt/60)}:${String(Math.round(lt%60)).padStart(2,"0")}`,q=lt=>lt>=1e3?(lt/1e3).toFixed(2)+" kW":Math.round(lt)+" W",ot=[`${this._t("lbl.from_start",{},"From start")} ${K(this._pgHoverT)}`,`${this._t("lbl.to_end",{},"To end")} ${K(Math.max(0,M-this._pgHoverT))}`,`${this._t("lbl.power",{},"Power")} ${q(N)}`];i.font=`${9.5*n}px sans-serif`;const ht=Math.max(...ot.map(lt=>i.measureText(lt).width))+12*n,bt=ot.length*13*n+8*n;let pt=T+8*n;pt+ht>a-k&&(pt=T-ht-8*n);const yt=$+4*n;i.fillStyle=u,i.globalAlpha=.95,i.fillRect(pt,yt,ht,bt),i.globalAlpha=1,i.strokeStyle=_,i.lineWidth=n,i.strokeRect(pt,yt,ht,bt),i.fillStyle=p,i.textAlign="left",i.textBaseline="top",ot.forEach((lt,wt)=>i.fillText(lt,pt+6*n,yt+5*n+wt*13*n)),i.restore()}i.fillStyle=p,i.font=`${9*n}px sans-serif`,i.textAlign="center",i.textBaseline="top";const te=T=>`${Math.floor(T/60)}:${String(Math.round(T%60)).padStart(2,"0")}`;for(let T=0;T<=4;T++){const N=V+it*T/4,J=X(N);i.textAlign=T===0?"left":T===4?"right":"center",i.fillText(te(N),Math.max(S,Math.min(a-k,J)),P+g+1*n)}it<M-1&&(i.fillStyle=p,i.font=`${8*n}px sans-serif`,i.textAlign="right",i.textBaseline="top",i.fillText(this._t("lbl.pg_zoom_hint",{},"double-click to reset zoom"),a-k,$))}_pgEventMeta(t){const s={detected:["\u25CF","#42a5f5",this._t("lbl.pg_ev_detected",{},"Detected")],match_commit:["\u25C6","#66bb6a",this._t("lbl.pg_ev_match",{},"Match committed")],match_changed:["\u25C7","#eda100",this._t("lbl.pg_ev_match_changed",{},"Match changed")],match_ambiguous:["\u25C8","#eda100",this._t("lbl.pg_ev_ambiguous",{},"Ambiguous")],unmatched:["\u25CB","#9e9e9e",this._t("lbl.pg_ev_unmatched",{},"Unmatched")],notify_start:["\u{1F514}","#2a78d6",this._t("lbl.pg_ev_notify_start",{},"Start notification")],notify_pre_complete:["\u{1F514}","#2a78d6",this._t("lbl.pg_ev_notify_pre",{},"Almost-done notification")],notify_finish:["\u{1F514}","#2a78d6",this._t("lbl.pg_ev_notify_finish",{},"Finish notification")],notify_milestone:["\u{1F3C6}","#2a78d6",this._t("lbl.pg_ev_notify_milestone",{},"Milestone notification")],notify_held:["\u{1F319}","#7e57c2",this._t("lbl.pg_ev_notify_held",{},"Notification held (quiet hours)")],finished:["\u2713","#4caf50",this._t("lbl.pg_ev_finished",{},"Finished")]}[t]||["\u2022","var(--secondary-text-color)",t];return{glyph:s[0],color:s[1],label:s[2]}}_pgEventDescription(t){return{detected:this._t("pg_evd.detected",{},"The detector recognized a cycle had started."),match_commit:this._t("pg_evd.match_commit",{},"The matcher committed to a program with enough confidence."),match_changed:this._t("pg_evd.match_changed",{},"The leading program changed as more of the cycle was seen."),match_ambiguous:this._t("pg_evd.match_ambiguous",{},"Two programs scored close together, so the match was uncertain."),unmatched:this._t("pg_evd.unmatched",{},"No saved program fit this cycle."),notify_start:this._t("pg_evd.notify_start",{},"A start notification would be sent."),notify_pre_complete:this._t("pg_evd.notify_pre_complete",{},"An almost-done notification would be sent."),notify_finish:this._t("pg_evd.notify_finish",{},"A finish notification would be sent."),notify_milestone:this._t("pg_evd.notify_milestone",{},"A milestone notification would be sent."),notify_held:this._t("pg_evd.notify_held",{},"A notification was held back for quiet hours."),finished:this._t("pg_evd.finished",{},"The cycle reached a terminal state and ended.")}[t]||""}_pgUpdateParamInput(t,e){const s=this.shadowRoot,r=s&&s.querySelector(`input[data-pgkey="${t}"]`);r&&(r.dataset.pgtype==="bool"?r.checked=!!e:r.value=typeof e=="number"?Math.round(e):e)}_pgUpdateStripAt(t){const e=this._pgPowerPts;if(!e?.length)return;const r=(this._cycles||[]).find(M=>M.id===this._pgCycleId)?._pg_duration||e[e.length-1].t||1,n=t??r,a=this._pgInterpPower(e,n),o=this._pgSeriesAt(n),i=o?this._pgMapState(o.state):"idle",l={idle:[this._t("lbl.pg_idle",{},"Idle"),"var(--secondary-background-color)"],detecting:[this._t("lbl.pg_detecting",{},"Detecting"),"#42a5f5"],running:[this._t("lbl.pg_ev_running",{},"Running"),"#66bb6a"],ending:[this._t("lbl.pg_ev_ending",{},"Ending"),"#ef5350"],anti_wrinkle:[this._t("lbl.pg_anti_wrinkle",{},"Anti-wrinkle"),"#ab47bc"]},[c,_]=l[i]||l.idle,p=o&&o.progress!=null?Math.round(o.progress):null,u=o&&o.remaining_s!=null?o.remaining_s:null,g=o&&o.energy_wh!=null?o.energy_wh:this._pgTrapEnergy(e,n),b=o&&o.confidence!=null?Math.round(o.confidence*100)+"%":"\u2014",v=o&&o.phase?o.phase:"\u2014",S=M=>Math.floor(M/60)+":"+String(Math.round(M%60)).padStart(2,"0"),k=M=>M>=1e3?(M/1e3).toFixed(2)+" kWh":M.toFixed(0)+" Wh",$=M=>M>=1e3?(M/1e3).toFixed(1)+" kW":Math.round(M)+" W",C=this.shadowRoot,j=M=>C&&C.getElementById(M),z=(M,E)=>{const L=j(M);L&&(L.textContent=E)},I=(M,E,L)=>{const F=j(M);F&&(F.style[E]=L)},A=j("wd-pg-state-badge");A&&(A.textContent=c,A.style.background=_,A.style.color=_.includes("var")?"":"#fff"),z("wd-pg-power",$(a)),z("wd-pg-pct",p!=null?p+"%":"\u2014%"),I("wd-pg-pbar","width",(p??0)+"%"),z("wd-pg-rem",u!=null?S(u):"\u2014"),z("wd-pg-energy",k(g)),z("wd-pg-conf",b),z("wd-pg-phase",v)}_pgIsUnknownCmd(t){const e=t&&(t.code||t.error&&t.error.code)||"",s=(t&&(t.message||t.error)||"").toString().toLowerCase();return e==="unknown_command"||s.includes("unknown command")||s.includes("unknown_command")}_pgInterpPower(t,e){if(!t.length)return 0;if(e<=t[0].t)return t[0].w;for(let s=1;s<t.length;s++)if(e<=t[s].t){const r=t[s].t-t[s-1].t;if(r<=0)return t[s].w;const n=(e-t[s-1].t)/r;return t[s-1].w+n*(t[s].w-t[s-1].w)}return t[t.length-1].w}_pgTrapEnergy(t,e){let s=0;for(let r=1;r<t.length;r++){const n=t[r-1].t,a=t[r-1].w,o=Math.min(t[r].t,e),i=this._pgInterpPower(t,o);if(!(o<=n)&&(s+=(a+i)/2*(o-n)/3600,o>=e))break}return s}_drawPlaygroundCanvases(){this._tab==="playground"&&this._pgDrawCanvas()}_htmlPhases(){const t=this._devices[this._selIdx],e=t&&t.options.device_type||"washing_machine",s=this._canEdit(),r=this._phases.map(o=>{const i=o.is_default,l=o.translation_key?this._t(o.translation_key,{},o.description||""):o.description||"",c=s?`
        <td>
    ...
    </div>`:"",_=n.length?n.map(u=>{const g=t?`<button class="wd-btn wd-btn-danger wd-btn-sm" data-action="maint-delete" data-mid="${d(u.id)}">${this._t("btn.delete",{},"Delete")}</button>`:"",b=u.notes?`<div class="wd-info" style="margin-top:2px">${d(u.notes)}</div>`:"";return`<div class="wd-card" style="background:var(--secondary-background-color);padding:10px 12px">
      ...
      <div class="wd-form-grid">
        ${s.map(u=>`<div class="wd-field"><label>${d(this._maintLabel(u))}</label><input type="number" min="0" step="1" data-maint-rem="${d(u)}" value="${a[u]!=null?d(a[u]):""}" placeholder="${d(this._t("lbl.reminder_every",{},"Remind every (cycles)"))}"></div>`).join("")}
      </div>
    ...
    ${r||`<div class="wd-card"><p class="wd-info">${this._t("msg.no_other_users",{},"No other Home Assistant users found.")}</p></div>`}`}_htmlStore(){if(!this._canEdit())return`<div class="wd-empty">${this._t("msg.no_device_selected",{},"No device selected.")}</div>`;const t=this._storeStatus,e=`<a href="https://3dg1luk43.github.io/washdata-store" target="_blank" rel="noopener noreferrer" style="font-size:.8em;font-weight:400;color:var(--primary-color);text-decoration:none;white-space:nowrap" title="${d(this._t("store.website_tip",{},"Open the community store website"))}">${this._t("store.website",{},"Store website \u2197")}</a>`,s=`<div style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap"><div class="wd-card-title" style="margin:0">${this._t("hdr.community_store",{},"Community Store")}</div>${e}</div>`;if(!this._onlineEnabled()||t&&t.enabled===!1)return`<div class="wd-card">${s}
      ...
      ${Tt(`data-action="store-toggle-online" ${t?"checked":""} ${r?"disabled":""}`,this._t("lbl.enable_online",{},"Enable online features"))}
      ${t?this._htmlStorePrefs(r):""}
      ${t?n:""}
    </div>`}_htmlStorePrefs(t){const e=this._constants&&this._constants.storePrefs||{},s=ie.map(n=>{const a=e[n.key]!==!1;return Tt(`data-action="store-toggle-pref" data-pref="${d(n.key)}" ${a?"checked":""} ${t?"disabled":""}`,this._t(n.labelKey,{},n.labelFb),st(this._t(n.docKey,{},n.docFb)))}).join(""),r=this._busy.has("store-refresh-catalog");return s+`
      ...
      <div class="wd-log-drawer-body">
    ...
    </div>`}_drawCurves(t,e){const s=this.shadowRoot&&this.shadowRoot.getElementById(t);if(!s)return null;const r=s.getBoundingClientRect(),n=window.devicePixelRatio||1,a=Math.max(1,Math.round(r.width*n)),o=Math.max(1,Math.round((r.height||240)*n));(s.width!==a||s.height!==o)&&(s.width=a,s.height=o);const i=s.getContext("2d"),l=getComputedStyle(this),c=(l.getPropertyValue("--primary-color")||"#03a9f4").trim()||"#03a9f4",_=(l.getPropertyValue("--divider-color")||"rgba(127,127,127,.3)").trim()||"rgba(127,127,127,.3)",p=(l.getPropertyValue("--secondary-text-color")||"#888").trim()||"#888",u=44*n,g=12*n,b=12*n,v=22*n,S=e.series||[];let k=e.xMax||0;k||(S.forEach(E=>(E.points||[]).forEach(L=>{L[0]>k&&(k=L[0])})),e.band&&(e.band.max||[]).forEach(E=>{E[0]>k&&(k=E[0])})),k=k||1;let $=e.yMax||0;if(!$){const E=L=>(L||[]).forEach(F=>{F[1]>$&&($=F[1])});S.forEach(L=>{L.noScale||E(L.points)}),e.band&&E(e.band.max),$=($||10)*1.08}const C=this._canvasZoom&&this._canvasZoom[t],j=C?C.xMin:0,z=C?C.xMax:k,I=a-u-g,A=E=>u+(E-j)/(z-j)*I,M=E=>o-v-E/$*(o-b-v);i.clearRect(0,0,a,o),i.strokeStyle=_,i.lineWidth=n,i.fillStyle=p,i.font=`${11*n}px sans-serif`,i.textAlign="right",i.textBaseline="middle";for(let E=0;E<=2;E++){const L=b+E/2*(o-b-v);i.beginPath(),i.moveTo(u,L),i.lineTo(a-g,L),i.stroke(),i.fillText(Math.round($*(1-E/2))+"W",u-4*n,L)}if(i.save(),i.beginPath(),i.rect(u,b,I,o-b),i.clip(),(e.bands||[]).forEach(E=>{i.fillStyle=E.fill;const L=A(E.x0),F=A(E.x1);i.fillRect(Math.min(L,F),b,Math.abs(F-L),o-b-v)}),e.band&&(e.band.min||[]).length&&(e.band.max||[]).length){i.beginPath(),e.band.max.forEach((E,L)=>L?i.lineTo(A(E[0]),M(E[1])):i.moveTo(A(E[0]),M(E[1])));for(let E=e.band.min.length-1;E>=0;E--)i.lineTo(A(e.band.min[E][0]),M(e.band.min[E][1]));i.closePath(),i.fillStyle=e.band.fill||kt(c,.13),i.fill()}return S.forEach(E=>{const L=E.points||[];if(!L.length)return;const F=E.stroke==="primary"?c:E.stroke;if(E.fill){i.beginPath(),L.forEach((Y,it)=>it?i.lineTo(A(Y[0]),M(Y[1])):i.moveTo(A(Y[0]),M(Y[1]))),i.lineTo(A(L[L.length-1][0]),M(0)),i.lineTo(A(L[0][0]),M(0)),i.closePath();const V=i.createLinearGradient(0,b,0,o-v);V.addColorStop(0,kt(F,.33)),V.addColorStop(1,kt(F,.03)),i.fillStyle=V,i.fill()}i.beginPath(),L.forEach((V,Y)=>Y?i.lineTo(A(V[0]),M(V[1])):i.moveTo(A(V[0]),M(V[1]))),i.strokeStyle=F,i.lineWidth=(E.width||1.5)*n,i.lineJoin="round",E.dash&&i.setLineDash([6*n,4*n]),i.globalAlpha=E.alpha!=null?E.alpha:1,i.stroke(),i.globalAlpha=1,E.dash&&i.setLineDash([])}),i.textAlign="center",i.textBaseline="top",(e.vlines||[]).forEach(E=>{const L=A(E.x);i.beginPath(),i.moveTo(L,b),i.lineTo(L,o-v),i.strokeStyle=E.color,i.lineWidth=2*n,i.setLineDash([4*n,3*n]),i.stroke(),i.setLineDash([]),E.handle&&(i.fillStyle=E.color,i.beginPath(),i.arc(L,b+4*n,4.5*n,0,Math.PI*2),i.fill()),E.label&&(i.fillStyle=E.color,i.fillText(E.label,L,b+(E.handle?12*n:2*n)))}),i.restore(),i.fillStyle=p,i.font=`${11*n}px sans-serif`,i.textBaseline="bottom",C&&(i.textAlign="left",i.fillText((j/60).toFixed(1)+" min",u,o-2*n)),i.textAlign="right",i.fillText((z/60).toFixed(0)+" min",a-g,o-2*n),s._wd={xMax:k,xMin:j,xViewMax:z,yMax:$,dpr:n,padT:b,padB:v,ch:o,primary:c,Xpx:A,Ypx:M,xToCss:E=>A(E)/n,cssToX:E=>Math.max(j,Math.min(z,j+(E*n-u)/I*(z-j))),series:(e.series||[]).map(E=>({points:E.points,stroke:E.stroke,name:E.name,cid:E.cid})),band:e.band||null,artifacts:e.artifacts||null,_opts:e},s._wd}_drawModalCanvas(){const t=this._modal;t&&(t.type==="cycle-detail"?this._drawCycleEditor():t.type==="compare-cycles"?this._drawCompareCanvas():t.type==="profile-group"?this._drawGroupCanvas():t.type==="profile-panel"&&(t.tab==="stats"?this._drawProfileEnvelope():t.tab==="phases"?this._drawPhaseEditor():t.tab==="cleanup"&&this._drawSpaghetti()))}_redrawCanvas(t){if(t==="wd-status-canvas")this._drawStatusCurve();else if(t==="wd-cyc-canvas")this._drawCycleEditor();else if(t==="wd-compare-canvas")this._drawCompareCanvas();else if(t==="wd-env-canvas")this._drawProfileEnvelope();else if(t==="wd-phase-canvas")this._drawPhaseEditor();else if(t==="wd-spag-canvas")this._drawSpaghetti();else if(t==="wd-pgroup-canvas")this._drawGroupCanvas();else{const e=this.shadowRoot&&this.shadowRoot.getElementById(t);e&&e._wd&&e._wd._opts&&this._drawCurves(t,e._wd._opts)}}_pref(t,e){const s=this._panelCfg&&this._panelCfg.prefs||{};return s[t]===void 0?e:s[t]}_setPref(t,e){this._panelCfg||(this._panelCfg={}),this._panelCfg.prefs={...this._panelCfg.prefs||{},[t]:e},this._ws({type:`${y}/set_user_prefs`,prefs:{[t]:e}}).catch(()=>{})}_drawStatusCurve(){const t=this._powerData||{},e=t.live||[];if(e.length<2)return;const s=this._statusEnv,r=this._pref("show_expected",!0),n=this._pref("show_raw_active",!1),a=[];let o=e[e.length-1][0];if(t.cycle_active&&s&&(s.avg||[]).length&&r){const l=s.target_duration||s.avg[s.avg.length-1][0];a.push({points:s.avg,stroke:"#ff9800",width:2,alpha:.4,name:this._t("lbl.expected",{},"Expected")}),o=Math.max(o,l)}a.push({points:e,stroke:"primary",fill:!0,width:2,name:this._t("lbl.power",{},"Power")}),n&&(t.raw||[]).length>1&&a.push({points:t.raw,stroke:"#9e9e9e",width:1,alpha:.65,name:this._t("lbl.raw_socket",{},"Raw socket"),noScale:!0});const i=[];(t.restart_gaps||[]).forEach(l=>{if(!e.length)return;const c=t.cycle_start_iso;if(!c)return;const _=new Date(c).getTime(),p=Math.max(0,(new Date(l.start_ts).getTime()-_)/1e3),u=Math.max(p+1,(new Date(l.end_ts).getTime()-_)/1e3);i.push({x0:p,x1:u,fill:"rgba(96,125,139,.20)"})}),this._drawCurves("wd-status-canvas",{series:a,xMax:o,bands:i})}_attachHover(t){const e=this.shadowRoot,s=e&&e.getElementById(t);s&&(s.addEventListener("pointermove",r=>this._onGraphHover(r,t)),s.addEventListener("pointerleave",()=>this._hideGraphTip()),s.addEventListener("wheel",r=>{const n=s._wd;if(!n)return;r.preventDefault();const a=s.getBoundingClientRect(),o=n.cssToX(r.clientX-a.left),i=this._canvasZoom[t],l=n.xMax,c=i?i.xMin:0,p=(i?i.xMax:l)-c,u=p*(r.deltaY>0?1.3:.75);if(u>=l*.99)delete this._canvasZoom[t];else{const g=(o-c)/p,b=Math.min(l,o-g*u+u),v=Math.max(0,b-u);this._canvasZoom[t]={xMin:v,xMax:Math.min(l,v+u)}}this._redrawCanvas(t)},{passive:!1}),s.addEventListener("dblclick",()=>{delete this._canvasZoom[t],this._redrawCanvas(t)}))}_onGraphHover(t,e){const s=t.clientX,r=t.clientY;if(this._hoverRafId){this._hoverPending={px:s,py:r,id:e};return}this._hoverPending={px:s,py:r,id:e},this._hoverRafId=requestAnimationFrame(()=>{this._hoverRafId=null;const{px:n,py:a,id:o}=this._hoverPending||{};o&&this._onGraphHoverInner(n,a,o)})}_onGraphHoverInner(t,e,s){const r=this.shadowRoot&&this.shadowRoot.getElementById(s),n=r&&r._wd;if(!n)return;const a=r.getBoundingClientRect(),o=n.cssToX(t-a.left),i=(e-a.top)*n.dpr;this._redrawCanvas(s);const l=r.getContext("2d"),c=n.Xpx(o);l.save(),l.strokeStyle="rgba(140,140,140,.75)",l.lineWidth=n.dpr,l.setLineDash([3*n.dpr,3*n.dpr]),l.beginPath(),l.moveTo(c,n.padT),l.lineTo(c,n.ch-n.padB),l.stroke(),l.setLineDash([]);const _=b=>b.stroke==="primary"?n.primary:b.stroke,p=(b,v)=>{l.fillStyle=v,l.beginPath(),l.arc(c,n.Ypx(b),3.4*n.dpr,0,6.2832),l.fill()},u=[`${this._t("lbl.from_start",{},"From start")}: <b>${Mt(o)}</b>`,`${this._t("lbl.to_end",{},"To end")}: <b>${Mt(Math.max(0,n.xMax-o))}</b>`],g=n.series||[];if(this._hoverNearest=null,g.length>4){let b=null,v=1/0;if(g.forEach(S=>{const k=Dt(S.points,o);if(k==null)return;const $=Math.abs(n.Ypx(k)-i);$<v&&(v=$,b={s:S,v:k})}),b){const S=_(b.s);l.strokeStyle=S,l.lineWidth=3*n.dpr,l.beginPath(),(b.s.points||[]).forEach((k,$)=>$?l.lineTo(n.Xpx(k[0]),n.Ypx(k[1])):l.moveTo(n.Xpx(k[0]),n.Ypx(k[1]))),l.stroke(),p(b.v,S),u.push(`${d(b.s.name||"")}: <b>${b.v.toFixed(b.v<100?1:0)} W</b>`),b.s.cid&&(u.push(`<span style="opacity:.7">${this._t("lbl.click_to_select",{},"click to select")}</span>`),this._hoverNearest={id:s,cid:b.s.cid})}}else g.forEach(b=>{const v=Dt(b.points,o);v!=null&&(p(v,_(b)),u.push(`${d(b.name||this._t("lbl.power",{},"Power"))}: <b>${v.toFixed(v<100?1:0)} W</b>`))});if(n.band){const b=Dt(n.band.min,o),v=Dt(n.band.max,o);b!=null&&v!=null&&u.push(`${this._t("lbl.envelope",{},"Envelope")}: ${b.toFixed(0)}\u2013${v.toFixed(0)} W`)}(n.artifacts||[]).forEach(b=>{if(o>=b.start_s&&o<=b.end_s){const v=b.detail_key?this._t(b.detail_key,b.detail_params||{},b.detail||""):b.detail||"";u.push(`<span style="color:var(--warning-color,#ff9800)">\u26A0 ${d(Yt(b.type,(S,k,$)=>this._t(S,k,$)))}</span>: ${d(v)}`)}}),this._canvasZoom[s]&&u.push(`<span style="opacity:.45">${this._t("lbl.zoom_hint",{},"scroll to zoom \xB7 dblclick to reset")}</span>`),l.restore(),this._showGraphTip(t,e,u),this._syncSpagRowHighlight(this._hoverNearest?this._hoverNearest.cid:null)}_showGraphTip(t,e,s){const r=this._gtip;if(!r)return;r.innerHTML=s.join("<br>"),r.style.display="block";const n=r.offsetWidth,a=r.offsetHeight,o=16;let i=t+o,l=e+o;i+n>window.innerWidth-6&&(i=t-n-o),l+a>window.innerHeight-6&&(l=e-a-o),r.style.left=Math.max(6,i)+"px",r.style.top=Math.max(6,l)+"px"}_hideGraphTip(){this._hoverRafId&&(cancelAnimationFrame(this._hoverRafId),this._hoverRafId=null,this._hoverPending=null),this._gtip&&(this._gtip.style.display="none"),this._syncSpagRowHighlight(null)}_positionTip(t){const e=t.querySelector(".wd-tip-pop");if(!e)return;e.style.transform="";const s=!e.offsetWidth,r=s?e.style.cssText:"";s&&(e.style.visibility="hidden",e.style.display="block");const n=e.getBoundingClientRect();if(s&&(e.style.cssText=r),!n.width)return;const a=de(t),o=6,i=a.left+o,l=a.right-o-n.width,c=l<i?i:Math.min(Math.max(n.left,i),l),_=Math.round(c-n.left);_&&(e.style.transform=`translateX(calc(-50% + ${_}px))`)}_syncSpagRowHighlight(t){if(t===this._spagHoverCid)return;this._spagHoverCid=t;const e=this.shadowRoot;e&&e.querySelectorAll("tr[data-cid]").forEach(s=>{s.style.backgroundColor=t&&s.dataset.cid===t?"var(--secondary-background-color,rgba(0,0,0,.06))":""})}_showToast(t,e="success",s={}){this._toastTimer&&clearTimeout(this._toastTimer);const r=s.duration||3500;this._toast={msg:t,cls:`wd-toast-${e}`,actionLabel:s.actionLabel||null,actionToken:s.actionToken||null},this._render(),this._toastTimer=setTimeout(()=>{this._toast=null,this._render()},r)}_profileOptions(t){return(this._profiles||[]).map(e=>`<option value="${d(e.name)}" ${String(t)===String(e.name)?"selected":""}>${d(e.name)}</option>`).join("")}_htmlModal(){const t=this._modal;if(t.type==="cycle-detail")return`<div class="wd-overlay"><div class="wd-modal wd-modal-lg" role="dialog" aria-modal="true" aria-labelledby="wd-modal-title" tabindex="-1">${this._htmlCycleModal(t)}</div></div>`;if(t.type==="profile-panel")return`<div class="wd-overlay"><div class="wd-modal wd-modal-lg" role="dialog" aria-modal="true" aria-labelledby="wd-modal-title" tabindex="-1">${this._htmlProfilePanel(t)}</div></div>`;if(t.type==="profile-group")return`<div class="wd-overlay"><div class="wd-modal wd-modal-lg" role="dialog" aria-modal="true" aria-labelledby="wd-modal-title" tabindex="-1">${this._htmlProfileGroupModal(t)}</div></div>`;if(t.type==="compare-cycles")return`<div class="wd-overlay"><div class="wd-modal wd-modal-lg" role="dialog" aria-modal="true" aria-labelledby="wd-modal-title" tabindex="-1">${this._htmlCompareModal(t)}</div></div>`;if(t.type==="gear-settings")return`<div class="wd-overlay"><div class="wd-modal wd-modal-lg" role="dialog" aria-modal="true" aria-labelledby="wd-modal-title" tabindex="-1">${this._htmlGearModal(t)}</div></div>`;if(t.type==="export-select")return`<div class="wd-overlay"><div class="wd-modal wd-modal-lg" role="dialog" aria-modal="true" aria-labelledby="wd-modal-title" tabindex="-1">${this._htmlExportSelectModal(t)}</div></div>`;if(t.type==="import-wizard")return`<div class="wd-overlay"><div class="wd-modal wd-modal-lg" role="dialog" aria-modal="true" aria-labelledby="wd-modal-title" tabindex="-1">${this._htmlImportWizardModal(t)}</div></div>`;let e="";if(t.type==="confirm")e=`<h2>${d(t.title)}</h2><p class="wd-info">${d(t.message)}</p>
        ...
        <div class="wd-modal-actions"><button class="wd-btn wd-btn-secondary" data-maction="cancel">${this._t("btn.cancel",{},"Cancel")}</button>
        <button class="wd-btn wd-btn-primary" data-maction="label-ok">${this._t("btn.apply_label",{},"Apply Label")}</button></div>`;else if(t.type==="create-profile"){const s=(this._cycles||[]).slice(0,40).map(r=>`<option value="${d(r.id)}">${et(r.start_time)} - ${Math.round((r.duration||0)/60)}m - ${d(r.profile_name||this._t("lbl.unlabelled",{},"Unlabelled"))}</option>`).join("");e=`<h2>${this._t("modal.create_profile",{},"Create Profile")}</h2>
        ...
        <button class="wd-btn wd-btn-primary" data-maction="store-share-ok" ${this._busy.has("store-share")?"disabled":""}>${this._busy.has("store-share")?'<span class="wd-spin"></span> ':""}${this._t("btn.share",{},"Share")}</button></div>`}else t.type==="store-share-device"&&(e=this._htmlShareDeviceModal(t));return`<div class="wd-overlay"><div class="wd-modal" role="dialog" aria-modal="true" aria-labelledby="wd-modal-title" tabindex="-1">${e}</div></div>`}_htmlShareDeviceModal(t){const e=this._shareableByProgram(),s=this._busy.has("store-share-device"),r=t.selected||new Set,n=!!t.consented,a=e.reduce((g,b)=>g+b.cycles.filter(v=>r.has(v.id)).length,0),o=e.some(g=>!g.noCycles);let i;t.loading?i=`<div class="wd-empty" style="padding:24px"><div class="wd-icon">\u23F3</div>${this._t("msg.loading",{},"Loading\u2026")}</div>`:e.length?i=e.map(g=>{if(g.noCycles)return`<div class="wd-sd-group wd-sd-group-nocyc">
          ...
          </div>`;const b=g.cycles.every(C=>r.has(C.id)),v=!b&&g.cycles.some(C=>r.has(C.id)),S=g.cycles.map(C=>{const j=C.start_time?et(C.start_time):"",z=C.duration!=null?dt(C.duration):"";return`<label class="wd-sd-cyc">
            <input type="checkbox" data-maction="sd-toggle-cyc" data-cid="${d(C.id)}" ${r.has(C.id)?"checked":""} ${s?"disabled":""}>
            <span class="wd-sd-cyc-meta">${d(j)} \xB7 ${d(z)} \u2B50</span>
          ...
          <label class="wd-sd-prof">
            <input type="checkbox" data-maction="sd-toggle-prof" data-prog="${d(g.program)}" ${b?"checked":""} ${v?'data-indeterminate="1"':""} ${s?"disabled":""}>
            <span class="wd-sd-prof-name">${d(g.program)}</span>
            <span class="wd-sd-count">${g.cycles.filter(C=>r.has(C.id)).length}/${g.cycles.length}</span>
          </label>
      ...
      <span class="wd-sd-prof-name"><strong>${this._t("btn.select_all",{},"Select all")}</strong></span>
    </label>`,c=a.map(_=>{const p=r[_],u=s.importableOnly&&p.importable===!1,g=this._wizCatState(t,_,e),v=["profiles","real_cycles","reference_cycles"].includes(_)?`${g.sel}/${p.count}`:"",S=`<label class="wd-sd-prof">
      ...
      </label>`;let k="";if(!u&&_==="profiles")k=`<div class="wd-sd-cycles">${(p.items||[]).map($=>`<label class="wd-sd-cyc">
          ...
          <span class="wd-sd-cyc-meta">${d($.name)} \xB7 ${($.real_cycles||0)+($.reference_cycles||0)} ${this._t("lbl.cycles_short",{},"cycles")}${n.has($.name)?` <span class="wd-tag" style="color:var(--warning-color,#e6a700)">${this._t("badge.exists",{},"exists")}</span>`:""}</span>
        </label>`).join("")}</div>`;else if(!u&&(_==="real_cycles"||_==="reference_cycles")){const $=_==="real_cycles"?t.sel.realIds:t.sel.refIds;k=`<div class="wd-sd-cycles">${(p.groups||[]).map(C=>{const j=C.cycles.map(F=>String(F.id)),z=j.filter(F=>$.has(F)).length,I=z===j.length&&j.length>0,A=z>0&&!I,M=`${_}:${C.profile}`,E=t.expanded&&t.expanded.has(M),L=E?C.cycles.map(F=>`<label class="wd-sd-cyc" style="margin-left:18px">
            <input type="checkbox" data-maction="wiz-toggle-cyc" data-cat="${_}" data-cid="${d(String(F.id))}" ${$.has(String(F.id))?"checked":""}>
            ...
            <div class="wd-sd-cyc" style="display:flex;align-items:center">
              <label style="flex:1;display:flex;align-items:center;gap:8px;cursor:pointer">
                <input type="checkbox" data-maction="wiz-toggle-cycgroup" data-cat="${_}" data-prof="${d(C.profile)}" ${I?"checked":""} ${A?'data-indeterminate="1"':""}>
                <span class="wd-sd-cyc-meta"><strong>${d(C.profile||this._t("lbl.unlabelled",{},"Unlabelled"))}</strong> (${z}/${C.count})</span>
              </label>
      ...
      </div>`}_htmlImportWizardModal(t){const e=this._busy.has("import-wizard"),s=`<h2 id="wd-modal-title">${this._t("modal.import_wizard",{},"Import - choose data")}</h2>`;if(t.step==="input"||t.step==="analyze"){const S=t.step==="analyze";return`${s}
    ...
    </div>`;let u="";const b=(r.categories&&r.categories.profiles&&r.categories.profiles.items||[]).filter(S=>S.conflict&&t.sel.profiles.has(S.name));t.mode==="merge"&&b.length&&(u=`<div class="wd-field"><label>${this._t("lbl.conflict_resolution",{},"Name clashes")}</label>
        ${b.map(S=>`<div style="display:flex;align-items:center;gap:8px;margin:4px 0">
      ...
      </div>`);const v=t.sel&&(t.sel.profiles.size||t.sel.realIds.size||t.sel.refIds.size||t.sel.cats.size);return`${s}
      ${i}
      ${l}
      ${c}
      ${p}
      ...
      <div class="wd-modal-actions">
        ...
        <div class="wd-modal-actions"><button class="wd-btn wd-btn-secondary" data-maction="cancel">${this._t("btn.close",{},"Close")}</button></div>`;const e=t.curve||{},s=!!e.is_reference,r=e.full_duration_s||e.duration||0,n=e.energy_kwh!=null?e.energy_kwh:null,a=t.ml||null;let o="";if(a&&a.ml_quality_score!=null){const I=a.ml_quality_label,A=I==="ok"?"var(--success-color,#4caf50)":I==="uncertain"?"var(--warning-color,#ff9800)":"var(--error-color,#f44336)",M=Math.round((1-a.ml_quality_score)*100);o=`<div class="wd-kv-item"><div class="wd-kv-val" style="font-size:.95em;color:${A}">${M}%</div><div class="wd-kv-lbl">${this._t("lbl.cycle_health",{},"Cycle health")}</div></div>`}const i=`<div class="wd-kv">
      <div class="wd-kv-item"><div class="wd-kv-val">${dt(e.duration||r)}</div><div class="wd-kv-lbl">${this._t("lbl.duration",{},"Duration")}</div></div>
      ...
      ${o}
    </div>`,l=a&&a.ml_review||{},c=(this._feedbacks||[]).some(I=>I.cycle_id===t.cycleId),_=a&&a.ml_quality_label,p=!l.reviewed_at&&(c||["uncertain","review"].includes(_)||["force_stopped","interrupted"].includes(e.status)),u=p&&t.mode!=="review"?` <span title="${this._t("hdr.automation_needs_review",{},"This cycle needs review")}" style="color:var(--warning-color,#ff9800);font-size:1.1em;line-height:0">\u25CF</span>`:"",g=this._canEdit()&&!s?`<div class="wd-mode-bar">
      ...
      <button class="wd-btn wd-btn-sm ${t.mode==="review"?"wd-btn-primary":"wd-btn-secondary"}" data-maction="cyc-review" title="${p?this._t("hdr.automation_needs_review",{},"This cycle needs review"):this._t("hdr.automation_review_this_cycle",{},"Review this cycle")}">${this._t("btn.review",{},"Review")}${u}</button>
    </div>`:s?`<div class="wd-info" style="margin:0 0 8px"><span style="color:var(--info-color,#2196f3)">\u{1F4E5}</span> ${this._t("msg.imported_readonly",{},"Imported from the community store. Shown for reference and matching. It is not counted in your stats and cannot be edited.")}</div>`:"",b=this._canEdit()?(this._feedbacks||[]).find(I=>I.cycle_id===t.cycleId):null,v=b?b.detected_profile||b.profile_name||this._t("lbl.unknown",{},"Unknown"):"",S=b?`
        ...
        </div>`}else if(t.mode==="split"){const I=this._busy.has("cyc-split-apply"),A=(t.split.offsets||[]).slice().sort((L,F)=>L-F),M=[0,...A,r],E=M.slice(0,-1).map((L,F)=>{const V=M[F+1];return`<div class="wd-seg-row"><span class="wd-swatch" style="background:${Q[F%Q.length]}"></span>
      ...
      </div>`}let j="";const z=t.mode==="view"||t.mode==="review"?e.restart_gaps||[]:[];if(z.length){const I=z.map(A=>{const M=Math.round((A.gap_seconds||0)/60),E=M>=1?`${M}m`:`${Math.round(A.gap_seconds||0)}s`,L=A.match_confidence!=null?` \xB7 ${this._t("lbl.pct_match_confidence",{pct:Math.round(A.match_confidence*100)},`${Math.round(A.match_confidence*100)}% match confidence`)}`:"",F=A.profile?` (${d(A.profile)})`:"";return`<li>${this._t("msg.restart_gap_item",{dur:E},`${E} gap`)}: ${this._t("lbl.ha_restarted",{},"HA restarted")}${F}${L}</li>`}).join("");j=`<div class="wd-card" style="margin:10px 0 0;padding:10px 12px;border-left:3px solid var(--info-color,#2196f3)">
        <div style="font-weight:600;font-size:.9em">\u21BB ${this._t("msg.restart_gap_header",{n:z.length},`${z.length} HA restart gap${z.length>1?"s":""} during this cycle`)}</div>
        <ul class="wd-info" style="margin:4px 0 0;padding-left:18px;font-size:.82em">${I}</ul>
      ...
      ${$}
      ${j}
      ${k}`}_htmlProfilePanel(t){const e=this._canEdit();t.tab==="danger"&&!e&&(t.tab="stats");const s=[["stats",this._t("tab.pp_overview",{},"Overview")],["phases",this._t("tab.pp_phases",{},"Phases")],["cleanup",this._t("tab.pp_cleanup",{},"Cleanup")]];e&&s.push(["danger",this._t("tab.pp_manage",{},"Manage")]);const r=s.map(([o,i])=>`<button class="wd-mini-tab ${t.tab===o?"active":""}" data-maction="pp-tab-${o}">${i}</button>`).join("");let n="";if(!t.loaded)n=`<div class="wd-empty" style="padding:32px"><div class="wd-icon">\u23F3</div>${this._t("msg.loading",{},"Loading\u2026")}</div>`;else if(t.tab==="stats"){const o=t.stats||{},i=t.env||{},l=this._hass&&this._hass.config&&this._hass.config.currency||"",c=o.avg_energy!=null&&o.cycle_count?o.avg_energy*o.cycle_count:null,_=v=>v?Math.round(v/60)+"m":"-",p=(this._profileHealth||{})[t.name],u=(this._profileTrends||{})[t.name],g=p&&p.health_status!=="unknown"?(()=>{const v={healthy:["var(--success-color,#4caf50)","rgba(76,175,80,.12)"],fair:["var(--warning-color,#ff9800)","rgba(255,152,0,.12)"],poor:["var(--error-color,#f44336)","rgba(244,67,54,.12)"]},[S,k]=v[p.health_status]||v.fair,$=Math.round((p.health_score||0)*100),C=p.duration_cv!=null?` \xB7 ${this._t("stat.duration_cv",{pct:Math.round(p.duration_cv*100)},`duration CV ${Math.round(p.duration_cv*100)}%`)}`:"",j=p.confidence_mean!=null?` \xB7 ${this._t("stat.avg_confidence",{pct:Math.round(p.confidence_mean*100)},`avg confidence ${Math.round(p.confidence_mean*100)}%`)}`:"";return`<div style="margin:8px 0 4px;padding:8px 12px;border-radius:6px;background:${k};border:1px solid color-mix(in srgb, ${S} 13%, transparent);display:flex;align-items:center;gap:8px;flex-wrap:wrap">
          <span style="font-weight:600;color:${S}">${p.health_status==="poor"?this._t("health.poor",{},"\u26A0 Poor match fit"):p.health_status==="fair"?this._t("health.fair",{},"Fair match fit"):this._t("health.good",{},"\u2713 Good match fit")}</span>
          ...
          <div class="wd-sg">
            ...
            <div class="wd-sg-main">${_t(o.avg_energy)}<span>${this._t("stat.avg",{},"avg")}</span></div>
          ...
          <div class="wd-sg">
            ...
            <div class="wd-sg-sub">${this._t("stat.last_run",{v:o.last_run?et(o.last_run):"-"},`last run ${o.last_run?et(o.last_run):"-"}`)}</div>
          </div>
        </div>
        ${g}
        ${b}
        ${p&&p.shape_drift?(()=>{const v=p.shape_drift_correlation!=null?` (r=${Number(p.shape_drift_correlation).toFixed(2)})`:"";return`<div style="margin-top:8px;padding:8px 10px;background:color-mix(in srgb, var(--warning-color,#ff9800) 9%, transparent);border-radius:6px;border-left:3px solid var(--warning-color,#ff9800)">
        ...
        ${e?`<div class="wd-mode-bar">
          ...
          <td style="width:26px;padding:6px 4px"><input type="checkbox" data-cleanidx="${k}" ${i.has(v.cycle_id)?"checked":""}></td>
        ...
        ${e?"<th></th>":""}
      </tr></thead>`,b=this._busy.has("pp-cleanup-del");n=`<p class="wd-info" style="margin-bottom:10px">${this._t("msg.cleanup_intro",{},"Every labelled cycle overlaid. Tick outliers and delete to clean up the profile.")}</p>
