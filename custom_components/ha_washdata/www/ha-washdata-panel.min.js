...
`;function _t(h){if(h==null||h<0)return"-";const t=Math.floor(h/3600),e=Math.floor(h%3600/60),s=Math.floor(h%60);return t>0?`${t}h ${e}m`:e>0?`${e}m ${s}s`:`${s}s`}function Jt(h){return h==null?"-":h>=100?`${Math.round(h)} W`:`${h.toFixed(1)} W`}function yt(h){return h==null?"-":`${h.toFixed(2)} kWh`}let Zt="relative";const fe=3700;function Qt(h){const t=e=>String(e).padStart(2,"0");return`${h.getFullYear()}-${t(h.getMonth()+1)}-${t(h.getDate())}`}function te(h){const t=new Date;return t.setDate(t.getDate()-h),Qt(t)}function me(){return Qt(new Date)}function Wt(){return te(10)}function ye(){return te(fe-1)}function we(h){const t=Math.round((h-Date.now())/1e3);let e;try{e=new Intl.RelativeTimeFormat(void 0,{numeric:"auto"})}catch{return ee(h)}const s=Math.abs(t),r=[["year",31536e3],["month",2592e3],["week",604800],["day",86400],["hour",3600],["minute",60]];for(const[n,a]of r)if(s>=a)return e.format(Math.round(t/a),n);return e.format(t,"second")}function ee(h){return new Date(h).toLocaleString(void 0,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}function rt(h,t){if(!h)return"-";let e;if(typeof h=="number")e=h>=1e12?h:h*1e3;else{const s=String(h),r=/^(\d{4})-(\d{2})-(\d{2})$/.exec(s);e=r?new Date(+r[1],+r[2]-1,+r[3]).getTime():new Date(s).getTime()}return isNaN(e)?"-":(t||Zt)==="relative"?we(e):ee(e)}function Lt(h){if(h==null)return"unknown error";if(h instanceof Error)return`${h.name}: ${h.message}`;const t=h.code!=null?String(h.code):"",e=h.message!=null?String(h.message):"";if(!t&&!e)try{return JSON.stringify(h)}catch{return String(h)}const s=t==="unknown_command"?" (the integration is probably still starting up; this should stop on its own)":"";return`${t||"error"}${e?": "+e:""}${s}`}function d(h){return String(h??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function be(h){const t=String(h??"").trim();return/^https?:\/\//i.test(t)?t:""}function Pt(h,t){const e=String(h??"").trim();let s=e.match(/^#([0-9a-fA-F]{3,8})$/);if(s){let r=s[1];if((r.length===3||r.length===4)&&(r=r.split("").map(n=>n+n).join("")),r.length===6||r.length===8){const n=parseInt(r.slice(0,2),16),a=parseInt(r.slice(2,4),16),o=parseInt(r.slice(4,6),16);return`rgba(${n}, ${a}, ${o}, ${t})`}}if(s=e.match(/^rgba?\(([^)]+)\)$/i),s){const r=s[1].split(",").map(n=>n.trim());if(r.length>=3)return`rgba(${r[0]}, ${r[1]}, ${r[2]}, ${t})`}return e}function se(h,t){const e=parseFloat(h);return isNaN(e)?t:e}function jt(h){return h?Array.from(h.querySelectorAll('a[href],button:not([disabled]),input:not([disabled]),select:not([disabled]),textarea:not([disabled]),[tabindex]:not([tabindex="-1"])')).filter(e=>e.getClientRects().length>0):[]}function At(h){return h==null||h===""?"(none)":h===!0?"on":h===!1?"off":Array.isArray(h)?h.join(", ")||"(none)":String(h)}function ie(h,t,e){return h.slice().sort((s,r)=>{const n=t(s),a=t(r);return n==null&&a==null?0:n==null?1:a==null?-1:(n<a?-1:n>a?1:0)*e})}function vt(h,t,e,s,r,n,a){const o=e?s===1?" \u25B2":" \u25BC":' <span style="opacity:.35">\u2195</span>',i=n==="right"?"text-align:right;":"",l=a?` title="${d(a)}"`:"";return`<th class="wd-th-sort" data-sortcol="${t}" data-sortact="${r}" style="cursor:pointer;user-select:none;${i}"${l}>${h}${o}</th>`}function zt(h){h=Math.max(0,Math.round(h));const t=Math.floor(h/3600),e=Math.floor(h%3600/60),s=h%60;return t>0?`${t}:${String(e).padStart(2,"0")}:${String(s).padStart(2,"0")}`:`${e}:${String(s).padStart(2,"0")}`}function ae(h,t){const e={pause:["lbl.artifact_interruption","Interruption"],dip:["lbl.artifact_low_power","Low power"],spike:["lbl.artifact_high_power","High power"]},[s,r]=e[h]||["lbl.artifact_anomaly","Anomaly"];return t?t(s,{},r):r}function ne(h){return h.toLowerCase().replace(/[\s&/\-]+/g,"_").replace(/^_+|_+$/g,"").replace(/_+/g,"_")}function re(h){let t=String(h??"").trim().toLowerCase();try{t=t.normalize("NFKD").replace(/[\u0300-\u036f]/g,"")}catch{}return t.replace(/[^\p{L}\p{N}]+/gu,"-").replace(/^-+|-+$/g,"")}function oe(h){const t=new Set;return String(h??"").split(",").forEach(e=>{const s=parseInt(e.trim(),10);Number.isFinite(s)&&s>0&&t.add(s)}),Array.from(t).sort((e,s)=>e-s)}function Bt(h,t){if(!h||!h.length)return null;if(t<=h[0][0])return h[0][1];if(t>=h[h.length-1][0])return h[h.length-1][1];for(let e=1;e<h.length;e++)if(h[e][0]>=t){const s=h[e-1],r=h[e],n=r[0]-s[0]||1;return s[1]+(r[1]-s[1])*((t-s[0])/n)}return h[h.length-1][1]}function ve(h,t,e){e=e||{};const s=h.key,r=h.unit?`${h.label} (${h.unit})`:h.label,n=h.unit?` ${h.unit}`:"",a=h.doc?ct(h.doc,h.diagram||xe[s]):"",o=e.changed?`<span class="wd-chg-dot" title="${d(e.changed)}" aria-label="${d(e.changed)}"></span>`:"";if(h.type==="checkbox")return`<div class="wd-field wd-field-switch"><div class="wd-switch-row"><label class="wd-switch-lbl"><span class="wd-switch"><input type="checkbox" data-opt="${s}" ${t?"checked":""}><span class="wd-switch-slider"></span></span><span class="wd-switch-text">${d(h.label)}</span></label>${o}${a}</div>${h.hint?`<div class="wd-field-hint">${d(h.hint)}</div>`:""}</div>`;let i="";const l=t??"";if(h.type==="select"||h.type==="devicetype"||h.type==="device"){const T=(e.opts||[]).map(([I,D])=>`<option value="${d(I)}" ${String(l)===String(I)?"selected":""}>${d(D)}</option>`).join("");i=`<select data-opt="${s}" data-ftype="${h.type}">${T}</select>`}else if(h.type==="textarea")i=`<textarea data-opt="${s}" data-ftype="textarea">${d(l)}</textarea>`;else if(h.type==="json"){const v=l===""||l==null?"":typeof l=="string"?l:JSON.stringify(l,null,2);i=`<textarea data-opt="${s}" data-ftype="json" rows="3" placeholder='${d(e.t("placeholder.json_buttons",{},'[{"action":"ID","title":"Label"}]'))}'>${d(v)}</textarea>`}else if(h.type==="checkboxlist"){const v=Array.isArray(t)&&t.length?t.map(String):Array.isArray(h.def)?h.def.map(String):[];i=`<div data-opt="${s}" data-ftype="checkboxlist" class="wd-checkboxlist">`+(h.choices||[]).map(([T,I])=>`<label class="wd-switch-lbl" style="display:flex;align-items:center;gap:8px;margin:2px 0"><span class="wd-switch"><input type="checkbox" data-choice="${d(T)}" ${v.includes(String(T))?"checked":""}><span class="wd-switch-slider"></span></span><span class="wd-switch-text">${d(e.t?e.t("lbl.evidence_"+T,{},I):I)}</span></label>`).join("")+"</div>"}else if(h.type==="entitylist"){const T=(Array.isArray(t)?t:t?[t]:[]).map(I=>`<span class="wd-pill" data-val="${d(I)}">${d(I)}<button type="button" class="wd-pill-x" aria-label="${d(e.t?e.t("btn.remove",{},"Remove"):"Remove")}">\xD7</button></span>`).join("");i=`<div class="wd-pillbox" data-opt="${s}" data-ftype="entitylist">${T}<div class="wd-combo wd-combo-pill"><input type="text" class="wd-pill-add" autocomplete="off" spellcheck="false" placeholder="${d(e.t("placeholder."+(h.domain||"add"),{},h.placeholder||"add\u2026"))}"><div class="wd-combo-drop" hidden></div></div></div>`}else if(h.type==="timerlist"){const v=Array.isArray(l)?l:[],T=e.t?e.t("lbl.timer_min",{},"min"):"min",I=e.t?e.t("lbl.timer_msg_placeholder",{},"Message (optional, {device}/{program}/{minutes})"):"Message (optional, {device}/{program}/{minutes})",D=e.t?e.t("lbl.timer_auto_pause",{},"Auto-pause"):"Auto-pause",F=e.t?e.t("btn.remove_timer",{},"Delete"):"Delete",B=e.t?e.t("btn.add_timer",{},"+ Add timer"):"+ Add timer",R=(A,M)=>{const O=A&&A.offset_minutes?String(A.offset_minutes):"",G=A&&A.message?d(A.message):"",et=A&&A.auto_pause?" checked":"";return`<div class="wd-timer-row" data-tidx="${M}"><div class="wd-timer-top"><input type="number" min="1" placeholder="${d(T)}" data-field="offset_minutes" value="${d(O)}"><textarea placeholder="${d(I)}" data-field="message">${G}</textarea></div><div class="wd-timer-footer"><label class="wd-switch-lbl"><span class="wd-switch"><input type="checkbox" data-field="auto_pause"${et}><span class="wd-switch-slider"></span></span><span class="wd-switch-text">${d(D)}</span></label><button type="button" class="wd-btn wd-btn-sm wd-btn-danger wd-timer-remove">${d(F)}</button></div></div>`},P=v.map((A,M)=>R(A,M)).join("");i=`<div class="wd-timerlist" data-opt="${s}" data-ftype="timerlist">${P}<button type="button" class="wd-btn wd-btn-sm wd-btn-secondary wd-timer-add">${d(B)}</button></div>`}else if(h.type==="entity"){const v=h.placeholder?` placeholder="${d(h.placeholder)}"`:"";i=`<div class="wd-combo"><input type="text" class="wd-combo-inp" data-opt="${s}" data-ftype="entity" value="${d(l)}" autocomplete="off" spellcheck="false"${v}><div class="wd-combo-drop" hidden></div></div>`}else if(h.type==="list"){const v=Array.isArray(l)?l.join(", "):d(l);i=`<input type="text" data-opt="${s}" data-ftype="list" value="${d(v)}" placeholder="${d(e.t("placeholder.notify_services_list",{},"notify.mobile_app_phone, ..."))}">`}else if(h.type==="intlist"){const v=Array.isArray(l)?l.join(", "):String(l??""),T=h.placeholder?` placeholder="${d(h.placeholder)}"`:"";i=`<input type="text" data-opt="${s}" data-ftype="intlist" value="${d(v)}"${T}>`}else{const v=h.type==="number"?"number":"text",T=e.datalistId?` list="${e.datalistId}"`:"",I=h.step!=null?` step="${h.step}"`:"",D=h.min!=null?` min="${h.min}"`:"",F=h.max!=null?` max="${h.max}"`:"",B=h.placeholder?` placeholder="${d(h.placeholder)}"`:"";i=`<input type="${v}" data-opt="${s}" data-ftype="${h.type}" value="${d(l)}"${I}${D}${F}${T}${B}>${e.datalist||""}`}const c=e.suggestion,p=e.mlSuggestion,_=c&&c.suggested!=null&&!qt(c.suggested,t)?c.suggested:null,u=p&&p.value!=null&&!qt(p.value,t)?p.value:null,g=e.t,k=c?c.reason_key?g(c.reason_key,c.reason_params||{},c.reason||""):c.reason||"":"",$=p?p.reason_key?g(p.reason_key,p.reason_params||{},p.reason||""):p.reason||"":"",x=v=>`<button type="button" class="wd-sug-use" data-sugkey="${s}" data-sugval="${d(v)}">${e.useBtnLabel||"Use"}</button>`;let w="";if(_!=null&&u!=null){const v=parseFloat(_),T=parseFloat(u);if((!isNaN(v)&&!isNaN(T)?Math.abs(v-T)/Math.max(Math.abs(v),Math.abs(T),1e-9):1)<.05){const D=g("suggestion.calibrated_label",{},"Calibrated"),F=ct([k,$?`${D}: ${$}`:""].filter(Boolean).join(`

`));w=`<div class="wd-sug"><span class="wd-sug-chip wd-sug-chip-obs">\u{1F4A1} ${d(g("suggestion.both_agree",{},"WashData recommends"))}</span><span class="wd-sug-val">${d(_)}${n}</span>${x(_)}${F}</div>`}else{const D=k?ct(k):"",F=$?ct($):"",B=g("suggestion.observed_label",{},"Observed"),R=g("suggestion.calibrated_label",{},"Calibrated");let P="",A="";if(!isNaN(v)&&!isNaN(T)){const G=T>v;A=g(`suggestion.impact.${s}.${G?"higher":"lower"}`,{},""),P=g(`suggestion.impact.${s}.${G?"lower":"higher"}`,{},"")}const M=P?`<div class="wd-sug-impact-line">${d(P)}</div>`:"",O=A?`<div class="wd-sug-impact-line">${d(A)}</div>`:"";w=`<div class="wd-sug wd-sug-split"><div class="wd-sug-opt"><span class="wd-sug-chip wd-sug-chip-obs">\u{1F4A1} ${d(B)}</span><span class="wd-sug-val">${d(_)}${n}</span>${x(_)}${D}${M}</div><div class="wd-sug-opt"><span class="wd-sug-chip wd-sug-chip-cal">\u{1F916} ${d(R)}</span><span class="wd-sug-val">${d(u)}${n}</span>${x(u)}${F}${O}</div></div>`}}else if(_!=null){const v=k?ct(k):"",T=t!=null&&t!==""?` <span style="opacity:.6;font-size:.9em">(now ${d(t)}${n})</span>`:"";w=`<div class="wd-sug"><span class="wd-sug-chip wd-sug-chip-obs">\u{1F4A1} ${d(g("suggestion.observed_label",{},"Observed"))}</span><span class="wd-sug-val">${d(_)}${n}</span>${T}${x(_)}${v}</div>`}else if(u!=null){const v=$?ct($):"";w=`<div class="wd-sug"><span class="wd-sug-chip wd-sug-chip-cal">\u{1F916} ${d(g("suggestion.calibrated_label",{},"Calibrated"))}</span><span class="wd-sug-val">${d(u)}${n}</span>${x(u)}${v}</div>`}if(w&&s){const v=d(g("btn.mute_suggestion",{},"Stop suggesting this setting")),T=`<button type="button" class="wd-sug-lock" data-suglock="${s}" title="${v}" aria-label="${v}">\u{1F515}</button>`;w=w.replace(/<\/div>\s*$/,T+"</div>")}return`<div class="wd-field" data-field="${s}"><div class="wd-label-row"><label style="margin:0">${d(r)}</label>${o}${a}</div>${i}${h.hint?`<div class="wd-field-hint">${d(h.hint)}</div>`:""}<div class="wd-conflict-err" data-cerr="${s}" hidden></div>${w}</div>`}function qt(h,t){if(h==null||t==null)return!1;const e=parseFloat(h),s=parseFloat(t);return!isNaN(e)&&!isNaN(s)?Math.abs(e-s)<1e-6:String(h)===String(t)}const xe={min_power:"min_power",off_delay:"off_delay",smoothing_window:"smoothing",start_threshold_w:"hysteresis",stop_threshold_w:"hysteresis",start_energy_threshold:"start_energy",running_dead_zone:"dead_zone",profile_duration_tolerance:"duration_tolerance",profile_match_min_duration_ratio:"match_ratios",profile_match_max_duration_ratio:"match_ratios",progress_reset_delay:"progress_reset",completion_min_seconds:"min_duration",min_off_gap:"min_off_gap",start_duration_threshold:"start_duration",end_energy_threshold:"end_energy_thresh",end_repeat_count:"end_repeat",profile_match_threshold:"confidence",profile_unmatch_threshold:"confidence",auto_label_confidence:"confidence",learning_confidence:"confidence",no_update_active_timeout:"watchdog_timeout",anti_wrinkle_enabled:"anti_wrinkle",anti_wrinkle_max_power:"anti_wrinkle",anti_wrinkle_max_duration:"anti_wrinkle",anti_wrinkle_exit_power:"anti_wrinkle",anti_wrinkle_idle_timeout:"anti_wrinkle",anti_crease_finalize_ratio:"anti_wrinkle",sampling_interval:"sampling"};function le(h){let t=0,e=window.innerWidth,s=0,r=window.innerHeight;for(let n=h.parentElement;n;n=n.parentElement){const a=getComputedStyle(n),o=a.overflowX!=="visible",i=a.overflowY!=="visible";if(!o&&!i)continue;const l=n.getBoundingClientRect();o&&(l.left>t&&(t=l.left),l.right<e&&(e=l.right)),i&&(l.top>s&&(s=l.top),l.bottom<r&&(r=l.bottom))}return{left:t,right:e,top:s,bottom:r}}function ct(h,t){return`<span class="wd-tip">i<span class="wd-tip-pop">${t?$e(t):""}<span class="wd-tip-txt">${d(h)}</span></span></span>`}function Vt(h,t,e=""){return`<label class="wd-switch-lbl"><span class="wd-switch"><input type="checkbox" ${h}><span class="wd-switch-slider"></span></span><span class="wd-switch-text wd-switch-text--plain">${d(t)}</span></label>${e||""}`}function Mt(h,t,e="",s=""){return`<div class="wd-field wd-field-switch"><div class="wd-switch-row">${Vt(h,t,e)}</div>${s?`<div class="wd-field-hint">${d(s)}</div>`:""}</div>`}function $e(h){const t=s=>`<svg class="wd-dg" viewBox="0 0 200 90" preserveAspectRatio="xMidYMid meet">${s}</svg>`,e='<line class="ax" x1="8" y1="78" x2="192" y2="78"/>';switch(h){case"smoothing":return t(`${e}
    ...
    `}_htmlDeviceBar(){const t=this._isAdmin()?`<button class="wd-devcard wd-devadd" data-action="add-device" title="${d(this._t("btn.add_device_tip",{},"Add another WashData device"))}">${this._t("btn.add_device",{},"+ Add device")}</button>`:"";return this._devices.length<=1?t?`<div class="wd-devbar">${t}</div>`:"":`<div class="wd-devbar">${this._devices.map((e,s)=>{const r=e.is_user_paused?"user_paused":e.detector_state||"unknown",n=Rt.includes(r),a=!!e.recording,o=a?"var(--error-color, #f44336)":this._stateColor(r),i=a?this._t("status.recording",{},"Recording"):this._stateLabel(r),l=[],c=this._conflictCountForOpts(e.options||{},e.option_defaults||{});c&&l.push(`<span class="wd-dbadge conf">\u26A0 ${c}</span>`);const p=this._sugCountsForDevice(e).total;return p&&l.push(`<span class="wd-dbadge sug">\u{1F4A1} ${p}</span>`),e.feedback_count&&l.push(`<span class="wd-dbadge fb">\u{1F4AC} ${e.feedback_count}</span>`),`<button class="wd-devcard ${s===this._selIdx?"active":""}" data-idx="${s}">
        ...
        ${l.join("")}
      </button>`}).join("")}${t}</div>`}_htmlStatus(){const t=this._devices[this._selIdx];if(!t)return`<div class="wd-empty">${this._t("msg.no_device_selected",{},"No device selected.")}</div>`;const e=!!t.is_user_paused,s=e?"user_paused":t.detector_state||"unknown",r=!!t.recording,n=r?"var(--error-color, #f44336)":this._stateColor(s),a=r?this._t("status.recording",{},"Recording"):this._stateLabel(s),o=r||Rt.includes(s),i=t.cycle_progress_pct,l=t.time_remaining_s,c=t.current_program,p=!!t.manual_program,_=c?null:t.armed_program||null,u=c||_||"auto_detect",g=(this._profiles||[]).map(E=>E.name);c&&!g.includes(c)&&g.unshift(c),_&&!g.includes(_)&&g.unshift(_);const k=g.map(E=>`<option value="${d(E)}" ${u===E?"selected":""}>${d(E)}</option>`).join(""),$=c?p?this._t("badge.manual",{},"(manually selected)"):this._t("badge.auto",{},"(auto-detected)"):_?this._t("badge.armed",{},"(applies to the next cycle)"):"",w=$?`<span class="wd-prog-tag ${c?p?"manual":"auto":"manual"}">${$}</span>`:"",v=`<div class="wd-prog-ctl"><label>${this._t("lbl.program",{},"Program")}</label>${ct(this._t("lbl.program_tip",{},"Override which profile is matched to the current cycle. Auto-detect lets the integration pick the best match automatically. Pin a specific program to force-match it when auto-detect is wrong or you know what is running. Pick one before starting the appliance and it is applied as soon as the next cycle begins."))}
          <select id="wd-status-prog">
            <option value="auto_detect" ${u==="auto_detect"?"selected":""}>${this._t("status.auto_detect",{},"Auto-detect")}</option>
            ${k}
          </select>${w}</div>`,T=[];t.recording&&this._canEdit()&&T.push(`<div class="wd-attn-card"><span class="wd-attn-icon">\u25CF</span><div class="wd-attn-body"><div class="wd-attn-title">${this._t("msg.recording_in_progress",{},"Recording in progress")}</div><div class="wd-attn-sub">${this._t("msg.see_recorder",{},"See recorder widget below")}</div></div></div>`),t.feedback_count&&this._canEdit()&&T.push(`<button class="wd-attn-card" type="button" data-action="goto-feedbacks"><span class="wd-attn-icon">\u{1F4AC}</span><div class="wd-attn-body"><div class="wd-attn-title">${this._t("msg.feedback_cycles_pending",{n:t.feedback_count},`To review: ${t.feedback_count}`)}</div><div class="wd-attn-sub">${this._t("msg.review_to_cycles",{},"Open the Cycles review queue")}</div></div></button>`);const I=this._conflictKeysFromOpts();if(I.size&&this._canEdit()){const E=I.size,W=E>1?"s":"";T.push(`<button class="wd-attn-card" type="button" style="border-color:var(--error-color,#b71c1c)" data-action="goto-conflicts"><span class="wd-attn-icon">\u26A0</span><div class="wd-attn-body"><div class="wd-attn-title" style="color:var(--error-color,#b71c1c)">${this._t("conflict.attn_title",{n:E},`Setting conflicts: ${E}`)}</div><div class="wd-attn-sub">${this._t("conflict.attn_sub",{},"Fix conflicts before saving")}</div></div></button>`)}const D=this._sugCountsForDevice(t);if(D.total&&this._canEdit()){const E=D.total,W=[];D.classic&&W.push(this._t("lbl.n_classic_suggestions",{n:D.classic},`${D.classic} classic`)),D.ml&&W.push(this._t("lbl.n_ml_suggestions",{n:D.ml},`${D.ml} ML`)),T.push(`<button class="wd-attn-card" type="button" data-action="goto-suggestions"><span class="wd-attn-icon">\u{1F4A1}</span><div class="wd-attn-body"><div class="wd-attn-title">${this._t("lbl.n_tuning_suggestions",{n:E},`${E} tuning suggestion${E>1?"s":""}`)}</div><div class="wd-attn-sub">${W.join(" \xB7 ")} \xB7 ${this._t("msg.review_in_settings",{},"Review in Settings")}</div></div></button>`)}const F=T.length?`<div class="wd-attn">${T.join("")}</div>`:"",B=t.envelope_position,R=B!=null?Math.round(B*100):null,P=R!=null?` <span style="opacity:.75" title="${d(this._t("lbl.envelope_position_tip",{pct:R},`Position on the matched program's own recorded curve, ${R}%. Measured by aligning this cycle against the profile instead of counting time, so it stays right when a run is longer or shorter than usual. It refreshes while the appliance is quiet, so it can lag behind during an active phase.`))}">${this._t("lbl.envelope_position",{pct:R},`curve ${R}%`)}</span>`:"",A=o&&i!=null?`
      <div class="wd-prog-bg"><div class="wd-prog-fill" style="width:${Math.min(100,i)}%"></div></div>
      <div class="wd-prog-row"><span>${i.toFixed(1)}%${P}</span>${l!=null?`<span>${this._t("lbl.time_remaining",{v:_t(l)},`~${_t(l)} remaining`)}</span>`:""}</div>
    `:"",O=((this._powerData||{}).live||[]).length>1,G=this._pref("show_expected",!0),et=this._pref("show_raw_active",!1),st=`<div class="wd-leg">
      <span class="wd-leg-i"><span class="wd-leg-sw" style="background:var(--primary-color)"></span> ${this._t("lbl.power",{},"Power")}</span>
      ${this._statusEnv?`<label class="wd-leg-i"><input type="checkbox" data-statustoggle="show_expected" ${G?"checked":""}><span class="wd-leg-sw" style="background:#ff9800"></span> ${this._t("lbl.expected",{},"Expected")}</label>`:""}
      ${this._pref("show_raw",!1)?`<label class="wd-leg-i"><input type="checkbox" data-statustoggle="show_raw_active" ${et?"checked":""}><span class="wd-leg-sw" style="background:#9e9e9e"></span> ${this._t("lbl.raw_socket",{},"Raw socket")}</label>`:""}
    </div>`,J=this._cyclesTotal||0,tt=(this._profiles||[]).length,wt=this._pref("setup_card_dismissed",!1),U=this._setupStatus,ht=U&&!O,Z=ht?this._htmlSetupCard(U,wt):"",ot=O?`<div class="wd-canvas-wrap" style="margin-top:14px"><canvas id="wd-status-canvas" role="img" aria-label="${d(this._t("lbl.aria_power_chart",{},"Power consumption chart"))}" style="height:160px"></canvas></div>${st}`:ht?Z:`<p class="wd-info" style="margin-top:12px">${this._t("msg.live_chart_loading",{},"Live power chart appears as readings arrive.")}</p>`,pt=this._pref("show_debug",!1);let m="";if(pt){const E=this._matchDebug||{},W=E.confidence!=null?`${(E.confidence*100).toFixed(1)}%`:"-",N=(E.candidates||[]).map(H=>`<tr><td>${d(H.profile_name)}</td><td>${H.confidence_pct}%</td><td>${H.mae}</td><td>${H.correlation}</td><td>${H.duration_ratio>=0?"+":""}${H.duration_ratio}%</td></tr>`).join("");m=`<div class="wd-card">
        ...
        <div class="wd-kv" style="margin-bottom:12px">
          ...
          <div class="wd-kv-item"><div class="wd-kv-val" style="font-size:1em;color:${E.ambiguous?"var(--warning-color,#ff9800)":"var(--success-color,#4caf50)"}">${E.ambiguous?this._t("status.ambiguous",{},"Ambiguous"):this._t("status.clear",{},"Clear")}</div><div class="wd-kv-lbl">${this._t("lbl.label",{},"Match")}</div></div>
        </div>
        ${N?`<table class="wd-table"><thead><tr><th>Profile</th><th>Conf</th><th>MAE</th><th>Corr</th><th>Duration</th></tr></thead><tbody>${N}</tbody></table>`:`<p class="wd-info">${this._t("msg.no_match_yet",{},"No match attempt yet - this populates during a running cycle.")}</p>`}
      </div>`}const f=[];this._canEdit()&&f.push(`<button class="wd-attn-card" type="button" data-action="open-advanced" data-sub="diagnostics"><span class="wd-attn-icon">\u{1FA7A}</span><div class="wd-attn-body"><div class="wd-attn-title">${this._t("hdr.logs_diagnostics",{},"Diagnostics")}</div><div class="wd-attn-sub">${this._t("msg.storage_diagnostics",{},"Storage stats, maintenance, export/import")}</div></div></button>`),f.push(`<button class="wd-attn-card" type="button" data-action="open-settings"><span class="wd-attn-icon">\u2699\uFE0F</span><div class="wd-attn-body"><div class="wd-attn-title">${this._t("settings.gear.title",{},"Settings")}</div><div class="wd-attn-sub">${this._isAdmin()?this._t("msg.preferences_admin",{},"Preferences, panel & access control"):this._t("msg.preferences_adv",{},"Preferences")}</div></div></button>`);const S=`<div class="wd-card"><div class="wd-card-title">${this._t("hdr.tools_and_data",{},"Tools & Data")}</div><div class="wd-attn" style="margin-bottom:0;margin-top:12px">${f.join("")}</div></div>`,b=(()=>{if(!this._canEdit())return"";const E=Rt.includes(s),W=E&&!e,N=e,H=E||e;return!W&&!N&&!H?"":`<div class="wd-cycle-ctrl" style="margin-top:0">
        ${N?`<button class="wd-btn wd-btn-sm wd-btn-primary" data-action="resume-cycle" title="${d(this._t("btn.resume_cycle_tip",{},"Resume the paused cycle"))}">${this._t("btn.resume_cycle",{},"Resume")}</button>`:""}
        ...
        ${H?`<button class="wd-btn wd-btn-sm wd-btn-danger" data-action="terminate-cycle" title="${d(this._t("btn.force_stop_tip",{},"Immediately end the current cycle and mark it as force-stopped"))}">${this._t("btn.force_stop",{},"Force Stop")}</button>`:""}
      </div>`})();return`
      ${F}
      <div class="wd-card">
        ...
        ${ht?"":`<div class="wd-card-title" style="margin-top:18px">${this._t("hdr.live_power",{},"Live Power")}</div>`}
        ${ot}
      </div>
      ${this._canEdit()?this._htmlRecordingWidget():""}
      ${m}
      ${S}
    `}_htmlSetupCard(t,e=!1){if(!t)return"";const{phase:s,message_key:r,message_params:n,cta_label_key:a,cta_action:o,secondary_label_key:i,secondary_action:l,skippable:c,dismissible:p,step_key:_}=t;if(s==="phase4")return`
        ...
        </div>`;const u=this._t(r,n||{},""),g=this._t(a,{},"Continue"),k=i?this._t(i,{},""):null,$=c?`
      ...
      <div class="wd-setup-card" data-phase="${d(s)}">
        ...
        <div style="display:flex;flex-wrap:wrap;gap:8px;align-items:center">
          ...
          ${k?`<a class="wd-link" data-action="setup-cta"
                           data-cta-action="${d(l||"")}"
                           data-cta-params="${d(JSON.stringify(n||{}))}">
            ${d(k)}
          </a>`:""}
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;flex-wrap:wrap">
    ...
    </div>`}_htmlHistory(){const t=this._cycles||[],e=this._refCycles||[],s=e.concat(t),r=this._canEdit(),n=this._selectMode&&r,a=this._cycleSel,{col:o,dir:i}=this._cycleSort,{text:l,status:c}=this._cycleFilter,p=this._mlById||{},_=new Set((this._feedbacks||[]).map(b=>b.cycle_id)),u=b=>p[b.id],g=b=>{const E=u(b);return!!(E&&E.ml_review&&E.ml_review.reviewed_at)},k=b=>{const E=u(b);return!!(E&&E.ml_review&&E.ml_review.golden)},$=b=>{if(_.has(b.id))return!0;if(g(b))return!1;const E=u(b),W=E&&E.ml_quality_label;return["uncertain","review"].includes(W)||["force_stopped","interrupted"].includes(b.status)},x=s.filter($).length;let w=s;if(l){const b=l.toLowerCase();w=w.filter(E=>(E.profile_name||E.matched_profile||"").toLowerCase().includes(b))}c==="unlabelled"?w=w.filter(b=>!b.profile_name&&!b.matched_profile):c==="needs_review"?w=w.filter($):c==="imported"?w=w.filter(b=>b.is_reference):c&&(w=w.filter(b=>(b.status||"completed")===c));const v={date:b=>b.start_time?new Date(b.start_time).getTime():0,confidence:b=>b.match_confidence,duration:b=>b.duration,energy:b=>b.energy_kwh!=null?b.energy_kwh:b.energy_wh!=null?b.energy_wh/1e3:null,cost:b=>b.cost!=null?b.cost:-1,status:b=>b.status||"completed",profile:b=>(b.profile_name||b.matched_profile||"\uFFFF").toLowerCase()};w=ie(w,v[o]||v.date,i);const T=b=>b==="completed"?"var(--success-color, #4caf50)":b==="interrupted"?"var(--error-color, #f44336)":b==="force_stopped"?"var(--warning-color, #ff9800)":"var(--secondary-text-color)",I=b=>{if(!b.is_reference)return"";const E=b.cycle_origin==="backfill",W=E?this._t("badge.backfilled_tip",{},"Detected in imported power history. Shapes program matching only, not counted in stats."):this._t("badge.imported_tip",{},"Imported from the community store. Used for matching only, not counted in stats.");return` <span title="${d(W)}" style="color:var(--info-color,#2196f3)">${E?"\u{1F557}":"\u{1F4E5}"}</span>`},D=b=>k(b)?' <span title="'+d(this._t("badge.golden_cycle",{},"Recorded reference cycle"))+'" style="color:var(--warning-color,#ff9800)">\u2B50</span>':_.has(b.id)?' <span title="'+d(this._t("badge.feedback_requested",{},"Feedback requested"))+'" style="color:var(--info-color,#2196f3)">\u{1F4AC}</span>':g(b)?' <span title="'+d(this._t("badge.reviewed",{},"Reviewed"))+'" style="color:var(--success-color,#4caf50)">\u2713</span>':$(b)?' <span title="'+d(this._t("badge.needs_review",{},"Needs review"))+'" style="color:var(--error-color,#f44336)">\u25CF</span>':"",F=b=>{if(b.anomaly!=="overrun")return"";const E=b.overrun_ratio?" "+this._t("badge.overrun_ratio",{x:Number(b.overrun_ratio).toFixed(1)},`(${Number(b.overrun_ratio).toFixed(1)}x expected)`):"";return` <span title="${d(this._t("badge.overrun",{},"Ran longer than usual"))}${d(E)}" style="color:var(--warning-color,#ff9800)">\u23F1</span>`},B=b=>{if(b.anomaly!=="underrun")return"";const E=b.underrun_ratio?" "+this._t("badge.underrun_ratio",{pct:Math.round(b.underrun_ratio*100)},`(${Math.round(b.underrun_ratio*100)}% of expected)`):"";return` <span title="${d(this._t("badge.underrun",{},"Finished faster than usual"))}${d(E)}" style="color:var(--info-color,#2196f3)">\u26A1</span>`},R=b=>{if(!b.energy_anomaly||b.energy_anomaly==="none")return"";const E=b.energy_anomaly==="energy_spike",W=b.energy_z_score!=null?` (${b.energy_z_score>0?"+":""}${Number(b.energy_z_score).toFixed(1)}\u03C3)`:"",N=E?"badge.energy_spike":"badge.energy_low",H=E?"Higher energy than usual":"Lower energy than usual",j=E?"\u{1F53A}":"\u{1F53B}",q=E?"var(--error-color,#f44336)":"var(--info-color,#2196f3)";return` <span title="${d(this._t(N,{},H))}${d(W)}" style="color:${q}">${j}</span>`},P=b=>{const E=Array.isArray(b.artifacts)?b.artifacts.length:0;return E?` <span title="${d(this._t("badge.artifact_tip",{n:E},`${E} anomal${E>1?"ies":"y"} detected (e.g. door opened mid-cycle) \u2014 open to see them on the graph`))}" style="color:var(--warning-color,#ff9800)">\u26A0</span>`:""},A=b=>{const E=Array.isArray(b.restart_gaps)?b.restart_gaps.length:0;return E?` <span title="${d(this._t("badge.restart_gap_tip",{n:E},`${E} HA restart gap${E>1?"s":""} during this cycle \u2014 power trace has a hole`))}" style="color:var(--info-color,#2196f3)">\u21BB</span>`:""},M=this._hass&&this._hass.config&&this._hass.config.currency||"",O=b=>{if(b.cost==null)return"-";const E=`${b.cost.toFixed(2)}${M?" "+M:""}`;if(b.energy_price_mode==="dynamic"&&b.energy_price!=null){const W=this._t("col.cost_dynamic_tip",{price:b.energy_price.toFixed(4),cur:M},`Time-weighted: charged at the price in force during the cycle (effective ${b.energy_price.toFixed(4)} ${M}/kWh).`);return`<span title="${d(W)}" style="border-bottom:1px dotted var(--secondary-text-color)">${E}</span>`}return E},G=w.map(b=>{const E=b.profile_name||b.matched_profile,W=b.match_confidence!=null?b.match_confidence*100:null,N=b.status||"completed",H=b.energy_kwh!=null?b.energy_kwh:b.energy_wh!=null?b.energy_wh/1e3:null,j=n,q=j?`<input type="checkbox" class="wd-csel" ${a.has(b.id)?"checked":""} style="width:auto;margin:0">`:`<span class="wd-devdot" style="background:${T(N)}" title="${d(N)}"></span>`,L={completed:this._t("status.completed",{},"Completed"),interrupted:this._t("status.interrupted",{},"Interrupted"),force_stopped:this._t("status.force_stopped",{},"Force stopped"),active:this._t("status.active",{},"Active")}[N]||N,V=`${I(b)}${D(b)}${F(b)}${B(b)}${R(b)}${P(b)}${A(b)}`.trim();return`<tr data-cid="${d(b.id)}" data-selmode="${j?1:0}" style="cursor:pointer">
        ...
        <td class="wd-tc-flags">${V}</td>
        <td><span style="color:${T(N)};font-size:.9em">${d(L)}</span></td>
        <td class="wd-tc-date">${rt(b.start_time)}</td>
        <td class="wd-tc-num">${_t(b.duration)}</td>
        <td class="wd-tc-num">${H!=null?yt(H):"-"}</td>
        <td class="wd-tc-num">${O(b)}</td>
        <td class="wd-tc-num">${W!=null?W.toFixed(0)+"%":"-"}</td>
      </tr>`}).join(""),et=`<thead><tr>
    ...
    </div>`,J=w.length!==s.length?this._t("lbl.n_shown",{n:w.length},`, ${w.length} shown`):"",tt=e.length?this._t("lbl.n_imported_note",{n:e.length},`, ${e.length} imported`):"",wt=this._t("lbl.cycles_title",{n:`${t.length}${tt}${J}`},`Cycles (${t.length}${tt}${J})`),U=r?`<div class="wd-card-actions" style="margin:0 0 4px;justify-content:flex-end">
    ...
    </div>`:"",ht=new Set(e.map(b=>b.id)),Z=[...a].some(b=>ht.has(b)),ot=n?`<div class="wd-card-actions" style="margin:0 0 10px">
      ...
      <div class="wd-card">
        ...
        ${st}
        ${U}${ot}
        ${w.length===0?`<div class="wd-empty" style="padding:24px"><div class="wd-icon">\u{1F4CB}</div>${s.length?this._t("msg.no_cycles_match",{},"No cycles match the current filter."):this._t("msg.no_cycles_yet",{},"No cycles recorded yet.")}</div>`:`<div class="wd-table-wrap"><table class="wd-table">${et}<tbody>${G}</tbody></table></div>`}
        ${m}
      </div>`;return(this._cyclesError?`<div class="wd-error-state"><span>${this._t("msg.fetch_error",{},"Failed to load data.")}</span><button class="wd-btn" type="button" data-action="retry-cycles">${this._t("btn.retry",{},"Retry")}</button></div>`:"")+f}_trendIcon(t){return t==="up"?`<span title="${d(this._t("trend.up",{},"Trending up"))}" style="color:var(--warning-color,#ff9800)">\u2191</span>`:t==="down"?`<span title="${d(this._t("trend.down",{},"Trending down"))}" style="color:var(--info-color,#2196f3)">\u2193</span>`:""}_profileCardHtml(t){const e=t.avg_duration?this._t("lbl.duration_avg",{v:Math.round(t.avg_duration/60)},`~${Math.round(t.avg_duration/60)}m avg`):this._t("lbl.no_duration",{},"no duration"),s=t.avg_energy!=null?` \xB7 ${yt(t.avg_energy)}/cycle`:"",r=t.avg_energy!=null&&t.cycle_count?` \xB7 <strong>${yt(t.avg_energy*t.cycle_count)}</strong> total`:"",n=this._hass&&this._hass.config&&this._hass.config.currency||"",a=t.avg_cost!=null?` \xB7 ${this._t("lbl.avg_cost",{},"Avg")} ${t.avg_cost.toFixed(2)}${n?" "+n:""}/${this._t("lbl.per_cycle_short",{},"cycle")}`:"",o=(this._profileHealth||{})[t.name],i=(this._profileTrends||{})[t.name];let l="";o&&o.health_status==="poor"?l=`<span class="wd-badge" style="color:var(--error-color,#f44336);background:rgba(244,67,54,.12)" title="${d(this._t("badge.poor_fit_tip",{},"Inconsistent match history \u2014 consider rebuilding this profile"))}">\u26A0 ${this._t("badge.poor_fit",{},"poor fit")}</span>`:o&&o.health_status==="fair"&&(l=`<span class="wd-badge" style="color:var(--warning-color,#ff9800);background:rgba(255,152,0,.12)" title="${d(this._t("badge.fair_fit_tip",{},"Moderate match consistency \u2014 some cycles assigned to this profile have lower confidence scores. Label more cycles or re-record the profile to improve accuracy."))}">${this._t("badge.fair_fit",{},"fair fit")}</span>`);let c="";if(i){const D=this._trendIcon(i.duration_trend),F=i.energy_trend?this._trendIcon(i.energy_trend):"";if(i.duration_trend!=="stable"||i.energy_trend==="up"){const B=[];if(i.duration_trend!=="stable"){const P=`${i.duration_slope_pct>0?"+":""}${i.duration_slope_pct}`;B.push(i.duration_trend==="up"?this._t("msg.duration_trend_up_tip",{pct:P},`Duration up (${P}%/cycle)`):this._t("msg.duration_trend_down_tip",{pct:P},`Duration down (${P}%/cycle)`))}if(i.energy_trend&&i.energy_trend!=="stable"){const P=`${i.energy_slope_pct>0?"+":""}${i.energy_slope_pct}`;B.push(i.energy_trend==="up"?this._t("msg.energy_trend_up_tip",{pct:P},`Energy up (${P}%/cycle)`):this._t("msg.energy_trend_down_tip",{pct:P},`Energy down (${P}%/cycle)`))}const R=B.join(", ")||this._t("msg.performance_trending",{},"Performance trending");c=`<span class="wd-badge" style="color:var(--secondary-text-color,#888)" title="${d(R)}">${D}${F||""}</span>`}}const p=this._constants&&this._constants.PROFILE_MIN_WARMUP_CYCLES||5,_=o&&o.cycle_count||0,g=_<p&&!t.is_imported?`<span class="wd-badge" title="${d(this._t("msg.warmup_detail",{needed:p},`This profile needs ${p} labelled cycles before auto-matching begins. Every confirmed cycle helps it learn.`))}" style="background:var(--info-color,#2196f3);color:#fff">${this._t("msg.warmup_badge",{done:_,needed:p},`Still learning (${_}/${p} cycles)`)}</span>`:"",k=t.is_imported?`<span class="wd-badge" title="${d(this._t("badge.imported_tip",{},"Imported from the community store. Used for matching only, not counted in stats."))}" style="background:var(--info-color,#2196f3);color:#fff">\u{1F4E5} ${this._t("status.imported",{},"Imported")}</span>`:"",$=(this._profileAdvisories||[]).find(D=>D&&D.profile===t.name&&D.code==="unmatchable"),x=$?`<span class="wd-badge" style="color:var(--error-color,#f44336);background:rgba(244,67,54,.12)" title="${d(this._t($.message_key,$.message_params,$.message))}">\u26A0 ${this._t("badge.unmatchable",{},"can't be matched")}</span>`:"",w=(this._profileTerminal||{})[t.name];let v="";if(w&&w.quiet_before_s>0&&w.seen_in>=2){const D=Math.max(1,Math.round(w.quiet_before_s/60)),F=Number(w.event_watts||0).toFixed(0),B=Math.round(w.event_seconds||0),R=this._t("badge.quiet_tail_tip",{mins:D,secs:B,watts:F,seen:w.seen_in,measured:w.measured},`Near the end this program goes quiet for about ${D} min, then draws about ${F} W for ${B} s before finishing. Seen in ${w.seen_in} of ${w.measured} measured cycles: the appliance does not do it every run.`);v=`<span class="wd-badge" style="color:var(--secondary-text-color,#888)" title="${d(R)}">${this._t("badge.quiet_tail",{mins:D},`~${D}m quiet tail`)}</span>`}const T=[x,l,c,v,g,k].filter(Boolean).join(" "),I=Array.isArray(t.signature_curve)&&t.signature_curve.length>=3?`<canvas class="wd-prof-spark" data-spark-prof="${d(t.name)}" width="64" height="20" aria-label="${d(this._t("lbl.sparkline",{name:t.name},"Average power curve"))}"></canvas>`:"";return`
      <div class="wd-prof-wrap">
        ...
        ${w}
        <div class="wd-profiles-grid">${k}</div>
      </div>`}).join(""),o=this._profiles.filter(g=>!n.has(g.name)),i=o.map(g=>this._profileCardHtml(g)).join(""),l=this._profiles.length===0&&!(this._cycles||[]).length&&!(this._refCycles||[]).length,p=(e&&this._onlineEnabled()&&l?`
    ...
    `}_htmlProfileGroupModal(t){const e=this._busy.has("pg-save"),s=this._profileEnvCache||{},r=_=>it[Math.max(0,this._profiles.findIndex(u=>u.name===_))%it.length],n=t.members||[],a=this._profiles.map(_=>{const u=n.includes(_.name),g=_.avg_duration?`~${Math.round(_.avg_duration/60)}m`:"",k=_.avg_energy!=null?` \xB7 ${yt(_.avg_energy)}`:"",$=u?`<span style="display:inline-block;width:10px;height:10px;border-radius:2px;background:${r(_.name)};margin:0 2px"></span>`:"";return`<label class="wd-rev-tag"><input type="checkbox" class="wd-pg-mem" value="${d(_.name)}" ${u?"checked":""}> ${$}${d(_.name)} <span style="color:var(--secondary-text-color);font-size:.85em">${g}${k}</span></label>`}).join(""),o=n.filter(_=>s[_]&&(s[_].avg||[]).length),i=o.length?`<div class="wd-leg">${o.map(_=>`<span class="wd-leg-i"><span class="wd-leg-sw" style="background:${r(_)}"></span> ${d(_)}</span>`).join("")}</div>`:"",l=o.length?`<div class="wd-canvas-wrap" style="margin-top:8px"><canvas id="wd-pgroup-canvas" role="img" aria-label="${d(this._t("lbl.aria_envelope_chart",{},"Profile power envelope chart"))}" style="height:150px"></canvas></div>${i}`:`<p class="wd-info">${this._t("msg.group_preview_hint",{},"Tick 2+ members to preview and compare their power curves.")}</p>`,c=((this._profileGroups||{}).groups||[]).find(_=>_.name===t.orig),p=c&&c.cohesion!=null?`<span class="wd-badge" style="color:${c.cohesive?"var(--success-color,#4caf50)":"var(--warning-color,#ff9800)"};background:${c.cohesive?"rgba(76,175,80,.14)":"rgba(255,152,0,.14)"}">${c.cohesive?this._t("lbl.cohesion_good",{pct:Math.round(c.cohesion*100)},"cohesion "+Math.round(c.cohesion*100)+"%"):this._t("lbl.cohesion_low",{pct:Math.round(c.cohesion*100)},"\u26A0 low cohesion "+Math.round(c.cohesion*100)+"%")}</span>`:"";return`<h2>${t.orig?this._t("modal.edit_group",{},"Edit profile group"):this._t("modal.new_group",{},"New profile group")}</h2>
    ...
    </label>`,$=r?`<p class="wd-info" style="margin:0 0 10px;font-size:.82em">${this._t("msg.settings_basic_note",{},"Showing essential settings. Switch to Advanced for the full list.")}</p>`:"",x=this._busy.has("save-settings"),w=l.size,v=w!==1?"s":"",T=w?`
      ...
      </div>`:"",I=this._suggestions.length,D=[...a].filter(U=>!n.has(U)).length,F=I+D,B=this._settingsSugOnly&&!this._settingsSearch,R=I?`<button class="wd-btn wd-btn-sm wd-btn-primary" data-action="sug-apply-all">${this._t("btn.apply_all",{},"Apply all")}</button>`:"",P=I?`<button class="wd-btn wd-btn-sm wd-btn-secondary" data-action="sug-dismiss">${this._t("btn.dismiss",{},"Dismiss")}</button>`:"",A=F?B?`
      ...
      <div class="wd-sug-banner">
        ...
        ${R}
        ${P}
      </div>`:"",M=this._busy.has("sug-analyze"),O=`<button class="wd-btn wd-btn-secondary wd-btn-sm" data-action="sug-analyze" ${M?"disabled":""} title="${d(this._t("btn.run_analysis_tip",{},"Analyze your recorded cycles now and refresh tuning suggestions"))}">${M?'<span class="wd-spin"></span> '+this._t("status.analyzing",{},"Analyzing\u2026"):this._t("btn.run_analysis",{},"\u{1F50D} Run suggestion analysis")}</button>`,G=this._settingsSearch||"",et=G.trim().toLowerCase(),st=`<input type="text" id="wd-settings-search" class="wd-filter-input" placeholder="${this._t("msg.search_placeholder",{},"Search settings\u2026")}" value="${d(G)}" autocomplete="off" style="flex:0 0 auto;width:200px;max-width:40%">`,J=et?this._htmlSettingsSearch(t,et):B?this._htmlSettingsSugOnly(t):this._htmlSettingsSection(t),tt=(this._lockedSuggestions||[]).length,wt=tt?`
      <div class="wd-sug-banner" style="opacity:.92">
        <span>\u{1F515} ${this._t("msg.n_suggestions_muted",{count:tt},`${tt} suggestion${tt>1?"s":""} muted; the auto-tuner will not propose these.`)}</span>
      ...
      <div class="wd-card">
        <form id="wd-settings-form">${J}</form>
    ...
    </div>`:"";return`<div class="wd-field"><label>${d(r)} ${n?ct(n):""}${c}${p}</label>
      <div class="wd-combo-row">
        <div class="wd-combo">
    ...
    `}_htmlMlStatusSection(t,e){const s=e&&this._busy.has("ml-train-now:"+e)||t&&t.running,r=this._canEdit()?`<button class="wd-btn wd-btn-primary wd-btn-sm" data-action="ml-train-now" ${s?"disabled":""}>${s?`<span class="wd-spin"></span> ${this._t("status.training",{},"Training\u2026")}`:this._t("btn.train_now",{},"Train now")}</button>`:"";if(!t)return`<div class="wd-card"><div class="wd-card-title" style="margin:0 0 4px">${this._t("hdr.status",{},"Status")}</div><p class="wd-info" style="margin:0">${this._t("msg.loading",{},"Loading\u2026")}</p></div>`;const n=Object.keys(t.on_device_models||{}).length,a=n?`<span style="color:var(--success-color,#4caf50);font-weight:600">${this._t("ml.personalized",{},"\u25CF Personalized to this machine")}</span> <span style="color:var(--secondary-text-color)">${this._t("lbl.models_fine_tuned",{count:n},"(fine-tuned models: "+n+")")}</span>`:`<span style="color:var(--secondary-text-color)">${this._t("ml.builtin_models",{},"\u25CF Using built-in models")}</span>`,o=t.cycle_count||0,i=t.min_cycles||0,l=o>=i,c=i>0?Math.min(100,Math.round(o/i*100)):100,p=l?"var(--success-color,#4caf50)":"var(--warning-color,#ff9800)",_=Math.max(0,i-o),u=l?this._t("msg.enough_data",{current:o,min:i},`Enough data to learn from (${o}/${i} cycles).`):this._t("msg.collecting_data",{need:_,current:o,min:i},`Collecting data. Cycles still needed before fine-tuning can start: ${_} (${o}/${i}).`),g=`<div style="height:8px;border-radius:6px;background:var(--secondary-background-color);overflow:hidden;margin:8px 0"><div style="width:${c}%;height:100%;background:${p}"></div></div>`,k=t.last_trained?rt(t.last_trained):"never",$=s?`<span style="color:var(--info-color,#2196f3)"><span class="wd-spin"></span> ${this._t("status.fine_tuning",{},"fine-tuning now\u2026")}</span>`:t.enabled?this._t("lbl.auto_fine_tune_on",{hour:String(t.hour).padStart(2,"0")},`auto fine-tune on (around ${String(t.hour).padStart(2,"0")}:00)`):this._t("lbl.auto_fine_tune_off",{},"auto fine-tune off");return`<div class="wd-card">
      ...
      ${g}
      <p class="wd-info" style="margin:0">${this._t("lbl.last_checked",{},"Last checked:")} <strong>${d(k)}</strong> \xB7 ${$}</p>
    ...
    </div>`}_mlTrendBadge(t){if(!t)return"";const s={improving:[this._t("badge.improving",{},"\u2197 improving"),"var(--success-color,#4caf50)",this._t("ml.trend_improving_tip",{},"This model's fit has improved across recent re-checks.")],declining:[this._t("badge.declining",{},"\u2198 declining"),"var(--warning-color,#ff9800)",this._t("ml.trend_declining_tip",{},"This model's fit has slipped across recent re-checks \u2014 reviewing more cycles may help it re-learn.")],steady:[this._t("badge.steady",{},"\u2192 steady"),"var(--secondary-text-color)",this._t("ml.trend_steady_tip",{},"This model's fit has held roughly steady across recent re-checks.")]}[t];return s?` <span title="${d(s[2])}" style="font-size:.72em;font-weight:600;color:${s[1]};margin-left:6px">${s[0]}</span>`:""}_htmlMatchingTuningCard(){const t=this._mlTrainingStatus,e=t&&t.matching;if(!e)return"";const s=e.defaults||{},r=e.tuned||null,n=r&&r.config||null,a=e.active==="tuned"&&n,o=this._busy.has("ml-revert-match"),i=u=>u==null||isNaN(u)?"-":Number(u).toFixed(2),l=[["corr_weight","Shape (correlation)"],["duration_weight","Duration agreement"],["energy_weight","Energy agreement"],["dtw_ensemble_w","DTW derivative blend (DDTW)"]].map(([u,g])=>{const k=s[u],$=a?n[u]:s[u],x=a&&k!=null&&$!=null&&Math.abs(k-$)>1e-9;return`<tr>
        <td>${g}</td>
        <td style="text-align:right;color:var(--secondary-text-color)">${i(k)}</td>
        <td style="text-align:right;font-weight:${x?"700":"400"};color:${x?"var(--primary-color)":"inherit"}">${i($)}</td>
      </tr>`}).join(""),c=a?`<span class="wd-badge" style="color:var(--success-color,#4caf50);background:rgba(76,175,80,.14)">${this._t("badge.using_tuned",{},"Using tuned weights")}</span>`:`<span class="wd-badge" style="color:var(--secondary-text-color);background:var(--secondary-background-color)">${this._t("badge.using_defaults",{},"Using shipped defaults")}</span>`;let p="";if(a){const u=r.trained_at?rt(r.trained_at):"unknown",g=r.baseline_test_top1,k=r.tuned_test_top1,$=g!=null&&k!=null?` \xB7 held-out top-1 ${(g*100).toFixed(0)}% \u2192 <strong>${(k*100).toFixed(0)}%</strong>`:"";p=`<p class="wd-info" style="margin:8px 0 0">Tuned ${d(u)} from ${r.cycle_count||0} cycles${$}.</p>`}const _=a?`<button class="wd-btn wd-btn-secondary wd-btn-sm" data-action="ml-revert-match" ${o?"disabled":""}>${o?'<span class="wd-spin"></span> '+this._t("status.reverting",{},"Reverting\u2026"):this._t("btn.reset_to_defaults",{},"Reset to defaults")}</button>`:"";return`<div class="wd-card" style="margin-top:12px">
      ...
      ${r}
    </section>`}_htmlPgParamRows(){const t=this._pgOverrideFields(),e=new Set(["start_threshold_w","stop_threshold_w"]),s={detection:"#2a78d6",timing:"#1baf7a",advanced:"#eda100",matching:"#a05cd6"},r={detection:this._t("lbl.pg_group_detection",{},"Detection triggers"),timing:this._t("lbl.pg_group_timing",{},"Timing rules"),advanced:this._t("lbl.pg_group_advanced",{},"Edge cases"),matching:this._t("lbl.pg_group_matching",{},"Program matching")};let n="";const a=t.map(([_,u,g,k,$,x])=>{const w=this._t("setting."+_+".label",{},u),v=this._pgFieldVal(_,{});let T;_==="start_threshold_w"?T=this._pgThreshStart??v:_==="stop_threshold_w"?T=this._pgThreshStop??v:T=this._pgParamOverrides[_]??v;const I=x==="bool",D=e.has(_),F=g||"",B=s[$]||"#2a78d6",R=r[$]||"";let P="";$&&$!==n&&(P=`<div style="display:flex;align-items:center;gap:8px;margin:${n?"12px":"2px"} 0 5px">
          <div style="width:3px;height:14px;border-radius:2px;background:${B};flex-shrink:0"></div>
        ...
        </div>`,n=$);const A=this._pgStagedVal(_),M=A!=null&&!(v!==""&&v!==null&&v!==void 0&&this._pgSameVal(A,v)),O=this._canEdit()&&M&&this._pgIsPublishable(_)?`<button class="wd-pg-pub" data-action="pg-publish-one" data-pgkey="${d(_)}" aria-label="${d(this._t("btn.pg_publish_one",{label:w},"Publish "+w+" to the integration"))}" title="${d(this._t("btn.pg_publish_one",{label:w},"Publish "+w+" to the integration"))}">\u2191</button>`:'<span class="wd-pg-pub-slot" aria-hidden="true"></span>';return`${P}<div style="display:flex;align-items:flex-start;gap:6px;margin:0 0 6px 11px">
        <div style="flex:1;min-width:0">
          <div style="font-size:.82em;font-weight:600;margin-bottom:1px">${d(w)}${D?` <span style="color:${B};font-size:.85em" title="${d(this._t("lbl.pg_drag_hint",{},"Drag line on graph"))}">\u2195</span>`:""}</div>
          ${k?`<div style="font-size:.72em;color:var(--secondary-text-color);line-height:1.3">${d(this._t("pg_desc."+_,{},k))}</div>`:""}
        </div>
        <div style="display:flex;align-items:center;gap:4px;flex-shrink:0">
          ${I?`<input class="wd-pg-param-chk" type="checkbox" data-pgkey="${d(_)}" data-pgtype="bool" aria-label="${d(w)}" ${T?"checked":""} style="width:18px;height:18px;margin:1px 27px 0 27px">`:`<input class="wd-pg-param-inp" type="text" inputmode="decimal" data-pgkey="${d(_)}" value="${T!==""?d(String(T)):""}" placeholder="${v!==""?d(String(v)):""}" aria-label="${d(w)}" style="width:72px">`}
      ...
      </div>`}const i=e.summary||{},l=`<div style="font-size:.82em;color:var(--secondary-text-color);margin-bottom:8px">${this._t("msg.pg_history_summary",{detected:i.detected,correct:i.match_correct,total:i.cycles},`${i.detected}/${i.cycles} detected \xB7 ${i.match_correct} matched correctly`)}</div>`,c=g=>(this._cycles||[]).find(k=>k.id===g),p={};(e.baseline_rows||[]).forEach(g=>{p[g.cycle_id]=g});const _=e.rows.map(g=>{const k=c(g.cycle_id),$=k&&k.start_time?rt(k.start_time):(g.cycle_id||"").slice(0,8),x=g.match_correct===!0?"\u2713":g.match_correct===!1?"\u2717":"\u2014",w=g.match_correct===!0?"var(--success-color,#4caf50)":g.match_correct===!1?"var(--error-color,#f44336)":"var(--secondary-text-color)",v=g.matched_profile||this._t("lbl.unlabelled",{},"Unlabelled"),T=g.duration_s?Math.round(g.duration_s/60)+"m":"\u2014",I=g.overrun_ratio!=null?` (${Math.round(g.overrun_ratio*100)}%)`:"",D=g.alerts&&g.alerts.length?` <span title="${d(g.alerts.join(", "))}" style="color:var(--warning-color,#ff9800)">\u26A0</span>`:"",F=p[g.cycle_id];let B="";return F&&(F.match_correct!==!0&&g.match_correct===!0?B=`<span style="color:var(--success-color,#4caf50)">\u25B2 ${this._t("lbl.pg_fixed",{},"fixed")}</span>`:F.match_correct===!0&&g.match_correct!==!0?B=`<span style="color:var(--error-color,#f44336)">\u25BC ${this._t("lbl.pg_broke",{},"broke")}</span>`:String(F.termination_reason)!==String(g.termination_reason)&&(B=`<span style="color:var(--warning-color,#ff9800)">${d(String(F.termination_reason||"\u2014"))}\u2192${d(String(g.termination_reason||"\u2014"))}</span>`)),`<tr class="wd-pg-hrow${g.cycle_id===this._pgCycleId?" selected":""}" data-action="pg-open-cycle" data-cid="${d(g.cycle_id)}" title="${d(this._t("msg.pg_row_load_hint",{},"Load this cycle in the graph above"))}">
        <td>${d($)}</td>
        <td><span style="color:${w};font-weight:700">${x}</span> ${d(v)}</td>
        <td>${d(String(g.termination_reason||"\u2014"))}</td>
        <td style="font-variant-numeric:tabular-nums">${T}${I}${D}</td>
        ${e.diff?`<td>${B}</td>`:""}
    ...
    </tr></thead><tbody>${_}</tbody></table>`;return`${a}${r}${this._htmlPgRecentRuns("pg_history")}${o}${l}${u}`}_htmlPgBatchBar(){const t=this._pgBatchProgress;return t?`<div class="wd-pg-batchrow">
    ...
    </div>`,o=t?this._htmlPgBatchBar():"";return`${n}${a}${this._htmlPgRecentRuns("pg_sweep")}${o}${this._htmlPgSweepResult()}`}_htmlPgSweepResult(){const t=this._pgSweepNew;if(!t||!Array.isArray(t.points)||!t.points.length)return"";const e=this._pgSweepObjectives().find(_=>_[0]===t.objective),s=e?e[2]:!1,r=t.points.filter(_=>_.metric!=null).map(_=>_.metric);if(!r.length)return`<div class="wd-empty" style="padding:16px">${this._t("msg.pg_sweep_no_metric",{},"Not enough data to score this objective.")}</div>`;const n=Math.min(...r),a=Math.max(...r),o=s?Math.min(...r):Math.max(...r),i=(t.points.find(_=>_.metric===o)||{}).value,l=_=>t.objective==="median_overrun"?Math.round(_*100)+"% off":Math.round(_*100)+"%",c=t.points.map(_=>{const u=a>n&&_.metric!=null?(_.metric-n)/(a-n):_.metric!=null?1:0,g=_.metric===o,k=t.current_value!=null&&Math.abs(_.value-t.current_value)<1e-6,$=g?"var(--success-color,#4caf50)":"var(--primary-color)";return`<div style="display:flex;align-items:center;gap:8px;margin:3px 0;font-size:.82em">
        <span style="flex:0 0 74px;text-align:right;font-variant-numeric:tabular-nums">${d(String(_.value))}${k?" \u25C0":""}</span>
      ...
      <span style="font-size:.75em;color:var(--secondary-text-color);text-transform:uppercase">${this._t("lbl.phase",{},"Phase")} <span id="wd-pg-phase" style="color:var(--primary-text-color,inherit)">\u2014</span></span>
        ...
        <span class="wd-pg-cand-pct">${i}%</span>
      </div>`}return`<div>${s||`<p class="wd-info" style="margin:0">${this._t("msg.pg_analysis_hint2",{},"Pick a cycle and press Run to load match analysis.")}</p>`}</div>`}async _pgLoad(){const t=this._devices[this._selIdx];if(!t||this._pgLoading)return;const e=this._pgCycleId||this._cycles?.[0]?.id||"";if(!e)return;this._pgCycleId=e,this._pgLoading=!0,this._pgView=null,this._pgHoverT=null,this._pgPowerPts=null,this._pgDtwData=null,this._pgEnvData=null,this._pgDetail=null;const s=++this._pgLoadSeq;this._render();try{const r=await this._ws({type:`${y}/get_cycle_power_data`,entry_id:t.entry_id,cycle_id:e}),n=r.samples||[],a=[];for(const l of n){if(!Array.isArray(l)||l.length<2)continue;const c=+l[0],p=+l[1];!isNaN(c)&&!isNaN(p)&&a.push({t:c,w:p})}if(this._pgPowerPts=a.length?a:null,typeof r.full_duration_s=="number"&&r.full_duration_s>0){const l=(this._cycles||[]).find(c=>c.id===e);l&&(l._pg_duration=r.full_duration_s)}const o=this._pgProfileName||(this._cycles||[]).find(l=>l.id===e)?.profile_name||"";if(a.length)try{const l={type:`${y}/get_dtw_debug`,entry_id:t.entry_id,cycle_id:e};o&&(l.profile_name=o),this._pgDtwData=await this._ws(l),this._pgNeedsRestart=!1}catch(l){this._pgIsUnknownCmd(l)&&(this._pgNeedsRestart=!0),this._pgDtwData=null}const i=this._pgDtwData?.profile_name||o;if(i)try{const l=await this._ws({type:`${y}/get_profile_envelope`,entry_id:t.entry_id,profile_name:i});this._pgEnvData=l.envelope||null}catch{this._pgEnvData=null}if(s!==this._pgLoadSeq)return;await this._pgLoadDetail(t.entry_id,e)}catch(r){this._showToast(this._t("msg.toast_error",{error:r.message||r},"Error: "+(r.message||r)),"error")}s===this._pgLoadSeq&&(this._pgLoading=!1,this._render(),requestAnimationFrame(()=>{this._pgDrawCanvas(),this._pgUpdateStripAt(null)}),this._pgNeedsRestart&&this._tab==="playground"?(this._pgRestartRetries||0)<5&&!this._pgRestartRetryTimer&&(this._pgRestartRetries=(this._pgRestartRetries||0)+1,this._pgRestartRetryTimer=setTimeout(()=>{this._pgRestartRetryTimer=null,this._pgNeedsRestart&&this._tab==="playground"&&this._pgLoad()},3e3)):this._pgRestartRetries=0)}_pgCancelRun(){this._pgLoadSeq++,this._pgLoading=!1,this._render()}_pgSelectCycle(t){t&&(this._pgCycleId=t,this._pgProfileName="",this._pgView=null,this._pgHoverT=null,this._pgLoading=!1,this._pgLoad(),requestAnimationFrame(()=>{const e=this.shadowRoot&&this.shadowRoot.getElementById("wd-pg-canvas");e&&typeof e.scrollIntoView=="function"&&e.scrollIntoView({behavior:"smooth",block:"center"})}))}async _pgLoadDetail(t,e){const s={...this._pgParamOverrides};this._pgThreshStart!=null&&(s.start_threshold_w=this._pgThreshStart),this._pgThreshStop!=null&&(s.stop_threshold_w=this._pgThreshStop);try{const r=await this._ws({type:`${y}/start_playground_cycle_detail`,entry_id:t,cycle_id:e,settings_override:s,stress_tail:this._pgStressTail,stress_idle_w:this._pgStressIdleW!=null?parseFloat(this._pgStressIdleW):null}),n=r&&r.task_id;if(!n)throw new Error("no task id");this._addProvisionalTask(n,"pg_detail",t,0);const a=await new Promise(o=>{this._taskCallbacks[n]=async l=>{if(l.state==="done"||l.state==="cancelled")try{const c=await this._ws({type:`${y}/get_task_result`,task_id:l.id});o(c&&c.result)}catch{o(null)}else o(null)};const i=this._tasks[n];i&&i.state!=="running"?this._settleTaskCallback(i):this._tasksSubscribed||this._pollTaskGeneric(n)});if(!this._isActiveEntry(t))return;this._pgDetail=a&&!a.error?a:null,this._pgNeedsRestart=!1}catch(r){this._pgIsUnknownCmd(r)&&(this._pgNeedsRestart=!0),this._pgDetail=null}}_pgRerunDetail(){const t=this._devices[this._selIdx];!t||!this._pgCycleId||(clearTimeout(this._pgDetailDebounceTimer),this._pgDetailDebounceTimer=setTimeout(async()=>{this._pgDetailBusy=!0,this._render(),await this._pgLoadDetail(t.entry_id,this._pgCycleId),this._pgDetailBusy=!1,this._render(),requestAnimationFrame(()=>this._pgDrawCanvas())},220))}_pgMapState(t){return t==="running"||t==="paused"?"running":t==="ending"?"ending":t==="starting"?"detecting":t==="anti_wrinkle"?"anti_wrinkle":"idle"}_pgSeriesAt(t){const e=this._pgDetail&&this._pgDetail.series;if(!e||!e.length)return null;let s=e[0];for(const r of e)if(r.t<=t)s=r;else break;return s}_pgStateSegsFromSeries(t){const e=this._pgDetail&&this._pgDetail.series;if(!e||!e.length)return[];const s=[];let r=null;for(const n of e){const a=this._pgMapState(n.state);!r||r.state!==a?(r={start:n.t,end:n.t,state:a},s.push(r)):r.end=n.t}return s.length&&(s[s.length-1].end=t),s}_pgDrawCanvas(){if(this._tab!=="playground")return;const t=this.shadowRoot,e=t&&t.getElementById("wd-pg-canvas");if(!e)return;const s=this._pgPowerPts,r=e.getBoundingClientRect(),n=window.devicePixelRatio||1,a=Math.max(1,Math.round(r.width*n)),o=Math.max(1,Math.round((r.height||280)*n));(e.width!==a||e.height!==o)&&(e.width=a,e.height=o);const i=e.getContext("2d"),l=getComputedStyle(this),c=(l.getPropertyValue("--primary-color")||"#03a9f4").trim(),p=(l.getPropertyValue("--divider-color")||"rgba(127,127,127,.2)").trim(),_=(l.getPropertyValue("--secondary-text-color")||"#888").trim(),u=(l.getPropertyValue("--secondary-background-color")||"#1a1a1a").trim();i.clearRect(0,0,a,o);const g=34*n,k=14*n,$=Ht*n,x=44*n,w=8*n,v=$+8*n,T=g+k+4*n,I=o-v-T;if(!s||!s.length){this._pgLoading&&(i.fillStyle=_,i.font=`${12*n}px sans-serif`,i.textAlign="center",i.textBaseline="middle",i.fillText(this._t("msg.loading",{},"Loading\u2026"),a/2,o/2));return}const D=this._pgDetail&&this._pgDetail.outcome&&this._pgDetail.outcome.stress,F=D&&D.enabled?D.synthetic_from_s:null,B=F!=null&&this._pgDetail&&this._pgDetail.series?this._pgDetail.series.filter(C=>C.t>=F):[];let R=(this._cycles||[]).find(C=>C.id===this._pgCycleId)?._pg_duration||s[s.length-1].t||1;D&&D.terminated&&D.terminated_after_s!=null&&F!=null?R=Math.max(R,F+D.terminated_after_s):B.length&&(R=Math.max(R,B[B.length-1].t));const P=Math.max(...s.map(C=>C.w),...B.map(C=>C.power||0),1),A=this._pgThreshStart??this._pgFieldVal("start_threshold_w",{})??50,M=this._pgThreshStop??this._pgFieldVal("stop_threshold_w",{})??5;let O=0,G=R;this._pgView&&this._pgView.max-this._pgView.min>1&&(O=Math.max(0,this._pgView.min),G=Math.min(R,this._pgView.max),G-O<=1&&(O=0,G=R));const et=Math.max(1e-6,G-O),st=a-x-w,J=C=>x+(C-O)/et*st,tt=C=>v+(1-Math.max(0,C)/P)*I;this._pgMap={vMin:O,vMax:G,totalDur:R,padLpx:x/n,plotWpx:st/n},i.strokeStyle="rgba(127,127,127,0.12)",i.lineWidth=n,i.setLineDash([]),[.25,.5,.75,1].map(C=>Math.round(C*P/100)*100||Math.round(C*P)).forEach(C=>{const z=tt(C);z<v||z>v+I||(i.beginPath(),i.moveTo(x,z),i.lineTo(a-w,z),i.stroke(),i.fillStyle=_,i.font=`${9*n}px sans-serif`,i.textAlign="right",i.textBaseline="middle",i.fillText(C+"W",x-4*n,z))}),i.save(),i.beginPath(),i.rect(x,v,st,I),i.clip();const U=this._pgEnvData,ht=U&&Array.isArray(U.avg)?U.avg.filter(C=>Array.isArray(C)&&C.length>=2):[];if(ht.length){const C=Math.max(...ht.map(K=>K[0]))||1,z=K=>J(K/C*R),Y=Array.isArray(U.min)?U.min.filter(K=>Array.isArray(K)&&K.length>=2):[],X=Array.isArray(U.max)?U.max.filter(K=>Array.isArray(K)&&K.length>=2):[];if(Y.length&&X.length){i.beginPath(),X.forEach((K,gt)=>gt?i.lineTo(z(K[0]),tt(K[1])):i.moveTo(z(K[0]),tt(K[1])));for(let K=Y.length-1;K>=0;K--)i.lineTo(z(Y[K][0]),tt(Y[K][1]));i.closePath(),i.fillStyle="#eda10012",i.fill()}else i.beginPath(),i.strokeStyle="#eda100",i.lineWidth=2*n,i.setLineDash([5*n,4*n]),ht.forEach((K,gt)=>gt?i.lineTo(z(K[0]),tt(K[1])):i.moveTo(z(K[0]),tt(K[1]))),i.stroke(),i.setLineDash([])}const Z=this._pgDtwData,ot=Z&&Array.isArray(Z.profile_trace)?Z.profile_trace.filter(C=>Array.isArray(C)&&C.length>=2):[],pt=Z&&Array.isArray(Z.cycle_trace)?Z.cycle_trace.filter(C=>Array.isArray(C)&&C.length>=2):[],m=Z&&Array.isArray(Z.warp_path)?Z.warp_path:[];if(pt.length&&ot.length&&m.length){const C=Math.max(...ot.map(X=>X[0]))||1,z=Math.max(...pt.map(X=>X[0]))||1,Y=Math.max(1,Math.floor(m.length/25));i.save(),i.globalAlpha=.13,i.strokeStyle="#fff",i.lineWidth=n;for(let X=0;X<m.length;X+=Y){const K=m[X];if(!Array.isArray(K)||K.length<2)continue;const gt=Math.min(K[0],pt.length-1),ft=Math.min(K[1],ot.length-1),St=J(pt[gt][0]/z*R),mt=tt(pt[gt][1]),$t=J(ot[ft][0]/C*R),ut=tt(ot[ft][1]);i.beginPath(),i.moveTo(St,mt),i.lineTo($t,ut),i.stroke()}i.restore()}if(ot.length){const C=Math.max(...ot.map(z=>z[0]))||1;i.beginPath(),i.strokeStyle="#eda100",i.lineWidth=2*n,i.setLineDash([6*n,4*n]),ot.forEach((z,Y)=>{const X=J(z[0]/C*R),K=tt(z[1]);Y?i.lineTo(X,K):i.moveTo(X,K)}),i.stroke(),i.setLineDash([])}if(i.beginPath(),i.moveTo(J(0),tt(0)),s.forEach(C=>i.lineTo(J(C.t),tt(C.w))),i.lineTo(J(s[s.length-1].t),tt(0)),i.closePath(),i.fillStyle=Pt(c,.1),i.fill(),i.beginPath(),i.strokeStyle=c,i.lineWidth=2*n,s.forEach((C,z)=>z?i.lineTo(J(C.t),tt(C.w)):i.moveTo(J(C.t),tt(C.w))),i.stroke(),B.length){const C=s[s.length-1];i.beginPath(),i.moveTo(J(C.t),tt(C.w)),B.forEach(z=>i.lineTo(J(z.t),tt(z.power))),i.lineTo(J(B[B.length-1].t),tt(0)),i.lineTo(J(C.t),tt(0)),i.closePath(),i.fillStyle=Pt(c,.06),i.fill(),i.beginPath(),i.strokeStyle=c,i.lineWidth=1.5*n,i.setLineDash([4*n,4*n]),i.moveTo(J(C.t),tt(C.w)),B.forEach(z=>i.lineTo(J(z.t),tt(z.power))),i.stroke(),i.setLineDash([])}if(F!=null&&F<G){const C=J(F);i.fillStyle="rgba(192,57,43,0.07)",i.fillRect(C,v,Math.max(0,x+st-C),I),i.save(),i.strokeStyle="rgba(192,57,43,0.5)",i.lineWidth=n,i.setLineDash([4*n,4*n]),i.beginPath(),i.moveTo(C,v),i.lineTo(C,v+I),i.stroke(),i.restore()}i.restore();const f=(C,z,Y)=>{const X=tt(C);X<v-2||X>v+I+2||(i.save(),i.strokeStyle=z,i.lineWidth=2*n,i.setLineDash([8*n,4*n]),i.beginPath(),i.moveTo(x,X),i.lineTo(a-w,X),i.stroke(),i.setLineDash([]),i.fillStyle=z,i.beginPath(),i.arc(x+(a-x-w)*.06,X,5*n,0,Math.PI*2),i.fill(),i.fillStyle=z,i.font=`bold ${9*n}px sans-serif`,i.textAlign="left",i.textBaseline="bottom",i.fillText(Y+" "+Math.round(C)+"W",x+14*n,X-2*n),i.restore())};f(+A,"#2a78d6",this._t("lbl.start",{},"Start")),f(+M,"#e34948",this._t("btn.stop",{},"Stop"));const S={idle:u,detecting:"#42a5f566",running:"#66bb6a66",ending:"#ef535066",anti_wrinkle:"#ab47bc66"},b={idle:this._t("lbl.pg_idle",{},"Idle"),detecting:this._t("lbl.pg_detecting",{},"Detecting"),running:this._t("lbl.pg_ev_running",{},"Running"),ending:this._t("lbl.pg_ev_ending",{},"Ending"),anti_wrinkle:this._t("lbl.pg_anti_wrinkle",{},"Anti-wrinkle")},E=o-g-k;i.fillStyle=u,i.fillRect(x,E,a-x-w,g),i.save(),i.beginPath(),i.rect(x,E,st,g),i.clip(),this._pgStateSegsFromSeries(R).forEach(C=>{const z=J(C.start),Y=J(C.end);i.fillStyle=S[C.state]||p,i.fillRect(z,E,Math.max(1,Y-z),g),Y-z>50*n&&(i.fillStyle=_,i.font=`${8*n}px sans-serif`,i.textAlign="center",i.textBaseline="middle",i.fillText((b[C.state]||C.state).toUpperCase(),(z+Y)/2,E+g/2))}),i.restore();const N=(this._cycles||[]).find(C=>C.id===this._pgCycleId),H=this._pgDtwData?.profile_name||this._pgProfileName||N?.profile_name,j=H?(this._profiles||[]).find(C=>C.name===H):null,q=o-k;j&&Array.isArray(j.phases)&&j.phases.length&&(i.save(),i.beginPath(),i.rect(x,q,st,k),i.clip(),j.phases.forEach((C,z)=>{const Y=J((C.start||0)*R),X=J((C.end||1)*R),K=z*47%360;i.fillStyle=`hsla(${K},60%,55%,0.55)`,i.fillRect(Y,q,Math.max(1,X-Y),k),X-Y>40*n&&(i.fillStyle="#fff",i.font=`${7*n}px sans-serif`,i.textAlign="center",i.textBaseline="middle",i.fillText(C.name,(Y+X)/2,q+k/2))}),i.restore()),this._pgEventHits=[];const L=this._pgDetail&&Array.isArray(this._pgDetail.events)?this._pgDetail.events.filter(C=>C.type!=="state"&&C.t>=O&&C.t<=G).slice().sort((C,z)=>C.t-z.t):[],V=11*n,Q=v-$/2-2*n,lt=V*2+3*n;let at=-1/0;const nt=this._pgHoverEvent;if(L.forEach(C=>{const z=this._pgEventMeta(C.type),Y=J(C.t);let X=Math.max(Y,at+lt);X=Math.max(x+V,Math.min(a-w-V,X)),at=X;const K=nt&&nt.t===C.t&&nt.type===C.type;i.save(),i.strokeStyle=z.color,i.globalAlpha=K?.9:.5,i.lineWidth=(K?2:1)*n,i.beginPath(),i.moveTo(X,Q+V),i.lineTo(Y,v),i.stroke(),K&&(i.globalAlpha=.3,i.setLineDash([2*n,3*n]),i.beginPath(),i.moveTo(Y,v),i.lineTo(Y,o-k),i.stroke(),i.setLineDash([])),i.restore(),i.save(),i.beginPath(),i.arc(X,Q,V,0,Math.PI*2),i.fillStyle=K?z.color:u||"#1a1a1a",i.fill(),i.lineWidth=(K?2:1.5)*n,i.strokeStyle=z.color,i.stroke(),i.fillStyle=K?"#fff":z.color,i.font=`${14*n}px sans-serif`,i.textAlign="center",i.textBaseline="middle",i.fillText(z.glyph,X,Q+n),i.restore(),this._pgEventHits.push({cx:X/n,cy:Q/n,r:V/n,type:C.type,label:z.label,detail:C.detail,t:C.t})}),nt){const C=this._pgEventHits.find(z=>z.t===nt.t&&z.type===nt.type);if(C){const z=this._pgEventMeta(C.type),Y=bt=>`${Math.floor(bt/60)}:${String(Math.round(bt%60)).padStart(2,"0")}`,X=this._pgEventDescription(C.type),K=[{t:C.label,bold:!0,col:z.color},...X?[{t:X}]:[],...C.detail?[{t:C.detail,dim:!0}]:[],{t:`${this._t("lbl.from_start",{},"From start")} ${Y(C.t)}`,dim:!0}];i.save(),i.font=`${10*n}px sans-serif`;const gt=Math.min(220*n,a-2*x),ft=[];K.forEach(bt=>{const It=String(bt.t).split(" ");let Tt="";It.forEach(Ft=>{const Ut=Tt?Tt+" "+Ft:Ft;i.measureText(Ut).width>gt&&Tt?(ft.push({...bt,t:Tt}),Tt=Ft):Tt=Ut}),ft.push({...bt,t:Tt})});const St=14*n,mt=Math.min(gt+14*n,a-8*n),$t=ft.length*St+10*n;let kt=C.cx*n-mt/2;kt=Math.max(4*n,Math.min(a-mt-4*n,kt));const Ot=Q+V+6*n;i.fillStyle=u,i.globalAlpha=.97,i.fillRect(kt,Ot,mt,$t),i.globalAlpha=1,i.strokeStyle=z.color,i.lineWidth=n,i.strokeRect(kt,Ot,mt,$t),i.textAlign="left",i.textBaseline="top",ft.forEach((bt,It)=>{i.font=`${bt.bold?"600 ":""}${10*n}px sans-serif`,i.fillStyle=bt.col||(bt.dim?_:(l.getPropertyValue("--primary-text-color")||"#ddd").trim()),i.fillText(bt.t,kt+7*n,Ot+5*n+It*St)}),i.restore()}}if(this._pgHoverT!=null&&this._pgHoverT>=O&&this._pgHoverT<=G){const C=J(this._pgHoverT);i.save(),i.strokeStyle="#e34948",i.lineWidth=1.5*n,i.setLineDash([4*n,3*n]),i.beginPath(),i.moveTo(C,v),i.lineTo(C,o-k),i.stroke(),i.setLineDash([]);const z=this._pgInterpPower(s,this._pgHoverT),Y=tt(z);i.fillStyle="#e34948",i.beginPath(),i.arc(C,Y,3.5*n,0,Math.PI*2),i.fill();const X=ut=>`${Math.floor(ut/60)}:${String(Math.round(ut%60)).padStart(2,"0")}`,K=ut=>ut>=1e3?(ut/1e3).toFixed(2)+" kW":Math.round(ut)+" W",gt=[`${this._t("lbl.from_start",{},"From start")} ${X(this._pgHoverT)}`,`${this._t("lbl.to_end",{},"To end")} ${X(Math.max(0,R-this._pgHoverT))}`,`${this._t("lbl.power",{},"Power")} ${K(z)}`];i.font=`${9.5*n}px sans-serif`;const ft=Math.max(...gt.map(ut=>i.measureText(ut).width))+12*n,St=gt.length*13*n+8*n;let mt=C+8*n;mt+ft>a-w&&(mt=C-ft-8*n);const $t=v+4*n;i.fillStyle=u,i.globalAlpha=.95,i.fillRect(mt,$t,ft,St),i.globalAlpha=1,i.strokeStyle=p,i.lineWidth=n,i.strokeRect(mt,$t,ft,St),i.fillStyle=_,i.textAlign="left",i.textBaseline="top",gt.forEach((ut,kt)=>i.fillText(ut,mt+6*n,$t+5*n+kt*13*n)),i.restore()}i.fillStyle=_,i.font=`${9*n}px sans-serif`,i.textAlign="center",i.textBaseline="top";const dt=C=>`${Math.floor(C/60)}:${String(Math.round(C%60)).padStart(2,"0")}`;for(let C=0;C<=4;C++){const z=O+et*C/4,Y=J(z);i.textAlign=C===0?"left":C===4?"right":"center",i.fillText(dt(z),Math.max(x,Math.min(a-w,Y)),E+g+1*n)}et<R-1&&(i.fillStyle=_,i.font=`${8*n}px sans-serif`,i.textAlign="right",i.textBaseline="top",i.fillText(this._t("lbl.pg_zoom_hint",{},"double-click to reset zoom"),a-w,v))}_pgEventMeta(t){const s={detected:["\u25CF","#42a5f5",this._t("lbl.pg_ev_detected",{},"Detected")],match_commit:["\u25C6","#66bb6a",this._t("lbl.pg_ev_match",{},"Match committed")],match_changed:["\u25C7","#eda100",this._t("lbl.pg_ev_match_changed",{},"Match changed")],match_ambiguous:["\u25C8","#eda100",this._t("lbl.pg_ev_ambiguous",{},"Ambiguous")],unmatched:["\u25CB","#9e9e9e",this._t("lbl.pg_ev_unmatched",{},"Unmatched")],notify_start:["\u{1F514}","#2a78d6",this._t("lbl.pg_ev_notify_start",{},"Start notification")],notify_pre_complete:["\u{1F514}","#2a78d6",this._t("lbl.pg_ev_notify_pre",{},"Almost-done notification")],notify_finish:["\u{1F514}","#2a78d6",this._t("lbl.pg_ev_notify_finish",{},"Finish notification")],notify_milestone:["\u{1F3C6}","#2a78d6",this._t("lbl.pg_ev_notify_milestone",{},"Milestone notification")],notify_held:["\u{1F319}","#7e57c2",this._t("lbl.pg_ev_notify_held",{},"Notification held (quiet hours)")],finished:["\u2713","#4caf50",this._t("lbl.pg_ev_finished",{},"Finished")]}[t]||["\u2022","var(--secondary-text-color)",t];return{glyph:s[0],color:s[1],label:s[2]}}_pgEventDescription(t){return{detected:this._t("pg_evd.detected",{},"The detector recognized a cycle had started."),match_commit:this._t("pg_evd.match_commit",{},"The matcher committed to a program with enough confidence."),match_changed:this._t("pg_evd.match_changed",{},"The leading program changed as more of the cycle was seen."),match_ambiguous:this._t("pg_evd.match_ambiguous",{},"Two programs scored close together, so the match was uncertain."),unmatched:this._t("pg_evd.unmatched",{},"No saved program fit this cycle."),notify_start:this._t("pg_evd.notify_start",{},"A start notification would be sent."),notify_pre_complete:this._t("pg_evd.notify_pre_complete",{},"An almost-done notification would be sent."),notify_finish:this._t("pg_evd.notify_finish",{},"A finish notification would be sent."),notify_milestone:this._t("pg_evd.notify_milestone",{},"A milestone notification would be sent."),notify_held:this._t("pg_evd.notify_held",{},"A notification was held back for quiet hours."),finished:this._t("pg_evd.finished",{},"The cycle reached a terminal state and ended.")}[t]||""}_pgUpdateParamInput(t,e){const s=this.shadowRoot,r=s&&s.querySelector(`input[data-pgkey="${t}"]`);r&&(r.dataset.pgtype==="bool"?r.checked=!!e:r.value=typeof e=="number"?Math.round(e):e)}_pgUpdateStripAt(t){const e=this._pgPowerPts;if(!e?.length)return;const r=(this._cycles||[]).find(R=>R.id===this._pgCycleId)?._pg_duration||e[e.length-1].t||1,n=t??r,a=this._pgInterpPower(e,n),o=this._pgSeriesAt(n),i=o?this._pgMapState(o.state):"idle",l={idle:[this._t("lbl.pg_idle",{},"Idle"),"var(--secondary-background-color)"],detecting:[this._t("lbl.pg_detecting",{},"Detecting"),"#42a5f5"],running:[this._t("lbl.pg_ev_running",{},"Running"),"#66bb6a"],ending:[this._t("lbl.pg_ev_ending",{},"Ending"),"#ef5350"],anti_wrinkle:[this._t("lbl.pg_anti_wrinkle",{},"Anti-wrinkle"),"#ab47bc"]},[c,p]=l[i]||l.idle,_=o&&o.progress!=null?Math.round(o.progress):null,u=o&&o.remaining_s!=null?o.remaining_s:null,g=o&&o.energy_wh!=null?o.energy_wh:this._pgTrapEnergy(e,n),k=o&&o.confidence!=null?Math.round(o.confidence*100)+"%":"\u2014",$=o&&o.phase?o.phase:"\u2014",x=R=>Math.floor(R/60)+":"+String(Math.round(R%60)).padStart(2,"0"),w=R=>R>=1e3?(R/1e3).toFixed(2)+" kWh":R.toFixed(0)+" Wh",v=R=>R>=1e3?(R/1e3).toFixed(1)+" kW":Math.round(R)+" W",T=this.shadowRoot,I=R=>T&&T.getElementById(R),D=(R,P)=>{const A=I(R);A&&(A.textContent=P)},F=(R,P,A)=>{const M=I(R);M&&(M.style[P]=A)},B=I("wd-pg-state-badge");B&&(B.textContent=c,B.style.background=p,B.style.color=p.includes("var")?"":"#fff"),D("wd-pg-power",v(a)),D("wd-pg-pct",_!=null?_+"%":"\u2014%"),F("wd-pg-pbar","width",(_??0)+"%"),D("wd-pg-rem",u!=null?x(u):"\u2014"),D("wd-pg-energy",w(g)),D("wd-pg-conf",k),D("wd-pg-phase",$)}_pgIsUnknownCmd(t){const e=t&&(t.code||t.error&&t.error.code)||"",s=(t&&(t.message||t.error)||"").toString().toLowerCase();return e==="unknown_command"||s.includes("unknown command")||s.includes("unknown_command")}_pgInterpPower(t,e){if(!t.length)return 0;if(e<=t[0].t)return t[0].w;for(let s=1;s<t.length;s++)if(e<=t[s].t){const r=t[s].t-t[s-1].t;if(r<=0)return t[s].w;const n=(e-t[s-1].t)/r;return t[s-1].w+n*(t[s].w-t[s-1].w)}return t[t.length-1].w}_pgTrapEnergy(t,e){let s=0;for(let r=1;r<t.length;r++){const n=t[r-1].t,a=t[r-1].w,o=Math.min(t[r].t,e),i=this._pgInterpPower(t,o);if(!(o<=n)&&(s+=(a+i)/2*(o-n)/3600,o>=e))break}return s}_drawPlaygroundCanvases(){this._tab==="playground"&&this._pgDrawCanvas()}_htmlPhases(){const t=this._devices[this._selIdx],e=t&&t.options.device_type||"washing_machine",s=this._canEdit(),r=this._phases.map(o=>{const i=o.is_default,l=o.translation_key?this._t(o.translation_key,{},o.description||""):o.description||"",c=s?`
        <td>
      ...
      </div>`}).join(""):`<p class="wd-info">${this._t("msg.no_maintenance",{},"No maintenance recorded yet.")}</p>`,k=t?`<div class="wd-card">
      ...
      <div class="wd-card-actions"><button class="wd-btn wd-btn-primary" data-action="maint-save-reminders">${this._t("btn.save_reminders",{},"Save reminders")}</button></div>
    </div>`:"",$=s.map(v=>{const T=parseInt(a[v],10)||0;if(T<=0)return"";const I=Math.max(0,parseInt(o[v],10)||0),D=Math.max(0,Math.min(100,Math.round(I/T*100))),B=I>=T?"var(--warning-color,#ff9800)":"var(--primary-color)";return`<div style="margin-top:10px">
        ...
        <div style="height:6px;margin-top:4px;border-radius:3px;background:var(--divider-color);overflow:hidden">
          <div style="height:100%;border-radius:3px;width:${D}%;background:${B}"></div>
        </div>
    ...
    ${r||`<div class="wd-card"><p class="wd-info">${this._t("msg.no_other_users",{},"No other Home Assistant users found.")}</p></div>`}`}_htmlStore(){if(!this._canEdit())return`<div class="wd-empty">${this._t("msg.no_device_selected",{},"No device selected.")}</div>`;const t=this._storeStatus,e=`<a href="https://3dg1luk43.github.io/washdata-store" target="_blank" rel="noopener noreferrer" style="font-size:.8em;font-weight:400;color:var(--primary-color);text-decoration:none;white-space:nowrap" title="${d(this._t("store.website_tip",{},"Open the community store website"))}">${this._t("store.website",{},"Store website \u2197")}</a>`,s=`<div style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap"><div class="wd-card-title" style="margin:0">${this._t("hdr.community_store",{},"Community Store")}</div>${e}</div>`;if(!this._onlineEnabled()||t&&t.enabled===!1)return`<div class="wd-card">${s}
      ...
      ${Vt(`data-action="store-toggle-dl-settings" ${this._dlSettings?"checked":""} ${n?"disabled":""}`,this._t("lbl.adopt_settings",{},"Also adopt settings"),ct(this._t("msg.adopt_settings_hint",{},"Overwrite this device's detection & matching thresholds with the shared ones. Your notifications, entities and energy price are never changed.")))}
      ...
      ${t?n:""}
    </div>`}_htmlStorePrefs(t){const e=this._constants&&this._constants.storePrefs||{},s=he.map(n=>{const a=e[n.key]!==!1;return Mt(`data-action="store-toggle-pref" data-pref="${d(n.key)}" ${a?"checked":""} ${t?"disabled":""}`,this._t(n.labelKey,{},n.labelFb),ct(this._t(n.docKey,{},n.docFb)))}).join(""),r=this._busy.has("store-refresh-catalog");return s+`
      ...
      <div class="wd-log-drawer-body">
        ...
        <button class="wd-btn wd-btn-primary" data-maction="store-share-ok" ${this._busy.has("store-share")?"disabled":""}>${this._busy.has("store-share")?'<span class="wd-spin"></span> ':""}${this._t("btn.share",{},"Share")}</button></div>`}else t.type==="store-share-device"&&(e=this._htmlShareDeviceModal(t));return`<div class="wd-overlay"><div class="wd-modal" role="dialog" aria-modal="true" aria-labelledby="wd-modal-title" tabindex="-1">${e}</div></div>`}_htmlShareDeviceModal(t){const e=this._shareableByProgram(),s=this._busy.has("store-share-device"),r=t.selected||new Set,n=!!t.consented,a=e.reduce((g,k)=>g+k.cycles.filter($=>r.has($.id)).length,0),o=e.some(g=>!g.noCycles);let i;t.loading?i=`<div class="wd-empty" style="padding:24px"><div class="wd-icon">\u23F3</div>${this._t("msg.loading",{},"Loading\u2026")}</div>`:e.length?i=e.map(g=>{if(g.noCycles)return`<div class="wd-sd-group wd-sd-group-nocyc">
          ...
          </div>`;const k=g.cycles.every(T=>r.has(T.id)),$=!k&&g.cycles.some(T=>r.has(T.id)),x=g.cycles.map(T=>{const I=T.start_time?rt(T.start_time):"",D=T.duration!=null?_t(T.duration):"";return`<label class="wd-sd-cyc">
            <input type="checkbox" data-maction="sd-toggle-cyc" data-cid="${d(T.id)}" ${r.has(T.id)?"checked":""} ${s?"disabled":""}>
            <span class="wd-sd-cyc-meta">${d(I)} \xB7 ${d(D)} \u2B50</span>
          ...
          <label class="wd-sd-prof">
            <input type="checkbox" data-maction="sd-toggle-prof" data-prog="${d(g.program)}" ${k?"checked":""} ${$?'data-indeterminate="1"':""} ${s?"disabled":""}>
            <span class="wd-sd-prof-name">${d(g.program)}</span>
            <span class="wd-sd-count">${g.cycles.filter(T=>r.has(T.id)).length}/${g.cycles.length}</span>
          </label>
      ...
      <span class="wd-sd-prof-name"><strong>${this._t("btn.select_all",{},"Select all")}</strong></span>
    </label>`,c=a.map(p=>{const _=r[p],u=s.importableOnly&&_.importable===!1,g=this._wizCatState(t,p,e),$=["profiles","real_cycles","reference_cycles"].includes(p)?`${g.sel}/${_.count}`:"",x=`<label class="wd-sd-prof">
          ...
          <span class="wd-sd-cyc-meta">${d(v.name)} \xB7 ${(v.real_cycles||0)+(v.reference_cycles||0)} ${this._t("lbl.cycles_short",{},"cycles")}${n.has(v.name)?` <span class="wd-tag" style="color:var(--warning-color,#e6a700)">${this._t("badge.exists",{},"exists")}</span>`:""}</span>
        </label>`).join("")}</div>`;else if(!u&&(p==="real_cycles"||p==="reference_cycles")){const v=p==="real_cycles"?t.sel.realIds:t.sel.refIds;w=`<div class="wd-sd-cycles">${(_.groups||[]).map(T=>{const I=T.cycles.map(M=>String(M.id)),D=I.filter(M=>v.has(M)).length,F=D===I.length&&I.length>0,B=D>0&&!F,R=`${p}:${T.profile}`,P=t.expanded&&t.expanded.has(R),A=P?T.cycles.map(M=>`<label class="wd-sd-cyc" style="margin-left:18px">
            ...
            <div class="wd-sd-cyc" style="display:flex;align-items:center">
              <label style="flex:1;display:flex;align-items:center;gap:8px;cursor:pointer">
                <input type="checkbox" data-maction="wiz-toggle-cycgroup" data-cat="${p}" data-prof="${d(T.profile)}" ${F?"checked":""} ${B?'data-indeterminate="1"':""}>
                <span class="wd-sd-cyc-meta"><strong>${d(T.profile||this._t("lbl.unlabelled",{},"Unlabelled"))}</strong> (${D}/${T.count})</span>
              </label>
    ...
    </div>`,p=r.real_history_allowed!==!1,_=`<div class="wd-field"><label>${this._t("lbl.cycle_destination",{},"Where should imported cycles go?")}</label>
      <div class="wd-mode-bar" style="display:flex;gap:8px">
    ...
    </div>`;let u="";const k=(r.categories&&r.categories.profiles&&r.categories.profiles.items||[]).filter(x=>x.conflict&&t.sel.profiles.has(x.name));t.mode==="merge"&&k.length&&(u=`<div class="wd-field"><label>${this._t("lbl.conflict_resolution",{},"Name clashes")}</label>
        ${k.map(x=>`<div style="display:flex;align-items:center;gap:8px;margin:4px 0">
      ...
      <div class="wd-modal-actions">
        ...
        <div class="wd-field">
          ...
          <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
            ...
            <input type="date" id="wd-hist-since" value="${d(t.since||Wt())}" min="${d(ye())}" max="${d(me())}" ${w}>
        ...
        </div>`;const k=n.map(x=>{const w=i.has(x.index),v=this._histSegReason(x.reason);return`<tr data-hist-row="${x.index}" style="${w?"":"opacity:.55"}">
      ...
      </tr>`}).join(""),$=n.every(x=>i.has(x.index));return`${e}
      ...
      ${c}
    </div>`,_=l&&l.ml_review||{},u=(this._feedbacks||[]).some(A=>A.cycle_id===t.cycleId),g=l&&l.ml_quality_label,k=!_.reviewed_at&&(u||["uncertain","review"].includes(g)||["force_stopped","interrupted"].includes(e.status)),$=k&&t.mode!=="review"?` <span title="${this._t("hdr.automation_needs_review",{},"This cycle needs review")}" style="color:var(--warning-color,#ff9800);font-size:1.1em;line-height:0">\u25CF</span>`:"",x=this._canEdit()&&n?`<div class="wd-mode-bar">
      ...
      <button class="wd-btn wd-btn-sm ${t.mode==="review"?"wd-btn-primary":"wd-btn-secondary"}" data-maction="cyc-review" title="${k?this._t("hdr.automation_needs_review",{},"This cycle needs review"):this._t("hdr.automation_review_this_cycle",{},"Review this cycle")}">${this._t("btn.review",{},"Review")}${$}</button>
    </div>`:s?`<div class="wd-info" style="margin:0 0 8px"><span style="color:var(--info-color,#2196f3)">\u{1F4E5}</span> ${a?this._t("msg.imported_history_readonly",{},"Detected in imported power history. It shapes program matching but is not counted in your statistics, and cannot be trimmed or split. Label it to name the program."):this._t("msg.imported_readonly",{},"Imported from the community store. Shown for reference and matching. It is not counted in your stats and cannot be edited.")}</div>`:"",w=this._canEdit()?(this._feedbacks||[]).find(A=>A.cycle_id===t.cycleId):null,v=w?w.detected_profile||w.profile_name||this._t("lbl.unknown",{},"Unknown"):"",T=w?`
        ...
        <div class="wd-form-grid">
          <div class="wd-field"><label>${this._t("lbl.start",{},"Start")}${J}</label><input type="${et}" id="wd-trim-start" ${st} value="${O}"></div>
          <div class="wd-field"><label>${this._t("lbl.end",{},"End")}${J}</label><input type="${et}" id="wd-trim-end" ${st} value="${G}"></div>
        </div>
        <div class="wd-modal-actions">
          <button class="wd-btn wd-btn-secondary" data-maction="cancel">${this._t("btn.close",{},"Close")}</button>
        ...
        </div>`}else if(t.mode==="split"){const A=this._busy.has("cyc-split-apply"),M=(t.split.offsets||[]).slice().sort((et,st)=>et-st),O=[0,...M,o],G=O.slice(0,-1).map((et,st)=>{const J=O[st+1];return`<div class="wd-seg-row"><span class="wd-swatch" style="background:${it[st%it.length]}"></span>
          <span style="min-width:120px">${_t(et)} \u2013 ${_t(J)}</span>
        ...
        </div>`}else if(t.mode==="review"){const A=l&&l.ml_review||{},M=this._busy.has("cyc-review-save"),O=(Z,ot)=>`<option value="${Z}" ${(A.quality||"")===Z?"selected":""}>${ot}</option>`,et=[["late_start",this._t("tag.late_start",{},"Late start")],["early_end",this._t("tag.early_end",{},"Early end")],["merged",this._t("tag.merged",{},"Merged cycles")],["split",this._t("tag.split",{},"Split cycle")],["noise",this._t("tag.noise",{},"Noise")],["wrong_profile",this._t("tag.wrong_profile",{},"Wrong profile")],["sensor_gap",this._t("tag.sensor_gap",{},"Sensor gap")]].map(([Z,ot])=>`<label class="wd-rev-tag"><input type="checkbox" class="wd-cyc-rev-tag" value="${Z}" ${(A.tags||[]).includes(Z)?"checked":""}> ${ot}</label>`).join(""),st=A.reviewed_at?`<span style="font-size:.75em;color:var(--secondary-text-color)">${this._t("lbl.reviewed_on",{date:new Date(A.reviewed_at).toLocaleDateString()},`reviewed ${new Date(A.reviewed_at).toLocaleDateString()}`)}</span>`:"",J=ct(this._t("msg.review_profile_tip",{},"The program this cycle is labelled as. If the auto-detected program was wrong, correct it here - labelling teaches matching for future cycles.")),tt=ct(this._t("msg.review_quality_tip",{},"How clean this cycle is. Good = a textbook example of this program; Bad = detected but noisy or atypical; Unusable = mis-detected (merged, truncated or spurious). Drives the health score and which cycles are allowed to train the model.")),wt=ct(this._t("msg.review_recorded_tip",{},'Mark this as a hand-picked reference cycle for its program - the same role as a manually recorded cycle. Reference cycles are always kept, seed the matching template, and are never dropped by cleanup. (This is the "golden"/recorded flag; both are the same thing.)')),U=ct(this._t("msg.review_tags_tip",{},"Optional flags describing what went wrong with this cycle, so training and cleanup can account for it.")),ht=ct(this._t("msg.review_notes_tip",{},"Free-text notes for your own reference. Not used by matching or training."));I=`
        ${T}
        ...
        <div style="display:flex;gap:16px;flex-wrap:wrap;align-items:center;margin:6px 0">
          <label style="display:inline-flex;align-items:center;gap:6px">${this._t("lbl.profile",{},"Profile")}${J}
          ...
          <label style="display:inline-flex;align-items:center;gap:6px">${this._t("lbl.quality",{},"Quality")}${tt}
        ...
        <div class="wd-rev-tags">${(this._profiles||[]).map(Z=>{const ot=(t.overlays||[]).includes(Z.name),pt=ot?`<span style="display:inline-block;width:10px;height:10px;border-radius:2px;background:${it[Math.max(0,this._profiles.findIndex(m=>m.name===Z.name))%it.length]};margin:0 2px"></span>`:"";return`<label class="wd-rev-tag"><input type="checkbox" class="wd-cyc-overlay" value="${d(Z.name)}" ${ot?"checked":""}> ${pt}${d(Z.name)}</label>`}).join("")||`<span class="wd-info">${this._t("msg.no_profiles_compare",{},"No profiles to compare.")}</span>`}</div>
        ...
        </div>`}let D="";const F=t.mode==="view"||t.mode==="review"?e.artifacts||[]:[];if(F.length){const A=F.map(M=>{const O=M.detail_key?this._t(M.detail_key,M.detail_params||{},M.detail||""):M.detail||"";return`<li><b>${d(ae(M.type,(G,et,st)=>this._t(G,et,st)))}</b> ${this._t("lbl.at",{},"at")} ${zt(M.start_s)}\u2013${zt(M.end_s)} \u2014 ${d(O)}</li>`}).join("");D=`<div class="wd-card" style="margin:10px 0 0;padding:10px 12px;border-left:3px solid var(--warning-color,#ff9800)">
      ...
      ${D}
      ${B}
      ${I}`}_htmlProfilePanel(t){const e=this._canEdit();t.tab==="danger"&&!e&&(t.tab="stats");const s=[["stats",this._t("tab.pp_overview",{},"Overview")],["phases",this._t("tab.pp_phases",{},"Phases")],["cleanup",this._t("tab.pp_cleanup",{},"Cleanup")]];e&&s.push(["danger",this._t("tab.pp_manage",{},"Manage")]);const r=s.map(([o,i])=>`<button class="wd-mini-tab ${t.tab===o?"active":""}" data-maction="pp-tab-${o}">${i}</button>`).join("");let n="";if(!t.loaded)n=`<div class="wd-empty" style="padding:32px"><div class="wd-icon">\u23F3</div>${this._t("msg.loading",{},"Loading\u2026")}</div>`;else if(t.tab==="stats"){const o=t.stats||{},i=t.env||{},l=this._hass&&this._hass.config&&this._hass.config.currency||"",c=o.avg_energy!=null&&o.cycle_count?o.avg_energy*o.cycle_count:null,p=$=>$?Math.round($/60)+"m":"-",_=(this._profileHealth||{})[t.name],u=(this._profileTrends||{})[t.name],g=_&&_.health_status!=="unknown"?(()=>{const $={healthy:["var(--success-color,#4caf50)","rgba(76,175,80,.12)"],fair:["var(--warning-color,#ff9800)","rgba(255,152,0,.12)"],poor:["var(--error-color,#f44336)","rgba(244,67,54,.12)"]},[x,w]=$[_.health_status]||$.fair,v=Math.round((_.health_score||0)*100),T=_.duration_cv!=null?` \xB7 ${this._t("stat.duration_cv",{pct:Math.round(_.duration_cv*100)},`duration CV ${Math.round(_.duration_cv*100)}%`)}`:"",I=_.confidence_mean!=null?` \xB7 ${this._t("stat.avg_confidence",{pct:Math.round(_.confidence_mean*100)},`avg confidence ${Math.round(_.confidence_mean*100)}%`)}`:"";return`<div style="margin:8px 0 4px;padding:8px 12px;border-radius:6px;background:${w};border:1px solid color-mix(in srgb, ${x} 13%, transparent);display:flex;align-items:center;gap:8px;flex-wrap:wrap">
        ...
        ${e?`<div class="wd-mode-bar">
          ...
          <td class="wd-tc-date">${rt(w.start_time)}</td>
        ...
        ${e?"<th></th>":""}
      </tr></thead>`,k=this._busy.has("pp-cleanup-del"),$=this._busy.has("pp-cleanup-unlabel"),x=k||$;n=`<p class="wd-info" style="margin-bottom:10px">${this._t("msg.cleanup_intro",{},"Every labelled cycle overlaid. Tick outliers, then unlabel them (kept in history, but no longer shaping this profile) or delete them outright.")}</p>
        ...
        ${e?`<div class="wd-modal-actions">
          ...
          <button class="wd-btn wd-btn-danger" data-maction="pp-cleanup-del" ${x||i.size===0?"disabled":""}>${k?'<span class="wd-spin"></span> '+this._t("status.deleting",{},"Deleting\u2026"):this._t("btn.delete_selected",{n:i.size},`Delete selected (${i.size})`)}</button>
