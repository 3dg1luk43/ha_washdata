...
`;function ot(h){if(h==null||h<0)return"-";const t=Math.floor(h/3600),e=Math.floor(h%3600/60),s=Math.floor(h%60);return t>0?`${t}h ${e}m`:e>0?`${e}m ${s}s`:`${s}s`}function Vt(h){return h==null?"-":h>=100?`${Math.round(h)} W`:`${h.toFixed(1)} W`}function gt(h){return h==null?"-":`${h.toFixed(2)} kWh`}let Ut="relative";function ne(h){const t=Math.round((h-Date.now())/1e3);let e;try{e=new Intl.RelativeTimeFormat(void 0,{numeric:"auto"})}catch{return Gt(h)}const s=Math.abs(t),r=[["year",31536e3],["month",2592e3],["week",604800],["day",86400],["hour",3600],["minute",60]];for(const[n,a]of r)if(s>=a)return e.format(Math.round(t/a),n);return e.format(t,"second")}function Gt(h){return new Date(h).toLocaleString(void 0,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}function it(h,t){if(!h)return"-";let e;if(typeof h=="number")e=h>=1e12?h:h*1e3;else{const s=String(h),r=/^(\d{4})-(\d{2})-(\d{2})$/.exec(s);e=r?new Date(+r[1],+r[2]-1,+r[3]).getTime():new Date(s).getTime()}return isNaN(e)?"-":(t||Ut)==="relative"?ne(e):Gt(e)}function Pt(h){if(h==null)return"unknown error";if(h instanceof Error)return`${h.name}: ${h.message}`;const t=h.code!=null?String(h.code):"",e=h.message!=null?String(h.message):"";if(!t&&!e)try{return JSON.stringify(h)}catch{return String(h)}const s=t==="unknown_command"?" (the integration is probably still starting up; this should stop on its own)":"";return`${t||"error"}${e?": "+e:""}${s}`}function d(h){return String(h??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function re(h){const t=String(h??"").trim();return/^https?:\/\//i.test(t)?t:""}function kt(h,t){const e=String(h??"").trim();let s=e.match(/^#([0-9a-fA-F]{3,8})$/);if(s){let r=s[1];if((r.length===3||r.length===4)&&(r=r.split("").map(n=>n+n).join("")),r.length===6||r.length===8){const n=parseInt(r.slice(0,2),16),a=parseInt(r.slice(2,4),16),o=parseInt(r.slice(4,6),16);return`rgba(${n}, ${a}, ${o}, ${t})`}}if(s=e.match(/^rgba?\(([^)]+)\)$/i),s){const r=s[1].split(",").map(n=>n.trim());if(r.length>=3)return`rgba(${r[0]}, ${r[1]}, ${r[2]}, ${t})`}return e}function Kt(h,t){const e=parseFloat(h);return isNaN(e)?t:e}function zt(h){return h?Array.from(h.querySelectorAll('a[href],button:not([disabled]),input:not([disabled]),select:not([disabled]),textarea:not([disabled]),[tabindex]:not([tabindex="-1"])')).filter(e=>e.getClientRects().length>0):[]}function St(h){return h==null||h===""?"(none)":h===!0?"on":h===!1?"off":Array.isArray(h)?h.join(", ")||"(none)":String(h)}function Xt(h,t,e){return h.slice().sort((s,r)=>{const n=t(s),a=t(r);return n==null&&a==null?0:n==null?1:a==null?-1:(n<a?-1:n>a?1:0)*e})}function ft(h,t,e,s,r,n,a){const o=e?s===1?" \u25B2":" \u25BC":' <span style="opacity:.35">\u2195</span>',i=n==="right"?"text-align:right;":"",l=a?` title="${d(a)}"`:"";return`<th class="wd-th-sort" data-sortcol="${t}" data-sortact="${r}" style="cursor:pointer;user-select:none;${i}"${l}>${h}${o}</th>`}function Mt(h){h=Math.max(0,Math.round(h));const t=Math.floor(h/3600),e=Math.floor(h%3600/60),s=h%60;return t>0?`${t}:${String(e).padStart(2,"0")}:${String(s).padStart(2,"0")}`:`${e}:${String(s).padStart(2,"0")}`}function Yt(h,t){const e={pause:["lbl.artifact_interruption","Interruption"],dip:["lbl.artifact_low_power","Low power"],spike:["lbl.artifact_high_power","High power"]},[s,r]=e[h]||["lbl.artifact_anomaly","Anomaly"];return t?t(s,{},r):r}function Jt(h){return h.toLowerCase().replace(/[\s&/\-]+/g,"_").replace(/^_+|_+$/g,"").replace(/_+/g,"_")}function Qt(h){const t=new Set;return String(h??"").split(",").forEach(e=>{const s=parseInt(e.trim(),10);Number.isFinite(s)&&s>0&&t.add(s)}),Array.from(t).sort((e,s)=>e-s)}function Dt(h,t){if(!h||!h.length)return null;if(t<=h[0][0])return h[0][1];if(t>=h[h.length-1][0])return h[h.length-1][1];for(let e=1;e<h.length;e++)if(h[e][0]>=t){const s=h[e-1],r=h[e],n=r[0]-s[0]||1;return s[1]+(r[1]-s[1])*((t-s[0])/n)}return h[h.length-1][1]}function oe(h,t,e){e=e||{};const s=h.key,r=h.unit?`${h.label} (${h.unit})`:h.label,n=h.unit?` ${h.unit}`:"",a=h.doc?at(h.doc,h.diagram||le[s]):"",o=e.changed?`<span class="wd-chg-dot" title="${d(e.changed)}" aria-label="${d(e.changed)}"></span>`:"";if(h.type==="checkbox")return`<div class="wd-field wd-field-switch"><div class="wd-switch-row"><label class="wd-switch-lbl"><span class="wd-switch"><input type="checkbox" data-opt="${s}" ${t?"checked":""}><span class="wd-switch-slider"></span></span><span class="wd-switch-text">${d(h.label)}</span></label>${o}${a}</div>${h.hint?`<div class="wd-field-hint">${d(h.hint)}</div>`:""}</div>`;let i="";const l=t??"";if(h.type==="select"||h.type==="devicetype"||h.type==="device"){const P=(e.opts||[]).map(([L,R])=>`<option value="${d(L)}" ${String(l)===String(L)?"selected":""}>${d(R)}</option>`).join("");i=`<select data-opt="${s}" data-ftype="${h.type}">${P}</select>`}else if(h.type==="textarea")i=`<textarea data-opt="${s}" data-ftype="textarea">${d(l)}</textarea>`;else if(h.type==="json"){const S=l===""||l==null?"":typeof l=="string"?l:JSON.stringify(l,null,2);i=`<textarea data-opt="${s}" data-ftype="json" rows="3" placeholder='${d(e.t("placeholder.json_buttons",{},'[{"action":"ID","title":"Label"}]'))}'>${d(S)}</textarea>`}else if(h.type==="checkboxlist"){const S=Array.isArray(t)?t.map(String):[];i=`<div data-opt="${s}" data-ftype="checkboxlist" class="wd-checkboxlist">`+(h.choices||[]).map(([P,L])=>`<label class="wd-switch-lbl" style="display:flex;align-items:center;gap:8px;margin:2px 0"><span class="wd-switch"><input type="checkbox" data-choice="${d(P)}" ${S.includes(String(P))?"checked":""}><span class="wd-switch-slider"></span></span><span class="wd-switch-text">${d(e.t?e.t("lbl.evidence_"+P,{},L):L)}</span></label>`).join("")+"</div>"}else if(h.type==="entitylist"){const P=(Array.isArray(t)?t:t?[t]:[]).map(L=>`<span class="wd-pill" data-val="${d(L)}">${d(L)}<button type="button" class="wd-pill-x" aria-label="${d(e.t?e.t("btn.remove",{},"Remove"):"Remove")}">\xD7</button></span>`).join("");i=`<div class="wd-pillbox" data-opt="${s}" data-ftype="entitylist">${P}<div class="wd-combo wd-combo-pill"><input type="text" class="wd-pill-add" autocomplete="off" spellcheck="false" placeholder="${d(e.t("placeholder."+(h.domain||"add"),{},h.placeholder||"add\u2026"))}"><div class="wd-combo-drop" hidden></div></div></div>`}else if(h.type==="timerlist"){const S=Array.isArray(l)?l:[],P=e.t?e.t("lbl.timer_min",{},"min"):"min",L=e.t?e.t("lbl.timer_msg_placeholder",{},"Message (optional, {device}/{program}/{minutes})"):"Message (optional, {device}/{program}/{minutes})",R=e.t?e.t("lbl.timer_auto_pause",{},"Auto-pause"):"Auto-pause",W=e.t?e.t("btn.remove_timer",{},"Delete"):"Delete",O=e.t?e.t("btn.add_timer",{},"+ Add timer"):"+ Add timer",M=(C,B)=>{const V=C&&C.offset_minutes?String(C.offset_minutes):"",H=C&&C.message?d(C.message):"",J=C&&C.auto_pause?" checked":"";return`<div class="wd-timer-row" data-tidx="${B}"><div class="wd-timer-top"><input type="number" min="1" placeholder="${d(P)}" data-field="offset_minutes" value="${d(V)}"><textarea placeholder="${d(L)}" data-field="message">${H}</textarea></div><div class="wd-timer-footer"><label class="wd-switch-lbl"><span class="wd-switch"><input type="checkbox" data-field="auto_pause"${J}><span class="wd-switch-slider"></span></span><span class="wd-switch-text">${d(R)}</span></label><button type="button" class="wd-btn wd-btn-sm wd-btn-danger wd-timer-remove">${d(W)}</button></div></div>`},T=S.map((C,B)=>M(C,B)).join("");i=`<div class="wd-timerlist" data-opt="${s}" data-ftype="timerlist">${T}<button type="button" class="wd-btn wd-btn-sm wd-btn-secondary wd-timer-add">${d(O)}</button></div>`}else if(h.type==="entity"){const S=h.placeholder?` placeholder="${d(h.placeholder)}"`:"";i=`<div class="wd-combo"><input type="text" class="wd-combo-inp" data-opt="${s}" data-ftype="entity" value="${d(l)}" autocomplete="off" spellcheck="false"${S}><div class="wd-combo-drop" hidden></div></div>`}else if(h.type==="list"){const S=Array.isArray(l)?l.join(", "):d(l);i=`<input type="text" data-opt="${s}" data-ftype="list" value="${d(S)}" placeholder="${d(e.t("placeholder.notify_services_list",{},"notify.mobile_app_phone, ..."))}">`}else if(h.type==="intlist"){const S=Array.isArray(l)?l.join(", "):String(l??""),P=h.placeholder?` placeholder="${d(h.placeholder)}"`:"";i=`<input type="text" data-opt="${s}" data-ftype="intlist" value="${d(S)}"${P}>`}else{const S=h.type==="number"?"number":"text",P=e.datalistId?` list="${e.datalistId}"`:"",L=h.step!=null?` step="${h.step}"`:"",R=h.min!=null?` min="${h.min}"`:"",W=h.max!=null?` max="${h.max}"`:"",O=h.placeholder?` placeholder="${d(h.placeholder)}"`:"";i=`<input type="${S}" data-opt="${s}" data-ftype="${h.type}" value="${d(l)}"${L}${R}${W}${P}${O}>${e.datalist||""}`}const c=e.suggestion,p=e.mlSuggestion,_=c&&c.suggested!=null&&!Ft(c.suggested,t)?c.suggested:null,g=p&&p.value!=null&&!Ft(p.value,t)?p.value:null,u=e.t,v=c?c.reason_key?u(c.reason_key,c.reason_params||{},c.reason||""):c.reason||"":"",b=p?p.reason_key?u(p.reason_key,p.reason_params||{},p.reason||""):p.reason||"":"",$=S=>`<button type="button" class="wd-sug-use" data-sugkey="${s}" data-sugval="${d(S)}">${e.useBtnLabel||"Use"}</button>`;let k="";if(_!=null&&g!=null){const S=parseFloat(_),P=parseFloat(g);if((!isNaN(S)&&!isNaN(P)?Math.abs(S-P)/Math.max(Math.abs(S),Math.abs(P),1e-9):1)<.05){const R=u("suggestion.calibrated_label",{},"Calibrated"),W=at([v,b?`${R}: ${b}`:""].filter(Boolean).join(`

`));k=`<div class="wd-sug"><span class="wd-sug-chip wd-sug-chip-obs">\u{1F4A1} ${d(u("suggestion.both_agree",{},"WashData recommends"))}</span><span class="wd-sug-val">${d(_)}${n}</span>${$(_)}${W}</div>`}else{const R=v?at(v):"",W=b?at(b):"",O=u("suggestion.observed_label",{},"Observed"),M=u("suggestion.calibrated_label",{},"Calibrated");let T="",C="";if(!isNaN(S)&&!isNaN(P)){const H=P>S;C=u(`suggestion.impact.${s}.${H?"higher":"lower"}`,{},""),T=u(`suggestion.impact.${s}.${H?"lower":"higher"}`,{},"")}const B=T?`<div class="wd-sug-impact-line">${d(T)}</div>`:"",V=C?`<div class="wd-sug-impact-line">${d(C)}</div>`:"";k=`<div class="wd-sug wd-sug-split"><div class="wd-sug-opt"><span class="wd-sug-chip wd-sug-chip-obs">\u{1F4A1} ${d(O)}</span><span class="wd-sug-val">${d(_)}${n}</span>${$(_)}${R}${B}</div><div class="wd-sug-opt"><span class="wd-sug-chip wd-sug-chip-cal">\u{1F916} ${d(M)}</span><span class="wd-sug-val">${d(g)}${n}</span>${$(g)}${W}${V}</div></div>`}}else if(_!=null){const S=v?at(v):"",P=t!=null&&t!==""?` <span style="opacity:.6;font-size:.9em">(now ${d(t)}${n})</span>`:"";k=`<div class="wd-sug"><span class="wd-sug-chip wd-sug-chip-obs">\u{1F4A1} ${d(u("suggestion.observed_label",{},"Observed"))}</span><span class="wd-sug-val">${d(_)}${n}</span>${P}${$(_)}${S}</div>`}else if(g!=null){const S=b?at(b):"";k=`<div class="wd-sug"><span class="wd-sug-chip wd-sug-chip-cal">\u{1F916} ${d(u("suggestion.calibrated_label",{},"Calibrated"))}</span><span class="wd-sug-val">${d(g)}${n}</span>${$(g)}${S}</div>`}if(k&&s){const S=d(u("btn.mute_suggestion",{},"Stop suggesting this setting")),P=`<button type="button" class="wd-sug-lock" data-suglock="${s}" title="${S}" aria-label="${S}">\u{1F515}</button>`;k=k.replace(/<\/div>\s*$/,P+"</div>")}return`<div class="wd-field" data-field="${s}"><div class="wd-label-row"><label style="margin:0">${d(r)}</label>${o}${a}</div>${i}${h.hint?`<div class="wd-field-hint">${d(h.hint)}</div>`:""}<div class="wd-conflict-err" data-cerr="${s}" hidden></div>${k}</div>`}function Ft(h,t){if(h==null||t==null)return!1;const e=parseFloat(h),s=parseFloat(t);return!isNaN(e)&&!isNaN(s)?Math.abs(e-s)<1e-6:String(h)===String(t)}const le={min_power:"min_power",off_delay:"off_delay",smoothing_window:"smoothing",start_threshold_w:"hysteresis",stop_threshold_w:"hysteresis",start_energy_threshold:"start_energy",running_dead_zone:"dead_zone",profile_duration_tolerance:"duration_tolerance",profile_match_min_duration_ratio:"match_ratios",profile_match_max_duration_ratio:"match_ratios",progress_reset_delay:"progress_reset",completion_min_seconds:"min_duration",min_off_gap:"min_off_gap",start_duration_threshold:"start_duration",end_energy_threshold:"end_energy_thresh",end_repeat_count:"end_repeat",profile_match_threshold:"confidence",profile_unmatch_threshold:"confidence",auto_label_confidence:"confidence",learning_confidence:"confidence",no_update_active_timeout:"watchdog_timeout",anti_wrinkle_enabled:"anti_wrinkle",anti_wrinkle_max_power:"anti_wrinkle",anti_wrinkle_max_duration:"anti_wrinkle",anti_wrinkle_exit_power:"anti_wrinkle",anti_wrinkle_idle_timeout:"anti_wrinkle",sampling_interval:"sampling"};function de(h){let t=0,e=window.innerWidth;for(let s=h.parentElement;s;s=s.parentElement){if(getComputedStyle(s).overflowX==="visible")continue;const r=s.getBoundingClientRect();r.left>t&&(t=r.left),r.right<e&&(e=r.right)}return{left:t,right:e}}function at(h,t){return`<span class="wd-tip">i<span class="wd-tip-pop">${t?ce(t):""}<span class="wd-tip-txt">${d(h)}</span></span></span>`}function Ot(h,t,e=""){return`<label class="wd-switch-lbl"><span class="wd-switch"><input type="checkbox" ${h}><span class="wd-switch-slider"></span></span><span class="wd-switch-text wd-switch-text--plain">${d(t)}</span></label>${e||""}`}function Tt(h,t,e="",s=""){return`<div class="wd-field wd-field-switch"><div class="wd-switch-row">${Ot(h,t,e)}</div>${s?`<div class="wd-field-hint">${d(s)}</div>`:""}</div>`}function ce(h){const t=s=>`<svg class="wd-dg" viewBox="0 0 200 90" preserveAspectRatio="xMidYMid meet">${s}</svg>`,e='<line class="ax" x1="8" y1="78" x2="192" y2="78"/>';switch(h){case"smoothing":return t(`${e}
    ...
    `}_htmlDeviceBar(){const t=this._isAdmin()?`<button class="wd-devcard wd-devadd" data-action="add-device" title="${d(this._t("btn.add_device_tip",{},"Add another WashData device"))}">${this._t("btn.add_device",{},"+ Add device")}</button>`:"";return this._devices.length<=1?t?`<div class="wd-devbar">${t}</div>`:"":`<div class="wd-devbar">${this._devices.map((e,s)=>{const r=e.is_user_paused?"user_paused":e.detector_state||"unknown",n=["running","starting","paused","user_paused","ending","anti_wrinkle","rinse"].includes(r),a=!!e.recording,o=a?"var(--error-color, #f44336)":this._stateColor(r),i=a?this._t("status.recording",{},"Recording"):this._stateLabel(r),l=[],c=this._conflictCountForOpts(e.options||{});c&&l.push(`<span class="wd-dbadge conf">\u26A0 ${c}</span>`);const p=this._sugCountsForDevice(e).total;return p&&l.push(`<span class="wd-dbadge sug">\u{1F4A1} ${p}</span>`),e.feedback_count&&l.push(`<span class="wd-dbadge fb">\u{1F4AC} ${e.feedback_count}</span>`),`<button class="wd-devcard ${s===this._selIdx?"active":""}" data-idx="${s}">
        ...
        ${l.join("")}
      </button>`}).join("")}${t}</div>`}_htmlStatus(){const t=this._devices[this._selIdx];if(!t)return`<div class="wd-empty">${this._t("msg.no_device_selected",{},"No device selected.")}</div>`;const e=!!t.is_user_paused,s=e?"user_paused":t.detector_state||"unknown",r=!!t.recording,n=r?"var(--error-color, #f44336)":this._stateColor(s),a=r?this._t("status.recording",{},"Recording"):this._stateLabel(s),o=r||["running","starting","paused","user_paused","ending","anti_wrinkle","rinse"].includes(s),i=t.cycle_progress_pct,l=t.time_remaining_s,c=t.current_program,p=!!t.manual_program,_=c||"auto_detect",g=(this._profiles||[]).map(x=>x.name);c&&!g.includes(c)&&g.unshift(c);const u=g.map(x=>`<option value="${d(x)}" ${_===x?"selected":""}>${d(x)}</option>`).join(""),v=c?p?this._t("badge.manual",{},"(manually selected)"):this._t("badge.auto",{},"(auto-detected)"):"",b=v?`<span class="wd-prog-tag ${p?"manual":"auto"}">${v}</span>`:"",$=`<div class="wd-prog-ctl"><label>${this._t("lbl.program",{},"Program")}</label>${at(this._t("lbl.program_tip",{},"Override which profile is matched to the current cycle. Auto-detect lets the integration pick the best match automatically. Pin a specific program to force-match it when auto-detect is wrong or you know what is running."))}
          <select id="wd-status-prog">
            <option value="auto_detect" ${_==="auto_detect"?"selected":""}>${this._t("status.auto_detect",{},"Auto-detect")}</option>
            ${u}
          </select>${b}</div>`,k=[];t.recording&&this._canEdit()&&k.push(`<div class="wd-attn-card"><span class="wd-attn-icon">\u25CF</span><div class="wd-attn-body"><div class="wd-attn-title">${this._t("msg.recording_in_progress",{},"Recording in progress")}</div><div class="wd-attn-sub">${this._t("msg.see_recorder",{},"See recorder widget below")}</div></div></div>`),t.feedback_count&&this._canEdit()&&k.push(`<button class="wd-attn-card" type="button" data-action="goto-feedbacks"><span class="wd-attn-icon">\u{1F4AC}</span><div class="wd-attn-body"><div class="wd-attn-title">${this._t("msg.feedback_cycles_pending",{n:t.feedback_count},`To review: ${t.feedback_count}`)}</div><div class="wd-attn-sub">${this._t("msg.review_to_cycles",{},"Open the Cycles review queue")}</div></div></button>`);const S=this._conflictKeysFromOpts();if(S.size&&this._canEdit()){const x=S.size,I=x>1?"s":"";k.push(`<button class="wd-attn-card" type="button" style="border-color:var(--error-color,#b71c1c)" data-action="goto-conflicts"><span class="wd-attn-icon">\u26A0</span><div class="wd-attn-body"><div class="wd-attn-title" style="color:var(--error-color,#b71c1c)">${this._t("conflict.attn_title",{n:x},`Setting conflicts: ${x}`)}</div><div class="wd-attn-sub">${this._t("conflict.attn_sub",{},"Fix conflicts before saving")}</div></div></button>`)}const P=this._sugCountsForDevice(t);if(P.total&&this._canEdit()){const x=P.total,I=[];P.classic&&I.push(this._t("lbl.n_classic_suggestions",{n:P.classic},`${P.classic} classic`)),P.ml&&I.push(this._t("lbl.n_ml_suggestions",{n:P.ml},`${P.ml} ML`)),k.push(`<button class="wd-attn-card" type="button" data-action="goto-suggestions"><span class="wd-attn-icon">\u{1F4A1}</span><div class="wd-attn-body"><div class="wd-attn-title">${this._t("lbl.n_tuning_suggestions",{n:x},`${x} tuning suggestion${x>1?"s":""}`)}</div><div class="wd-attn-sub">${I.join(" \xB7 ")} \xB7 ${this._t("msg.review_in_settings",{},"Review in Settings")}</div></div></button>`)}const L=k.length?`<div class="wd-attn">${k.join("")}</div>`:"",R=o&&i!=null?`
    ...
    `:"",O=((this._powerData||{}).live||[]).length>1,M=this._pref("show_expected",!0),T=this._pref("show_raw_active",!1),C=`<div class="wd-leg">
      ...
      ${this._pref("show_raw",!1)?`<label class="wd-leg-i"><input type="checkbox" data-statustoggle="show_raw_active" ${T?"checked":""}><span class="wd-leg-sw" style="background:#9e9e9e"></span> ${this._t("lbl.raw_socket",{},"Raw socket")}</label>`:""}
    </div>`,B=this._cyclesTotal||0,V=(this._profiles||[]).length,H=this._pref("setup_card_dismissed",!1),J=this._setupStatus,st=J&&!O,K=st?this._htmlSetupCard(J,H):"",X=O?`<div class="wd-canvas-wrap" style="margin-top:14px"><canvas id="wd-status-canvas" role="img" aria-label="${d(this._t("lbl.aria_power_chart",{},"Power consumption chart"))}" style="height:160px"></canvas></div>${C}`:st?K:`<p class="wd-info" style="margin-top:12px">${this._t("msg.live_chart_loading",{},"Live power chart appears as readings arrive.")}</p>`,ct=this._pref("show_debug",!1);let U="";if(ct){const x=this._matchDebug||{},I=x.confidence!=null?`${(x.confidence*100).toFixed(1)}%`:"-",j=(x.candidates||[]).map(D=>`<tr><td>${d(D.profile_name)}</td><td>${D.confidence_pct}%</td><td>${D.mae}</td><td>${D.correlation}</td><td>${D.duration_ratio>=0?"+":""}${D.duration_ratio}%</td></tr>`).join("");U=`<div class="wd-card">
        <div class="wd-card-title">Live Match Debug ${at("Confidence: how closely the current power curve matches the top candidate profile (0-100%). Ambiguous: the two best candidates score within 5% of each other - the label is uncertain until the cycle finishes.")}</div>
        <div class="wd-kv" style="margin-bottom:12px">
          <div class="wd-kv-item"><div class="wd-kv-val">${I}</div><div class="wd-kv-lbl">${this._t("lbl.confidence",{},"Confidence")}</div></div>
      ...
      </div>`}const et=[];this._canEdit()&&et.push(`<button class="wd-attn-card" type="button" data-action="open-advanced" data-sub="diagnostics"><span class="wd-attn-icon">\u{1FA7A}</span><div class="wd-attn-body"><div class="wd-attn-title">${this._t("hdr.logs_diagnostics",{},"Diagnostics")}</div><div class="wd-attn-sub">${this._t("msg.storage_diagnostics",{},"Storage stats, maintenance, export/import")}</div></div></button>`),et.push(`<button class="wd-attn-card" type="button" data-action="open-settings"><span class="wd-attn-icon">\u2699\uFE0F</span><div class="wd-attn-body"><div class="wd-attn-title">${this._t("settings.gear.title",{},"Settings")}</div><div class="wd-attn-sub">${this._isAdmin()?this._t("msg.preferences_admin",{},"Preferences, panel & access control"):this._t("msg.preferences_adv",{},"Preferences")}</div></div></button>`);const f=`<div class="wd-card"><div class="wd-card-title">${this._t("hdr.tools_and_data",{},"Tools & Data")}</div><div class="wd-attn" style="margin-bottom:0;margin-top:12px">${et.join("")}</div></div>`,m=(()=>{if(!this._canEdit())return"";const I=["running","starting","ending","anti_wrinkle","rinse"].includes(s),j=I&&!e,D=e,w=I||e;return!j&&!D&&!w?"":`<div class="wd-cycle-ctrl" style="margin-top:0">
        ${D?`<button class="wd-btn wd-btn-sm wd-btn-primary" data-action="resume-cycle" title="${d(this._t("btn.resume_cycle_tip",{},"Resume the paused cycle"))}">${this._t("btn.resume_cycle",{},"Resume")}</button>`:""}
      ...
      <div class="wd-card">
        ...
        ${st?"":`<div class="wd-card-title" style="margin-top:18px">${this._t("hdr.live_power",{},"Live Power")}</div>`}
        ${X}
      </div>
    ...
    `}_htmlSetupCard(t,e=!1){if(!t)return"";const{phase:s,message_key:r,message_params:n,cta_label_key:a,cta_action:o,secondary_label_key:i,secondary_action:l,skippable:c,dismissible:p,step_key:_}=t;if(s==="phase4")return`
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;flex-wrap:wrap">
        <div style="display:flex;align-items:center;gap:8px">
          ...
          <div><strong>${this._t("hdr.manual_recording",{},"Manual Recording")}</strong>${at(this._t("hdr.manual_recording_tip",{},"Run a cycle intentionally while WashData records the power trace. Start just before the appliance starts, Stop when it finishes, then Process to save it as a named profile."))}${o?`<span class="wd-field-hint" style="margin-left:8px">${o}</span>`:""}</div>
        </div>
        <div style="display:flex;gap:6px;flex-wrap:wrap">${i}</div>
      </div>
    </div>`}_htmlHistory(){const t=this._cycles||[],e=this._refCycles||[],s=e.concat(t),r=this._canEdit(),n=this._selectMode&&r,a=this._cycleSel,{col:o,dir:i}=this._cycleSort,{text:l,status:c}=this._cycleFilter,p=this._mlById||{},_=new Set((this._feedbacks||[]).map(w=>w.cycle_id)),g=w=>p[w.id],u=w=>{const A=g(w);return!!(A&&A.ml_review&&A.ml_review.reviewed_at)},v=w=>{const A=g(w);return!!(A&&A.ml_review&&A.ml_review.golden)},b=w=>{if(_.has(w.id))return!0;if(u(w))return!1;const A=g(w),z=A&&A.ml_quality_label;return["uncertain","review"].includes(z)||["force_stopped","interrupted"].includes(w.status)},$=s.filter(b).length;let k=s;if(l){const w=l.toLowerCase();k=k.filter(A=>(A.profile_name||A.matched_profile||"").toLowerCase().includes(w))}c==="unlabelled"?k=k.filter(w=>!w.profile_name&&!w.matched_profile):c==="needs_review"?k=k.filter(b):c==="imported"?k=k.filter(w=>w.is_reference):c&&(k=k.filter(w=>(w.status||"completed")===c));const S={date:w=>w.start_time?new Date(w.start_time).getTime():0,confidence:w=>w.match_confidence,duration:w=>w.duration,energy:w=>w.energy_kwh!=null?w.energy_kwh:w.energy_wh!=null?w.energy_wh/1e3:null,cost:w=>w.cost!=null?w.cost:-1,status:w=>w.status||"completed",profile:w=>(w.profile_name||w.matched_profile||"\uFFFF").toLowerCase()};k=Xt(k,S[o]||S.date,i);const P=w=>w==="completed"?"var(--success-color, #4caf50)":w==="interrupted"?"var(--error-color, #f44336)":w==="force_stopped"?"var(--warning-color, #ff9800)":"var(--secondary-text-color)",L=w=>{if(!w.is_reference)return"";const A=w.cycle_origin==="backfill",z=A?this._t("badge.backfilled_tip",{},"Detected in imported power history. Shapes program matching only, not counted in stats."):this._t("badge.imported_tip",{},"Imported from the community store. Used for matching only, not counted in stats.");return` <span title="${d(z)}" style="color:var(--info-color,#2196f3)">${A?"\u{1F557}":"\u{1F4E5}"}</span>`},R=w=>v(w)?' <span title="'+d(this._t("badge.golden_cycle",{},"Recorded reference cycle"))+'" style="color:var(--warning-color,#ff9800)">\u2B50</span>':_.has(w.id)?' <span title="'+d(this._t("badge.feedback_requested",{},"Feedback requested"))+'" style="color:var(--info-color,#2196f3)">\u{1F4AC}</span>':u(w)?' <span title="'+d(this._t("badge.reviewed",{},"Reviewed"))+'" style="color:var(--success-color,#4caf50)">\u2713</span>':b(w)?' <span title="'+d(this._t("badge.needs_review",{},"Needs review"))+'" style="color:var(--error-color,#f44336)">\u25CF</span>':"",W=w=>{if(w.anomaly!=="overrun")return"";const A=w.overrun_ratio?" "+this._t("badge.overrun_ratio",{x:Number(w.overrun_ratio).toFixed(1)},`(${Number(w.overrun_ratio).toFixed(1)}x expected)`):"";return` <span title="${d(this._t("badge.overrun",{},"Ran longer than usual"))}${d(A)}" style="color:var(--warning-color,#ff9800)">\u23F1</span>`},O=w=>{if(w.anomaly!=="underrun")return"";const A=w.underrun_ratio?" "+this._t("badge.underrun_ratio",{pct:Math.round(w.underrun_ratio*100)},`(${Math.round(w.underrun_ratio*100)}% of expected)`):"";return` <span title="${d(this._t("badge.underrun",{},"Finished faster than usual"))}${d(A)}" style="color:var(--info-color,#2196f3)">\u26A1</span>`},M=w=>{if(!w.energy_anomaly||w.energy_anomaly==="none")return"";const A=w.energy_anomaly==="energy_spike",z=w.energy_z_score!=null?` (${w.energy_z_score>0?"+":""}${Number(w.energy_z_score).toFixed(1)}\u03C3)`:"",F=A?"badge.energy_spike":"badge.energy_low",G=A?"Higher energy than usual":"Lower energy than usual",Q=A?"\u{1F53A}":"\u{1F53B}",rt=A?"var(--error-color,#f44336)":"var(--info-color,#2196f3)";return` <span title="${d(this._t(F,{},G))}${d(z)}" style="color:${rt}">${Q}</span>`},T=w=>{const A=Array.isArray(w.artifacts)?w.artifacts.length:0;return A?` <span title="${d(this._t("badge.artifact_tip",{n:A},`${A} anomal${A>1?"ies":"y"} detected (e.g. door opened mid-cycle) \u2014 open to see them on the graph`))}" style="color:var(--warning-color,#ff9800)">\u26A0</span>`:""},C=w=>{const A=Array.isArray(w.restart_gaps)?w.restart_gaps.length:0;return A?` <span title="${d(this._t("badge.restart_gap_tip",{n:A},`${A} HA restart gap${A>1?"s":""} during this cycle \u2014 power trace has a hole`))}" style="color:var(--info-color,#2196f3)">\u21BB</span>`:""},B=this._hass&&this._hass.config&&this._hass.config.currency||"",V=w=>w.cost!=null?`${w.cost.toFixed(2)}${B?" "+B:""}`:"-",H=k.map(w=>{const A=w.profile_name||w.matched_profile,z=w.match_confidence!=null?w.match_confidence*100:null,F=w.status||"completed",G=w.energy_kwh!=null?w.energy_kwh:w.energy_wh!=null?w.energy_wh/1e3:null,Q=n,rt=Q?`<input type="checkbox" class="wd-csel" ${a.has(w.id)?"checked":""} style="width:auto;margin:0">`:`<span class="wd-devdot" style="background:${P(F)}" title="${d(F)}"></span>`,ht={completed:this._t("status.completed",{},"Completed"),interrupted:this._t("status.interrupted",{},"Interrupted"),force_stopped:this._t("status.force_stopped",{},"Force stopped"),active:this._t("status.active",{},"Active")}[F]||F,nt=`${L(w)}${R(w)}${W(w)}${O(w)}${M(w)}${T(w)}${C(w)}`.trim();return`<tr data-cid="${d(w.id)}" data-selmode="${Q?1:0}" style="cursor:pointer">
        <td style="width:26px;padding:6px 4px 6px 8px">${rt}</td>
        <td>${A?d(A):`<span style="color:var(--secondary-text-color)">${this._t("lbl.unlabelled",{},"Unlabelled")}</span>`}</td>
        <td class="wd-tc-flags">${nt}</td>
        <td><span style="color:${P(F)};font-size:.9em">${d(ht)}</span></td>
        <td class="wd-tc-date">${it(w.start_time)}</td>
        <td class="wd-tc-num">${ot(w.duration)}</td>
        <td class="wd-tc-num">${G!=null?gt(G):"-"}</td>
    ...
    </div>`,K=k.length!==s.length?this._t("lbl.n_shown",{n:k.length},`, ${k.length} shown`):"",X=e.length?this._t("lbl.n_imported_note",{n:e.length},`, ${e.length} imported`):"",ct=this._t("lbl.cycles_title",{n:`${t.length}${X}${K}`},`Cycles (${t.length}${X}${K})`),U=r?`<div class="wd-card-actions" style="margin:0 0 4px;justify-content:flex-end">
    ...
    </div>`:"",et=new Set(e.map(w=>w.id)),f=[...a].some(w=>et.has(w)),m=n?`<div class="wd-card-actions" style="margin:0 0 10px">
    ...
    </div>`:"",x=this._busy.has("cyc-load-more"),I=this._cyclesHasMore?`<div style="text-align:center;margin-top:12px">
      ...
      </div>`}).join(""),o=this._profiles.filter(u=>!n.has(u.name)),i=o.map(u=>this._profileCardHtml(u)).join(""),l=this._profiles.length===0&&!(this._cycles||[]).length&&!(this._refCycles||[]).length,p=(e&&this._onlineEnabled()&&l?`
    ...
    `}_htmlProfileGroupModal(t){const e=this._busy.has("pg-save"),s=this._profileEnvCache||{},r=_=>tt[Math.max(0,this._profiles.findIndex(g=>g.name===_))%tt.length],n=t.members||[],a=this._profiles.map(_=>{const g=n.includes(_.name),u=_.avg_duration?`~${Math.round(_.avg_duration/60)}m`:"",v=_.avg_energy!=null?` \xB7 ${gt(_.avg_energy)}`:"",b=g?`<span style="display:inline-block;width:10px;height:10px;border-radius:2px;background:${r(_.name)};margin:0 2px"></span>`:"";return`<label class="wd-rev-tag"><input type="checkbox" class="wd-pg-mem" value="${d(_.name)}" ${g?"checked":""}> ${b}${d(_.name)} <span style="color:var(--secondary-text-color);font-size:.85em">${u}${v}</span></label>`}).join(""),o=n.filter(_=>s[_]&&(s[_].avg||[]).length),i=o.length?`<div class="wd-leg">${o.map(_=>`<span class="wd-leg-i"><span class="wd-leg-sw" style="background:${r(_)}"></span> ${d(_)}</span>`).join("")}</div>`:"",l=o.length?`<div class="wd-canvas-wrap" style="margin-top:8px"><canvas id="wd-pgroup-canvas" role="img" aria-label="${d(this._t("lbl.aria_envelope_chart",{},"Profile power envelope chart"))}" style="height:150px"></canvas></div>${i}`:`<p class="wd-info">${this._t("msg.group_preview_hint",{},"Tick 2+ members to preview and compare their power curves.")}</p>`,c=((this._profileGroups||{}).groups||[]).find(_=>_.name===t.orig),p=c&&c.cohesion!=null?`<span class="wd-badge" style="color:${c.cohesive?"var(--success-color,#4caf50)":"var(--warning-color,#ff9800)"};background:${c.cohesive?"rgba(76,175,80,.14)":"rgba(255,152,0,.14)"}">${c.cohesive?this._t("lbl.cohesion_good",{pct:Math.round(c.cohesion*100)},"cohesion "+Math.round(c.cohesion*100)+"%"):this._t("lbl.cohesion_low",{pct:Math.round(c.cohesion*100)},"\u26A0 low cohesion "+Math.round(c.cohesion*100)+"%")}</span>`:"";return`<h2>${t.orig?this._t("modal.edit_group",{},"Edit profile group"):this._t("modal.new_group",{},"New profile group")}</h2>
    ...
    </label>`,b=r?`<p class="wd-info" style="margin:0 0 10px;font-size:.82em">${this._t("msg.settings_basic_note",{},"Showing essential settings. Switch to Advanced for the full list.")}</p>`:"",$=this._busy.has("save-settings"),k=l.size,S=k!==1?"s":"",P=k?`
      ...
      </div>`:"",L=this._suggestions.length,R=[...a].filter(U=>!n.has(U)).length,W=L+R,O=this._settingsSugOnly&&!this._settingsSearch,M=L?`<button class="wd-btn wd-btn-sm wd-btn-primary" data-action="sug-apply-all">${this._t("btn.apply_all",{},"Apply all")}</button>`:"",T=L?`<button class="wd-btn wd-btn-sm wd-btn-secondary" data-action="sug-dismiss">${this._t("btn.dismiss",{},"Dismiss")}</button>`:"",C=W?O?`
      ...
      <div class="wd-sug-banner">
        ...
        ${M}
        ${T}
      </div>`:"",B=this._busy.has("sug-analyze"),V=`<button class="wd-btn wd-btn-secondary wd-btn-sm" data-action="sug-analyze" ${B?"disabled":""} title="${d(this._t("btn.run_analysis_tip",{},"Analyze your recorded cycles now and refresh tuning suggestions"))}">${B?'<span class="wd-spin"></span> '+this._t("status.analyzing",{},"Analyzing\u2026"):this._t("btn.run_analysis",{},"\u{1F50D} Run suggestion analysis")}</button>`,H=this._settingsSearch||"",J=H.trim().toLowerCase(),st=`<input type="text" id="wd-settings-search" class="wd-filter-input" placeholder="${this._t("msg.search_placeholder",{},"Search settings\u2026")}" value="${d(H)}" autocomplete="off" style="flex:0 0 auto;width:200px;max-width:40%">`,K=J?this._htmlSettingsSearch(t,J):O?this._htmlSettingsSugOnly(t):this._htmlSettingsSection(t),X=(this._lockedSuggestions||[]).length,ct=X?`
      ...
      <div class="wd-card">
        <form id="wd-settings-form">${K}</form>
    ...
    `}_htmlSettingsHistory(){const t=this._settingsChangelog||[];if(!t.length)return"";const e=this._settingsHistoryOpen,s=this._canEdit(),r=t.slice(0,100).map(n=>{const a=s?`<button class="wd-btn wd-btn-secondary wd-btn-xs" data-action="settings-revert-key" data-key="${d(n.key)}" data-val="${d(JSON.stringify(n.old))}" title="${d(this._t("btn.revert_to_previous",{},"Restore this setting to its previous value"))}">${this._t("btn.revert",{},"Revert")}</button>`:"";return`<tr>
        ...
        <td class="wd-tc-date">${it(n.timestamp)}</td>
      ...
      <div class="wd-combo-row">
        ...
        <div class="wd-field-hint">${d(this._t("msg.pick_brand_first",{},"Pick an appliance brand first."))}</div></div>`;this._ensureCatalogEntry(),this._catalog.forBrand!==o&&(this._catalog.forBrand=o,this._catalog.devices=void 0);const i=Array.isArray(this._catalog.devices)?this._catalog.devices:[];this._entityListCache=this._entityListCache||{},this._entityListCache[t]=i.map(u=>u.model).filter(Boolean);const l=this._catalogEntryFor(s,"device"),c=this._statusTag(l),p=this._catalog.devices===null?` <span class="wd-info" style="font-size:.85em">${this._t("msg.loading",{},"Loading\u2026")}</span>`:"";let _="";if(l){const u=[],v=re(l.manualUrl);v&&u.push(`<a href="${d(v)}" target="_blank" rel="noopener noreferrer nofollow">${this._t("link.manual",{},"Manual \u2197")}</a>`),(this._constants&&this._constants.storePrefs||{}).show_contributor!==!1&&u.push(this._t("store.contributed_by",{name:d(l.createdByName||this._t("lbl.anonymous",{},"Anonymous"))},`by ${d(l.createdByName||"Anonymous")}`));const b=!!(this._storeStatus&&this._storeStatus.connected);let $="";if(b){const k=[1,2,3,4,5].map(S=>`<button type="button" class="wd-star-btn" data-action="store-rate-device" data-device-id="${d(l.id)}" data-rating="${S}" aria-label="${this._t("lbl.rate_n",{n:S},`${S} stars`)}">\u2605</button>`).join("");$=`<button type="button" class="wd-btn wd-btn-secondary wd-btn-sm" data-action="store-confirm-device" data-device-id="${d(l.id)}" ${l.status!=="pending"?"disabled":""}>${l.status==="pending"?this._t("btn.confirm_appliance",{},"Confirm this appliance"):this._t("btn.confirmed",{},"Confirmed")}</button>
          <span class="wd-star-row" title="${d(this._t("lbl.rate_quality",{},"Rate quality"))}">${k}</span>`}else $=`<span class="wd-info" style="font-size:.85em">${this._t("msg.connect_to_confirm",{},"Connect in the settings gear to confirm or rate.")}</span>`;_=`<div class="wd-store-picker-detail">${u.join(" \xB7 ")}<div class="wd-store-picker-actions">${$}</div></div>`}const g=this._onlineEnabled()&&this._storeDeviceDeclared()?`<div class="wd-store-actions">
    ...
    </div>`:"";return`<div class="wd-field"><label>${d(r)} ${n?at(n):""}${c}${p}</label>
      <div class="wd-combo-row">
        <div class="wd-combo">
    ...
    `}_htmlMlStatusSection(t,e){const s=e&&this._busy.has("ml-train-now:"+e)||t&&t.running,r=this._canEdit()?`<button class="wd-btn wd-btn-primary wd-btn-sm" data-action="ml-train-now" ${s?"disabled":""}>${s?`<span class="wd-spin"></span> ${this._t("status.training",{},"Training\u2026")}`:this._t("btn.train_now",{},"Train now")}</button>`:"";if(!t)return`<div class="wd-card"><div class="wd-card-title" style="margin:0 0 4px">${this._t("hdr.status",{},"Status")}</div><p class="wd-info" style="margin:0">${this._t("msg.loading",{},"Loading\u2026")}</p></div>`;const n=Object.keys(t.on_device_models||{}).length,a=n?`<span style="color:var(--success-color,#4caf50);font-weight:600">${this._t("ml.personalized",{},"\u25CF Personalized to this machine")}</span> <span style="color:var(--secondary-text-color)">${this._t("lbl.models_fine_tuned",{count:n},"(fine-tuned models: "+n+")")}</span>`:`<span style="color:var(--secondary-text-color)">${this._t("ml.builtin_models",{},"\u25CF Using built-in models")}</span>`,o=t.cycle_count||0,i=t.min_cycles||0,l=o>=i,c=i>0?Math.min(100,Math.round(o/i*100)):100,p=l?"var(--success-color,#4caf50)":"var(--warning-color,#ff9800)",_=Math.max(0,i-o),g=l?this._t("msg.enough_data",{current:o,min:i},`Enough data to learn from (${o}/${i} cycles).`):this._t("msg.collecting_data",{need:_,current:o,min:i},`Collecting data. Cycles still needed before fine-tuning can start: ${_} (${o}/${i}).`),u=`<div style="height:8px;border-radius:6px;background:var(--secondary-background-color);overflow:hidden;margin:8px 0"><div style="width:${c}%;height:100%;background:${p}"></div></div>`,v=t.last_trained?it(t.last_trained):"never",b=s?`<span style="color:var(--info-color,#2196f3)"><span class="wd-spin"></span> ${this._t("status.fine_tuning",{},"fine-tuning now\u2026")}</span>`:t.enabled?this._t("lbl.auto_fine_tune_on",{hour:String(t.hour).padStart(2,"0")},`auto fine-tune on (around ${String(t.hour).padStart(2,"0")}:00)`):this._t("lbl.auto_fine_tune_off",{},"auto fine-tune off");return`<div class="wd-card">
      ...
      </tr>`}).join(""),c=a?`<span class="wd-badge" style="color:var(--success-color,#4caf50);background:rgba(76,175,80,.14)">${this._t("badge.using_tuned",{},"Using tuned weights")}</span>`:`<span class="wd-badge" style="color:var(--secondary-text-color);background:var(--secondary-background-color)">${this._t("badge.using_defaults",{},"Using shipped defaults")}</span>`;let p="";if(a){const g=r.trained_at?it(r.trained_at):"unknown",u=r.baseline_test_top1,v=r.tuned_test_top1,b=u!=null&&v!=null?` \xB7 held-out top-1 ${(u*100).toFixed(0)}% \u2192 <strong>${(v*100).toFixed(0)}%</strong>`:"";p=`<p class="wd-info" style="margin:8px 0 0">Tuned ${d(g)} from ${r.cycle_count||0} cycles${b}.</p>`}const _=a?`<button class="wd-btn wd-btn-secondary wd-btn-sm" data-action="ml-revert-match" ${o?"disabled":""}>${o?'<span class="wd-spin"></span> '+this._t("status.reverting",{},"Reverting\u2026"):this._t("btn.reset_to_defaults",{},"Reset to defaults")}</button>`:"";return`<div class="wd-card" style="margin-top:12px">
      ...
      ${r}
        ...
        </div>`,n=b);const C=this._pgStagedVal(_),B=C!=null&&!(S!==""&&S!==null&&S!==void 0&&this._pgSameVal(C,S)),V=this._canEdit()&&B&&this._pgIsPublishable(_)?`<button class="wd-pg-pub" data-action="pg-publish-one" data-pgkey="${d(_)}" aria-label="${d(this._t("btn.pg_publish_one",{label:k},"Publish "+k+" to the integration"))}" title="${d(this._t("btn.pg_publish_one",{label:k},"Publish "+k+" to the integration"))}">\u2191</button>`:'<span class="wd-pg-pub-slot" aria-hidden="true"></span>';return`${T}<div style="display:flex;align-items:flex-start;gap:6px;margin:0 0 6px 11px">
        <div style="flex:1;min-width:0">
          <div style="font-size:.82em;font-weight:600;margin-bottom:1px">${d(k)}${R?` <span style="color:${O};font-size:.85em" title="${d(this._t("lbl.pg_drag_hint",{},"Drag line on graph"))}">\u2195</span>`:""}</div>
          ${v?`<div style="font-size:.72em;color:var(--secondary-text-color);line-height:1.3">${d(this._t("pg_desc."+_,{},v))}</div>`:""}
        </div>
        <div style="display:flex;align-items:center;gap:4px;flex-shrink:0">
          ${L?`<input class="wd-pg-param-chk" type="checkbox" data-pgkey="${d(_)}" data-pgtype="bool" aria-label="${d(k)}" ${P?"checked":""} style="width:18px;height:18px;margin:1px 27px 0 27px">`:`<input class="wd-pg-param-inp" type="text" inputmode="decimal" data-pgkey="${d(_)}" value="${P!==""?d(String(P)):""}" placeholder="${S!==""?d(String(S)):""}" aria-label="${d(k)}" style="width:72px">`}
      ...
      </div>`}const i=e.summary||{},l=`<div style="font-size:.82em;color:var(--secondary-text-color);margin-bottom:8px">${this._t("msg.pg_history_summary",{detected:i.detected,correct:i.match_correct,total:i.cycles},`${i.detected}/${i.cycles} detected \xB7 ${i.match_correct} matched correctly`)}</div>`,c=u=>(this._cycles||[]).find(v=>v.id===u),p={};(e.baseline_rows||[]).forEach(u=>{p[u.cycle_id]=u});const _=e.rows.map(u=>{const v=c(u.cycle_id),b=v&&v.start_time?it(v.start_time):(u.cycle_id||"").slice(0,8),$=u.match_correct===!0?"\u2713":u.match_correct===!1?"\u2717":"\u2014",k=u.match_correct===!0?"var(--success-color,#4caf50)":u.match_correct===!1?"var(--error-color,#f44336)":"var(--secondary-text-color)",S=u.matched_profile||this._t("lbl.unlabelled",{},"Unlabelled"),P=u.duration_s?Math.round(u.duration_s/60)+"m":"\u2014",L=u.overrun_ratio!=null?` (${Math.round(u.overrun_ratio*100)}%)`:"",R=u.alerts&&u.alerts.length?` <span title="${d(u.alerts.join(", "))}" style="color:var(--warning-color,#ff9800)">\u26A0</span>`:"",W=p[u.cycle_id];let O="";return W&&(W.match_correct!==!0&&u.match_correct===!0?O=`<span style="color:var(--success-color,#4caf50)">\u25B2 ${this._t("lbl.pg_fixed",{},"fixed")}</span>`:W.match_correct===!0&&u.match_correct!==!0?O=`<span style="color:var(--error-color,#f44336)">\u25BC ${this._t("lbl.pg_broke",{},"broke")}</span>`:String(W.termination_reason)!==String(u.termination_reason)&&(O=`<span style="color:var(--warning-color,#ff9800)">${d(String(W.termination_reason||"\u2014"))}\u2192${d(String(u.termination_reason||"\u2014"))}</span>`)),`<tr class="wd-pg-hrow${u.cycle_id===this._pgCycleId?" selected":""}" data-action="pg-open-cycle" data-cid="${d(u.cycle_id)}" title="${d(this._t("msg.pg_row_load_hint",{},"Load this cycle in the graph above"))}">
        <td>${d(b)}</td>
        <td><span style="color:${k};font-weight:700">${$}</span> ${d(S)}</td>
        <td>${d(String(u.termination_reason||"\u2014"))}</td>
        <td style="font-variant-numeric:tabular-nums">${P}${L}${R}</td>
        ${e.diff?`<td>${O}</td>`:""}
    ...
    </tr></thead><tbody>${_}</tbody></table>`;return`${a}${r}${this._htmlPgRecentRuns("pg_history")}${o}${l}${g}`}_htmlPgBatchBar(){const t=this._pgBatchProgress;return t?`<div class="wd-pg-batchrow">
    ...
    </div>`,o=t?this._htmlPgBatchBar():"";return`${n}${a}${this._htmlPgRecentRuns("pg_sweep")}${o}${this._htmlPgSweepResult()}`}_htmlPgSweepResult(){const t=this._pgSweepNew;if(!t||!Array.isArray(t.points)||!t.points.length)return"";const e=this._pgSweepObjectives().find(_=>_[0]===t.objective),s=e?e[2]:!1,r=t.points.filter(_=>_.metric!=null).map(_=>_.metric);if(!r.length)return`<div class="wd-empty" style="padding:16px">${this._t("msg.pg_sweep_no_metric",{},"Not enough data to score this objective.")}</div>`;const n=Math.min(...r),a=Math.max(...r),o=s?Math.min(...r):Math.max(...r),i=(t.points.find(_=>_.metric===o)||{}).value,l=_=>t.objective==="median_overrun"?Math.round(_*100)+"% off":Math.round(_*100)+"%",c=t.points.map(_=>{const g=a>n&&_.metric!=null?(_.metric-n)/(a-n):_.metric!=null?1:0,u=_.metric===o,v=t.current_value!=null&&Math.abs(_.value-t.current_value)<1e-6,b=u?"var(--success-color,#4caf50)":"var(--primary-color)";return`<div style="display:flex;align-items:center;gap:8px;margin:3px 0;font-size:.82em">
        <span style="flex:0 0 74px;text-align:right;font-variant-numeric:tabular-nums">${d(String(_.value))}${v?" \u25C0":""}</span>
        <div style="flex:1;height:16px;border-radius:4px;background:var(--secondary-background-color);overflow:hidden"><div style="height:100%;width:${Math.round((_.metric!=null?.15+.85*g:0)*100)}%;background:${b};border-radius:4px"></div></div>
      ...
      <span style="font-size:.75em;color:var(--secondary-text-color);text-transform:uppercase">${this._t("lbl.phase",{},"Phase")} <span id="wd-pg-phase" style="color:var(--primary-text-color,inherit)">\u2014</span></span>
        ...
        <span class="wd-pg-cand-pct">${i}%</span>
      </div>`}return`<div>${s||`<p class="wd-info" style="margin:0">${this._t("msg.pg_analysis_hint2",{},"Pick a cycle and press Run to load match analysis.")}</p>`}</div>`}async _pgLoad(){const t=this._devices[this._selIdx];if(!t||this._pgLoading)return;const e=this._pgCycleId||this._cycles?.[0]?.id||"";if(!e)return;this._pgCycleId=e,this._pgLoading=!0,this._pgView=null,this._pgHoverT=null,this._pgPowerPts=null,this._pgDtwData=null,this._pgEnvData=null,this._pgDetail=null;const s=++this._pgLoadSeq;this._render();try{const r=await this._ws({type:`${y}/get_cycle_power_data`,entry_id:t.entry_id,cycle_id:e}),n=r.samples||[],a=[];for(const l of n){if(!Array.isArray(l)||l.length<2)continue;const c=+l[0],p=+l[1];!isNaN(c)&&!isNaN(p)&&a.push({t:c,w:p})}if(this._pgPowerPts=a.length?a:null,typeof r.full_duration_s=="number"&&r.full_duration_s>0){const l=(this._cycles||[]).find(c=>c.id===e);l&&(l._pg_duration=r.full_duration_s)}const o=this._pgProfileName||(this._cycles||[]).find(l=>l.id===e)?.profile_name||"";if(a.length)try{const l={type:`${y}/get_dtw_debug`,entry_id:t.entry_id,cycle_id:e};o&&(l.profile_name=o),this._pgDtwData=await this._ws(l),this._pgNeedsRestart=!1}catch(l){this._pgIsUnknownCmd(l)&&(this._pgNeedsRestart=!0),this._pgDtwData=null}const i=this._pgDtwData?.profile_name||o;if(i)try{const l=await this._ws({type:`${y}/get_profile_envelope`,entry_id:t.entry_id,profile_name:i});this._pgEnvData=l.envelope||null}catch{this._pgEnvData=null}if(s!==this._pgLoadSeq)return;await this._pgLoadDetail(t.entry_id,e)}catch(r){this._showToast(this._t("msg.toast_error",{error:r.message||r},"Error: "+(r.message||r)),"error")}s===this._pgLoadSeq&&(this._pgLoading=!1,this._render(),requestAnimationFrame(()=>{this._pgDrawCanvas(),this._pgUpdateStripAt(null)}),this._pgNeedsRestart&&this._tab==="playground"?(this._pgRestartRetries||0)<5&&!this._pgRestartRetryTimer&&(this._pgRestartRetries=(this._pgRestartRetries||0)+1,this._pgRestartRetryTimer=setTimeout(()=>{this._pgRestartRetryTimer=null,this._pgNeedsRestart&&this._tab==="playground"&&this._pgLoad()},3e3)):this._pgRestartRetries=0)}_pgCancelRun(){this._pgLoadSeq++,this._pgLoading=!1,this._render()}_pgSelectCycle(t){t&&(this._pgCycleId=t,this._pgProfileName="",this._pgView=null,this._pgHoverT=null,this._pgLoading=!1,this._pgLoad(),requestAnimationFrame(()=>{const e=this.shadowRoot&&this.shadowRoot.getElementById("wd-pg-canvas");e&&typeof e.scrollIntoView=="function"&&e.scrollIntoView({behavior:"smooth",block:"center"})}))}async _pgLoadDetail(t,e){const s={...this._pgParamOverrides};this._pgThreshStart!=null&&(s.start_threshold_w=this._pgThreshStart),this._pgThreshStop!=null&&(s.stop_threshold_w=this._pgThreshStop);try{const r=await this._ws({type:`${y}/start_playground_cycle_detail`,entry_id:t,cycle_id:e,settings_override:s,stress_tail:this._pgStressTail,stress_idle_w:this._pgStressIdleW!=null?parseFloat(this._pgStressIdleW):null}),n=r&&r.task_id;if(!n)throw new Error("no task id");this._addProvisionalTask(n,"pg_detail",t,0);const a=await new Promise(o=>{this._taskCallbacks[n]=async l=>{if(l.state==="done"||l.state==="cancelled")try{const c=await this._ws({type:`${y}/get_task_result`,task_id:l.id});o(c&&c.result)}catch{o(null)}else o(null)};const i=this._tasks[n];i&&i.state!=="running"?this._settleTaskCallback(i):this._tasksSubscribed||this._pollTaskGeneric(n)});if(!this._isActiveEntry(t))return;this._pgDetail=a&&!a.error?a:null,this._pgNeedsRestart=!1}catch(r){this._pgIsUnknownCmd(r)&&(this._pgNeedsRestart=!0),this._pgDetail=null}}_pgRerunDetail(){const t=this._devices[this._selIdx];!t||!this._pgCycleId||(clearTimeout(this._pgDetailDebounceTimer),this._pgDetailDebounceTimer=setTimeout(async()=>{this._pgDetailBusy=!0,this._render(),await this._pgLoadDetail(t.entry_id,this._pgCycleId),this._pgDetailBusy=!1,this._render(),requestAnimationFrame(()=>this._pgDrawCanvas())},220))}_pgMapState(t){return t==="running"||t==="paused"?"running":t==="ending"?"ending":t==="starting"?"detecting":t==="anti_wrinkle"?"anti_wrinkle":"idle"}_pgSeriesAt(t){const e=this._pgDetail&&this._pgDetail.series;if(!e||!e.length)return null;let s=e[0];for(const r of e)if(r.t<=t)s=r;else break;return s}_pgStateSegsFromSeries(t){const e=this._pgDetail&&this._pgDetail.series;if(!e||!e.length)return[];const s=[];let r=null;for(const n of e){const a=this._pgMapState(n.state);!r||r.state!==a?(r={start:n.t,end:n.t,state:a},s.push(r)):r.end=n.t}return s.length&&(s[s.length-1].end=t),s}_pgDrawCanvas(){if(this._tab!=="playground")return;const t=this.shadowRoot,e=t&&t.getElementById("wd-pg-canvas");if(!e)return;const s=this._pgPowerPts,r=e.getBoundingClientRect(),n=window.devicePixelRatio||1,a=Math.max(1,Math.round(r.width*n)),o=Math.max(1,Math.round((r.height||280)*n));(e.width!==a||e.height!==o)&&(e.width=a,e.height=o);const i=e.getContext("2d"),l=getComputedStyle(this),c=(l.getPropertyValue("--primary-color")||"#03a9f4").trim(),p=(l.getPropertyValue("--divider-color")||"rgba(127,127,127,.2)").trim(),_=(l.getPropertyValue("--secondary-text-color")||"#888").trim(),g=(l.getPropertyValue("--secondary-background-color")||"#1a1a1a").trim();i.clearRect(0,0,a,o);const u=34*n,v=14*n,b=Bt*n,$=44*n,k=8*n,S=b+8*n,P=u+v+4*n,L=o-S-P;if(!s||!s.length){this._pgLoading&&(i.fillStyle=_,i.font=`${12*n}px sans-serif`,i.textAlign="center",i.textBaseline="middle",i.fillText(this._t("msg.loading",{},"Loading\u2026"),a/2,o/2));return}const R=this._pgDetail&&this._pgDetail.outcome&&this._pgDetail.outcome.stress,W=R&&R.enabled?R.synthetic_from_s:null,O=W!=null&&this._pgDetail&&this._pgDetail.series?this._pgDetail.series.filter(E=>E.t>=W):[];let M=(this._cycles||[]).find(E=>E.id===this._pgCycleId)?._pg_duration||s[s.length-1].t||1;R&&R.terminated&&R.terminated_after_s!=null&&W!=null?M=Math.max(M,W+R.terminated_after_s):O.length&&(M=Math.max(M,O[O.length-1].t));const T=Math.max(...s.map(E=>E.w),...O.map(E=>E.power||0),1),C=this._pgThreshStart??this._pgFieldVal("start_threshold_w",{})??50,B=this._pgThreshStop??this._pgFieldVal("stop_threshold_w",{})??5;let V=0,H=M;this._pgView&&this._pgView.max-this._pgView.min>1&&(V=Math.max(0,this._pgView.min),H=Math.min(M,this._pgView.max),H-V<=1&&(V=0,H=M));const J=Math.max(1e-6,H-V),st=a-$-k,K=E=>$+(E-V)/J*st,X=E=>S+(1-Math.max(0,E)/T)*L;this._pgMap={vMin:V,vMax:H,totalDur:M,padLpx:$/n,plotWpx:st/n},i.strokeStyle="rgba(127,127,127,0.12)",i.lineWidth=n,i.setLineDash([]),[.25,.5,.75,1].map(E=>Math.round(E*T/100)*100||Math.round(E*T)).forEach(E=>{const N=X(E);N<S||N>S+L||(i.beginPath(),i.moveTo($,N),i.lineTo(a-k,N),i.stroke(),i.fillStyle=_,i.font=`${9*n}px sans-serif`,i.textAlign="right",i.textBaseline="middle",i.fillText(E+"W",$-4*n,N))}),i.save(),i.beginPath(),i.rect($,S,st,L),i.clip();const U=this._pgEnvData,et=U&&Array.isArray(U.avg)?U.avg.filter(E=>Array.isArray(E)&&E.length>=2):[];if(et.length){const E=Math.max(...et.map(q=>q[0]))||1,N=q=>K(q/E*M),Z=Array.isArray(U.min)?U.min.filter(q=>Array.isArray(q)&&q.length>=2):[],Y=Array.isArray(U.max)?U.max.filter(q=>Array.isArray(q)&&q.length>=2):[];if(Z.length&&Y.length){i.beginPath(),Y.forEach((q,lt)=>lt?i.lineTo(N(q[0]),X(q[1])):i.moveTo(N(q[0]),X(q[1])));for(let q=Z.length-1;q>=0;q--)i.lineTo(N(Z[q][0]),X(Z[q][1]));i.closePath(),i.fillStyle="#eda10012",i.fill()}else i.beginPath(),i.strokeStyle="#eda100",i.lineWidth=2*n,i.setLineDash([5*n,4*n]),et.forEach((q,lt)=>lt?i.lineTo(N(q[0]),X(q[1])):i.moveTo(N(q[0]),X(q[1]))),i.stroke(),i.setLineDash([])}const f=this._pgDtwData,m=f&&Array.isArray(f.profile_trace)?f.profile_trace.filter(E=>Array.isArray(E)&&E.length>=2):[],x=f&&Array.isArray(f.cycle_trace)?f.cycle_trace.filter(E=>Array.isArray(E)&&E.length>=2):[],I=f&&Array.isArray(f.warp_path)?f.warp_path:[];if(x.length&&m.length&&I.length){const E=Math.max(...m.map(Y=>Y[0]))||1,N=Math.max(...x.map(Y=>Y[0]))||1,Z=Math.max(1,Math.floor(I.length/25));i.save(),i.globalAlpha=.13,i.strokeStyle="#fff",i.lineWidth=n;for(let Y=0;Y<I.length;Y+=Z){const q=I[Y];if(!Array.isArray(q)||q.length<2)continue;const lt=Math.min(q[0],x.length-1),pt=Math.min(q[1],m.length-1),bt=K(x[lt][0]/N*M),_t=X(x[lt][1]),yt=K(m[pt][0]/E*M),dt=X(m[pt][1]);i.beginPath(),i.moveTo(bt,_t),i.lineTo(yt,dt),i.stroke()}i.restore()}if(m.length){const E=Math.max(...m.map(N=>N[0]))||1;i.beginPath(),i.strokeStyle="#eda100",i.lineWidth=2*n,i.setLineDash([6*n,4*n]),m.forEach((N,Z)=>{const Y=K(N[0]/E*M),q=X(N[1]);Z?i.lineTo(Y,q):i.moveTo(Y,q)}),i.stroke(),i.setLineDash([])}if(i.beginPath(),i.moveTo(K(0),X(0)),s.forEach(E=>i.lineTo(K(E.t),X(E.w))),i.lineTo(K(s[s.length-1].t),X(0)),i.closePath(),i.fillStyle=kt(c,.1),i.fill(),i.beginPath(),i.strokeStyle=c,i.lineWidth=2*n,s.forEach((E,N)=>N?i.lineTo(K(E.t),X(E.w)):i.moveTo(K(E.t),X(E.w))),i.stroke(),O.length){const E=s[s.length-1];i.beginPath(),i.moveTo(K(E.t),X(E.w)),O.forEach(N=>i.lineTo(K(N.t),X(N.power))),i.lineTo(K(O[O.length-1].t),X(0)),i.lineTo(K(E.t),X(0)),i.closePath(),i.fillStyle=kt(c,.06),i.fill(),i.beginPath(),i.strokeStyle=c,i.lineWidth=1.5*n,i.setLineDash([4*n,4*n]),i.moveTo(K(E.t),X(E.w)),O.forEach(N=>i.lineTo(K(N.t),X(N.power))),i.stroke(),i.setLineDash([])}if(W!=null&&W<H){const E=K(W);i.fillStyle="rgba(192,57,43,0.07)",i.fillRect(E,S,Math.max(0,$+st-E),L),i.save(),i.strokeStyle="rgba(192,57,43,0.5)",i.lineWidth=n,i.setLineDash([4*n,4*n]),i.beginPath(),i.moveTo(E,S),i.lineTo(E,S+L),i.stroke(),i.restore()}i.restore();const j=(E,N,Z)=>{const Y=X(E);Y<S-2||Y>S+L+2||(i.save(),i.strokeStyle=N,i.lineWidth=2*n,i.setLineDash([8*n,4*n]),i.beginPath(),i.moveTo($,Y),i.lineTo(a-k,Y),i.stroke(),i.setLineDash([]),i.fillStyle=N,i.beginPath(),i.arc($+(a-$-k)*.06,Y,5*n,0,Math.PI*2),i.fill(),i.fillStyle=N,i.font=`bold ${9*n}px sans-serif`,i.textAlign="left",i.textBaseline="bottom",i.fillText(Z+" "+Math.round(E)+"W",$+14*n,Y-2*n),i.restore())};j(+C,"#2a78d6",this._t("lbl.start",{},"Start")),j(+B,"#e34948",this._t("btn.stop",{},"Stop"));const D={idle:g,detecting:"#42a5f566",running:"#66bb6a66",ending:"#ef535066",anti_wrinkle:"#ab47bc66"},w={idle:this._t("lbl.pg_idle",{},"Idle"),detecting:this._t("lbl.pg_detecting",{},"Detecting"),running:this._t("lbl.pg_ev_running",{},"Running"),ending:this._t("lbl.pg_ev_ending",{},"Ending"),anti_wrinkle:this._t("lbl.pg_anti_wrinkle",{},"Anti-wrinkle")},A=o-u-v;i.fillStyle=g,i.fillRect($,A,a-$-k,u),i.save(),i.beginPath(),i.rect($,A,st,u),i.clip(),this._pgStateSegsFromSeries(M).forEach(E=>{const N=K(E.start),Z=K(E.end);i.fillStyle=D[E.state]||p,i.fillRect(N,A,Math.max(1,Z-N),u),Z-N>50*n&&(i.fillStyle=_,i.font=`${8*n}px sans-serif`,i.textAlign="center",i.textBaseline="middle",i.fillText((w[E.state]||E.state).toUpperCase(),(N+Z)/2,A+u/2))}),i.restore();const F=(this._cycles||[]).find(E=>E.id===this._pgCycleId),G=this._pgDtwData?.profile_name||this._pgProfileName||F?.profile_name,Q=G?(this._profiles||[]).find(E=>E.name===G):null,rt=o-v;Q&&Array.isArray(Q.phases)&&Q.phases.length&&(i.save(),i.beginPath(),i.rect($,rt,st,v),i.clip(),Q.phases.forEach((E,N)=>{const Z=K((E.start||0)*M),Y=K((E.end||1)*M),q=N*47%360;i.fillStyle=`hsla(${q},60%,55%,0.55)`,i.fillRect(Z,rt,Math.max(1,Y-Z),v),Y-Z>40*n&&(i.fillStyle="#fff",i.font=`${7*n}px sans-serif`,i.textAlign="center",i.textBaseline="middle",i.fillText(E.name,(Z+Y)/2,rt+v/2))}),i.restore()),this._pgEventHits=[];const ht=this._pgDetail&&Array.isArray(this._pgDetail.events)?this._pgDetail.events.filter(E=>E.type!=="state"&&E.t>=V&&E.t<=H).slice().sort((E,N)=>E.t-N.t):[],nt=11*n,Et=S-b/2-2*n,Zt=nt*2+3*n;let Nt=-1/0;const xt=this._pgHoverEvent;if(ht.forEach(E=>{const N=this._pgEventMeta(E.type),Z=K(E.t);let Y=Math.max(Z,Nt+Zt);Y=Math.max($+nt,Math.min(a-k-nt,Y)),Nt=Y;const q=xt&&xt.t===E.t&&xt.type===E.type;i.save(),i.strokeStyle=N.color,i.globalAlpha=q?.9:.5,i.lineWidth=(q?2:1)*n,i.beginPath(),i.moveTo(Y,Et+nt),i.lineTo(Z,S),i.stroke(),q&&(i.globalAlpha=.3,i.setLineDash([2*n,3*n]),i.beginPath(),i.moveTo(Z,S),i.lineTo(Z,o-v),i.stroke(),i.setLineDash([])),i.restore(),i.save(),i.beginPath(),i.arc(Y,Et,nt,0,Math.PI*2),i.fillStyle=q?N.color:g||"#1a1a1a",i.fill(),i.lineWidth=(q?2:1.5)*n,i.strokeStyle=N.color,i.stroke(),i.fillStyle=q?"#fff":N.color,i.font=`${14*n}px sans-serif`,i.textAlign="center",i.textBaseline="middle",i.fillText(N.glyph,Y,Et+n),i.restore(),this._pgEventHits.push({cx:Y/n,cy:Et/n,r:nt/n,type:E.type,label:N.label,detail:E.detail,t:E.t})}),xt){const E=this._pgEventHits.find(N=>N.t===xt.t&&N.type===xt.type);if(E){const N=this._pgEventMeta(E.type),Z=ut=>`${Math.floor(ut/60)}:${String(Math.round(ut%60)).padStart(2,"0")}`,Y=this._pgEventDescription(E.type),q=[{t:E.label,bold:!0,col:N.color},...Y?[{t:Y}]:[],...E.detail?[{t:E.detail,dim:!0}]:[],{t:`${this._t("lbl.from_start",{},"From start")} ${Z(E.t)}`,dim:!0}];i.save(),i.font=`${10*n}px sans-serif`;const lt=Math.min(220*n,a-2*$),pt=[];q.forEach(ut=>{const At=String(ut.t).split(" ");let vt="";At.forEach(Lt=>{const jt=vt?vt+" "+Lt:Lt;i.measureText(jt).width>lt&&vt?(pt.push({...ut,t:vt}),vt=Lt):vt=jt}),pt.push({...ut,t:vt})});const bt=14*n,_t=Math.min(lt+14*n,a-8*n),yt=pt.length*bt+10*n;let wt=E.cx*n-_t/2;wt=Math.max(4*n,Math.min(a-_t-4*n,wt));const It=Et+nt+6*n;i.fillStyle=g,i.globalAlpha=.97,i.fillRect(wt,It,_t,yt),i.globalAlpha=1,i.strokeStyle=N.color,i.lineWidth=n,i.strokeRect(wt,It,_t,yt),i.textAlign="left",i.textBaseline="top",pt.forEach((ut,At)=>{i.font=`${ut.bold?"600 ":""}${10*n}px sans-serif`,i.fillStyle=ut.col||(ut.dim?_:(l.getPropertyValue("--primary-text-color")||"#ddd").trim()),i.fillText(ut.t,wt+7*n,It+5*n+At*bt)}),i.restore()}}if(this._pgHoverT!=null&&this._pgHoverT>=V&&this._pgHoverT<=H){const E=K(this._pgHoverT);i.save(),i.strokeStyle="#e34948",i.lineWidth=1.5*n,i.setLineDash([4*n,3*n]),i.beginPath(),i.moveTo(E,S),i.lineTo(E,o-v),i.stroke(),i.setLineDash([]);const N=this._pgInterpPower(s,this._pgHoverT),Z=X(N);i.fillStyle="#e34948",i.beginPath(),i.arc(E,Z,3.5*n,0,Math.PI*2),i.fill();const Y=dt=>`${Math.floor(dt/60)}:${String(Math.round(dt%60)).padStart(2,"0")}`,q=dt=>dt>=1e3?(dt/1e3).toFixed(2)+" kW":Math.round(dt)+" W",lt=[`${this._t("lbl.from_start",{},"From start")} ${Y(this._pgHoverT)}`,`${this._t("lbl.to_end",{},"To end")} ${Y(Math.max(0,M-this._pgHoverT))}`,`${this._t("lbl.power",{},"Power")} ${q(N)}`];i.font=`${9.5*n}px sans-serif`;const pt=Math.max(...lt.map(dt=>i.measureText(dt).width))+12*n,bt=lt.length*13*n+8*n;let _t=E+8*n;_t+pt>a-k&&(_t=E-pt-8*n);const yt=S+4*n;i.fillStyle=g,i.globalAlpha=.95,i.fillRect(_t,yt,pt,bt),i.globalAlpha=1,i.strokeStyle=p,i.lineWidth=n,i.strokeRect(_t,yt,pt,bt),i.fillStyle=_,i.textAlign="left",i.textBaseline="top",lt.forEach((dt,wt)=>i.fillText(dt,_t+6*n,yt+5*n+wt*13*n)),i.restore()}i.fillStyle=_,i.font=`${9*n}px sans-serif`,i.textAlign="center",i.textBaseline="top";const te=E=>`${Math.floor(E/60)}:${String(Math.round(E%60)).padStart(2,"0")}`;for(let E=0;E<=4;E++){const N=V+J*E/4,Z=K(N);i.textAlign=E===0?"left":E===4?"right":"center",i.fillText(te(N),Math.max($,Math.min(a-k,Z)),A+u+1*n)}J<M-1&&(i.fillStyle=_,i.font=`${8*n}px sans-serif`,i.textAlign="right",i.textBaseline="top",i.fillText(this._t("lbl.pg_zoom_hint",{},"double-click to reset zoom"),a-k,S))}_pgEventMeta(t){const s={detected:["\u25CF","#42a5f5",this._t("lbl.pg_ev_detected",{},"Detected")],match_commit:["\u25C6","#66bb6a",this._t("lbl.pg_ev_match",{},"Match committed")],match_changed:["\u25C7","#eda100",this._t("lbl.pg_ev_match_changed",{},"Match changed")],match_ambiguous:["\u25C8","#eda100",this._t("lbl.pg_ev_ambiguous",{},"Ambiguous")],unmatched:["\u25CB","#9e9e9e",this._t("lbl.pg_ev_unmatched",{},"Unmatched")],notify_start:["\u{1F514}","#2a78d6",this._t("lbl.pg_ev_notify_start",{},"Start notification")],notify_pre_complete:["\u{1F514}","#2a78d6",this._t("lbl.pg_ev_notify_pre",{},"Almost-done notification")],notify_finish:["\u{1F514}","#2a78d6",this._t("lbl.pg_ev_notify_finish",{},"Finish notification")],notify_milestone:["\u{1F3C6}","#2a78d6",this._t("lbl.pg_ev_notify_milestone",{},"Milestone notification")],notify_held:["\u{1F319}","#7e57c2",this._t("lbl.pg_ev_notify_held",{},"Notification held (quiet hours)")],finished:["\u2713","#4caf50",this._t("lbl.pg_ev_finished",{},"Finished")]}[t]||["\u2022","var(--secondary-text-color)",t];return{glyph:s[0],color:s[1],label:s[2]}}_pgEventDescription(t){return{detected:this._t("pg_evd.detected",{},"The detector recognized a cycle had started."),match_commit:this._t("pg_evd.match_commit",{},"The matcher committed to a program with enough confidence."),match_changed:this._t("pg_evd.match_changed",{},"The leading program changed as more of the cycle was seen."),match_ambiguous:this._t("pg_evd.match_ambiguous",{},"Two programs scored close together, so the match was uncertain."),unmatched:this._t("pg_evd.unmatched",{},"No saved program fit this cycle."),notify_start:this._t("pg_evd.notify_start",{},"A start notification would be sent."),notify_pre_complete:this._t("pg_evd.notify_pre_complete",{},"An almost-done notification would be sent."),notify_finish:this._t("pg_evd.notify_finish",{},"A finish notification would be sent."),notify_milestone:this._t("pg_evd.notify_milestone",{},"A milestone notification would be sent."),notify_held:this._t("pg_evd.notify_held",{},"A notification was held back for quiet hours."),finished:this._t("pg_evd.finished",{},"The cycle reached a terminal state and ended.")}[t]||""}_pgUpdateParamInput(t,e){const s=this.shadowRoot,r=s&&s.querySelector(`input[data-pgkey="${t}"]`);r&&(r.dataset.pgtype==="bool"?r.checked=!!e:r.value=typeof e=="number"?Math.round(e):e)}_pgUpdateStripAt(t){const e=this._pgPowerPts;if(!e?.length)return;const r=(this._cycles||[]).find(M=>M.id===this._pgCycleId)?._pg_duration||e[e.length-1].t||1,n=t??r,a=this._pgInterpPower(e,n),o=this._pgSeriesAt(n),i=o?this._pgMapState(o.state):"idle",l={idle:[this._t("lbl.pg_idle",{},"Idle"),"var(--secondary-background-color)"],detecting:[this._t("lbl.pg_detecting",{},"Detecting"),"#42a5f5"],running:[this._t("lbl.pg_ev_running",{},"Running"),"#66bb6a"],ending:[this._t("lbl.pg_ev_ending",{},"Ending"),"#ef5350"],anti_wrinkle:[this._t("lbl.pg_anti_wrinkle",{},"Anti-wrinkle"),"#ab47bc"]},[c,p]=l[i]||l.idle,_=o&&o.progress!=null?Math.round(o.progress):null,g=o&&o.remaining_s!=null?o.remaining_s:null,u=o&&o.energy_wh!=null?o.energy_wh:this._pgTrapEnergy(e,n),v=o&&o.confidence!=null?Math.round(o.confidence*100)+"%":"\u2014",b=o&&o.phase?o.phase:"\u2014",$=M=>Math.floor(M/60)+":"+String(Math.round(M%60)).padStart(2,"0"),k=M=>M>=1e3?(M/1e3).toFixed(2)+" kWh":M.toFixed(0)+" Wh",S=M=>M>=1e3?(M/1e3).toFixed(1)+" kW":Math.round(M)+" W",P=this.shadowRoot,L=M=>P&&P.getElementById(M),R=(M,T)=>{const C=L(M);C&&(C.textContent=T)},W=(M,T,C)=>{const B=L(M);B&&(B.style[T]=C)},O=L("wd-pg-state-badge");O&&(O.textContent=c,O.style.background=p,O.style.color=p.includes("var")?"":"#fff"),R("wd-pg-power",S(a)),R("wd-pg-pct",_!=null?_+"%":"\u2014%"),W("wd-pg-pbar","width",(_??0)+"%"),R("wd-pg-rem",g!=null?$(g):"\u2014"),R("wd-pg-energy",k(u)),R("wd-pg-conf",v),R("wd-pg-phase",b)}_pgIsUnknownCmd(t){const e=t&&(t.code||t.error&&t.error.code)||"",s=(t&&(t.message||t.error)||"").toString().toLowerCase();return e==="unknown_command"||s.includes("unknown command")||s.includes("unknown_command")}_pgInterpPower(t,e){if(!t.length)return 0;if(e<=t[0].t)return t[0].w;for(let s=1;s<t.length;s++)if(e<=t[s].t){const r=t[s].t-t[s-1].t;if(r<=0)return t[s].w;const n=(e-t[s-1].t)/r;return t[s-1].w+n*(t[s].w-t[s-1].w)}return t[t.length-1].w}_pgTrapEnergy(t,e){let s=0;for(let r=1;r<t.length;r++){const n=t[r-1].t,a=t[r-1].w,o=Math.min(t[r].t,e),i=this._pgInterpPower(t,o);if(!(o<=n)&&(s+=(a+i)/2*(o-n)/3600,o>=e))break}return s}_drawPlaygroundCanvases(){this._tab==="playground"&&this._pgDrawCanvas()}_htmlPhases(){const t=this._devices[this._selIdx],e=t&&t.options.device_type||"washing_machine",s=this._canEdit(),r=this._phases.map(o=>{const i=o.is_default,l=o.translation_key?this._t(o.translation_key,{},o.description||""):o.description||"",c=s?`
        <td>
    ...
    ${r||`<div class="wd-card"><p class="wd-info">${this._t("msg.no_other_users",{},"No other Home Assistant users found.")}</p></div>`}`}_htmlStore(){if(!this._canEdit())return`<div class="wd-empty">${this._t("msg.no_device_selected",{},"No device selected.")}</div>`;const t=this._storeStatus,e=`<a href="https://3dg1luk43.github.io/washdata-store" target="_blank" rel="noopener noreferrer" style="font-size:.8em;font-weight:400;color:var(--primary-color);text-decoration:none;white-space:nowrap" title="${d(this._t("store.website_tip",{},"Open the community store website"))}">${this._t("store.website",{},"Store website \u2197")}</a>`,s=`<div style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap"><div class="wd-card-title" style="margin:0">${this._t("hdr.community_store",{},"Community Store")}</div>${e}</div>`;if(!this._onlineEnabled()||t&&t.enabled===!1)return`<div class="wd-card">${s}
      ...
      ${Ot(`data-action="store-toggle-dl-settings" ${this._dlSettings?"checked":""} ${n?"disabled":""}`,this._t("lbl.adopt_settings",{},"Also adopt settings"),at(this._t("msg.adopt_settings_hint",{},"Overwrite this device's detection & matching thresholds with the shared ones. Your notifications, entities and energy price are never changed.")))}
      ...
      ${t?n:""}
    </div>`}_htmlStorePrefs(t){const e=this._constants&&this._constants.storePrefs||{},s=ie.map(n=>{const a=e[n.key]!==!1;return Tt(`data-action="store-toggle-pref" data-pref="${d(n.key)}" ${a?"checked":""} ${t?"disabled":""}`,this._t(n.labelKey,{},n.labelFb),at(this._t(n.docKey,{},n.docFb)))}).join(""),r=this._busy.has("store-refresh-catalog");return s+`
      ...
      <div class="wd-log-drawer-body">
        ...
        <div class="wd-modal-actions"><button class="wd-btn wd-btn-secondary" data-maction="cancel">${this._t("btn.cancel",{},"Cancel")}</button>
        <button class="wd-btn wd-btn-primary" data-maction="label-ok">${this._t("btn.apply_label",{},"Apply Label")}</button></div>`;else if(t.type==="create-profile"){const s=(this._cycles||[]).slice(0,40).map(r=>`<option value="${d(r.id)}">${it(r.start_time)} - ${Math.round((r.duration||0)/60)}m - ${d(r.profile_name||this._t("lbl.unlabelled",{},"Unlabelled"))}</option>`).join("");e=`<h2>${this._t("modal.create_profile",{},"Create Profile")}</h2>
        ...
        <button class="wd-btn wd-btn-primary" data-maction="store-share-ok" ${this._busy.has("store-share")?"disabled":""}>${this._busy.has("store-share")?'<span class="wd-spin"></span> ':""}${this._t("btn.share",{},"Share")}</button></div>`}else t.type==="store-share-device"&&(e=this._htmlShareDeviceModal(t));return`<div class="wd-overlay"><div class="wd-modal" role="dialog" aria-modal="true" aria-labelledby="wd-modal-title" tabindex="-1">${e}</div></div>`}_htmlShareDeviceModal(t){const e=this._shareableByProgram(),s=this._busy.has("store-share-device"),r=t.selected||new Set,n=!!t.consented,a=e.reduce((u,v)=>u+v.cycles.filter(b=>r.has(b.id)).length,0),o=e.some(u=>!u.noCycles);let i;t.loading?i=`<div class="wd-empty" style="padding:24px"><div class="wd-icon">\u23F3</div>${this._t("msg.loading",{},"Loading\u2026")}</div>`:e.length?i=e.map(u=>{if(u.noCycles)return`<div class="wd-sd-group wd-sd-group-nocyc">
          ...
          </div>`;const v=u.cycles.every(P=>r.has(P.id)),b=!v&&u.cycles.some(P=>r.has(P.id)),$=u.cycles.map(P=>{const L=P.start_time?it(P.start_time):"",R=P.duration!=null?ot(P.duration):"";return`<label class="wd-sd-cyc">
            <input type="checkbox" data-maction="sd-toggle-cyc" data-cid="${d(P.id)}" ${r.has(P.id)?"checked":""} ${s?"disabled":""}>
            <span class="wd-sd-cyc-meta">${d(L)} \xB7 ${d(R)} \u2B50</span>
          ...
          <label class="wd-sd-prof">
            ...
            <span class="wd-sd-prof-name">${d(u.program)}</span>
            <span class="wd-sd-count">${u.cycles.filter(P=>r.has(P.id)).length}/${u.cycles.length}</span>
          </label>
        ...
        ${at(this._t("msg.include_settings_hint",{},"Share this device's recognition and matching thresholds (not your notifications, entities or energy price). Adopters choose whether to apply them."))}
      ...
      <span class="wd-sd-prof-name"><strong>${this._t("btn.select_all",{},"Select all")}</strong></span>
    </label>`,c=a.map(p=>{const _=r[p],g=s.importableOnly&&_.importable===!1,u=this._wizCatState(t,p,e),b=["profiles","real_cycles","reference_cycles"].includes(p)?`${u.sel}/${_.count}`:"",$=`<label class="wd-sd-prof">
        <input type="checkbox" data-maction="wiz-toggle-cat" data-cat="${p}" ${u.state==="all"?"checked":""} ${u.state==="some"?'data-indeterminate="1"':""} ${g?"disabled":""}>
        <span class="wd-sd-prof-name">${d(this._wizCatLabel(p))}${g?` <span class="wd-tag">${this._t("badge.not_importable",{},"n/a here")}</span>`:""}</span>
        <span class="wd-sd-count">${b}</span>
      </label>`;let k="";if(!g&&p==="profiles")k=`<div class="wd-sd-cycles">${(_.items||[]).map(S=>`<label class="wd-sd-cyc">
          ...
          <span class="wd-sd-cyc-meta">${d(S.name)} \xB7 ${(S.real_cycles||0)+(S.reference_cycles||0)} ${this._t("lbl.cycles_short",{},"cycles")}${n.has(S.name)?` <span class="wd-tag" style="color:var(--warning-color,#e6a700)">${this._t("badge.exists",{},"exists")}</span>`:""}</span>
        </label>`).join("")}</div>`;else if(!g&&(p==="real_cycles"||p==="reference_cycles")){const S=p==="real_cycles"?t.sel.realIds:t.sel.refIds;k=`<div class="wd-sd-cycles">${(_.groups||[]).map(P=>{const L=P.cycles.map(B=>String(B.id)),R=L.filter(B=>S.has(B)).length,W=R===L.length&&L.length>0,O=R>0&&!W,M=`${p}:${P.profile}`,T=t.expanded&&t.expanded.has(M),C=T?P.cycles.map(B=>`<label class="wd-sd-cyc" style="margin-left:18px">
            <input type="checkbox" data-maction="wiz-toggle-cyc" data-cat="${p}" data-cid="${d(String(B.id))}" ${S.has(String(B.id))?"checked":""}>
            <span class="wd-sd-cyc-meta">${d(B.date?it(B.date):String(B.id))}${B.duration!=null?" \xB7 "+ot(B.duration):""}</span>
            ...
            <div class="wd-sd-cyc" style="display:flex;align-items:center">
              <label style="flex:1;display:flex;align-items:center;gap:8px;cursor:pointer">
                <input type="checkbox" data-maction="wiz-toggle-cycgroup" data-cat="${p}" data-prof="${d(P.profile)}" ${W?"checked":""} ${O?'data-indeterminate="1"':""}>
                <span class="wd-sd-cyc-meta"><strong>${d(P.profile||this._t("lbl.unlabelled",{},"Unlabelled"))}</strong> (${R}/${P.count})</span>
              </label>
              <button type="button" class="wd-linkbtn" data-maction="wiz-expand" data-key="${d(M)}" style="margin-left:auto;background:none;border:none;color:var(--primary-color);cursor:pointer">${T?"\u25BE":"\u25B8"}</button>
            </div>
            ${C}
      ...
      <div class="wd-modal-actions">
        ...
        <div class="wd-modal-actions"><button class="wd-btn wd-btn-secondary" data-maction="cancel">${this._t("btn.close",{},"Close")}</button></div>`;const e=t.curve||{},s=!!e.is_reference,r=e.labelable!==!1,n=e.editable!==!1,a=e.cycle_origin==="backfill",o=e.full_duration_s||e.duration||0,i=e.energy_kwh!=null?e.energy_kwh:null,l=t.ml||null;let c="";if(l&&l.ml_quality_score!=null){const T=l.ml_quality_label,C=T==="ok"?"var(--success-color,#4caf50)":T==="uncertain"?"var(--warning-color,#ff9800)":"var(--error-color,#f44336)",B=Math.round((1-l.ml_quality_score)*100);c=`<div class="wd-kv-item"><div class="wd-kv-val" style="font-size:.95em;color:${C}">${B}%</div><div class="wd-kv-lbl">${this._t("lbl.cycle_health",{},"Cycle health")}</div></div>`}const p=`<div class="wd-kv">
      ...
      <button class="wd-btn wd-btn-sm ${t.mode==="review"?"wd-btn-primary":"wd-btn-secondary"}" data-maction="cyc-review" title="${v?this._t("hdr.automation_needs_review",{},"This cycle needs review"):this._t("hdr.automation_review_this_cycle",{},"Review this cycle")}">${this._t("btn.review",{},"Review")}${b}</button>
    </div>`:s?`<div class="wd-info" style="margin:0 0 8px"><span style="color:var(--info-color,#2196f3)">\u{1F4E5}</span> ${a?this._t("msg.imported_history_readonly",{},"Detected in imported power history. It shapes program matching but is not counted in your statistics, and cannot be trimmed or split. Label it to name the program."):this._t("msg.imported_readonly",{},"Imported from the community store. Shown for reference and matching. It is not counted in your stats and cannot be edited.")}</div>`:"",k=this._canEdit()?(this._feedbacks||[]).find(T=>T.cycle_id===t.cycleId):null,S=k?k.detected_profile||k.profile_name||this._t("lbl.unknown",{},"Unknown"):"",P=k?`
        ...
        </div>`:"";let L="";if(t.mode==="view"){const T=!!(l&&l.ml_review&&l.ml_review.golden),B=this._canEdit()&&this._onlineEnabled()&&this._storeConnected&&T?`<button class="wd-btn wd-btn-secondary" data-action="store-share-cycle" data-cid="${d(t.cycleId)}" data-prof="${d(e.profile_name||"")}">${this._t("btn.share_to_store",{},"Share to store")}</button>`:"",V=this._canEdit()?`<button class="wd-btn wd-btn-danger" data-maction="cyc-delete">${this._t("btn.delete",{},"Delete")}</button>${r?`
        <button class="wd-btn wd-btn-primary" data-maction="cyc-label">${this._t("btn.label",{},"Label")}</button>`:""}`:"";L=`${P}<div class="wd-modal-actions">
        <button class="wd-btn wd-btn-secondary" data-maction="cancel">${this._t("btn.close",{},"Close")}</button>
        ${B}
        ${V}</div>`}else if(t.mode==="trim"){const T=this._busy.has("cyc-trim-apply"),C=t.timeMode||"s",B=C==="clock"?this._offsetToClock(t.trim.start):Math.round(t.trim.start),V=C==="clock"?this._offsetToClock(t.trim.end):Math.round(t.trim.end),H=C==="clock"?"time":"number",J=C==="clock"?'step="1"':`min="0" max="${Math.ceil(o)}" step="1"`,st=C==="clock"?"":" "+this._t("lbl.unit_s",{},"(s)");L=`<p class="wd-info" style="margin:4px 0 8px">${this._t("msg.trim_intro",{},"Drag the red handles, or enter values. Everything outside the window is removed.")}</p>
        <div class="wd-mode-bar" style="margin-bottom:8px;align-items:center">
          ...
          <button class="wd-btn wd-btn-sm ${C==="s"?"wd-btn-primary":"wd-btn-secondary"}" data-maction="trim-mode-s">${this._t("lbl.seconds_from_start",{},"Seconds from start")}</button>
          <button class="wd-btn wd-btn-sm ${C==="clock"?"wd-btn-primary":"wd-btn-secondary"}" data-maction="trim-mode-clock">${this._t("lbl.clock_time",{},"Clock time")}</button>
        </div>
        <div class="wd-form-grid">
          <div class="wd-field"><label>${this._t("lbl.start",{},"Start")}${st}</label><input type="${H}" id="wd-trim-start" ${J} value="${B}"></div>
          <div class="wd-field"><label>${this._t("lbl.end",{},"End")}${st}</label><input type="${H}" id="wd-trim-end" ${J} value="${V}"></div>
        </div>
        <div class="wd-modal-actions">
          <button class="wd-btn wd-btn-secondary" data-maction="cancel">${this._t("btn.close",{},"Close")}</button>
          <button class="wd-btn wd-btn-secondary" data-maction="cyc-reset-trim">${this._t("btn.reset",{},"Reset")}</button>
          <button class="wd-btn wd-btn-primary" data-maction="cyc-apply-trim" ${T?"disabled":""}>${T?'<span class="wd-spin"></span> '+this._t("status.trimming",{},"Trimming\u2026"):this._t("btn.apply_trim",{},"Apply Trim")}</button>
        </div>`}else if(t.mode==="split"){const T=this._busy.has("cyc-split-apply"),C=(t.split.offsets||[]).slice().sort((H,J)=>H-J),B=[0,...C,o],V=B.slice(0,-1).map((H,J)=>{const st=B[J+1];return`<div class="wd-seg-row"><span class="wd-swatch" style="background:${tt[J%tt.length]}"></span>
          <span style="min-width:120px">${ot(H)} \u2013 ${ot(st)}</span>
          <select data-segidx="${J}"><option value="">${this._t("lbl.unlabelled_paren",{},"(unlabelled)")}</option>${this._profileOptions(t.split.profiles[J])}</select></div>`}).join("");L=`<p class="wd-info" style="margin:4px 0 8px">${this._t("msg.split_intro",{},"Click the graph to add or remove a split point, or auto-detect by idle gaps. Each resulting segment can get its own profile.")}</p>
        <div class="wd-mode-bar">
        ...
        <div style="margin:10px 0">${C.length?V:`<p class="wd-info">${this._t("msg.no_split_points",{},"No split points yet.")}</p>`}</div>
        <div class="wd-modal-actions">
          <button class="wd-btn wd-btn-secondary" data-maction="cancel">${this._t("btn.close",{},"Close")}</button>
          <button class="wd-btn wd-btn-primary" data-maction="cyc-apply-split" ${T||!C.length?"disabled":""}>${T?'<span class="wd-spin"></span> '+this._t("status.splitting",{},"Splitting\u2026"):this._t("btn.apply_split",{},"Apply Split")}</button>
        </div>`}else if(t.mode==="review"){const T=l&&l.ml_review||{},C=this._busy.has("cyc-review-save"),B=(et,f)=>`<option value="${et}" ${(T.quality||"")===et?"selected":""}>${f}</option>`,H=[["late_start",this._t("tag.late_start",{},"Late start")],["early_end",this._t("tag.early_end",{},"Early end")],["merged",this._t("tag.merged",{},"Merged cycles")],["split",this._t("tag.split",{},"Split cycle")],["noise",this._t("tag.noise",{},"Noise")],["wrong_profile",this._t("tag.wrong_profile",{},"Wrong profile")],["sensor_gap",this._t("tag.sensor_gap",{},"Sensor gap")]].map(([et,f])=>`<label class="wd-rev-tag"><input type="checkbox" class="wd-cyc-rev-tag" value="${et}" ${(T.tags||[]).includes(et)?"checked":""}> ${f}</label>`).join(""),J=T.reviewed_at?`<span style="font-size:.75em;color:var(--secondary-text-color)">${this._t("lbl.reviewed_on",{date:new Date(T.reviewed_at).toLocaleDateString()},`reviewed ${new Date(T.reviewed_at).toLocaleDateString()}`)}</span>`:"",st=at(this._t("msg.review_profile_tip",{},"The program this cycle is labelled as. If the auto-detected program was wrong, correct it here - labelling teaches matching for future cycles.")),K=at(this._t("msg.review_quality_tip",{},"How clean this cycle is. Good = a textbook example of this program; Bad = detected but noisy or atypical; Unusable = mis-detected (merged, truncated or spurious). Drives the health score and which cycles are allowed to train the model.")),X=at(this._t("msg.review_recorded_tip",{},'Mark this as a hand-picked reference cycle for its program - the same role as a manually recorded cycle. Reference cycles are always kept, seed the matching template, and are never dropped by cleanup. (This is the "golden"/recorded flag; both are the same thing.)')),ct=at(this._t("msg.review_tags_tip",{},"Optional flags describing what went wrong with this cycle, so training and cleanup can account for it.")),U=at(this._t("msg.review_notes_tip",{},"Free-text notes for your own reference. Not used by matching or training."));L=`
        ${P}
        ...
        <div style="display:flex;gap:16px;flex-wrap:wrap;align-items:center;margin:6px 0">
          <label style="display:inline-flex;align-items:center;gap:6px">${this._t("lbl.profile",{},"Profile")}${st}
          ...
          <label style="display:inline-flex;align-items:center;gap:6px">${this._t("lbl.quality",{},"Quality")}${K}
            <select id="wd-cyc-rev-quality" class="wd-filter-select">${B("","-")}${B("good",this._t("quality.good",{},"Good"))}${B("bad",this._t("quality.bad",{},"Bad"))}${B("unusable",this._t("quality.unusable",{},"Unusable"))}</select>
          </label>
          <label style="display:inline-flex;align-items:center;gap:6px"><input type="checkbox" id="wd-cyc-rev-golden" ${T.golden?"checked":""}> ${this._t("badge.golden_cycle",{},"Recorded reference cycle")}${X}</label>
          ${J}
        </div>
        <div class="wd-rev-sub">${this._t("lbl.compare_profiles",{},"Compare with profiles")}${at(this._t("msg.compare_profiles_tip",{},"Overlay other profile envelopes on the chart above to see which one best fits this cycle."))}</div>
        <div class="wd-rev-tags">${(this._profiles||[]).map(et=>{const f=(t.overlays||[]).includes(et.name),m=f?`<span style="display:inline-block;width:10px;height:10px;border-radius:2px;background:${tt[Math.max(0,this._profiles.findIndex(x=>x.name===et.name))%tt.length]};margin:0 2px"></span>`:"";return`<label class="wd-rev-tag"><input type="checkbox" class="wd-cyc-overlay" value="${d(et.name)}" ${f?"checked":""}> ${m}${d(et.name)}</label>`}).join("")||`<span class="wd-info">${this._t("msg.no_profiles_compare",{},"No profiles to compare.")}</span>`}</div>
        <div class="wd-rev-sub">${this._t("lbl.tags",{},"Tags")}${ct}</div>
        <div class="wd-rev-tags">${H}</div>
        <div class="wd-rev-sub">${this._t("lbl.notes",{},"Notes")}${U}</div>
        <textarea id="wd-cyc-rev-notes" class="wd-rev-notes" rows="3" placeholder="${this._t("msg.review_notes_placeholder",{},"Notes (optional)")}">${d(T.notes||"")}</textarea>
        <div class="wd-modal-actions" style="margin-top:16px">
          <button class="wd-btn wd-btn-secondary" data-maction="cancel">${this._t("btn.close",{},"Close")}</button>
          <button class="wd-btn wd-btn-primary" data-maction="cyc-review-save" ${C?"disabled":""}>${C?'<span class="wd-spin"></span> '+this._t("status.saving",{},"Saving\u2026"):this._t("btn.save_review",{},"Save Review")}</button>
        </div>`}let R="";const W=t.mode==="view"||t.mode==="review"?e.artifacts||[]:[];if(W.length){const T=W.map(C=>{const B=C.detail_key?this._t(C.detail_key,C.detail_params||{},C.detail||""):C.detail||"";return`<li><b>${d(Yt(C.type,(V,H,J)=>this._t(V,H,J)))}</b> ${this._t("lbl.at",{},"at")} ${Mt(C.start_s)}\u2013${Mt(C.end_s)} \u2014 ${d(B)}</li>`}).join("");R=`<div class="wd-card" style="margin:10px 0 0;padding:10px 12px;border-left:3px solid var(--warning-color,#ff9800)">
        ...
        <ul class="wd-info" style="margin:4px 0 0;padding-left:18px;font-size:.82em">${T}</ul>
      ...
      </div>`}let O="";const M=t.mode==="view"||t.mode==="review"?e.restart_gaps||[]:[];if(M.length){const T=M.map(C=>{const B=Math.round((C.gap_seconds||0)/60),V=B>=1?`${B}m`:`${Math.round(C.gap_seconds||0)}s`,H=C.match_confidence!=null?` \xB7 ${this._t("lbl.pct_match_confidence",{pct:Math.round(C.match_confidence*100)},`${Math.round(C.match_confidence*100)}% match confidence`)}`:"",J=C.profile?` (${d(C.profile)})`:"";return`<li>${this._t("msg.restart_gap_item",{dur:V},`${V} gap`)}: ${this._t("lbl.ha_restarted",{},"HA restarted")}${J}${H}</li>`}).join("");O=`<div class="wd-card" style="margin:10px 0 0;padding:10px 12px;border-left:3px solid var(--info-color,#2196f3)">
        <div style="font-weight:600;font-size:.9em">\u21BB ${this._t("msg.restart_gap_header",{n:M.length},`${M.length} HA restart gap${M.length>1?"s":""} during this cycle`)}</div>
        <ul class="wd-info" style="margin:4px 0 0;padding-left:18px;font-size:.82em">${T}</ul>
      ...
      </div>`}return`<h2>${this._t("lbl.cycle",{},"Cycle")} \xB7 ${d(it(e.start_time))}</h2>
      ${p}${$}
      ...
      ${R}
      ${O}
      ${L}`}_htmlProfilePanel(t){const e=this._canEdit();t.tab==="danger"&&!e&&(t.tab="stats");const s=[["stats",this._t("tab.pp_overview",{},"Overview")],["phases",this._t("tab.pp_phases",{},"Phases")],["cleanup",this._t("tab.pp_cleanup",{},"Cleanup")]];e&&s.push(["danger",this._t("tab.pp_manage",{},"Manage")]);const r=s.map(([o,i])=>`<button class="wd-mini-tab ${t.tab===o?"active":""}" data-maction="pp-tab-${o}">${i}</button>`).join("");let n="";if(!t.loaded)n=`<div class="wd-empty" style="padding:32px"><div class="wd-icon">\u23F3</div>${this._t("msg.loading",{},"Loading\u2026")}</div>`;else if(t.tab==="stats"){const o=t.stats||{},i=t.env||{},l=this._hass&&this._hass.config&&this._hass.config.currency||"",c=o.avg_energy!=null&&o.cycle_count?o.avg_energy*o.cycle_count:null,p=b=>b?Math.round(b/60)+"m":"-",_=(this._profileHealth||{})[t.name],g=(this._profileTrends||{})[t.name],u=_&&_.health_status!=="unknown"?(()=>{const b={healthy:["var(--success-color,#4caf50)","rgba(76,175,80,.12)"],fair:["var(--warning-color,#ff9800)","rgba(255,152,0,.12)"],poor:["var(--error-color,#f44336)","rgba(244,67,54,.12)"]},[$,k]=b[_.health_status]||b.fair,S=Math.round((_.health_score||0)*100),P=_.duration_cv!=null?` \xB7 ${this._t("stat.duration_cv",{pct:Math.round(_.duration_cv*100)},`duration CV ${Math.round(_.duration_cv*100)}%`)}`:"",L=_.confidence_mean!=null?` \xB7 ${this._t("stat.avg_confidence",{pct:Math.round(_.confidence_mean*100)},`avg confidence ${Math.round(_.confidence_mean*100)}%`)}`:"";return`<div style="margin:8px 0 4px;padding:8px 12px;border-radius:6px;background:${k};border:1px solid color-mix(in srgb, ${$} 13%, transparent);display:flex;align-items:center;gap:8px;flex-wrap:wrap">
          <span style="font-weight:600;color:${$}">${_.health_status==="poor"?this._t("health.poor",{},"\u26A0 Poor match fit"):_.health_status==="fair"?this._t("health.fair",{},"Fair match fit"):this._t("health.good",{},"\u2713 Good match fit")}</span>
          <span style="font-size:.85em;opacity:.8">${this._t("stat.score",{pct:S},`score ${S}%`)}${P}${L}</span>
          ...
          <div class="wd-sg">
            ...
            <div class="wd-sg-sub">${this._t("stat.last_run",{v:o.last_run?it(o.last_run):"-"},`last run ${o.last_run?it(o.last_run):"-"}`)}</div>
          </div>
        </div>
        ${u}
        ${v}
        ${_&&_.shape_drift?(()=>{const b=_.shape_drift_correlation!=null?` (r=${Number(_.shape_drift_correlation).toFixed(2)})`:"";return`<div style="margin-top:8px;padding:8px 10px;background:color-mix(in srgb, var(--warning-color,#ff9800) 9%, transparent);border-radius:6px;border-left:3px solid var(--warning-color,#ff9800)">
        ...
        ${i.avg&&i.avg.length?`<div class="wd-canvas-wrap"><canvas id="wd-env-canvas" role="img" aria-label="${d(this._t("lbl.aria_envelope_chart",{},"Profile power envelope chart"))}"></canvas></div>`:`<p class="wd-info">${this._t("msg.no_envelope",{},"No envelope yet - rebuild after labelling cycles.")}</p>`}`}else if(t.tab==="phases"){const o=t.catalog||[],i=(t.phases||[]).map((c,p)=>{const _=o.map(g=>`<option value="${d(g)}" ${c.name===g?"selected":""}>${d(g)}</option>`).join("");return`<div class="wd-phase-row"><span class="wd-swatch" style="background:${tt[p%tt.length]}"></span>
        ...
        ${e?`<div class="wd-mode-bar">
          ...
          <td style="width:10px;padding:6px 2px"><span class="wd-swatch" style="background:${tt[k%tt.length]}"></span></td>
          <td class="wd-tc-date">${it(b.start_time)}</td>
