...
async def async_setup_entry(
    ...
    async_add_entities([
        ...
        WashDataMarkUnloadedButton(manager, entry),
    ])
...
class WashDataMarkUnloadedButton(ButtonEntity):
    """Button to confirm the load has been taken out (#451).

    The automation-facing half of unload confirmation: a physical button, an NFC
    tag or any other automation can press this instead of the door sensor WashData
    would otherwise need. Available only while the machine is in the Clean state
    (cycle finished, load still inside) - there is nothing to confirm otherwise,
    ...
    _attr_icon = "mdi:basket-check-outline"

    def __init__(self, manager: WashDataManager, entry: ConfigEntry) -> None:
        """Initialize the button."""
        self._manager = manager
        self._entry = entry
        self._attr_unique_id = f"{entry.entry_id}_mark_unloaded"
        self._attr_device_info = {
            "identifiers": {(DOMAIN, entry.entry_id)},
            "name": entry.title,
            "manufacturer": "WashData",
    ...
    @property
    def available(self) -> bool:
        """Only available while the finished load is still waiting to be taken out."""
        return self._manager.is_clean_state

    async def async_press(self) -> None:
        """Handle the button press."""
        self._manager.mark_unloaded("mark unloaded button")
