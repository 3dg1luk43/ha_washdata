...
Open:
- Two design decisions are open for the maintainer, both raised in the PR #420 review and recorded in the register: **item 195** (replace-mode imports have no recoverable snapshot) and **item 196** (`profile_terminal_high_block`'s `start_frac` uses the quiet-trimmed span while the detector divides it into `avg_duration`, which measurably tracks the *untrimmed* span - both candidate fixes break the other's guarantee, so it needs its own A/B). The six items previously listed have all since been fixed: the options-schema dict leak (per-device defaults), the dead `cycle_detector._abrupt_drop` branch + vestigial `abrupt_drop_watts`/`abrupt_drop_ratio` (register item 27), the always-0.0 `features.CycleSignature.event_density` column (item 28), the `auto_label_cycles` schema/handler default mismatch now both 0.75 (item 30), the dead `WashDataStore.get_storage_stats()`/`async_clear_debug_data()` (item 39), and the dropped `reconcile_suggestions` keys now present in `_SUGGESTION_KEYS` (item 40). The `[FIXED]` register in `docs/internal/INTEGRATION_REFERENCE.md §7` is the live tech-debt tracker.
